...
C4SoundInstance *StartSoundEffect(const char *szSndName, bool fLoop,
...
void SoundLevel(const char *szSndName, C4Object *pObj, int32_t iLevel) {
  ...
  if (!iLevel) {
    StopSoundEffect(szSndName, pObj);
    return;
  }
...
void GameMsgObject(const char *szText, C4Object *pTarget, int32_t iFCol) {
  Game.Messages.New(C4GM_Target, szText, pTarget, NO_OWNER, 0, 0,
                    (uint8_t)iFCol);
}

void GameMsgObjectPlayer(const char *szText, C4Object *pTarget, int32_t iPlayer,
                         int32_t iFCol) {
  Game.Messages.New(C4GM_TargetPlayer, szText, pTarget, iPlayer, 0, 0,
                    (uint8_t)iFCol);
}
...
void GameMsgPlayer(const char *szText, int32_t iPlayer, int32_t iFCol) {
  Game.Messages.New(C4GM_GlobalPlayer, szText, NULL, iPlayer, 0, 0,
                    (uint8_t)iFCol);
}
...
int32_t ValidPlr(int32_t plr) { return Game.Players.Valid(plr); }
