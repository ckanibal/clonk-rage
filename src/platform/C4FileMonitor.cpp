...
C4FileMonitor::C4FileMonitor(ChangeNotify pCallback)
    : pCallback(pCallback), fStarted(false) {
  fd = inotify_init();
...
void C4FileMonitor::AddDirectory(const char *file) {
  // Add IN_CLOSE_WRITE?
  int wd = inotify_add_watch(fd, file, IN_CREATE | IN_MODIFY | IN_MOVED_TO |
                                           IN_MOVE_SELF | IN_ONLYDIR);
  if (wd == -1) LogF("inotify_add_watch %s", strerror(errno));
...
bool C4FileMonitor::Execute(int iTimeout)  // some other thread
{
  ...
  if (read(fd, buf, sizeof(buf)) > 0) {
    ...
    C4InteractiveThread &Thread = Application.InteractiveThread;
    if (mask & IN_CREATE) Thread.PushEvent(Ev_FileChange, (void *)file);
    if (mask & IN_MODIFY) Thread.PushEvent(Ev_FileChange, (void *)file);
    if (mask & IN_MOVED_TO) Thread.PushEvent(Ev_FileChange, (void *)file);
    if (mask & IN_MOVE_SELF) Thread.PushEvent(Ev_FileChange, (void *)file);
...
void C4FileMonitor::OnThreadEvent(C4InteractiveEventType eEvent,
                                  void *pEventData)  // main thread
{
...
const DWORD C4FileMonitorNotifies = FILE_NOTIFY_CHANGE_FILE_NAME |
...
void C4FileMonitor::AddDirectory(const char *szDir) {
  ...
  if (!ReadDirectoryChangesW(hDir, pWatch->Buffer, sizeof(pWatch->Buffer),
                             FALSE, C4FileMonitorNotifies, NULL, &pWatch->ov,
                             NULL))
...
bool C4FileMonitor::Execute(int iTimeout) {
  // Check event
  if (WaitForSingleObject(hEvent, iTimeout) != WAIT_OBJECT_0) return true;
  // Check handles
    ...
    if (GetOverlappedResult(pWatch->hDir, &pWatch->ov, &dwBytes, FALSE)) {
      ...
      for (;;) {
        const _FILE_NOTIFY_INFORMATION *pNotify =
            reinterpret_cast<const _FILE_NOTIFY_INFORMATION *>(pPos);
        // Handle
        ...
        if (pPos >=
            pWatch->Buffer + Min<size_t>(sizeof(pWatch->Buffer), dwBytes))
          break;
        break;
      }
      // Restart directory change notification (flush queue)
      ReadDirectoryChangesW(pWatch->hDir, pWatch->Buffer,
                            sizeof(pWatch->Buffer), FALSE,
                            C4FileMonitorNotifies, NULL, &pWatch->ov, NULL);
      dwBytes = 0;
...
HANDLE C4FileMonitor::GetEvent() { return hEvent; }

void C4FileMonitor::HandleNotify(const char *szDir,
                                 const _FILE_NOTIFY_INFORMATION *pNotify) {
  // Get filename length
  UINT iCodePage = CP_ACP /* future: CP_UTF8 */;
  int iFileNameBytes =
      WideCharToMultiByte(iCodePage, 0, pNotify->FileName,
                          pNotify->FileNameLength / 2, NULL, 0, NULL, NULL);
  ...
  // Convert filename
  int iWritten = WideCharToMultiByte(iCodePage, 0, pNotify->FileName,
                                     pNotify->FileNameLength / 2, pFilename,
                                     iFileNameBytes, NULL, NULL);
  if (iWritten != iFileNameBytes) Path.Shrink(iFileNameBytes + 1);
  // Send notification
...
C4FileMonitor::~C4FileMonitor() {}
bool C4FileMonitor::Execute(int iTimeout) {
  return false; /* blarg... function must return a value */
}
void C4FileMonitor::StartMonitoring() {}
void C4FileMonitor::OnThreadEvent(C4InteractiveEventType eEvent,
                                  void *pEventData) {}
