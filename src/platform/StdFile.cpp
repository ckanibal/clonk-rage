...
char *GetFilename(char *szPath) {
  ...
  for (pPos = szPath; *pPos; pPos++)
    if (*pPos == DirectorySeparator || *pPos == '/') pFilename = pPos + 1;
  return pFilename;
}
...
const char *GetC4Filename(const char *szPath) {
  ...
  for (pPos = szPath; *pPos; pPos++) {
    if (*pPos == DirectorySeparator || *pPos == '/') {
      if (pPos >= szPath + 4 && SEqual2NoCase(pPos - 4, ".c4"))
        return pFilename;
      pFilename = pPos + 1;
    }
  }
...
char *GetExtension(char *szFilename) {
  ...
  while ((pos > 0) && (szFilename[pos - 1] != '.') &&
         (szFilename[pos - 1] != DirectorySeparator))
    --pos;
...
void RealPath(const char *szFilename, char *pFullFilename) {
  ...
  for (;;) {
    // Try to convert to full filename. Note this might fail if the given file
    // doesn't exist
    if (realpath(szFilename, pFullFilename)) break;
    // ... which is undesired behaviour here. Try to reduce the filename until
    // it works.
    if (!pSuffix) {
...
#ifdef _WIN32
  if (!SCharCount(DirectorySeparator, szFilename))
    _fullpath(szBuffer, szFilename, _MAX_PATH);
#else
  if (!SCharCount(DirectorySeparator, szFilename))
    RealPath(szFilename, szBuffer);
#endif
  // Truncate path
...
bool GetRelativePath(const char *strPath, const char *strRelativeTo,
                     char *strBuffer, int iBufferSize) {
  // Specified path is relative to base path
  // Copy relative section
  const char *szCpy;
  SCopy(szCpy = GetRelativePathS(strPath, strRelativeTo), strBuffer,
        iBufferSize);
...
// Specified path is relative to base path
...
#endif
  {
    ...
    return strPath + SLen(strRelativeTo) +
           ((strPath[SLen(strRelativeTo)] == DirectorySeparator) ? +1 : 0);
  }
...
bool TruncatePath(char *szPath) {
  ...
  if (iBSPos2 > iBSPos)
    fprintf(stderr, "Warning: TruncatePath with a \\ (%s)\n", szPath);
#endif
...
void AppendBackslash(char *szFilename) {
  ...
  if (i > 0)
    if ((szFilename[i - 1] == DirectorySeparator)) return;
...
void TruncateBackslash(char *szFilename) {
  int i = SLen(szFilename);
  if (i > 0)
    if ((szFilename[i - 1] == DirectorySeparator)) szFilename[i - 1] = 0;
}
...
void DefaultExtension(StdStrBuf *sFilename, const char *szExtension) {
  ...
  if (!(*GetExtension(sFilename->getData()))) {
    sFilename->AppendChar('.');
    sFilename->Append(szExtension);
  }
}
...
void EnforceExtension(StdStrBuf *sFilename, const char *szExtension) {
  ...
  if (ext[0]) {
    sFilename->ReplaceEnd(ext - sFilename->getData(), szExtension);
  } else {
    sFilename->AppendChar('.');
    sFilename->Append(szExtension);
  }
...
void MakeFilenameFromTitle(char *szTitle) {
  ...
  while (IsWhiteSpace(*--szFilename))
    if (szFilename == szTitle) {
      --szFilename;
      break;
    }
  // terminate
...
bool FileExists(const char *szFilename) { return (!access(szFilename, F_OK)); }
...
#endif
  // either unlink or remove could be used. Well, stick to ANSI C where
  // possible.
...
bool CopyFile(const char *szSource, const char *szTarget, bool FailIfExists) {
  ...
  fstat(fds, &info);
  int fdt =
      open(szTarget, O_WRONLY | O_CREAT | (FailIfExists ? O_EXCL : O_TRUNC),
           info.st_mode);
  if (!fdt) {
    close(fds);
    return false;
...
bool DirectoryExists(const char *szFilename) {
  ...
  if (szFilename && szFilename[0])
    if ((szFilename[SLen(szFilename) - 1] == '\\') ||
        (szFilename[SLen(szFilename) - 1] == '/')) {
...
bool CopyDirectory(const char *szSource, const char *szTarget,
  ...
  while ((ent = readdir(d))) {
    SCopy(szSource, itemsource);
    AppendBackslash(itemsource);
    SAppend(ent->d_name, itemsource);
    SCopy(szTarget, itemtarget);
    AppendBackslash(itemtarget);
    SAppend(ent->d_name, itemtarget);
    if (!CopyItem(itemsource, itemtarget, fResetAttributes)) status = false;
  }
  closedir(d);
#endif
...
#ifdef _WIN32
  if (fResetAttributes)
    if (!SetFileAttributes(szTarget, FILE_ATTRIBUTE_NORMAL)) return false;
#else
  if (fResetAttributes)
    if (chmod(szTarget, S_IRWXU)) return false;
#endif
  return true;
}
...
bool ItemIdentical(const char *szFilename1, const char *szFilename2) {
  ...
  RealPath(szFilename1, szFullFile1);
  RealPath(szFilename2, szFullFile2);
#ifdef _WIN32
...
DirectoryIterator::DirectoryIterator(const DirectoryIterator &) : fdthnd(0) {
  *filename = 0;
}
...
DirectoryIterator::DirectoryIterator(const DirectoryIterator &) : d(0) {
  filename[0] = 0;
}
...
int ForEachFile(const char *szDirName, bool (*fnCallback)(const char *)) {
  if (!szDirName || !fnCallback) return 0;
  ...
  if ((fdthnd = _findfirst((char *)szFilename, &fdt)) < 0) return 0;
  do {
...
#else
  if (fHasWildcard)
    fprintf(stderr, "Warning: ForEachFile with * (%s)", szDirName);
  DIR *d = opendir(szDirName);
  dirent *ent;
  while ((ent = readdir(d))) {
