...
C4MusicSystem::C4MusicSystem()
    : PlayMusicFile(NULL), Songs(NULL), SongCount(0), Volume(100) {}

C4MusicSystem::~C4MusicSystem() { Clear(); }
...
bool C4MusicSystem::InitializeMOD() {
  ...
  if (FSOUND_GetVersion() < FMOD_VERSION) {
    sprintf(
        OSTR,
        "FMod: You are using the wrong DLL version!  You should be using %.02f",
        FMOD_VERSION);
    Log(OSTR);
    return FALSE;
  }
...
bool C4MusicSystem::InitForScenario(C4Group &hGroup) {
  ...
  if (GrpContainsMusic(hGroup)) {
    ...
    // log
    LogF(LoadResStr("IDS_PRC_LOCALMUSIC"),
         SGetRelativePath(MusicDir.getData()));
  }
...
// get extension
  ...
  switch (GetMusicFileTypeByExtension(GetExtension(szFile))) {
    ...
    case MUSICTYPE_MID:
      if (MODInitialized) NewSong = new C4MusicFileMID;
      break;
  }
#endif
#ifdef HAVE_LIBSDL_MIXER
  if (GetMusicFileTypeByExtension(GetExtension(szFile)) == MUSICTYPE_UNKNOWN)
    return;
...
void C4MusicSystem::LoadMoreMusic() {
  ...
  // load MoreMusic.txt
  if (!MoreMusicFile.Load(Config.AtExePath(C4CFN_MoreMusic), &szMoreMusic, NULL,
                          1))
    return;
  // read contents
  ...
  while (pPos && *pPos) {
    ...
    if (*pLine == '#') {
      ...
      if (SEqual(pLine, "#clear")) ClearSongs();
      continue;
    }
    // try to load file(s)
    LoadDir(pLine);
...
bool C4MusicSystem::Play(const char *szSongname, bool fLoop) {
  ...
  if (szSongname && szSongname[0]) {
    ...
    for (NewFile = Songs; NewFile; NewFile = NewFile->pNext) {
      ...
      SCopy(szSongname, songname, _MAX_FNAME - 4);
      DefaultExtension(songname, "mid");
      if (SEqual(GetFilename(NewFile->FileName), songname)) break;
      SCopy(szSongname, songname, _MAX_FNAME - 4);
      DefaultExtension(songname, "ogg");
      if (SEqual(GetFilename(NewFile->FileName), songname)) break;
    }
  }
  ...
  // Random song
    ...
    for (int i = 0; i <= 1000; i++) {
      int nmb = SafeRandom(
          Max(ASongCount / 2 + ASongCount % 2, ASongCount - SCounter));
      int j;
      for (j = 0, NewFile = Songs; NewFile; NewFile = NewFile->pNext)
        if (!NewFile->NoPlay)
          if (NewFile->LastPlayed == -1 ||
              NewFile->LastPlayed < SCounter - ASongCount / 2) {
            j++;
            if (j > nmb) break;
          }
      if (NewFile) break;
...
int C4MusicSystem::SetVolume(int iLevel) {
  ...
  if (PlayMusicFile) PlayMusicFile->SetVolume(iLevel);
  return iLevel;
}

MusicType GetMusicFileTypeByExtension(const char *ext) {
  if (SEqualNoCase(ext, "mid")) return MUSICTYPE_MID;
#if defined USE_FMOD || defined HAVE_LIBSDL_MIXER
  else if (SEqualNoCase(ext, "xm") || SEqualNoCase(ext, "it") ||
           SEqualNoCase(ext, "s3m") || SEqualNoCase(ext, "mod"))
...
bool C4MusicSystem::GrpContainsMusic(C4Group &rGrp) {
  ...
  return rGrp.FindEntry("*.mid")
         ...
         || rGrp.FindEntry("*.mp3")
#endif
         || rGrp.FindEntry("*.xm") || rGrp.FindEntry("*.it") ||
         rGrp.FindEntry("*.s3m") || rGrp.FindEntry("*.mod") ||
         rGrp.FindEntry("*.ogg");
}
...
int C4MusicSystem::SetPlayList(const char *szPlayList) {
  ...
  if (*szPlayList) {
    ...
    for (int cnt = 0; SGetModule(szPlayList, cnt, szFileName, _MAX_FNAME);
         cnt++)
      for (pFile = Songs; pFile; pFile = pFile->pNext)
        if (pFile->NoPlay &&
            WildcardMatch(szFileName, GetFilename(pFile->FileName))) {
          ASongCount++;
          pFile->NoPlay = FALSE;
        }
  } else {
...
bool C4MusicSystem::ToggleOnOff() {
  ...
  // use different settings for game/menu (lobby also counts as "menu", so go by
  // Game.IsRunning-flag rather than startup)
  if (Game.IsRunning) {
    // game music
    ...
    if (!Config.Sound.RXMusic)
      ...
      Play();
    Game.GraphicsSystem.FlashMessageOnOff(LoadResStr("IDS_CTL_MUSIC"),
