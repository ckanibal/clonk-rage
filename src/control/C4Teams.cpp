...
C4Team::C4Team(const C4Team &rCopy)
    : piPlayers(new int32_t[rCopy.GetPlayerCount()]),
      ...
      iID(rCopy.GetID()),
      iPlrStartIndex(rCopy.iPlrStartIndex),
      dwClr(rCopy.dwClr),
      sIconSpec(rCopy.GetIconSpec()),
      iMaxPlayer(rCopy.iMaxPlayer) {
  // copy name
...
void C4Team::AddPlayer(C4PlayerInfo &rInfo, bool fAdjustPlayer) {
  ...
  if (iPlayerCount >= iPlayerCapacity) {
    int32_t *piNewPlayers =
        new int32_t[iPlayerCapacity = iPlayerCount + 4 & ~3];
    if (iPlayerCount)
      memcpy(piNewPlayers, piPlayers, iPlayerCount * sizeof(int32_t));
    delete[] piPlayers;
    piPlayers = piNewPlayers;
  }
  // store new player
...
int32_t C4Team::GetFirstUnjoinedPlayerID() const {
  ...
  while (i--)
    if (pInfo = Game.PlayerInfos.GetPlayerInfoByID(idPlr = *piPlr++))
      if (!pInfo->HasJoinIssued()) return idPlr;
  // none found
  return 0;
}
...
int32_t C4Team::GetFirstActivePlayerID() const {
  ...
  C4Player *pPlr;
  while (i--)
    if (pPlr = Game.Players.GetByInfoID((idPlr = *piPlr++))) return idPlr;
  // none found
  return 0;
}
...
void C4Team::CompileFunc(StdCompiler *pComp) {
  ...
  if (pComp->isCompiler()) {
    delete[] piPlayers;
    piPlayers = new int32_t[iPlayerCapacity = iPlayerCount];
    ZeroMem(piPlayers, sizeof(*piPlayers) * iPlayerCount);
  }
  pComp->Value(
      mkNamingAdapt(mkArrayAdapt(piPlayers, iPlayerCount, -1), "Players"));
    ...
    if (id = piPlayers[i])
      if (pInfo = Game.PlayerInfos.GetPlayerInfoByID(id))
        if (pInfo->GetTeam() == GetID())
          if (pInfo->IsUsingTeam()) fIsValid = true;
  ...
  while (pInfo = Game.PlayerInfos.GetNextPlayerInfoByID(id)) {
    ...
    if (pInfo->GetTeam() == GetID())
      if (pInfo->IsUsingTeam())
        if (!IsPlayerIDInTeam(id)) AddPlayer(*pInfo, false);
  }
}
...
void C4Team::RecheckColor(C4TeamList &rForList) {
  ...
  if (!dwClr) {
    ...
    DWORD defTeamColorRGB[defTeamColorCount] = {
        ...
        0x00FFFF, 0x784830};                     // cyan, dk brown
...
StdStrBuf C4Team::GetNameWithParticipants() const {
  ...
  if (GetPlayerCount()) {
    ...
    for (int32_t j = 0; j < GetPlayerCount(); ++j) {
      ...
      if (iPlr)
        if (pPlrInfo = Game.PlayerInfos.GetPlayerInfoByID(iPlr)) {
...
C4TeamList &C4TeamList::operator=(const C4TeamList &rCopy) {
  ...
  for (int i = 0; i < iTeamCount; i++) ppList[i] = new C4Team(*rCopy.ppList[i]);
...
bool C4TeamList::CanLocalChooseTeam() const {
  ...
  switch (eTeamDist) {
    case TEAMDIST_Free:
      return true;
    case TEAMDIST_Host:
      return Game.Control.isCtrlHost();
    case TEAMDIST_None:
...
void C4TeamList::ClearTeams() {
  ...
  if (iTeamCount) {
    while (iTeamCount--) delete *(ppTeam++);
    iTeamCount = 0;
  }
  iLastTeamID = 0;
}
...
C4Team *C4TeamList::GetTeamByID(int32_t iID) const {
  C4Team **ppCheck = ppList;
  int32_t iCnt = iTeamCount;
  for (; iCnt--; ++ppCheck)
    if ((*ppCheck)->GetID() == iID) return *ppCheck;
  return NULL;
}
...
C4Team *C4TeamList::GetTeamByName(const char *szName) const {
  ...
  int32_t iCnt = iTeamCount;
  for (; iCnt--; ++ppCheck)
    if (SEqual((*ppCheck)->GetName(), szName)) return *ppCheck;
  return NULL;
}
...
C4Team *C4TeamList::GetTeamByPlayerID(int32_t iID) const {
  ...
  int32_t iCnt = iTeamCount;
  for (; iCnt--; ++ppCheck)
    if ((*ppCheck)->IsPlayerIDInTeam(iID)) return *ppCheck;
  return NULL;
}
...
C4Team *C4TeamList::GetRandomSmallestTeam() const {
  ...
  for (; iCnt--; ++ppCheck) {
    ...
    if (!pLowestTeam ||
        pLowestTeam->GetPlayerCount() > (*ppCheck)->GetPlayerCount()) {
      pLowestTeam = *ppCheck;
      iLowestTeamCount = 1;
    } else if (pLowestTeam->GetPlayerCount() == (*ppCheck)->GetPlayerCount())
      if (!SafeRandom(++iLowestTeamCount)) pLowestTeam = *ppCheck;
  }
  return pLowestTeam;
}
...
bool C4TeamList::IsTeamVisible() const {
  ...
  if (eTeamDist == TEAMDIST_RandomInv)
    if (Game.Network.isLobbyActive()) return false;
  return true;
}
...
bool C4TeamList::RecheckPlayerInfoTeams(C4PlayerInfo &rNewJoin, bool fByHost) {
  ...
  // assert(Game.Control.isCtrlHost()); -- bla, control is not initialized at
  // this point.
  ...
  if (rNewJoin.GetTeam()) {
    ...
    // the player had a different team assigned: Check if changes are allowed at
    // all
    if (eTeamDist == TEAMDIST_Free || (eTeamDist == TEAMDIST_Host && fByHost))
      // also make sure that selecting this team is allowed, e.g. doesn't break
      // the team limit
      // this also checks whether the team number is a valid team - but it would
      // accept TEAMID_New, which shouldn't be used in player infos!
      if (rNewJoin.GetTeam() != TEAMID_New &&
          IsJoin2TeamAllowed(rNewJoin.GetTeam()))
        // okay; accept change
        return true;
  ...
  // for runtime joins, teams are needed if specified by teams.txt or if any
  // teams have been created before (to avoid mixed team-noteam-scenarios)
  // but only assign teams in runtime join if the player won't pick it himself
  bool fWillHaveLobby = Game.Network.isEnabled() &&
                        !Game.Network.Status.isPastLobby() && Game.fLobby;
  bool fHasOrWillHaveLobby = Game.Network.isLobbyActive() || fWillHaveLobby;
  bool fCanPickTeamAtRuntime = !IsRandomTeam() &&
                               (rNewJoin.GetType() == C4PT_User) &&
                               IsRuntimeJoinTeamChoice();
  bool fIsTeamNeeded = IsRuntimeJoinTeamChoice() || GetTeamCount();
  if (!fHasOrWillHaveLobby && (!fIsTeamNeeded || fCanPickTeamAtRuntime))
    return false;
  // get least-used team
...
void C4TeamList::CompileFunc(StdCompiler *pComp) {
  // if (pComp->isCompiler()) Clear(); - do not clear, because this would
  // corrupt  the fCustom-flag
  pComp->Value(mkNamingAdapt(fActive, "Active", true));
  pComp->Value(mkNamingAdapt(fCustom, "Custom", true));
  pComp->Value(
      mkNamingAdapt(fAllowHostilityChange, "AllowHostilityChange", false));
  ...
  pComp->Value(mkNamingAdapt(mkEnumAdaptT<uint8_t>(eTeamDist, TeamDistEntries),
  ...
  pComp->Value(
      mkNamingAdapt(sScriptPlayerNames, "ScriptPlayerNames", StdStrBuf()));
  ...
  if (iTeamCount) {
    ...
    pComp->Value(mkNamingAdapt(
        mkArrayAdaptMap(ppList, iTeamCount, mkPtrAdaptNoNull<C4Team>), "Team"));
  }

  if (pComp->isCompiler()) {
    // adjust last team ID, which may not be set properly for player-generated
    // team files
...
bool C4TeamList::Load(C4Group &hGroup, class C4Scenario *pInitDefault,
  ...
  if (!hGroup.LoadEntryString(C4CFN_Teams, Buf)) {
    ...
    if (pInitDefault->Game.Goals.GetIDCount(C4ID_Melee, 1) ||
        ...
        Game.C4S.Game.Mode == C4S_Melee ||
        Game.C4S.Game.Mode == C4S_MeleeTeamwork) {
      fAllowHostilityChange = true;
      fActive = true;
      fAutoGenerateTeams = true;
    } else {
      // No goals/rules whatsoever: They could be present in the objects.txt,
      // but parsing that would be a bit of
      //  overkill
    ...
    if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(
            mkNamingAdapt(*this, "Teams"), Buf, C4CFN_Teams))
      return false;
  }
  // post-initialization: Generate default team colors
  int32_t iTeam = 0;
  C4Team *pTeam;
  while (pTeam = GetTeamByIndex(iTeam++)) pTeam->RecheckColor(*this);
  return true;
}
...
bool C4TeamList::Save(C4Group &hGroup) {
  ...
  try {
    StdStrBuf Buf =
        DecompileToBuf<StdCompilerINIWrite>(mkNamingAdapt(*this, "Teams"));
    // save it
...
void C4TeamList::RecheckTeams() {
  ...
  // redistribute players of largest team that has relocatable players left
  // towards smaller teams
  for (;;) {
    C4Team *pLowestTeam = GetRandomSmallestTeam();
    ...
    for (; iCnt--; ++ppCheck)
      if (!pLargestTeam ||
          pLargestTeam->GetPlayerCount() > (*ppCheck)->GetPlayerCount())
        if ((*ppCheck)->GetFirstUnjoinedPlayerID()) pLargestTeam = *ppCheck;
    // no team can redistribute?
    if (!pLargestTeam) break;
    // redistribution won't help much?
    if (pLargestTeam->GetPlayerCount() - pLowestTeam->GetPlayerCount() <= 1)
      break;
    ...
    C4ClientPlayerInfos *pClrInfo =
        Game.PlayerInfos.GetClientInfoByPlayerID(idRedistPlayer);
    assert(pClrInfo);
...
void C4TeamList::ReassignAllTeams() {
  ...
  while (pNfo = Game.PlayerInfos.GetNextPlayerInfoByID(idStart)) {
    ...
    C4ClientPlayerInfos *pCltInfo =
        Game.PlayerInfos.GetClientInfoByPlayerID(idStart);
    assert(pCltInfo);
    if (pCltInfo) {
      pCltInfo->SetUpdated();
    }
...
StdStrBuf C4TeamList::GetTeamDistName(TeamDist eTeamDist) const {
  switch (eTeamDist) {
    case TEAMDIST_Free:
      return (StdStrBuf(LoadResStr("IDS_MSG_TEAMDIST_FREE"), true));
    case TEAMDIST_Host:
      return (StdStrBuf(LoadResStr("IDS_MSG_TEAMDIST_HOST"), true));
    case TEAMDIST_None:
      return (StdStrBuf(LoadResStr("IDS_MSG_TEAMDIST_NONE"), true));
    case TEAMDIST_Random:
      return (StdStrBuf(LoadResStr("IDS_MSG_TEAMDIST_RND"), true));
    case TEAMDIST_RandomInv:
      return (StdStrBuf(LoadResStr("IDS_MSG_TEAMDIST_RNDINV"), true));
    default:
      return (FormatString("TEAMDIST_undefined(%d)", (int)eTeamDist));
  }
}
...
void C4TeamList::FillTeamDistOptions(C4GUI::ComboBox_FillCB *pFiller) const {
  ...
  if (IsAutoGenerateTeams())
    pFiller->AddEntry(GetTeamDistName(TEAMDIST_None).getData(),
                      TEAMDIST_None);  // no teams: only for regular melees
  pFiller->AddEntry(GetTeamDistName(TEAMDIST_Random).getData(),
                    TEAMDIST_Random);
  pFiller->AddEntry(GetTeamDistName(TEAMDIST_RandomInv).getData(),
                    TEAMDIST_RandomInv);
}
...
void C4TeamList::SendSetTeamDist(TeamDist eNewTeamDist) {
  ...
  Game.Control.DoInput(CID_Set,
                       new C4ControlSet(C4CVT_TeamDistribution, eNewTeamDist),
                       CDT_Sync);
}
...
void C4TeamList::SetTeamDistribution(TeamDist eToVal) {
  if (!Inside(eToVal, TEAMDIST_First, TEAMDIST_Last)) {
    assert(false);
    return;
  }
  eTeamDist = eToVal;
  ...
  if (Game.Control.isCtrlHost()) {
    ...
    if (IsRandomTeam() || eTeamDist == TEAMDIST_None)
      ...
      // otherwise, it's sufficient to just reassign any teams that are
      // incorrect for the current mode
      RecheckTeams();
    }
...
void C4TeamList::SendSetTeamColors(bool fEnabled) {
  // set it for all clients
  Game.Control.DoInput(CID_Set, new C4ControlSet(C4CVT_TeamColors, fEnabled),
                       CDT_Sync);
}
...
void C4TeamList::EnforceLeagueRules() {
  ...
  // fAllowHostilitySwitch = false; - temp hostility is OK; often used e.g. to
  // unstick friendly Clonks
...
int32_t C4TeamList::GetForcedTeamSelection(int32_t idForPlayer) const {
  ...
  if (pOKTeam) {
    // if teams are generated on the fly, there would always be the possibility
...
StdStrBuf C4TeamList::GetScriptPlayerName() const {
  ...
  if (!sScriptPlayerNames.getLength())
    return StdStrBuf(LoadResStr("IDS_TEXT_COMPUTER"));  // default name
