...
// *** C4ControlPacket
C4ControlPacket::C4ControlPacket() : iByClient(Game.Control.ClientID()) {}
...
void C4Control::Clear() { Pkts.Clear(); }
...
bool C4Control::PreExecute() const {
  ...
  for (C4IDPacket *pPkt = firstPkt(); pPkt; pPkt = nextPkt(pPkt)) {
    ...
    if (pPkt->getPktType() & CID_First) {
      C4ControlPacket *pCtrlPkt =
          static_cast<C4ControlPacket *>(pPkt->getPkt());
      if (pCtrlPkt) fReady &= pCtrlPkt->PreExecute();
    } else {
      LogF(
          "C4Control::PreExecute: WARNING: Ignoring packet type %2x (not "
          "control.)",
          pPkt->getPktType());
    }
  }
...
void C4Control::Execute() const {
  for (C4IDPacket *pPkt = firstPkt(); pPkt; pPkt = nextPkt(pPkt)) {
    ...
    if (pPkt->getPktType() & CID_First) {
      C4ControlPacket *pCtrlPkt =
          static_cast<C4ControlPacket *>(pPkt->getPkt());
      if (pCtrlPkt) pCtrlPkt->Execute();
    } else {
      LogF(
          "C4Control::Execute: WARNING: Ignoring packet type %2x (not "
          "control.)",
          pPkt->getPktType());
    }
  }
...
void C4Control::PreRec(C4Record *pRecord) const {
  for (C4IDPacket *pPkt = firstPkt(); pPkt; pPkt = nextPkt(pPkt)) {
    ...
    if (pCtrlPkt) pCtrlPkt->PreRec(pRecord);
  }
}

void C4Control::CompileFunc(StdCompiler *pComp) { pComp->Value(Pkts); }
...
void C4ControlSet::Execute() const {
  switch (eValType) {
    case C4CVT_ControlRate:  // adjust control rate
      ...
      Game.Control.ControlRate =
          BoundBy<int32_t>(Game.Control.ControlRate, 1, C4MaxControlRate);
      Game.Parameters.ControlRate = Game.Control.ControlRate;
      // write back adjusted control rate to network settings
      if (Game.Control.isCtrlHost() && !Game.Control.isReplay() &&
          Game.Control.isNetwork())
        Config.Network.ControlRate = Game.Control.ControlRate;
      // always show msg
      sprintf(OSTR, LoadResStr("IDS_NET_CONTROLRATE"), Game.Control.ControlRate,
              Game.FrameCounter);
    ...
    case C4CVT_FairCrew:
      ...
      if (Game.Parameters.FairCrewForced) {
        if (Game.Control.isCtrlHost())
          Log(LoadResStr("IDS_MSG_NOMODIFYFAIRCREW"));
        break;
      }
      // set new value
      if (iData < 0) {
      ...
      if (Game.IsRunning) {
        ...
        if (Game.Parameters.UseFairCrew) {
          int iRank =
              Game.Rank.RankByExperience(Game.Parameters.FairCrewStrength);
          Game.GraphicsSystem.FlashMessage(
              FormatString(LoadResStr("IDS_MSG_FAIRCREW_ACTIVATED"),
                           Game.Rank.GetRankName(iRank, true).getData())
                  .getData());
        } else
          Game.GraphicsSystem.FlashMessage(
              LoadResStr("IDS_MSG_FAIRCREW_DEACTIVATED"));
      }
      // lobby updates
...
void C4ControlScript::Execute() const {
  ...
  // user script: forbidden in league mode, from anyone in debug mode, otherwise
  // from host only
  if (!fInternal && (Game.Parameters.isLeague() ||
                     (iByClient != C4ClientIDHost) && !Console.Active))
    return;
  // execute
  C4Object *pObj = NULL;
  C4AulScript *pScript;
  ...
  if (!fInternal) {
    ...
    if (!LocalControl()) {
      ...
      if (pClient)
        LogF(" = %s (by %s)", rVal.GetDataString().getData(),
             pClient->getName());
      else
...
C4ControlPlayerSelect::C4ControlPlayerSelect(int32_t iPlr,
                                             const C4ObjectList &Objs)
...
void C4ControlPlayerSelect::Execute() const {
  ...
  if (!SelectObjs.IsClear() || !iObjCnt) pPlr->SelectCrew(SelectObjs);
}
...
C4ControlPlayerCommand::C4ControlPlayerCommand(int32_t iPlr, int32_t iCmd,
                                               int32_t iX, int32_t iY,
                                               C4Object *pTarget,
                                               C4Object *pTarget2,
                                               int32_t iData, int32_t iAddMode)
    : iPlr(iPlr),
      iCmd(iCmd),
      iX(iX),
      iY(iY),
      iTarget(Game.Objects.ObjectNumber(pTarget)),
      iTarget2(Game.Objects.ObjectNumber(pTarget2)),
      iData(iData),
      iAddMode(iAddMode) {}
...
void C4ControlPlayerCommand::Execute() const {
  C4Player *pPlr = Game.Players.Get(iPlr);
  if (pPlr) {
    pPlr->CountControl(C4Player::PCID_Command,
                       iCmd + iX + iY + iTarget + iTarget2);
    pPlr->ObjectCommand(iCmd, Game.Objects.ObjectPointer(iTarget), iX, iY,
                        Game.Objects.ObjectPointer(iTarget2), iData, iAddMode);
  }
}
...
void C4ControlSyncCheck::Execute() const {
  ...
  C4ControlSyncCheck *pSyncCheck = Game.Control.GetSyncCheck(Frame),
                     &SyncCheck = *pSyncCheck;
  if (!pSyncCheck) {
  ...
  if (Frame != pSyncCheck->Frame ||
    ...
    const char *szThis = "Client",
               *szOther = Game.Control.isReplay() ? "Rec " : "Host";
    if (iByClient != Game.Control.ClientID()) {
      const char *szTemp = szThis;
      szThis = szOther;
      szOther = szTemp;
    }
    // Message
    ...
    LogFatal(FormatString(
                 "Network: %s Frm %i Ctrl %i Rnc %i Rn3 %i Cpx %i PXS %i MMi "
                 "%i Obc %i Oei %i Sct %i",
                 szThis, Frame, ControlTick, RandomCount, Random3, AllCrewPosX,
                 PXSCount, MassMoverIndex, ObjectCount, ObjectEnumerationIndex,
                 SectShapeSum).getData());
    LogFatal(FormatString(
                 "Network: %s Frm %i Ctrl %i Rnc %i Rn3 %i Cpx %i PXS %i MMi "
                 "%i Obc %i Oei %i Sct %i",
                 szOther, SyncCheck.Frame, SyncCheck.ControlTick,
                 SyncCheck.RandomCount, SyncCheck.Random3,
                 SyncCheck.AllCrewPosX, SyncCheck.PXSCount,
                 SyncCheck.MassMoverIndex, SyncCheck.ObjectCount,
                 SyncCheck.ObjectEnumerationIndex,
                 SyncCheck.SectShapeSum).getData());
...
void C4ControlSyncCheck::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(mkNamingAdapt(mkIntPackAdapt(ObjectEnumerationIndex),
...
void C4ControlClientUpdate::Execute() const {
  ...
  if (!pClient) return;
  StdCopyStrBuf strClient(LoadResStr(pClient->isLocal() ? "IDS_NET_LOCAL_CLIENT"
                                                        : "IDS_NET_CLIENT"));
  ...
  switch (eType) {
    case CUT_Activate:
      ...
      // log
      LogF(LoadResStr(iData ? "IDS_NET_CLIENT_ACTIVATED"
                            : "IDS_NET_CLIENT_DEACTIVATED"),
           strClient.getData(), pClient->getName());
      // activate/deactivate
      pClient->SetActivated(!!iData);
      // local?
      if (pClient->isLocal()) Game.Control.SetActivated(!!iData);
      break;
    case CUT_SetObserver:
      // nothing to do?
      if (pClient->isObserver()) break;
      // log
      LogF(LoadResStr("IDS_NET_CLIENT_OBSERVE"), strClient.getData(),
           pClient->getName());
      // set observer (will deactivate)
      pClient->SetObserver();
      // local?
      if (pClient->isLocal()) Game.Control.SetActivated(false);
...
void C4ControlClientUpdate::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(
      mkNamingAdapt(mkIntPackAdapt(iID), "ClientID", C4ClientIDUnknown));
...
void C4ControlClientRemove::Execute() const {
  ...
  if (pClient->isLocal()) {
    StdStrBuf sMsg;
    sMsg.Format(LoadResStr("IDS_NET_CLIENT_REMOVED"), strClient.getData(),
                pClient->getName(), strReason.getData());
    Log(sMsg.getData());
    Game.RoundResults.EvaluateNetwork(C4RoundResults::NR_NetError,
                                      sMsg.getData());
  ...
  // log
  LogF(LoadResStr("IDS_NET_CLIENT_REMOVED"), strClient.getData(),
       pClient->getName(), strReason.getData());
  // remove all players
  Game.Players.RemoveAtClient(iID, true);
  // remove all resources
  if (Game.Network.isEnabled()) Game.Network.ResList.RemoveAtClient(iID);
  // lobby callback
  C4GameLobby::MainDlg *pLobby = Game.Network.GetLobby();
...
C4ControlJoinPlayer::C4ControlJoinPlayer(const char *szFilename,
                                         int32_t iAtClient, int32_t iIDInfo,
...
C4ControlJoinPlayer::C4ControlJoinPlayer(const char *szFilename,
    ...
    : Filename(szFilename, true),
      ...
      idInfo(iIDInfo),
      fByRes(false) {
  // load from file if filename is given - which may not be the case for script
  // players
  if (szFilename) {
...
void C4ControlJoinPlayer::Execute() const {
  ...
  if (!pInfo) {
    ...
    if (PlrData.getSize()) {
      // create temp file
      StdStrBuf PlayerFilename;
      PlayerFilename.Format("%s-%s", pClient->getName(),
      ...
      if (PlrData.SaveToFile(PlayerFilename.getData())) {
        Game.JoinPlayer(PlayerFilename.getData(), iAtClient, pClient->getName(),
                        pInfo);
      ...
      LogF("ERROR: Ghost player join: No player data for %s",
    ...
    StdStrBuf PlayerFilename;
    PlayerFilename.Format("%s" DirSep "%d-%s", Game.ScenarioFilename,
                          ResCore.getID(), GetFilename(ResCore.getFileName()));
    Game.JoinPlayer(PlayerFilename.getData(), iAtClient,
                    pClient ? pClient->getName() : "Unknown", pInfo);
  } else
    // Shouldn't happen
    assert(false);
}
...
void C4ControlJoinPlayer::Strip() {
  ...
  // create temp file
  StdStrBuf PlayerFilename;
  PlayerFilename = GetFilename(Filename.getData());
  PlayerFilename = Config.AtTempPath(PlayerFilename.getData());
  // Copy to it
  if (PlrData.SaveToFile(PlayerFilename.getData())) {
    // open as group
...
bool C4ControlJoinPlayer::PreExecute() const {
  ...
  if (!pRes) {
    pRes = Game.Network.ResList.AddByCore(ResCore, true);
  }
  if (!pRes) return true;
...
void C4ControlJoinPlayer::PreRec(C4Record *pRecord) {
  ...
  if (fByRes) {
    ...
    StdStrBuf szTemp;
    szTemp.Copy(pRes->getFile());
    MakeTempFilename(&szTemp);
    if (C4Group_CopyItem(pRes->getFile(), szTemp.getData())) {
      // add to record
      StdStrBuf szTarget = FormatString("%d-%s", ResCore.getID(),
                                        GetFilename(ResCore.getFileName()));
...
C4ControlEMMoveObject::C4ControlEMMoveObject(C4ControlEMObjectAction eAction,
    ...
    : eAction(eAction),
      tx(tx),
      ty(ty),
      iTargetObj(Game.Objects.ObjectNumber(pTargetObj)),
      iObjectNum(iObjectNum),
      pObjects(pObjects),
      Script(szScript, true) {}
...
void C4ControlEMMoveObject::Execute() const {
  ...
  if (Game.Parameters.isLeague()) return;
  ...
  switch (eAction) {
    ...
    case EMMO_Duplicate: {
      ...
      for (int i = 0; i < iObjectNum; ++i)
        if (pObj = Game.Objects.SafeObjectPointer(pObjects[i])) {
          pObj =
              Game.CreateObject(pObj->id, pObj, pObj->Owner, pObj->x, pObj->y);
          if (pObj && fLocalCall)
            Console.EditCursor.GetSelection().Add(pObj, C4ObjectList::stNone);
        }
      // update status
      if (fLocalCall) {
    ...
    case EMMO_Script: {
      if (!pObjects) return;
      // execute script ...
      C4ControlScript ScriptCtrl(Script.getData(),
                                 C4ControlScript::SCOPE_Global, false);
  ...
  if (fLocalCall) Console.EditCursor.OnSelectionChanged();
}
...
void C4ControlEMMoveObject::CompileFunc(StdCompiler *pComp) {
  ...
  if (pComp->isCompiler()) {
    delete[] pObjects;
    pObjects = new int32_t[iObjectNum];
  }
  pComp->Value(mkNamingAdapt(mkArrayAdapt(pObjects, iObjectNum), "Objs", -1));
  if (eAction == EMMO_Script) pComp->Value(mkNamingAdapt(Script, "Script", ""));
  C4ControlPacket::CompileFunc(pComp);
}

// *** C4ControlEMDrawTool

C4ControlEMDrawTool::C4ControlEMDrawTool(C4ControlEMDrawAction eAction,
                                         int32_t iMode, int32_t iX, int32_t iY,
                                         int32_t iX2, int32_t iY2,
                                         int32_t iGrade, bool fIFT,
    ...
    : eAction(eAction),
      ...
      iGrade(iGrade),
      fIFT(fIFT),
      Material(szMaterial, true),
      Texture(szTexture, true) {}
...
void C4ControlEMDrawTool::Execute() const {
  ...
  const char *szMaterial = Material.getData(), *szTexture = Texture.getData();
  // perform action
  switch (eAction) {
    ...
    case EMDT_Line:  // line tool
      if (!Texture.getSize()) break;
      Game.Landscape.DrawLine(iX, iY, iX2, iY2, iGrade, szMaterial, szTexture,
                              fIFT);
      break;
    case EMDT_Rect:  // rect tool
      if (!Texture.getSize()) break;
      Game.Landscape.DrawBox(iX, iY, iX2, iY2, iGrade, szMaterial, szTexture,
                             fIFT);
      break;
    case EMDT_Fill:  // fill tool
    {
      ...
      for (int cnt = 0; cnt < iGrade; cnt++)
        Game.Landscape.InsertMaterial(iMat, iX + Random(iGrade) - iGrade / 2,
                                      iY + Random(iGrade) - iGrade / 2);
    } break;
  }
}
  ...
  switch (eType) {
    ...
    case C4CMT_Me:
      ...
      if (pPlr) {
        ...
        sprintf(OSTR, (eType == C4CMT_Normal ? "<c %x><<i></i>%s> %s</c>"
                                             : "<c %x> * %s %s</c>"),
    ...
    case C4CMT_Say: {
      ...
      if (Game.C4S.Head.Film == C4SFilm_Cinematic) {
        StdStrBuf sMessage;
        sMessage.Format("<%s> %s", pPlr->Cursor ? pPlr->Cursor->Name.getData()
                                                : pPlr->GetName(),
                        szMessage);
    ...
    case C4CMT_Team: {
      ...
      if (pPlr) {
        ...
        for (int cnt = 0; pLocalPlr = Game.Players.GetLocalByIndex(cnt); cnt++)
          if (!Hostile(pLocalPlr->Number, iPlayer)) break;
        if (pLocalPlr)
          Log(FormatString("<c %x>{%s} %s</c>", pPlr->ColorDw, pPlr->GetName(),
                           szMessage).getData());
      } else if (pLobby) {
        // in lobby mode, no player has joined yet - check teams of unjoined
        // players
        if (!Game.PlayerInfos.HasSameTeamPlayers(iByClient,
                                                 Game.Clients.getLocalID()))
          break;
        // OK - permit message
        C4Client *pClient = Game.Clients.getClientByID(iByClient);
        pLobby->OnMessage(
            Game.Clients.getClientByID(iByClient),
            FormatString("{%s} %s", pClient ? pClient->getNick() : "???",
                         szMessage).getData());
      }
    } break;
    ...
    case C4CMT_Private: {
      ...
      C4Player *pLocalPlr;
      for (int cnt = 0; pLocalPlr = Game.Players.GetLocalByIndex(cnt); cnt++)
        if (pLocalPlr->Number == iToPlayer) break;
      if (pLocalPlr) {
        Log(FormatString("<c %x>[%s] %s</c>", pPlr->ColorDw, pPlr->GetName(),
                         szMessage).getData());
      }
    ...
    case C4CMT_Sound:
      ...
      if (StartSoundEffect(szMessage, false, 100, NULL)) {
        if (pLobby)
          pLobby->OnClientSound(Game.Clients.getClientByID(iByClient));
      }
      break;
...
void C4ControlMessage::CompileFunc(StdCompiler *pComp) {
  pComp->Value(
      mkNamingAdapt(mkIntAdaptT<uint8_t>(eType), "Type", C4CMT_Normal));
...
void C4ControlDebugRec::CompileFunc(StdCompiler *pComp) { pComp->Value(Data); }
...
StdStrBuf C4ControlVote::getDesc() const {
  ...
  switch (eType) {
    case VT_Cancel:
      Action = LoadResStr("IDS_VOTE_CANCELTHEROUND");
      break;
    case VT_Kick:
      if (iData == iByClient)
        ...
        Action.Format(LoadResStr("IDS_VOTE_KICKCLIENT"),
                      pTargetClient ? pTargetClient->getName() : "???");
      }
      break;
    case VT_Pause:
      if (iData)
...
StdStrBuf C4ControlVote::getDescWarning() const {
  ...
  switch (eType) {
    case VT_Cancel:
      Warning = LoadResStr("IDS_TEXT_WARNINGIFTHEGAMEISCANCELL");
      break;
    case VT_Kick:
      Warning = LoadResStr("IDS_TEXT_WARNINGNOLEAGUEPOINTSWILL");
      break;
    default:
      Warning = "";
      break;
...
void C4ControlVote::Execute() const {
  ...
  C4Client *pClient = Game.Clients.getClientByID(iByClient);
  if (fApprove)
    LogF(LoadResStr("IDS_VOTE_WANTSTO"), pClient->getName(),
         getDesc().getData());
  else
    LogF(LoadResStr("IDS_VOTE_DOESNOTWANTTO"), pClient->getName(),
         getDesc().getData());
  // Save vote back
  if (Game.Network.isEnabled()) Game.Network.AddVote(*this);
  // Vote done?
  if (Game.Control.isCtrlHost()) {
    // Count votes
    ...
    for (int32_t i = 0; i < Max<int32_t>(Game.Teams.GetTeamCount(), 1); i++) {
      ...
      // Check each player
      for (int32_t j = 0; j < (pTeam ? pTeam->GetPlayerCount()
                                     : Game.PlayerInfos.GetPlayerCount());
           j++) {
        ...
        if (!pTeam) {
          ...
          iClientID = Game.PlayerInfos.GetClientInfoByPlayerID(pNfo->GetID())
                          ->GetClientID();
        } else {
          pNfo = Game.PlayerInfos.GetPlayerInfoByID(pTeam->GetIndexedPlayer(j),
                                                    &iClientID);
          if (!pNfo) continue;  // shouldn't happen
        }
      ...
      if (iVotesTeam) {
        ...
        if (iPositiveTeam * 2 > iVotesTeam) iPositive++;
    ...
    if (iPositive * 2 > iVotes)
      Game.Control.DoInput(CID_VoteEnd,
                           new C4ControlVoteEnd(eType, true, iData), CDT_Sync);
    // Disapproval?
    else if (iNegative * 2 >= iVotes)
      Game.Control.DoInput(CID_VoteEnd,
                           new C4ControlVoteEnd(eType, false, iData), CDT_Sync);
  }
}
...
void C4ControlVoteEnd::Execute() const {
  ...
  switch (getType()) {
    case VT_Cancel:
      // Flag players
      if (!Game.GameOver)
        for (iClient = 0; pInfos = Game.PlayerInfos.GetIndexedInfo(iClient);
             iClient++)
          for (iInfo = 0; pInfo = pInfos->GetPlayerInfo(iInfo); iInfo++)
            if (!pInfo->IsRemoved()) pInfo->SetVotedOut();
      // Abort the game
      Game.Abort(true);
      break;
    case VT_Kick:
      ...
      if (getData() == Game.Clients.getLocalID()) {
        ...
        // Do a regular disconnect and display reason in game over dialog, so
        // the client knows what has happened!
        Game.RoundResults.EvaluateNetwork(
            C4RoundResults::NR_NetError,
            FormatString(LoadResStr("IDS_ERR_YOUHAVEBEENREMOVEDBYVOTIN"),
                         sMsg.getData()).getData());
        Game.Network.Clear();
