...
class C4Control : public C4PacketBase {
 ...
 public:
  ...
  void AddHead(C4PacketType eType, C4ControlPacket *pCtrl) {
    Pkts.AddHead(eType, pCtrl);
  }
  void Add(C4PacketType eType, C4ControlPacket *pCtrl) {
    Pkts.Add(eType, pCtrl);
  }
...
class C4ControlSet : public C4ControlPacket  // sync, lobby
                     {
 public:
  C4ControlSet() : eValType(C4CVT_None), iData(0) {}
 ...
 public:
  ...
  virtual bool Lobby() const {
    return eValType == C4CVT_TeamDistribution || eValType == C4CVT_TeamColors;
  }
...
class C4ControlScript : public C4ControlPacket  // sync
 ...
 public:
  enum {
    SCOPE_Console = -2,
    SCOPE_Global = -1,
  };  // special scopes to be passed as target objects

  C4ControlScript() : iTargetObj(-1), fInternal(true) {}
...
class C4ControlPlayerSelect : public C4ControlPacket  // sync
                              {
 public:
  C4ControlPlayerSelect() : iPlr(-1), iObjCnt(0), pObjNrs(NULL) {}
...
class C4ControlPlayerControl : public C4ControlPacket  // sync
                               {
 public:
  C4ControlPlayerControl() : iPlr(-1), iCom(-1), iData(-1) {}
...
class C4ControlPlayerCommand : public C4ControlPacket  // sync
                               {
 public:
  C4ControlPlayerCommand() : iPlr(-1), iCmd(-1) {}
  C4ControlPlayerCommand(int32_t iPlr, int32_t iCmd, int32_t iX, int32_t iY,
                         C4Object *pTarget, C4Object *pTarget2, int32_t iData,
                         int32_t iAddMode);
...
class C4ControlClientUpdate : public C4ControlPacket  // sync, lobby
 ...
 public:
  ...
  C4ControlClientUpdate(int32_t iID, C4ControlClientUpdType eType,
                        int32_t iData = 0)
...
class C4ControlPlayerInfo : public C4ControlPacket  // not sync, lobby
 ...
 public:
  C4ControlPlayerInfo() {}
  C4ControlPlayerInfo(const C4ClientPlayerInfos &PlrInfo) : PlrInfo(PlrInfo) {}
...
struct C4ControlJoinPlayer : public C4ControlPacket  // sync
 ...
 public:
  ...
  C4ControlJoinPlayer(const char *szFilename, int32_t iAtClient,
                      int32_t iIDInfo, const C4Network2ResCore &ResCore);
  C4ControlJoinPlayer(const char *szFilename, int32_t iAtClient,
                      int32_t iIDInfo);
...
class C4ControlEMMoveObject : public C4ControlPacket  // sync
 ...
 public:
  ...
  C4ControlEMMoveObject(C4ControlEMObjectAction eAction, int32_t tx, int32_t ty,
                        C4Object *pTargetObj, int32_t iObjectNum = 0,
                        int32_t *pObjects = NULL, const char *szScript = NULL);
...
class C4ControlEMDrawTool : public C4ControlPacket  // sync
 ...
 public:
  ...
  C4ControlEMDrawTool(C4ControlEMDrawAction eAction, int32_t iMode,
                      int32_t iX = -1, int32_t iY = -1, int32_t iX2 = -1,
                      int32_t iY2 = -1, int32_t iGrade = -1, bool fIFT = true,
                      const char *szMaterial = NULL,
                      const char *szTexture = NULL);

 protected:
  C4ControlEMDrawAction eAction;  // action to be performed
  int32_t iMode;  // new mode, or mode action was performed in (action will fail
                  // if changed)
...
class C4ControlMessage : public C4ControlPacket  // not sync, lobby
                         {
 public:
  C4ControlMessage() : eType(C4CMT_Normal), iPlayer(-1) {}
  C4ControlMessage(C4ControlMessageType eType, const char *szMessage,
                   int32_t iPlayer = -1, int32_t iToPlayer = -1)
      : eType(eType),
        iPlayer(iPlayer),
        iToPlayer(iToPlayer),
        Message(szMessage, true) {}
...
class C4ControlDebugRec : public C4ControlPacket  // sync
 ...
 public:
  C4ControlDebugRec() {}
  C4ControlDebugRec(StdBuf &Data) : Data(Data) {}
...
class C4ControlVote : public C4ControlPacket {
 public:
  C4ControlVote(C4ControlVoteType eType = VT_None, bool fApprove = true,
                int iData = 0)
...
class C4ControlVoteEnd : public C4ControlVote {
 public:
  C4ControlVoteEnd(C4ControlVoteType eType = VT_None, bool fApprove = true,
                   int iData = 0)
