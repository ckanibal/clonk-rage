...
C4GameControl::C4GameControl() : Network(this) { Default(); }

C4GameControl::~C4GameControl() { Clear(); }

bool C4GameControl::InitLocal(C4Client *pLocal) {
  eMode = CM_Local;
  fPreInit = fInitComplete = true;
  fHost = true;
  iClientID = pLocal->getID();
  ControlRate = 1;
  // ok
  return true;
}
...
void C4GameControl::ChangeToLocal() {
  ...
  if (Game.Clients.getLocal()) Game.Clients.getLocal()->SetActivated(true);

  // network: clear network
  if (eMode == CM_Network) {
    Network.Clear();
    if (Game.Network.isEnabled()) Game.Network.Clear();
  }
...
bool C4GameControl::StartRecord(bool fInitial, bool fStreaming) {
  ...
  if (pExecutingControl) pRecord->Rec(*pExecutingControl, Game.FrameCounter);
  // ok
  return true;
}
...
// currnetly do not request, but start record with next gamesync, so network
// runtime join can be debugged
#ifndef DEBUGREC
...
bool C4GameControl::Prepare() {
  ...
  switch (eMode) {
    ...
    case CM_Network:
      ...
      // control tick?
      if (Game.FrameCounter % ControlRate) return true;
...
void C4GameControl::Execute() {
  ...
  if (!isReplay() && Game.FrameCounter % ControlRate) return;
  ...
  // Record: Save ctrl
  if (pRecord) pRecord->Rec(Control, Game.FrameCounter);
...
void C4GameControl::Ticks() {
  ...
  if (!(Game.FrameCounter % ControlRate)) ControlTick++;
  if (!(Game.FrameCounter % SyncRate)) DoSync = true;
...
void C4GameControl::AdjustControlRate(int32_t iBy) {
  // control host only
  if (isCtrlHost())
    Game.Control.DoInput(CID_Set, new C4ControlSet(C4CVT_ControlRate, iBy),
                         CDT_Decide);
}

void C4GameControl::SetActivated(bool fnActivated) {
  fActivated = fnActivated;
  if (eMode == CM_Network) Network.SetActivated(fnActivated);
}

void C4GameControl::DoInput(C4PacketType eCtrlType, C4ControlPacket *pPkt,
  ...
  if (eDelivery == CDT_Direct || eDelivery == CDT_Private)
    assert(!pPkt->Sync());
  if (!fInitComplete) assert(pPkt->Lobby());

  // decide control type
  if (eDelivery == CDT_Decide) eDelivery = DecideControlDelivery();
...
void C4GameControl::DbgRec(C4RecordChunkType eType, const uint8_t *pData,
                           size_t iSize) {
#ifdef DEBUGREC
  ...
  if (pRecord)
    pRecord->Rec(Game.FrameCounter,
                 DecompileToBuf<StdCompilerBinWrite>(
                     C4PktDebugRec(eType, StdBuf(pData, iSize))),
                 eType);
  // check against playback
  if (pPlayback) pPlayback->Check(eType, pData, iSize);
#endif  // DEBUGREC
}

C4ControlDeliveryType C4GameControl::DecideControlDelivery() {
  // network
  if (eMode == CM_Network) return Network.DecideControlDelivery();
  // use direct
  return CDT_Direct;
}
...
void C4GameControl::DoSyncCheck() {
  ...
  // host?
  if (fHost)
    // add sync check to control queue or send it directly if the queue isn't
    // active
...
void C4GameControl::ExecControl(const C4Control &rCtrl) {
  ...
  rCtrl.Execute();
  // record
  if (pRecord) pRecord->Rec(rCtrl, Game.FrameCounter);
}
...
void C4GameControl::ExecControlPacket(C4PacketType eCtrlType,
  ...
  // execute it
  if (!pPkt->PreExecute())
    Log("Control: PreExecute failed for direct control!");
  pPkt->Execute();
  // record it
  if (pRecord) pRecord->Rec(eCtrlType, pPkt, Game.FrameCounter);
}

C4ControlSyncCheck *C4GameControl::GetSyncCheck(int32_t iTick) {
  for (C4IDPacket *pPkt = SyncChecks.firstPkt(); pPkt;
       pPkt = SyncChecks.nextPkt(pPkt)) {
    // should be a sync check
    if (pPkt->getPktType() != CID_SyncCheck) continue;
    // get sync check
    C4ControlSyncCheck *pCheck =
        static_cast<C4ControlSyncCheck *>(pPkt->getPkt());
    // packet that's searched for?
    if (pCheck->getFrame() == iTick) return pCheck;
  }
  return NULL;
}
