...
void C4RoundResultsPlayer::CompileFunc(StdCompiler *pComp) {
  ...
  // pComp->Value(mkNamingAdapt(fctBigIcon,    "Icon",      C4FacetEx())); - not
  // possible
  ...
  pComp->Value(mkNamingAdapt(iLeagueScoreNew, "Score",
                             -1));  // name used in league reply!
  pComp->Value(mkNamingAdapt(iLeagueScoreGain, "GameScore",
                             -1));  // name used in league reply!
  pComp->Value(
      mkNamingAdapt(iLeagueRankNew, "Rank", 0));  // name used in league reply!
  pComp->Value(mkNamingAdapt(iLeagueRankSymbolNew, "RankSymbol",
                             0));  // name used in league reply!
  pComp->Value(mkNamingAdapt(sLeagueProgressData, "LeagueProgressData",
                             StdCopyStrBuf()));
  StdEnumEntry<LeagueStatus> LeagueStatusEntries[] = {
      {"", RRPLS_Unknown}, {"Lost", RRPLS_Lost}, {"Won", RRPLS_Won},
  };
  pComp->Value(
      mkNamingAdapt(mkEnumAdaptT<uint8_t>(eLeagueStatus, LeagueStatusEntries),
                    "Status", RRPLS_Unknown));  // name used in league reply!
}
...
void C4RoundResultsPlayer::EvaluatePlayer(C4Player *pPlr) {
  ...
  // BigIcon from info: Doesn't work for some cases when player files got
  // deleted already
  /*assert(pInfo);
  if (pInfo) pInfo->LoadBigIcon(fctBigIcon);*/
}

void C4RoundResultsPlayer::EvaluateLeague(
    C4RoundResultsPlayer *pLeaguePlayerInfo) {
...
void C4RoundResultsPlayer::AddCustomEvaluationString(
  ...
  if (sCustomEvaluationStrings.getLength())
    sCustomEvaluationStrings.Append("   ");
...
C4RoundResultsPlayer &C4RoundResultsPlayer::operator=(
    const C4RoundResultsPlayer &cpy) {
...
void C4RoundResultsPlayers::CompileFunc(StdCompiler *pComp) {
  ...
  // Compile
  pComp->Value(
      mkNamingAdapt(mkArrayAdaptMap(ppPlayers, iPlayerCount,
                                    mkPtrAdaptNoNull<C4RoundResultsPlayer>),
                    "Player"));
...
C4RoundResultsPlayer *C4RoundResultsPlayers::GetByID(int32_t id) const {
  for (int32_t idx = 0; idx < iPlayerCount; ++idx)
    if (ppPlayers[idx]->GetID() == id) return ppPlayers[idx];
  return NULL;
}
...
void C4RoundResultsPlayers::GrowList(size_t iByVal) {
  ...
  // valid state)
  C4RoundResultsPlayer **ppNew =
      new C4RoundResultsPlayer *[iPlayerCapacity += iByVal];
  // move existing
  if (ppPlayers) {
...
bool C4RoundResultsPlayers::operator==(const C4RoundResultsPlayers &cmp) {
  ...
  for (int32_t i = 0; i < iPlayerCount; ++i)
    if (!(*ppPlayers[i] == *cmp.ppPlayers[i])) return false;
  // equal
  return true;
}

C4RoundResultsPlayers &C4RoundResultsPlayers::operator=(
    const C4RoundResultsPlayers &cpy) {
  Clear();
  C4RoundResultsPlayer *pPlr;
  int32_t i = 0;
  while (pPlr = cpy.GetByIndex(i++)) Add(new C4RoundResultsPlayer(*pPlr));
  return *this;
}
...
void C4RoundResults::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(mkNamingAdapt(fHideSettlementScore, "HideSettlementScore",
                             !!Game.C4S.Game.IsMelee()));
  pComp->Value(mkNamingAdapt(sCustomEvaluationStrings,
                             "CustomEvaluationStrings", StdCopyStrBuf()));
  ...
  pComp->Value(
      mkNamingAdapt(mkEnumAdaptT<uint8_t>(eNetResult, NetResultEntries),
                    "NetResult", NR_None));
}
...
void C4RoundResults::EvaluateGame() {
  ...
  int32_t iFirstLocalPlayer =
      pFirstLocalPlayer ? pFirstLocalPlayer->Number : NO_OWNER;
...
void C4RoundResults::EvaluateNetwork(C4RoundResults::NetResult eNetResult,
                                     const char *szResultMsg) {
  // take result only if there was no previous result (the previous one is
  // usually more specific)
  if (!HasNetResult()) {
    this->eNetResult = eNetResult;
    if (szResultMsg)
      sNetResult.Copy(szResultMsg);
    else
      sNetResult.Clear();
  }
}

void C4RoundResults::EvaluateLeague(const char *szResultMsg, bool fSuccess,
                                    const C4RoundResultsPlayers &rLeagueInfo) {
  // League evaluation imples network evaluation
  Game.RoundResults.EvaluateNetwork(
      fSuccess ? C4RoundResults::NR_LeagueOK : C4RoundResults::NR_LeagueError,
      szResultMsg);
  // Evaluation called by league: Sets new league scores and ranks
  C4RoundResultsPlayer *pPlr, *pOwnPlr;
  int32_t i = 0;
...
void C4RoundResults::AddCustomEvaluationString(const char *szCustomString,
  ...
  if (!idPlayer) {
    if (sCustomEvaluationStrings.getLength())
      sCustomEvaluationStrings.AppendChar('|');
    sCustomEvaluationStrings.Append(szCustomString);
  } else {
...
bool C4RoundResults::SettlementScoreIsHidden() { return fHideSettlementScore; }
...
bool C4RoundResults::Load(C4Group &hGroup, const char *szFilename) {
  ...
  if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(
          mkNamingAdapt(*this, "RoundResults"), Buf, szFilename))
    return false;
  // done, success
  return true;
}
...
bool C4RoundResults::Save(C4Group &hGroup, const char *szFilename) {
  ...
  try {
    StdStrBuf Buf = DecompileToBuf<StdCompilerINIWrite>(
        mkNamingAdapt(*this, "RoundResults"));
    // save it, if not empty
    if (Buf.getLength())
      if (!hGroup.Add(szFilename, Buf, false, true)) return false;
