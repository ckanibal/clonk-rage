...
DWORD GenerateRandomPlayerColor(
    int32_t iTry)  // generate a random player color for the iTry'th try
{
...
bool IsColorConflict(
    DWORD dwClr1,
    DWORD dwClr2)  // return whether dwClr1 and dwClr2 are closely together
{
  ...
  int B2 = 0xff & (dwClr2);
  double r1 = 0, g1 = 0, b1 = 0, r2 = 0, g2 = 0, b2 = 0, x1 = 0, y1 = 0, Y1 = 0,
         x2 = 0, y2 = 0, Y2 = 0, u1 = 0, v1 = 0, u2 = 0, v2 = 0;
  ...
  double Y = (Y1 + Y2) / 2.0;
  double clrdiff =
      sqrt((u2 - u1) * (u2 - u1) + (v2 - v1) * (v2 - v1)) * Y * Y * 150;
...
class C4PlayerInfoListAttributeConflictResolver {
 private:
  ...
  C4ClientPlayerInfos *pLowPrioOriginalConflictPacket,
      *pLowPrioAlternateConflictPacket;

 public:
  C4PlayerInfoListAttributeConflictResolver(
      C4PlayerInfoList &rPriCheckList, const C4PlayerInfoList &rSecCheckList,
      C4ClientPlayerInfos *pSecPacket);
  ~C4PlayerInfoListAttributeConflictResolver();

 private:
  void ReaddInfoForCheck(C4ClientPlayerInfos *pCheckAdd);  // temp readd info to
                                                           // be checked,
                                                           // because another
                                                           // packet reset its
                                                           // attribute with
                                                           // higher priority
  int32_t GetAttributePriorityDifference(const C4PlayerInfo *pInfo1,
                                         const C4ClientPlayerInfos *pPck1,
                                         const C4PlayerInfo *pInfo2,
                                         const C4ClientPlayerInfos *pPck2);
  bool IsAttributeConflict(const C4PlayerInfo *pInfo1,
                           const C4PlayerInfo *pInfo2,
                           C4PlayerInfo::AttributeLevel eLevel);
...
void C4PlayerInfoList::ResolvePlayerAttributeConflicts(
    C4ClientPlayerInfos *pSecPacket) {
  C4PlayerInfoListAttributeConflictResolver r(*this, Game.RestorePlayerInfos,
                                              pSecPacket);
  r.Resolve();
}
...
C4PlayerInfoListAttributeConflictResolver::
    ...
    : ppCheckInfos(NULL),
      iCheckInfoCount(0),
      rPriCheckList(rPriCheckList),
      rSecCheckList(rSecCheckList),
      pSecPacket(pSecPacket) {
  // prepare check array
  ...
  // resolve all primary packets in reverse order, so late clients have lower
  // priority
  for (int32_t i = 0; i < rPriCheckList.GetInfoCount(); ++i) {
    C4ClientPlayerInfos *pInfos = rPriCheckList.GetIndexedInfo(i);
    if (pInfos !=
        pSecPacket)  // skip packet if it was explicitly passed for update
      ppCheckInfos[iCheckInfoCount++] = pInfos;
    else {
      // if the additional packet is in the list already, it needn't be a check
      // packed (only resolve packet)
...
void C4PlayerInfoListAttributeConflictResolver::ReaddInfoForCheck(
    C4ClientPlayerInfos *pCheckAdd) {
  // do not add twice<
  for (int32_t i = 0; i < iCheckInfoCount; ++i)
    if (ppCheckInfos[i] == pCheckAdd) return;
...
int32_t
C4PlayerInfoListAttributeConflictResolver::GetAttributePriorityDifference(
    const C4PlayerInfo *pInfo1, const C4ClientPlayerInfos *pPck1,
    const C4PlayerInfo *pInfo2, const C4ClientPlayerInfos *pPck2) {
  // return info1.prio - info2.prio
  // highest priority if joined already and attributes may not be changed
  if (pInfo1->IsJoined()) return pInfo2->IsJoined() ? 0 : +1;
  if (pInfo2->IsJoined()) return -1;
  ...
  if (pPck1->IsDeveloperPacket()) return pPck2->IsDeveloperPacket() ? 0 : +1;
  if (pPck2->IsDeveloperPacket()) return -1;
  // equal priority
  return 0;
}

bool C4PlayerInfoListAttributeConflictResolver::IsAttributeConflict(
    const C4PlayerInfo *pInfo1, const C4PlayerInfo *pInfo2,
    C4PlayerInfo::AttributeLevel eLevel) {
  // check for conflict of colors and names
  if (eAttr == C4PlayerInfo::PLRATT_Color) {
    uint32_t dwClr1 = pInfo1->GetColor(), dwClr2 = 0;
    switch (eLevel) {
      case C4PlayerInfo::PLRAL_Current:
        dwClr2 = pInfo2->GetColor();
        break;
      case C4PlayerInfo::PLRAL_Original:
        dwClr2 = pInfo2->GetOriginalColor();
        break;
      case C4PlayerInfo::PLRAL_Alternate:
        dwClr2 = pInfo2->GetAlternateColor();
        break;
    }
    ...
    switch (eLevel) {
      case C4PlayerInfo::PLRAL_Current:
        szName2 = pInfo2->GetName();
        break;
      case C4PlayerInfo::PLRAL_Original:
        szName2 = pInfo2->GetOriginalName();
        break;
    }
...
void C4PlayerInfoListAttributeConflictResolver::MarkConflicts(
    C4ClientPlayerInfos &rCheckPacket, bool fTestOriginal) {
  ...
  for (int32_t j = 0; pCheckAgainstInfo = rCheckPacket.GetPlayerInfo(j); ++j) {
    if (pCheckAgainstInfo->IsUsingAttribute(eAttr))
      if (!pResolveInfo->GetID() ||
          pResolveInfo->GetID() != pCheckAgainstInfo->GetID())
        if (pResolveInfo != pCheckAgainstInfo) {
          // current conflict is marked only if the checked packet has same of
          // lower priority than the one compared to
          // if the priority is higher, the attribute shall be changed in the
          // other, low priority info instead!
          bool fHasHigherPrio = (GetAttributePriorityDifference(
                                     pResolveInfo, pResolvePacket,
                                     pCheckAgainstInfo, &rCheckPacket) > 0);
          if (!fHasHigherPrio)
            if (IsAttributeConflict(pCheckAgainstInfo, pResolveInfo,
                                    C4PlayerInfo::PLRAL_Current))
              fCurrentConflict = true;
          if (fTestOriginal) {
            if (IsAttributeConflict(pCheckAgainstInfo, pResolveInfo,
                                    C4PlayerInfo::PLRAL_Original))
              if (fHasHigherPrio && !fOriginalConflict &&
                  !pLowPrioOriginalConflictPacket) {
                // original attribute is taken by a low prio packet - do not
                // mark an original conflict, but remember the packet
                // that's blocking it
                pLowPrioOriginalConflictPacket = &rCheckPacket;
              } else {
                // original attribute is taken by either one higher/equal
                // priority by packet, or by two low prio packets
                // in this case, don't revert to original
                pLowPrioOriginalConflictPacket = NULL;
                fOriginalConflict = true;
              }
            if (IsAttributeConflict(pCheckAgainstInfo, pResolveInfo,
                                    C4PlayerInfo::PLRAL_Alternate))
              if (fHasHigherPrio && !fAlternateConflict &&
                  !pLowPrioAlternateConflictPacket)
...
void C4PlayerInfoListAttributeConflictResolver::MarkConflicts(
    const C4PlayerInfoList &rCheckList, bool fTestOriginal) {
...
void C4PlayerInfoListAttributeConflictResolver::ResolveInInfo() {
  ...
  fAlternateConflict =
      (eAttr == C4PlayerInfo::PLRATT_Name) ||
      !pResolveInfo->GetAlternateColor();  // mark as conflict if there is no
                                           // alternate color/name
  for (;;) {
    // check against all other player infos, and given info, too (may be
    // redundant)
    ...
    // check secondary list, too. But only for colors, not for names, because
    // secondary list is Restore list
    // and colors are retained in restore while names are always taken from new
    // joins
    if (eAttr != C4PlayerInfo::PLRATT_Name)
      MarkConflicts(rSecCheckList, !iTries);
    ...
    if (eAttr == C4PlayerInfo::PLRATT_Color) {
      ...
      if (!iTries) {
        if (pResolveInfo->GetColor() != pResolveInfo->GetOriginalColor())
          if (!fOriginalConflict) {
            ...
            // in case a lower priority packet was blocking the attribute,
            // re-check that packet
            // note that the may readd the current resolve packet, but the
            // conflict will occur with the
            // lower priority packet in the next loop
            if (pLowPrioOriginalConflictPacket)
              ReaddInfoForCheck(pLowPrioOriginalConflictPacket);
            // done with this player (breaking the trial-loop)
            break;
          }
          // neither original nor alternate color used but alternate color free?
          else if (pResolveInfo->GetColor() !=
                       pResolveInfo->GetAlternateColor() &&
                   !fAlternateConflict) {
            // revert to alternate
            pResolveInfo->SetColor(pResolveInfo->GetAlternateColor());
            if (pLowPrioAlternateConflictPacket)
              ReaddInfoForCheck(pLowPrioAlternateConflictPacket);
            // done with this player (breaking the trial-loop)
            break;
          }
      }
      ...
      if (++iTries > C4MaxPlayerColorChangeTries) {
        LogF(LoadResStr("IDS_PRC_NOREPLPLRCLR"),
             pResolveInfo->GetName() ? pResolveInfo->GetName() : "<NONAME>");
    ...
    } else  // if (eAttr == PLRATT_Name)
    {
      ...
      if (!SEqualNoCase(pResolveInfo->GetName(),
                        pResolveInfo->GetOriginalName()))
        if (!fOriginalConflict) {
      ...
      pResolveInfo->SetForcedName(FormatString("%s (%d)",
                                               pResolveInfo->GetOriginalName(),
                                               iTries + 1).getData());
    }
  }
...
void C4PlayerInfoListAttributeConflictResolver::ResolveInPacket() {
  ...
  while (pResolveInfo = pResolvePacket->GetPlayerInfo(iCheck++)) {
    ...
    StdStrBuf sPrevForcedName;
    sPrevForcedName.Copy(pResolveInfo->GetForcedName());
    // check attributes: Name and color
    for (eAttr = C4PlayerInfo::PLRATT_Color; eAttr != C4PlayerInfo::PLRATT_Last;
         eAttr = (C4PlayerInfo::Attribute)(eAttr + 1)) {
      if (eAttr == C4PlayerInfo::PLRATT_Color) {
        ...
        if (Game.Teams.IsTeamColors() &&
            Game.Teams.GetTeamByID(pResolveInfo->GetTeam()))
          continue;
      } else if (eAttr == C4PlayerInfo::PLRATT_Name) {
        // no name change if a league name is used
        if (pResolveInfo->getLeagueAccount() &&
            *pResolveInfo->getLeagueAccount())
          continue;
      }
    ...
    if (pResolveInfo->GetColor() != dwPrevColor ||
        (pResolveInfo->GetForcedName() != sPrevForcedName))
      fAnyChange = true;
