...
class C4RoundResultsPlayer {
 private:
  ...
  // player icon
  // not compiled, so it will be lost for eliminated players after savegame
  // resume,
  ...
  // league data
  int32_t iLeagueScoreNew;  // score on league server after this round - -1 for
                            // unknown
  int32_t
      iLeagueScoreGain;    // league score gained by this round - -1 for unknown
  int32_t iLeagueRankNew;  // rank on league server after this round
  int32_t
      iLeagueRankSymbolNew;    // rank symbol on league server after this round
  int32_t iLeaguePerformance;  // script-set performance value, effect
                               // league-dependent
  StdCopyStrBuf sLeagueProgressData;  // scenario-specific data to store more
                                      // proigress info (which levels were done,
                                      // etc.)
 ...
 public:
  C4RoundResultsPlayer()
      : id(0),
        ...
        iLeagueScoreGain(0),
        iLeagueRankNew(0),
        iLeagueRankSymbolNew(0),
        iLeaguePerformance(0),
        sLeagueProgressData(),
        eLeagueStatus(RRPLS_Unknown) {}
  ...
  const char *GetCustomEvaluationStrings() {
    return sCustomEvaluationStrings.getData();
  }
  int32_t GetLeagueScoreNew() const {
    return iLeagueScoreNew;
  }  // returns score number on league server after round evaluation (0 for not
     // assigned)
  ...
  int32_t GetLeagueRankNew() const {
    return iLeagueRankNew;
  }  // returns rank on league server  after round evaluation (0 for not
     // assigned)
  ...
  void EvaluateLeague(C4RoundResultsPlayer *pLeaguePlayer);  // called from
                                                             // league
                                                             // evaluation; set
                                                             // league fields
  void EvaluatePlayer(
      C4Player *pPlr);  // called from C4Player::Evaluate; set fields by player
...
class C4RoundResultsPlayers {
 ...
 public:
  C4RoundResultsPlayers()
      : ppPlayers(NULL), iPlayerCount(0), iPlayerCapacity(0) {}
...
class C4RoundResults {
 ...
 private:
  ...
  bool fHideSettlementScore;   // to hide the score in the evaluation dialogue
                               // (for melees)
 ...
 public:
  ...
  static void EvaluateGoals(C4IDList &GoalList, C4IDList &FulfilledGoalList,
                            int32_t iPlayerNumber);

  // Evaluation called by C4Game::Evaluate
  // Caution: This does script callbacks for goal fulfillment check and must be
  // called in sync,
  //  i.e. even for dedicated server
  void EvaluateGame();

  // Evaluation called by league: Sets new league scores and ranks
  void EvaluateLeague(const char *szResultMsg, bool fSuccess,
  ...
  // Used for special league scenarios, e.g. settlement scenarios that wish to
  // use a
...
class C4PacketLeagueRoundResults : public C4PacketBase {
 public:
  ...
  C4PacketLeagueRoundResults(const char *szResultsString, bool fSuccess,
      ...
      : Players(Players),
        sResultsString(szResultsString),
        fSuccess(fSuccess) {}  // ctor
  C4PacketLeagueRoundResults(const char *szResultsString, bool fSuccess)
      : sResultsString(szResultsString), fSuccess(fSuccess) {}  // ctor
