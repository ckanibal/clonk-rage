...
bool C4GameSave::SaveCore() {
  ...
  rC4S.Head.C4XVer[0] = C4XVER1;
  rC4S.Head.C4XVer[1] = C4XVER2;
  rC4S.Head.C4XVer[2] = C4XVER3;
  rC4S.Head.C4XVer[3] = C4XVER4;
  // Some flags are not to be set for initial settings:
  //  They depend on whether specific runtime data is present, which may simply
  //  not be stored into initial
  //  saves, because they rely on any data present and up-to-date within the
  //  scenario!
  if (!fInitial) {
    // NoInitialize: Marks whether object data is contained and not to be
    // created from core
    rC4S.Head.NoInitialize = TRUE;
    // the SaveGame-value, despite it's name, marks whether exact runtime data
    // is contained
  ...
  // Title in language game was started in (not: save scenarios and net
  // references)
  if (!GetKeepTitle())
    SCopy(Game.ScenarioTitle.getData(), rC4S.Head.Title, C4MaxTitle);
  ...
  if (IsExact()) {
    ...
    rC4S.Definitions.SetModules(Game.DefinitionFilenames,
                                Config.General.ExePath,
                                Config.General.DefinitionPath);
  ...
  *rC4S.Head.MissionAccess = 0;
  ...
  if (!fInitial) rC4S.Head.EnableUnregisteredAccess = FALSE;
  // OldGfx is no longer supported
  // checks for IsExact() || ExactLandscape wouldn't catch scenarios using more
  // than 23 materials, so let's make it easy
...
bool C4GameSave::SaveScenarioSections() {
  ...
  for (C4ScenarioSection *pSect = Game.pScenarioSections; pSect;
       pSect = pSect->pNext) {
    // compose section filename
    SCopy(C4CFN_ScenarioSections, fn);
    SDelete(fn, 1, iWildcardPos);
    SInsert(fn, pSect->szName, iWildcardPos);
    // do not save self, because that is implied in CurrentScenarioSection and
    // the main landscape/object data
...
bool C4GameSave::SaveRuntimeData() {
  ...
  if (IsExact())
    if (!SaveScenarioSections()) {
      Log(LoadResStr("IDS_ERR_SAVE_SCENSECTIONS"));
      return false;
    }
  // landscape
  if (!SaveLandscape()) {
    Log(LoadResStr("IDS_ERR_SAVE_LANDSCAPE"));
    return false;
  }
  // Strings
  Game.ScriptEngine.Strings.EnumStrings();
  if (!Game.ScriptEngine.Strings.Save((*pSaveGroup))) {
    Log(LoadResStr("IDS_ERR_SAVE_SCRIPTSTRINGS"));
    return false;
  }
  // Objects
  if (!Game.Objects.Save((*pSaveGroup), IsExact(), true)) {
    Log(LoadResStr("IDS_ERR_SAVE_OBJECTS"));
    return false;
  }
  // Round results
  if (GetSaveUserPlayers())
    if (!Game.RoundResults.Save(*pSaveGroup)) {
      Log(LoadResStr("IDS_ERR_ERRORSAVINGROUNDRESULTS"));
      return false;
    }
  // Teams
  if (!Game.Teams.Save(*pSaveGroup)) {
    Log(LoadResStr(LoadResStr("IDS_ERR_ERRORSAVINGTEAMS")));
    return false;
  }
  ...
  // Script
  if (!Game.Script.Save((*pSaveGroup)))
    Log(LoadResStr("IDS_ERR_SAVE_SCRIPT")); /* nofail */
  // Title - unexact only, because in savegames, the title will be set in core
  if (!IsExact())
    if (!Game.Title.Save((*pSaveGroup)))
      Log(LoadResStr("IDS_ERR_SAVE_TITLE")); /* nofail */
  // Info
  if (!Game.Info.Save((*pSaveGroup)))
    Log(LoadResStr("IDS_ERR_SAVE_INFO")); /* nofail */
  if (GetSaveUserPlayers() || GetSaveScriptPlayers()) {
    // player infos
    // the stored player info filenames will point into the scenario file, and
    // no ressource information
    // will be saved. PlayerInfo must be saved first, because those will
    // generate the storage filenames to be used by
    // C4PlayerList
    C4PlayerInfoList RestoreInfos;
    RestoreInfos.SetAsRestoreInfos(
        Game.PlayerInfos, GetSaveUserPlayers(), GetSaveScriptPlayers(),
        GetSaveUserPlayerFiles(), GetSaveScriptPlayerFiles());
    if (!RestoreInfos.Save(*pSaveGroup, C4CFN_SavePlayerInfos)) {
      Log(LoadResStr("IDS_ERR_SAVE_RESTOREPLAYERINFOS"));
      return false;
    }
    // Players
    ...
    if (GetSaveUserPlayerFiles() || GetSaveScriptPlayerFiles()) {
      if (!Game.Players.Save((*pSaveGroup), GetCreateSmallFile(),
                             RestoreInfos)) {
        Log(LoadResStr("IDS_ERR_SAVE_PLAYERS"));
        return false;
      }
    }
  } else {
...
bool C4GameSave::SaveDesc(C4Group &hToGroup) {
  ...
  sBuffer.AppendFormat(
      ...
      "{\\f0\\fnil\\fcharset%d Times New Roman;}}",
      GetCharsetCode(Config.General.LanguageCharset));
  sBuffer.Append(LineFeed);

  // Scenario title
  sBuffer.AppendFormat("\\uc1\\pard\\ulnone\\b\\f0\\fs20 %s\\par",
...
void C4GameSave::WriteDescDate(StdStrBuf &sBuf, bool fRecord) {
  ...
  sBuf.AppendFormat(
      LoadResStr(fRecord ? "IDS_DESC_DATEREC"
                         : (Game.Network.isEnabled() ? "IDS_DESC_DATENET"
                                                     : "IDS_DESC_DATE")),
...
void C4GameSave::WriteDescGameTime(StdStrBuf &sBuf) {
  // Write game duration
  if (Game.Time) {
    sBuf.AppendFormat(LoadResStr("IDS_DESC_DURATION"), Game.Time / 3600,
...
void C4GameSave::WriteDescEngine(StdStrBuf &sBuf) {
  char ver[5];
  sprintf(ver, "%03d", (int)C4XVERBUILD);
  sBuf.AppendFormat(LoadResStr("IDS_DESC_VERSION"), ver);
  WriteDescLineFeed(sBuf);
}

void C4GameSave::WriteDescLeague(StdStrBuf &sBuf, bool fLeague,
...
void C4GameSave::WriteDescPlayers(StdStrBuf &sBuf, bool fByTeam,
                                  int32_t idTeam) {
  // write out all players; only if they match the given team if specified
  C4PlayerInfo *pPlr;
  bool fAnyPlrWritten = false;
...
void C4GameSave::WriteDescPlayers(StdStrBuf &sBuf) {
  ...
  if (Game.PlayerInfos.GetPlayerCount()) {
    ...
    if (Game.Teams.IsMultiTeams() && !Game.Teams.IsAutoGenerateTeams()) {
      ...
      // Finally, print out players outside known teams (those can only be
      // achieved by script using SetPlayerTeam)
...
bool C4GameSave::Save(const char *szFilename) {
  ...
  if (!SaveCreateGroup(szFilename, *pLSaveGroup)) {
    LogF(LoadResStr("IDS_ERR_SAVE_TARGETGRP"),
...
bool C4GameSave::Save(C4Group &hToGroup, bool fKeepGroup) {
  ...
  // set group
  pSaveGroup = &hToGroup;
  fOwnGroup = fKeepGroup;
  // PreSave-actions (virtual call)
  if (!OnSaving()) return false;
  // always save core
  if (!SaveCore()) {
    Log(LoadResStr("IDS_ERR_SAVE_CORE"));
    return false;
  }
  // cleanup group
  ...
  if (!GetKeepTitle()) {
    ...
    pSaveGroup->Delete(OSTR);
    pSaveGroup->Delete(C4CFN_Titles);
    pSaveGroup->Delete(C4CFN_Info);
  }
  // Always save Game.txt; even for saved scenarios, because global effects need
  // to be saved
  if (!Game.SaveData(*pSaveGroup, false, fInitial, IsExact())) {
    Log(LoadResStr("IDS_ERR_SAVE_RUNTIMEDATA"));
    return false;
  }
  // save additional runtime data
  if (GetSaveRuntimeData())
    if (!SaveRuntimeData()) return false;
  // Desc
  if (GetSaveDesc())
...
void C4GameSaveSavegame::AdjustCore(C4Scenario &rC4S) {
  ...
  int iSaveGameIndex =
      GetTrailingNumber(GetFilenameOnly(pSaveGroup->GetFullName().getData()));
  // Looks like a decent index: set numbered icon
  if (Inside(iSaveGameIndex, 1, 10)) rC4S.Head.Icon = 2 + (iSaveGameIndex - 1);
...
void C4GameSaveRecord::AdjustCore(C4Scenario &rC4S) {
  ...
  char buf[1024 + 1];
  sprintf(buf, "%03i %s [%d]", iNum, Game.ScenarioTitle.getData(),
          (int)C4XVERBUILD);
