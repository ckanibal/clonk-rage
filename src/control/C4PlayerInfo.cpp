...
bool C4PlayerInfo::LoadFromLocalFile(const char *szFilename) {
  ...
  // network: ressource (not for replays, because everyone has the player files
  // there...)
  if (Game.Network.isEnabled() && !Game.C4S.Head.Replay) {
    // add ressource
    // 2do: rejoining players need to update their ressource version when saving
    // the player
    // otherwise, player file versions may differ
    pRes = Game.Network.ResList.getRefRes(Config.AtExeRelativePath(szFilename),
                                          true);
    // not found? add
    if (!pRes)
      pRes = Game.Network.ResList.AddByFile(
          Config.AtExeRelativePath(szFilename), false, NRT_Player);
    if (!pRes)
      return false;
    // set core and flag
    ResCore = pRes->getCore();
    dwFlags |= PIF_HasRes;
    // filename is no longer needed in network mode, because it's stored in the
    // res-core
  }
  // done, success
  return true;
...
bool C4PlayerInfo::SetAsScriptPlayer(const char *szName, uint32_t dwColor,
                                     uint32_t dwFlags, C4ID idExtra) {
  // clear previous
  Clear();
  // set parameters
  eType = C4PT_Script;
...
uint32_t C4PlayerInfo::GetLobbyColor() const {
  ...
  // Unjoined players do not show their team! Instead, they just display their
  // original color
...
StdStrBuf C4PlayerInfo::GetLobbyName() const {
  ...
  if (sLeagueAccount.getLength()) {
    if (sClanTag.getLength()) {
      ...
      sResult.Format("<c afafaf>%s</c> %s", sClanTag.getData(),
                     sLeagueAccount.getData());
    } else
      sResult.Ref(sLeagueAccount);
  } else {
    // fallback to regular player name
    sResult.Ref(sForcedName.getLength()
                    ? static_cast<const StdStrBuf &>(sForcedName)
                    : static_cast<const StdStrBuf &>(sName));
  }
  return sResult;
}
...
void C4PlayerInfo::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(
      mkNamingAdapt(mkBitfieldAdapt(dwSyncFlags, Entries), "Flags", 0u));
  if (pComp->isCompiler())
    dwFlags = dwSyncFlags;
  ...
  pComp->Value(mkNamingAdapt(mkEnumAdaptT<uint8_t>(eType, PlayerTypes), "Type",
                             C4PT_User));

  // safety: Do not allow invisible regular players
  if (pComp->isCompiler()) {
    if (eType != C4PT_Script)
      dwFlags &= ~PIF_Invisible;
  }
  ...
  // load savegame ID
  pComp->Value(
      mkNamingAdapt(mkIntPackAdapt(idSavegamePlayer), "SavgamePlayer", 0));
  // load team ID
  ...
  pComp->Value(
      mkNamingAdapt(mkIntPackAdapt(iLeagueRankSymbol), "LeagueRankSymbol", 0));
  pComp->Value(
      mkNamingAdapt(mkIntPackAdapt(iLeagueProjectedGain), "ProjectedGain", -1));
  pComp->Value(
      mkNamingAdapt(mkParAdapt(sClanTag, StdCompiler::RCT_All), "ClanTag", ""));
  pComp->Value(mkNamingAdapt(mkIntPackAdapt(iLeaguePerformance),
                             "LeaguePerformance", 0));
...
void C4PlayerInfo::LoadResource() {
  ...
  if (IsRemoved() || !(dwFlags & PIF_HasRes) || pRes)
    return;
  // Ignore res if a local file is to be used
  // the PIF_InScenarioFile is not set for startup players in initial replays,
  // because ressources are used for player joins but emulated in playback
  // control
  // if there will ever be ressources in replay mode, this special case can be
  // removed
  if (Game.C4S.Head.Replay || (dwFlags & PIF_InScenarioFile))
    ...
    LogF("Error: Could not add resource %d for player %s! Player file too "
         "large to load?",
         (int)ResCore.getID(), (const char *)GetFilename());
  }
}
...
bool C4PlayerInfo::SetSavegameResume(C4PlayerInfo *pSavegameInfo) {
  // copy some data fields; but not the file fields, because the join method is
  // determined by this player
  if (!pSavegameInfo)
    return false;
  iID = pSavegameInfo->GetID();
  dwFlags = (dwFlags & ~PIF_SavegameTakeoverFlags) |
            (pSavegameInfo->GetFlags() & PIF_SavegameTakeoverFlags);
  dwColor =
      pSavegameInfo->GetColor(); // redundant; should be done by host already
...
bool C4PlayerInfo::LoadBigIcon(C4FacetExSurface &fctTarget) {
  bool fSuccess = false;
  // load BigIcon.png of player into target facet; return false if no bigicon
  // present or player file not yet loaded
  C4Group Plr;
...
C4ClientPlayerInfos::C4ClientPlayerInfos(const char *szJoinFilenames, bool fAdd,
                                         C4PlayerInfo *pAddInfo)
    : iPlayerCount(0), iClientID(-1), iPlayerCapacity(0), ppPlayers(NULL),
      dwFlags(0) {
  ...
  if (szJoinFilenames || pAddInfo) {
    ...
    if (!Game.Control.isNetwork() && iClientID < 0)
      iClientID = 0;
    ...
    if (iPlayerCapacity =
            (szJoinFilenames ? SModuleCount(szJoinFilenames) : 0) +
            !!pAddInfo) {
...
C4ClientPlayerInfos::C4ClientPlayerInfos(const C4ClientPlayerInfos &rCopy) {
  ...
  if (iPlayerCount = rCopy.iPlayerCount) {
    ...
    while (i--)
      *ppCurrPlrInfo++ = new C4PlayerInfo(**ppSrcPlrInfo++);
  }
  // no players
  else {
    ppPlayers = NULL;
...
operator=(const C4ClientPlayerInfos &rCopy) {
  Clear();
  // copy fields
  iClientID = rCopy.iClientID;
  if (iPlayerCount = rCopy.iPlayerCount) {
...
void C4ClientPlayerInfos::GrabMergeFrom(C4ClientPlayerInfos &rFrom) {
  ...
  if (iPlayerCount) {
    ...
    memcpy(ppPlayers + iPlayerCount, rFrom.ppPlayers,
           rFrom.iPlayerCount * sizeof(C4PlayerInfo *));
    iPlayerCount += rFrom.iPlayerCount;
    rFrom.iPlayerCount = rFrom.iPlayerCapacity = 0;
    delete[] rFrom.ppPlayers;
    rFrom.ppPlayers = NULL;
  } else {
    // no own players: take over buffer of pFrom
    if (ppPlayers)
      delete[] ppPlayers;
    ppPlayers = rFrom.ppPlayers;
    rFrom.ppPlayers = NULL;
    iPlayerCount = rFrom.iPlayerCount;
    rFrom.iPlayerCount = 0;
    iPlayerCapacity = rFrom.iPlayerCapacity;
    rFrom.iPlayerCapacity = 0;
  }
}
...
void C4ClientPlayerInfos::RemoveIndexedInfo(int32_t iAtIndex) {
  // bounds check
  if (iAtIndex < 0 || iAtIndex >= iPlayerCount)
    return;
...
void C4ClientPlayerInfos::GrowList(size_t iByVal) {
  // create new list (out of mem: simply returns here; info list remains in a
  // valid state)
...
C4PlayerInfo *C4ClientPlayerInfos::GetPlayerInfo(int32_t iIndex) const {
  // check range
  if (iIndex < 0 || iIndex >= iPlayerCount)
    return NULL;
  // return indexed info
...
void C4ClientPlayerInfos::CompileFunc(StdCompiler *pComp) {
  ...
  // Flags
  StdBitfieldEntry<uint32_t> Entries[] = {{"AddPlayers", CIF_AddPlayers},
                                          {"Updated", CIF_Updated},
                                          {"Initial", CIF_Initial},
  ...
  if (iPlayerCount < 0 || iPlayerCount > C4MaxPlayer) {
    pComp->excCorrupt("player count out of range");
    return;
  }
  // Grow list, if necessary
  ...
  pComp->Value(mkNamingAdapt(
      mkArrayAdaptMap(ppPlayers, iPlayerCount, mkPtrAdaptNoNull<C4PlayerInfo>),
      "Player"));
...
void C4ClientPlayerInfos::LoadResources() {
  ...
  C4PlayerInfo **ppCurrPlrInfo = ppPlayers;
  while (i--)
    (*ppCurrPlrInfo++)->LoadResource();
}

// *** C4PlayerInfoList

C4PlayerInfoList::C4PlayerInfoList()
    : iClientCount(0), iClientCapacity(0), ppClients(NULL), iLastPlayerID(0) {
  // ctor: no need to alloc mem yet
}

C4PlayerInfoList::C4PlayerInfoList(const C4PlayerInfoList &rCpy)
    : iClientCount(rCpy.iClientCount), iClientCapacity(rCpy.iClientCapacity),
  ...
  if (rCpy.ppClients) {
    ...
    while (i--)
      *ppInfo++ = new C4ClientPlayerInfos(**ppCpy++);
  }
}
...
void C4PlayerInfoList::Clear() {
  // delete client infos
  C4ClientPlayerInfos **ppInfo = ppClients;
  int32_t i = iClientCount;
  while (i--)
    delete *ppInfo++;
  // clear client infos
...
void C4PlayerInfoList::GrowList(size_t iByVal) {
  // create new list (out of mem: simply returns here; info list remains in a
  // valid state)
  C4ClientPlayerInfos **ppNewInfo =
      new C4ClientPlayerInfos *[iClientCapacity += iByVal];
  // move existing
  if (ppClients) {
...
bool C4PlayerInfoList::DoLocalNonNetworkPlayerInfoUpdate(
    C4ClientPlayerInfos *pUpdate) {
  ...
  // add through queue: This will add directly, do the record and put player
  // joins into the queue
  // in running mode, this call will also put the actual player joins into the
  // queue
  Game.Control.DoInput(CID_PlrInfo, new C4ControlPlayerInfo(*pUpdate),
                       Game.IsRunning ? CDT_Queue : CDT_Direct);
  // done, success
  return true;
}
...
void C4PlayerInfoList::UpdatePlayerAttributes(C4ClientPlayerInfos *pForInfo,
  ...
  C4PlayerInfo *pInfo, *pInfo2;
  int32_t i = 0;
  while (pInfo = pForInfo->GetPlayerInfo(i++))
    if (!pInfo->HasJoined()) {
      // assign savegame colors
      int32_t idSavegameID;
      bool fHasForcedColor = false;
      DWORD dwForceClr;
  ...
  if (fResolveConflicts)
    ResolvePlayerAttributeConflicts(pForInfo);
}
...
void C4PlayerInfoList::UpdatePlayerAttributes() {
  ...
  while (pForInfo = GetIndexedInfo(iIdx++))
    UpdatePlayerAttributes(pForInfo, false);
  ...
  while (pPlrInfo = pNewClientInfo->GetPlayerInfo(i++))
    if (!pPlrInfo->GetID()) {
      ...
      if (GetFreePlayerSlotCount() - iJoinsGranted < 1) {
        ...
        Log(FormatString(LoadResStr("IDS_MSG_TOOMANYPLAYERS"),
                         (int)Game.Parameters.MaxPlayers).getData());
...
void C4PlayerInfoList::RecheckAutoGeneratedTeams() {
  ...
  while (pPlrInfos = GetIndexedInfo(j++)) {
    ...
    while (pPlrInfo = pPlrInfos->GetPlayerInfo(i++)) {
      ...
      if (idTeam)
        Game.Teams.GetGenerateTeamByID(idTeam);
    }
  }
}

C4ClientPlayerInfos *
C4PlayerInfoList::AddInfo(C4ClientPlayerInfos *pNewClientInfo) {
  ...
  // ensure all teams specified in the list exist (this should be done for
  // savegame teams as well)
  C4PlayerInfo *pInfo;
  int32_t i = 0;
  ...
  C4ClientPlayerInfos **ppExistingInfo =
      GetInfoPtrByClientID(pNewClientInfo->GetClientID());
  if (ppExistingInfo) {
  ...
  if (iClientCount >= iClientCapacity)
    GrowList(4);
...
C4ClientPlayerInfos **
C4PlayerInfoList::GetInfoPtrByClientID(int32_t iClientID) const {
  // search list
...
int32_t
C4PlayerInfoList::GetActiveScriptPlayerCount(bool fCountSavegameResumes,
                                             bool fCountInvisible) const {
  // count players of all clients
  int32_t iCount = 0;
  for (int32_t i = 0; i < iClientCount; ++i) {
    C4ClientPlayerInfos *pClient = ppClients[i];
...
StdStrBuf C4PlayerInfoList::GetActivePlayerNames(bool fCountInvisible,
                                                 int32_t iAtClientID) const {
  // add up players of all clients
  StdStrBuf sPlr;
  int32_t iCount = 0;
  for (int32_t i = 0; i < iClientCount; ++i) {
    C4ClientPlayerInfos *pClient = ppClients[i];
    if (iAtClientID != -1 && pClient->GetClientID() != iAtClientID)
      continue;
    for (int32_t j = 0; j < pClient->GetPlayerCount(); ++j) {
      C4PlayerInfo *pInfo = pClient->GetPlayerInfo(j);
      if (!pInfo->IsRemoved())
...
C4ClientPlayerInfos *
C4PlayerInfoList::GetClientInfoByPlayerID(int32_t id) const {
...
C4PlayerInfo *C4PlayerInfoList::GetPlayerInfoBySavegameID(int32_t id) const {
  ...
  for (int32_t i = 0; i < iClientCount; ++i) {
    ...
    C4PlayerInfo *pInfo;
    while (pInfo = ppClients[i]->GetPlayerInfo(j++))
      if (pInfo->GetAssociatedSavegamePlayerID() == id)
        return pInfo;
  }
  // nothing found
  return NULL;
...
C4PlayerInfo *
C4PlayerInfoList::FindSavegameResumePlayerInfo(const C4PlayerInfo *pMatchInfo,
                                               MatchingLevel mlMatchStart,
                                               MatchingLevel mlMatchEnd) const {
  assert(pMatchInfo);
  ...
  for (int iMatchLvl = mlMatchStart; iMatchLvl <= mlMatchEnd; ++iMatchLvl) {
    for (int32_t i = 0; i < iClientCount; ++i) {
      ...
      C4PlayerInfo *pInfo;
      while (pInfo = ppClients[i]->GetPlayerInfo(j++))
        if (!Game.PlayerInfos.GetPlayerInfoByID(pInfo->GetID()) &&
            !Game.PlayerInfos.GetPlayerInfoBySavegameID(
                pInfo->GetID())) // only unassigned player infos
          switch (iMatchLvl) {
          case PML_PlrFileName: // file name and player name must match
            if (!pMatchInfo->GetFilename() || !pInfo->GetFilename())
              break;
            if (!SEqualNoCase(GetFilename(pMatchInfo->GetFilename()),
                              GetFilename(pInfo->GetFilename())))
              break;
...
C4PlayerInfo *C4PlayerInfoList::FindUnassociatedRestoreInfo(
    const C4PlayerInfoList &rRestoreInfoList) {
  // search given list for a player that's not associated locally
  C4ClientPlayerInfos *pRestoreClient;
  int32_t iClient = 0;
  while (pRestoreClient = rRestoreInfoList.GetIndexedInfo(iClient++)) {
    C4PlayerInfo *pRestoreInfo;
    int32_t iInfo = 0;
    while (pRestoreInfo = pRestoreClient->GetPlayerInfo(iInfo++))
      if (pRestoreInfo->IsJoined())
        // match association either by savegame ID (before C4Game::InitPlayers)
        // or real ID (after C4Game::InitPlayers)
        if (!GetPlayerInfoBySavegameID(pRestoreInfo->GetID()) &&
            !GetPlayerInfoByID(pRestoreInfo->GetID()))
          return pRestoreInfo;
  }
...
bool C4PlayerInfoList::HasSameTeamPlayers(int32_t iClient1,
  ...
  if (!pCnfo1 || !pCnfo2)
    return false;
  int32_t i = 0, j;
  const C4PlayerInfo *pNfo1, *pNfo2;
  while (pNfo1 = pCnfo1->GetPlayerInfo(i++)) {
    if (!pNfo1->IsUsingTeam())
      continue;
    j = 0;
    while (pNfo2 = pCnfo2->GetPlayerInfo(j++)) {
      if (!pNfo2->IsUsingTeam())
        continue;
...
bool C4PlayerInfoList::Load(C4Group &hGroup, const char *szFromFile,
  ...
  // replace strings
  if (pLang)
    pLang->ReplaceStrings(Buf);
  // (try to) compile
  if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(
          mkNamingAdapt(*this, "PlayerInfoList"), Buf, szFromFile))
    return false;
  // done, success
  return true;
}
...
bool C4PlayerInfoList::LoadFromGameText(const char *pSource) {
  ...
  // hacking some data out of the game text. Luckily, the format is different
  // nowadays.
  const char *szPos;
    ...
    while (TRUE) {
      ...
      if (SEqual2(szLinebuf, "Player") && (SCharPos('=', szLinebuf) > -1)) {
        ...
        if (!pNewInfo->LoadFromLocalFile(szPlrInSzenName)) {
          ...
          if (!pkPlrInfo)
            pkPlrInfo = new C4ClientPlayerInfos();
          // determine player number
          int iJoinedNumber = C4P_Number_None;
          // store "Player1"
          StdStrBuf sPlrIndex;
          sPlrIndex.Copy(szLinebuf, SCharPos('=', szLinebuf));
          // search for section "[Player1]" in game text
          StdStrBuf sPlrSect;
          sPlrSect.Format("[%s]", sPlrIndex.getData());
          const char *szPlrSect = SSearch(pSource, sPlrSect.getData());
          // get "Index=%d" from that section
          if (szPlrSect && (szPlrSect = SSearch(szPlrSect, "Index=")))
            sscanf(szPlrSect, "%d", &iJoinedNumber);
  ...
  // Workaround: Because MaxPlayers may not be loaded yet, make sure it's large
  // enough to allow all currently joined players
  if (Game.Parameters.MaxPlayers < iJoinedPlrCount)
    Game.Parameters.MaxPlayers = iJoinedPlrCount;
  // add it
  AddInfo(pkPlrInfo);
  // success
  return true;
...
bool C4PlayerInfoList::InitLocal() {
  ...
  C4ClientPlayerInfos *pLocalInfo =
      new C4ClientPlayerInfos(Game.PlayerFilenames);
  // register local info immediately
  pLocalInfo = AddInfo(pLocalInfo);
  // Script players in restore infos need to be associated with matching script
  // players in main info list
  ...
  while (pInfo = pkLocal->GetPlayerInfo(i++))
    ...
    if (!pInfo->HasJoinIssued()) {
      ...
      Game.Input.Add(CID_JoinPlr,
                     new C4ControlJoinPlayer(
                         szFilename, Game.Control.ClientID(), pInfo->GetID()));
    }
  // done, success
  return true;
}

void C4PlayerInfoList::CreateRestoreInfosForJoinedScriptPlayers(
    C4PlayerInfoList &rSavegamePlayers) {
    ...
    while (pInfo = pkInfo->GetPlayerInfo(j++))
      if (pInfo->GetType() == C4PT_Script) {
        ...
        if (pRejoinInfo = GetPlayerInfoBySavegameID(pInfo->GetID())) {
          LogF("Warning: User player %s takes over script player %s!",
               pRejoinInfo->GetName(), pInfo->GetName());
          continue;
        }
        if (!pHostInfo) {
          LogF("Error restoring savegame script players: No host player infos "
               "to add to!");
          continue;
        }
...
bool
C4PlayerInfoList::RestoreSavegameInfos(C4PlayerInfoList &rSavegamePlayers) {
  ...
  if (rSavegamePlayers.GetPlayerCount()) {
    ...
    if (!Game.Network.isEnabled() && Game.C4S.Head.SaveGame) {
      // do several passes: First passes using regular player matching;
      // following passes matching anything but with a warning message
      for (int eMatchingLevel = 0; eMatchingLevel <= PML_Any; ++eMatchingLevel)
        for (int32_t i = 0; i < iClientCount; ++i) {
          C4ClientPlayerInfos *pkInfo = GetIndexedInfo(i);
          int32_t j = 0, id;
          C4PlayerInfo *pInfo, *pSavegameInfo;
          while (pInfo = pkInfo->GetPlayerInfo(j++))
            if (!(id = pInfo->GetAssociatedSavegamePlayerID()))
              if (pSavegameInfo = rSavegamePlayers.FindSavegameResumePlayerInfo(
                      pInfo, (MatchingLevel)eMatchingLevel,
                      (MatchingLevel)eMatchingLevel)) {
                pInfo->SetAssociatedSavegamePlayer(pSavegameInfo->GetID());
                if (eMatchingLevel > PML_PlrName) {
                  // this is a "wild" match: Warn the player (but not in
                  // replays)
                  StdStrBuf sMsg;
                  sMsg.Format(LoadResStr("IDS_MSG_PLAYERASSIGNMENT"),
                              pInfo->GetName(), pSavegameInfo->GetName());
                  Log(sMsg.getData());
                  if (Game.pGUI && FullScreen.Active && !Game.C4S.Head.Replay)
                    Game.pGUI->ShowMessageModal(
                        sMsg.getData(), LoadResStr("IDS_MSG_FREESAVEGAMEPLRS"),
                        C4GUI::MessageDialog::btnOK, C4GUI::Ico_Notify,
                        &Config.Startup.HideMsgPlrTakeOver);
                }
              }
        }
    }
    ...
    for (i = 0; i < iClientCount; ++i) {
      ...
      while (pInfo = pkInfo->GetPlayerInfo(j++))
        if (id = pInfo->GetAssociatedSavegamePlayerID()) {
          ...
          // in savegames, this is unusual. For regular script player restore,
          // it's not
          if (Game.C4S.Head.SaveGame)
            LogF(LoadResStr("IDS_PRC_RESUMENOPLRASSOCIATION"),
                 (const char *)pInfo->GetName());
        }
    }
    // otherwise any remaining players
    int32_t iCountRemaining = rSavegamePlayers.GetPlayerCount() - iNumGrabbed;
    if (iCountRemaining) {
      // in replay mode, if there are no regular player joins, it must have been
      // a runtime record
      // i.e., a record that was started during the game
      // in this case, the savegame player infos equal the real player infos to
      // be used
...
bool C4PlayerInfoList::RecreatePlayerFiles() {
  // Note that this method will be called on the main list for savegame resumes
  // (even in network) or regular games with RecreateInfos,
  ...
  for (int32_t i = 0; i < iClientCount; ++i) {
    ...
    while (pInfo = pkInfo->GetPlayerInfo(j++))
      if (pInfo->IsJoined()) {
        // all players in replays and runtime joins; script players even in
        // savegames need to be restored from the scenario goup
        if (Game.C4S.Head.Replay || Game.C4S.Head.NetworkRuntimeJoin ||
            pInfo->GetType() == C4PT_Script) {
          ...
          if (Game.C4S.Head.Replay) {
            // replay of resumed savegame: RecreatePlayers saves used player
            // files into the record group in this manner
          ...
          if (!szCurrPlrFile || !*szCurrPlrFile) {
            // that's okay for script players, because those may join w/o
            // recreation files
            if (pInfo->GetType() != C4PT_Script) {
          ...
          if (!Game.ScenarioFile.FindEntry(GetFilename(szCurrPlrFile)) ||
              !Game.ScenarioFile.Extract(GetFilename(szCurrPlrFile),
                                         szJoinPath.getData())) {
            // that's okay for script players, because those may join w/o
            // recreation files
            if (pInfo->GetType() != C4PT_Script) {
              LogF(LoadResStr("IDS_ERR_LOAD_RECR_NOEXTRACT"), szPlrName,
                   GetFilename(szCurrPlrFile));
            }
            continue;
          }
          // set join source
          ...
          // setting a temp file here will cause the player file to be deleted
          // directly after recreation
          // if recreation fails (e.g. the game gets aborted due to invalid
          // files), the info dtor will delete the file
          pInfo->SetTempFile();
        } else {
...
bool C4PlayerInfoList::RecreatePlayers() {
  ...
  for (int32_t i = 0; i < iClientCount; ++i) {
    ...
    if (!pkInfo->GetJoinedPlayerCount())
      continue;
    ...
    if (Game.C4S.Head.Replay)
      ...
      if (pClient)
        ...
        LogF(LoadResStr("IDS_PRC_RESUMENOCLIENT"), idAtClient,
             pkInfo->GetPlayerCount());
        continue;
      }
    }
    ...
    while (pInfo = pkInfo->GetPlayerInfo(j++))
      if (pInfo->IsJoined()) {
        ...
        if (szFilename && pJoinRes && pJoinRes->isLoading()) {
          ...
          if (!Game.Network.RetrieveRes(
                  pJoinRes->getCore(), C4NetResRetrieveTimeout,
                  FormatString(LoadResStr("IDS_NET_RES_PLRFILE"), szName)
                      .getData()))
            szFilename = NULL;
        }
        // file present?
        if (!szFilename || !*szFilename) {
          if (pInfo->GetType() == C4PT_User) {
            // for user players, this could happen only if the user cancelled
            // the ressource
            const char *szPlrName = pInfo->GetName();
            if (!szPlrName)
              szPlrName = "???";
        ...
        // record file handling: Save to the record file in the manner it's
        // expected by C4PlayerInfoList::RecreatePlayers
...
bool C4PlayerInfoList::RemoveUnassociatedPlayers(
    C4PlayerInfoList &rSavegamePlayers) {
      ...
      if (pInfo->IsJoined()) {
        ...
        if (pInfo->GetType() == C4PT_User) {
          ...
          if (fSetUserPlrRefToLocalGroup) {
            ...
            StdStrBuf sNewName;
            if (Game.Network.isEnabled()) {
              C4Client *pGameClient =
                  Game.Clients.getClientByID(pClient->GetClientID());
              const char *szName =
                  pGameClient ? pGameClient->getName() : "Unknown";
              sNewName.Format(
                  "%s-%s", szName,
                  (const char *)GetFilename(pInfo->GetLocalJoinFilename()));
            } else
          ...
          if (fSetScriptPlrRefToLocalGroup) {
            ...
            pInfo->SetFilename(FormatString("ScriptPlr-%d.c4p",
                                            (int)pInfo->GetID()).getData());
          }
        }
      }
      if (!fKeepInfo) {
...
void C4PlayerInfoList::CompileFunc(StdCompiler *pComp) {
  ...
  if (!fCompiler && pComp->hasNaming() && iLastPlayerID == 0 &&
      iClientCount == 0)
    return;
  // header
  ...
  if (iTemp < 0 || iTemp > C4MaxClient) {
    pComp->excCorrupt("client count out of range");
    return;
  }
  // grow list
  if (fCompiler) {
    if (iTemp > iClientCapacity)
      GrowList(iTemp - iClientCapacity);
    iClientCount = iTemp;
    ZeroMem(ppClients, sizeof(*ppClients) * iClientCount);
  }
  // client packets
  pComp->Value(
      mkNamingAdapt(mkArrayAdaptMap(ppClients, iClientCount,
                                    mkPtrAdaptNoNull<C4ClientPlayerInfos>),
                    "Client"));
...
void C4PlayerInfoList::LoadResources() {
  // load for all players
  int32_t i = iClientCount;
  C4ClientPlayerInfos **ppClient = ppClients;
  while (i--)
    (*ppClient++)->LoadResources();
}
