...
#ifdef DEBUGREC
void AddDbgRec(C4RecordChunkType eType, const void *pData = NULL,
               int iSize = 0);  // record debug stuff
#endif
...
class C4PktDebugRec : public C4PacketBase {
 ...
 public:
  ...
  C4PktDebugRec(const C4PktDebugRec &rCopy)
      : Data(rCopy.Data), eType(rCopy.eType) {}
...
class C4Record  // demo recording
 ...
 public:
  ...
  C4Record(
      const char *szPlaybackFile, const char *szRecordFile,
      const char *szTempRecFile);  // start recording from replay into record
  ...
  bool Rec(C4PacketType eCtrlType, C4ControlPacket *pCtrl,
           int iFrame);  // record control packet
  bool Rec(int iFrame, const StdBuf &sBuf, C4RecordChunkType eType);

  bool AddFile(const char *szLocalFilename, const char *szAddAs,
               bool fDelete = false);
...
class C4Playback  // demo playback
 ...
 private:
  ...
  CStdFile
      playbackFile;  // if open, try reading additional chunks from this file
 ...
 public:
  ...
  bool NextSequentialChunk();  // read from seq file until a new chunk has been
                               // filled
  ...
  void Clear();
#ifdef DEBUGREC
  void Check(C4RecordChunkType eType, const uint8_t *pData,
             int iSize);  // compare with debugrec
