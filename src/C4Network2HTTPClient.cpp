...
size_t C4Network2HTTPClient::UnpackPacket(const StdBuf &rInBuf,
  ...
  if (!iDataOffset) {
    ...
    StdStrBuf Data;
    Data.Copy(getBufPtr<char>(rInBuf), rInBuf.getSize());
...
bool C4Network2HTTPClient::ReadHeader(StdStrBuf Data) {
  ...
  if (sscanf(pData, "HTTP/%d.%d %d %n", &iHTTPVer1, &iHTTPVer2, &iResponseCode,
             &iStatusStringPtr) != 3) {
  ...
  if (iResponseCode != 200) {
    // Get status string
    StdStrBuf StatusString;
    StatusString.CopyUntil(pData + iStatusStringPtr, '\r');
    // Create error message
    Error.Format("HTTP server responded %d: %s", iResponseCode,
                 StatusString.getData());
    return false;
  }
  // Get content length
  ...
  if (pContentEncoding) {
    ...
    StdStrBuf Encoding;
    Encoding.CopyUntil(pContentEncoding, '\r');
    if (Encoding == "gzip")
      fCompressed = true;
    else
      fCompressed = false;
...
bool C4Network2HTTPClient::Decompress(StdBuf *pData) {
  ...
  // Create buffer
  uint32_t iOutSize =
      *getBufPtr<uint32_t>(*pData, pData->getSize() - sizeof(uint32_t));
...
bool C4Network2HTTPClient::OnConn(const C4NetIO::addr_t &AddrPeer,
  ...
  if (!AddrEqual(AddrConnect, ServerAddr)) return false;
...
void C4Network2HTTPClient::OnDisconn(const C4NetIO::addr_t &AddrPeer,
  ...
  fConnected = false;
  // Notify
  if (pNotify) pNotify->PushEvent(Ev_HTTP_Response, this);
}

void C4Network2HTTPClient::OnPacket(const class C4NetIOPacket &rPacket,
                                    C4NetIO *pNetIO) {
  // Everything worthwhile was already done in UnpackPacket. Only do notify
  // callback
  if (pNotify) pNotify->PushEvent(Ev_HTTP_Response, this);
}
...
int C4Network2HTTPClient::GetTimeout() {
  if (!fBusy) return C4NetIOTCP::GetTimeout();
  return MaxTimeout(C4NetIOTCP::GetTimeout(),
                    1000 * Max<int>(time(NULL) - iRequestTimeout, 0));
}

bool C4Network2HTTPClient::Query(const StdBuf &Data, bool fBinary) {
  if (Server.isNull()) return false;
  // Cancel previous request
  if (fBusy) Cancel("Cancelled");
  // No result known yet
  ResultString.Clear();
  // store mode
  ...
  if (Data.getSize())
    Header.Format(
        ...
        "\r\n"
        "\r\n",
        RequestPath.getData(), Server.getData(), Data.getSize(), szCharset,
        szCharset, Config.General.LanguageEx);
  else
    Header.Format(
        "GET %s HTTP/1.0\r\n"
        "Host: %s\r\n"
        ...
        "\r\n"
        "\r\n",
        RequestPath.getData(), Server.getData(), szCharset,
        Config.General.LanguageEx);
  // Compose query
  Request.Take(Header.GrabPointer(), Header.getLength());
  Request.Append(Data);
  // Start connecting
  if (!Connect(ServerAddr)) return false;
...
bool C4Network2HTTPClient::SetServer(const char *szServerAddress) {
  ...
  if (!ResolveAddress(Server.getData(), &ServerAddr, GetDefaultPort())) {
    SetError(FormatString("Could not resolve server address %s!",
                          Server.getData()).getData());
    return false;
  }
  // Remove port
  const char *pColon = strchr(Server.getData(), ':');
  if (pColon) Server.SetLength(pColon - Server.getData());
  // Done
  ResetError();
  return true;
}