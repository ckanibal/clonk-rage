...
void C4VideoFile::Clear() {
  if (sFilename.getLength()) {
    if (fIsTemp)
      if (!EraseFile(sFilename.getData())) {
        LogF("C4VideoFile::Clear: Error deleting temporary video file %s",
             (const char *)sFilename.getData());
      }
    sFilename.Clear();
  }
  fIsTemp = false;
...
bool C4VideoShowDialog::LoadVideo(C4VideoFile *pVideoFile) {
#ifdef _WIN32
  // load video file
  if (!AVIFile.OpenFile(pVideoFile->GetFilename(), FullScreen.hWindow,
                        lpDDraw->GetByteCnt() * 8))
    return false;
  ...
  if (!fctBuffer.Create(mpeg_info->width, mpeg_info->height, C4FCT_Full,
                        C4FCT_Full))
    return false;
  surface =
      SDL_AllocSurface(SDL_SWSURFACE, mpeg_info->width, mpeg_info->height, 32,
...
bool C4VideoPlayer::PreloadVideos(class C4Group &rFromGroup) {
  ...
  int32_t iNumLoaded = 0;
  bool fAnyLoaded = false;
  while (rFromGroup.FindNextEntry(C4CFN_AnimationFiles, szFilename)) {
    if (!fAnyLoaded) {
      // log message: Only if there are videos, because in 99.99% of the cases,
      // there are none
...
bool C4VideoPlayer::PlayVideo(const char *szVideoFilename) {
  // video speicifed by filename - uses preload database if possible; otherwise
  // gets video from root video collection
  ...
  if (!pVidFile) {
    ...
    if (SCharPos('/', szVideoFilename) >= 0 ||
        SCharPos('\\', szVideoFilename) >= 0) {
      // subfolders involved: UseC4Group to load the video
      char szParentPath[_MAX_PATH + 1];
      if (!GetParentPath(szVideoFilename, szParentPath)) {
        delete pVidFile;
        return false;
      }
      C4Group Grp;
      if (!Grp.Open(szParentPath)) {
        delete pVidFile;
        return false;
      }
      if (!pVidFile->Load(Grp, GetFilename(szVideoFilename))) {
        delete pVidFile;
        return false;
      }
    } else {
      // Direct filename in Clonk directory
      if (!pVidFile->Load(szVideoFilename)) {
        delete pVidFile;
        return false;
      }
    }
