...
C4ObjectInfoList::~C4ObjectInfoList() { Clear(); }
...
int32_t C4ObjectInfoList::Load(C4Group &hGroup, bool fLoadPortraits) {
  ...
  while (hGroup.FindNextEntry(C4CFN_ObjectInfoFiles, entryname))
    if (ninf = new C4ObjectInfo)
      if (ninf->Load(hGroup, entryname, fLoadPortraits)) {
        Add(ninf);
        infn++;
      } else
        delete ninf;
...
BOOL C4ObjectInfoList::NameExists(const char *szName) {
  ...
  for (cinf = First; cinf; cinf = cinf->Next)
    if (SEqualNoCase(szName, cinf->Name)) return TRUE;
  return FALSE;
}
...
void C4ObjectInfoList::Evaluate() {
  C4ObjectInfo *cinf;
  for (cinf = First; cinf; cinf = cinf->Next) cinf->Evaluate();
}

BOOL C4ObjectInfoList::Save(C4Group &hGroup, bool fSavegame, bool fStoreTiny,
                            C4DefList *pDefs) {
  ...
  for (pInfo = GetLast(); pInfo; pInfo = GetPrevious(pInfo)) {
    ...
    if (!fSavegame) {
      ...
      if (pDef)
        if (pDef->TemporaryCrew) continue;
    }
    // save
    if (!pInfo->Save(hGroup, fStoreTiny, pDefs)) return FALSE;
  }
  return TRUE;
}
...
void C4ObjectInfoList::DetachFromObjects() {
  C4ObjectInfo *cinf;
  for (cinf = First; cinf; cinf = cinf->Next) Game.Objects.ClearInfo(cinf);
}
...
C4ObjectInfo *C4ObjectInfoList::GetPrevious(C4ObjectInfo *pInfo) {
  for (C4ObjectInfo *cInfo = First; cInfo; cInfo = cInfo->Next)
    if (cInfo->Next == pInfo) return cInfo;
  return NULL;
}
