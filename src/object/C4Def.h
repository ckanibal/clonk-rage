...
const int32_t C4D_None = 0, C4D_All = ~C4D_None,
              ...
              C4D_Background = 1 << 20, C4D_Parallax = 1 << 21,
              C4D_MouseSelect = 1 << 22, C4D_Foreground = 1 << 23,
              C4D_MouseIgnore = 1 << 24, C4D_IgnoreFoW = 1 << 25,

              C4D_BackgroundOrForeground = (C4D_Background | C4D_Foreground);

const int32_t C4D_Grab_Put = 1, C4D_Grab_Get = 2,
              ...
              C4D_Connect_Rope = 128, C4D_EnergyHolder = 256,

              C4D_Place_Surface = 0, C4D_Place_Liquid = 1, C4D_Place_Air = 2;

const int32_t C4D_VehicleControl_None = 0, C4D_VehicleControl_Outside = 1,
              C4D_VehicleControl_Inside = 2;

const int32_t C4D_Sell = C4D_StaticBack | C4D_Structure | C4D_Vehicle |
...
const DWORD C4D_Load_None = 0, C4D_Load_Picture = 1, C4D_Load_Bitmap = 2,
            ...
            C4D_Load_ClonkNames = 128, C4D_Load_RankNames = 256,
            C4D_Load_RankFaces = 512,
            C4D_Load_FE = C4D_Load_Image | C4D_Load_Desc,
            C4D_Load_RX = C4D_Load_Bitmap | C4D_Load_Script |
                          C4D_Load_ClonkNames | C4D_Load_Desc |
                          C4D_Load_ActMap | C4D_Load_Sounds |
                          C4D_Load_RankNames | C4D_Load_RankFaces,
...
class C4DefCore {
 ...
 public:
  ...
  int32_t ClosedContainer;  // if set, contained objects are not damaged by
                            // lava/acid etc. 1: Contained objects can't view
                            // out; 2: They can
  ...
  int32_t TemporaryCrew;    // if set, info objects are not written into regular
                            // player files
  ...
  int32_t ConSizeOff;  // number of pixels to be subtracted from the needed
                       // height for this building
  int32_t NoSell;      // if set, object can't be sold (doesn't even appear in
                       // sell-menu)
  int32_t NoGet;  // if set, object can't be taken out of a containers manually
                  // (doesn't appear in get/activate-menus)
  int32_t NoFight;            // if set, object is never OCF_FightReady
  int32_t RotatedSolidmasks;  // if set, solidmasks can be rotated
  int32_t
      NeededGfxMode;  // if set, the def will only be loaded in given gfx mode
  int32_t RotatedEntrance;  // 0 entrance not rotateable, 1 entrance always,
                            // 2-360 entrance within this rotation
  int32_t NoTransferZones;
  int32_t AutoContextMenu;    // automatically open context menu for this object
  int32_t AllowPictureStack;  // allow stacking of multiple items in menus even
                              // if some attributes do not match. APS_*-values
 public:
  void Default();
...
class C4Def : public C4DefCore {
 ...
 public:
  ...
  C4PortraitGraphics *
      Portraits;  // Portraits (linked list of C4AdditionalDefGraphics)
 ...
 public:
  ...
  void Draw(C4Facet &cgo, BOOL fSelected = FALSE, DWORD iColor = 0,
            C4Object *pObj = NULL, int32_t iPhaseX = 0, int32_t iPhaseY = 0);

#ifdef C4ENGINE
  inline C4Facet &GetMainFace(C4DefGraphics *pGraphics, DWORD dwClr = 0) {
    MainFace.Surface = pGraphics->GetBitmap(dwClr);
    return MainFace;
  }
  int32_t GetValue(C4Object *pInBase, int32_t iBuyPlayer);  // get value of def;
                                                            // calling script
                                                            // functions if
                                                            // defined
  C4PhysicalInfo *GetFairCrewPhysicals();  // get fair crew physicals at current
                                           // fair crew strength
  void ClearFairCrewPhysicals();  // remove cached fair crew physicals, will be
 ...
 private:
  C4ValueArray *GetCustomComponents(C4Value *pvArrayHolder, C4Object *pBuilder,
                                    C4Object *pObjInstance = NULL);
 ...
 public:
  ...
  void GetComponents(C4IDList *pOutList, C4Object *pObjInstance = NULL,
                     C4Object *pBuilder = NULL);

  void IncludeDefinition(
      C4Def *pIncludeDef);          // inherit components from other definition
  void ResetIncludeDependencies();  // resets all pointers into foreign
                                    // definitions caused by include chains
};
...
class C4DefList
 ...
 public:
  void Default();
  void Clear();
  int32_t Load(C4Group &hGroup, DWORD dwLoadWhat, const char *szLanguage,
               C4SoundSystem *pSoundSystem = NULL, BOOL fOverload = FALSE,
               BOOL fSearchMessage = FALSE, int32_t iMinProgress = 0,
               int32_t iMaxProgress = 0, bool fLoadSysGroups = true);
  int32_t Load(const char *szSearch, DWORD dwLoadWhat, const char *szLanguage,
               C4SoundSystem *pSoundSystem = NULL, BOOL fOverload = FALSE,
               int32_t iMinProgress = 0, int32_t iMaxProgress = 0);
  int32_t LoadFolderLocal(const char *szPath, DWORD dwLoadWhat,
                          const char *szLanguage,
                          C4SoundSystem *pSoundSystem = NULL,
                          BOOL fOverload = FALSE, char *szStoreName = NULL,
                          int32_t iMinProgress = 0, int32_t iMaxProgress = 0);
  int32_t LoadForScenario(const char *szScenario, const char *szSpecified,
                          DWORD dwLoadWhat, const char *szLanguage,
                          C4SoundSystem *pSoundSystem = NULL,
                          BOOL fOverload = FALSE, int32_t iMinProgress = 0,
