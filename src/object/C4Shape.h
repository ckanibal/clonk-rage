...
class C4Rect {
 ...
 public:
  ...
  bool operator==(const C4Rect &r2) {
    return !((x - r2.x) | (y - r2.y) | (Wdt - r2.Wdt) | (Hgt - r2.Hgt));
  }
  bool operator!=(const C4Rect &r2) {
    return 0 != ((x - r2.x) | (y - r2.y) | (Wdt - r2.Wdt) | (Hgt - r2.Hgt));
  }
  ...
  C4Rect(
      const FLOAT_RECT &rcfOuter)  // set to surround floating point rectangle
  {
    x = static_cast<int32_t>(rcfOuter.left);
    y = static_cast<int32_t>(rcfOuter.top);
...
class C4TargetRect : public C4Rect {
 ...
 public:
  C4TargetRect(int32_t iX, int32_t iY, int32_t iWdt, int32_t iHgt, int32_t iTX,
               int32_t iTY)
      : C4Rect(iX, iY, iWdt, iHgt), tx(iTX), ty(iTY) {}
  C4TargetRect() {}  // default ctor; doesn't initialize
  void Set(int32_t iX, int32_t iY, int32_t iWdt, int32_t iHgt, int32_t iTX,
           int32_t iTY);
  void Default();
  bool ClipBy(C4TargetRect &rClip);  // clip this rectangle by the given one
                                     // (adding target positions) - return false
...
class C4Shape : public C4Rect {
 ...
 public:
  // remember to adjust C4Shape::CopyFrom and CreateOwnOriginalCopy when adding
  // members here!
  int32_t FireTop;
  int32_t VtxNum;
 ...
 public:
  ...
  BOOL LineConnect(int32_t tx, int32_t ty, int32_t cvtx, int32_t ld,
  ...
  int32_t GetVertexContact(int32_t iVtx, DWORD dwCheckMask, int32_t tx,
                           int32_t ty,
                           const C4DensityProvider &rDensityProvider =
                               DefaultDensityProvider);  // get CNAT-mask for
                                                         // given vertex - does
                                                         // not check range for
                                                         // iVtx!
  void CreateOwnOriginalCopy(C4Shape &rFrom);  // create copy of all vertex
                                               // members in back area of own
                                               // buffers
...
class C4RectList : public std::vector<C4Rect> {
 public:
  void AddRect(const C4Rect &rNewRect) { push_back(rNewRect); }
