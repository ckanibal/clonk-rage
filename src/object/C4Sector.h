...
// constants
const int32_t C4LSectorWdt = 50, C4LSectorHgt = 50;
...
class C4LSectors {
 ...
 public:
  ...
  void Update(C4Object *pObj,
              C4ObjectList *pMainList);  // does not update object order!
  void Remove(C4Object *pObj);

  void AssertObjectNotInList(C4Object *pObj);  // searches all sector lists for
                                               // object, and assert if it's
                                               // inside a list
...
class C4LArea {
 public:
  ...
  C4LArea(C4LSectors *pSectors, int ix, int iy, int iwdt,
          int ihgt)  // initializing constructor
  {
  ...
  void Set(C4LSectors *pSectors,
           const C4Rect &rect);  // set rect, calc bounds and get pitch
  ...
  // void MoveObject(C4Object *pObj, const C4LArea &toArea); // store object
  // into new area

  BOOL Contains(
      C4LSector *pSct) const;  // return whether sector is contained in area

  inline C4ObjectList *FirstObjects(
      C4LSector **ppSct)  // get first object list of this area
  {
    *ppSct = NULL;
    return NextObjects(NULL, ppSct);
  }
  C4ObjectList *NextObjects(
      C4ObjectList *pPrev,
      C4LSector **ppSct);  // get next object list of this area

  inline C4ObjectList *FirstObjectShapes(
      C4LSector **ppSct)  // get first object shapes list of this area
  {
    *ppSct = NULL;
    return NextObjectShapes(NULL, ppSct);
  }
  C4ObjectList *NextObjectShapes(
      C4ObjectList *pPrev,
      C4LSector **ppSct);  // get next object shapes list of this area
