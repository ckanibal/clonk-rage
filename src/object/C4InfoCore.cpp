...
const char *GetAName(const char *szNameFile) {
  // always eat the Random-value, so having or not having a Names.txt makes no
  // difference
  ...
  for (int iCnt = 0; iCnt < iName; iCnt++) AdvanceFileLine(hNamefile);
  GetANameBuffer[0] = 0;
  int iLoops = 0;
  do {
...
DWORD C4PlayerInfoCore::GetPrefColorValue(int32_t iPrefColor) {
  ...
  if (Inside<int32_t>(iPrefColor, 0, 11)) return valRGB[iPrefColor];
  // return RGB(valRGB[iPrefColor] >> 16, valRGB[iPrefColor] >> 8,
  // valRGB[iPrefColor] >> 0);
  return 0xAAAAAA;
}
...
BOOL C4PlayerInfoCore::Load(C4Group &hGroup) {
  ...
  if (hGroup.LoadEntryString(C4CFN_PlayerInfoCore, Source)) {
    ...
    if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(*this, Source,
                                                    GrpName.getData()))
      return FALSE;
    // Pref for AutoContextMenus is still undecided: default by player's control
    // style
    if (PrefAutoContextMenu == -1) PrefAutoContextMenu = PrefControlStyle;
    // Determine true color from indexed pref color
    if (!PrefColorDw) PrefColorDw = GetPrefColorValue(PrefColor);
    // Validate colors
...
BOOL C4PlayerInfoCore::Save(C4Group &hGroup) {
  StdStrBuf Source, Name = hGroup.GetFullName();
  Name.Append(DirSep C4CFN_PlayerInfoCore);
  if (!DecompileToBuf_Log<StdCompilerINIWrite>(*this, &Source, Name.getData()))
    return FALSE;
  if (!hGroup.Add(C4CFN_PlayerInfoCore, Source, FALSE, TRUE)) return FALSE;
...
void C4PlayerInfoCore::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(mkNamingAdapt(
      toC4CStr(RankName), "RankName",
      /*pRanks ? pRanks->Name(Rank,false) :*/ LoadResStr(
          "IDS_MSG_RANK")));  // TODO: check if this would be desirable
  ...
  pComp->Value(mkNamingAdapt(PrefAutoContextMenu, "AutoContextMenu",
                             -1));  // compiling default is -1  (if this is
                                    // detected, AutoContextMenus will be
                                    // defaulted by control style)
...
struct C4PhysInfoNameMap_t {
  const char *szName;
  C4PhysicalInfo::Offset off;
} C4PhysInfoNameMap[] = {{"Energy", &C4PhysicalInfo::Energy},
...
void C4PhysicalInfo::PromotionUpdate(int32_t iRank,
                                     bool fUpdateTrainablePhysicals,
                                     C4Def *pTrainDef) {
#ifdef C4ENGINE
  if (iRank >= 0) {
    CanDig = 1;
  ...
  Energy = Max<int32_t>(
      Energy, (50 + 5 * BoundBy<int32_t>(iRank, 0, 10)) * C4MaxPhysical / 100);
  if (fUpdateTrainablePhysicals && pTrainDef) {
    ...
    Scale = pTrainDef->Physical.Scale +
            (C4MaxPhysical - pTrainDef->Physical.Scale) * iTrainRank / 20;
    Hangle = pTrainDef->Physical.Hangle +
             (C4MaxPhysical - pTrainDef->Physical.Hangle) * iTrainRank / 20;
    Swim = pTrainDef->Physical.Swim +
           (C4MaxPhysical - pTrainDef->Physical.Swim) * iTrainRank / 20;
    Fight = pTrainDef->Physical.Fight +
            (C4MaxPhysical - pTrainDef->Physical.Fight) * iTrainRank / 20;
    // do script updates for any physicals as required (this will train stuff
    // like magic)
    const char *szPhysName;
    C4PhysicalInfo::Offset PhysOff;
...
bool C4PhysicalInfo::GetOffsetByName(const char *szPhysicalName,
                                     Offset *pmpiOut) {
  // query map
...
const char *C4PhysicalInfo::GetNameByOffset(Offset mpiOff) {
  // query map
  for (C4PhysInfoNameMap_t *entry = C4PhysInfoNameMap; entry->szName; ++entry)
    if (entry->off == mpiOff) return entry->szName;
  return NULL;
}

const char *C4PhysicalInfo::GetNameByIndex(int32_t iIdx, Offset *pmpiOut) {
  // query map
  if (!Inside<int32_t>(iIdx, 0,
                       sizeof(C4PhysInfoNameMap) / sizeof(C4PhysInfoNameMap_t)))
    return NULL;
...
void C4PhysicalInfo::TrainValue(int32_t *piVal, int32_t iTrainBy,
                                int32_t iMaxTrain) {
  // only do training if value was nonzero before (e.g., Magic for revaluated
  // Clonks)
  if (*piVal)
...
void C4PhysicalInfo::Train(Offset mpiOffset, int32_t iTrainBy,
                           int32_t iMaxTrain) {
  // train own value
...
bool C4PhysicalInfo::operator==(const C4PhysicalInfo &cmp) const {
  ...
  for (C4PhysInfoNameMap_t *entry = C4PhysInfoNameMap; entry->szName; ++entry)
    if (this->*(entry->off) != cmp.*(entry->off)) return false;
  return true;
}
...
#ifdef C4ENGINE
  pComp->Value(mkNamingAdapt(mkSTLContainerAdapt(Changes), "Changes",
                             std::vector<C4PhysicalChange>()));
#endif
}

void C4TempPhysicalInfo::Train(Offset mpiOffset, int32_t iTrainBy,
                               int32_t iMaxTrain) {
  // train own value
  C4PhysicalInfo::Train(mpiOffset, iTrainBy, iMaxTrain);
// train all temp values
#ifdef C4ENGINE
  for (std::vector<C4PhysicalChange>::iterator i = Changes.begin();
       i != Changes.end(); ++i)
...
#ifdef C4ENGINE
  // search last matching physical check (should always be last if well
  // scripted)
  for (std::vector<C4PhysicalChange>::reverse_iterator i = Changes.rbegin();
       i != Changes.rend(); ++i)
    if ((*i).mpiOffset == mpiOffset) {
...
void C4PhysicalChange::CompileFunc(StdCompiler *pComp) {
  ...
  if (szPhyn)
    SCopy(szPhyn, phyn, C4MaxName);
  else
    *phyn = '\0';
  pComp->Value(mkStringAdapt(phyn, C4MaxName, StdCompiler::RCT_Idtf));
  if (!C4PhysicalInfo::GetOffsetByName(phyn, &mpiOffset))
    pComp->excNotFound("Physical change name \"%s\" not found.");
...
void C4ObjectInfoCore::Promote(int32_t iRank, C4RankSystem &rRanks,
                               bool fForceRankName) {
  Rank = iRank;
  Physical.PromotionUpdate(Rank);
  // copy new rank name if defined only, or forced to use highest defined rank
  // for too high info ranks
...
BOOL C4ObjectInfoCore::Save(C4Group &hGroup, C4DefList *pDefs) {
  ...
  char *Buffer;
  size_t BufferSize;
  if (!Decompile(&Buffer, &BufferSize)) return FALSE;
...
BOOL C4ObjectInfoCore::Compile(const char *szSource) {
  bool ret = CompileFromBuf_LogWarn<StdCompilerINIRead>(
      mkNamingAdapt(*this, "ObjectInfo"), StdStrBuf(szSource), "ObjectInfo");
...
BOOL C4ObjectInfoCore::Decompile(char **ppOutput, size_t *ipSize) {
  ...
  if (!DecompileToBuf_Log<StdCompilerINIWrite>(
          mkNamingAdapt(*this, "ObjectInfo"), &Buf, "ObjectInfo")) {
