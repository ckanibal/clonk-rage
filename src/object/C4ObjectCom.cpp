...
BOOL CreateConstructionSite(int32_t ctx, int32_t bty, C4ID strid, int32_t owner,
                            C4Object *pByObj);
...
BOOL ObjectActionJump(C4Object *cObj, FIXED xdir, FIXED ydir, bool fByCom) {
  // scripted jump?
  assert(cObj);
  C4AulParSet pars(C4VInt(fixtoi(xdir, 100)), C4VInt(fixtoi(ydir, 100)),
                   C4VBool(fByCom));
  ...
  cObj->ydir = ydir;
  cObj->Mobile = 1;
  // unstick from ground, because jump command may be issued in an
  // Action-callback,
...
BOOL ObjectActionThrow(C4Object *cObj, C4Object *pThing) {
  ...
  int32_t iDir = 1;
  if (cObj->Action.Dir == DIR_Left) iDir = -1;
  // Set action
  ...
  if (!(cObj->t_contact & CNAT_Top))
    if (!(cObj->t_contact & CNAT_Left))
      if (!(cObj->t_contact & CNAT_Right))
        if (!(cObj->t_contact & CNAT_Bottom)) return TRUE;
  return FALSE;
}
...
BOOL CheckCornerScale(C4Object *cObj, int32_t &iRangeX, int32_t &iRangeY) {
  for (iRangeX = CornerRange; iRangeX >= 1; iRangeX--)
    for (iRangeY = CornerRange; iRangeY >= 1; iRangeY--)
      if (CornerScaleOkay(cObj, iRangeX, iRangeY)) return TRUE;
  return FALSE;
}
...
BOOL ObjectActionCornerScale(C4Object *cObj) {
  ...
  // Swimming: check range min to max
    ...
    while (!CornerScaleOkay(cObj, iRangeY, iRangeY)) {
      iRangeY++;
      if (iRangeY > CornerRange) return FALSE;
    }
    iRangeX = iRangeY;
  }
  // Do corner scale
  if (!cObj->SetActionByName("KneelUp")) cObj->SetActionByName("Walk");
  cObj->xdir = cObj->ydir = 0;
  // if (cObj->Action.Dir==DIR_Left) cObj->Action.ComDir=COMD_Left; else
...
BOOL ObjectComMovement(C4Object *cObj, int32_t comdir) {
  ...
  if (!cObj->xdir &&
      (cObj->GetProcedure() == DFA_WALK || cObj->GetProcedure() == DFA_HANGLE))
    switch (comdir) {
      case COMD_Left:
      case COMD_UpLeft:
      case COMD_DownLeft:
...
BOOL ObjectComJump(C4Object *cObj)  // by ObjectComUp, ExecCMDFMoveTo, FnJump
{
  ...
  FIXED iPhysicalWalk =
      ValByPhysical(280, pPhysical->Walk) * itofix(cObj->GetCon(), FullCon);
  FIXED iPhysicalJump =
      ValByPhysical(1000, pPhysical->Jump) * itofix(cObj->GetCon(), FullCon);

  if (cObj->Action.ComDir == COMD_Left || cObj->Action.ComDir == COMD_UpLeft)
    TXDir = -iPhysicalWalk;
  else if (cObj->Action.ComDir == COMD_Right ||
  ...
  if (cObj->Shape.ContactDensity > C4M_Liquid)
    if (SimFlightHitsLiquid(x, y, TXDir, -iPhysicalJump))
      if (ObjectActionDive(cObj, TXDir, -iPhysicalJump)) return TRUE;
  // Regular jump
...
BOOL ObjectComLetGo(
    C4Object *cObj,
    int32_t xdirf) {  // by ACTSCALE, ACTHANGLE or ExecCMDFMoveTo
...
BOOL ObjectComLineConstruction(C4Object *cObj) {
  ...
  if (!(linekit = cObj->Contents.Find(C4ID_Linekit))) {
    ...
    // Create new linekit
    if (!(linekit = Game.CreateObject(C4ID_Linekit, cObj, cline->Owner)))
      return FALSE;
  ...
  // Check power
  if (linetype == C4ID_None)
    if (tstruct->Def->LineConnect & C4D_Power_Output) linetype = C4ID_PowerLine;
  // No good
  if (linetype == C4ID_None) {
    StartSoundEffect("Error", false, 100, cObj);
    sprintf(OSTR, LoadResStr("IDS_OBJ_NONEWLINE"));
    ...
    C4Object *cObj)  // "Activation" by DFA_WALK, DFA_DIG, DFA_SWIM
{
  ...
  if (cObj->Contents.GetObject())
    if (!!cObj->Contents.GetObject()->Call(PSF_Activate,
                                           &C4AulParSet(C4VObj(cObj))))
      return;
    ...
    if (!cObj->Contents.GetObject())
      if ((pTarget = Game.Objects.AtObject(cObj->x, cObj->y, ocf, cObj)))
        if (ocf & OCF_LineConstruct)
          if (ObjectComLineConstruction(cObj)) return;
...
BOOL ObjectComDrop(C4Object *cObj, C4Object *pThing) {
  ...
  // When doing a diagonal forward drop during flight, exit a bit closer to the
  // Clonk to allow planned tumbling
  // Except when hangling, so you can mine effectively form the ceiling, and
  // when swimming because you cannot tumble then
  FIXED pthrow = ValByPhysical(400, cObj->GetPhysical()->Throw);
  int32_t tdir = 0;
  int right = 0;
  ...
  int32_t iOutposReduction =
      1;                   // don't exit object too far forward during jump
  if (iProc != DFA_SCALE)  // never diagonal during scaling (can have com into
                           // wall during scaling!)
  {
    if (ComDirLike(cObj->Action.ComDir, COMD_Left)) {
      tdir = -1;
      right = 0;
      if (cObj->xdir < FIXED10(15) && !isHanglingOrSwimming) --iOutposReduction;
    }
    if (ComDirLike(cObj->Action.ComDir, COMD_Right)) {
      tdir = +1;
      right = 1;
      if (cObj->xdir > FIXED10(-15) && !isHanglingOrSwimming)
        --iOutposReduction;
    }
  }
  // Exit object
  pThing->Exit(
      cObj->x +
          (cObj->Shape.x + cObj->Shape.Wdt * right) * !!tdir * iOutposReduction,
      cObj->y + cObj->Shape.y + cObj->Shape.Hgt -
          (pThing->Shape.y + pThing->Shape.Hgt),
      0, pthrow * tdir, Fix0, Fix0);
  // NoCollectDelay
...
{                                        // by C4CMD_Drop
  ...
  if (pThing) return ObjectComPut(cObj, pTarget, pThing);
  // If target is own container, activate activation menu
  if (pTarget == cObj->Contained) return ObjectComTake(cObj);  // carlo
...
BOOL ObjectComPunch(C4Object *cObj, C4Object *pTarget, int32_t punch) {
  ...
  if (!punch)
    if (pTarget->GetPhysical()->Fight)
      punch = BoundBy<int32_t>(
          5 * cObj->GetPhysical()->Fight / pTarget->GetPhysical()->Fight, 0,
          10);
  if (!punch) return TRUE;
  bool fBlowStopped =
      !!pTarget->Call(PSF_QueryCatchBlow, &C4AulParSet(C4VObj(cObj)));
  if (fBlowStopped && punch > 1)
    punch = punch / 2;  // half damage for caught blow, so shield+armor help in
                        // fistfight and vs monsters
  pTarget->DoEnergy(-punch, false, C4FxCall_EngGetPunched, cObj->Controller);
  int32_t tdir = +1;
  if (cObj->Action.Dir == DIR_Left) tdir = -1;
  pTarget->Action.ComDir = COMD_Stop;
  // No tumbles when blow was caught
  if (fBlowStopped) return FALSE;
  // Hard punch
  if (punch >= 10)
    if (ObjectActionTumble(pTarget, pTarget->Action.Dir, FIXED100(150) * tdir,
                           itofix(-2))) {
      pTarget->LastEnergyLossCausePlayer =
          cObj->Controller;  // for kill tracing when pushing enemies off a
                             // cliff
  ...
  if (ObjectActionGetPunched(pTarget, FIXED100(250) * tdir, Fix0)) {
    pTarget->LastEnergyLossCausePlayer =
        cObj->Controller;  // for kill tracing when pushing enemies off a cliff
    pTarget->Call(PSF_CatchBlow, &C4AulParSet(C4VInt(punch), C4VObj(cObj)));
    return TRUE;
  }
...
BOOL ObjectComCancelAttach(C4Object *cObj) {
  if (cObj->GetProcedure() == DFA_ATTACH) return cObj->SetAction(ActIdle);
  return FALSE;
}
...
void ObjectComStopDig(C4Object *cObj) {
  ...
  if (cObj->Command)
    if (cObj->Command->Command == C4CMD_Dig) cObj->ClearCommand(cObj->Command);
}
...
int32_t Control2Com(int32_t iControl, bool fUp) {
  ...
  static const char con2com_r[C4MaxKey] = {
      COM_None,    COM_None,  COM_None,      COM_Throw_R,
      COM_Up_R,    COM_Dig_R, COM_Left_R,    COM_Down_R,
      COM_Right_R, COM_None,  COM_Special_R, COM_Special2_R};
  if (fUp) {
    if (Inside<int32_t>(iControl, 0, C4MaxKey - 1)) return con2com_r[iControl];
  } else {
    if (Inside<int32_t>(iControl, 0, C4MaxKey - 1)) return con2com[iControl];
  }
  return COM_None;
}
...
int32_t Coms2ComDir(int32_t iComs) {
  ...
  static int32_t DirComs =
      (1 << COM_Left) | (1 << COM_Right) | (1 << COM_Up) | (1 << COM_Down);
  switch (iComs & DirComs) {
    case (1 << COM_Up):
      return COMD_Up;
    case (1 << COM_Up) | (1 << COM_Right):
      return COMD_UpRight;
    case (1 << COM_Right):
      return COMD_Right;
    case (1 << COM_Down) | (1 << COM_Right):
      return COMD_DownRight;
    case (1 << COM_Down):
      return COMD_Down;
    case (1 << COM_Down) | (1 << COM_Left):
      return COMD_DownLeft;
    case (1 << COM_Left):
      return COMD_Left;
    case (1 << COM_Up) | (1 << COM_Left):
      return COMD_UpLeft;
    // up, right and left could be interpreted as COMD_Up etc., but that's too
    // complicated for now
    default:
      return COMD_Stop;
  }
...
void DrawCommandKey(C4Facet &cgo, int32_t iCom, BOOL fPressed,
                    const char *szText) {
  // Draw key
  ...
  CStdFont &rFont = (cgo.Hgt <= C4MN_SymbolSize)
                        ? Game.GraphicsResource.FontTiny
                        : Game.GraphicsResource.FontRegular;
  // Draw text
  if (szText && Config.Graphics.ShowCommandKeys)
    Application.DDraw->TextOut(szText, rFont, 1.0, cgo.Surface,
                               cgo.X + cgo.Wdt / 2,
                               cgo.Y + cgo.Hgt - rFont.iLineHgt - 2,
                               CStdDDraw::DEFAULT_MESSAGE_COLOR, ACenter);
}

void DrawControlKey(C4Facet &cgo, int32_t iControl, BOOL fPressed,
                    const char *szText) {
  // Draw key
  Game.GraphicsResource.fctKey.Draw(cgo, FALSE, fPressed);
  // Draw control symbol
...
BOOL SellFromBase(int32_t iPlr, C4Object *pBaseObj, C4ID id,
                  C4Object *pSellObj) {
  C4Object *pThing;
  // Valid checks
  ...
  if (Game.Players.Get(pBaseObj->Base)->Eliminated) {
    ...
    sprintf(OSTR, LoadResStr("IDS_PLR_ELIMINATED"),
            Game.Players.Get(pBaseObj->Base)->GetName());
  ...
  if (pThing = pSellObj)
    if (!pThing->Status || pThing->Contained != pBaseObj) pThing = NULL;
  // Get object from home pBaseObj via selected id, if no or an anvalid thing
  // has been specified
  if (!pThing)
...
BOOL Buy2Base(int32_t iPlr, C4Object *pBase, C4ID id, BOOL fShowErrors) {
  ...
  if (Hostile(iPlr, pBase->Base)) {
    ...
    sprintf(OSTR, LoadResStr("IDS_PLR_HOSTILE"),
            Game.Players.Get(pBase->Base)->GetName());
    GameMsgPlayer(OSTR, iPlr);
    return FALSE;
  }
  // buy
  if (!(pThing =
            Game.Players.Get(pBase->Base)->Buy(id, fShowErrors, iPlr, pBase)))
    return FALSE;
...
BOOL PlayerObjectCommand(int32_t plr, int32_t cmdf, C4Object *pTarget,
  ...
  if (cmdf == C4CMD_Throw || cmdf == C4CMD_Drop)
  ...
  if (cmdf == C4CMD_Throw) {
    ...
    if (pPlr->ControlStyle && (pPlr->PressedComs & (1 << COM_Down)))
      fConvertToDrop = true;
    if (fConvertToDrop)
      return pPlr->ObjectCommand(C4CMD_Drop, pTarget, tx, ty, NULL, 0,
                                 iAddMode);
  }
  // Route to player
