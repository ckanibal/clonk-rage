...
void C4LSectors::Init(int iWdt, int iHgt) {
  ...
  for (int cnt = 0; cnt < Size; cnt++, sct++) sct->Init(cnt % Wdt, cnt / Wdt);
  SectorOut.Init(
      -1, -1);  // outpos at -1,-1 - MUST NOT intersect with an inside sector!
}
...
C4LSector *C4LSectors::SectorAt(int ix, int iy) {
  // check bounds
  if (ix < 0 || iy < 0 || ix >= PxWdt || iy >= PxHgt) return &SectorOut;
  // get sector
...
void C4LSectors::Update(C4Object *pObj, C4ObjectList *pMainList) {
  ...
  for (pOld = pObj->Area.First(); pOld; pOld = pObj->Area.Next(pOld))
    if (!NewArea.Contains(pOld)) pOld->ObjectShapes.Remove(pObj);
...
#ifdef _DEBUG
    LogF(
        "WARNING: Object %d of type %s deleted but not found in pos sector "
        "list!",
        pObj->Number, C4IdText(pObj->id));
#endif
...
void C4LSectors::Dump() {
  LogSilent(
      DecompileToBuf<StdCompilerINIWrite>(
          mkNamingAdapt(mkArrayAdapt(Sectors, Size), "Sector")).getData());
}

bool C4LSectors::CheckSort() {
  for (int cnt = 0; cnt < Size; cnt++)
    if (!Sectors[cnt].Objects.CheckSort(&Game.Objects)) return false;
...
bool C4LArea::operator==(const C4LArea &Area) const {
  return pFirst == Area.pFirst && xL == Area.xL && yL == Area.yL &&
         pOut == Area.pOut;
}
...
void C4LArea::Set(C4LSectors *pSectors, const C4Rect &Rect) {
  ...
  pOut = NULL;
  // check bounds
  C4Rect ClippedRect(Rect), Bounds(0, 0, pSectors->PxWdt, pSectors->PxHgt);
  ...
  // (note this will associate areas that are above landscape bounds with
  // sectors inside)
  ...
  // calc pitch
  dpitch = pSectors->Wdt -
           (ClippedRect.x + ClippedRect.Wdt - 1) / C4LSectorWdt +
           ClippedRect.x / C4LSectorWdt;
}

void C4LArea::Set(C4LSectors *pSectors, C4Object *pObj) {
  // set to object facet rect
  Set(pSectors,
      C4Rect(pObj->Left(), pObj->Top(), pObj->Width(), pObj->Height()));
}

C4LSector *C4LArea::Next(C4LSector *pPrev) const {
  // the outside-sector is the last sector that is returned
  if (pPrev == pOut) return NULL;
  // within one line?
  if (pPrev->x < xL) return pPrev + 1;
  // within the area?
  if (pPrev->y < yL) return pPrev + dpitch;
...
BOOL C4LArea::Contains(C4LSector *pSct) const {
  ...
  // check bounds
  return (pSct->x >= pFirst->x && pSct->y >= pFirst->y && pSct->x <= xL &&
          pSct->y <= yL);
}
