...
C4GameObjects::C4GameObjects() { Default(); }

C4GameObjects::~C4GameObjects() { Sectors.Clear(); }
...
BOOL C4GameObjects::Add(C4Object *nObj) {
  ...
  if (!C4ObjectList::Add(nObj, C4ObjectList::stMain)) return FALSE;
  // add to sectors
...
void C4GameObjects::PutSolidMasks() {
  C4ObjectLink *cLnk;
  for (cLnk = First; cLnk; cLnk = cLnk->Next)
    if (cLnk->Obj->Status) cLnk->Obj->UpdateSolidMask(false);
}
...
BOOL C4GameObjects::OrderObjectBefore(C4Object *pObj1, C4Object *pObj2) {
  ...
  if ((pObj1->Category & C4D_SortLimit) < (pObj2->Category & C4D_SortLimit))
    return FALSE;
  // reorder
  if (!C4ObjectList::OrderObjectBefore(pObj1, pObj2)) return FALSE;
  // update area lists
...
BOOL C4GameObjects::OrderObjectAfter(C4Object *pObj1, C4Object *pObj2) {
  ...
  if ((pObj1->Category & C4D_SortLimit) > (pObj2->Category & C4D_SortLimit))
    return FALSE;
  // reorder
  if (!C4ObjectList::OrderObjectAfter(pObj1, pObj2)) return FALSE;
  // update area lists
  UpdatePosResort(pObj1);
  // done, success
  return TRUE;
...
uint32_t C4GameObjects::GetNextMarker() {
  ...
  // If all markers are exceeded, restart marker at 1 and reset all object
  // markers to zero.
  if (!marker) {
    C4Object *cobj;
    C4ObjectLink *clnk;
