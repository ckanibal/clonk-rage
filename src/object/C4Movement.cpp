...
BOOL ContactVtxCNAT(C4Object *cobj, BYTE cnat_dir) {
  ...
  for (cnt = 0; cnt < cobj->Shape.VtxNum; cnt++)
    if (cobj->Shape.VtxContactCNAT[cnt] & cnat_dir) fcontact = TRUE;
  return fcontact;
}
...
int32_t ContactVtxFriction(C4Object *cobj) {
  int32_t cnt;
  for (cnt = 0; cnt < cobj->Shape.VtxNum; cnt++)
    if (cobj->Shape.VtxContactCNAT[cnt]) return cobj->Shape.VtxFriction[cnt];
  return 0;
}
...
void C4Object::TargetBounds(int32_t &ctco, int32_t limit_low, int32_t limit_hi,
...
void C4Object::SideBounds(int32_t &ctcox) {
  // layer bounds
  if (pLayer)
    if (pLayer->Def->BorderBound & C4D_Border_Layer)
      if (Action.Act <= ActIdle ||
          Def->ActMap[Action.Act].Procedure != DFA_ATTACH)
        if (Category & C4D_StaticBack)
          TargetBounds(ctcox, pLayer->x + pLayer->Shape.x,
                       pLayer->x + pLayer->Shape.x + pLayer->Shape.Wdt,
                       CNAT_Left, CNAT_Right);
        else
          TargetBounds(
              ctcox, pLayer->x + pLayer->Shape.x - Shape.x,
              pLayer->x + pLayer->Shape.x + pLayer->Shape.Wdt + Shape.x,
              CNAT_Left, CNAT_Right);
  // landscape bounds
  ...
  if (pLayer)
    if (pLayer->Def->BorderBound & C4D_Border_Layer)
      if (Action.Act <= ActIdle ||
          Def->ActMap[Action.Act].Procedure != DFA_ATTACH)
        if (Category & C4D_StaticBack)
          TargetBounds(ctcoy, pLayer->y + pLayer->Shape.y,
                       pLayer->y + pLayer->Shape.y + pLayer->Shape.Hgt,
                       CNAT_Top, CNAT_Bottom);
        else
          TargetBounds(
              ctcoy, pLayer->y + pLayer->Shape.y - Shape.y,
              pLayer->y + pLayer->Shape.y + pLayer->Shape.Hgt + Shape.y,
              CNAT_Top, CNAT_Bottom);
  // landscape bounds
...
void C4Object::DoMovement() {
  ...
  if (OCF & OCF_Rotate && !!rdir) {
    ...
    while (r != ctcor) {
      ...
      if (Action.t_attach && !fNoAttach) {
        // more accurately, attachment should be evaluated by a rotation around
        // the attachment vertex
        // however, as long as this code is only used for some surfaces
      ...
      if (iContact = ContactCheck(ctx, cty))  // Contact
      {
        ...
        // Redirect to y
        if (iContact == 1)
          if (!fRedirectYR) RedirectForce(rdir, ydir, -1);
        // Stop rotation
        rdir = 0;
      } else {
        fTurned = 1;
  ...
  if (fNoAttach) NoAttachAction();
  // Movement Script Execution
  if (fAnyContact) {
    C4AulParSet pars(C4VInt(fixtoi(oldxdir, 100)),
                     C4VInt(fixtoi(oldydir, 100)));
...
BOOL C4Object::ExecMovement()  // Every Tick1 by Execute
{
  ...
  // Out of bounds check
  if ((!Inside<int32_t>(x, 0, GBackWdt) &&
       !(Def->BorderBound & C4D_Border_Sides)) ||
      (y > GBackHgt && !(Def->BorderBound & C4D_Border_Bottom)))
    // Never remove attached objects: If they are truly outside landscape, their
    // target will be removed,
    //  and the attached objects follow one frame later
    if (Action.Act < 0 || !Action.Target ||
        Def->ActMap[Action.Act].Procedure != DFA_ATTACH) {
      bool fRemove = true;
      // never remove HUD objects
      if (Category & C4D_Parallax) {
        fRemove = false;
        if (x > GBackWdt || y > GBackHgt)
          fRemove = true;  // except if they are really out of the viewport to
                           // the right...
        else if (x < 0 && Local[0].Data)
          fRemove =
              true;  // ...or it's not HUD horizontally and it's out to the left
        else if (!Local[0].Data && x < -GBackWdt)
          fRemove =
              true;  // ...or it's HUD horizontally and it's out to the left
      }
      if (fRemove) {
        AssignDeath(true);
        AssignRemoval();
...
BOOL SimFlightHitsLiquid(FIXED fcx, FIXED fcy, FIXED xdir, FIXED ydir) {
  ...
  if (DensityLiquid(GBackDensity(fixtoi(fcx), fixtoi(fcy))))
    if (!SimFlight(fcx, fcy, xdir, ydir, 0, C4M_Liquid - 1, 10)) return FALSE;
  // Hits liquid?
  if (!SimFlight(fcx, fcy, xdir, ydir, C4M_Liquid, 100, -1)) return FALSE;
  // liquid & deep enough?
  return GBackLiquid(fixtoi(fcx), fixtoi(fcy)) &&
         GBackLiquid(fixtoi(fcx), fixtoi(fcy) + 9);
}
