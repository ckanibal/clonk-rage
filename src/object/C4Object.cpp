...
void DrawVertex(C4Facet &cgo, int32_t tx, int32_t ty, int32_t col,
                int32_t contact) {
  if (Inside<int32_t>(tx, 1, cgo.Wdt - 2) &&
      Inside<int32_t>(ty, 1, cgo.Hgt - 2)) {
    Application.DDraw->DrawHorizontalLine(cgo.Surface, cgo.X + tx - 1,
                                          cgo.X + tx + 1, cgo.Y + ty, col);
    Application.DDraw->DrawVerticalLine(cgo.Surface, cgo.X + tx, cgo.Y + ty - 1,
                                        cgo.Y + ty + 1, col);
    if (contact)
      Application.DDraw->DrawFrame(cgo.Surface, cgo.X + tx - 2, cgo.Y + ty - 2,
                                   cgo.X + tx + 2, cgo.Y + ty + 2, CWhite);
  }
}

void C4Action::SetBridgeData(int32_t iBridgeTime, bool fMoveClonk, bool fWall,
  ...
  // mask in this->Data
  Data = (uint32_t(iBridgeTime) << 16) + (uint32_t(fMoveClonk) << 8) +
         (uint32_t(fWall) << 9) + iBridgeMaterial;
}

void C4Action::GetBridgeData(int32_t &riBridgeTime, bool &rfMoveClonk,
                             bool &rfWall, int32_t &riBridgeMaterial) {
  // mask from this->Data
...
BOOL C4Object::Init(C4Def *pDef, C4Object *pCreator, int32_t iOwner,
                    C4ObjectInfo *pInfo, int32_t nx, int32_t ny, int32_t nr,
  ...
  // Color
  if (Def->ColorByOwner)
    if (ValidPlr(Owner)) Color = Game.Players.Get(Owner)->ColorDw;
...
void C4Object::UpdateShape(bool bUpdateVertices) {
  ...
  // Rotation
  if (Def->Rotateable)
    if (r != 0) Shape.Rotate(r, bUpdateVertices);
...
void C4Object::UpdatePos() {
  ...
  // do *NOT* do this while initializing, because object cannot be sorted by
  // main list
  if (!Initializing && Status == C4OS_NORMAL) Game.Objects.UpdatePos(this);
}

void C4Object::UpdateFace(bool bUpdateShape, bool fTemp) {
  // Update shape - NOT for temp call, because temnp calls are done in drawing
  // routine
  // must not change sync relevant data here (although the shape and pos
  // *should* be updated at that time anyway,
  ...
  if (Con >= FullCon || Def->GrowthType)
    if (!Def->Rotateable || (r == 0))
      if (Def->TopFace.Wdt > 0)  // Fullcon & no rotation
        TopFace.Set(GetGraphics()->GetBitmap(Color), Def->TopFace.x,
                    Def->TopFace.y, Def->TopFace.Wdt, Def->TopFace.Hgt);
...
void C4Object::UpdateGraphics(bool fGraphicsChanged, bool fTemp) {
  // check color
  if (!fTemp)
    if (!pGraphics->IsColorByOwner()) Color = 0;
  ...
  if (Action.Act > ActIdle)
    ...
    if (iFlipDir = Def->ActMap[Action.Act].FlipDir)
      ...
      if (Action.Dir >= iFlipDir) {
        // Calculate flipped drawing dir (from the flipdir direction going
        // backwards)
...
void C4Object::DrawFace(C4FacetEx &cgo, int32_t cgoX, int32_t cgoY,
  ...
  if ((!Def->Rotateable || (r == 0)) && !pDrawTransform) {
    lpDDraw->Blit(GetGraphics()->GetBitmap(Color), fx, fy, fwdt, fhgt,
                  cgo.Surface, (int)tx, (int)ty, (int)twdt, (int)thgt, TRUE,
                  NULL);
  }
  // Rotated or transformed
    ...
    C4DrawTransform rot;
    if (pDrawTransform) {
      rot.SetTransformAt(*pDrawTransform, cgoX + Shape.Wdt / 2.0f,
                         cgoY + Shape.Hgt / 2.0f);
      if (r)
        rot.Rotate(r * 100, cgoX + Shape.Wdt / 2.0f, cgoY + Shape.Hgt / 2.0f);
    } else {
...
void C4Object::ComponentConCutoff() {
  ...
  for (cnt = 0; Component.GetID(cnt); cnt++)
    Component.SetCount(
        cnt, Min<int32_t>(Component.GetCount(cnt),
...
void C4Object::ComponentConGain() {
  ...
  for (cnt = 0; Component.GetID(cnt); cnt++)
    Component.SetCount(
        cnt, Max<int32_t>(Component.GetCount(cnt),
                          Def->Component.GetCount(cnt) * Con / FullCon));
}
...
void C4Object::SetOCF() {
  ...
  if (Contained && !Game.Objects.ObjectNumber(Contained)) {
    ...
    LogF("Warning: contained in deleted object %p (%s)!", Contained,
         Contained->GetName());
  }
#endif
  if (Contained)
  ...
  if (Def->Grab && !(Category & C4D_StaticBack)) OCF |= OCF_Grab;
  // OCF_Carryable: Can be picked up
  if (Def->Carryable) OCF |= OCF_Carryable;
  // OCF_OnFire: Is burning
  if (OnFire) OCF |= OCF_OnFire;
  // OCF_Inflammable: Is not burning and is inflammable
  if (!OnFire && Def->ContactIncinerate > 0)
    // Is not a dead living
    if (!(Category & C4D_Living) || Alive) OCF |= OCF_Inflammable;
  // OCF_FullCon: Is fully completed/grown
  if (Con >= FullCon) OCF |= OCF_FullCon;
  // OCF_Chop: Can be chopped
  DWORD cocf = OCF_Exclusive;
  if (Def->Chopable)
    if (Category & C4D_StaticBack)  // Must be static back: this excludes trees
                                    // that have already been chopped
      if (!Game.Objects.AtObject(x, y, cocf))  // Can only be chopped if the
                                               // center is not blocked by an
                                               // exclusive object
        OCF |= OCF_Chop;
  // OCF_Rotate: Can be rotated
  if (Def->Rotateable)
    // Don't rotate minimum (invisible) construction sites
    if (Con > 100) OCF |= OCF_Rotate;
  // OCF_Exclusive: No action through this, no construction in front of this
  if (Def->Exclusive) OCF |= OCF_Exclusive;
  // OCF_Entrance: Can currently be entered/activated
  if ((Def->Entrance.Wdt > 0) && (Def->Entrance.Hgt > 0))
    if ((OCF & OCF_FullCon) &&
        ((Def->RotatedEntrance == 1) || (r <= Def->RotatedEntrance)))
      OCF |= OCF_Entrance;
  // HitSpeeds
    ...
    if ((Def->Collection.Wdt > 0) && (Def->Collection.Hgt > 0))
      if (!Def->CollectionLimit ||
        ...
        if ((Action.Act <= ActIdle) || (!Def->ActMap[Action.Act].Disabled))
          if (NoCollectDelay == 0) OCF |= OCF_Collection;
  // OCF_Living
  ...
  if (OCF & OCF_Alive)
    if ((Action.Act <= ActIdle) || (!Def->ActMap[Action.Act].Disabled))
      if (!Def->NoFight) OCF |= OCF_FightReady;
  // OCF_LineConstruct
  if (OCF & OCF_FullCon)
    if (Def->LineConnect) OCF |= OCF_LineConstruct;
  // OCF_Prey
  if (Def->Prey)
    if (Alive) OCF |= OCF_Prey;
  // OCF_CrewMember
  if (Def->CrewMember)
    if (Alive) OCF |= OCF_CrewMember;
  // OCF_AttractLightning
  if (Def->AttractLightning)
    if (OCF & OCF_FullCon) OCF |= OCF_AttractLightning;
  // OCF_NotContained
  if (!Contained) OCF |= OCF_NotContained;
  // OCF_Edible
  if (Def->Edible) OCF |= OCF_Edible;
  // OCF_InLiquid
  if (InLiquid)
    if (!Contained) OCF |= OCF_InLiquid;
  // OCF_InSolid
  if (!Contained)
    if (GBackSolid(x, y)) OCF |= OCF_InSolid;
  // OCF_InFree
  if (!Contained)
    if (!GBackSemiSolid(x, y - 1)) OCF |= OCF_InFree;
  // OCF_Available
...
void C4Object::UpdateOCF() {
  ...
  if (Contained)
    ...
    InMat = GBackMat(x, y);
  // Keep the bits that only have to be updated with SetOCF (def, category, con,
  // alive, onfire)
  OCF = OCF & (OCF_Normal | OCF_Carryable | OCF_Exclusive | OCF_Edible |
               OCF_Grab | OCF_FullCon
               /*| OCF_Chop - now updated regularly, see below */
               |
               OCF_Rotate | OCF_OnFire | OCF_Inflammable | OCF_Living |
               OCF_Alive | OCF_LineConstruct | OCF_Prey | OCF_CrewMember |
               OCF_AttractLightning | OCF_PowerConsumer);
  // OCF_Construct: Can be built outside
  if (Def->Constructable && (Con < FullCon) && (r == 0) && !OnFire)
    OCF |= OCF_Construct;
  // OCF_Entrance: Can currently be entered/activated
...
BOOL C4Object::ExecFire(int32_t iFireNumber, int32_t iCausedByPlr) {
  // Fire Phase
  FirePhase++;
  if (FirePhase >= MaxFirePhase) FirePhase = 0;
  // Extinguish in base
  if (!Tick5)
    if (Category & C4D_Living)
  ...
  // Decay
  if (!Def->NoBurnDecay) DoCon(-100);
  // Damage
  if (!Tick10)
    if (!Def->NoBurnDamage) DoDamage(+2, iCausedByPlr, C4FxCall_DmgFire);
  // Energy
  ...
  if (smoke_rate) {
    ...
    if (!((Game.FrameCounter + (Number * 7)) % Max<int32_t>(smoke_rate, 3)) ||
        (Abs(xdir) > 2))
  ...
  if (!Tick5) {
    ...
    if (MatValid(mat = GBackMat(x, y))) {
      ...
      if (Game.Material.Map[mat].Extinguisher) {
        Extinguish(iFireNumber);
        if (GBackLiquid(x, y)) StartSoundEffect("Pshshsh", false, 100, this);
      }
      // Inflame
      if (!Random(3)) Game.Landscape.Incinerate(x, y);
    }
  }
...
BOOL C4Object::BuyEnergy() {
  ...
  if (pPlr->Wealth < Game.C4S.Game.Realism.BaseRegenerateEnergyPrice)
    return FALSE;
...
BOOL C4Object::ExecLife() {
      ...
      if (Contained && ValidPlr(Contained->Base))
        if (!Hostile(Owner, Contained->Base))
          if (Energy < GetPhysical()->Energy)
            if (Game.C4S.Game.Realism.BaseFunctionality &
                BASEFUNC_RegenerateEnergy) {
              if (Contained->Energy <= 0) Contained->BuyEnergy();
              transfer = Min<int32_t>(
                  Min<int32_t>(2 * C4MaxPhysical / 100, Contained->Energy),
                  GetPhysical()->Energy - Energy);
              if (transfer) {
                Contained->DoEnergy(-transfer, true, C4FxCall_EngBaseRefresh,
                                    Contained->Owner);
                DoEnergy(transfer, true, C4FxCall_EngBaseRefresh,
                         Contained->Owner);
              }
            }
      ...
      if (Contained)
        if (!Hostile(Owner, Contained->Owner))
          if (MagicEnergy < GetPhysical()->Magic) {
            transfer = Min<int32_t>(Min<int32_t>(2 * MagicPhysicalFactor,
                                                 Contained->MagicEnergy),
                                    GetPhysical()->Magic - MagicEnergy) /
                       MagicPhysicalFactor;
            if (transfer) {
              // do energy transfer via script, so it can be overloaded by
              // No-Magic-Energy-rule
              // always use global func instead of local to save double search
              C4AulFunc *pMagicEnergyFn =
                  Game.ScriptEngine.GetFuncRecursive(PSF_DoMagicEnergy);
  ...
  if (!Tick5)
    if (Alive && !Def->NoBreath) {
      ...
      if (!Breathe) {
        ...
        // Asphyxiation cause is last energy loss cause player, so kill tracing
        // works when player is pushed into liquid
        if (Breath > 0)
          Breath = Max(Breath - 2 * C4MaxPhysical / 100, 0);
        else
          DoEnergy(-1, false, C4FxCall_EngAsphyxiation,
      ...
      // Supply
        ...
        if (takebreath > GetPhysical()->Breath / 2) Call(PSF_DeepBreath);
        Breath += takebreath;
      }
    }
      ...
      if (InMat != MNone)
        if (Game.Material.Map[InMat].Corrosive)
          if (!GetPhysical()->CorrosionResist)
            // Inflict corrision damage by last energy loss cause player, so
            // tumbling enemies into an acid lake attribute kills properly
            DoEnergy(-Game.Material.Map[InMat].Corrosive / 15, false,
                     C4FxCall_EngCorrosion, LastEnergyLossCausePlayer);
    ...
    if (InMat != MNone)
      if (Game.Material.Map[InMat].Incindiary)
        if (Def->ContactIncinerate)
          // Inflict fire by last energy loss cause player, so tumbling enemies
          // into a lava lake attribute kills properly
  ...
  if (!Tick10)
    if (Energy)
      if (!(Category & C4D_Living))
        if (!ValidPlr(Base) || (~Game.C4S.Game.Realism.BaseFunctionality &
                                BASEFUNC_RegenerateEnergy))
  ...
  if (!Tick255)
    if (Alive)
      if (Info) {
        int32_t iPlayingTime =
            Info->TotalPlayingTime + (Game.Time - Info->InActionTime);

        int32_t iNewAge = iPlayingTime / 3600 / 5;

        if (Info->Age != iNewAge && Info->Age) {
          // message
          sprintf(OSTR, LoadResStr("IDS_OBJ_BIRTHDAY"), GetName(),
                  Info->TotalPlayingTime / 3600 / 5);
...
BOOL C4Object::At(int32_t ctx, int32_t cty) {
  if (Status)
    if (!Contained)
      if (Def)
        if (Inside<int32_t>(cty - (y + Shape.y - addtop()), 0,
                            Shape.Hgt - 1 + addtop()))
...
void C4Object::AssignDeath(bool fForced) {
  ...
  // get death causing player before doing effect calls, because those might
  // meddle around with the flags
  ...
  // ensure objects that won't be affected by dead-plrview-decay are handled
  // properly
  ...
  Call(PSF_Death, &pars);
  // Update OCF. Done here because previously it would have been done in the
  // next frame
  // Whats worse: Having the OCF change because of some unrelated script-call
  // like
...
BOOL C4Object::ChangeDef(C4ID idNew) {
  ...
  // Pre change resets
  SetAction(ActIdle);
  Action.Act = ActIdle;  // Enforce ActIdle because SetAction may have failed
                         // due to NoOtherAction
  ...
  Def->Count--;
  // change the name to the name of the new def, if the name of the old def was
  // in use before
  ...
  // if it had been ColorByOwner, but is not now, this will be caught in
  // UpdateGraphics()
  if (!Color && ValidPlr(Owner)) Color = Game.Players.Get(Owner)->ColorDw;
  if (!Def->Rotateable) {
    r = 0;
    fix_r = rdir = Fix0;
  }
  ...
  UpdateFace(true);
  SetOCF();
  // Any effect callbacks to this object might need to reinitialize their target
  // functions
...
BOOL C4Object::Incinerate(int32_t iCausedBy, BOOL fBlasted,
  ...
  // add effect
  int32_t iEffNumber;
  C4Value Par1 = C4VInt(iCausedBy), Par2 = C4VBool(!!fBlasted),
          Par3 = C4VObj(pIncineratingObject), Par4;
  new C4Effect(this, C4Fx_Fire, C4Fx_FirePriority, C4Fx_FireTimer, NULL, 0,
               Par1, Par2, Par3, Par4, true, iEffNumber);
  return !!iEffNumber;
}
...
BOOL C4Object::Extinguish(int32_t iFireNumber) {
  ...
  if (iFireNumber) {
    ...
    // (keep checking from beginning of pEffects, as Kill might delete or change
    // effects)
...
void C4Object::DoEnergy(int32_t iChange, bool fExact, int32_t iCause,
                        int32_t iCausedByPlr) {
  // iChange 100% = Physical 100000
  if (!fExact) iChange = iChange * C4MaxPhysical / 100;
  // Was zero?
  BOOL fWasZero = (Energy == 0);
  // Mark last damage causing player to trace kills. Always update on
  // C4FxCall_EngObjHit even if iChange==0
  // so low mass objects also correctly set the killer
  if (iChange < 0 || iCause == C4FxCall_EngObjHit)
    UpdatLastEnergyLossCause(iCausedByPlr);
...
void C4Object::UpdatLastEnergyLossCause(int32_t iNewCausePlr) {
  ...
  // do not regard self-administered damage if there was a previous damage
  // causing player, because that would steal kills
  // if people tumble themselves via stop-stop-(left/right)-throw  while falling
  // into teh abyss
...
void C4Object::Blast(int32_t iLevel, int32_t iCausedBy) {
  ...
  if (Def->BlastIncinerate)
    if (Damage >= Def->BlastIncinerate) Incinerate(iCausedBy, TRUE);
}
...
void C4Object::DoCon(int32_t iChange, BOOL fInitial, bool fNoComponentChange) {
  ...
  // components, etc.
  if (iStepDiff || (Con >= FullCon) || (Con == 0) ||
    ...
    if (!fNoComponentChange) {
      ...
      if (iChange < 0) ComponentConCutoff();
    ...
    if (fWasFull && (Con < FullCon))
      if (!Def->IncompleteActivity) SetAction(ActIdle);
  } else
      // set first position
      if (fInitial)
    UpdatePos();
  ...
  } else if (Category & C4D_Structure)
    if (iStepDiff > 0) {
      // even rotated buildings need to be moved upwards
      // but by con difference, because with keep-bottom-method, they might
      // still be sinking
      // besides, moving the building up may often stabilize it
      y -= ((iLastStep + iStepDiff) * Def->Shape.Hgt / 100) -
           (iLastStep * Def->Shape.Hgt / 100);
      UpdatePos();
      UpdateSolidMask(false);
    }
...
void C4Object::DoExperience(int32_t change) {
  ...
  Info->Experience =
      BoundBy<int32_t>(Info->Experience + change, 0, MaxExperience);
...
BOOL C4Object::Exit(int32_t iX, int32_t iY, int32_t iR, FIXED iXDir,
                    FIXED iYDir, FIXED iRDir, BOOL fCalls) {
...
BOOL C4Object::Enter(C4Object *pTarget, BOOL fCalls, bool fCopyMotion,
  ...
  if (pfRejectCollect) {
    if (!!pTarget->Call(PSF_RejectCollection,
                        &C4AulParSet(C4VID(Def->id), C4VObj(this)))) {
  ...
  // Assume that the new container controls this object, if it cannot control
  // itself (i.e.: Alive)
  // So it can be traced back who caused the damage, if a projectile hits its
  // target
  if (!(Alive && (Category & C4D_Living))) Controller = pTarget->Controller;
  // Misc updates
  // motion must be copied immediately, so the position will be correct when OCF
  // is set, and
  // OCF_Available will be set for newly bought items, even if 50/50 is solid in
  // the landscape
  // however, the motion must be preserved sometimes to keep flags like
  // OCF_HitSpeed upon collection
  if (fCopyMotion) {
...
void C4Object::Fling(FIXED txdir, FIXED tydir, bool fAddSpeed,
  ...
  if (OCF & OCF_Alive)
    UpdatLastEnergyLossCause(iCausedBy);
  else if (!Contained)
    Controller = iCausedBy;
  // fling object
  if (fAddSpeed) {
    txdir += xdir / 2;
    tydir += ydir / 2;
...
BOOL C4Object::ActivateEntrance(int32_t by_plr, C4Object *by_obj) {
  ...
  if (Hostile(by_plr, Base) &&
      (Game.C4S.Game.Realism.BaseFunctionality & BASEFUNC_RejectEntrance)) {
    if (ValidPlr(Owner)) {
      sprintf(OSTR, LoadResStr("IDS_OBJ_HOSTILENOENTRANCE"),
              Game.Players.Get(Owner)->GetName());
      GameMsgObject(OSTR, this);
    }
    return FALSE;
  }
  ...
  if (OCF & OCF_Entrance)
    if (!!Call(PSF_ActivateEntrance, &C4AulParSet(C4VObj(by_obj)))) return TRUE;
  // Failure
  return FALSE;
}
...
BOOL C4Object::Build(int32_t iLevel, C4Object *pBuilder) {
  ...
  bool fNeedMaterial = (Game.Rules & C4RULE_ConstructionNeedsMaterial) ||
                       !(Category & (C4D_Structure | C4D_StaticBack));
  if (fNeedMaterial) {
        ...
        if ((pMaterial = pBuilder->Contents.Find(idMat)))
          if (!pMaterial->OnFire)
            if (pMaterial->OCF & OCF_FullCon) {
              Component.SetIDCount(idMat, Component.GetIDCount(idMat) + 1,
                                   true);
    ...
    for (cnt = 0; idMat = NeededComponents.GetID(cnt); cnt++)
      if (NeededComponents.GetCount(cnt) != 0)
        if ((100 * Component.GetIDCount(idMat) /
             NeededComponents.GetCount(cnt)) < (100 * Con / FullCon)) {
          NeededMaterial = NeededComponents.GetID(cnt);
          NeededMaterialCount =
              NeededComponents.GetCount(cnt) - Component.GetCount(cnt);
          break;
        }
  }
  ...
  if (NeededMaterialCount) {
    ...
    if (!pBuilder->Call(
      ...
      if (pBuilder->OCF & OCF_CrewMember)
        ...
        pBuilder->AddCommand(C4CMD_Acquire, NULL, 0, 0, 50, NULL, TRUE,
                             NeededMaterial, FALSE, 1);
      // ...game message if not overloaded
      Game.Messages.New(C4GM_Target, GetNeededMatStr(pBuilder), pBuilder,
                        pBuilder->Controller);
    }
  ...
  int32_t iBuildSpeed = 100;
  C4PhysicalInfo *pPhys;
  if (pBuilder)
    if (pPhys = pBuilder->GetPhysical()) {
      iBuildSpeed = pPhys->CanConstruct;
      if (!iBuildSpeed) {
        // shouldn't even have gotten here. Looks like the Clonk lost the
        // ability to build recently
        return FALSE;
      }
  ...
  // TurnTo
  if (Def->BuildTurnTo != C4ID_None) ChangeDef(Def->BuildTurnTo);
...
BOOL C4Object::Chop(C4Object *pByObject) {
  // Valid check
  if (!Status || !Def || Contained || !(OCF & OCF_Chop)) return FALSE;
  // Chop
  if (!Tick10)
    DoDamage(+10, pByObject ? pByObject->Owner : NO_OWNER, C4FxCall_DmgChop);
  return TRUE;
}
...
BOOL C4Object::CreateContentsByList(C4IDList &idlist) {
  ...
  for (cnt = 0; idlist.GetID(cnt); cnt++)
    for (cnt2 = 0; cnt2 < idlist.GetCount(cnt); cnt2++)
      if (!CreateContents(idlist.GetID(cnt))) return FALSE;
  return TRUE;
}
...
BOOL C4Object::ActivateMenu(int32_t iMenu, int32_t iMenuSelect,
  ...
  if (Menu && Menu->IsActive())
    if (!Menu->TryClose(true, false)) return FALSE;
  // Create menu
  if (!Menu)
    Menu = new C4ObjectMenu;
  else
  ...
  switch (iMenu) {
    ...
    case C4MN_Buy:
      ...
      // Init menu
      Menu->Init(fctSymbol, LoadResStr("IDS_PLR_NOBUY"), this, C4MN_Extra_Value,
                 0, iMenu);
      Menu->SetPermanent(TRUE);
      Menu->SetRefillObject(pTarget);
      // Success
    ...
    case C4MN_Construction:
      ...
      for (cnt = 0; pDef = C4Id2Def(pPlayer->Knowledge.GetID(
        ...
        // Picture
        fctSymbol.Set(pDef->Graphics.GetBitmap(), pDef->PictureRect.x,
        ...
        sprintf(szCommand, "SetCommand(this,\"Construct\",0,0,0,0,%s)",
                C4IdText(pDef->id));
        // Add menu item
        Menu->AddRefSym(szCaption, fctSymbol, szCommand, C4MN_Item_NoCount,
                        NULL, pDef->GetDesc(), pDef->id);
      }
      // Preselect
      Menu->SetSelection(iMenuSelect, false, true);
      Menu->SetPosition(iMenuPosition);
    ...
    case C4MN_Info:
      ...
      // Add info item
      fctSymbol.Create(C4PictureSize, C4PictureSize);
      pTarget->Def->Draw(fctSymbol, FALSE, pTarget->Color, pTarget);
      Menu->Add(pTarget->GetName(), fctSymbol, "", C4MN_Item_NoCount, NULL,
                pTarget->GetInfoString().getData());
      fctSymbol.Default();
      // Success
      return TRUE;
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
BOOL C4Object::CloseMenu(bool fForce) {
  if (Menu) {
    if (Menu->IsActive())
      if (!Menu->TryClose(fForce, false)) return FALSE;
    if (!Menu->IsCloseQuerying()) {
      delete Menu;
      Menu = NULL;
    }  // protect menu deletion from recursive menu operation calls
  }
  return TRUE;
}
    ...
    if (Contained && Contained->Def->AutoContextMenu)
      ...
      if (OCF & OCF_CrewMember) {
        ...
        if (pPlayer && pPlayer->AutoContextMenu) {
          ...
          Menu->SetCloseCommand(
              "PlayerObjectCommand(GetOwner(), \"Exit\") && ExecuteCommand()");
        }
      }
}
...
BYTE C4Object::GetEntranceArea(int32_t &aX, int32_t &aY, int32_t &aWdt,
                               int32_t &aHgt) {
  if (!Status || !Def) return 0;
...
const char *C4Object::GetName() { return Name.getData(); }
...
int32_t C4Object::GetValue(C4Object *pInBase, int32_t iForPlayer) {
  ...
  if (C4AulScriptFunc *f = Def->Script.SFn_CalcValue)
    iValue = f->Exec(this, &C4AulParSet(C4VObj(pInBase), C4VInt(iForPlayer)))
                 .getInt();
  else {
    // get value of def
    // Caution: Do not pass pInBase here, because the def base value is to be
    // queried
  ...
  if (pInBase) {
    ...
    if (pFn = pInBase->Def->Script.GetSFunc(PSF_CalcSellValue, AA_PROTECTED))
      iValue = pFn->Exec(pInBase, &C4AulParSet(C4VObj(this), C4VInt(iValue)))
                   .getInt();
  }
  // Return value
  return iValue;
...
C4PhysicalInfo *C4Object::GetPhysical(bool fPermanent) {
  ...
  if (Info)
    if (!Game.Parameters.UseFairCrew)
      ...
      // Maybe some time it will be possible to have crew infos that aren't tied
...
bool C4Object::TrainPhysical(C4PhysicalInfo::Offset mpiOffset, int32_t iTrainBy,
                             int32_t iMaxTrain) {
  int i = 0;
  // Train temp
...
BOOL C4Object::Promote(int32_t torank, BOOL exception, bool fForceRankName) {
  ...
  GameMsgObject(FormatString(LoadResStr("IDS_OBJ_PROMOTION"), GetName(),
                             Info->sRankName.getData()).getData(),
                this);
...
void C4Object::ClearPointers(C4Object *pObj) {
  ...
  // contents/contained: not necessary, because it's done in AssignRemoval and
  // StatusDeactivate
  // Action targets
  ...
  // Commands
  C4Command *cCom;
  for (cCom = Command; cCom; cCom = cCom->Next) cCom->ClearPointers(pObj);
  // Menu
...
C4Value C4Object::Call(const char *szFunctionCall, C4AulParSet *pPars,
                       bool fPassError) {
...
void C4Object::Draw(C4FacetEx &cgo, int32_t iByPlayer, DrawMode eDrawMode) {
  ...
  if (eDrawMode != ODM_Overlay) TargetPos(cotx, coty, cgo);
  int32_t cox = x + Shape.x - cotx, coy = y + Shape.y - coty;

  BOOL fYStretchObject = FALSE;
  if (Action.Act > ActIdle)
    if (Def->ActMap[Action.Act].FacetTargetStretch) fYStretchObject = TRUE;
  ...
  if (!fYStretchObject && !eDrawMode)
    if (Action.Act > ActIdle && !r && !Def->ActMap[Action.Act].FacetBase &&
        Con <= FullCon) {
      // active
  ...
  if (Game.GraphicsSystem.ShowCommand && !eDrawMode) {
    ...
    for (pCom = Command; pCom; pCom = pCom->Next) {
      switch (pCom->Command) {
        case C4CMD_MoveTo:
          // Angle
          int32_t iAngle;
          iAngle = Angle(ccx, ccy, pCom->Tx._getInt(), pCom->Ty);
          while (iAngle > 180) iAngle -= 360;
          // Path
          x1 = ccx - cotx;
          y1 = ccy - coty;
          x2 = pCom->Tx._getInt() - cotx;
          y2 = pCom->Ty - coty;
          Application.DDraw->DrawLine(cgo.Surface, cgo.X + x1, cgo.Y + y1,
                                      cgo.X + x2, cgo.Y + y2, CRed);
          Application.DDraw->DrawFrame(cgo.Surface, cgo.X + x2 - 1,
                                       cgo.Y + y2 - 1, cgo.X + x2 + 1,
                                       cgo.Y + y2 + 1, CRed);
          if (x1 > x2) Swap(x1, x2);
          if (y1 > y2) Swap(y1, y2);
          ccx = pCom->Tx._getInt();
        ...
        case C4CMD_Put:
          sprintf(szCommand, "%s %s to %s", CommandName(pCom->Command),
                  pCom->Target2 ? pCom->Target2->GetName()
                                : pCom->Data ? C4IdText(pCom->Data) : "Content",
        ...
        case C4CMD_Call:
          sprintf(szCommand, "%s %s in %s", CommandName(pCom->Command),
                  pCom->Text,
                  pCom->Target ? pCom->Target->GetName() : "(null)");
          break;
      ...
      if (szCommand[0]) {
        ...
        if (iMoveTos) {
          Cmds.AppendChar('|');
          Cmds.AppendFormat("%dx MoveTo", iMoveTos);
          iMoveTos = 0;
        }
        // Current message
        Cmds.AppendChar('|');
    ...
    // Draw message
    int32_t cmwdt, cmhgt;
    Game.GraphicsResource.FontRegular.GetTextExtent(Cmds.getData(), cmwdt,
                                                    cmhgt, true);
    Application.DDraw->TextOut(
        Cmds.getData(), Game.GraphicsResource.FontRegular, 1.0, cgo.Surface,
        cgo.X + cox - Shape.x, cgo.Y + coy - 10 - cmhgt,
  ...
  if (Game.GraphicsSystem.ShowSolidMask)
    if (SolidMask.Wdt) {
      // DrawSolidMask(cgo); - no need to draw it, because the 8bit-surface will
      // be shown
      return;
    }
  ...
  if (OnFire /*&& !Game.Particles.IsFireParticleLoaded()*/)
      ...
      // Regular action facet
        ...
        if (r != 0 && Def->Rotateable) {
          ...
          if (Def->GrowthType || Con == FullCon) {
            ...
            C4DrawTransform rot;
            if (pDrawTransform) {
              rot.SetTransformAt(*pDrawTransform,
                                 cgo.X + cox + float(Shape.Wdt) / 2,
                                 cgo.Y + coy + float(Shape.Hgt) / 2);
              rot.Rotate(r * 100, float(cgo.X + cox + Shape.Wdt / 2),
                         float(cgo.Y + coy + Shape.Hgt / 2));
            } else
              rot.SetRotate(r * 100, float(cgo.X + cox + Shape.Wdt / 2),
                            float(cgo.Y + coy + Shape.Hgt / 2));
            // draw stretched towards shape center with transform
            Action.Facet.DrawXT(
                cgo.Surface, (Def->Shape.x + Action.FacetX) * Con / FullCon +
                                 cgo.X + cox - Shape.x,
                (Def->Shape.y + Action.FacetY) * Con / FullCon + cgo.Y + coy -
                    Shape.y,
                Action.Facet.Wdt * Con / FullCon,
                Action.Facet.Hgt * Con / FullCon, iPhase, Action.DrawDir, &rot);
          } else {
          ...
          if (Con == FullCon)
            Action.Facet.DrawT(
                cgo.Surface, cgo.X + cox + Action.FacetX,
                cgo.Y + coy + Action.FacetY, iPhase, Action.DrawDir,
                pDrawTransform
                    ? &C4DrawTransform(*pDrawTransform,
                                       (float)Shape.Wdt / 2 + cgo.X + cox,
                                       (float)Shape.Hgt / 2 + cgo.Y + coy)
                    : NULL);
          // Growth strechted
          else
            Action.Facet.DrawXT(
                cgo.Surface, cgo.X + cox, cgo.Y + coy, Shape.Wdt, Shape.Hgt,
  ...
  if (pGfxOverlay)
    if (eDrawMode != ODM_BaseOnly)
      for (C4GraphicsOverlay *pGfxOvrl = pGfxOverlay; pGfxOvrl;
           pGfxOvrl = pGfxOvrl->GetNext())
        if (!pGfxOvrl->IsPicture()) pGfxOvrl->Draw(cgo, this, iByPlayer);

  // local particles in front of the object
  if (FrontParticles)
    if (eDrawMode != ODM_BaseOnly) FrontParticles.Draw(cgo, this);
  ...
  if (Select)
    if (eDrawMode != ODM_BaseOnly)
      if (ValidPlr(Owner))
        if (Owner == iByPlayer)
          if (Game.Players.Get(Owner)->SelectFlash) DrawSelectMark(cgo);
  ...
  if (NeedEnergy)
    if (Tick35 > 12)
      if (eDrawMode != ODM_BaseOnly) {
        ...
        int32_t tx = cox + Shape.Wdt / 2 - fctEnergy.Wdt / 2,
      ...
      if (Shape.VtxNum > 1)
        for (cnt = 0; cnt < Shape.VtxNum; cnt++) {
          DrawVertex(cgo, cox - Shape.x + Shape.VtxX[cnt],
                     ...
                     (Shape.VtxCNAT[cnt] & CNAT_NoCollision)
                         ? CBlue
                         : (Mobile ? CRed : CYellow),
  ...
  if (Game.GraphicsSystem.ShowAction)
    if (eDrawMode != ODM_BaseOnly) {
      if (Action.Act > ActIdle) {
        ...
        int32_t cmwdt, cmhgt;
        Game.GraphicsResource.FontRegular.GetTextExtent(OSTR, cmwdt, cmhgt,
                                                        true);
        Application.DDraw->TextOut(
            OSTR, Game.GraphicsResource.FontRegular, 1.0, cgo.Surface,
            cgo.X + cox - Shape.x, cgo.Y + coy - cmhgt,
...
void C4Object::DrawTopFace(C4FacetEx &cgo, int32_t iByPlayer,
                           DrawMode eDrawMode) {
  // Status
  if (!Status || !Def) return;
  // visible?
  ...
  if (OCF & OCF_CrewMember)
    if ((Config.Graphics.ShowCrewNames || Config.Graphics.ShowCrewCNames) &&
        (!Game.C4S.Head.Film || !Game.C4S.Head.Replay))
      if (!eDrawMode)
        if (Owner != iByPlayer && !Contained) {
          // inside screen range?
          if (!Inside(x + Shape.x - cotx, 1 - Shape.Wdt, cgo.Wdt) ||
              !Inside(y + Shape.y - coty, 1 - Shape.Hgt, cgo.Hgt))
            return;
          // get player
          ...
          if (pOwner)
            if (!Hostile(Owner, iByPlayer))
              if (!pOwner->IsInvisible()) {
                ...
                Game.GraphicsResource.FontRegular.GetTextExtent("m", iCharWdt,
                                                                dummy, false);
                ...
                Game.GraphicsResource.FontRegular.GetTextExtent(szText, iTWdt,
                                                                iTHgt, true);
                iTX =
                    BoundBy<int32_t>(X - cotx, iTWdt / 2, cgo.Wdt - iTWdt / 2);
                iTY = BoundBy<int32_t>(Y - coty - iTHgt, 0, cgo.Hgt - iTHgt);
                // Draw
                Application.DDraw->TextOut(
                    szText, Game.GraphicsResource.FontRegular, 1.0, cgo.Surface,
                    cgo.X + iTX, cgo.Y + iTY, pOwner->ColorDw | 0x7f000000,
                    ACenter);
              }
        }
  // TopFace
  ...
  // Don't draw (show solidmask)
  if (Game.GraphicsSystem.ShowSolidMask)
    if (SolidMask.Wdt) return;
  // Contained
  if (Contained)
    if (eDrawMode != ODM_Overlay) return;
  // Construction sign
  ...
  if ((Action.Act > ActIdle) && Def->ActMap[Action.Act].FacetTopFace) {
    ...
    TopFace.Y =
        actdef->Facet.y + Def->TopFace.y + actdef->Facet.Hgt * Action.DrawDir;
  }
  // ensure correct color is set
  if (GetGraphics()->BitmapClr) GetGraphics()->BitmapClr->SetClr(Color);
  // color modulation (including construction sign...)
  ...
  if (Con != FullCon && Def->GrowthType)
    // stretched
    TopFace.DrawXT(cgo.Surface, cgo.X + cox + Def->TopFace.tx * Con / FullCon,
                   cgo.Y + coy + Def->TopFace.ty * Con / FullCon,
                   TopFace.Wdt * Con / FullCon, TopFace.Hgt * Con / FullCon, 0,
                   0, pDrawTransform
                          ? &C4DrawTransform(*pDrawTransform,
                                             cgo.X + cox + float(Shape.Wdt) / 2,
                                             cgo.Y + coy + float(Shape.Hgt) / 2)
    ...
    // normal
    TopFace.DrawT(cgo.Surface, cgo.X + cox + Def->TopFace.tx,
                  cgo.Y + coy + Def->TopFace.ty, 0, 0,
                  pDrawTransform
                      ? &C4DrawTransform(*pDrawTransform,
                                         cgo.X + cox + float(Shape.Wdt) / 2,
                                         cgo.Y + coy + float(Shape.Hgt) / 2)
...
void C4Object::DrawLine(C4FacetEx &cgo) {
  ...
  SetAudibilityAt(cgo, Shape.VtxX[Shape.VtxNum - 1],
                  Shape.VtxY[Shape.VtxNum - 1]);
  // additive mode?
  PrepareDrawing();
  // Draw line segments
  for (int32_t vtx = 0; vtx + 1 < Shape.VtxNum; vtx++) switch (Def->Line) {
      case C4D_Line_Power:
      ...
      case C4D_Line_Lightning:
        cgo.DrawBolt(Shape.VtxX[vtx], Shape.VtxY[vtx], Shape.VtxX[vtx + 1],
                     Shape.VtxY[vtx + 1], CWhite, CWhite);
        break;
      case C4D_Line_Rope:
        cgo.DrawLine(Shape.VtxX[vtx], Shape.VtxY[vtx], Shape.VtxX[vtx + 1],
      ...
      case C4D_Line_Colored:
        cgo.DrawLine(Shape.VtxX[vtx], Shape.VtxY[vtx], Shape.VtxX[vtx + 1],
                     Shape.VtxY[vtx + 1], BYTE(Local[0].getInt()),
                     BYTE(Local[1].getInt()));
        break;
    }
  // reset blit mode
...
void C4Object::DrawEnergy(C4Facet &cgo) {
  ...
  cgo.DrawEnergyLevelEx(Energy, GetPhysical()->Energy,
                        Game.GraphicsResource.fctEnergyBars, 0);
}

void C4Object::DrawMagicEnergy(C4Facet &cgo) {
  // draw in units of MagicPhysicalFactor, so you can get a full magic energy
  // bar by script even if partial magic energy training is not fulfilled
  // cgo.DrawEnergyLevel(MagicEnergy/MagicPhysicalFactor,GetPhysical()->Magic/MagicPhysicalFactor,39);
  cgo.DrawEnergyLevelEx(MagicEnergy / MagicPhysicalFactor,
                        GetPhysical()->Magic / MagicPhysicalFactor,
                        Game.GraphicsResource.fctEnergyBars, 1);
}

void C4Object::DrawBreath(C4Facet &cgo) {
  // cgo.DrawEnergyLevel(Breath,GetPhysical()->Breath,99);
  cgo.DrawEnergyLevelEx(Breath, GetPhysical()->Breath,
                        Game.GraphicsResource.fctEnergyBars, 2);
}
...
void C4Object::CompileFunc(StdCompiler *pComp) {
  ...
  if (fCompiler) {
    ...
    if (!Def) {
      pComp->excNotFound(LoadResStr("IDS_PRC_UNDEFINEDOBJECT"), C4IdText(id));
      return;
    }
  }

  // Write the name only if the object has an individual name, use def name as
  // default for reading.
  ...
  pComp->Value(
      mkNamingAdapt(LastEnergyLossCausePlayer, "LastEngLossPlr", NO_OWNER));
  ...
  pComp->Value(
      mkNamingAdapt(iLastAttachMovementFrame, "LastSolidAtchFrame", -1));
  ...
  pComp->Value(
      mkNamingAdapt(C4DefGraphicsAdapt(pGraphics), "Graphics", &Def->Graphics));
  pComp->Value(mkNamingPtrAdapt(pDrawTransform, "DrawTransform"));
  pComp->Value(mkNamingPtrAdapt(pEffects, "Effects"));
  pComp->Value(mkNamingAdapt(C4GraphicsOverlayListAdapt(pGfxOverlay),
                             "GfxOverlay", (C4GraphicsOverlay *)NULL));
  ...
  if (pComp->FollowName("Commands"))
    if (fCompiler) {
      ...
      for (int i = 1;; i++) {
        ...
        pComp->Value(
            mkNamingPtrAdapt(pCmd ? pCmd->Next : Command, Naming.getData()));
        // Last command?
  ...
  if (fCompiler) {
    ...
    if (OnFire && !pEffects)
      new C4Effect(this, C4Fx_Fire, C4Fx_FirePriority, C4Fx_FireTimer, NULL, 0,
                   Par1, Par2, Par3, Par4, false, iFireNumber);

    // blit mode not assigned? use definition default then
    if (!BlitMode) BlitMode = Def->BlitMode;

    // object needs to be resorted? May happen if there's unsorted objects in
    // savegame
...
bool DrawCommandQuery(int32_t controller, C4ScriptHost &scripthost,
                      int32_t *mask, int com) {
  int method = scripthost.GetControlMethod(com, mask[0], mask[1]);
  C4Player *player = Game.Players.Get(controller);
  if (!player) return false;

  switch (method) {
    case C4AUL_ControlMethod_All:
      return true;
    case C4AUL_ControlMethod_None:
      return false;
    case C4AUL_ControlMethod_Classic:
      return !player->ControlStyle;
    case C4AUL_ControlMethod_JumpAndRun:
      return !!player->ControlStyle;
    default:
      return false;
  }
...
void C4Object::DrawCommands(C4Facet &cgoBottom, C4Facet &cgoSide,
  ...
  if (Action.ComDir == COMD_Stop && iDFA == DFA_WALK &&
      (tObj = Game.Objects.AtObject(x, y, ocf, this))) {
    ...
    tObj->DrawCommand(cgoBottom, C4FCT_Right, NULL, com, pRegions, Owner, OSTR,
                      &ccgo);
    tObj->Def->Draw(ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Right, C4FCT_Top),
                    FALSE, tObj->Color, tObj);
    Game.GraphicsResource.fctBuild.Draw(
        ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Left, C4FCT_Bottom), TRUE);
  }

  // Grab target control (control flag)
  if (iDFA == DFA_PUSH && Action.Target) {
    for (cnt = ComOrderNum - 1; cnt >= 0; cnt--)
      if (DrawCommandQuery(Controller, Action.Target->Def->Script,
                           Action.Target->Def->Script.ControlMethod, cnt)) {
        Action.Target->DrawCommand(cgoBottom, C4FCT_Right, PSF_Control,
                                   ComOrder(cnt), pRegions, Owner);
      } else if (ComOrder(cnt) == COM_Down_D) {
        // Let Go
        sprintf(OSTR, LoadResStr("IDS_CON_UNGRAB"), Action.Target->GetName());
        Action.Target->DrawCommand(cgoBottom, C4FCT_Right, NULL, COM_Down_D,
                                   pRegions, Owner, OSTR, &ccgo);
        Action.Target->Def->Draw(
            ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Right, C4FCT_Top), FALSE,
            Action.Target->Color, Action.Target);
        Game.GraphicsResource.fctHand.Draw(
            ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Left, C4FCT_Bottom), TRUE,
            6);
        ...
        if ((tObj = Contents.GetObject()) &&
          ...
          sprintf(OSTR, LoadResStr("IDS_CON_PUT"), tObj->GetName(),
  ...
  if (Contained) {
    for (cnt = ComOrderNum - 1; cnt >= 0; cnt--)
      if (DrawCommandQuery(Controller, Contained->Def->Script,
                           Contained->Def->Script.ContainedControlMethod,
                           cnt)) {
        Contained->DrawCommand(cgoBottom, C4FCT_Right, PSF_ContainedControl,
                               ComOrder(cnt), pRegions, Owner);
        // Contained control down overrides contained exit control
        if (Com2Control(ComOrder(cnt)) == CON_Down)
          fContainedDownOverride = TRUE;
        // carlo - Contained controls left/right override contained Take, Take2
        // controls
        if (Com2Control(ComOrder(cnt)) == CON_Left)
          fContainedLeftOverride = TRUE;
        if (Com2Control(ComOrder(cnt)) == CON_Right)
          fContainedRightOverride = TRUE;
      }
    // Contained exit
    ...
    if (nContents) {
      ...
      if (!fContainedRightOverride) {
        ...
        Contained->DrawCommand(cgoBottom, C4FCT_Right, NULL, COM_Right,
                               pRegions, Owner, OSTR, &ccgo);
        Contained->Def->Draw(
            ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Right, C4FCT_Top), FALSE,
            Contained->Color, Contained);
        Game.GraphicsResource.fctHand.Draw(
            ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Left, C4FCT_Bottom), TRUE,
            1);
      }
      ...
      if (!fContainedLeftOverride) {
        ...
        Contained->DrawCommand(cgoBottom, C4FCT_Right, NULL, COM_Left, pRegions,
                               Owner, OSTR, &ccgo);
        Contained->Def->Draw(
            ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Right, C4FCT_Top), FALSE,
            Contained->Color, Contained);
    ...
    if (tObj = Contents.GetObject()) {
      ...
      tObj->Def->Draw(ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Right, C4FCT_Top),
                      FALSE, tObj->Color, tObj);
      Game.GraphicsResource.fctHand.Draw(
          ccgo2 = ccgo.GetFraction(85, 85, C4FCT_Left, C4FCT_Bottom), TRUE, 0);
    } else if (nContents) {
      // Get
      sprintf(OSTR, LoadResStr("IDS_CON_ACTIVATEFROM"), Contained->GetName());
      Contained->DrawCommand(cgoBottom, C4FCT_Right, NULL, COM_Throw, pRegions,
  ...
  if (!Contained) {
    if ((iDFA == DFA_WALK) || (iDFA == DFA_SWIM) || (iDFA == DFA_DIG))
      if (tObj = Contents.GetObject())
        if (DrawCommandQuery(Controller, tObj->Def->Script,
                             tObj->Def->Script.ActivationControlMethod,
                             COM_Dig_D)) {
    ...
    if (!fContentsActivationOverride)
      if ((iDFA == DFA_WALK) || (iDFA == DFA_SWIM) || (iDFA == DFA_FLOAT))
        if (DrawCommandQuery(Controller, Def->Script,
                             Def->Script.ActivationControlMethod, COM_Dig_D))
  ...
  for (cnt = 6; cnt < ComOrderNum; cnt++) {
    ...
    if (DrawCommandQuery(Controller, Def->Script, Def->Script.ControlMethod,
                         cnt))
...
void C4Object::Picture2Facet(C4FacetExSurface &cgo) {
  ...
  C4Facet fctPicture;
  fctPicture.Set(GetGraphics()->GetBitmap(Color), fctPicRect.x, fctPicRect.y,
                 fctPicRect.Wdt, fctPicRect.Hgt);
  ...
  if (pGfxOverlay)
    for (C4GraphicsOverlay *pGfxOvrl = pGfxOverlay; pGfxOvrl;
         pGfxOvrl = pGfxOvrl->GetNext())
      if (pGfxOvrl->IsPicture()) pGfxOvrl->DrawPicture(cgo, this);
...
BOOL C4Object::ValidateOwner() {
  ...
  // Color is not reset any more, because many scripts change colors to
  // non-owner-colors these days
  // Additionally, player colors are now guarantueed to remain the same in
  // savegame resumes
  return TRUE;
}
...
void C4Object::ClearInfo(C4ObjectInfo *pInfo) {
  if (Info == pInfo) {
    if (Info)
      if (Name.getData() == Info->Name) Name = Def->Name;
    Info = NULL;
  }
}
...
BOOL C4Object::ContainedControl(BYTE byCom) {
  ...
  C4AulFunc *sf = Contained->Def->Script.GetSFunc(
      FormatString(PSF_ContainedControl, ComName(byCom)).getData());
  // in old versions, do hardcoded actions first (until gwe3)
  // new objects may overload them
  C4Def *pCDef = Contained->Def;
  bool fCallSfEarly =
      CompareVersion(pCDef->rC4XVer[0], pCDef->rC4XVer[1], pCDef->rC4XVer[2],
                     pCDef->rC4XVer[3], 4, 9, 1, 3) >= 0;
  bool result = false;
  ...
  if (fCallSfEarly) {
    ...
    if (Contained && !(byCom & (COM_Single | COM_Double)) &&
        pPlr->ControlStyle) {
      int32_t PressedComs = pPlr->PressedComs;
      C4AulParSet set(C4VObj(this), C4VInt(Coms2ComDir(PressedComs)),
...
BOOL C4Object::CallControl(C4Player *pPlr, BYTE byCom, C4AulParSet *pPars) {
  ...
  bool result =
      !!Call(FormatString(PSF_Control, ComName(byCom)).getData(), pPars);
...
void C4Object::DirectCom(BYTE byCom, int32_t iData)  // By player ObjectCom
{
  ...
  // COM_Special and COM_Contents specifically bypass the menu and always go to
  // the object
  ...
  // contents shift must always be done to container object, which is not
  // necessarily this
  ...
  if (Contained)
    if (byCom != COM_Special && byCom != COM_Special2 && byCom != COM_WheelUp &&
        byCom != COM_WheelDown) {
  ...
  if (pController = Game.Players.Get(Controller))
    if (CallControl(pController, byCom)) return;

  // direct wheel control
  if (byCom == COM_WheelUp || byCom == COM_WheelDown)
  // scroll contents
  {
    ShiftContents(byCom == COM_WheelUp, TRUE);
    return;
  }

  // The Player updates Controller before calling this, so trust Players.Get
  // will return it
  ...
  switch (GetProcedure()) {
      ...
      switch (byCom) {
        ...
        case COM_Dig_S:
          if (ObjectComDig(this)) {
            Action.ComDir =
                (Action.Dir == DIR_Right) ? COMD_DownRight : COMD_DownLeft;
          }
          break;
        case COM_Dig_D:
    ...
    case DFA_DIG:
      switch (byCom) {
        case COM_Left:
          if (Inside<int32_t>(Action.ComDir, COMD_UpRight, COMD_Left))
            Action.ComDir++;
          break;
        case COM_Right:
          if (Inside<int32_t>(Action.ComDir, COMD_Right, COMD_UpLeft))
            Action.ComDir--;
          break;
        case COM_Down:
          ObjectComStop(this);
    ...
    case DFA_PUSH: {
      ...
      if (Action.Target) {
        // Make sure controller is up-to-date, so if e.g. multiple people push a
        // catapult the controller is correct for whoever issued the
        // ControlThrow
        ...
        if (CompareVersion(pTDef->rC4XVer[0], pTDef->rC4XVer[1],
                           pTDef->rC4XVer[2], pTDef->rC4XVer[3], 4, 9, 5,
                           0) >= 0)
      ...
      if (fGrabControlOverload)
        if (Action.Target)
          if (Action.Target->CallControl(pController, byCom,
                                         &C4AulParSet(C4VObj(this))))
            return;
      ...
      switch (byCom) {
        ...
        case COM_Up:
          // Target -> enter
          if (ObjectComEnter(Action.Target)) ObjectComMovement(this, COMD_Stop);
      ...
      if (!fGrabControlOverload)
        if (Action.Target)
          Action.Target->CallControl(pController, byCom,
                                     &C4AulParSet(C4VObj(this)));
      break;
    }
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  switch (GetProcedure()) {
    case DFA_WALK:
      switch (byCom) {
        ...
        case COM_Down:
          ...
          if (ObjectComDownDouble(this)) pPlayer->LastCom = COM_None;
          break;
        case COM_Dig_S:
          ObjectComDig(this);
          break;
    ...
    case DFA_FLIGHT:
      switch (byCom) {
        case COM_Throw:
          ...
          if (pPlayer->PressedComs &
              ((1 << COM_Left) | (1 << COM_Right) | (1 << COM_Down)))
            PlayerObjectCommand(Owner, C4CMD_Drop);
          else
    ...
    case DFA_SCALE:
      switch (byCom) {
        ...
        case COM_Right:
          ...
          break;
        case COM_Dig:
          ObjectComLetGo(this, (Action.Dir == DIR_Left) ? +1 : -1);
        case COM_Throw:
          PlayerObjectCommand(Owner, C4CMD_Drop);
          break;
        default:
    ...
    case DFA_PUSH: {
      ...
      if (Action.Target) {
        ...
        // Call object control first in case it overloads
        if (fGrabControlOverload) {
          if (Action.Target->CallControl(pPlayer, byCom,
                                         &C4AulParSet(C4VObj(this)))) {
            return;
          }
        }
      ...
      switch (byCom) {
        ...
        case COM_Down:
          ...
          // ComOrder(3) is COM_Down, ComOrder(11) is COM_Down_S and
          // ComOrder(19) is COM_Down_D
          if (Action.Target &&
              !DrawCommandQuery(Controller, Action.Target->Def->Script,
                                Action.Target->Def->Script.ControlMethod, 3) &&
              !DrawCommandQuery(Controller, Action.Target->Def->Script,
                                Action.Target->Def->Script.ControlMethod, 11) &&
              !DrawCommandQuery(Controller, Action.Target->Def->Script,
                                Action.Target->Def->Script.ControlMethod, 19)) {
            ObjectComUnGrab(this);
          }
          break;
        case COM_Down_D:
...
C4Object *C4Object::ComposeContents(C4ID id) {
  ...
  StdStrBuf Needs;
  Needs.Format(LoadResStr("IDS_CON_BUILDMATNEED"), pDef->GetName());
  for (cnt = 0; c_id = NeededComponents.GetID(cnt); cnt++)
    if (NeededComponents.GetCount(cnt) > Contents.ObjectCount(c_id)) {
      Needs.AppendFormat(
          "|%ix %s",
          NeededComponents.GetCount(cnt) - Contents.ObjectCount(c_id),
          C4Id2Def(c_id) ? C4Id2Def(c_id)->GetName() : C4IdText(c_id));
      if (!idNeeded) {
        idNeeded = c_id;
        iNeeded = NeededComponents.GetCount(cnt) - Contents.ObjectCount(c_id);
      }
      fInsufficient = TRUE;
    }
  // Insufficient
  if (fInsufficient) {
    ...
    if (!Call(PSF_BuildNeedsMaterial,
              &C4AulParSet(C4VID(idNeeded), C4VInt(iNeeded))))
      // ...game message if not overloaded
  ...
  // this is done because some objects (e.g. arrow packs) will set custom
  // components during initialization, which should not be overriden
  return CreateContents(id);
}

void C4Object::SetSolidMask(int32_t iX, int32_t iY, int32_t iWdt, int32_t iHgt,
...
bool C4Object::CheckSolidMaskRect() {
  ...
  SolidMask.Set(Max<int32_t>(SolidMask.x, 0), Max<int32_t>(SolidMask.y, 0),
                Min<int32_t>(SolidMask.Wdt, sfcGraphics->Wdt - SolidMask.x),
                Min<int32_t>(SolidMask.Hgt, sfcGraphics->Hgt - SolidMask.y),
...
void C4Object::DrawSelectMark(C4FacetEx &cgo) {
  ...
  // target pos (parallax)
  int32_t cotx = cgo.TargetX, coty = cgo.TargetY;
  TargetPos(cotx, coty, cgo);
  // Output boundary
  if (!Inside<int32_t>(x - cotx, 0, cgo.Wdt - 1) ||
      !Inside<int32_t>(y - coty, 0, cgo.Hgt - 1))
    return;
  // Draw select marks
...
bool C4Object::AddCommand(int32_t iCommand, C4Object *pTarget, C4Value iTx,
                          int32_t iTy, int32_t iUpdateInterval,
                          C4Object *pTarget2, bool fInitEvaluation,
                          int32_t iData, bool fAppend, int32_t iRetries,
                          const char *szText, int32_t iBaseMode) {
  ...
  pCom->Set(iCommand, this, pTarget, iTx, iTy, pTarget2, iData, iUpdateInterval,
            !fInitEvaluation, iRetries, szText, iBaseMode);
...
void C4Object::SetCommand(int32_t iCommand, C4Object *pTarget, C4Value iTx,
                          int32_t iTy, C4Object *pTarget2, BOOL fControl,
                          int32_t iData, int32_t iRetries, const char *szText) {
  // Decrease NoCollectDelay
  if (NoCollectDelay > 0) NoCollectDelay--;
  // Clear stack
  ClearCommands();
  ...
  // Script overload
  if (fControl)
    if (!!Call(PSF_ControlCommand,
               &C4AulParSet(C4VString(CommandName(iCommand)), C4VObj(pTarget),
                            iTx, C4VInt(iTy), C4VObj(pTarget2), C4VInt(iData))))
      return;
  ...
  if (GetProcedure() == DFA_PUSH)
    if (Action.Target)
      if (Action.Target->Def->VehicleControl & C4D_VehicleControl_Outside) {
        Action.Target->Controller = Controller;
        if (!!Action.Target->Call(
                PSF_ControlCommand,
                &C4AulParSet(C4VString(CommandName(iCommand)), C4VObj(pTarget),
                             iTx, C4VInt(iTy), C4VObj(pTarget2),
                             C4VInt(iData))))
          return;
      }
  // Add new command
  AddCommand(iCommand, pTarget, iTx, iTy, 0, pTarget2, TRUE, iData, FALSE,
             iRetries, szText, C4CMD_Mode_Base);
}
...
BOOL C4Object::ExecuteCommand() {
  ...
  if (Command && Command->Finished)
    Call(PSF_ControlCommandFinished,
         &C4AulParSet(C4VString(CommandName(Command->Command)),
                      C4VObj(Command->Target), Command->Tx, C4VInt(Command->Ty),
                      C4VObj(Command->Target2),
                      C4Value(Command->Data, C4V_Any)));
        ...
        if (Game.Material.Map[iMaterial].Dig2ObjectRatio != 0)
          if (fRequest || !Game.Material.Map[iMaterial].Dig2ObjectOnRequestOnly)
            if (MaterialContents->Amount[iMaterial] >=
              ...
              Game.CreateObject(Game.Material.Map[iMaterial].Dig2Object, this,
...
void C4Object::DrawCommand(C4Facet &cgoBar, int32_t iAlign,
  ...
  if (pPlr = Game.Players.Get(Owner))
    if (iCom == pPlr->FlashCom) fFlash = TRUE;
  ...
  if (pfctImage) *pfctImage = cgoRight;
  // Specified def
  else if (pDescImageDef)
    pDescImageDef->Draw(
        cgoRight, FALSE, Color, NULL,
        iDescImagePhase);  // ...use specified color, but not object.
  // Contents image
  ...
  if (!fFlash || Tick35 > 15)
    DrawCommandKey(
        ...
        Config.Graphics.ShowCommandKeys
            ? PlrControlKeyName(iPlayer, Com2Control(iCom), true).getData()
            : NULL);
...
BOOL C4Object::SetAction(int32_t iAct, C4Object *pTarget, C4Object *pTarget2,
  ...
  if (iLastAction > ActIdle)
    if (Def->ActMap[iLastAction].NoOtherAction && !fForce)
      if (iAct != iLastAction) return FALSE;

  // Invalid action
  if (Def && !Inside<int32_t>(iAct, ActIdle, Def->ActNum - 1)) return FALSE;
  ...
  if (Con < FullCon)
    if (!Def->IncompleteActivity) iAct = ActIdle;
  ...
  if (iAct != iLastAction) {
    ...
    if (((iAct > ActIdle) ? Def->ActMap[iAct].Procedure : DFA_NONE) !=
        ((iLastAction > ActIdle) ? Def->ActMap[iLastAction].Procedure
                                 : DFA_NONE))
      Action.Data = 0;
  }
  ...
  if (((iLastAction > ActIdle) ? Def->ActMap[iLastAction].FlipDir : 0) !=
      ((iAct > ActIdle) ? Def->ActMap[iAct].FlipDir : 0))
    UpdateFlipDir();
...
void C4Object::UpdateActionFace() {
  ...
  if (Action.Act > ActIdle) {
    ...
    if (pAction->Facet.Wdt > 0) {
      Action.Facet.Set(GetGraphics()->GetBitmap(Color), pAction->Facet.x,
                       pAction->Facet.y, pAction->Facet.Wdt,
                       pAction->Facet.Hgt);
...
BOOL C4Object::SetActionByName(const char *szActName, C4Object *pTarget,
                               C4Object *pTarget2, int32_t iCalls,
                               bool fForce) {
  int32_t cnt;
...
void C4Object::NoAttachAction() {
  ...
  if (Action.Act > ActIdle) {
    ...
    if (iProcedure == DFA_SCALE && Action.ComDir != COMD_Stop &&
        ComDirLike(Action.ComDir, COMD_Up))
      if (ObjectActionCornerScale(this)) return;
    if (iProcedure == DFA_SCALE && Action.ComDir == COMD_Left &&
        Action.Dir == DIR_Left)
      if (ObjectActionCornerScale(this)) return;
    if (iProcedure == DFA_SCALE && Action.ComDir == COMD_Right &&
    ...
    // Fighting: Set last energy loss player for kill tracing (pushing people
    // off a cliff)
    else if (iProcedure == DFA_FIGHT && Action.Target)
      LastEnergyLossCausePlayer = Action.Target->Controller;
    // Else jump
...
void C4Object::ContactAction() {
  ...
  //------------------------------- Hit Bottom
  //---------------------------------------------
  if (t_contact & CNAT_Bottom) switch (iProcedure) {
      case DFA_FLIGHT:
        if (ydir < 0) break;
  ...
  //------------------------------- Hit Ceiling
  //-----------------------------------------
  if (t_contact & CNAT_Top) switch (iProcedure) {
      case DFA_WALK:
        // Walk: Stop
        ObjectActionStand(this);
        return;
      ...
      case DFA_FLIGHT:
        ...
        if ((OCF & OCF_HitSpeed3) || fDisabled) {
          ObjectActionTumble(this, Action.Dir, Fix0, Fix0);
          break;
        }
        if (pPhysical->CanHangle) {
          ObjectActionHangle(this, Action.Dir);
          return;
        }
        break;
      case DFA_DIG:
  ...
  if (t_contact & CNAT_Left) {
    switch (iProcedure) {
      case DFA_FLIGHT:
        // High Speed Flight: Tumble
        if ((OCF & OCF_HitSpeed3) || fDisabled) {
          ObjectActionTumble(this, DIR_Left, FIXED100(+150), Fix0);
          break;
        }
        // Else
        else if (pPhysical->CanScale) {
          ObjectActionScale(this, DIR_Left);
          return;
        }
        break;
      case DFA_WALK:
  ...
  if (t_contact & CNAT_Right) {
    switch (iProcedure) {
      case DFA_FLIGHT:
        // High Speed Flight: Tumble
        if ((OCF & OCF_HitSpeed3) || fDisabled) {
          ObjectActionTumble(this, DIR_Right, FIXED100(-150), Fix0);
          break;
        }
        // Else Scale
        else if (pPhysical->CanScale) {
          ObjectActionScale(this, DIR_Right);
          return;
        }
        break;
      case DFA_WALK:
...
bool DoBridge(C4Object *clk) {
  ...
  if (fWall) switch (clk->Action.ComDir) {
      ...
      case COMD_Right:
        dtp = 4;
        fMoveClonk = false;
        break;  // vertical wall: default 25 pixels
      case COMD_UpLeft:
      case COMD_UpRight:
        dtp = 5;
        fMoveClonk = false;
        break;  // diagonal roof over Clonk: default 20 pixels up and 20 pixels
                // side (28 pixels - optimized to close tunnels completely)
      case COMD_Up:
        dtp = 5;
        break;  // horizontal roof over Clonk
      default:
        return true;  // bridge procedure just for show
    }
  else
    switch (clk->Action.ComDir) {
      ...
      case COMD_Right:
        dtp = 5;
        break;  // horizontal bridges: default 20 pixels
      case COMD_Up:
        dtp = 4;
        break;  // vertical bridges: default 25 pixels (same as
      case COMD_UpLeft:
      case COMD_UpRight:
        dtp = 6;
        break;  // diagonal bridges: default 16 pixels up and 16 pixels side (23
                // pixels)
      default:
        return true;  // bridge procedure  just for show
    }
  ...
  if (fWall) switch (clk->Action.ComDir) {
      ...
      case COMD_Up: {
        ...
        if (fMoveClonk)
          ...
          x0 = (iBridgeTime / dtp) / -2;
        tx += (x0 + dt) * ((clk->Action.Dir == DIR_Right) * 2 - 1);
        cx += ((clk->Action.Dir == DIR_Right) * 2 - 1);
        ty -= ch + 3;
        break;
      }
      case COMD_UpLeft:
    ...
    switch (clk->Action.ComDir) {
      ...
      case COMD_Right:
        ...
        break;
      case COMD_Up:
        tx += (-cw / 2 + (cw - 1) * (clk->Action.Dir == DIR_Right)) *
              (!fMoveClonk);
        ty += -dt - fMoveClonk;
        --cy;
        break;
      case COMD_UpLeft:
        tx += -5 - dt + fMoveClonk * 3;
        ty += 2 - dt - fMoveClonk * 3;
        --cx;
        --cy;
        break;
      case COMD_UpRight:
        tx += +5 + dt - fMoveClonk * 2;
        ty += 2 - dt - fMoveClonk * 3;
        ++cx;
        --cy;
        break;
  ...
  if (fMoveClonk) {
    ...
    if (/*!clk->Shape.Attach(cx2, cy2, (clk->Action.t_attach & CNAT_Flags) | CNAT_Bottom) ||*/ clk
            ->Shape.CheckContact(cx2, cy2 - 1)) {
      // Clonk would collide here: Change to nonmoving Clonk mode and redo
      // bridging
      iBridgeTime -= clk->Action.Time;
      clk->Action.Time = 0;
      if (fWall && clk->Action.ComDir == COMD_Up) {
        // special for roof above Clonk: The nonmoving roof is started at
        // bridgelength before the Clonkl
        // so, when interrupted, an action time halfway through the action must
        // be set
...
void DoGravity(C4Object *cobj, BOOL fFloatFriction) {
  ...
  if (cobj->InLiquid && cobj->Def->Float) {
    ...
    if (!GBackLiquid(
            cobj->x,
            cobj->y - 1 + cobj->Def->Float * cobj->GetCon() / FullCon - 1))
...
BOOL ReduceLineSegments(C4Shape &rShape, bool fAlternate) {
  // try if line could go by a path directly when skipping on evertex. If
  // fAlternate is true, try by skipping two vertices
  for (int32_t cnt = 0; cnt + 2 + fAlternate < rShape.VtxNum; cnt++)
    if (PathFree(rShape.VtxX[cnt], rShape.VtxY[cnt],
                 rShape.VtxX[cnt + 2 + fAlternate],
                 rShape.VtxY[cnt + 2 + fAlternate])) {
...
void C4Object::ExecAction() {
  ...
  // this parameter had always been ignored for actions using an internal
  // procedure,
  ...
  // if an object is in controllable state, so it can be assumed that if it dies
  // later because of NO_OWNER's cause,
  // it has been its own fault and not the fault of the last one who threw a
  // flint on it
  // do not reset for burning objects to make sure the killer is set correctly
  // if they fall out of the map while burning
  // also keep state when swimming, so you get kills for pushing people into the
  // lava/acid/shark lake
  if (!pAction->Disabled && pAction->Procedure != DFA_FLIGHT &&
      pAction->Procedure != DFA_SWIM && !OnFire)
    LastEnergyLossCausePlayer = NO_OWNER;
  ...
  switch (pAction->Procedure) {
    ...
    case DFA_WALK:
      ...
      Action.t_attach |= CNAT_Bottom;
      Mobile = 1;
      // object is rotateable? adjust to ground, if in horizontal movement or
      // not attached to the center vertex
      if (Def->Rotateable && Shape.AttachMat != MNone &&
          (!!xdir || Def->Shape.VtxX[Shape.iAttachVtx]))
    ...
    case DFA_BUILD:
      ...
      if (!Inside<int32_t>(x - (Action.Target->x + Action.Target->Shape.x), 0,
                           Action.Target->Shape.Wdt) ||
          !Inside<int32_t>(y - (Action.Target->y + Action.Target->Shape.y), -16,
    ...
    case DFA_PUSH:
      ...
      // Push object
      if (!Action.Target->Push(iTXDir, ValByPhysical(250, pPhysical->Push),
                               fStraighten)) {
        StopActionDelayCommand(this);
        return;
      }
      ...
      // Vertical follow: If object moves out at top, assume it's being pushed
      // upwards and the Clonk must run after it
      if (y - iPushDistance > say + sahgt && iTXDir) {
        if (iTXDir > 0) sax += sawdt / 2;
        sawdt /= 2;
      }
      // Horizontal follow
      iTargetX =
          BoundBy(x, sax - iPushDistance, sax + sawdt - 1 + iPushDistance);
      if (x == iTargetX)
        xdir = 0;
      else {
        if (x < iTargetX) xdir = +lLimit;
        if (x > iTargetX) xdir = -lLimit;
      }
      // Phase by XDir
      if (xdir < 0) {
    ...
    case DFA_PULL:
      ...
      iTXDir =
          fMove +
          fWalk * BoundBy<int32_t>(iPullX - Action.Target->x, -10, +10) / 10;
    ...
    case DFA_FIGHT:
      ...
      if (Action.Target->Contained != Contained)
        if ((Contained && !Contained->EntranceStatus) ||
            (Action.Target->Contained &&
             !Action.Target->Contained->EntranceStatus)) {
          ObjectActionStand(this);
          return;
        }

      // Physical training
      if (!Tick5) TrainPhysical(&C4PhysicalInfo::Fight, 1, C4MaxPhysical);
    ...
    case DFA_LIFT:
      ...
      if (Def->LiftTop)
        if (Action.Target->y <= (y + Def->LiftTop))
          if (Action.ComDir == COMD_Up) Call(PSF_LiftTop);
      // General
      DoGravity(this);
      break;
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    case DFA_ATTACH:
      ...
      // Force position
      ForcePosition(
          Action.Target->x + Action.Target->Shape.VtxX[Action.Data & 255] -
    ...
    case DFA_CONNECT:
      ...
      if (Action.Target) {
        ...
        if (Def->Line == C4D_Line_Vertex) {
          iConnectX = Action.Target->x +
                      Action.Target->Shape.GetVertexX(Local[2].getInt());
          iConnectY = Action.Target->y +
                      Action.Target->Shape.GetVertexY(Local[2].getInt());
        }
        ...
        if ((iConnectX != Shape.VtxX[0]) || (iConnectY != Shape.VtxY[0])) {
          ...
          if (Def->LineIntersect == 0)
            if (!Shape.LineConnect(iConnectX, iConnectY, 0, +1, Shape.VtxX[0],
                                   Shape.VtxY[0]))
              fBroke = TRUE;
      ...
      if (Action.Target2) {
        ...
        if (Def->Line == C4D_Line_Vertex) {
          iConnectX = Action.Target2->x +
                      Action.Target2->Shape.GetVertexX(Local[3].getInt());
          iConnectY = Action.Target2->y +
                      Action.Target2->Shape.GetVertexY(Local[3].getInt());
        }
        // Connect to bottom center
        else {
          iConnectX = Action.Target2->x;
      ...
      if (!Tick35) ReduceLineSegments(Shape, !Tick2);
    ...
    if (Action.PhaseDelay >= pAction->Delay) {
      ...
      if (Action.Phase >= pAction->Length) {
        ...
        if (pAction->NextAction == ActHold)
          ...
          // Set new action
          SetAction(pAction->NextAction, NULL, NULL,
                    SAC_StartCall | SAC_EndCall);
      }
    }
  }
...
void C4Object::SetAudibilityAt(C4FacetEx &cgo, int32_t iX, int32_t iY) {
  ...
  Audible = Max<int32_t>(
      Audible, BoundBy(100 -
                           100 * Distance(cotx + cgo.Wdt / 2,
                                          coty + cgo.Hgt / 2, iX, iY) /
                               700,
                       0, 100));
...
bool C4Object::IsVisible(int32_t iForPlr, bool fAsOverlay) {
  ...
  if (iForPlr != NO_OWNER) {
    // check all
    if (Visibility & VIS_Allies)
      fDraw = fDraw || (iForPlr != Owner && !Hostile(iForPlr, Owner));
    if (Visibility & VIS_Enemies)
      fDraw = fDraw || (iForPlr != Owner && Hostile(iForPlr, Owner));
    if (Visibility & VIS_Local)
      fDraw = fDraw || (Local[iForPlr / 32].getInt() & (1 << (iForPlr % 32)));
  } else
    fDraw = fDraw || (Visibility & VIS_God);
  return fDraw;
}
...
void C4Object::PrepareDrawing() {
  // color modulation
  if (ColorMod || (BlitMode & (C4GFXBLIT_MOD2 | C4GFXBLIT_CLRSFC_MOD2)))
    Application.DDraw->ActivateBlitModulation(ColorMod);
  // other blit modes
...
BOOL C4Object::Collect(C4Object *pObj) {
  ...
  bool fRejectCollect;
  if (!pObj->Enter(this, TRUE, false, &fRejectCollect)) return FALSE;
  // Cancel attach (hacky)
...
void C4Object::DirectComContents(C4Object *pTarget, bool fDoCalls) {
  ...
  if (fDoCalls)
    if (!!Call("~ControlContents", &C4AulParSet(C4VID(pTarget->id)))) return;
  // default action
  if (!(Contents.ShiftContents(pTarget))) return;
  // Selection sound
  if (fDoCalls)
    if (!Contents.GetObject()->Call("~Selection", &C4AulParSet(C4VObj(this))))
      StartSoundEffect("Grab", false, 100, this);
...
void C4Object::ApplyParallaxity(int32_t &riTx, int32_t &riTy,
                                const C4Facet &fctViewport) {
  // parallaxity by locals
  // special: Negative positions with parallaxity 0 mean HUD elements positioned
  // to the right/bottom
...
void C4Object::GetViewPosPar(int32_t &riX, int32_t &riY, int32_t tx, int32_t ty,
                             const C4Facet &fctViewport) {
  int iParX = Local[0].Data.Int, iParY = Local[1].Data.Int;
  // get drawing pos, then subtract original target pos to get drawing pos on
  // landscape
  if (!iParX && x < 0)
...
bool C4Object::PutAwayUnusedObject(C4Object *pToMakeRoomForObject) {
  ...
  if (pFnObj2Drop = Def->Script.GetSFunc(PSF_GetObject2Drop))
    pUnusedObject =
        pFnObj2Drop->Exec(this, pToMakeRoomForObject
                                    ? &C4AulParSet(C4VObj(pToMakeRoomForObject))
                                    : NULL).getObj();
  else {
...
C4GraphicsOverlay *C4Object::GetGraphicsOverlay(int32_t iForID, bool fCreate) {
  ...
  while (pOverlay && pOverlay->GetID() < iForID) {
  ...
  if (pPrevOverlay)
    pPrevOverlay->SetNext(pNewOverlay);
  else
    pGfxOverlay = pNewOverlay;
...
bool C4Object::RemoveGraphicsOverlay(int32_t iOverlayID) {
  // search in list until ID is found or passed
  C4GraphicsOverlay *pOverlay = pGfxOverlay, *pPrevOverlay = NULL;
  while (pOverlay && pOverlay->GetID() < iOverlayID) {
    pPrevOverlay = pOverlay;
    pOverlay = pOverlay->GetNext();
  }
  // exact match found?
  if (pOverlay && pOverlay->GetID() == iOverlayID) {
    // remove it
    if (pPrevOverlay)
      pPrevOverlay->SetNext(pOverlay->GetNext());
    else
      pGfxOverlay = pOverlay->GetNext();
...
bool C4Object::StatusDeactivate(bool fClearPointers) {
  ...
  if (fClearPointers) {
    // in this case, the object must also exit any container, and any contained
    // objects must be exited
...
bool C4Object::AdjustWalkRotation(int32_t iRangeX, int32_t iRangeY,
                                  int32_t iSpeed) {
  int32_t iDestAngle;
  ...
  if (Shape.iAttachVtx < 0 || !Def->Shape.VtxX[Shape.iAttachVtx]) {
    ...
    // attachment at other than horizontal middle vertex: get your feet to the
    // ground!
    // rotate target to large angle is OK, because rotation will stop once the
    // real
...
void C4Object::DelRef(const C4Value *pRef, C4Value *pNextRef) {
  ...
  if (pRef == FirstRef)
    ...
    while (pVal->NextRef && pVal->NextRef != pRef) pVal = pVal->NextRef;
    assert(pVal->NextRef);
...
StdStrBuf C4Object::GetInfoString() {
  ...
  for (C4Effect *pEff = pEffects; pEff; pEff = pEff->pNext) {
    C4Value par[7];
    C4Value vInfo = pEff->DoCall(this, PSFS_FxInfo, par[0], par[1], par[2],
                                 par[3], par[4], par[5], par[6]);
    if (!vInfo) continue;
    // debug: warn for wrong return types
    if (vInfo.GetType() != C4V_String)
      DebugLogF("Effect %s(%d) on object %s (#%d) returned wrong info type %d.",
                pEff->Name, pEff->iNumber, Def->GetName(), Number,
                vInfo.GetType());
    // get string val
    C4String *psInfo = vInfo.getStr();
    const char *szEffInfo;
...
void C4Object::GrabContents(C4Object *pFrom) {
  ...
  C4ObjectList tmpList;
  tmpList.Copy(pFrom->Contents);
  C4ObjectLink *cLnk;
  for (cLnk = tmpList.First; cLnk; cLnk = cLnk->Next)
    if (cLnk->Obj->Status) cLnk->Obj->Enter(this);
}
...
bool C4Object::CanConcatPictureWith(C4Object *pOtherObject) {
  ...
  if (!(allow_picture_stack & APS_Name)) {
    ...
    if (Name.getData() != pOtherObject->Name.getData())
      if (Name != pOtherObject->Name) return false;
  }
  if (!(allow_picture_stack & APS_Overlay)) {
    // check overlay graphics
    for (C4GraphicsOverlay *pOwnOverlay = pGfxOverlay; pOwnOverlay;
         pOwnOverlay = pOwnOverlay->GetNext())
      if (pOwnOverlay->IsPicture()) {
        C4GraphicsOverlay *pOtherOverlay =
            pOtherObject->GetGraphicsOverlay(pOwnOverlay->GetID(), false);
        if (!pOtherOverlay || !(*pOtherOverlay == *pOwnOverlay)) return false;
      }
    for (C4GraphicsOverlay *pOtherOverlay = pOtherObject->pGfxOverlay;
         pOtherOverlay; pOtherOverlay = pOtherOverlay->GetNext())
...
int32_t C4Object::GetFireCausePlr() {
  ...
  // return if valid
  if (ValidPlr(iFireCausePlr))
    return iFireCausePlr;
  else
    return NO_OWNER;
}

void C4Object::UpdateScriptPointers() {
  if (pEffects) pEffects->ReAssignAllCallbackFunctions();
}
...
StdStrBuf C4Object::GetNeededMatStr(C4Object *pBuilder) {
  ...
  StdStrBuf result;
  if (!!NeededMats) {
    result.Format(LoadResStr("IDS_CON_BUILDMATNEED"), GetName());
    result.Append(NeededMats.getData());
  } else
...
bool C4Object::IsPlayerObject(int32_t iPlayerNumber) {
  ...
  if (fAnyPlr || Owner == iPlayerNumber) {
    ...
    if (pOwner) {
      ...
      // Do not force that the owner exists because the function must work for
      // unjoined players (savegame resume)
      if (Category & C4D_CrewMember) return true;
    }
  }
