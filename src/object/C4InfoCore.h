...
const int32_t C4MaxPhysical = 100000, C4MaxDeathMsg = 75;
...
class C4PhysicalInfo {
 ...
 public:
  void Default();
  void PromotionUpdate(int32_t iRank, bool fUpdateTrainablePhysicals = false,
                       class C4Def *pTrainDef = NULL);
...
class C4PhysicalChange {
 public:
  ...
  C4PhysicalChange(const C4PhysicalChange &rCpy)
      : PrevVal(rCpy.PrevVal), mpiOffset(rCpy.mpiOffset) {}
  ...
  C4PhysicalChange &operator=(const C4PhysicalChange &rSet) {
    PrevVal = rSet.PrevVal;
    mpiOffset = rSet.mpiOffset;
    return *this;
  }
...
// changes done to the original physicals; used for backtracing
 ...
 public:
  ...
  void Default() {
    Clear();
    C4PhysicalInfo::Default();
  }  // clears
  void CompileFunc(StdCompiler *pComp);

  void RegisterChange(
      C4PhysicalInfo::Offset mpiOffset);  // append physical change to list
  bool ResetPhysical(
      C4PhysicalInfo::Offset mpiOffset);  // undo given physical change

  bool HasChanges(C4PhysicalInfo *pRefPhysical);  // return true if changes list
                                                  // is not empty
  ...
  C4PhysicalInfo &operator=(const C4PhysicalInfo &rSet) {
    Clear();
    static_cast<C4PhysicalInfo &>(*this) = rSet;
    return *this;
  }
};
...
class C4ObjectInfoCore {
 ...
 public:
  ...
  int32_t NextRankExp;  // EXP_NoPromotion for no more promotion; 0 if standard
                        // rank system is used
 ...
 public:
  ...
  void Default(C4ID n_id = C4ID_None, class C4DefList *pDefs = NULL,
  ...
  bool GetNextRankInfo(C4RankSystem &rDefaultRanks, int32_t *piNextRankExp,
 ...
 protected:
  ...
  void UpdateCustomRanks(
      C4DefList *pDefs);  // sets NextRankName and NextRankExp
};
...
class C4PlayerInfoCore {
 ...
 public:
  ...
  int32_t PrefAutoContextMenu;  // enable automatically opened context menus in
                                // structures
 public:
...
inline FIXED ValByPhysical(
    int32_t iPercent,
    int32_t iPhysical)  // get percentage of max physical value
{
