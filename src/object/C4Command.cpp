...
const int32_t JumpAngle = 35, JumpLowAngle = 80, JumpAngleRange = 10,
              JumpHighAngle = 0;
const int32_t FlightAngleRange = 60;
const int32_t DigOutDirectRange = 130;
const int32_t LetGoHangleAngle = 110;

StdEnumAdapt<int32_t>::Entry
    EnumAdaptCommandEntries[C4CMD_Last - C4CMD_First + 2];
...
const char *CommandName(int32_t iCommand) {
  static const char *szCommandName[] = {
      ...
      "Acquire",   "Energy",   "Retry",  "Home",    "Call",     "Take",
      "Take2"};
...
const char *CommandNameID(int32_t iCommand) {
  static const char *dwCommandNameID[] = {
      ...
      "IDS_CON_HOME",       "IDS_COMM_CALL",     "IDS_COMM_TAKE",
      "IDS_COMM_TAKE2"};

  if (!Inside<int32_t>(iCommand, C4CMD_First, C4CMD_Last))
...
int32_t CommandByName(const char *szCommand) {
  for (int32_t cnt = C4CMD_First; cnt <= C4CMD_Last; cnt++)
    if (SEqual(szCommand, CommandName(cnt))) return cnt;
  return C4CMD_None;
}

void AdjustMoveToTarget(int32_t &rX, int32_t &rY, BOOL fFreeMove,
                        int32_t iShapeHgt) {
  // Above solid (always)
  int32_t iY;
  ...
  if (!fFreeMove) {
    ...
    if (GBackSolid(rX, rY + 1) || GBackSolid(rX, rY + 5))
      if (!GBackSemiSolid(rX, rY - iShapeHgt / 2)) rY -= iShapeHgt / 2;
  }
}
...
static bool ObjectAddWaypoint(int32_t iX, int32_t iY, intptr_t iTransferTarget,
                              intptr_t ipObject) {
  ...
  // Transfer waypoint
  if (iTransferTarget)
    return cObj->AddCommand(C4CMD_Transfer, (C4Object *)iTransferTarget, iX, iY,
                            0, NULL, FALSE);
...
void C4Command::MoveTo() {
  ...
  if ((cObj->OCF & OCF_CrewMember) || cObj->Def->Pathfinder)
    if (!PathChecked)
      ...
      if (Distance(cx, cy, Tx._getInt(), Ty) < MaxPathRange)
        // Not too close
        if (!(Inside(cx - Tx._getInt(), -PathRange, +PathRange) &&
          ...
          if (!PathFree(cx, cy, Tx._getInt(), Ty)) {
            ...
            if (!Game.PathFinder.Find(cObj->x, cObj->y, Tx._getInt(), Ty,
                                      &ObjectAddWaypoint,
  ...
  if (cObj->GetProcedure() == DFA_PUSH)
    if (cObj->Action.Target)
      if (cObj->Action.Target->Def->Grab == 2 ||
          !(Data & C4CMD_MoveTo_PushTarget)) {
  ...
  if (Inside(cx - Tx._getInt(), -iRangeFactorSide * iTargetRange,
  ...
  switch (cObj->GetProcedure()) {
    ...
    // - - - - - - - -
    case DFA_FLOAT: {
      FIXED dx = itofix(Tx._getInt()) - cObj->fix_x,
            dy = itofix(Ty) - cObj->fix_y;
      // normalize
      FIXED dScale =
          FIXED100(cObj->GetPhysical()->Float) / Max(Abs(dx), Abs(dy));
      dx *= dScale;
      dy *= dScale;
      ...
      dy -= cObj->ydir;
      // steer
      if (Abs(dx) + Abs(dy) < FIXED100(20))
        cObj->Action.ComDir = COMD_None;
      else if (Abs(dy) * 3 < dx)
...
void C4Command::Dig() {
  ...
  if (cx < tx - DigRange)
    if (cy < ty - DigRange) cObj->Action.ComDir = COMD_DownRight;
  if (cx > tx - DigRange)
    if (cy < ty - DigRange) cObj->Action.ComDir = COMD_DownLeft;
  if (cx < tx - DigRange)
    if (cy > ty + DigRange) cObj->Action.ComDir = COMD_UpRight;
  if (cx > tx + DigRange)
    if (cy > ty + DigRange) cObj->Action.ComDir = COMD_UpLeft;
}
...
void C4Command::Follow() {
  ...
  if (cObj->Def->CrewMember)
    ...
    if (!cObj->Select && cObj->Owner != NO_OWNER) {
      Finish(TRUE);
      return;
    }
...
void C4Command::Enter() {
  ...
  if (!Target || cObj->Def->NoPushEnter) {
    Finish();
    return;
  }
  ...
  } else  // Else, move to object's entrance
  {
    ...
    if (Target->GetEntranceArea(ex, ey, ewdt, ehgt))
      cObj->AddCommand(
          C4CMD_MoveTo, NULL, ex + ewdt / 2, ey + ehgt / 2, 50, NULL, true,
          (Data & C4CMD_Enter_PushTarget) ? C4CMD_MoveTo_PushTarget : 0);
  }
}
...
void C4Command::Exit() {
  ...
  // Building: stop
  if (cObj->GetProcedure() == DFA_BUILD) ObjectComStop(cObj);
...
void C4Command::Grab() {
  ...
  // Else, move to object
    ...
    cObj->AddCommand(C4CMD_MoveTo, NULL, Target->x + Tx._getInt(),
                     Target->y + Ty, 50);
  }
}
...
void C4Command::PushTo() {
  ...
  if (Target2) {
    cObj->AddCommand(C4CMD_Enter, Target2, 0, 0, 40, NULL, true,
                     C4CMD_Enter_PushTarget);
    return;
  } else {
    cObj->AddCommand(C4CMD_MoveTo, NULL, Tx, Ty, 40, NULL, true,
...
void C4Command::Chop() {
  ...
  ocf = OCF_All;
  if (!cObj->Contained && Target->At(cObj->x, cObj->y, ocf) &&
      Inside<int32_t>(Abs(cObj->x - Target->x), 4, 9)) {
    cObj->Action.ComDir = COMD_Stop;
  ...
  // Else, move to object
    ...
    if (Abs(cObj->x - Target->x) < 5)
      cObj->AddCommand(C4CMD_MoveTo, NULL,
                       (cObj->x > Target->x) ? Target->x + 15 : Target->x - 15,
                       Target->y, 50);
  }
}
...
void C4Command::Build() {
  ...
  if (cObj->GetPhysical() && !cObj->GetPhysical()->CanConstruct) {
    Finish(FALSE, FormatString(LoadResStr("IDS_TEXT_CANTBUILD"),
                               cObj->GetName()).getData());
    return;
  }
    ...
    if (Game.Rules & C4RULE_StructuresNeedEnergy)
      if (Target->Def->LineConnect & C4D_Power_Input)
        if (!Game.FindObjectByCommand(C4CMD_Energy, Target)) {
          // if another Clonk is also building this structure and carries a
          // linekit already, that Clonk should rather perform the energy
          // command
          C4Object *pOtherBuilder = NULL;
          if (!cObj->Contents.Find(C4ID_Linekit)) {
            while (pOtherBuilder = Game.FindObjectByCommand(
                       C4CMD_Build, Target, C4VNull, 0, NULL, pOtherBuilder))
              if (pOtherBuilder->Contents.Find(C4ID_Linekit)) break;
          }
          if (!pOtherBuilder) cObj->AddCommand(C4CMD_Energy, Target);
        }
    // Done
    cObj->Action.ComDir = COMD_Stop;
    Finish(TRUE);
  ...
  if (cObj->GetProcedure() == DFA_BUILD)
    if (cObj->Action.Target == Target) return;
  // Grabbing: let go
  if (cObj->GetProcedure() == DFA_PUSH) {
    cObj->AddCommand(C4CMD_UnGrab, NULL, 0, 0, 50);
    return;
...
void C4Command::Throw() {
  ...
  if ((Tx._getInt() != 0) || (Ty != 0)) {
    ...
    int32_t iDir = +1;
    if (cObj->x > Tx._getInt()) iDir = -1;

    // Find throwing position
    int32_t iTx, iTy;
    FIXED pthrow = ValByPhysical(400, cObj->GetPhysical()->Throw);
    int32_t iHeight = cObj->Shape.Hgt;
    if (!FindThrowingPosition(Tx._getInt(), Ty, pthrow * iDir, -pthrow, iHeight,
                              iTx, iTy))
      if (!FindThrowingPosition(Tx._getInt(), Ty, pthrow * iDir * -1, -pthrow,
                                iHeight, iTx, iTy))
    ...
    if (Inside<int32_t>(cObj->x - iTx, -iMoveToRange, +iMoveToRange) &&
        Inside<int32_t>(cObj->y - iTy, -15, +15)) {
      if (cObj->x < Tx._getInt())
...
void C4Command::Drop() {
  ...
  if ((Tx._getInt() != 0) || (Ty != 0)) {
    ...
    if (Inside<int32_t>(cObj->x - Tx._getInt(), -iMoveToRange, +iMoveToRange) &&
        Inside<int32_t>(cObj->y - Ty, -15, +15)) {
      cObj->Action.ComDir = COMD_Stop;
...
bool C4Command::GetTryEnter() {
  ...
  if (Target->Contained && CheckMinimumCon(Target)) { /* fail??! */
    return false;
  }
  // Target contained and container has RejectContents: fail
  if (Target->Contained && !!Target->Contained->Call(PSF_RejectContents)) {
    Finish();
    return false;
  }
  ...
  if (cObj->Def->CollectionLimit &&
    ...
    if (!cObj->PutAwayUnusedObject(Target)) {
      Finish();
      return false;
    }
    return false;
...
void C4Command::Get() {
  ...
  if (!Target->Contained && (Target->OCF & OCF_InSolid)) {
    ...
    if (FindClosestFree(iX2, iY2, -140, +140, -40, +40))
      ...
      if (Distance(Target->x, Target->y, iX2, iY2) <
  ...
  if ((pObj->Category & C4D_Vehicle) || (pObj->Category & C4D_Object))
    if (pObj->Category & C4D_SelectKnowledge)
      if (pObj->GetCon() < FullCon) {
        ...
        Finish(false, FormatString(LoadResStr("IDS_OBJ_NOCONACTIV"),
                                   pObj->GetName()).getData());
        return true;
      }
  return false;
}
...
void C4Command::Activate() {
  // Container specified, but no Target & no type: open activate menu for
  // container
  ...
  if (!Target2)
    if (Target) Target2 = Target->Contained;
  ...
  if (cObj->Contained == Target2) {
    for (Tx.SetInt(Data ? Max<int32_t>(Tx._getInt(), 1) : 1); Tx._getInt();
         --Tx) {
      ...
      C4ObjectLink *cLnk;
      if (!Target)
        for (cLnk = Target2->Contents.First; cLnk && (pObj = cLnk->Obj);
             cLnk = cLnk->Next)
...
void C4Command::Put()  // Notice: Put command is currently using Ty as an
                       // internal reminder flag for letting go after putting...
{
  ...
  if (!Target2)
    ...
    // No contents object to put - most likely we did have a target but it was
    // deleted,
    // e.g. by AutoSellContents in a base. New behaviour: if there is nothing to
    // put, we
  ...
  if (!cObj->Contents.GetLink(Target2)) {
    ...
    if (!Target2->Contained)
      if (Distance(cObj->x, cObj->y, Target2->x, Target2->y) < 80)
        if (Target2->OCF & OCF_HitSpeed1) return;
    // Go get it
  ...
  if (cObj->GetProcedure() == DFA_PUSH) pGrabbing = cObj->Action.Target;
  ...
  if (cObj->Contained == Target) {
    // Try to put
    if (!ObjectComPut(cObj, Target, Target2)) Finish();  // Putting failed
    return;
  }
  ...
  if (Target->OCF & OCF_Collection)
    if (!Target2->Def->Fragile)
      if (pGrabbing != Target) {
        int32_t iTx = Target->x + Target->Def->Collection.x +
                      Target->Def->Collection.Wdt / 2;
        int32_t iTy = Target->y + Target->Def->Collection.y +
                      Target->Def->Collection.Hgt / 2;
        FIXED pthrow = ValByPhysical(400, cObj->GetPhysical()->Throw);
        int32_t iHeight = cObj->Shape.Hgt;
        int32_t iPosX, iPosY;
        int32_t iObjDist = Distance(cObj->x, cObj->y, Target->x, Target->y);
        if ((FindThrowingPosition(iTx, iTy, pthrow, -pthrow, iHeight, iPosX,
                                  iPosY) &&
             (Distance(iPosX, iPosY, cObj->x, cObj->y) < iObjDist)) ||
            (FindThrowingPosition(iTx, iTy, pthrow * -1, -pthrow, iHeight,
                                  iPosX, iPosY) &&
             (Distance(iPosX, iPosY, cObj->x, cObj->y) < iObjDist))) {
          // Throw
    ...
    if (iExpGain && cObj)
      if (cObj->Info)
        for (int32_t i = iExpGain; i; --i)
          if (!(++cObj->Info->ControlCount % 5)) cObj->DoExperience(1);
  }
}
...
BOOL C4Command::InitEvaluation() {
  ...
  switch (Command) {
    ...
    case C4CMD_MoveTo: {
      ...
      if (~Data & C4CMD_MoveTo_NoPosAdjust)
...
void C4Command::Construct() {
  ...
  if (Target) {
    ...
    // construct command still present? (might find another stacked command,
    // which doesn't really matter for now...)
    if (!Target->FindCommand(C4CMD_Construct))
    // command aborted (or done?): failed to help; don't issue another construct
    // command, because it is likely to fail anyway
    // (and maybe, it had been finished while this Clonk was still moving to the
    // site)
    {
      Finish(FALSE);
      return;
    ...
    if (Tx._getInt() || Ty)
      if (!Inside<int32_t>(cObj->x - Tx._getInt(), -iMoveToRange,
                           +iMoveToRange) ||
  ...
  if (pPlayer)
    if (!pPlayer->Knowledge.GetIDCount(Data, 1)) {
      Finish();
      return;
    }
  ...
  if ((Tx._getInt() == 0) && (Ty == 0)) {
    ...
    Ty = cObj->y;
    int32_t iTx = Tx._getInt();
    if (!FindConSiteSpot(iTx, Ty, pDef->Shape.Wdt, pDef->Shape.Hgt,
                         pDef->Category, 20))
    // No site found: fail
    {
      Finish();
  ...
  int32_t scriptresult =
      cObj->Call(PSF_ControlCommandConstruction,
                 &C4AulParSet(C4VObj(Target), Tx, C4VInt(Ty), C4VObj(Target2),
                              C4VID(Data))).getInt();
  ...
  // Create construction
  C4Object *pConstruction = Game.CreateObjectConstruction(
      Data, NULL, cObj->Owner, Tx._getInt(), Ty, 1, TRUE);
...
BOOL C4Command::FlightControl()  // Called by DFA_WALK, DFA_FLIGHT
{
  ...
  int32_t iAngle = Angle(cx, cy, Tx._getInt(), Ty);
  while (iAngle > 180) iAngle -= 360;

  // Target in flight angle (sector +/- from straight up), beyond minimum
  // distance, and top free
  if (Inside(iAngle, -FlightAngleRange, +FlightAngleRange) ||
    ...
    if (Distance(cx, cy, Tx._getInt(), Ty) > 30) {
      ...
      for (iTopFree = 0;
        ...
        ;
      if (iTopFree >= 15) {
        // sprintf(OSTR,"Flight take off at %d
        // (%d)",iAngle,Distance(cx,cy,Tx,Ty)); Log(OSTR);
        // GameMsgObject(OSTR,cObj);
        // cObj->AddCommand(C4CMD_Jump,NULL,Tx,Ty); return TRUE;

        // Take off
        cObj->SetActionByName("Fly");  // This is a little primitive... we
                                       // should have a ObjectActionFly or maybe
                                       // a command for this...
      }
    }
...
BOOL C4Command::JumpControl()  // Called by DFA_WALK
{
  ...
  if (Inside<int32_t>(iAngle - JumpHighAngle, -3 * JumpAngleRange,
                      +3 * JumpAngleRange))
    // Vertical range
    if (Inside<int32_t>(cy - Ty, 10, 40)) {
      int32_t iSide = SolidOnWhichSide(
          Tx._getInt(),
          Ty);  // take jump height of side move position into consideration...!
      int32_t iDist = 5 * Abs(cy - Ty) / 6;
      int32_t iSideX = cx - iDist * iSide, iSideY = cy;
...
void C4Command::Transfer() {
  ...
  if (!Tick5) {
    ...
    if (fHandled)
      fHandled =
          f->Exec(Target, &C4AulParSet(C4VObj(cObj), Tx, C4VInt(Ty))).getBool();
...
void C4Command::Buy() {
  ...
  if (!Target)
    for (cnt = 0; pBase = Game.FindFriendlyBase(cObj->Owner, cnt); cnt++)
      if (!Target ||
          Distance(cObj->x, cObj->y, pBase->x, pBase->y) <
  ...
  if (!Game.Players.Get(Target->Base)->HomeBaseMaterial.GetIDCount(Data)) {
    Finish(false, FormatString(LoadResStr("IDS_PLR_NOTAVAIL"), pDef->GetName())
                      .getData());
    return;
  }
  // Base owner has not enough funds: fail
  if (Game.Players.Get(Target->Base)->Wealth <
      pDef->GetValue(Target, cObj->Owner)) {
    Finish(false, LoadResStr("IDS_PLR_NOWEALTH"));
    return;
  }
...
void C4Command::Acquire() {
  ...
  // script overload
  int32_t scriptresult =
      cObj->Call(PSF_ControlCommandAcquire,
                 &C4AulParSet(C4VObj(Target), Tx, C4VInt(Ty), C4VObj(Target2),
                              C4VID(Data))).getInt();
  ...
  cObj->AddCommand(C4CMD_Buy, NULL, 0, 0, 100, NULL, TRUE, Data, false, 0, 0,
                   C4CMD_Mode_Sub);
}
...
void C4Command::Fail(const char *szFailMessage) {
  ...
  if (ExecFail && cObj && (cObj->OCF & OCF_CrewMember)) {
    ...
    switch (Command) {
      case C4CMD_Build:
        ...
        if (!!cObj->Call(PSF_BuildNeedsMaterial,
                         &C4AulParSet(C4VID(Target->Component.GetID(0)),
                                      C4VInt(Target->Component.GetCount(
                                          0)))))  // WTF? This is passing
                                                  // current components. Not
                                                  // needed ones!
          break;                                  // no message
      ...
      case C4CMD_Construct:
        ...
        C4Def *pDef;
        pDef = Game.Defs.ID2Def(Data);
        SCopy(pDef ? pDef->GetName() : LoadResStr("IDS_OBJ_UNKNOWN"),
              szObjectName, C4MaxName);
        sprintf(OSTR, LoadResStr("IDS_CON_FAILUREOF"), szCommandName,
                szObjectName);
        break;
      default:
        // Already has a fail message
    ...
    if (l_Obj)
      if (l_Obj->Status && !l_Obj->Def->SilentCommands) {
        // Message (if not empty)
        if (OSTR[0]) {
          Game.Messages.Append(C4GM_Target, OSTR, l_Obj, NO_OWNER, 0, 0, FWhite,
                               TRUE);
        }
        // Fail sound
...
C4Object *CreateLine(C4ID idType, int32_t iOwner, C4Object *pFrom,
                     C4Object *pTo);
...
void C4Command::Set(int32_t iCommand, C4Object *pObj, C4Object *pTarget,
                    C4Value nTx, int32_t iTy, C4Object *pTarget2, int32_t iData,
                    int32_t iUpdateInterval, BOOL fEvaluated, int32_t iRetries,
...
void C4Command::Call() {
  ...
  // Object call
  Target->Call(Text,
               &C4AulParSet(C4VObj(cObj), Tx, C4VInt(Ty), C4VObj(Target2)));
...
void C4Command::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(Tx);
  pComp->Seperator(StdCompiler::SEP_SEP);
  pComp->Value(mkIntPackAdapt(Ty));
  pComp->Seperator(StdCompiler::SEP_SEP);
  // Target
  pComp->Value(mkIntPackAdapt(reinterpret_cast<int32_t &>(Target)));
  pComp->Seperator(StdCompiler::SEP_SEP);
  pComp->Value(mkIntPackAdapt(reinterpret_cast<int32_t &>(Target2)));
  pComp->Seperator(StdCompiler::SEP_SEP);
  // Data
  pComp->Value(mkIntPackAdapt(Data));
  pComp->Seperator(StdCompiler::SEP_SEP);
  // Update interval
  pComp->Value(mkIntPackAdapt(UpdateInterval));
  pComp->Seperator(StdCompiler::SEP_SEP);
  // Flags
  pComp->Value(mkIntPackAdapt(Evaluated));
  pComp->Seperator(StdCompiler::SEP_SEP);
  pComp->Value(mkIntPackAdapt(PathChecked));
  pComp->Seperator(StdCompiler::SEP_SEP);
  pComp->Value(mkIntPackAdapt(Finished));
  pComp->Seperator(StdCompiler::SEP_SEP);
  // Retries
  pComp->Value(mkIntPackAdapt(Failures));
  pComp->Seperator(StdCompiler::SEP_SEP);
  pComp->Value(mkIntPackAdapt(Retries));
  pComp->Seperator(StdCompiler::SEP_SEP);
  pComp->Value(mkIntPackAdapt(Permit));
  pComp->Seperator(StdCompiler::SEP_SEP);
  // Base mode
  if (iVersion > 0) {
    pComp->Value(mkIntPackAdapt(BaseMode));
    pComp->Seperator(StdCompiler::SEP_SEP);
  }
  // Text
  StdStrBuf TextBuf;
...
int32_t C4Command::CallFailed() {
  ...
  return Target->Call(szFunctionFailed,
                      &C4AulParSet(C4VObj(cObj), Tx, C4VInt(Ty),
                                   C4VObj(Target2)))._getInt();
  // Extreme caution notice: the script call might do just about anything
  // including clearing all commands (including this) i.e. through a call
  // to SetCommand. Thus, we must not do anything in this command anymore
