...
class C4ObjectMenu : public C4Menu {
 ...
 protected:
  ...
  bool UserMenu;  // set for script created menus; user menus do CloseQuery and
                  // MenuSelection callbacks
 ...
 public:
  ...
  bool Init(C4FacetExSurface &fctSymbol, const char *szEmpty, C4Object *pObject,
  ...
  bool InitRefSym(const C4FacetEx &fctSymbol, const char *szEmpty,
                  C4Object *pObject, int32_t iExtra = C4MN_Extra_None,
                  int32_t iExtraData = 0, int32_t iId = 0,
                  int32_t iStyle = C4MN_Style_Normal, bool fUserMenu = false);
  void Execute();
 ...
 protected:
  ...
  virtual void OnSelectionChanged(int32_t iNewSelection);  // do object
                                                           // callbacks if
                                                           // selection changed
                                                           // in user menus
  virtual bool IsCloseDenied();  // do MenuQueryCancel-callbacks for user menus
  virtual bool IsReadOnly();  // determine whether the menu is just viewed by an
                              // observer, and should not issue any calls
