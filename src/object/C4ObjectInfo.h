...
class C4ObjectInfo : public C4ObjectInfoCore {
 ...
 public:
  ...
  class C4Def *pDef;  // definition to ID - only eresolved if defs were loaded
                      // at object info loading time
#ifdef C4ENGINE
  C4Portrait Portrait;  // portrait link (usually to def graphics)
  C4Portrait *
      pNewPortrait;  // new permanent portrait link (usually to def graphics)
  C4Portrait *pCustomPortrait;  // if assigned, the Clonk has a custom portrait
                                // to be set via SetPortrait("custom")
#endif
...
#ifdef C4ENGINE
  bool SetRandomPortrait(C4ID idSourceDef, bool fAssignPermanently,
                         bool fCopyFile);
  bool SetPortrait(const char *szPortraitName, C4Def *pSourceDef,
                   bool fAssignPermanently, bool fCopyFile);
  bool SetPortrait(C4PortraitGraphics *pNewPortraitGfx, bool fAssignPermanently,
                   bool fCopyFile);
