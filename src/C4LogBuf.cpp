...
C4LogBuffer::C4LogBuffer(int iSize, int iMaxLines, int iLBWidth,
                         const char *szIndentChars, bool fDynamicGrow,
                         bool fMarkup)
    : iBufSize(iSize),
      iFirstLinePos(0),
      iAfterLastLinePos(0),
      iLineDataPos(0),
      iNextLineDataPos(0),
      iMaxLineCount(iMaxLines),
      iLineCount(0),
      iLineBreakWidth(iLBWidth),
      fDynamicGrow(fDynamicGrow),
      fMarkup(fMarkup) {
  // copy indent
  if (szIndentChars && *szIndentChars) {
    ...
    szIndent = NULL;
  // create buffers, if buffer size is given. Otherwise, create/grow them
  // dynamically
  if (iBufSize)
    ...
    szBuf = NULL;
  if (iMaxLineCount)
    pLineDataBuf = new LineData[iMaxLineCount];
  else
    pLineDataBuf = NULL;
...
void C4LogBuffer::GrowTextBuffer(size_t iGrowBy) {
  ...
  if (iAfterLastLinePos)
    memcpy(pNewBuf, szBuf, sizeof(char) * iAfterLastLinePos);
  delete[] szBuf;
  szBuf = pNewBuf;
}
...
void C4LogBuffer::AppendSingleLine(const char *szLine, int iLineLength,
                                   const char *szIndent, CStdFont *pFont,
                                   DWORD dwClr, bool fNewPar) {
  ...
  // but do not add a message that is longer than the buffer (shouldn't happen
  // anyway)
  ...
  if (!fDynamicGrow)
    while (iLineCount && Inside(iFirstLinePos, iAfterLastLinePos,
                                iAfterLastLinePos + iLineLength - 1))
      DiscardFirstLine();
  // copy indent
  int iIndentLen = 0;
  if (szIndent) {
  ...
  if (iLineLength - iIndentLen > 1)
    memcpy(szBuf + iAfterLastLinePos + iIndentLen, szLine,
           iLineLength - iIndentLen - 1);
  // add delimeter
  iAfterLastLinePos += iLineLength;
  szBuf[iAfterLastLinePos - 1] = 0;
  // no need to add any double delimeters, because this is currently the end of
  // the message list
  // also no need to check for end of buffer here, because that will be done
  // when the next message is inserted
  // add line data
...
void C4LogBuffer::AppendLines(const char *szLine, CStdFont *pFont, DWORD dwClr,
                              CStdFont *pFirstLineFont) {
    ...
    while (szBufPos = szPos2) {
      ...
      for (int i = 0; i < iLineBreakCharCount; ++i)
        if (szBufFind = strchr(szBufPos, LineBreakChars[i]))
          if (!szPos2 || szBufFind < szPos2) szPos2 = szBufFind;
  ...
  if (!iLineBreakWidth || !pFont) {
    ...
    while (*szLine) {
      ...
      // add them
      AppendSingleLine(szLine, iNumChars, iLineIndex ? szIndent : NULL, pFont,
                       dwClr, !iLineIndex);
      // next line
      szLine = szNextLine;
      ++iLineIndex;
...
const char *C4LogBuffer::GetLine(int iLineIndex, CStdFont **ppFont,
                                 DWORD *pdwClr, bool *pfNewPar) const {
...
void C4LogBuffer::Clear() {
  // clear buffer usage
  iFirstLinePos = iAfterLastLinePos = iLineCount = iNextLineDataPos =
      iLineDataPos = 0;
}

void C4LogBuffer::SetLBWidth(int iToWidth) { iLineBreakWidth = iToWidth; }
