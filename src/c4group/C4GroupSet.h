...
#define C4GSPrio_Definition \
  4  // overloads by definition file - latter defined definition files have
     // higher priority
#define C4GSPrio_Definition2 99  // highest priority a given definition may have
#define C4GSPrio_Folder \
  100  // overloads by local scenario folder - each child folder has higher
       // priority
...
#define C4GSCnt_Folder                                                     \
  (C4GSCnt_Graphics | C4GSCnt_Loaders | C4GSCnt_Material | C4GSCnt_Music | \
   C4GSCnt_FontDefs)
#define C4GSCnt_OriginFolder                                               \
  (C4GSCnt_Graphics | C4GSCnt_Loaders | C4GSCnt_Material | C4GSCnt_Music | \
   C4GSCnt_FontDefs)
...
class C4GroupSetNode {
 ...
 public:
  C4GroupSetNode(C4GroupSet &rParent, C4GroupSetNode *pPrev, C4Group &rGroup,
                 bool fGrpOwned, int32_t id);  // ctor
  ~C4GroupSetNode();                           // dtor
...
class C4GroupSet {
 ...
 public:
  ...
  C4GroupSet();                   // ctor
  C4GroupSet(C4GroupSet &rCopy);  // copy-constructor that registers all groups
                                  // with all contents
  ~C4GroupSet();                  // dtor

  bool RegisterGroup(C4Group &rGroup, bool fOwnGrp, int32_t Priority,
                     int32_t Contents,
                     bool fCheckContent = true);  // add group to list
  bool RegisterGroups(
      C4GroupSet &rCopy, int32_t Contents, const char *szFilename = NULL,
      int32_t iMaxSkipID = 0);  // add all matching (child-)groups of the set
  C4Group *FindGroup(
      int32_t Contents, C4Group *pAfter = NULL,
      bool fSamePrio = false);  // search for suitable group in list
  C4Group *FindEntry(const char *szWildcard, int32_t *pPriority = NULL,
                     int32_t *pID = NULL);  // find entry in groups; store
                                            // priority of group if ptr is given
  ...
  C4Group *RegisterParentFolders(const char *szScenFilename);  // register all
                                                               // parent .c4f
                                                               // groups to the
                                                               // given scenario
                                                               // filename and
  ...
  int32_t GetLastID() {
    return iIndex;
  }  // return ID assigned to the last added group
