...
C4GroupSetNode::C4GroupSetNode(C4GroupSet &rParent, C4GroupSetNode *pPrev,
                               C4Group &rGroup, bool fGrpOwned, int32_t id) {
  // set parent
  pParent = &rParent;
  // link into list
  this->pPrev = pPrev;
...
bool C4GroupSet::RegisterGroup(C4Group &rGroup, bool fOwnGrp, int32_t Priority,
                               int32_t Contents, bool fCheckContent) {
  // get node to sort in
...
int32_t C4GroupSet::CheckGroupContents(C4Group &rGroup, int32_t Contents) {
  // update mask
  if (Contents & C4GSCnt_Graphics)
    if (!rGroup.FindEntry(C4CFN_Graphics))
      Contents = Contents & ~C4GSCnt_Graphics;
  if (Contents & C4GSCnt_Loaders) {
    if (!rGroup.FindEntry("Loader*.bmp") && !rGroup.FindEntry("Loader*.png") &&
        !rGroup.FindEntry("Loader*.jpg") && !rGroup.FindEntry("Loader*.jpeg")) {
      Contents = Contents & ~C4GSCnt_Loaders;
    }
  }
  if (Contents & C4GSCnt_Material)
  ...
  if (Contents & C4GSCnt_FontDefs)
    if (!rGroup.FindEntry(C4CFN_FontFiles))
      if (!rGroup.FindEntry(C4CFN_FontDefs))
        Contents = Contents & ~C4GSCnt_FontDefs;
  // return it
  return Contents;
}

bool C4GroupSet::RegisterGroups(C4GroupSet &rCopy, int32_t Contents,
                                const char *szFilename, int32_t iMaxSkipID) {
    ...
    if (Contents2 = pNode->Contents & Contents)
      if (pNode->id > iMaxSkipID)
        if (!szFilename)
          ...
          RegisterGroup(*pNode->pGroup, false, pNode->Priority, Contents2,
                        false);
        else {
...
C4Group *C4GroupSet::FindGroup(int32_t Contents, C4Group *pAfter,
                               bool fSamePrio) {
  // get priority
  int32_t iPriority = -1;
...
C4Group *C4GroupSet::FindEntry(const char *szWildcard, int32_t *pPriority,
                               int32_t *pID) {
...
bool C4GroupSet::LoadEntry(const char *szEntryName, char **lpbpBuf,
                           size_t *ipSize, int32_t iAppendZeros) {
...
bool C4GroupSet::CloseFolders() {
  ...
  for (C4GroupSetNode *pNode = pFirst, *pNext; pNode; pNode = pNext) {
    ...
    if (Inside<int32_t>(pNode->Priority, C4GSPrio_Folder, C4GSPrio_Folder2) ||
        pNode->Priority == C4GSPrio_Scenario)
      // clear it!
      delete pNode;
  }
  // done, success
...
C4Group *C4GroupSet::RegisterParentFolders(const char *szScenFilename) {
  ...
  if (fParentC4F) {
    ...
    for (int32_t i = 0; i < iOriginalLen; ++i)
      if (szParentfolder[i] == DirectorySeparator || szParentfolder[i] == '/')
        szParentfolder[i] = 0;
    ...
    while (iPos < iOriginalLen) {
      ...
      if (pParentGroup) {
        if (!pGroup->OpenAsChild(pParentGroup, szParentfolder + iPos)) {
          LogFatal(FormatString("%s: %s", LoadResStr("IDS_PRC_FILENOTFOUND"),
                                szParentfolder + iPos).getData());
          delete pGroup;
          return nullptr;
        }
      ...
      if (!RegisterGroup(*pParentGroup, true, C4GSPrio_Folder + iGroupIndex++,
                         iContentsMask)) {
        delete pParentGroup;
