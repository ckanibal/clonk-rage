...
class C4UpdatePackageCore {
 ...
 public:
  ...
  int32_t GrpUpdate;
  int32_t
      UpGrpCnt;  // number of file versions that can be updated by this package
...
class C4UpdatePackage : public C4UpdatePackageCore {
 public:
  ...
  BOOL MakeUpdate(const char *strFile1, const char *strFile2,
                  const char *strUpdateFile, const char *strName = NULL);

 protected:
  BOOL DoUpdate(C4Group *pGrpFrom, class C4GroupEx *pGrpTo,
                const char *strFileName);
  BOOL DoGrpUpdate(C4Group *pUpdateData, class C4GroupEx *pGrpTo);
  static BOOL Optimize(C4Group *pGrpFrom, class C4GroupEx *pGrpTo,
                       const char *strFileName);

  BOOL MkUp(C4Group *pGrp1, C4Group *pGrp2, C4GroupEx *pUpGr, BOOL *fModified);

  BOOL OpenUnpackParents(C4GroupEx &rGroup, const char *strGroup,
