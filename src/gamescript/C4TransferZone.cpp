...
void C4TransferZones::ClearPointers(C4Object *pObj) {
  ...
  for (C4TransferZone *pZone = First; pZone; pZone = pZone->Next)
    if (pZone->Object == pObj) pZone->Object = NULL;
  // Remove cleared zones immediately
  RemoveNullZones();
}

BOOL C4TransferZones::Set(int32_t iX, int32_t iY, int32_t iWdt, int32_t iHgt,
                          C4Object *pObj) {
...
BOOL C4TransferZones::Add(int32_t iX, int32_t iY, int32_t iWdt, int32_t iHgt,
                          C4Object *pObj) {
  C4TransferZone *pZone;
  // Allocate and add new zone
...
C4TransferZone *C4TransferZones::Find(int32_t iX, int32_t iY) {
  for (C4TransferZone *pZone = First; pZone; pZone = pZone->Next)
    if (Inside<int32_t>(iX - pZone->X, 0, pZone->Wdt - 1))
      if (Inside<int32_t>(iY - pZone->Y, 0, pZone->Hgt - 1)) return pZone;
  return NULL;
}
...
BOOL C4TransferZone::At(int32_t iX, int32_t iY) {
  return (Inside<int32_t>(iX - X, 0, Wdt - 1) &&
          Inside<int32_t>(iY - Y, 0, Hgt - 1));
}
...
BOOL C4TransferZone::GetEntryPoint(int32_t &rX, int32_t &rY, int32_t iToX,
                                   int32_t iToY) {
  // Target inside zone: move outside horizontally
  if (Inside<int32_t>(iToX - X, 0, Wdt - 1) &&
      Inside<int32_t>(iToY - Y, 0, Hgt - 1))
    if (iToX < X + Wdt / 2)
      iToX = X - 1;
    else
      iToX = X + Wdt;
...
C4TransferZone *C4TransferZones::Find(C4Object *pObj) {
  for (C4TransferZone *pZone = First; pZone; pZone = pZone->Next)
    if (pZone->Object == pObj) return pZone;
  return NULL;
}
