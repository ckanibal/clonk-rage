...
// an effect itself only supplies the callback mechanism for effects assigned to
// objects
// the effect itself supplies the callback mechanism for creation, destruction,
// timers
...
#define C4Fx_Effect_Annul \
  -2  // delete effect, because it has annulled a countereffect
#define C4Fx_Effect_AnnulCalls \
  -3  // delete effect, because it has annulled a countereffect; temp readd
      // countereffect
...
#define C4FxCall_Temp \
  1  // temp call; effect is being added or removed in responce to a lower-level
     // effect change
#define C4FxCall_TempAddForRemoval \
  2  // temp call; effect is being added because it had been temp removed and is
     // now removed forever
#define C4FxCall_RemoveClear \
  3  // effect is being removed because object is being removed
#define C4FxCall_RemoveDeath \
  4  // effect is being removed because object died - return -1 to avoid removal
...
#define C4FxCall_EngBaseRefresh \
  36  // energy reload in base (also by base object, but that's normally not
      // called)
...
#define C4FxCall_EngStruct \
  39  // regular structure energy loss (normally not called)
...
class C4Effect {
 public:
  ...
  C4Object *pCommandTarget;  // target object for script callbacks - if deleted,
                             // the effect is removed without callbacks
  C4ID idCommandTarget;      // ID of command target definition
  int32_t nCommandTarget;  // enumerated ptr for target object (argh, when will
                           // this system be changed?)

  int32_t iPriority;       // effect priority for sorting into effect list; -1
                           // indicates a dead effect
 ...
 public:
  C4Effect(C4Object *pForObj, const char *szName, int32_t iPrio,
           ...
           C4Value &rVal1, C4Value &rVal2, C4Value &rVal3, C4Value &rVal4,
           bool fDoCalls, int32_t &riStoredAsNumber);  // ctor
  ...
  void ClearPointers(C4Object *pObj);  // clear all pointers to object - may
                                       // kill some effects w/o callback,
                                       // because the callback target is lost

  void SetDead() {
    iPriority = 0;
  }  // mark effect to be removed in next execution cycle
  bool IsDead() {
    return !iPriority;
  }  // return whether effect is to be removed
  ...
  C4Effect *Get(const char *szName, int32_t iIndex = 0,
                int32_t iMaxPriority = 0);  // get effect by name
  C4Effect *Get(int32_t iNumber, bool fIncludeDead,
                int32_t iMaxPriority = 0);  // get effect by number
  int32_t GetCount(
      const char *szMask,
      int32_t iMaxPriority = 0);  // count effects that match the mask
  int32_t Check(C4Object *pForObj, const char *szCheckEffect, int32_t iPrio,
                int32_t iTimer, C4Value &rVal1, C4Value &rVal2, C4Value &rVal3,
                C4Value &rVal4);     // do some effect callbacks
  C4AulScript *GetCallbackScript();  // get script context for effect callbacks

  void Execute(C4Object *pObj);  // execute all effects
  void Kill(
      C4Object *pObj);  // mark this effect deleted and do approprioate calls
  void ClearAll(C4Object *pObj, int32_t iClearFlag);  // kill all effects doing
                                                      // removal calls w/o
                                                      // reagard of inactive
                                                      // effects
  void DoDamage(C4Object *pObj, int32_t &riDamage, int32_t iDamageType,
                int32_t iCausePlr);  // ask all effects for damage

  C4Value DoCall(C4Object *pObj, const char *szFn, C4Value &rVal1,
                 C4Value &rVal2, C4Value &rVal3, C4Value &rVal4, C4Value &rVal5,
                 C4Value &rVal6, C4Value &rVal7);  // custom call

  void ReAssignCallbackFunctions() { AssignCallbackFunctions(); }
 ...
 protected:
  void TempRemoveUpperEffects(C4Object *pObj, bool fTempRemoveThis,
                              C4Effect **ppLastRemovedEffect);  // temp remove
                                                                // all effects
                                                                // with higher
                                                                // priority
  void TempReaddUpperEffects(C4Object *pObj,
                             C4Effect *pLastReaddEffect);  // temp remove all
                                                           // effects with
                                                           // higher priority
};

// ctor for StdPtrAdapt
inline void CompileNewFunc(C4Effect *&pRes, StdCompiler *pComp) {
  pRes = new C4Effect(pComp);
}
