...
C4Effect::C4Effect(C4Object *pForObj, const char *szName, int32_t iPrio,
  ...
  iPriority =
      0;  // effect is not yet valid; some callbacks to other effects are
          // done before
  riStoredAsNumber = 0;
  ...
  do
    if (!pEff->IsDead())
      if (!szMask || SWildcardMatchEx(pEff->Name, szMask))
        if (!iMaxPriority || pEff->iPriority <= iMaxPriority) ++iCnt;
  while (pEff = pEff->pNext);
  // return count
  return iCnt;
}
    ...
    if (!pCheck->IsDead() && pCheck->pFnEffect && pCheck->iPriority >= iPrio) {
      int32_t iResult =
          pCheck->pFnEffect->Exec(pCheck->pCommandTarget,
                                  &C4AulParSet(
                                      C4VString(szCheckEffect), C4VObj(pForObj),
                                      C4VInt(pCheck->iNumber), C4Value(), rVal1,
                                      rVal2, rVal3, rVal4)).getInt();
    ...
    if (pEffect->IsDead()) {
      ...
      *ppPrevEffect = pEffect = pNextEffect;
      ...
      if (pEffect->iIntervall && !(pEffect->iTime % pEffect->iIntervall))
        if (pEffect->pFnTimer) {
          if (pEffect->pFnTimer->Exec(pEffect->pCommandTarget,
                                      &C4AulParSet(C4VObj(pObj),
                                                   C4VInt(pEffect->iNumber),
                                                   C4VInt(pEffect->iTime)))
                  .getInt() == C4Fx_Execute_Kill) {
...
void C4Effect::DoDamage(C4Object *pObj, int32_t &riDamage, int32_t iDamageType,
                        int32_t iCausePlr) {
  ...
  do {
    if (!pEff->IsDead() && pEff->pFnDamage)
      riDamage =
          pEff->pFnDamage->Exec(pEff->pCommandTarget,
                                &C4AulParSet(
                                    C4VObj(pObj), C4VInt(pEff->iNumber),
                                    C4VInt(riDamage), C4VInt(iDamageType),
                                    C4VInt(iCausePlr))).getInt();
    if (pObj && !pObj->Status) return;
...
void C4Effect::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Seperator();
  // read priority
  pComp->Value(iPriority);
  pComp->Seperator();
  // read time and intervall
  pComp->Value(iTime);
  pComp->Seperator();
  pComp->Value(iIntervall);
  pComp->Seperator();
  // read object number
  pComp->Value(nCommandTarget);
  pComp->Seperator();
  // read ID
  ...
  bool fNext = !!pNext;
  if (pComp->hasNaming()) {
    if (fNext || pComp->isCompiler()) fNext = pComp->Seperator();
  } else
    pComp->Value(fNext);
  if (!fNext) return;
  // read next
...
C4Value &FxFireVarIncineratingObj(C4Effect *pEffect) {
  return pEffect->EffectVars[3];
}
...
int32_t GetSmokeLevel() {
  ...
  if (Game.Control.SyncMode()) return 150;
