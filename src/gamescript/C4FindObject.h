...
enum C4SortObjectCondID {
  ...
  C4SO_Multiple = 102,  // multiple sorts; high priority first; lower priorities
                        // if higher prio returned equal
...
class C4FindObject {
 ...
 public:
  ...
  static C4FindObject *CreateByValue(
      const C4Value &Data,
      C4SortObject **ppSortObj = NULL);  // createFindObject or SortObject - if
                                         // ppSortObj==NULL, SortObject is not
                                         // allowed

  int32_t Count(const C4ObjectList &
                    Objs);  // Counts objects for which the condition is true
  C4Object *Find(const C4ObjectList &Objs);  // Returns first object for which
                                             // the condition is true
  C4ValueArray *FindMany(const C4ObjectList &Objs);  // Returns all objects for
                                                     // which the condition is
                                                     // true

  int32_t Count(
      const C4ObjectList &Objs,
      const C4LSectors &Sct);  // Counts objects for which the condition is true
  C4Object *Find(const C4ObjectList &Objs,
                 const C4LSectors &Sct);  // Returns first object for which the
                                          // condition is true
  C4ValueArray *FindMany(const C4ObjectList &Objs,
                         const C4LSectors &Sct);  // Returns all objects for
...
class C4FindObjectNot : public C4FindObject {
 public:
  C4FindObjectNot(C4FindObject *pCond) : pCond(pCond) {}
...
class C4FindObjectExclude : public C4FindObject {
 public:
  C4FindObjectExclude(C4Object *pExclude) : pExclude(pExclude) {}
...
class C4FindObjectInRect : public C4FindObject {
 public:
  C4FindObjectInRect(const C4Rect &rect) : rect(rect) {}
...
class C4FindObjectAtPoint : public C4FindObject {
 public:
  C4FindObjectAtPoint(int32_t x, int32_t y) : bounds(x, y, 1, 1) {}
...
class C4FindObjectOCF : public C4FindObject {
 public:
  C4FindObjectOCF(int32_t ocf) : ocf(ocf) {}
...
class C4FindObjectCategory : public C4FindObject {
 public:
  C4FindObjectCategory(int32_t iCategory) : iCategory(iCategory) {}
...
class C4FindObjectAction : public C4FindObject {
 public:
  C4FindObjectAction(const char *szAction) : szAction(szAction) {}
...
class C4FindObjectContainer : public C4FindObject {
 public:
  C4FindObjectContainer(C4Object *pContainer) : pContainer(pContainer) {}
...
class C4FindObjectOwner : public C4FindObject {
 public:
  C4FindObjectOwner(int32_t iOwner) : iOwner(iOwner) {}
...
class C4FindObjectController : public C4FindObject {
 public:
  C4FindObjectController(int32_t controller) : controller(controller) {}
...
class C4SortObject {
 ...
 public:
  // Overridables
  virtual int32_t Compare(C4Object *pObj1,
                          C4Object *pObj2) = 0;  // return value <0 if obj1 is
                                                 // to be sorted before obj2

  virtual bool PrepareCache(const C4ValueList *pObjs) { return false; }
  virtual int32_t CompareCache(int32_t iObj1, int32_t iObj2, C4Object *pObj1,
                               C4Object *pObj2) {
    return Compare(pObj1, pObj2);
  }
...
class C4SortObjectReverse : public C4SortObject  // reverse sort
                            {
 public:
  C4SortObjectReverse(C4SortObject *pSort) : C4SortObject(), pSort(pSort) {}
...
class C4SortObjectMultiple
    : public C4SortObject  // apply next sort if previous compares to equality
      {
 public:
  C4SortObjectMultiple(int32_t iCnt, C4SortObject **ppSorts,
...
class C4SortObjectDistance
    : public C4SortObjectByValue  // sort by distance from point x/y
      {
 public:
...
class C4SortObjectSpeed
    : public C4SortObjectByValue  // sort by object xdir/ydir
      {
 public:
