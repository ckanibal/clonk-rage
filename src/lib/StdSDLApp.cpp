...
void CStdApp::Clear() { SDL_Quit(); }

void CStdApp::Quit() { fQuitMsgReceived = true; }
...
bool CStdApp::GetIndexedDisplayMode(int32_t iIndex, int32_t *piXRes,
  ...
  if (!modes) {
    ...
    if (!modes)
      ...
      for (SDL_Rect **iter = modes; *iter; ++iter) ++modeCount;
  }

  if (iIndex >= modeCount) return false;
...
bool CStdApp::FindDisplayMode(unsigned int iXRes, unsigned int iYRes,
  ...
  if (!SDL_VideoModeOK(iXRes, iYRes, iColorDepth, SDL_OPENGL)) return false;
...
StdStrBuf CStdApp::Paste(bool fClipboard) { return StdStrBuf(0); }
bool CStdApp::IsClipboardFull(bool fClipboard) { return false; }
