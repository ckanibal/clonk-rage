...
#error \
    "Using DirectX > 8.0 headers! Program won't run on computers using DX 8.0!"
#endif
...
class CStdD3D : public CStdDDraw {
 ...
 protected:
  ...
  IDirect3DVertexBuffer8 *
      pVBClr;  // prepared vertex buffer for drawing in solid color
  IDirect3DVertexBuffer8 *pVBClrTex;  // prepared vertex buffer for blitting
                                      // iwth color/tex-modulation
  C4VERTEX bltVertices[8];  // prepared vertex data; need to insert x/y and u/v
  C4CLRVERTEX
      clrVertices[8];  // prepared vertex data; need to insert x/y and color
  C4CTVERTEX bltClrVertices[8];  // prepared vertex data; need to insert x/y,
                                 // color and u/v
  DWORD bltState[3];  // saved state block for blitting (0: copy; 1: blit; 2:
                      // blit additive)
  DWORD bltBaseState[4];  // saved state block for blitting with a base face (0:
                          // normal; 1: additive; 2: mod2; 3: mod2+additive)
  DWORD drawSolidState[2];  // saved state block for drawing in solid color (0:
                            // normal; 1: additive)
 ...
 public:
  ...
  // Surface
  bool PrepareRendering(SURFACE sfcToSurface);  // check if/make rendering
                                                // possible to given surface
  // Blit
  void PerformBlt(CBltData &rBltData, CTexRef *pTex, DWORD dwModClr, bool fMod2,
                  bool fExact);
  BOOL BlitTex2Window(CTexRef *pTexRef, HDC hdcTarget, RECT &rtFrom,
                      RECT &rtTo);
  BOOL BlitSurface2Window(SURFACE sfcSource, int fX, int fY, int fWdt, int fHgt,
                          HWND hWnd, int tX, int tY, int tWdt, int tHgt);
  void FillBG(DWORD dwClr = 0);
  // Drawing
  void DrawQuadDw(SURFACE sfcTarget, int *ipVtx, DWORD dwClr1, DWORD dwClr2,
                  DWORD dwClr3, DWORD dwClr4);
  void DrawLineDw(SURFACE sfcTarget, float x1, float y1, float x2, float y2,
                  DWORD dwClr);
  ...
  DWORD CreateStateBlock(bool fTransparent, bool fSolid, bool fBaseTex,
                         bool fAdditive,
                         bool fMod2);  // capture state blocks for blitting

 protected:
  BOOL FindDisplayMode(unsigned int iXRes, unsigned int iYRes,
