...
void StdCompiler::Warn(const char *szWarning, ...) {
  ...
  // Format message
  va_list args;
  va_start(args, szWarning);
  StdStrBuf Msg;
  Msg.FormatV(szWarning, args);
  // do callback
...
void StdCompilerBinWrite::String(char *szString, size_t iMaxLength,
...
void StdCompilerBinWrite::WriteValue(const T &rValue) {
  // Copy data
  if (fSecondPass) *getMBufPtr<T>(Buf, iPos) = rValue;
  iPos += sizeof(rValue);
}

void StdCompilerBinWrite::WriteData(const void *pData, size_t iSize) {
  // Copy data
  if (fSecondPass) Buf.Write(pData, iSize, iPos);
  iPos += iSize;
}
...
void StdCompilerBinRead::String(char *szString, size_t iMaxLength,
                                RawCompileType eType) {
...
void StdCompilerBinRead::Begin() { iPos = 0; }

// *** StdCompilerINIWrite

bool StdCompilerINIWrite::Name(const char *szName) {
  // Sub-Namesections exist, so it's a section. Write name if not already done
  // so.
...
void StdCompilerINIWrite::NameEnd(bool fBreak) {
  ...
  // Append newline
  if (!fPutName && !fInSection) Buf.Append("\r\n");
  fPutName = false;
...
void StdCompilerINIWrite::String(char *szString, size_t iMaxLength,
                                 RawCompileType eType) {
  PrepareForValue();
  switch (eType) {
    case RCT_Escaped:
...
void StdCompilerINIWrite::Raw(void *pData, size_t iSize, RawCompileType eType) {
  switch (eType) {
    case RCT_Escaped:
      WriteEscaped(reinterpret_cast<char *>(pData),
                   reinterpret_cast<char *>(pData) + iSize);
      break;
    case RCT_All:
    case RCT_Idtf:
    case RCT_IdtfAllowEmpty:
  ...
  for (; pPos < pEnd; pPos++)
    if (!isprint((unsigned char)(unsigned char) * pPos) || *pPos == '\\' ||
      ...
      switch (*pPos) {
        ...
        default:
          Buf.AppendFormat("\\%o",
                           *reinterpret_cast<const unsigned char *>(pPos));
          fLastNumEscape = true;
      }
      // Set pointer
      pStart = pPos + 1;
...
void StdCompilerINIWrite::PutName(bool fSection) {
  if (fSection && Buf.getLength()) Buf.Append("\r\n");
...
StdCompilerINIRead::~StdCompilerINIRead() { FreeNameTree(); }
...
bool StdCompilerINIRead::Name(const char *szName) {
  ...
  if (iDepth - 1 > iRealDepth) return false;
  ...
  for (pNode = pName->FirstChild; pNode; pNode = pNode->NextChild)
    if (pNode->Pos && pNode->Name == szName) break;
  // Not found?
  if (!pNode) {
    NotFoundName = szName;
    return false;
...
void StdCompilerINIRead::NameEnd(bool fBreak) {
  ...
  if (iRealDepth == iDepth) {
    ...
    for (NameNode *pNode = pName->FirstChild, *pNext; pNode; pNode = pNext) {
      ...
      if (pNode->Pos && !fBreak)
        Warn("Unexpected %s \"%s\"!", pNode->Section ? "section" : "value",
             pNode->Name.getData());
      // delete node
    ...
    (pName->PrevChild ? pName->PrevChild->NextChild : pParent->FirstChild) =
        pName->NextChild;
    (pName->NextChild ? pName->NextChild->PrevChild : pParent->LastChild) =
        pName->PrevChild;
    delete pName;
    // Go up
    pName = pParent;
...
bool StdCompilerINIRead::Seperator(Sep eSep) {
  ...
  SkipWhitespace();
  // Seperator mismatch? Let all read attempts fail until the correct seperator
  // is found or the naming ends.
  if (*pPos != SeperatorToChar(eSep)) {
    pReenter = pPos;
    pPos = NULL;
    return false;
  }
...
int StdCompilerINIRead::NameCount(const char *szName) {
  ...
  for (pNode = pName->FirstChild; pNode; pNode = pNode->NextChild)
    ...
    if (pNode->Pos && (!szName || pNode->Name == szName)) ++iCount;
  return iCount;
}

// Various data readers
void StdCompilerINIRead::DWord(int32_t &rInt) { rInt = ReadNum(); }
void StdCompilerINIRead::DWord(uint32_t &rInt) { rInt = ReadUNum(); }
...
void StdCompilerINIRead::Word(uint16_t &rShort) {
  ...
  if (iNum > MAX) Warn("number out of range (%u to %u): %u ", MIN, MAX, iNum);
  rShort = BoundBy(iNum, MIN, MAX);
}
...
void StdCompilerINIRead::String(char *szString, size_t iMaxLength,
                                RawCompileType eType) {
  // Read data
...
void StdCompilerINIRead::String(char **pszString, RawCompileType eType) {
  // For Backwards compatibility: Escaped strings default to normal strings if
  // no escaped string is given
...
StdStrBuf StdCompilerINIRead::getPosition() const {
  if (pPos)
    ...
    return FormatString(pName->Section ? "section \"%s\", after line %d"
                                       : "value \"%s\", line %d",
                        pName->Name.getData(),
                        SGetLine(Buf.getData(), pName->Pos));
  else if (iRealDepth)
    return FormatString(
        "missing value/section \"%s\" inside section \"%s\" (line %d)",
        NotFoundName.getData(), pName->Name.getData(),
        SGetLine(Buf.getData(), pName->Pos));
  else
...
void StdCompilerINIRead::CreateNameTree() {
  ...
  while (*pPos) {
    ...
    if (fSection || isalpha((unsigned char)*pPos)) {
      ...
      while (pName->Parent && pName->Indent >= iIndent) pName = pName->Parent;
      // Copy name
      StdStrBuf Name;
      ...
      if (*pPos != (fSection ? ']' : '='))
        // Warn, ignore
        Warn(isprint((unsigned char)*pPos)
                 ? "Unexpected character ('%c'): %s ignored"
                 : "Unexpected character ('0x%02x'): %s ignored",
             unsigned(*pPos), fSection ? "section" : "value");
      else {
        pPos++;
        // Create new node
        NameNode *pPrev = pName->LastChild;
        pName = pName->LastChild =
            (pName->LastChild ? pName->LastChild->NextChild
                              : pName->FirstChild) = new NameNode(pName);
        ...
        if (!fSection) pName = pName->Parent;
      }
    }
    // Skip line
    while (*pPos && (*pPos != '\n' && *pPos != '\r')) pPos++;
...
void StdCompilerINIRead::SkipNum() {
  while (*pPos == '+' || *pPos == '-' || isdigit((unsigned char)*pPos)) pPos++;
}
...
StdBuf StdCompilerINIRead::ReadString(size_t iLength, RawCompileType eRawType,
                                      bool fAppendNull) {
  // Excpect valid position
  if (!pPos) {
    notFound("String");
  ...
  // Create buffer
  StdBuf OutBuf;
  OutBuf.New(iLength + (fAppendNull ? sizeof('\0') : 0));
  // Read
  ...
  if (pOut == OutBuf.getData() &&
      (eRawType == RCT_Idtf || eRawType == RCT_ID)) {
    notFound("String");
    return StdBuf();
  }
  // Append null
  if (fAppendNull) *pOut = '\0';
...
bool StdCompilerINIRead::TestStringEnd(RawCompileType eType) {
  switch (eType) {
    case RCT_Escaped:
      return *pPos == '"' || !*pPos || *pPos == '\n' || *pPos == '\r';
    case RCT_All:
      return !*pPos || *pPos == '\n' || *pPos == '\r';
    // '-' is needed for Layers in Scenario.txt (C4NameList) and other
    // Material-Texture combinations
    case RCT_Idtf:
    case RCT_IdtfAllowEmpty:
    case RCT_ID:
      return !isalnum((unsigned char)*pPos) && *pPos != '_' && *pPos != '-';
  }
  // unreachable
  return true;
}
...
char StdCompilerINIRead::ReadEscapedChar() {
  ...
  if (*pPos >= 0 && iscntrl((unsigned char)*pPos)) {
    Warn("Nonprintable character found in string: %02x",
         static_cast<unsigned char>(*pPos));
    return *pPos;
  }
  ...
  switch (*++pPos) {
    ...
    case 'x':
      ...
      if (!isxdigit((unsigned char)*++pPos))
        ...
        // Read everything that looks like it might be hexadecimal - MSVC does
        // it
        // this way, so do not sue me.
        int iCode = 0;
        do {
          iCode =
              iCode * 16 +
              (isdigit((unsigned char)*pPos) ? *pPos - '0' : *pPos - 'a' + 10);
          pPos++;
        } while (isxdigit((unsigned char)*pPos));
        // Done. Don't bother to check the range (we aren't doing anything
        // mission-critical here, are we?)
        return char(iCode);
      }
    default:
