...
class ResTable {
 public:
  ResTable(const char *table)
      : Capacity(int(1.10 * SCharCount('\n', table))),
        Count(0),
        Entries(new Entry[Capacity]) {
    // reduce the capacity so that there is always an empty entry to mark the
    // end of the search for an nonexistant key
    --Capacity;
    while (Count < Capacity) {
      ...
      if (equalpos) {
        ...
        for (i = 0; i < pos - equalpos; ++i)
          if (e->Data.getMData()[i] == '\\' &&
              e->Data.getMData()[i + 1] == 'n') {
            e->Data.getMData()[i] = 0x0D;
            e->Data.getMData()[i + 1] = 0x0A;
          }
        // Count!
        ++Count;
      }
 ...
 private:
  ...
  static unsigned int Hash(const char *Key) {
    ...
    while (*Key && *Key != '=') h = (h ^ *(Key++)) * 16777619;
    return h;
  }
};
