...
inline DWORD GetTextShadowClr(DWORD dwTxtClr) {
  return RGBA(((dwTxtClr >> 0) % 256) / 3, ((dwTxtClr >> 8) % 256) / 3,
              ((dwTxtClr >> 16) % 256) / 3, (dwTxtClr >> 24) % 256);
}

void CBltTransform::SetRotate(int iAngle, float fOffX,
...
CPattern &CPattern::operator=(const CPattern &nPattern) {
  ...
  if (nPattern.CachedPattern) {
    ...
    memcpy(CachedPattern, nPattern.CachedPattern,
           sfcPattern32->Wdt * sfcPattern32->Hgt * 4);
  } else {
    CachedPattern = 0;
  }
  Wdt = nPattern.Wdt;
...
bool CPattern::Set(SURFACE sfcSource, int iZoom, bool fMonochrome) {
  ...
  Clear();
  // new style: simply store pattern for modulation or shifting, which will be
  // decided upon use
...
bool CPattern::PatternClr(int iX, int iY, BYTE &byClr, DWORD &dwClr,
                          CStdPalette &rPal) const {
  // pattern assigned?
  ...
  // old style?
    ...
    if (pClrs) {
      // IFT (alpha only)
      int iAShift = 0;
      if (byClr & 0xf0) iAShift = 3;
      // compose color
      dwClr = RGB(pClrs[byShift * 3 + 2], pClrs[byShift * 3 + 1],
                  pClrs[byShift * 3]) +
              (pAlpha[byShift + iAShift] << 24);
    } else {
...
void CGammaControl::SetClrChannel(uint16_t *pBuf, BYTE c1, BYTE c2, int c3,
                                  uint16_t *ref) {
  ...
  for (int i = 0; i < size / 2; ++i) {
    ...
    *pBuf1++ =
        BoundBy(((c1 * i2 + c2 * i) * size1 + (2 * c2 - c1 - c3) * 2 * i * i2) /
                    (size1 * size1 / 512),
                0, 0xffff);
    *pBuf2++ =
        BoundBy(((c2 * i2 + c3 * i) * size1 + (2 * c2 - c1 - c3) * 2 * i * i2) /
                    (size1 * size1 / 512),
                0, 0xffff);
  }
  if (ref) {
...
void CGammaControl::Set(DWORD dwClr1, DWORD dwClr2, DWORD dwClr3, int nsize,
                        CGammaControl *ref) {
  if (nsize != size) {
    delete[] red;
  ...
  SetClrChannel(red, GetBValue(dwClr1), GetBValue(dwClr2), GetBValue(dwClr3),
                ref ? ref->red : 0);
  SetClrChannel(green, GetGValue(dwClr1), GetGValue(dwClr2), GetGValue(dwClr3),
                ref ? ref->green : 0);
  SetClrChannel(blue, GetRValue(dwClr1), GetRValue(dwClr2), GetRValue(dwClr3),
                ref ? ref->blue : 0);
}

DWORD CGammaControl::ApplyTo(DWORD dwClr) {
  // apply to reg, green and blue color component
  return RGBA(red[GetBValue(dwClr) * 256 / size] >> 8,
              green[GetGValue(dwClr) * 256 / size] >> 8,
              blue[GetRValue(dwClr) * 256 / size] >> 8, dwClr >> 24);
}

//--------------------------------------------------------------------

void CClrModAddMap::Reset(int iResX, int iResY, int iWdtPx, int iHgtPx,
                          int iOffX, int iOffY, uint32_t dwModClr,
                          uint32_t dwAddClr, int x0, int y0, uint32_t dwBackClr,
                          class CSurface *backsfc) {
  // set values
  iResolutionX = iResX;
  iResolutionY = iResY;
  ...
  if (dwBackClr && backsfc) {
    ...
    lpDDraw->DrawBoxDw(backsfc, x0, y0, x0 + iWdtPx - 1, y0 + iHgtPx - 1,
                       dwBackClr);
    dwModClr = 0xffffffff;
...
void CClrModAddMap::ReduceModulation(int cx, int cy, int iRadius1,
                                     int iRadius2) {
    ...
    if (d < iRadius2Sq) {
      if (d < iRadius1Sq)
        ...
        pCurr->dwModClr =
            fFadeTransparent
                ? (0xffffff +
                   (Min<uint32_t>(pCurr->dwModClr >> 24, 255 - iVis) << 24))
...
void CClrModAddMap::AddModulation(int cx, int cy, int iRadius1, int iRadius2,
                                  uint8_t byTransparency) {
  ...
  for (CClrModAdd *pCurr = pMap; i--; ++pCurr) {
    ...
    if (d < iRadius2Sq) {
      if (d < iRadius1Sq && !byTransparency)
        ...
        // partly visible
        int iVis = Min<int>(
            255 - Min<int>((iRadius2Sq - d) * 255 / (iRadius2Sq - iRadius1Sq),
                           255) +
                byTransparency,
            255);
        pCurr->dwModClr =
            fFadeTransparent
                ? (0xffffff +
                   (Max<uint32_t>(pCurr->dwModClr >> 24, 255 - iVis) << 24))
...
uint32_t CClrModAddMap::GetModAt(int x, int y) const {
  ...
  CColorFadeMatrix clrs(
      tx * iResolutionX, ty * iResolutionY, iResolutionX, iResolutionY,
      pMap[ty * iWdt + tx].dwModClr, pMap[ty * iWdt + tx2].dwModClr,
      pMap[ty2 * iWdt + tx].dwModClr, pMap[ty2 * iWdt + tx2].dwModClr);
...
CColorFadeMatrix::CColorFadeMatrix(int iX, int iY, int iWdt, int iHgt,
                                   uint32_t dwClr1, uint32_t dwClr2,
                                   uint32_t dwClr3, uint32_t dwClr4)
...
uint32_t CColorFadeMatrix::GetColorAt(int iX, int iY) {
  ...
  for (int iChan = 0; iChan < 4; ++iChan) {
    int clr =
        clrs[iChan].c0 + clrs[iChan].cx * iX / w + clrs[iChan].cy * iY / h;
  ...
  for (int i = 0; i < byNumVertices; ++i) {
    ...
    if (!fVtxIn) {
      ...
      if (fLastVtxIn) {
        ...
        --byNumVertices;
        --i;
        if (i + 1 < byNumVertices)
          memmove(pVtx, pVtx + 1, (byNumVertices - i - 1) * sizeof(*pVtx));
        --pVtx;
      }
      // set last vtx
      pLastVtx = &v;
      ...
      if (!fLastVtxIn) {
        ...
        if (dst) {
          ...
          if (++i < ++byNumVertices)
            memmove(pVtx + 1, pVtx, (byNumVertices - i) * sizeof(*pVtx));
          // calculate hit point
          float l = (fMax - fX * lx - fY * ly) / dst;
          pVtx->ftx = lx + dx * l;
          pVtx->fty = ly + dy * l;
...
BOOL CStdDDraw::SubPrimaryClipper(int iX1, int iY1, int iX2, int iY2) {
  ...
  SetPrimaryClipper(Max(iX1, ClipX1), Max(iY1, ClipY1), Min(iX2, ClipX2),
                    Min(iY2, ClipY2));
  return TRUE;
}
...
bool CStdDDraw::ClipPoly(CBltData &rBltData)
        ...
        // calculate floating point clips for performance reasons; note the +1
because it's the beginning of the next pixel that lays outside
        float fClipX1=ClipX1, fClipY1=ClipY1, fClipX2=ClipX2+1,
fClipY2=ClipY2+1;

        // first, decide which points lay outside the screen, and where they do
so
        int OutEdges[8]; ZeroMemory(&OutEdges, sizeof(OutEdges)); int
i,iCheckDir,iVtx2;
        int byAnyClip=0;
        ...
        for (i=0; i<rBltData.byNumVertices; ++i)
                if (OutEdges[i] & CPO_Outside)
                        ...
                        // so this edge is outside the screen - process adjacent
lines
                        float fOldX = rBltData.vtVtx[i].ftx, fOldY =
rBltData.vtVtx[i].fty, fNewX, fNewY;
                        ...
                        for (iCheckDir=-1; iCheckDir<2; iCheckDir+=2)
                                ...
                                // had the adjecent vertex moved in a way that
it's no longer on the line?
                                if (OutEdges[iVtx2] & (CPO_MovedSingle |
CPO_Remove))
                                        {
                                        if (iCheckDir==1)
                                                {
                                                OutEdges[i] |= CPO_Line2Out;
                                                // if the next vertex has set
this flag (i.e., the first, and this is the last),
                                                // the vertex removal check has
to be done
                                                // which is simply: if it didn't
move (out of the void...) it has to be removed
                                                if (!fVertexMoved) OutEdges[i]
|= (CPO_Remove | CPO_MovedSingle);
                                ...
                                // first, the xdir...if any of the clauses
succeeds, fdx cannot have been zero, so fdy/fdx is safe
                                if (fNewX<fClipX1 && (fNewX+fdx)>=fClipX1)
{ fNewY-=(fdy/fdx)*(fNewX-fClipX1); fNewX=fClipX1; }
                                else if (fNewX>=fClipX2 && (fNewX+fdx)<fClipX2)
{ fNewY-=(fdy/fdx)*(fNewX-fClipX2); fNewX=fClipX2; }
                                // now, ydir must regard that vertices, that the
crossing might have happend at xdir first
                                // so check if they're still out, and don't just
copmpare the iOut-bits!
                                if (fNewY < fClipY1 && (fNewY+fdy)>=fClipY1)
{ fNewX-=(fdx/fdy)*(fNewY-fClipY1); fNewY=fClipY1; }
                                else if (fNewY>=fClipY2 && (fNewY+fdy)<fClipY2)
{ fNewX-=(fdx/fdy)*(fNewY-fClipY2); fNewY=fClipY2; }

                                // check if any valid crossing has been found
                                // (it might have been bogus crossings outside
the screen range)
                                if (fNewX>=fClipX1 && fNewX<=fClipX2 &&
fNewY>=fClipY1 && fNewY<=fClipY2)
                                        {
                                        // crossing found: move vertex!

                                        // set border flags
                                        int iBorderFlag=0;
                                        if (fNewX == fClipX1) iBorderFlag |=
CPO_BLeft;
                                        if (fNewY == fClipY1) iBorderFlag |=
CPO_BTop;
                                        if (fNewX == fClipX2) iBorderFlag |=
CPO_BRight;
                                        if (fNewY == fClipY2) iBorderFlag |=
CPO_BBottom;

                                        // move the vertex only if not already
done so...
                                        if (!fVertexMoved)
                                                {
...
vertex
                                                // vertex position is always the
one checked, because this routine is not called in the first (iCheckDir==-1)
call
                                                // move old vertices down
                                                for (int
j=rBltData.byNumVertices; j>iVtx2; --j)
                                                        {
                                                        // move vertices
                                                        rBltData.vtVtx[j] =
rBltData.vtVtx[j-1];
...
OutEdges[j-1];
                                                ...
                                                rBltData.vtVtx[iVtx2].ftx =
fNewX; rBltData.vtVtx[iVtx2].fty = fNewY;
                                                ++rBltData.byNumVertices;
                                                OutEdges[iVtx2]=(OutEdges[i] &
~CPO_Outside) | iBorderFlag;
                                                // Since this has been the last
direction, a jump of i can be done safely here
                                                // the jump skips the otherwise
unnecessarily checked vertex iVtx2 (i.e., i+1)
                                                ++i;
                                                }
                                        ...
                                        {
                                        // a crossing to the adjacent vertex
could not be found - so the line becomes unnecessary
                                        if (iCheckDir==1)
                                                {
                                                if (fVertexMoved)
                                                        {
                                                        // the vertex has
already moved (or was removed), processing the other line
                                                        // simply mark it as not
to be processed by the next vertex-movement
                                                        OutEdges[i] |=
CPO_MovedSingle;
                                                        }
                                                else
                                                        {
                                                        // no adjacent line
intersected: the vertex can really be removed
                                                        // however, it must stay
in place for now, so the next vertex can be checked correctly
                                                        OutEdges[i] |=
(CPO_Remove | CPO_MovedSingle);
                                                        }
                                                // in any case, the second line
lay outside
                                                OutEdges[i] |= CPO_Line2Out;
                                                }
                                        else
                                                {
                                                OutEdges[i] |= CPO_Line1Out;
                                                // normally, this can only
happen to the first vertex, because this line is also processed
                                                // by the previous vertex, and
either CPO_MovedSingle or CPO_Remove must have been set
                                                // - both causing these checks
to be skipped completely - however, floating point inaccuracy
                                                // could lead here as well
                                                // mark the vertex to be removed
- if a crossing is found by the next line, it has to insert
                                                // a new vertex, so set
fVertexMoved to true
        ...
        // searching for lost edges, first get the last valid edge-vertex to
begin with
        // searching backwards, so the following loop can iterate up to this
vertex
        ...
        if (iLastValidBrdVtx<0)
                {
                // this can only mean two things: either the whole screen is
covered, or nothing
                // well, better assume nothing for now...
                return false;
                // whole screen
                rBltData.byNumVertices = 4;
                rBltData.vtVtx[0].ftx = fClipX1; rBltData.vtVtx[0].fty =
fClipY1;
                rBltData.vtVtx[1].ftx = fClipX2; rBltData.vtVtx[1].fty =
fClipY1;
                rBltData.vtVtx[2].ftx = fClipX2; rBltData.vtVtx[2].fty =
fClipY2;
                rBltData.vtVtx[3].ftx = fClipX1; rBltData.vtVtx[3].fty =
fClipY2;
                return true;
                }
...
rBltData.vtVtx[i];
        ...
        // d'oh - now overlapped edges are lost; x and y-clips should be applied
seperately
        // gwe5...
...
bool CStdDDraw::ClipPoly(CBltData &rBltData) {
  ...
  bool transformed =
      rBltData.pTransform && inverse.SetAsInv(*rBltData.pTransform);
  if (transformed)
    for (int i = 0; i < rBltData.byNumVertices; ++i)
      rBltData.pTransform->TransformPoint(rBltData.vtVtx[i].ftx,
                                          rBltData.vtVtx[i].fty);
  // calculate floating point clips for performance reasons; note the +1 because
  // it's the beginning of the next pixel that lays outside
  float fClipX1 = float(ClipX1), fClipY1 = float(ClipY1),
        fClipX2 = float(ClipX2) + 1, fClipY2 = float(ClipY2) + 1;
  fClipX1 += DDrawCfg.fBlitOff;
  fClipY1 += DDrawCfg.fBlitOff;
  fClipX2 += DDrawCfg.fBlitOff;
  fClipY2 += DDrawCfg.fBlitOff;
  // clip against left
...
void CStdDDraw::BlitLandscape(SURFACE sfcSource, SURFACE sfcSource2,
                              SURFACE sfcSource3, int fx, int fy,
                              SURFACE sfcTarget, int tx, int ty, int wdt,
                              int hgt) {
  Blit(sfcSource, float(fx), float(fy), float(wdt), float(hgt), sfcTarget, tx,
       ty, wdt, hgt, FALSE);
}
...
void CStdDDraw::Blit8Fast(CSurface8 *sfcSource, int fx, int fy,
  ...
  for (int ycnt = 0; ycnt < hgt; ++ycnt)
    for (int xcnt = 0; xcnt < wdt; ++xcnt) {
      ...
      if (byPix)
        DrawPixInt(sfcTarget, (float)(tx + xcnt), (float)(ty + ycnt),
                   sfcSource->pPal->GetClr(byPix));
    }
  // unlock
  if (!fRender) sfcTarget->Unlock();
}

BOOL CStdDDraw::Blit(SURFACE sfcSource, float fx, float fy, float fwdt,
                     float fhgt, SURFACE sfcTarget, int tx, int ty, int twdt,
                     int thgt, BOOL fSrcColKey, CBltTransform *pTransform) {
  // safety
  if (!sfcSource || !sfcTarget || !twdt || !thgt || !fwdt || !fhgt)
    return FALSE;
  // emulated blit?
  if (!sfcTarget->IsRenderTarget())
    return Blit8(sfcSource, int(fx), int(fy), int(fwdt), int(fhgt), sfcTarget,
                 tx, ty, twdt, thgt, fSrcColKey, pTransform);
  // calc stretch
  ...
  for (int iY = iTexY; iY < iTexY2; ++iY) {
    for (int iX = iTexX; iX < iTexX2; ++iX) {
      ...
      // get current blitting offset in texture (beforing any
      // last-tex-size-changes)
      ...
      /*BltData.TexPos.SetMoveScale(-tTexBlt.left - DDrawCfg.fBlitOff,
      -tTexBlt.top  - DDrawCfg.fBlitOff, 1, 1);
      ...
      // now, finally, move in texture - this must be done last, so no stupid
      zoom is applied...
      BltData.TexPos.MoveScale(((float) fTexBlt.left + DDrawCfg.fTexIndent) /
      iTexSize,
              ((float) fTexBlt.top  + DDrawCfg.fTexIndent) / iTexSize, 1, 1);*/
      // Set resulting matrix directly
      BltData.TexPos.SetMoveScale(
          (fTexBlt.left + DDrawCfg.fTexIndent) / iTexSize -
              (tTexBlt.left + DDrawCfg.fBlitOff) / scaleX2,
          (fTexBlt.top + DDrawCfg.fTexIndent) / iTexSize -
              (tTexBlt.top + DDrawCfg.fBlitOff) / scaleY2,
          1 / scaleX2, 1 / scaleY2);
      // set up blit data as rect
      BltData.byNumVertices = 4;
      BltData.vtVtx[0].ftx = tTexBlt.left + DDrawCfg.fBlitOff;
      BltData.vtVtx[0].fty = tTexBlt.top + DDrawCfg.fBlitOff;
      BltData.vtVtx[1].ftx = tTexBlt.right + DDrawCfg.fBlitOff;
      BltData.vtVtx[1].fty = tTexBlt.top + DDrawCfg.fBlitOff;
      BltData.vtVtx[2].ftx = tTexBlt.right + DDrawCfg.fBlitOff;
      BltData.vtVtx[2].fty = tTexBlt.bottom + DDrawCfg.fBlitOff;
      BltData.vtVtx[3].ftx = tTexBlt.left + DDrawCfg.fBlitOff;
      BltData.vtVtx[3].fty = tTexBlt.bottom + DDrawCfg.fBlitOff;
      ...
      // base blit
      PerformBlt(BltData, pBaseTex, BlitModulated ? BlitModulateClr : 0xffffff,
                 !!(dwBlitMode & C4GFXBLIT_MOD2), fExact);
      // overlay
      if (fBaseSfc) {
        ...
        PerformBlt(BltData, pTex, dwModClr,
                   !!(dwBlitMode & C4GFXBLIT_CLRSFC_MOD2), fExact);
      }
    }
  }
  // reset texture
...
BOOL CStdDDraw::Blit8(SURFACE sfcSource, int fx, int fy, int fwdt, int fhgt,
  ...
  Transform.SetMoveScale(tx - (float)fx * twdt / fwdt,
                         ty - (float)fy * thgt / fhgt, (float)twdt / fwdt,
                         (float)thgt / fhgt);
  Transform *= *pTransform;
  CBltTransform TransformBack;
  TransformBack.SetAsInv(Transform);
  float ttx0 = (float)tx, tty0 = (float)ty, ttx1 = (float)(tx + twdt),
        tty1 = (float)(ty + thgt);
  float ttx2 = (float)ttx0, tty2 = (float)tty1, ttx3 = (float)ttx1,
        tty3 = (float)tty0;
  ...
  int ttxMax = Min<int>((int)ceil(Max(Max(ttx0, ttx1), Max(ttx2, ttx3))),
                        sfcTarget->Wdt);
  int ttyMin = Max<int>((int)floor(Min(Min(tty0, tty1), Min(tty2, tty3))), 0);
  int ttyMax = Min<int>((int)ceil(Max(Max(tty0, tty1), Max(tty2, tty3))),
                        sfcTarget->Hgt);
  ...
  for (int y = ttyMin; y < ttyMax; ++y)
    for (int x = ttxMin; x < ttxMax; ++x) {
      ...
      if (ifx < fx || ify < fy || ifx >= fx + fwdt || ify >= fy + fhgt)
        continue;
...
BOOL CStdDDraw::BlitRotate(SURFACE sfcSource, int fx, int fy, int fwdt,
                           ...
                           int twdt, int thgt, int iAngle, bool fTransparency) {
  // rendertarget?
  if (sfcTarget->IsRenderTarget()) {
    CBltTransform rot;
    rot.SetRotate(iAngle, (float)(tx + tx + twdt) / 2,
                  (float)(ty + ty + thgt) / 2);
    return Blit(sfcSource, float(fx), float(fy), float(fwdt), float(fhgt),
                sfcTarget, tx, ty, twdt, thgt, TRUE, &rot);
  }
      ...
      for (ycnt = 0; ycnt < thgt; ycnt++)
        if (Inside(cpcy = ty + tcy - thgt / 2 + ycnt, 0, sfcTarget->Hgt - 1))
          for (xcnt = 0; xcnt < twdt; xcnt++)
            if (Inside(cpcx = tx + tcx - twdt / 2 + xcnt, 0,
                       sfcTarget->Wdt - 1))
              sfcTarget->BltPix(cpcx, cpcy, sfcSource, xcnt * fwdt / twdt + fx,
                                ycnt * fhgt / thgt + fy, fTransparency);
      break;
    ...
    case 9000:
      for (ycnt = 0; ycnt < thgt; ycnt++)
        if (Inside(cpcx = ty + tcy + thgt / 2 - ycnt, 0, sfcTarget->Wdt - 1))
          for (xcnt = 0; xcnt < twdt; xcnt++)
            if (Inside(cpcy = tx + tcx - twdt / 2 + xcnt, 0,
                       sfcTarget->Hgt - 1))
              sfcTarget->BltPix(cpcx, cpcy, sfcSource, xcnt * fwdt / twdt + fx,
                                ycnt * fhgt / thgt + fy, fTransparency);
      break;
    ...
    default:
      ...
      for (ycnt = 0; ycnt < fhgt; ycnt++) {
        ...
        for (xcnt = 0; xcnt < fwdt; xcnt++) {
          ...
          // Place in dest
          sfcTarget->BltPix(tx + tcx + npcx, ty + tcy + npcy, sfcSource,
                            xcnt + fx, ycnt + fy, fTransparency);
          sfcTarget->BltPix(tx + tcx + npcx + 1, ty + tcy + npcy, sfcSource,
                            xcnt + fx, ycnt + fy, fTransparency);
        }
      }
      break;
...
BOOL CStdDDraw::AttachPrimaryPalette(
    SURFACE sfcSurface) { /*
                         if (sfcSurface->SetPaletteEntries(0, lpPalette)!=DD_OK)
                         return FALSE;*/
  return TRUE;
}

BOOL CStdDDraw::BlitSurface(SURFACE sfcSurface, SURFACE sfcTarget, int tx,
                            int ty, bool fBlitBase) {
  if (fBlitBase) {
    Blit(sfcSurface, 0.0f, 0.0f, (float)sfcSurface->Wdt, (float)sfcSurface->Hgt,
         sfcTarget, tx, ty, sfcSurface->Wdt, sfcSurface->Hgt, false);
    return TRUE;
  } else {
    if (!sfcSurface) return FALSE;
...
BOOL CStdDDraw::BlitSurfaceTile(SURFACE sfcSurface, SURFACE sfcTarget, int iToX,
  ...
  for (iY = iToY + iOffsetY; iY < iToY + iToHgt; iY += iSourceHgt) {
    // Vertical blit size
    iBlitY = Max(iToY - iY, 0);
    iBlitHgt = Min(iSourceHgt, iToY + iToHgt - iY) - iBlitY;
    // Horizontal blits
    for (iX = iToX + iOffsetX; iX < iToX + iToWdt; iX += iSourceWdt) {
      // Horizontal blit size
      iBlitX = Max(iToX - iX, 0);
      iBlitWdt = Min(iSourceWdt, iToX + iToWdt - iX) - iBlitX;
      // Blit
      if (!Blit(sfcSurface, float(iBlitX), float(iBlitY), float(iBlitWdt),
                float(iBlitHgt), sfcTarget, iX + iBlitX, iY + iBlitY, iBlitWdt,
                iBlitHgt, fSrcColKey))
        return FALSE;
    }
  }
...
BOOL CStdDDraw::BlitSurfaceTile2(SURFACE sfcSurface, SURFACE sfcTarget,
                                 int iToX, int iToY, int iToWdt, int iToHgt,
                                 int iOffsetX, int iOffsetY, BOOL fSrcColKey) {
  // if it's a render target, simply blit with repeating texture
  // repeating textures, however, aren't currently supported
  /*if (sfcTarget->IsRenderTarget())
          return Blit(sfcSurface, iOffsetX, iOffsetY, iToWdt, iToHgt, sfcTarget,
     iToX, iToY, iToWdt, iToHgt, FALSE);*/
  ...
  for (int iY = iStartY; ty < iToHgt; iY += iTileHgt) {
    ...
    int iOver = ty + iBlitHgt - iToHgt;
    if (iOver > 0) iBlitHgt -= iOver;
    // blit horizontal
    tx = 0;
    for (int iX = iStartX; tx < iToWdt; iX += iTileWdt) {
      ...
      iOver = tx + iBlitWdt - iToWdt;
      if (iOver > 0) iBlitWdt -= iOver;
      // blit
      if (!Blit(sfcSurface, float(iBlitX), float(iBlitY), float(iBlitWdt),
                float(iBlitHgt), sfcTarget, tx + iToX, ty + iToY, iBlitWdt,
                iBlitHgt, fSrcColKey))
        return FALSE;
      // next col
      tx += iBlitWdt;
...
BOOL CStdDDraw::TextOut(const char *szText, CStdFont &rFont, float fZoom,
                        SURFACE sfcDest, int iTx, int iTy, DWORD dwFCol,
                        BYTE byForm, bool fDoMarkup) {
  CMarkup Markup(true);
  static char szLinebuf[2500 + 1];
  for (int cnt = 0; SCopySegmentEx(szText, cnt, szLinebuf,
                                   fDoMarkup ? '|' : '\n', '\n', 2500);
       cnt++, iTy += int(fZoom * rFont.iLineHgt))
    if (!StringOut(szLinebuf, sfcDest, iTx, iTy, dwFCol, byForm, fDoMarkup,
                   Markup, &rFont, fZoom))
      return FALSE;
  return TRUE;
}

BOOL CStdDDraw::StringOut(const char *szText, CStdFont &rFont, float fZoom,
                          SURFACE sfcDest, int iTx, int iTy, DWORD dwFCol,
                          BYTE byForm, bool fDoMarkup) {
  // init markup
  CMarkup Markup(true);
  // output string
  return StringOut(szText, sfcDest, iTx, iTy, dwFCol, byForm, fDoMarkup, Markup,
                   &rFont, fZoom);
}

bool CStdDDraw::StringOut(const char *szText, SURFACE sfcDest, int iTx, int iTy,
                          DWORD dwFCol, BYTE byForm, bool fDoMarkup,
                          CMarkup &Markup, CStdFont *pFont, float fZoom) {
  // clip
  if (ClipAll) return true;
  // safety
  ...
  switch (byForm) {
    case ACenter:
      iFlags |= STDFONT_CENTERED;
      break;
    case ARight:
      iFlags |= STDFONT_RIGHTALGN;
      break;
  }
...
void CStdDDraw::DrawPix(SURFACE sfcDest, float tx, float ty, DWORD dwClr) {
  ...
  if (fUseClrModMap) ModulateClr(dwClr, pClrModMap->GetModAt((int)tx, (int)ty));
  // Draw
  DrawPixInt(sfcDest, tx, ty, dwClr);
}

void CStdDDraw::DrawBox(SURFACE sfcDest, int iX1, int iY1, int iX2, int iY2,
                        BYTE byCol) {
  // get color
  ...
  if (!sfcDest->IsRenderTarget()) {
    ...
    if (iX1 < Max(0, ClipX1)) iX1 = Max(0, ClipX1);
    if (iX2 > Min(iSfcWdt - 1, ClipX2)) iX2 = Min(iSfcWdt - 1, ClipX2);
    if (iY1 < Max(0, ClipY1)) iY1 = Max(0, ClipY1);
    if (iY2 > Min(iSfcHgt - 1, ClipY2)) iY2 = Min(iSfcHgt - 1, ClipY2);
    // Set lines
...
void CStdDDraw::DrawHorizontalLine(SURFACE sfcDest, int x1, int x2, int y,
  ...
  clpx1 = Max(0, ClipX1);
  clpy1 = Max(0, ClipY1);
  clpx2 = Min(lWdt - 1, ClipX2);
  clpy2 = Min(lHgt - 1, ClipY2);
  // Outside clip check
...
void CStdDDraw::DrawFrameDw(SURFACE sfcDest, int x1, int y1, int x2, int y2,
                            DWORD dwClr)  // make these parameters float...?
{
...
void CStdDDraw::DrawPatternedCircle(SURFACE sfcDest, int x, int y, int r,
                                    BYTE col, CPattern &Pattern1,
                                    CPattern &Pattern2, CStdPalette &rPal) {
  if (!sfcDest->Lock()) return;
...
void CStdDDraw::SurfaceAllowColor(SURFACE sfcSfc, DWORD *pdwColors,
                                  int iNumColors, BOOL fAllowZero) {
  // safety
  if (!sfcSfc) return;
...
void CStdDDraw::Grayscale(SURFACE sfcSfc, int32_t iOffset) {
  ...
  for (ycnt = 0; ycnt < hgt; ycnt++) {
    for (xcnt = 0; xcnt < wdt; xcnt++) {
      DWORD dwColor = sfcSfc->GetPixDw(xcnt, ycnt, false);
      uint32_t r = GetRValue(dwColor), g = GetGValue(dwColor),
               b = GetBValue(dwColor), a = dwColor >> 24;
...
DWORD CStdDDraw::ApplyGammaTo(DWORD dwClr) { return Gamma.ApplyTo(dwClr); }
...
CStdDDraw *DDrawInit(CStdApp *pApp, BOOL Fullscreen, BOOL fUsePageLock,
  ...
  switch (iGfxEngine = Engine) {
    ...
    case GFXENGN_DIRECTX:
      lpDDraw = new CStdD3D(false);
      break;
    case GFXENGN_DIRECTXS:
      lpDDraw = new CStdD3D(true);
      break;
#endif
#ifdef USE_GL
    case GFXENGN_OPENGL:
...
bool CStdDDraw::Init(CStdApp *pApp, BOOL Fullscreen, BOOL fUsePageLock,
                     int iBitDepth, unsigned int iMonitor) {
  ...
  if (!CreateDirectDraw()) return Error("  CreateDirectDraw failure.");

  // set monitor info (Monitor-var and target rect)
  DebugLog("  SetOutput adapter...");
  if (!SetOutputAdapter(iMonitor)) return Error("  Output adapter failure.");
  ...
  if (!CreatePrimaryClipper()) return Error("  Clipper failure.");
...
void CStdDDraw::DrawBoxFade(SURFACE sfcDest, int iX, int iY, int iWdt, int iHgt,
                            DWORD dwClr1, DWORD dwClr2, DWORD dwClr3,
                            DWORD dwClr4, int iBoxOffX, int iBoxOffY) {
  // clipping not performed - this fn should be called for clipped rects only
  // apply modulation map: Must sectionize blit
  if (fUseClrModMap) {
    int iModResX = pClrModMap ? pClrModMap->GetResolutionX()
                              : CClrModAddMap::iDefResolutionX;
    int iModResY = pClrModMap ? pClrModMap->GetResolutionY()
                              : CClrModAddMap::iDefResolutionY;
    iBoxOffX %= iModResX;
    iBoxOffY %= iModResY;
    if (iWdt + iBoxOffX > iModResX || iHgt + iBoxOffY > iModResY) {
      ...
      int w, h;
      for (int y = iY, H = iHgt; H > 0;
           (y += h), (H -= h), (iMaxH = iModResY)) {
        h = Min<int>(H, iMaxH);
        int iMaxW = iModResX - iBoxOffX;
        for (int x = iX, W = iWdt; W > 0;
             (x += w), (W -= w), (iMaxW = iModResX)) {
          w = Min<int>(W, iMaxW);
          DrawBoxFade(sfcDest, x, y, w, h, clrs.GetColorAt(x, y),
                      clrs.GetColorAt(x + w, y), clrs.GetColorAt(x, y + h),
                      clrs.GetColorAt(x + w, y + h), 0, 0);
        }
      }
      return;
    }
...
void CStdDDraw::DrawBoxDw(SURFACE sfcDest, int iX1, int iY1, int iX2, int iY2,
                          DWORD dwClr) {
  // manual clipping?
  if (DDrawCfg.ClipManuallyE) {
    int iOver;
  ...
  DrawBoxFade(sfcDest, iX1, iY1, iX2 - iX1 + 1, iY2 - iY1 + 1, dwClr, dwClr,
              dwClr, dwClr, 0, 0);
}
