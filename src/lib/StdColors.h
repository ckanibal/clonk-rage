...
// helper function
#define RGBA(r, g, b, a)                              \
  (((DWORD)(a) << 24) | (((DWORD)(r) & 0xff) << 16) | \
   (((DWORD)(g) & 0xff) << 8) | ((b)&0xff))
...
inline void BltAlpha(DWORD &dwDst, DWORD dwSrc) {
  ...
  dwDst = Min<uint32_t>((int(dwDst & 0xff) * byAlphaDst +
                         int(dwSrc & 0xff) * byAlphaSrc) >>
                            8,
                        0xff) |  // blue
          Min<uint32_t>((int(dwDst & 0xff00) * byAlphaDst +
                         int(dwSrc & 0xff00) * byAlphaSrc) >>
                                8 &
                            0xff00,
                        0xff00) |  // green
          Min<uint32_t>((int(dwDst & 0xff0000) * byAlphaDst +
                         int(dwSrc & 0xff0000) * byAlphaSrc) >>
                                8 &
                            0xff0000,
                        0xff0000) |                                 // red
...
inline void BltAlphaAdd(DWORD &dwDst, DWORD dwSrc) {
  ...
  dwDst =
      Min<uint32_t>((dwDst & 0xff) + ((int(dwSrc & 0xff) * byAlphaSrc) >> 8),
                    0xff) |  // blue
      Min<uint32_t>(
          (dwDst & 0xff00) + (int(dwSrc >> 8 & 0xff) * byAlphaSrc) & 0x00ffff00,
          0xff00) |  // green
      Min<uint32_t>(
          (dwDst & 0xff0000) + (int(dwSrc >> 8 & 0xff00) * byAlphaSrc) &
              0xffff0000,
          0xff0000) |                                           // red
      uint32_t(Max((int)(dwDst >> 24) - byAlphaSrc, 0)) << 24;  // alpha
}
...
inline void ModulateClr(DWORD &dwDst, DWORD dwMod)  // modulate two color values
{
  ...
  dwDst = ((dwDst & 0xff) * (dwMod & 0xff)) >> 8 |                     // blue
          ...
          uint32_t(Min(iA1 + iA2 - ((iA1 * iA2) >> 8), 255))
              << 24;  // alpha (=255-(255*iA1)(255*iA2)/255)
}

inline void ModulateClrA(
    DWORD &dwDst, DWORD dwMod)  // modulate two color values and add alpha value
{
...
inline void ModulateClrMOD2(DWORD &dwDst, DWORD dwMod)  // clr1+clr2-0.5
{
  ...
  dwDst =
      (BoundBy<int>(((int)(dwDst & 0xff) + (dwMod & 0xff) - 0x7f) << 1, 0,
                    0xff) &
       0xff) |  // B
      (BoundBy<int>(((int)(dwDst & 0xff00) + (dwMod & 0xff00) - 0x7f00) << 1, 0,
                    0xff00) &
       0xff00) |  // G
      (BoundBy<int>(
           ((int)(dwDst & 0xff0000) + (dwMod & 0xff0000) - 0x7f0000) << 1, 0,
           0xff0000) &
       0xff0000) |  // R
      Min<uint32_t>((dwDst >> 24) + (dwMod >> 24), 0xff) << 24;
}
...
inline DWORD GetClrModulation(DWORD dwSrcClr, DWORD dwDstClr, DWORD &dwBack) {
  ...
  // get source color
  BYTE sB = BYTE(dwSrcClr);
  dwSrcClr = dwSrcClr >> 8;
  BYTE sG = BYTE(dwSrcClr);
  dwSrcClr = dwSrcClr >> 8;
  BYTE sR = BYTE(dwSrcClr);
  dwSrcClr = dwSrcClr >> 8;
  // get dest color
  BYTE dB = BYTE(dwDstClr);
  dwDstClr = dwDstClr >> 8;
  BYTE dG = BYTE(dwDstClr);
  dwDstClr = dwDstClr >> 8;
  BYTE dR = BYTE(dwDstClr);
  dwDstClr = dwDstClr >> 8;
  // get difference
  int cR = (int)dR - sR;
  int cG = (int)dG - sG;
  ...
  if (!sG) sG = 1;
  if (!sB) sB = 1;
  return RGBA(Min((int)dR * 256 / sR, 255), Min((int)dG * 256 / sG, 255),
              Min((int)dB * 256 / sB, 255), diffN);
}

inline DWORD NormalizeColors(DWORD &dwClr1, DWORD &dwClr2, DWORD &dwClr3,
                             DWORD &dwClr4) {
  // normalize the colors to a color in the middle
  // combine clr1 and clr2 to clr1
  ModulateClr(dwClr1, dwClr2);
  LightenClr(dwClr1);
  // combine clr3 and clr4 to clr3
  ModulateClr(dwClr3, dwClr4);
  LightenClr(dwClr3);
  // combine clr1 and clr3 to clr1
  ModulateClr(dwClr1, dwClr3);
  LightenClr(dwClr1);
...
inline WORD ClrDw2W(DWORD dwClr) {
  return WORD((dwClr & 0x000000f0) >> 4) | WORD((dwClr & 0x0000f000) >> 8) |
         WORD((dwClr & 0x00f00000) >> 12) | WORD((dwClr & 0xf0000000) >> 16);
}

inline DWORD ClrW2Dw(WORD wClr) {
  return (DWORD)((wClr & 0x000f) << 4) | ((wClr & 0x00f0) << 8) |
         ((wClr & 0x0f00) << 12) | ((wClr & 0xf000) << 16);
}

inline bool rgb2xyY(double r, double g, double b, double *px, double *py,
                    double *pY)  // linear rgb to CIE xyY
{
...
inline bool xy2upvp(double x, double y, double *pu,
                    double *pv)  // CIE xy to u'v'
{
...
inline bool upvp2xy(double u, double v, double *px,
                    double *py)  // u'v' to CIE xy
{
  if (!v) return false;
...
inline bool RGB2rgb(int R, int G, int B, double *pr, double *pg, double *pb,
                    double gamma = 2.2)  // monitor RGB (0 to 255) to linear rgb
                                         // (0.0 to 1.0) assuming default gamma
                                         // 2.2
{
...
struct CStdPalette {
  ...
  DWORD GetClr(BYTE byCol) {
    return RGB(Colors[byCol * 3 + 2], Colors[byCol * 3 + 1],
               Colors[byCol * 3]) +
           (Alpha[byCol] << 24);
  }
...
// clrmod-add-map to cover a drawing range in which all draws shall be adjusted
// by the map
class CClrModAddMap {
 private:
  CClrModAdd *pMap;
  ...
  int iOffX,
      iOffY;  // offset to add to drawing positions before applying the map
  bool fFadeTransparent;  // if set, ReduceModulation and AddModulation fade
 ...
 public:
  CClrModAddMap()
      : pMap(NULL),
        ...
        iResolutionX(iDefResolutionX),
        iResolutionY(iDefResolutionY) {}
  ~CClrModAddMap() { delete[] pMap; }

  void Reset(int iResX, int iResY, int iWdtPx, int iHgtPx, int iOffX, int iOffY,
             uint32_t dwModClr, uint32_t dwAddClr, int x0, int y0,
             uint32_t dwBackClr = 0,
             class CSurface *backsfc = NULL);  // reset all of map to given
                                               // values; uses transparent mode
                                               // and clears rect if a back
                                               // color is given
  void ReduceModulation(
      int cx, int cy, int iRadius1,
      int iRadius2);  // reveal all within iRadius1; fade off until iRadius2
  void AddModulation(int cx, int cy, int iRadius1, int iRadius2,
                     uint8_t byTransparency);  // hide all within iRadius1; fade
                                               // off until iRadius2
...
class CColorFadeMatrix {
 ...
 public:
  CColorFadeMatrix(int iX, int iY, int iWdt, int iHgt, uint32_t dwClr1,
                   uint32_t dwClr2, uint32_t dwClr3, uint32_t dwClr4);
