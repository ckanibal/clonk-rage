...
struct StdDefaultAdapt {
  ...
  StdDefaultAdapt(T &rValue, const D &rDefault)
      : rValue(rValue), rDefault(rDefault) {}
  inline void CompileFunc(StdCompiler *pComp) const {
    try {
#ifdef STDCOMPILER_EXCEPTION_WORKAROUND
      if (!pComp->ValueSafe(rValue)) rValue = rDefault;
#else
      pComp->Value(rValue);
#endif
...
template <class T, class D>
inline StdDefaultAdapt<T, D> mkDefaultAdapt(T &&rValue, const D &rDefault) {
  return StdDefaultAdapt<T, D>(rValue, rDefault);
}
...
struct StdNamingAdapt {
  ...
  template <class D>
  inline StdNamingAdapt &operator=(const D &nValue) {
    rValue = nValue;
    return *this;
  }
...
template <class T>
inline StdNamingAdapt<T> mkNamingAdapt(T &&rValue, const char *szName) {
  return StdNamingAdapt<T>(rValue, szName);
}
...
struct StdNamingDefaultAdapt {
  ...
  bool fStoreDefault;
  StdNamingDefaultAdapt(T &rValue, const char *szName, const D &rDefault,
                        bool fPrefillDefault, bool fStoreDefault)
      : rValue(rValue),
        szName(szName),
        rDefault(rDefault),
        fPrefillDefault(fPrefillDefault),
        fStoreDefault(fStoreDefault) {}
  inline void CompileFunc(StdCompiler *pComp) const {
    // Default check
    if (pComp->hasNaming() && pComp->isDecompiler() && rValue == rDefault &&
        !fStoreDefault) {
    ...
    try {
      ...
      if (pComp->Name(szName)) {
        if (fPrefillDefault && pComp->isCompiler())
          rValue = rDefault;  // default prefill if desired
...
template <class T, class D>
inline StdNamingDefaultAdapt<T, D> mkNamingAdapt(T &&rValue, const char *szName,
                                                 const D &rDefault,
                                                 bool fPrefillDefault = false,
                                                 bool fStoreDefault = false) {
  return StdNamingDefaultAdapt<T, D>(rValue, szName, rDefault, fPrefillDefault,
                                     fStoreDefault);
}
...
template <class T>
inline StdDecompileAdapt<T> mkDecompileAdapt(const T &rValue) {
  return StdDecompileAdapt<T>(rValue);
}
...
struct StdRuntimeValueAdapt {
  ...
  template <class D>
  inline StdRuntimeValueAdapt<T> &operator=(const D &nValue) {
    rValue = nValue;
    return *this;
  }
...
template <class T>
inline StdRuntimeValueAdapt<T> mkRuntimeValueAdapt(T rValue) {
  return StdRuntimeValueAdapt<T>(rValue);
}
...
struct StdStringAdapt {
  ...
  inline bool operator==(const char *szDefault) const {
    return SEqual(szString, szDefault);
  }
  inline StdStringAdapt &operator=(const char *szDefault) {
    SCopy(szDefault, szString, iMaxLength);
    return *this;
  }
  ALLOW_TEMP_TO_REF(StdStringAdapt)
};
inline StdStringAdapt mkStringAdapt(
    char *szString, int iMaxLength,
    StdCompiler::RawCompileType eRawType = StdCompiler::RCT_Escaped) {
  return StdStringAdapt(szString, iMaxLength, eRawType);
}
#define mkStringAdaptM(szString) \
  mkStringAdapt(szString, (sizeof(szString) / sizeof(*szString)) - 1)
#define mkStringAdaptMA(szString)                                     \
  mkStringAdapt(szString, (sizeof(szString) / sizeof(*szString)) - 1, \
                StdCompiler::RCT_All)
#define mkStringAdaptMI(szString)                                     \
  mkStringAdapt(szString, (sizeof(szString) / sizeof(*szString)) - 1, \
                StdCompiler::RCT_Idtf)
#define mkStringAdaptMIE(szString)                                    \
  mkStringAdapt(szString, (sizeof(szString) / sizeof(*szString)) - 1, \
                StdCompiler::RCT_IdtfAllowEmpty)
...
struct StdRawAdapt {
  ...
  size_t iSize;
  StdCompiler::RawCompileType eRawType;
  StdRawAdapt(void *pData, size_t iSize,
              StdCompiler::RawCompileType eRawType = StdCompiler::RCT_Escaped)
      : pData(pData), iSize(iSize), eRawType(eRawType) {}
  inline void CompileFunc(StdCompiler *pComp) const {
    pComp->Raw(pData, iSize, eRawType);
  }
  inline bool operator==(const void *pDefault) const {
    return !memcmp(pDefault, pData, iSize);
  }
  inline StdRawAdapt &operator=(const void *pDefault) {
    memcpy(pData, pDefault, iSize);
    return *this;
  }
};
inline StdRawAdapt mkRawAdapt(
...
struct StdIntAdapt {
  ...
  template <class D>
  inline StdIntAdapt &operator=(const D &nValue) {
    rValue = nValue;
    return *this;
  }
...
template <class T>
inline StdIntAdapt<T> mkIntAdapt(T &rValue) {
  return StdIntAdapt<T>(rValue);
}
template <class int_t, class T>
StdIntAdapt<T, int_t> mkIntAdaptT(T &rValue) {
  return StdIntAdapt<T, int_t>(rValue);
}
...
struct StdCastAdapt {
  ...
  template <class D>
  inline StdCastAdapt &operator=(const D &nValue) {
    rValue = nValue;
    return *this;
  }
...
StdCastAdapt<T, to_t> mkCastAdapt(T &rValue) {
  return StdCastAdapt<T, to_t>(rValue);
}
template <class T>
StdCastAdapt<T, int32_t> mkCastIntAdapt(T &rValue) {
  return StdCastAdapt<T, int32_t>(rValue);
}
...
struct StdArrayAdapt {
  ...
  inline bool operator==(const T &rDefault) const {
    for (int i = 0; i < iSize; i++)
      if (pArray[i] != rDefault) return false;
    return true;
  }
  inline StdArrayAdapt &operator=(const T &rDefault) {
    for (int i = 0; i < iSize; i++) pArray[i] = rDefault;
    return *this;
  }
  inline bool operator==(const T *pDefaults) const {
    for (int i = 0; i < iSize; i++)
      if (pArray[i] != pDefaults[i]) return false;
    return true;
  }
  inline StdArrayAdapt &operator=(const T *pDefaults) {
    for (int i = 0; i < iSize; i++) pArray[i] = pDefaults[i];
    return *this;
  }
  ALLOW_TEMP_TO_REF(StdArrayAdapt)
};
template <class T>
inline StdArrayAdapt<T> mkArrayAdapt(T *pArray, int iSize) {
  return StdArrayAdapt<T>(pArray, iSize);
}
#define mkArrayAdaptM(A) mkArrayAdapt(A, sizeof(A) / sizeof(*(A)))
template <class T, class M>
inline StdArrayAdapt<T, M> mkArrayAdaptMap(T *pArray, int iSize, M map) {
  return StdArrayAdapt<T, M>(pArray, iSize, map);
}
...
struct StdArrayDefaultAdapt {
  StdArrayDefaultAdapt(T *pArray, size_t iSize, const D &rDefault,
                       const M &map = M())
  ...
  inline void CompileFunc(StdCompiler *pComp) const {
    ...
    if (!fCompiler && pComp->hasNaming())
      while (iWrite > 0 && pArray[iWrite - 1] == rDefault) iWrite--;
    // Read/write values
    for (i = 0; i < iWrite; i++) {
      // Seperator?
      if (i)
        if (!pComp->Seperator(StdCompiler::SEP_SEP)) break;
      // Expect a value. Default if not found.
      pComp->Value(mkDefaultAdapt(map(pArray[i]), rDefault));
    }
    // Fill rest of array
    if (fCompiler)
      for (; i < iSize; i++) pArray[i] = rDefault;
  }
  ...
  inline StdArrayDefaultAdapt &operator=(const T *pDefaults) {
    for (size_t i = 0; i < iSize; i++) pArray[i] = pDefaults[i];
    return *this;
  }
  ALLOW_TEMP_TO_REF(StdArrayDefaultAdapt)
};
template <class T, class D>
inline StdArrayDefaultAdapt<T, D> mkArrayAdapt(T *pArray, size_t iSize,
                                               const D &rDefault) {
  return StdArrayDefaultAdapt<T, D>(pArray, iSize, rDefault);
}
#define mkArrayAdaptDM(A, D) mkArrayAdapt(A, sizeof(A) / sizeof(*(A)), D)
template <class T, class D, class M>
inline StdArrayDefaultAdapt<T, D, M> mkArrayAdaptMap(T *pArray, size_t iSize,
                                                     const D &rDefault, M map) {
  return StdArrayDefaultAdapt<T, D, M>(pArray, iSize, rDefault, map);
}
#define mkArrayAdaptMapDM(A, D, M) \
...
template <class T, class I>
inline StdInsertAdapt<T, I> mkInsertAdapt(T rObj, I rIns, bool fBefore = true) {
  return StdInsertAdapt<T, I>(rObj, rIns, fBefore);
}
...
struct StdParameterAdapt {
  ...
  T &rObj;
  const P Par;
  void CompileFunc(StdCompiler *pComp) const { rObj.CompileFunc(pComp, Par); }
  // Operators for default checking/setting
  template <class D>
  inline bool operator==(const D &nValue) const {
    return rObj == nValue;
  ...
  template <class D>
  inline StdParameterAdapt &operator=(const D &nValue) {
    rObj = nValue;
    return *this;
  }
...
template <class T, class P>
inline StdParameterAdapt<T, P> mkParAdapt(T &rObj, const P &rPar) {
  return StdParameterAdapt<T, P>(rObj, rPar);
}
...
struct StdParameter2Adapt {
  StdParameter2Adapt(T &rObj, const P1 &rPar1, const P2 &rPar2)
      : rObj(rObj), rPar1(rPar1), rPar2(rPar2) {}
  T &rObj;
  const P1 &rPar1;
  const P2 &rPar2;
  void CompileFunc(StdCompiler *pComp) const {
  ...
  template <class D>
  inline StdParameter2Adapt &operator=(const D &nValue) {
    rObj = nValue;
    return *this;
  }
...
template <class T, class P1, class P2>
inline StdParameter2Adapt<T, P1, P2> mkParAdapt(T &rObj, const P1 &rPar1,
                                                const P2 &rPar2) {
  return StdParameter2Adapt<T, P1, P2>(rObj, rPar1, rPar2);
}
...
struct StdPtrAdapt {
  ...
  const char *szNaming;
  void CompileFunc(StdCompiler *pComp) const {
    bool fCompiler = pComp->isCompiler(), fNaming = pComp->hasNaming();
  ...
  // Operators for default checking/setting
  inline bool operator==(const T &nValue) const {
    return rpObj && *rpObj == nValue;
  }
  inline StdPtrAdapt &operator=(const T &nValue) {
    delete rpObj;
    rpObj = new T(nValue);
    return *this;
  }
  inline bool operator==(const T *pValue) const { return rpObj == pValue; }
  inline StdPtrAdapt &operator=(const T *pValue) {
    delete rpObj;
    rpObj = pValue;
    return *this;
  }
...
template <class T>
inline StdPtrAdapt<T> mkPtrAdapt(T *&rpObj, bool fAllowNull = true) {
  return StdPtrAdapt<T>(rpObj, fAllowNull);
}
template <class T>
inline StdPtrAdapt<T> mkNamingPtrAdapt(T *&rpObj, const char *szNaming) {
  return StdPtrAdapt<T>(rpObj, true, szNaming);
}
template <class T>
inline StdPtrAdapt<T> mkPtrAdaptNoNull(T *&rpObj) {
  return mkPtrAdapt<T>(rpObj, false);
}

// * Adaptor for STL containers
// Writes a comma-seperated list for compilers that support it. Otherwise, the
// size is calculated and safed.
...
struct StdSTLContainerAdapt {
  ...
  inline void CompileFunc(StdCompiler *pComp) const {
    ...
    if (!fCompiler) {
      ...
      // Write all entries
      for (typename C::const_iterator i = rStruct.begin(); i != rStruct.end();
           ++i) {
  ...
  inline StdSTLContainerAdapt &operator=(const C &nValue) {
    rStruct = nValue;
    return *this;
  }
...
template <class C>
inline StdSTLContainerAdapt<C> mkSTLContainerAdapt(
    C &rTarget, StdCompiler::Sep eSep = StdCompiler::SEP_SEP) {
  return StdSTLContainerAdapt<C>(rTarget, eSep);
}

// Write an integer that is supposed to be small most of the time. The adaptor
// writes it in
// 7-bit-pieces, bit 8 being a continuation marker: If it's set, more data is
// following, if not,
// all following bits are 0.
// Code lengths for uint32_t:
// 0x00000000 (0)					- 0x0000007F (127)
// : 1 byte
// 0x00000080 (128)				- 0x00003FFF (16383)			: 2
// byte
...
// So this sort of packing is always useful when the integer in question is
...
struct StdIntPackAdapt {
  ...
  template <class D>
  inline StdIntPackAdapt &operator=(const D &nValue) {
    rVal = nValue;
    return *this;
  }
...
struct StdEnumAdapt {
  ...
  StdEnumAdapt(T &rVal, const Entry *pNames) : rVal(rVal), pNames(pNames) {
    assert(pNames);
  }
  T &rVal;
  const Entry *pNames;
  ...
  void CompileFunc(StdCompiler *pComp) const {
    ...
    if (!pComp->isCompiler()) {
      ...
      for (; pName->Name; pName++)
        if (pName->Val == rVal) {
          // Put name
          pComp->String(const_cast<char **>(&pName->Name),
                        StdCompiler::RCT_Idtf);
          break;
        }
      // No name found?
    ...
    // reading?
      ...
      try {
        ...
        // Not found? Warn
        if (!pName->Name) pComp->Warn("Unknown bit name: %s", Name.getData());
      }
    }
  }
  ...
  template <class D>
  inline StdEnumAdapt<T, int_t> &operator=(const D &nValue) {
    rVal = nValue;
    return *this;
  }
...
StdEnumAdapt<T, int_t> mkEnumAdapt(T &rVal, const StdEnumEntry<T> *pNames) {
  return StdEnumAdapt<T, int_t>(rVal, pNames);
}
template <class int_t, class T>
...
// Convert a bitfield into/from something like "foo | bar | baz", where "foo",
// "bar" and "baz" are given constants.
template <class T>
struct StdBitfieldAdapt {
  typedef StdBitfieldEntry<T> Entry;

  StdBitfieldAdapt(T &rVal, const Entry *pNames) : rVal(rVal), pNames(pNames) {
    assert(pNames);
  }
  T &rVal;
  const Entry *pNames;
  ...
  template <class D>
  inline StdBitfieldAdapt<T> &operator=(const D &nValue) {
    rVal = nValue;
    return *this;
  }
...
StdBitfieldAdapt<T> mkBitfieldAdapt(T &rVal,
  ...
  return StdBitfieldAdapt<T>(rVal, pNames);
}

// * Name count adapter
// For compilers without name support, this just compiles the given value.
// Otherwise, the count
// of given namings is returned on compiling, and nothing is done for
// decompiling (The caller
...
struct StdNamingCountAdapt {
  ...
  StdNamingCountAdapt(int_t &iCount, const char *szName)
      : iCount(iCount), szName(szName) {}
  inline void CompileFunc(StdCompiler *pComp) const {
    if (pComp->hasNaming()) {
      if (pComp->isCompiler()) iCount = pComp->NameCount(szName);
    } else
...
template <class int_t>
inline StdNamingCountAdapt<int_t> mkNamingCountAdapt(int_t &iCount,
                                                     const char *szName) {
  return StdNamingCountAdapt<int_t>(iCount, szName);
}
...
class StdHexAdapt {
 ...
 public:
  ...
  inline void CompileFunc(StdCompiler *pComp) const {
    if (!pComp->isVerbose()) pComp->Raw(pData, iSize);
    char szData[2 + 1];
    bool fCompiler = pComp->isCompiler();
    for (size_t i = 0; i < iSize; i++) {
      ...
      if (fCompiler) {
        ...
        if (sscanf(szData, "%02x", &b) != 1)
          pComp->excNotFound(i ? "hexadecimal data: bytes missing!"
...
inline StdHexAdapt mkHexAdapt(void *pData, size_t iSize) {
  return StdHexAdapt(pData, iSize);
}
template <class T>
inline StdHexAdapt mkHexAdapt(T &rData) {
  return StdHexAdapt(&rData, sizeof(rData));
}
