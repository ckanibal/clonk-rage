...
bool CSurface8::HasOwnPal() { return pPal != lpDDrawPal; }
...
void CSurface8::SetPalette(BYTE *bpPalette, BOOL fAdapt)
        ...
        if (fAdapt)
                ...
                for (int iFromCol=1; iFromCol<256; iFromCol++) // Color zero
transparent
                        {
                        int iToCol = 1;
                        for (int cToCol=iToCol+1; cToCol<256; cToCol++)
                                if (
ColorDistance(&pPal->Colors[iFromCol*3],&bpPalette[cToCol*3])
                                         <
...
void CSurface8::MapBytes(BYTE *bpMap) {
  if (!bpMap) return;
  for (int cnt = 0; cnt < Wdt * Hgt; cnt++)
    SetPix(cnt % Wdt, cnt / Wdt, bpMap[GetPix(cnt % Wdt, cnt / Wdt)]);
}
...
void CSurface8::ClearBox8Only(int iX, int iY, int iWdt, int iHgt) {
  ...
  for (int y = iY; y < iY + iHgt; ++y)
    for (int x = iX; x < iX + iWdt; ++x) Bits[y * Pitch + x] = 0;
  // done
}
...
static void fill_edge_structure(CPolyEdge *edge, int *i1, int *i2) {
  ...
  edge->prev = NULL;
  edge->next = NULL;
  if (edge->dx < 0) edge->x += Min<int>(edge->dx + (1 << POLYGON_FIX_SHIFT), 0);
