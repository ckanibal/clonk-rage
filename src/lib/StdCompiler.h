...
// Try to avoid casting NotFoundExceptions for trivial cases (MSVC log flood
// workaround)
#if defined(_MSC_VER)
...
// (serialization/deserialization - note that the term "compile" is used for
// both directions)
...
class StdCompiler {
 public:
  ...
  // callback by runtime-write-allowed adaptor used by compilers that may set
  // runtime values only
  ...
  // Set the NameEnd parameter to true if you are stopping to parse the
  // structure
  // for whatever reason (suppress warning messages).
  virtual bool Name(const char *szName) { return true; }
  virtual void NameEnd(bool fBreak = false) {}

  // Special: A naming that follows to the currently active naming (on the same
  // level).
  // Note this will end the current naming, so no additional NameEnd() is
  // needed.
  // Only used to maintain backwards compatibility, should not be used in new
  // code.
  ...
  enum RawCompileType {
    ...
    RCT_ID,  // Like RCT_Idtf (only used for special compilers that treat IDs
             // differently)
  };
  // Note that string won't allow '\0' inside the buffer, even with escaped
  // compiling!
  virtual void String(char *szString, size_t iMaxLength,
                      RawCompileType eType = RCT_Escaped) = 0;
  virtual void String(char **pszString, RawCompileType eType = RCT_Escaped) = 0;
  virtual void Raw(void *pData, size_t iSize,
                   RawCompileType eType = RCT_Escaped) = 0;

  // * Position
  // May return information about the current position of compilation (used for
  // errors and warnings)
  ...
  void Value(const T &rStruct) {
    rStruct.CompileFunc(this);
  }
  template <class T>
  void Value(T &rStruct) {
    CompileFunc(rStruct, this);
  }
  ...
  template <class T>
  inline void Decompile(const T &rStruct) {
    assert(!isCompiler());
 ...
 public:
  // Compiler exception - thrown when something is wrong with the data to
  // compile
  struct Exception {
    StdStrBuf Pos;
    StdStrBuf Msg;
...
// Exception workaround
 ...
 public:
  template <class T>
  bool ValueSafe(const T &rStruct) {
    rStruct.CompileFunc(this);
    return true;
  }
  template <class T>
  ...
  bool ValueSafe(int32_t &rInt) {
    beginFailSafe();
    DWord(rInt);
    return endFailSafe();
  }
  bool ValueSafe(uint32_t &rInt) {
    beginFailSafe();
    DWord(rInt);
    return endFailSafe();
  }
  bool ValueSafe(int16_t &rInt) {
    beginFailSafe();
    Word(rInt);
    return endFailSafe();
  }
  bool ValueSafe(uint16_t &rInt) {
    beginFailSafe();
    Word(rInt);
    return endFailSafe();
  }
  bool ValueSafe(int8_t &rInt) {
    beginFailSafe();
    Byte(rInt);
    return endFailSafe();
  }
  bool ValueSafe(uint8_t &rInt) {
    beginFailSafe();
    Byte(rInt);
    return endFailSafe();
  }
  bool ValueSafe(bool &rBool) {
    beginFailSafe();
    Boolean(rBool);
    return endFailSafe();
  }
...
StructT *CompileFromBufToNewNamed(const typename CompT::InT &SrcBuf,
                                  const char *szName) {
  StructT *pStruct = NULL;
  CompileFromBuf<CompT>(mkNamingAdapt(mkPtrAdaptNoNull(pStruct), szName),
                        SrcBuf);
  return pStruct;
}
template <class CompT, class StructT>
...
class StdCompilerINIRead : public StdCompiler {
 ...
 protected:
  ...
  struct NameNode {
    ...
    bool Section;
    // Tree structure
    NameNode *Parent, *FirstChild, *PrevChild, *NextChild, *LastChild;
    // Indent level
    int Indent;
  ...
  // Current depth
  int iDepth;
  // Real depth (depth of recursive Name()-calls - if iDepth != iRealDepth, we
  // are in a nonexistant namespace)
  int iRealDepth;
  ...
  StdBuf ReadString(size_t iLength, RawCompileType eTyped,
