...
BOOL InitDirectSound(HWND hwnd) {
  if (!SUCCEEDED(DirectSoundCreate(NULL, &lpDS, NULL))) return FALSE;
  if (!SUCCEEDED(lpDS->SetCooperativeLevel(hwnd, DSSCL_NORMAL))) return FALSE;
  return TRUE;
}
...
BOOL DSGetWaveResource(BYTE *pvRes, WAVEFORMATEX **ppWaveHeader,
...
BOOL DSFillSoundBuffer(IDirectSoundBuffer *pDSB, BYTE *pbWaveData,
  ...
  if (pDSB && pbWaveData && cbWaveSize) {
    ...
    if (SUCCEEDED(
            pDSB->Lock(0, cbWaveSize, &pMem1, &dwSize1, &pMem2, &dwSize2, 0))) {
...
IDirectSoundBuffer *DSLoadSoundBuffer(IDirectSound *pDS, BYTE *bpWaveBuf) {
  ...
  DSBUFFERDESC dsBD = {0};
  BYTE *pbWaveData;
  if (DSGetWaveResource(bpWaveBuf, &dsBD.lpwfxFormat, &pbWaveData,
                        &dsBD.dwBufferBytes)) {
    dsBD.dwSize = sizeof(dsBD);
    dsBD.dwFlags =
        DSBCAPS_STATIC | DSBCAPS_CTRLVOLUME | DSBCAPS_GETCURRENTPOSITION2;
...
CSoundObject *DSndObjCreate(BYTE *bpWaveBuf, int iConcurrent) {
  ...
  if (DSGetWaveResource(bpWaveBuf, &pWaveHeader, &pbData, &cbData)) {
    ...
    if ((pSO = (CSoundObject *)LocalAlloc(
             LPTR, sizeof(CSoundObject) +
                       (iConcurrent - 1) * sizeof(IDirectSoundBuffer *))) !=
        NULL) {
      int i;
      ...
      for (i = 1; i < pSO->iAlloc; i++) {
        if (FAILED(
                pDS->DuplicateSoundBuffer(pSO->Buffers[0], &pSO->Buffers[i]))) {
...
BOOL DSndObjPlay(CSoundObject *pSO, DWORD dwPlayFlags) {
  ...
  if (!(dwPlayFlags & DSBPLAY_LOOPING) || (pSO->iAlloc == 1)) {
    ...
    if (pDSB) result = SUCCEEDED(pDSB->Play(0, 0, dwPlayFlags));
  }
...
BOOL DSndObjPlaying(CSoundObject *pSO) {
  ...
  if (pSO)
    for (i = 0; i < pSO->iAlloc; i++) {
      ...
      if ((dwStatus & DSBSTATUS_PLAYING) == DSBSTATUS_PLAYING) fPlaying = TRUE;
    }
  return fPlaying;
}
