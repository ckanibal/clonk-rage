...
class CStdEvent {
 public:
  CStdEvent(bool fManualReset) {
    hEvent = CreateEvent(NULL, fManualReset, false, NULL);
  }
...
class CStdEvent {
 ...
 public:
  ...
  void Pulse() { pthread_cond_broadcast(&cond); }
  void Reset() {
    pthread_mutex_lock(&mutex);
    fSet = false;
    pthread_mutex_unlock(&mutex);
  ...
  bool WaitFor(unsigned int iMillis) {
    ...
    // Already set?
    while (!fSet) {
      // Use pthread_cond_wait or pthread_cond_timedwait depending on wait
      // length. Check return value.
      // Note this will temporarily unlock the mutex, so no deadlock should
      // occur.
      timespec ts = {iMillis / 1000, (iMillis % 1000) * 1000000};
      if (0 != (iMillis != INFINITE ? pthread_cond_timedwait(&cond, &mutex, &ts)
...
#if defined __GNUC__ && \
    ((__GNUC__ >= 4 && __GNUC_MINOR__ >= 1) || (__GNUC__ >= 5))
...
class CStdLock {
 public:
  CStdLock(CStdCSec *pSec) : sec(pSec) { sec->Enter(); }
  ~CStdLock() { Clear(); }
...
class CStdCSecEx : public CStdCSec {
 public:
  CStdCSecEx() : lShareCnt(0), ShareFreeEvent(false), pCallbClass(NULL) {}
 ...
 public:
  ...
  void LeaveShared() {
    ...
    if (!--lShareCnt) {
      // do callback
      if (pCallbClass) pCallbClass->OnShareFree(this);
      // set event
      ShareFreeEvent.Set();
    }
    // unlock
...
class CStdShareLock {
 public:
  CStdShareLock(CStdCSecEx *pSec) : sec(pSec) { sec->EnterShared(); }
  ~CStdShareLock() { Clear(); }
