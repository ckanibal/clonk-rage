...
void CMarkupTagItalic::Apply(CBltTransform &rBltTrf, bool fDoClr,
                             DWORD &dwClr) {
  // do sheering
  rBltTrf.mat[1] -= 0.3f;
}
  ...
  // colored
    ...
    if (!fSkip) {
      ...
      for (int i = 0; i < iParLen; ++i) {
        BYTE b;
        if (szPars[i] >= '0' && szPars[i] <= '9')
          b = szPars[i] - '0';
        else if (szPars[i] >= 'a' && szPars[i] <= 'f')
          b = szPars[i] - 'a' + 10;
        else
          return false;
...
bool CMarkup::StripMarkup(char *szText) {
  ...
  do {
    ...
    if (szRead[0] == '{' && szRead[1] == '{' &&
        szRead[2] != '{')  // skip at {{{, because {{{id}} should be parsed as {
                           // {{id}} }.
    {
