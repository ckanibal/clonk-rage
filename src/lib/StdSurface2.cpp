...
CSurface::CSurface() : fIsBackground(false) { Default(); }
...
void CSurface::Default() {
  Wdt = Hgt = 0;
  PrimarySurfaceLockPitch = 0;
  PrimarySurfaceLockBits = NULL;
...
// grab data from other sfc
  ...
  ClipX = psfcFrom->ClipX;
  ClipY = psfcFrom->ClipY;
  ClipX2 = psfcFrom->ClipX2;
  ClipY2 = psfcFrom->ClipY2;
  ...
  iTexX = psfcFrom->iTexX;
  iTexY = psfcFrom->iTexY;
...
bool CSurface::IsRenderTarget() {
  ...
  return fPrimary
// other surfaces may be used as render targets, if offscreen rendertargets are
...
bool CSurface::CreateTextures() {
  ...
  if (pD3D) {
    ...
    if (int(d3dCaps.MaxTextureWidth) < iMaxTexSize)
      iMaxTexSize = d3dCaps.MaxTextureWidth;
  } else
#endif
#ifdef USE_GL
      if (pGL) {
    ...
    if (iMaxTexSize2 > 0)
      if (iMaxTexSize2 < iMaxTexSize) iMaxTexSize = iMaxTexSize2;
  } else
#endif
    /* keep standard texture size */;
  // get needed tex size - begin with smaller value of wdt/hgt, so there won't
  // be too much space wasted
  ...
  for (int i = iTexX * iTexY; i; --i, ++ppCTex) {
    // regular textures or if last texture fits exactly into the space by Wdt or
    // Hgt
...
/*bool ClrByOwner(DWORD &rClr) old style...
        ...
        BYTE byB=GetRValue(rClr); if (!byB || Inside(byB, Min(byR, byG),
   Max(byR, byG))) return false;
...
bool ClrByOwner(DWORD &dwClr)  // new style, based on Microsoft Knowledge Base
                               // Article - 29240
{
  int H, L, S;
  WORD R, G, B;
...
bool CSurface::SetAsClrByOwnerOf(CSurface *pOfSurface) {
  ...
  if (Wdt != pOfSurface->Wdt || Hgt != pOfSurface->Hgt) return false;
  // set main surface
  pMainSfc = pOfSurface;
  // success
  return true;
...
/*bool CSurface::CreatePrimaryGLTextures()
        ...
        for (int iY=0; iY<Hgt; iY+=iTexSize)
                for (int iX=0; iX<Wdt; iX+=iTexSize)
                        {
                        // get tex size
                        int txWdt=Min(Wdt-iX, iTexSize), txHgt=Min(Hgt-iY,
   iTexSize);
                        // copy surface into texture
                        glBindTexture(GL_TEXTURE_2D, (*ppTexRef)->texName);
                        glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, iX, iY,
   txWdt, txHgt, 0);
...
bool CSurface::Read(CStdStream &hGroup, bool fOwnPal) {
  ...
  if (BitmapInfo.Info.biBitCount == 8) {
    if (!hGroup.Read(((BYTE *)&BitmapInfo) + sizeof(CBitmapInfo),
                     Min(sizeof(BitmapInfo) - sizeof(CBitmapInfo),
                         sizeof(BitmapInfo) - sizeof(CBitmapInfo) +
                             BitmapInfo.FileBitsOffset())))
      return FALSE;
  ...
  for (lcnt = Hgt - 1; lcnt >= 0; lcnt--) {
    ...
    for (int x = 0; x < BitmapInfo.Info.biWidth; ++x)
      switch (BitmapInfo.Info.biBitCount) {
        case 8:
          if (fOwnPal)
            SetPixDw(x, lcnt, C4RGB(BitmapInfo.Colors[*pPix].rgbRed,
...
/*BOOL CSurface::Save(const char *szFilename)
        ...
        if(fPrimary)
                ...
                // write lines
                char bpEmpty[4]; int iEmpty =
  DWordAligned(Wdt*byBytesPP)-Wdt*byBytesPP;
...
*/
bool CSurface::SavePNG(const char *szFilename, bool fSaveAlpha,
                       bool fApplyGamma, bool fSaveOverlayOnly) {
  // Lock - WARNING - maybe locking primary surface here...
  if (!Lock()) return false;
  ...
  if (fPrimary && pGL) {
    ...
    for (int y = 0; y < Hgt; ++y)
      glReadPixels(0, Hgt - y, Wdt, 1, fSaveAlpha ? GL_BGRA : GL_BGR,
                   GL_UNSIGNED_BYTE,
                   png.GetImageData() + y * Wdt * (3 + fSaveAlpha));
  } else
#endif
  {
    // write pixel values
...
BOOL CSurface::AttachPalette() { return TRUE; }
  ...
  if (!Locked) {
    if (fPrimary) {
      ...
      if (pD3D) {
        ...
        // lock it
        if (pSfc->LockRect(&lock, NULL, 0) != D3D_OK) return FALSE;
  ...
  if (!Locked) {
    ...
    if (fPrimary) {
      ...
      if (pD3D) {
        if (!pSfc) return FALSE;
        // unlocking primary?
        if (pSfc->UnlockRect() != D3D_OK) return FALSE;
      ...
      CTexRef **ppTx = ppTex;
      for (int i = 0; i < iTexX * iTexY; ++i, ++ppTx) (*ppTx)->Unlock();
    }
  }
  return TRUE;
}
  ...
  if (fPrimary) {
    ...
    if (pGL) {
      if (!PrimarySurfaceLockBits) {
        ...
        glReadPixels(0, 0, Wdt, Hgt, GL_BGR, GL_UNSIGNED_BYTE,
                     PrimarySurfaceLockBits);
        PrimarySurfaceLockPitch = Wdt * 3;
      }
      return *(DWORD *)(PrimarySurfaceLockBits +
                        (Hgt - iY - 1) * PrimarySurfaceLockPitch + iX * 3);
    ...
    if (!PrimarySurfaceLockBits) {
      ...
      switch (dwClrFormat) {
        case D3DFMT_X1R5G5B5:
          // 16 bit 5-5-5
          pix16 = *(WORD *)(((BYTE *)PrimarySurfaceLockBits) +
                            iY * PrimarySurfaceLockPitch + iX * 2);
          return ((pix16 & 0x001f) << 3) | ((pix16 & 0x03e0) << 6) |
                 ((pix16 & 0x7c00) << 9);
        ...
        case D3DFMT_R5G6B5:
          ...
          pix16 = *(WORD *)(((BYTE *)PrimarySurfaceLockBits) +
                            iY * PrimarySurfaceLockPitch + iX * 2);
          return ((pix16 & 0x001f) << 3) | ((pix16 & 0x07e0) << 5) |
                 ((pix16 & 0xf800) << 8);
          break;

        case D3DFMT_X8R8G8B8:
          // 32 bit
          return *(DWORD *)(((BYTE *)PrimarySurfaceLockBits) +
                            iY * PrimarySurfaceLockPitch + iX * 4);
      }
    }
#endif
  } else {
  ...
  if (pMainSfc) {
    ...
    if (byAlpha == 0xff)
      ...
      if (fApplyModulation) {
        ...
        if (lpDDraw->BlitModulated &&
            !(lpDDraw->dwBlitMode & C4GFXBLIT_CLRSFC_OWNCLR))
...
bool CSurface::SetPixDw(int iX, int iY, DWORD dwClr) {
  ...
  // This optimization was moved to LockForUpdate, as it only slows down mass
  // updates here
  // Keep this code in case there is a need for fast single pixel updates again
  if (0 && pGL && pGL->pCurrCtx) {
    if (!GetTexAt(&pTexRef, iX, iY)) return false;
    // If the texture is not copied into system memory, modify it directly in
    // the video memory
    if (!pTexRef->texLock.pBits) {
      glBindTexture(GL_TEXTURE_2D, pTexRef->texName);
      if (byBytesPP == 4) {
        // 32 Bit
        glTexSubImage2D(GL_TEXTURE_2D, 0, iX, iY, 1, 1, GL_BGRA,
                        GL_UNSIGNED_INT_8_8_8_8_REV, &dwClr);
      } else {
        // 16 bit
        uint16_t wClr = ClrDw2W(dwClr);
        glTexSubImage2D(GL_TEXTURE_2D, 0, iX, iY, 1, 1, GL_BGRA,
                        GL_UNSIGNED_SHORT_4_4_4_4_REV, &wClr);
      }
      return true;
    }
...
bool CSurface::SetPixAlpha(int iX, int iY, BYTE byAlpha) {
  ...
  if (byBytesPP == 4)
    // 32 bit
    *(((BYTE *)pTexRef->texLock.pBits) + iY * pTexRef->texLock.Pitch + iX * 4 +
      3) = byAlpha;
  else {
    // 16 bit
    BYTE *pPix = ((BYTE *)pTexRef->texLock.pBits) +
                 iY * pTexRef->texLock.Pitch + iX * 2 + 1;
...
bool CSurface::BltPix(int iX, int iY, CSurface *sfcSource, int iSrcX, int iSrcY,
                      bool fTransparency) {
  ...
  if (byBytesPP == 4) {
    // 32 bit
    DWORD *pPix32 = (DWORD *)(((BYTE *)pTexRef->texLock.pBits) +
    ...
    // 16 bit
    WORD *pPix16 = (WORD *)(((BYTE *)pTexRef->texLock.pBits) +
...
void CSurface::ClearBoxDw(int iX, int iY, int iWdt, int iHgt) {
  ...
  bool fBaseSfc = false;
  if (pMainSfc)
    if (pMainSfc->ppTex) fBaseSfc = true;
...
// create it!
  ...
  if (pD3D) {
    ...
    if (pD3D->lpDevice->CreateTexture(
            iSize, iSize, 1, fRenderTarget ? D3DUSAGE_RENDERTARGET : 0,
            pD3D->dwSurfaceType,
            fRenderTarget ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED,
            &pTex) != D3D_OK) {
...
bool CTexRef::LockForUpdate(RECT &rtUpdate) {
  ...
  if (texLock.pBits) {
    // fully locked
    if (LockSize.left == 0 && LockSize.right == iSize && LockSize.top == 0 &&
        LockSize.bottom == iSize) {
      return true;
    } else {
...
// lock
  ...
  if (pD3D) {
    ...
    if (texName) {
      ...
      texLock.pBits =
          new unsigned char[(rtUpdate.right - rtUpdate.left) *
                            (rtUpdate.bottom - rtUpdate.top) * pGL->byByteCnt];
...
// lock
  ...
  if (pD3D) {
    ...
    if (texName) {
      ...
      if (!pGL->pCurrCtx)
        if (!pGL->MainCtx.Select()) return false;
      // get texture
      texLock.pBits = new unsigned char[iSize * iSize * pGL->byByteCnt];
      texLock.Pitch = iSize * pGL->byByteCnt;
      glBindTexture(GL_TEXTURE_2D, texName);
      glGetTexImage(GL_TEXTURE_2D, 0, GL_BGRA,
                    lpDDraw->byByteCnt == 2 ? GL_UNSIGNED_SHORT_4_4_4_4_REV
...
void CTexRef::Unlock() {
  ...
  if (pD3D) {
    ...
    if (!texName) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, 4, iSize, iSize, 0, GL_BGRA,
                   lpDDraw->byByteCnt == 2 ? GL_UNSIGNED_SHORT_4_4_4_4_REV
                                           : GL_UNSIGNED_INT_8_8_8_8_REV,
                   texLock.pBits);
    } else {
      ...
      glBindTexture(GL_TEXTURE_2D, texName);
      glTexSubImage2D(GL_TEXTURE_2D, 0, LockSize.left, LockSize.top,
                      LockSize.right - LockSize.left,
                      LockSize.bottom - LockSize.top, GL_BGRA,
                      lpDDraw->byByteCnt == 2 ? GL_UNSIGNED_SHORT_4_4_4_4_REV
                                              : GL_UNSIGNED_INT_8_8_8_8_REV,
                      texLock.pBits);
    }
    delete[] texLock.pBits;
    texLock.pBits = NULL;
...
CTexMgr *pTexMgr;
const BYTE FColors[] = {31, 16, 39, 47, 55, 63, 71,
                        79, 87, 95, 23, 30, 99, 103};
