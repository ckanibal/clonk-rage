...
gboolean OnXInputStatic(GIOChannel *channel, GIOCondition condition,
                        gpointer data) {
  static_cast<CStdApp *>(data)->OnXInput();
  return TRUE;
}

gboolean OnPipeInputStatic(GIOChannel *channel, GIOCondition condition,
                           gpointer data) {
  static_cast<CStdApp *>(data)->OnPipeInput();
  return TRUE;
}

gboolean OnStdInInputStatic(GIOChannel *channel, GIOCondition condition,
                            gpointer data) {
...
bool CStdApp::Init(int argc, char *argv[]) {
  ...
  if (!XF86VidModeQueryExtension(dpy, &xf86vmode_event_base,
                                 &xf86vmode_error_base) ||
      !XF86VidModeQueryVersion(dpy, &xf86vmode_major_version,
                               &xf86vmode_minor_version)) {
...
void CStdApp::NextTick(bool fYield) { DoNotDelay = true; }
...
gboolean HandleMessageTimeout(gpointer data) {
...
C4AppHandleResult CStdApp::HandleMessage(unsigned int iTimeout,
  ...
  if (DoNotDelay) {
    ...
    // (don't call Execute then, because it assumes it has been called because
    // of a timer event!)
  ...
  if (fCheckTimer || iTimeout != INFINITE) {
    ...
    // value.
    timeout_handle =
        g_timeout_add_full(G_PRIORITY_HIGH, timeout, &HandleMessageTimeout,
                           &timeout_elapsed, NULL);
  }

  g_main_context_iteration(g_main_loop_get_context(Priv->loop), TRUE);

  if (timeout_handle && !timeout_elapsed) {
    // FIXME: do not add a new timeout instead of deleting the old one in the
    // next call
  ...
  switch (select(max_fd + 1, &rfds, NULL, NULL,
                 (fCheckTimer || iTimeout != INFINITE) ? &tv : NULL)) {
    // error
    case -1:
...
void CStdApp::HandleXMessage() {
  ...
  switch (event.type) {
    ...
    case KeyPress:
      ...
      if (!filtered) {
        ...
        if (Priv->xic) {
          ...
          if (lsret == XLookupKeySym)
            fprintf(stderr, "FIXME: XmbLookupString returned XLookupKeySym\n");
          if (lsret == XBufferOverflow)
            fprintf(stderr,
                    "FIXME: XmbLookupString returned XBufferOverflow\n");
        } else {
    ...
    case SelectionRequest: {
      // We should compare the timestamp with the timespan when we owned the
      // selection
      // But slow network connections are not supported anyway, so do not bother
      CStdAppPrivate::ClipboardData &d =
          (event.xselectionrequest.selection == XA_PRIMARY)
              ? Priv->PrimarySelection
              : Priv->ClipboardSelection;
      ...
      if (d.Text.getData()) {
        ...
        XChangeProperty(
            dpy, re.requestor, re.property, re.target, 8, PropModeReplace,
    ...
    case ClientMessage:
      if (!strcmp(XGetAtomName(dpy, event.xclient.message_type),
                  "WM_PROTOCOLS")) {
        if (!strcmp(XGetAtomName(dpy, event.xclient.data.l[0]),
                    "WM_DELETE_WINDOW")) {
          CStdWindow *pWindow = Priv->GetWindow(event.xclient.window);
          if (pWindow) pWindow->Close();
        } else if (!strcmp(XGetAtomName(dpy, event.xclient.data.l[0]),
                           "_NET_WM_PING")) {
          // We're still alive
  ...
  CStdWindow *pWindow = Priv->GetWindow(event.xany.window);
  if (pWindow) pWindow->HandleMessage(event);
}
...
bool CStdApp::FindDisplayMode(unsigned int iXRes, unsigned int iYRes,
  ...
  log << "  XF86VidModeExtension version is " << xf86vmode_major_version << "."
      << xf86vmode_minor_version;
  Log(log.str().c_str());
  // Change resolution
  int mode_num;
  ...
  for (int i = 0; i < mode_num; i++) {
    if ((modes[i]->hdisplay == iXRes) && (modes[i]->vdisplay == iYRes)) {
      if (!Priv->modefound || Priv->targetmode.hdisplay != iXRes ||
          Priv->targetmode.vdisplay != iYRes)
        Priv->targetmode = *modes[i];
      Priv->modefound = true;
    }
    /*log.str("");
...
void CStdAppPrivate::SetEWMHFullscreen(CStdApp *pApp, bool fFullScreen,
                                       Window wnd) {
  static Atom atoms[2];
  ...
  XSendEvent(pApp->dpy, DefaultRootWindow(pApp->dpy), false,
...
bool CStdAppPrivate::SwitchToFullscreen(CStdApp *pApp, Window wnd) {
  ...
  if (!memcmp(&targetmode, &oldmode, sizeof(XF86VidModeModeInfo))) {
    // Set the window to fullscreen mode to get rid of window manager
    // decorations
    ...
    int wnd_x = 0;
    int wnd_y = 0;
    XTranslateCoordinates(pApp->dpy, wnd, DefaultRootWindow(pApp->dpy), 0, 0,
                          &wnd_x, &wnd_y, &bla);
    XF86VidModeSetViewPort(pApp->dpy, DefaultScreen(pApp->dpy), wnd_x, wnd_y);
  } else {
    // The user specified a impossible resolution or the x server does not
    // support the default
    return false;
  }
  XGrabPointer(pApp->dpy, wnd, true, 0, GrabModeAsync, GrabModeAsync, wnd, None,
               CurrentTime);
  return true;
}

void CStdAppPrivate::SwitchToDesktop(CStdApp *pApp, bool fMinimize,
                                     Window wnd) {
...
bool CStdApp::SetFullScreen(bool fFullscreen, bool fMinimize) {
  ...
  if (!fFullscreen && fMinimize)
    Priv->pending_desktop = Priv->PENDING_DESKTOP_DELAY;
  else if (!fFullscreen) {
    Priv->SwitchToDesktop(this, fMinimize, pWindow->wnd);
    fDspModeSet = false;
  } else
    return fDspModeSet = Priv->SwitchToFullscreen(this, pWindow->wnd);
  return true;
}
...
void CStdApp::Copy(const StdStrBuf &text, bool fClipboard) {
  CStdAppPrivate::ClipboardData &d =
      fClipboard ? Priv->ClipboardSelection : Priv->PrimarySelection;
  XSetSelectionOwner(dpy, fClipboard ? ClipboardAtoms[0] : XA_PRIMARY,
                     pWindow->wnd, Priv->LastEventTime);
  Window owner =
      XGetSelectionOwner(dpy, fClipboard ? ClipboardAtoms[0] : XA_PRIMARY);
...
StdStrBuf CStdApp::Paste(bool fClipboard) {
  ...
  XConvertSelection(dpy, fClipboard ? ClipboardAtoms[0] : XA_PRIMARY, XA_STRING,
                    XA_STRING, pWindow->wnd, Priv->LastEventTime);
  ...
  int result = XGetWindowProperty(dpy, pWindow->wnd, XA_STRING, 0, bytes_left,
                                  1,  // delete it now
...
void CStdApp::ClearClipboard(bool fClipboard) {
  ...
  XSetSelectionOwner(dpy, fClipboard ? ClipboardAtoms[0] : XA_PRIMARY, None,
                     d.AcquirationTime);
  d.Text.Clear();
}
...
bool CStdApp::ReadStdInCommand() {
  ...
  return true;
#else
  // Surely not the most efficient way to do it, but we won't have to read much
  // data anyway.
  char c;
  if (read(0, &c, 1) != 1) return false;
  if (c == '\n') {
    if (!CmdBuf.isNull()) {
