...
bool StdBuf::SaveToFile(const char *szFile) const {
  // Open file
  int fh = open(szFile, O_BINARY | O_CREAT | O_WRONLY | O_SEQUENTIAL | O_TRUNC,
                S_IREAD | S_IWRITE);
  if (fh < 0) return false;
  // Write data
...
void StdBuf::CompileFunc(StdCompiler *pComp, int iType) {
  // Size (guess it is a small value most of the time - if it's big, an extra
  // byte won't hurt anyway)
  uint32_t tmp = iSize;
  pComp->Value(mkIntPackAdapt(tmp));
  iSize = tmp;
  ...
  if (pComp->isCompiler()) {
    ...
    pComp->Raw(const_cast<void *>(getData()), iSize,
               StdCompiler::RawCompileType(iType));
  }
}
...
// replacements.
int StdStrBuf::Replace(const char *szOld, const char *szNew,
                       size_t iStartSearch) {
...
bool StdStrBuf::ValidateChars(const char *szInitialChars,
  ...
  for (size_t i = 0; i < getLength(); ++i)
    if (!strchr(i ? szMidChars : szInitialChars, getData()[i])) return false;
  return true;
}

bool StdStrBuf::GetSection(size_t idx, StdStrBuf *psOutSection,
                           char cSeparator) const {
  assert(psOutSection);
  psOutSection->Clear();
  const char *szStr = getData(), *szSepPos;
  if (!szStr) return false;  // invaid argument
  while ((szSepPos = strchr(szStr, cSeparator)) && idx) {
    szStr = szSepPos + 1;
    --idx;
  }
...
void StdStrBuf::ToLowerCase() {
  if (!isNull())
    for (char *szPos = getMPtr(0); *szPos; ++szPos) *szPos = tolower(*szPos);
}
...
void StdStrBuf::EnsureUnicode() {
  ...
  for (size_t i = 0; i < getSize(); ++i) {
    ...
    if (0xE0 <= c && c <= 0xEF) {
      ...
      continue;
      // FIXME: could check for UTF-16 surrogates from a broken utf-16->utf-8
      // converter here
    }
  ...
  if (need_continuation_bytes) valid = false;
  ...
  if (!valid) {
    ...
    for (size_t i = 0; i < getSize(); ++i) {
      unsigned char c = *getPtr(i);
      if (c < 0x80) {
        if (j >= buf.getLength()) buf.Grow(GROWSIZE);
        *buf.getMPtr(j++) = c;
        continue;
      }
      if (0xA0 <= c) {
        if (j + 1 >= buf.getLength()) buf.Grow(GROWSIZE);
...
bool StdStrBuf::TrimSpaces() {
  ...
  while (isspace((unsigned char)szStr[iLength - 1 - iSpaceRightCount]))
    ++iSpaceRightCount;
  // anything to trim?
  ...
  memmove(getMPtr(0), szStr + iSpaceLeftCount,
          iLength - iSpaceLeftCount - iSpaceRightCount);
