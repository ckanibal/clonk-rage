...
class C4Fixed {
 ...
 public:
  // constructors
  inline C4Fixed() { /*val=0;*/
  }                  // why initialize?
  inline C4Fixed(const C4Fixed &rCpy) : val(rCpy.val) {}

  // Conversion must be done by the conversion routines itofix, fixtoi, ftofix
  // and fixtof
  // in order to be backward compatible, so everything is private.
 private:
  explicit inline C4Fixed(int32_t iVal) : val(iVal * FIXED_FPF) {}
  explicit inline C4Fixed(int32_t iVal, int32_t iPrec)
      : val(iPrec < FIXED_FPF
                ? iVal * (FIXED_FPF / iPrec) +
                      (iVal * (FIXED_FPF % iPrec)) / iPrec
  ...
  float to_float() const { return float(val) / float(FIXED_FPF); }
 ...
 public:
...
#else
    int32_t x0 = val & (FIXED_FPF - 1), x1 = val >> FIXED_SHIFT;
    int32_t y0 = fVal2.val & (FIXED_FPF - 1), y1 = fVal2.val >> FIXED_SHIFT;
  ...
  inline C4Fixed operator+() const { return *this; }

  inline bool operator==(const C4Fixed &fVal2) const {
    return val == fVal2.val;
  }
  ...
  // and wrappers
  inline C4Fixed &operator+=(int32_t iVal2) {
    return operator+=(C4Fixed(iVal2));
  }
  inline C4Fixed &operator-=(int32_t iVal2) {
    return operator-=(C4Fixed(iVal2));
  }

  inline C4Fixed operator+(const C4Fixed &fVal2) const {
    return C4Fixed(*this) += fVal2;
  }
  inline C4Fixed operator-(const C4Fixed &fVal2) const {
    return C4Fixed(*this) -= fVal2;
  }
  ...
  inline C4Fixed operator+(int32_t iVal2) const {
    return C4Fixed(*this) += iVal2;
  }
  inline C4Fixed operator-(int32_t iVal2) const {
    return C4Fixed(*this) -= iVal2;
  }
  ...
  inline C4Fixed operator+(int iVal2) const {
    return operator+(int32_t(iVal2));
  }
  inline C4Fixed operator-(int iVal2) const {
    return operator-(int32_t(iVal2));
  }
  ...
  inline bool operator==(int32_t iVal2) const {
    return operator==(C4Fixed(iVal2));
  }
  inline bool operator<(int32_t iVal2) const {
    return operator<(C4Fixed(iVal2));
...
#if defined(_MSC_VER) && _MSC_VER <= 1200
  inline bool operator==(int iVal2) const {
    return operator==(C4Fixed(int32_t(iVal2)));
  }
  inline bool operator<(int iVal2) const {
    return operator<(C4Fixed(int32_t(iVal2)));
  }
  ...
  C4Fixed sin_deg() const {
    // adjust angle
    int32_t v = int32_t((int64_t(val) * 100) / FIXED_FPF);
    if (v < 0) v = 18000 - v;
    v %= 36000;
    // get sine
    C4Fixed fr;
    switch (v / 9000) {
      case 0:
        ...
        break;
      case 1:
        fr.val = +SineTable[18000 - v];
        break;
      case 2:
        fr.val = -SineTable[v - 18000];
        break;
      case 3:
        fr.val = -SineTable[36000 - v];
        break;
    }
    return fr;
  }
  C4Fixed cos_deg() const {
    ...
    switch (v / 9000) {
      case 0:
        fr.val = +SineTable[9000 - v];
        break;
      case 1:
        fr.val = -SineTable[v - 9000];
        break;
      case 2:
        fr.val = -SineTable[27000 - v];
        break;
      case 3:
        fr.val = +SineTable[v - 27000];
        break;
    }
    return fr;
  }
...
inline FIXED FIXED256(int x) {
  C4Fixed r;
  r.val = x * FIXED_FPF / 256;
  return r;
}
...
#ifdef USE_FIXED
  if (cFormat == 'f') FLOAT_TO_FIXED(&rValue);
#else
  if (cFormat == 'F') FIXED_TO_FLOAT(&rValue);
#endif
}
