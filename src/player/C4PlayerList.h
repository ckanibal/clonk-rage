...
class C4PlayerList {
 ...
 public:
  ...
  C4Player *Join(const char *szFilename, BOOL fScenarioInit, int iAtClient,
                 const char *szAtClientName, class C4PlayerInfo *pInfo);
  BOOL CtrlJoinLocalNoNetwork(const char *szFilename, int iAtClient,
                              const char *szAtClientName);
  BOOL CtrlJoin(const class C4Network2ResCore &ResCore, int iClientID,
                int idPlayerInfo);
  BOOL FileInUse(const char *szFilename) const;
  BOOL Retire(C4Player *pPlr);
  BOOL Evaluate();
  BOOL Save(bool fSaveLocalOnly);
  BOOL Save(C4Group &hGroup, bool fStoreTiny,
            const C4PlayerInfoList &
                rStoreList);  // save all players present in the restore list
  ...
  bool HostilityDeclared(int iPlayer1, int iPlayer2) const;  // check whether
                                                             // iPlayer1 treats
                                                             // iPlayer2 as
                                                             // hostile, but not
                                                             // vice versa!
  ...
  int CheckColorDw(DWORD dwColor, C4Player *pExclude);  // return minimum
                                                        // difference to the
                                                        // other player's colors
  BOOL ControlTaken(int iControl) const;
  BOOL SynchronizeLocalFiles();  // syncrhonize all local player files;
                                 // resetting InGame times
 protected:
