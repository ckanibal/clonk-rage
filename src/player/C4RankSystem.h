...
class C4RankSystem {
 public:
  ...
  enum {
    EXP_NoPromotion = -1
  };  // experience value for NextRankExp: No more promotion possible
 ...
 protected:
  ...
  char **pszRankExtensions;  // rank extensions (e.g. "%s First class") for even
                             // more ranks!
 ...
 public:
  ...
  bool Load(C4Group &hGroup, const char *szFilenames, int DefRankBase,
            const char *szLanguage);  // init based on nk file in group
...
#ifdef C4ENGINE
  static bool DrawRankSymbol(
      C4FacetExSurface *fctSymbol, int32_t iRank, C4FacetEx *pfctRankSymbols,
      int32_t iRankSymbolCount, bool fOwnSurface, int32_t iXOff = 0,
      C4Facet *cgoDrawDirect = NULL);  // create facet from rank symbol for
                                       // definition - use custom rank facets if
                                       // present
#endif
};
