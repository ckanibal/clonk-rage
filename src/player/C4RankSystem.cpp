...
bool C4RankSystem::Load(C4Group &hGroup, const char *szFilenames,
                        int DefRankBase, const char *szLanguage) {
  // clear any loaded rank names
  Clear();
  assert(szFilenames);
  assert(szLanguage);
        ...
        // no comment?
          ...
          if (iEqPos >= 0) {
            ...
            if (SEqual(pRank0, "Base"))
              // get rankbase
              // note that invalid numbers may cause desyncs here...not very
              // likely though :)
...
BOOL C4RankSystem::Check(int iRank, const char *szDefRankName) {
  ...
  if (GetRegistryString(Register, keyname, rankname, C4MaxName + 1))
    return FALSE;
  if (!szDefRankName || (SLen(szDefRankName) > C4MaxName)) return FALSE;
...
void C4RankSystem::Clear() {
  ...
  if (pszRankExtensions) {
    delete[] pszRankExtensions;
...
bool C4RankSystem::DrawRankSymbol(C4FacetExSurface *fctSymbol, int32_t iRank,
                                  C4FacetEx *pfctRankSymbols,
                                  int32_t iRankSymbolCount, bool fOwnSurface,
                                  int32_t iXOff, C4Facet *cgoDrawDirect) {
  // safety
  if (iRank < 0) iRank = 0;
  // symbol by rank
  ...
  if (pfctRankSymbols->GetPhaseNum(iMaxRankSym, Q)) {
    ...
    if (iRank / iRankSymbolCount) {
      // extended rank: draw
      // extension star defaults to captain star; but use extended symbols if
      // they are in the gfx
      ...
      if (!cgoDrawDirect) {
        ...
        pfctRankSymbols->DrawX(fctSymbol->Surface, 0, 0, iSize, iSize,
                               iBaseRank);
        fctExtended.DrawX(fctSymbol->Surface, 0, 0, iSize * 2 / 3,
                          iSize * 2 / 3);
      } else {
        pfctRankSymbols->Draw(cgoDrawDirect->Surface, cgoDrawDirect->X + iXOff,
                              cgoDrawDirect->Y, iBaseRank);
        fctExtended.Draw(cgoDrawDirect->Surface, cgoDrawDirect->X + iXOff - 4,
                         cgoDrawDirect->Y - 3);
      }
    } else {
