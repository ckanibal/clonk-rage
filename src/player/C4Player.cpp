...
C4Player::C4Player() : C4PlayerInfoCore() { Default(); }
...
void C4Player::ClearPointers(C4Object *pObj, bool fDeath) {
  ...
  // Cursor
  if (Cursor == pObj) {
    // object is to be deleted; do NOT do script calls (like in
    // Cursor->UnSelect(TRUE))
    Cursor = NULL;
    AdjustCursorCommand();  // also selects and eventually does a script call!
  }
  // View-Cursor
...
bool C4Player::ScenarioAndTeamInit(int32_t idTeam) {
  ...
  if (idTeam == TEAMID_New) {
    ...
    if (!Game.Teams.IsAutoGenerateTeams())
      ...
      if (pTeam = Game.Teams.GetGenerateTeamByID(idTeam))
        idTeam = pTeam->GetID();
    }
  } else {
...
void C4Player::Execute() {
  ...
  if (Status == PS_TeamSelection) {
    ...
    if (idSelectedTeam = Game.Teams.GetForcedTeamSelection(ID)) {
      ...
      if (Menu.IsActive() && Menu.GetIdentification() == C4MN_TeamSelection)
        Menu.TryClose(false, false);
      if (LocalControl && !Game.Control.isReplay()) {
        // team selection done through queue because TeamSelection-status may
        // not be in sync (may be TeamSelectionPending!)
        DoTeamSelection(idSelectedTeam);
      }
    } else if (!Menu.IsActive())
      ActivateMenuTeamSelection(false);
    else {
      // during team selection: Update view to selected team, if it has a
      // position assigned
      ...
      if (pSelectedTeamItem = Menu.GetSelectedItem()) {
        ...
        if (idSelectedTeam) {
          ...
          if (pSelectedTeam = Game.Teams.GetTeamByID(idSelectedTeam)) {
            ...
            if (iPlrStartIndex &&
                Inside<int32_t>(iPlrStartIndex, 1, C4S_MaxPlayer)) {
              if (Game.C4S.PlrStart[iPlrStartIndex - 1].Position[0] > -1) {
                // player has selected a team that has a valid start position
                // assigned
                // set view to this position!
                ViewX = Game.C4S.PlrStart[iPlrStartIndex - 1].Position[0] *
                        Game.Landscape.MapZoom;
                ViewY = Game.C4S.PlrStart[iPlrStartIndex - 1].Position[1] *
                        Game.Landscape.MapZoom;
              }
            }
          }
             ...
             Menu.GetIdentification() == C4MN_TeamSelection) {
    Menu.TryClose(false, false);
  }
  ...
  ExecuteControl();
  Menu.Execute();
  if (Cursor) Cursor->AutoContextMenu(-1);
  ...
  while (pLnk = pLnkNext) {
    ...
    if (!pDeadClonk->GetAlive() && (pDeadClonk->Category & C4D_Living) &&
        pDeadClonk->Status) {
      pDeadClonk->PlrViewRange -= 10;
      if (pDeadClonk->PlrViewRange <= 0) FoWViewObjs.Remove(pDeadClonk);
    }
  }
...
BOOL C4Player::Init(int32_t iNumber, int32_t iAtClient,
                    const char *szAtClientName, const char *szFilename,
                    BOOL fScenarioInit, class C4PlayerInfo *pInfo) {
  // safety
  if (!pInfo) {
  ...
  if (szFilename) {
    ...
    // this will prevent portraits from being shown for "remotely
    // controlled"-Clonks of other players
    bool fLoadPortraits =
        (AtClient == C4ClientIDUnknown) ||
        SEqualNoCase(AtClientName, Game.Clients.getLocalName());
    // fLoadPortraits = true
  ...
  // Take player name from player info; forcing overloads by the league or
  // because of doubled player names
  ...
  if (fScenarioInit) {
    ...
    // Number might have changed: Recheck list sorting before scenarioinit,
    // which will do script calls
    Game.Players.RecheckPlayerSort(this);

    // check for a postponed scenario init, if no team is specified
    // (post-lobby-join in network, or simply non-network)
    C4Team *pTeam = NULL;
    if (Team) {
      if (Game.Teams.IsAutoGenerateTeams())
    ...
    if (!pTeam && Game.Teams.IsRuntimeJoinTeamChoice()) {
      if (pInfo->GetType() == C4PT_Script) {
        // script player without team: This can usually not happen, because
        // RecheckPlayerInfoTeams should have been executed
    ...
    // Init control method before scenario init, because script callbacks may
    // need to know it!
    InitControl();

    // Special: Script players may skip scenario initialization altogether, and
    // just desire a single callback to all objects
    // of a given ID
    if (!pInfo->IsScenarioInitDesired()) {
      ...
      if (idCallback && (pDefCallback = C4Id2Def(idCallback))) {
        pDefCallback->Script.Call(PSF_InitializeScriptPlayer,
                                  &C4AulParSet(C4VInt(Number), C4VInt(Team)));
      }
    } else {
      // player preinit: In case a team needs to be chosen first, no
      // InitializePlayer-broadcast is done
      // this callback shall give scripters a chance to do stuff like starting
      // an intro or enabling FoW, which might need to be done
      Game.Script.GRBroadcast(PSF_PreInitializePlayer,
                              &C4AulParSet(C4VInt(Number)));
      // direct init
      if (Status != PS_TeamSelection)
        if (!ScenarioInit()) return FALSE;
      // ...and home base material from team
  ...
  // Load runtime data
    ...
    // (compile using DefaultRuntimeData) - also check if compilation returned
    // sane results, i.e. ID assigned
    if (!LoadRuntimeData(Game.ScenarioFile) || !ID) {
      // for script players in non-savegames, this is OK - it means they get
      // restored using default values
      // this happens when the users saves a scenario using the "Save
      // scenario"-option while a script player
      // was joined
    ...
    // change clients
    // (only call should be savegame recreation by
    // C4PlayerInfoList::RecreatePlayers)
    AtClient = iAtClient;
    SCopy(szAtClientName, AtClientName, C4MaxTitle);
    // Number might have changed: Recheck list sorting
    Game.Players.RecheckPlayerSort(this);
    // Init control after loading runtime data, because control init will
  ...
  for (C4ObjectLink *pLnk = Game.Objects.First; pLnk; pLnk = pLnk->Next) {
    ...
    if (pObj->PlrViewRange && pObj->Owner == NO_OWNER) pObj->PlrFoWActualize();
  }
...
// because we know the file to be temporary.
  ...
  // Open group
  if (!hGroup.Open(szPath, TRUE)) return FALSE;
  // Save
...
BOOL C4Player::Save(C4Group &hGroup, bool fSavegame, bool fStoreTiny) {
  // Save core
  if (!C4PlayerInfoCore::Save(hGroup)) return FALSE;
// Save crew
#ifndef C4ENGINE
  ...
  if (Game.C4S.PlrStart[PlrStartIndex].ReadyCrew.IsClear()) {
    ...
    for (cnt = 0; cnt < crewnum; cnt++) {
      ...
      while (!(pInfo = CrewInfoList.GetIdle(idStdCrew, Game.Defs)))
        if (!CrewInfoList.New(idStdCrew, &Game.Defs, cpNames)) break;
    ...
    for (cnt = 0;
      ...
      for (int32_t cnt2 = 0; cnt2 < iCount; cnt2++) {
        ...
        while (!(pInfo = CrewInfoList.GetIdle(id, Game.Defs)))
          if (!CrewInfoList.New(id, &Game.Defs, cpNames)) break;
        // Safety
        if (!pInfo || !(pDef = C4Id2Def(pInfo->id))) continue;
        // Crew placement location
        ctx = tx1 + Random(tx2 - tx1);
...
C4Object *CreateLine(C4ID linetype, int32_t owner, C4Object *fobj,
                     C4Object *tobj);
  ...
  for (cnt = 0; (cid = Game.C4S.PlrStart[PlrStartIndex].ReadyBase.GetID(cnt));
    ...
    if (def = C4Id2Def(cid))
      for (cnt2 = 0;
        ...
        cty = ty;
        if (Game.C4S.PlrStart[PlrStartIndex].EnforcePosition ||
            FindConSiteSpot(ctx, cty, def->Shape.Wdt, def->Shape.Hgt,
                            def->Category, 20))
          if (cbase = Game.CreateObjectConstruction(cid, NULL, Number, ctx, cty,
                                                    FullCon, TRUE)) {
            // FirstBase
            if (!(*pFirstBase))
...
void C4Player::PlaceReadyVehic(int32_t tx1, int32_t tx2, int32_t ty,
                               C4Object *FirstBase) {
  int32_t cnt, cnt2, ctx, cty;
  C4Def *def;
  C4ID cid;
...
void C4Player::PlaceReadyMaterial(int32_t tx1, int32_t tx2, int32_t ty,
                                  C4Object *FirstBase) {
  int32_t cnt, cnt2, ctx, cty;
  C4Def *def;
  C4ID cid;
...
{
  return RGB(Min(SafeRandom(302), 256), Min(SafeRandom(302), 256),
             Min(SafeRandom(302), 256));
}
...
BOOL C4Player::ScenarioInit() {
  ...
  if (!pInfo) {
    assert(false);
    LogF("Internal error: ScenarioInit for ghost player %s!", GetName());
    return FALSE;
  }
  ...
  // Zoomed position
  if (ptx > -1)
    ptx = BoundBy<int32_t>(ptx * Game.C4S.Landscape.MapZoom.Evaluate(), 0,
                           GBackWdt - 1);
  if (pty > -1)
    pty = BoundBy<int32_t>(pty * Game.C4S.Landscape.MapZoom.Evaluate(), 0,
                           GBackHgt - 1);
  ...
  Game.Script.GRBroadcast(
      ...
      &C4AulParSet(C4VInt(Number), C4VInt(ptx), C4VInt(pty), C4VObj(FirstBase),
                   C4VInt(Team), C4VID(GetInfo()->GetScriptPlayerExtraID())));
  return TRUE;
}
...
BOOL C4Player::FinalInit(BOOL fInitialValue) {
  ...
  if (LocalControl)
    if (MouseControl) Game.MouseControl.Init(Number);
...
void C4Player::SetFoW(bool fEnable) {
  // enable FoW
  if (fEnable && !fFogOfWarInitialized) Game.Objects.AssignPlrViewRange();
  // set flag
...
C4Object *C4Player::Buy(C4ID id, BOOL fShowErrors, int32_t iForPlr,
                        C4Object *pBuyObj) {
  int32_t iAvailable;
  C4Def *pDef;
  C4Object *pThing;
  ...
  if (Eliminated) {
    if (!fShowErrors) return NULL;
    StartSoundEffect("Error", false, 100, pBuyObj);
    GameMsgPlayer(
        FormatString(LoadResStr("IDS_PLR_ELIMINATED"), GetName()).getData(),
        Number);
    return NULL;
  }
  ...
  if (!HomeBaseMaterial.DecreaseIDCount(id, FALSE)) return NULL;
  if (Game.Rules & C4RULE_TeamHombase) SyncHomebaseMaterialToTeam();
  // Reduce wealth
  DoWealth(-iValue);
  // Create object (for player)
  if (!(pThing = Game.CreateObject(id, pBuyObj, iForPlr))) return NULL;
  // Make crew member
  if (pDef->CrewMember)
    if (ValidPlr(iForPlr)) Game.Players.Get(iForPlr)->MakeCrewMember(pThing);
  // success
  C4AulParSet parset(C4VInt(Number), C4VObj(pBuyObj));
  pThing->Call(PSF_Purchase, &parset);
  if (!pThing->Status) return NULL;
  return pThing;
}
...
void C4Player::Evaluate() {
  ...
  // Cooperative: shared score
    ...
    LastRound.Score = Max(Game.Players.AverageValueGain(), 0);
...
BOOL C4Player::SetHostility(int32_t iOpponent, int32_t iHostility,
                            BOOL fSilent) {
  ...
  if (hirank = GetHiRankActiveCrew(false))
    if (hirank->Info)
      if (hirank->Info->Rank < 1)  // No Fähnrich -> except. promo.
        if (hirank = GetHiExpActiveCrew(false)) hirank->Promote(1, TRUE, false);
}
...
BOOL C4Player::Strip(const char *szFilename, bool fAggressive) {
  ...
  if (!fAggressive) {
    ...
    if (Grp.FindEntry(C4CFN_BigIcon, NULL, &iBigIconSize))
      if (iBigIconSize > C4NetResMaxBigicon * 1024) Grp.Delete(C4CFN_BigIcon);
    Grp.Close();
  } else {
    // Load info core and crew info list
    C4PlayerInfoCore PlrInfoCore;
    C4ObjectInfoList CrewInfoList;
    if (!PlrInfoCore.Load(Grp) || !CrewInfoList.Load(Grp, false)) return FALSE;
    // Strip crew info list (remove object infos that are invalid for this
    // scenario)
    CrewInfoList.Strip(Game.Defs);
    // Create a new group that receives the bare essentials
    Grp.Close();
    if (!EraseItem(szFilename) || !Grp.Open(szFilename, TRUE)) return FALSE;
    // Save info core & crew info list to newly-created file
    if (!PlrInfoCore.Save(Grp) ||
        !CrewInfoList.Save(Grp, true, true, &Game.Defs))
      return FALSE;
    Grp.Close();
  }
  return TRUE;
...
BOOL C4Player::MakeCrewMember(C4Object *pObj, bool fForceInfo, bool fDoCalls) {
  ...
  if (!pObj->Info && fForceInfo) {
    ...
    if (pObj->nInfo) cInf = CrewInfoList.GetIdle(pObj->nInfo.getData());
    ...
    if (!cInf)
      while (!(cInf = CrewInfoList.GetIdle(pObj->id, Game.Defs)))
        if (!CrewInfoList.New(pObj->id, &Game.Defs, cpNames)) return FALSE;
  ...
  // Add to crew
  if (!Crew.GetLink(pObj)) Crew.Add(pObj, C4ObjectList::stMain);

  // add plr view
  if (!pObj->PlrViewRange)
    pObj->SetPlrViewRange(C4FOW_Def_View_RangeX);
  else
...
void C4Player::ExecuteControl() {
  ...
  if (LastCom != COM_None) {
    ...
    if (LastComDelay > C4DoubleClick) {
      // Pass additional COM_Single com (unless it already was a single com in
      // the first place)
      if (!(LastCom & COM_Single))
        DirectCom(LastCom | COM_Single,
                  0);  // Currently, com data is not stored for single coms...
      LastCom = COM_None;
      LastComDelay = 0;
    }
  }
...
void C4Player::AdjustCursorCommand() {
  ...
  if (!pHiRank) pHiRank = GetHiRankActiveCrew(false);
  ...
  if (Cursor) { /*Cursor->DoSelect(TRUE);*/
    Cursor->DoSelect();
  }  // Hmm, why DoSelect() twice? I hope the second one is the correct one...
  // Updates
  CursorFlash = 30;
}
...
void C4Player::UnselectCrew() {
  C4Object *cObj;
  C4ObjectLink *cLnk;
  bool fCursorDeselected = false;
  for (cLnk = Crew.First; cLnk && (cObj = cLnk->Obj); cLnk = cLnk->Next) {
    if (Cursor == cObj) fCursorDeselected = true;
    if (cObj->Status) cObj->UnSelect();
  }
  // if cursor is outside crew (done by some scenarios), unselect that one, too!
  // (script callback)
...
void C4Player::SelectAllCrew() {
  C4ObjectLink *clnk;
  // Select all crew
  for (clnk = Crew.First; clnk; clnk = clnk->Next) clnk->Obj->DoSelect();
  // Updates
  AdjustCursorCommand();
...
void C4Player::UpdateSelectionToggleStatus() {
  if (CursorSelection)
    ...
    if (CursorToggled) AdjustCursorCommand();
...
BOOL C4Player::ObjectCom(BYTE byCom, int32_t iData)  // By DirectCom
{
  ...
  if (!(byCom & COM_Single) && !(byCom & COM_Double) &&
      (byCom < COM_ReleaseFirst || byCom > COM_ReleaseLast))
...
BOOL C4Player::ObjectCommand(int32_t iCommand, C4Object *pTarget, int32_t iX,
                             int32_t iY, C4Object *pTarget2, int32_t iData,
                             int32_t iMode) {
  // Eliminated
  if (Eliminated) return FALSE;
  // Hide startup
  ...
  C4ObjectLink *cLnk;
  C4Object *cObj;
  bool fCursorProcessed = false;
  for (cLnk = Crew.First; cLnk && (cObj = cLnk->Obj); cLnk = cLnk->Next) {
    if (cObj == Cursor) fCursorProcessed = true;
    if (cObj->Status)
      if (cObj->Select)
        if (!(iMode & C4P_Command_Range) ||
            (Cursor && Inside<int32_t>(cObj->x - Cursor->x, -15, +15) &&
             Inside<int32_t>(cObj->y - Cursor->y, -15, +15)))
          if (cObj != pTarget) {
            // Put command with unspecified put object
            if ((iCommand == C4CMD_Put) && !pTarget2) {
              // Special: the put command is only applied by this function if
              // the clonk actually
              // has something to put. Also, the put count is adjusted to that
              // the clonk will not try to put
              // more items than he actually has. This workaround is needed so
              // the put command can be used
              // to tell all selected clonks to put when in a container,
              // simulating the old all-throw behavior.
              if (cObj->Contents.ObjectCount(iData))
                ObjectCommand2Obj(
                    cObj, iCommand, pTarget,
                    Min<int32_t>(iX, cObj->Contents.ObjectCount(iData)), iY,
                    pTarget2, iData, iMode);
            }
            // Other command
            else
              ObjectCommand2Obj(cObj, iCommand, pTarget, iX, iY, pTarget2,
                                iData, iMode);
            // don't issue multiple Construct-commands - store previous Clonk as
            // target for next command object
            // note that if three Clonks get the command, and the middle one
            // gets deleted (the Clonk, not the command), the third will start
            // to build anyway
            // that is very unlikely, though...could be catched in
            // ClearPointers, if need be?
            // also, if one Clonk of the chain aborts his command (controlled
            // elsewhere, for instance), the following ones will fail their
            // commands
            // It's not a perfect solution, after all. But certainly better than
            // placing tons of construction sites in the first place
  ...
  if (Cursor && !fCursorProcessed)
    if (Cursor->Status && Cursor != pTarget)
      ObjectCommand2Obj(Cursor, iCommand, pTarget, iX, iY, pTarget2, iData,
                        iMode);
...
void C4Player::ObjectCommand2Obj(C4Object *cObj, int32_t iCommand,
                                 C4Object *pTarget, int32_t iX, int32_t iY,
                                 C4Object *pTarget2, int32_t iData,
                                 int32_t iMode) {
  // forward to object
  if (iMode & C4P_Command_Append)
    cObj->AddCommand(iCommand, pTarget, iX, iY, 0, pTarget2, TRUE, iData, TRUE,
                     0, NULL, C4CMD_Mode_Base);  // append: by Shift-click and
                                                 // for dragging of multiple
                                                 // objects (all independant;
                                                 // thus C4CMD_Mode_Base)
  else if (iMode & C4P_Command_Add)
    cObj->AddCommand(iCommand, pTarget, iX, iY, 0, pTarget2, TRUE, iData, FALSE,
                     0, NULL, C4CMD_Mode_Base);  // append: by context menu and
                                                 // keyboard throw command (all
                                                 // independant; thus
                                                 // C4CMD_Mode_Base)
...
void C4Player::DirectCom(BYTE byCom,
                         int32_t iData)  // By InCom or ExecuteControl
{
  switch (byCom) {
    case COM_CursorLeft:
      CursorLeft();
  ...
  if (!Inside(byCom, COM_ReleaseFirst, COM_ReleaseLast)) {
    ...
    if (LastCom != COM_None)
      if (LastCom != byCom) {
        ...
        // So a COM_Down_S does what a COM_Down_D normally does, if generated by
        // another key
        // instead of a timeout
        if (ControlStyle && LastCom == COM_Down)
          LastComDownDouble = C4DoubleClick;
      }
    // Check LastCom buffer for COM_Double
    if (LastCom == byCom) byCom |= COM_Double;
    // LastCom/Del process
    // this is set before issuing the DirectCom, so DirectCom-scripts may delete
    // it
    LastCom = byCom;
    LastComDelay = 0;
  } else {
...
BOOL C4Player::LoadRuntimeData(C4Group &hGroup) {
  ...
  // safety: Do nothing if playeer section is not even present (could kill
  // initialized values)
  if (!SSearch(pSource, FormatString("[Player%i]", ID).getData())) return FALSE;
  // Compile (Search player section - runtime data is stored by unique player
  // ID)
  assert(ID);
  if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(
          mkNamingAdapt(*this, FormatString("Player%i", ID).getData()),
          StdStrBuf(pSource), Game.GameText.GetFilePath()))
    return FALSE;
  // Denumerate pointers
  DenumeratePointers();
  // Success
  ...
  if (ProductionDelay >= 60)  // Minute Production Unit
  {
    ...
    for (int32_t cnt = 0; HomeBaseProduction.GetID(cnt); cnt++)
      if (HomeBaseProduction.GetCount(cnt) > 0)
        if (ProductionUnit %
                BoundBy<int32_t>(11 - HomeBaseProduction.GetCount(cnt), 1,
                                 10) ==
            0)
...
bool C4Player::ActivateMenuTeamSelection(bool fFromMain) {
  ...
  Menu.InitRefSym(C4GUI::Icon::GetIconFacet(C4GUI::Ico_Team),
                  LoadResStr("IDS_MSG_SELTEAM"), Number, C4MN_Extra_None, 0,
                  fSwitch ? C4MN_TeamSwitch : C4MN_TeamSelection);
...
void C4Player::DoTeamSelection(int32_t idTeam) {
  // stop team selection. This might close the menu forever if the control gets
  // lost
  // let's hope it doesn't!
  Status = PS_TeamSelectionPending;
  Game.Control.DoInput(
      CID_Script,
      new C4ControlScript(FormatString("InitScenarioPlayer(%d,%d)", (int)Number,
                                       (int)idTeam).getData()),
      CDT_Queue);
}
...
void C4Player::EnumeratePointers() {
  ...
  // messageboard-queries
  for (C4MessageBoardQuery *pCheck = pMsgBoardQuery; pCheck;
       pCheck = pCheck->pNext)
    pCheck->nCallbackObj = pCheck->pCallbackObj
                               ? Game.Objects.ObjectNumber(pCheck->pCallbackObj)
                               : 0;
}
...
void C4Player::DenumeratePointers() {
  ...
  for (C4MessageBoardQuery *pCheck = pMsgBoardQuery; pCheck;
       pCheck = pCheck->pNext)
    pCheck->pCallbackObj =
        pCheck->nCallbackObj ? Game.Objects.ObjectPointer(pCheck->nCallbackObj)
                             : NULL;
}
...
void C4Player::NotifyOwnedObjects() {
  ...
  for (C4ObjectList *pList = &Game.Objects; pList;
       pList =
           ((pList == &Game.Objects) ? &Game.Objects.InactiveObjects : NULL))
    for (clnk = pList->First; clnk && (cobj = clnk->Obj); clnk = clnk->Next)
      if (cobj->Status)
        if (cobj->Owner == Number) {
          C4AulFunc *pFn =
              cobj->Def->Script.GetFuncRecursive(PSF_OnOwnerRemoved);
...
void C4Player::SetCursor(C4Object *pObj, bool fSelectFlash, bool fSelectArrow) {
  ...
  // Select object
  if (Cursor) {
    Cursor->DoSelect(TRUE); /*Cursor->DoSelect();*/
  }  // Sven2: why the second DoSelect()? - it breaks cursor-toggle-selection
     // View flash
...
void C4Player::SelectCrew(C4ObjectList &rList) {
  ...
  for (clnk = rList.First; clnk; clnk = clnk->Next)
    if (clnk->Obj->Status) clnk->Obj->DoSelect();
  // Updates
  AdjustCursorCommand();
  CursorSelection = CursorToggled = 0;
  SelectFlash = 30;
...
void C4Player::ScrollView(int32_t iX, int32_t iY, int32_t ViewWdt,
                          int32_t ViewHgt) {
  SetViewMode(C4PVM_Scrolling);
  int32_t ViewportScrollBorder =
      Application.isFullScreen ? C4ViewportScrollBorder : 0;
  ViewX = BoundBy<int32_t>(ViewX + iX, ViewWdt / 2 - ViewportScrollBorder,
                           GBackWdt + ViewportScrollBorder - ViewWdt / 2);
  ViewY = BoundBy<int32_t>(ViewY + iY, ViewHgt / 2 - ViewportScrollBorder,
                           GBackHgt + ViewportScrollBorder - ViewHgt / 2);
}
...
void C4Player::InitControl() {
  ...
  if (AtClient == Game.Control.ClientID())
    if (!GetInfo() || GetInfo()->GetType() == C4PT_User) LocalControl = TRUE;
  // Set control
  Control = C4P_Control_None;
  // Preferred control
  int32_t iControl = PrefControl;
  // gamepad control safety
  if (Inside<int32_t>(iControl, C4P_Control_GamePad1, C4P_Control_GamePadMax) &&
      !Config.General.GamepadEnabled) {
  ...
  if (Game.Players.ControlTaken(iControl)) {
    ...
    if (iControl > C4P_Control_Keyboard4) iControl = C4P_Control_None;
  }
  // Set control
  Control = iControl;
  // Cotrol style
  if (Game.C4S.Head.ForcedControlStyle > -1)
    ControlStyle = Game.C4S.Head.ForcedControlStyle;
  else
    ControlStyle = PrefControlStyle;
  if (Game.C4S.Head.ForcedAutoContextMenu > -1)
    AutoContextMenu = Game.C4S.Head.ForcedAutoContextMenu;
  else
    AutoContextMenu = PrefAutoContextMenu;
  // init gamepad
  if (pGamepad) {
    delete pGamepad;
    pGamepad = NULL;
  ...
  if (PrefMouse && !Game.Control.isReplay())
    if (!Game.C4S.Head.DisableMouse)
      if (Inside<int32_t>(Control, C4P_Control_Keyboard1,
                          C4P_Control_GamePadMax))
        if (!Game.Players.MouseControlTaken()) MouseControl = TRUE;
...
void C4Player::FoW2Map(CClrModAddMap &rMap, int iOffX, int iOffY) {
  // No fog of war
  if (!fFogOfWar) return;
  // Add view for all FoW-repellers - keep track of FoW-generators, which should
  // be avaluated finally
  ...
  for (clnk = FoWViewObjs.First; clnk && (cobj = clnk->Obj); clnk = clnk->Next)
    if (!cobj->Contained || cobj->Contained->Def->ClosedContainer != 1)
      if (cobj->PlrViewRange > 0)
        rMap.ReduceModulation(cobj->x + iOffX, cobj->y + iOffY,
                              cobj->PlrViewRange * 2 / 3, cobj->PlrViewRange);
      else
        fAnyGenerators = true;
  // Add view for target view object
  if (ViewMode == C4PVM_Target)
    if (ViewTarget)
      if (!ViewTarget->Contained ||
          ViewTarget->Contained->Def->ClosedContainer != 1) {
        int iRange = ViewTarget->PlrViewRange;
        if (!iRange && Cursor) iRange = Cursor->PlrViewRange;
        if (!iRange) iRange = C4FOW_Def_View_RangeX;
        rMap.ReduceModulation(ViewTarget->x + iOffX, ViewTarget->y + iOffY,
                              iRange * 2 / 3, iRange);
      }
  // apply generators
  // do this check, be cause in 99% of all normal scenarios, there will be no
  // FoW-generators
...
void C4Player::FoWGenerators2Map(CClrModAddMap &rMap, int iOffX, int iOffY) {
  ...
  for (clnk = FoWViewObjs.First; clnk && (cobj = clnk->Obj); clnk = clnk->Next)
    if (!cobj->Contained || cobj->Contained->Def->ClosedContainer != 1)
      if (cobj->PlrViewRange < 0)
        rMap.AddModulation(cobj->x + iOffX, cobj->y + iOffY,
                           -cobj->PlrViewRange, -cobj->PlrViewRange + 200,
                           ((uint32_t)cobj->ColorMod) >> 24);
}
  ...
  for (;;) {
    ...
    if (!cobj->Contained || cobj->Contained->Def->ClosedContainer != 1)
      if (Distance(cobj->x, cobj->y, x, y) < Abs(iRange))
        if (iRange < 0) {
          if (!(cobj->ColorMod & 0xff000000))  // faded generators generate
                                               // darkness only; no FoW blocking
...
void C4Player::Eliminate() {
  ...
  if (Game.Control.isCtrlHost() && AtClient > C4ClientIDHost) {
    ...
    for (int i = 0; pPlr = Game.Players.GetAtClient(AtClient, i); i++)
      if (!pPlr->Eliminated) break;
    // If not, deactivate the client
    if (!pPlr)
      Game.Control.DoInput(
          CID_ClientUpdate,
          new C4ControlClientUpdate(AtClient, CUT_Activate, false), CDT_Sync);
  }
}
...
void C4Player::EvaluateLeague(bool fDisconnected, bool fWon) {
  ...
  C4PlayerInfo *pInfo = GetInfo();
  if (pInfo) {
    if (fDisconnected) pInfo->SetDisconnected();
    if (fWon) pInfo->SetWinner();
  }
}
...
BOOL C4Player::SetObjectCrewStatus(C4Object *pCrew, BOOL fNewStatus) {
  ...
  if (fNewStatus) {
    ...
    // but then they would be reassigned to the player crew when loaded in a
    // savegame
    //  by the crew-assignment code kept for backwards compatibility with
    //  pre-4.95.2-savegames
...
void C4Player::CreateGraphs() {
  ...
  if (Game.pNetworkStatistics) {
    ...
    C4GUI::MakeColorReadableOnBlack(dwGraphClr);
    dwGraphClr &= 0xffffff;
    pstatControls = new C4TableGraph(C4TableGraph::DefaultBlockLength * 20,
                                     Game.pNetworkStatistics->ControlCounter);
    pstatControls->SetColorDw(dwGraphClr);
    pstatControls->SetTitle(GetName());
    pstatActions = new C4TableGraph(C4TableGraph::DefaultBlockLength * 20,
                                    Game.pNetworkStatistics->ControlCounter);
...
void C4Player::CountControl(ControlType eType, int32_t iID, int32_t iCntAdd) {
  ...
  if (Cursor && Cursor->Info) {
    if (Cursor->Info) {
      Cursor->Info->ControlCount++;
      if ((Cursor->Info->ControlCount % 5) == 0) Cursor->DoExperience(+1);
    }
  }
}
...
void C4Player::ExecMsgBoardQueries() {
  ...
  if (!pCheck) return;
  // open it
  Game.MessageInput.StartTypeIn(true, pCheck->pCallbackObj,
                                pCheck->fIsUppercase, false, Number,
                                pCheck->sInputQuery);
}
...
bool C4Player::RemoveMessageBoardQuery(C4Object *pForObj) {
  ...
  while (*ppCheck)
    if ((*ppCheck)->pCallbackObj == pForObj)
      break;
    else
...
bool C4Player::MarkMessageBoardQueryAnswered(C4Object *pForObj) {
  ...
  C4MessageBoardQuery *pCheck = pMsgBoardQuery;
  while (pCheck)
    if (pCheck->pCallbackObj == pForObj && !pCheck->fAnswered)
      break;
    else
      pCheck = pCheck->pNext;
...
void C4Player::OnTeamSelectionFailed() {
  // looks like a selected team was not available: Go back to team selection if
  // this is not a mislead call
  if (Status == PS_TeamSelectionPending) Status = PS_TeamSelection;
}
...
void C4Player::SetPlayerColor(uint32_t dwNewClr) {
  ...
  // this can never catch everything (thinking of overlays, etc.); scenarios
  // that allow team changes should take care of the rest
...
void C4Player::SyncHomebaseMaterialToTeam() {
  ...
  if (Team && ((pTeam = Game.Teams.GetTeamByID(Team)))) {
    ...
    for (int32_t i = 0; i < iTeamSize; ++i) {
      ...
      C4Player *pTeammate;
      if (iTeammate != ID &&
          ((pTeammate = Game.Players.GetByInfoID(iTeammate)))) {
...
void C4Player::SyncHomebaseMaterialFromTeam() {
  // Copy own home base material from team's first player (unless it's
  // ourselves)
  C4Team *pTeam;
  if (Team && ((pTeam = Game.Teams.GetTeamByID(Team)))) {
    ...
    if (iTeamCaptain != ID &&
        ((pTeamCaptain = Game.Players.GetByInfoID(iTeamCaptain)))) {
