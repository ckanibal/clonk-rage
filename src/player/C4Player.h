...
const int32_t C4PVM_Cursor = 0, C4PVM_Target = 1, C4PVM_Scrolling = 2;

const int32_t C4P_Number_None = -5;

const int32_t C4MaxPlayer = 5000;  // ought to be enough for everybody (used to
                                   // catch invalid player counts)
const int32_t C4MaxClient = 5000;  // ought to be enough for everybody (used to
                                   // catch invalid client counts)
...
class C4Player : public C4PlayerInfoCore {
 public:
  ...
  enum StatusTypes {
    ...
    PS_TeamSelectionPending =
        3,  // waiting for team selection packet to come through
  };
 ...
 public:
  ...
  bool NoEliminationCheck;  // Local-NoSave. Always set in init and restore by
                            // info //
  // Menu
  ...
  int32_t LastControlID;  // last control to capture perma-pressers in stats
 ...
 public:
  ...
  BOOL ObjectCommand(int32_t iCommand, C4Object *pTarget, int32_t iTx,
                     int32_t iTy, C4Object *pTarget2 = NULL, int32_t iData = 0,
                     int32_t iAddMode = C4P_Command_Set);
  void ObjectCommand2Obj(C4Object *cObj, int32_t iCommand, C4Object *pTarget,
                         int32_t iX, int32_t iY, C4Object *pTarget2,
                         int32_t iData, int32_t iMode);
  BOOL DoPoints(int32_t iChange);
  BOOL Init(int32_t iNumber, int32_t iAtClient, const char *szAtClientName,
  ...
  BOOL MakeCrewMember(C4Object *pObj, bool fForceInfo = true,
                      bool fDoCalls = true);
  ...
  C4Object *Buy(C4ID id, BOOL fShowErrors, int32_t iForPlr,
                C4Object *pBuyObj = NULL);  // buy object of player's wealth
  BOOL Sell2Home(C4Object *tobj);
  BOOL DoWealth(int32_t change);
  BOOL SetHostility(int32_t iOpponent, int32_t iHostility,
                    BOOL fSilent = FALSE);
  ...
  BOOL SetObjectCrewStatus(C4Object *pCrew,
                           BOOL fNewStatus);  // add/remove object from crew
  bool IsChosingTeam() const {
    return Status == PS_TeamSelection || Status == PS_TeamSelectionPending;
  }
 ...
 protected:
  ...
  void PlaceReadyVehic(int32_t tx1, int32_t tx2, int32_t ty,
                       C4Object *FirstBase);
  void PlaceReadyMaterial(int32_t tx1, int32_t tx2, int32_t ty,
                          C4Object *FirstBase);
 ...
 public:
  void SetTeamHostility();  // if Team!=0: Set hostile to all players in other
                            // teams and allied to all others (both ways)
  ...
  bool FoWIsVisible(
      int32_t x,
      int32_t y);  // check whether a point in the landscape is visible
 ...
 public:
  ...
  void CallMessageBoard(C4Object *pForObj, const StdStrBuf &sQueryString,
                        bool fUppercase);
  bool RemoveMessageBoardQuery(C4Object *pForObj);
  bool MarkMessageBoardQueryAnswered(C4Object *pForObj);
  bool
  HasMessageBoardQuery();  // return whether any object has a messageboard-query

  // callback by script execution of team selection: Restart team selection if
  // the team turned out to be not available
  void OnTeamSelectionFailed();

  // when the player changes team, his color changes. Relfect this in player
  // objects
