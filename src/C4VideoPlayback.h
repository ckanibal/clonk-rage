...
class C4VideoFile {
 private:
  ...
  bool fIsTemp;         // if set, the file is temporary will be deleted upon
 ...
 public:
  ...
  bool Load(class C4Group &hGrp, const char *szFilename);  // create from group:
                                                           // Extract to temp
                                                           // file if group is
                                                           // packed
...
class C4VideoShowDialog : public C4GUI::FullscreenDialog {
 ...
 public:
  C4VideoShowDialog()
      : C4GUI::FullscreenDialog(NULL, NULL)
#ifdef _WIN32
        ,
        pAudioTrack(NULL)
#endif
