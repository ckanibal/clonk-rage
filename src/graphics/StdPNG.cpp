...
bool CPNGFile::DoLoad() {
  ...
  png_get_IHDR(png_ptr, info_ptr, &uWdt, &uHgt, &iBPC, &iClrType, &iIntrlcType,
               &iCmprType, &iFltrType);
  // convert to bgra
  ...
  if (iClrType == PNG_COLOR_TYPE_GRAY || iClrType == PNG_COLOR_TYPE_GRAY_ALPHA)
    png_set_gray_to_rgb(png_ptr);
  /*if (iClrType == PNG_COLOR_TYPE_RGB || iClrType == PNG_COLOR_TYPE_RGB_ALPHA)*/ png_set_bgr(
      png_ptr);
  // update info
  ...
  switch (iClrType) {
    ...
    case PNG_COLOR_TYPE_RGB_ALPHA:
      ...
      break;
    default:
      return false;  // unrecognized image
  }
...
bool CPNGFile::Save(const char *szFilename) {
  ...
  // set header
  png_set_IHDR(png_ptr, info_ptr, iWdt, iHgt, iBPC, iClrType, iIntrlcType,
               iCmprType, iFltrType);
