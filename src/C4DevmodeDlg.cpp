...
void C4DevmodeDlg::AddPage(GtkWidget* widget, GtkWindow* parent,
                           const char* title) {
...
void C4DevmodeDlg::SwitchPage(GtkWidget* widget) {
  ...
  if (widget != NULL) {
    ...
    gtk_window_set_title(
        GTK_WINDOW(window),
        gtk_notebook_get_tab_label_text(GTK_NOTEBOOK(notebook), widget));

    // Show window if not visible
    if (!is_visible) {
      gtk_widget_show(window);
      if (x != -1 || y != -1) gtk_window_move(GTK_WINDOW(window), x, y);
    }
  } else {
    if (window != NULL && is_visible) gtk_widget_hide(window);
  }
}
