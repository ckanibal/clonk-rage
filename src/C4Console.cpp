  ...
  switch (Msg) {
    ...
    case WM_COMMAND:
      ...
      switch (LOWORD(wParam)) {
        ...
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_FILE_SAVEGAMEAS:
          Console.FileSaveAs(TRUE);
          return TRUE;
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_FILE_SAVEGAME:
        ...
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_FILE_RECORD:
          Console.FileRecord();
          return TRUE;
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_FILE_OPENWPLRS:
        ...
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_PLAYER_JOIN:
          Console.PlayerJoin();
          return TRUE;
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_VIEWPORT_NEW:
          Console.ViewportNew();
          return TRUE;
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_COMPONENTS_TITLE:
          Console.EditTitle();
          return TRUE;
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_COMPONENTS_INFO:
          Console.EditInfo();
          return TRUE;
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - -
        case IDM_COMPONENTS_SCRIPT:
          Console.EditScript();
          return TRUE;
      }
      // New player viewport
      ...
      if (Inside((int)LOWORD(wParam), IDM_NET_CLIENT1, IDM_NET_CLIENT2)) {
        ...
        Game.Clients.CtrlRemove(
            Game.Clients.getClientByID(LOWORD(wParam) - IDM_NET_CLIENT1),
            LoadResStr("IDS_MSG_KICKBYMENU"));
        return TRUE;
      }
      return FALSE;
    //------------------------------------------------------------------------------------------------------------
...
// Create dialog window
#ifdef _WIN32
  hWindow = CreateDialog(pApp->hInstance, MAKEINTRESOURCE(IDD_CONSOLE), NULL,
                         ConsoleDlgProc);
  if (!hWindow) {
    char* lpMsgBuf;
  ...
  SendMessage(hWindow, WM_SETICON, ICON_BIG,
              (LPARAM)LoadIcon(pApp->hInstance, MAKEINTRESOURCE(IDI_00_C4X)));
  ...
  hbmCursor2 =
      (HBITMAP)LoadBitmap(pApp->hInstance, MAKEINTRESOURCE(IDB_CURSOR2));
  ...
  // Calls InitGUI
  CStdWindow* retval =
      C4ConsoleBase::Init(pApp, LoadResStr("IDS_CNS_CONSOLE"), NULL, false);
  UpdateHaltCtrls(TRUE);
  EnableControls(fGameOpen);
  ClearViewportMenu();
  return retval;
...
GtkWidget* C4Console::InitGUI() {
  ...
  GtkWidget* itemFile =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_FILE"));
  GtkWidget* itemComponents =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_COMPONENTS"));
  GtkWidget* itemPlayer =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_PLAYER"));
  GtkWidget* itemViewport =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_VIEWPORT"));
  ...
  fileOpenWithPlayers =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_OPENWPLRS"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menuFile), fileOpenWithPlayers);

  gtk_menu_shell_append(GTK_MENU_SHELL(menuFile),
                        GTK_WIDGET(gtk_separator_menu_item_new()));

  fileSave =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_SAVESCENARIO"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menuFile), fileSave);

  fileSaveAs =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_SAVESCENARIOAS"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menuFile), fileSaveAs);

  fileSaveGame =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_SAVEGAME"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menuFile), fileSaveGame);

  fileSaveGameAs =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_MNU_SAVEGAMEAS"));
  ...
  compObjects =
      gtk_menu_item_new_with_label(LoadResStrUtf8I("IDS_BTN_OBJECTS"));
  ...
  // ------------ Signals ---------------------
  handlerPlay =
      g_signal_connect(G_OBJECT(btnPlay), "toggled", G_CALLBACK(OnPlay), this);
  handlerHalt =
      g_signal_connect(G_OBJECT(btnHalt), "toggled", G_CALLBACK(OnHalt), this);
  handlerModePlay = g_signal_connect(G_OBJECT(btnModePlay), "toggled",
                                     G_CALLBACK(OnModePlay), this);
  handlerModeEdit = g_signal_connect(G_OBJECT(btnModeEdit), "toggled",
                                     G_CALLBACK(OnModeEdit), this);
  handlerModeDraw = g_signal_connect(G_OBJECT(btnModeDraw), "toggled",
                                     G_CALLBACK(OnModeDraw), this);
  g_signal_connect(G_OBJECT(txtScript), "activate", G_CALLBACK(OnScriptEntry),
                   this);
  g_signal_connect(G_OBJECT(fileOpen), "activate", G_CALLBACK(OnFileOpen),
                   this);
  g_signal_connect(G_OBJECT(fileOpenWithPlayers), "activate",
                   G_CALLBACK(OnFileOpenWPlrs), this);
  g_signal_connect(G_OBJECT(fileSave), "activate", G_CALLBACK(OnFileSave),
                   this);
  g_signal_connect(G_OBJECT(fileSaveAs), "activate", G_CALLBACK(OnFileSaveAs),
                   this);
  g_signal_connect(G_OBJECT(fileSaveGame), "activate",
                   G_CALLBACK(OnFileSaveGame), this);
  g_signal_connect(G_OBJECT(fileSaveGameAs), "activate",
                   G_CALLBACK(OnFileSaveGameAs), this);
  g_signal_connect(G_OBJECT(fileRecord), "activate", G_CALLBACK(OnFileRecord),
                   this);
  g_signal_connect(G_OBJECT(fileClose), "activate", G_CALLBACK(OnFileClose),
                   this);
  g_signal_connect(G_OBJECT(fileQuit), "activate", G_CALLBACK(OnFileQuit),
                   this);
  g_signal_connect(G_OBJECT(compObjects), "activate", G_CALLBACK(OnCompObjects),
                   this);
  g_signal_connect(G_OBJECT(compScript), "activate", G_CALLBACK(OnCompScript),
                   this);
  g_signal_connect(G_OBJECT(compTitle), "activate", G_CALLBACK(OnCompTitle),
                   this);
  g_signal_connect(G_OBJECT(compInfo), "activate", G_CALLBACK(OnCompInfo),
                   this);
  ...
  g_signal_connect(G_OBJECT(helpAbout), "activate", G_CALLBACK(OnHelpAbout),
                   this);
...
bool C4Console::In(const char* szText) {
  ...
  // begins with '#'? then it's a message. Route cia ProcessInput to allow
  // #/sound
  if (*szText == '#') {
  ...
  Game.Control.DoInput(
      CID_Script,
      new C4ControlScript(szText, C4ControlScript::SCOPE_Console, false),
      CDT_Decide);
  return TRUE;
}
...
bool C4Console::Out(const char* szText) {
  ...
  char* buffer, *buffer2;
  len =
      65000;  // SendDlgItemMessage(hWindow,IDC_EDITOUTPUT,EM_LINELENGTH,(WPARAM)0,(LPARAM)0);
  ...
  if (strlen(buffer) > 60000)
    buffer2 = buffer + strlen(buffer) - 60000;
  else
    buffer2 = buffer;  // max log length: Otherwise, discard beginning
  SetDlgItemText(hWindow, IDC_EDITOUTPUT, buffer2);
  delete[] buffer;
  lines = SendDlgItemMessage(hWindow, IDC_EDITOUTPUT, EM_GETLINECOUNT,
                             (WPARAM)0, (LPARAM)0);
  SendDlgItemMessage(hWindow, IDC_EDITOUTPUT, EM_LINESCROLL, (WPARAM)0,
                     (LPARAM)lines);
  UpdateWindow(hWindow);
#elif WITH_DEVELOPER_MODE
  // Append text to log
  ...
  gtk_text_buffer_insert(buffer, &end, C4Language::IconvUtf8(szText).getData(),
                         -1);
  gtk_text_buffer_insert(buffer, &end, "\n", 1);

  gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(txtLog),
                               gtk_text_buffer_get_insert(buffer), 0.0, FALSE,
                               0.0, 0.0);
...
bool C4Console::ClearLog() {
  ...
  UpdateWindow(hWindow);
#elif WITH_DEVELOPER_MODE
  gtk_text_buffer_set_text(gtk_text_view_get_buffer(GTK_TEXT_VIEW(txtLog)), "",
                           0);
#endif  // WITH_DEVELOPER_MODE / _WIN32
  return true;
}
...
void C4Console::DoPlay() { Game.Unpause(); }

void C4Console::DoHalt() { Game.Pause(); }
...
bool C4Console::UpdateStatusBars() {
  ...
  if ((Game.Time != Time) || (Game.FPS != FPS)) {
    Time = Game.Time;
    FPS = Game.FPS;
    sprintf(OSTR, "%02d:%02d:%02d (%i FPS)", Time / 3600, (Time % 3600) / 60,
            Time % 60, FPS);
#ifdef _WIN32
...
BOOL C4Console::SaveGame(BOOL fSaveGame) {
  ...
  if (Game.Network.isEnabled() && !Game.Network.isHost()) {
    Message(LoadResStr("IDS_GAME_NOCLIENTSAVE"));
    return FALSE;
  }
  ...
  if (fSaveGame)
    ...
    pGameSave = new C4GameSaveScenario(
        !Console.Active || Game.Landscape.Mode == C4LSC_Exact, false);
  if (!pGameSave->Save(Game.ScenarioFile, false)) {
    Out("Game::Save failed");
    fOkay = FALSE;
  }
  delete pGameSave;
  ...
  if (!Game.ScenarioFile.Close()) {
    Out("ScenarioFile::Close failed");
    fOkay = FALSE;
  }
  if (!Game.ScenarioFile.Open(Game.ScenarioFilename)) {
    Out("ScenarioFile::Open failed");
    fOkay = FALSE;
  }
  ...
  // Status report
  if (!fOkay)
    Message(LoadResStr("IDS_CNS_SAVERROR"));
  else
    Out(LoadResStr(fSaveGame ? "IDS_CNS_GAMESAVED" : "IDS_CNS_SCENARIOSAVED"));
...
BOOL C4Console::FileSaveAs(BOOL fSaveGame) {
  ...
  if (!FileSelect(filename, 512, "Clonk 4 Scenario\0*.c4s\0\0",
                  OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY, TRUE))
    return FALSE;
...
bool C4Console::Message(const char* szMessage, bool fQuery) {
  if (!Active) return FALSE;
#ifdef _WIN32
  return (IDOK == MessageBox(hWindow, szMessage, C4ENGINECAPTION,
                             fQuery ? (MB_OKCANCEL | MB_ICONEXCLAMATION)
                                    : MB_ICONEXCLAMATION));
#elif WITH_DEVELOPER_MODE
  GtkWidget* dialog = gtk_message_dialog_new(
      GTK_WINDOW(window), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO,
      fQuery ? (GTK_BUTTONS_OK_CANCEL) : (GTK_BUTTONS_OK), "%s", szMessage);
...
void C4Console::EnableControls(bool fEnable) {
  ...
  SendDlgItemMessage(hWindow, IDC_BUTTONMODEPLAY, BM_SETIMAGE, IMAGE_BITMAP,
                     (LPARAM)(fEnable ? hbmMouse : hbmMouse2));
  SendDlgItemMessage(hWindow, IDC_BUTTONMODEEDIT, BM_SETIMAGE, IMAGE_BITMAP,
                     (LPARAM)((fEnable && Editing) ? hbmCursor : hbmCursor2));
  SendDlgItemMessage(hWindow, IDC_BUTTONMODEDRAW, BM_SETIMAGE, IMAGE_BITMAP,
                     (LPARAM)((fEnable && Editing) ? hbmBrush : hbmBrush2));
  SendDlgItemMessage(
      hWindow, IDC_BUTTONPLAY, BM_SETIMAGE, IMAGE_BITMAP,
      (LPARAM)(Game.Network.isLobbyActive() || fEnable ? hbmPlay : hbmPlay2));
  SendDlgItemMessage(
      hWindow, IDC_BUTTONHALT, BM_SETIMAGE, IMAGE_BITMAP,
      (LPARAM)(Game.Network.isLobbyActive() || fEnable ? hbmHalt : hbmHalt2));
  ...
  EnableWindow(GetDlgItem(hWindow, IDC_BUTTONPLAY),
               Game.Network.isLobbyActive() || fEnable);
  EnableWindow(GetDlgItem(hWindow, IDC_BUTTONHALT),
               Game.Network.isLobbyActive() || fEnable);
  ...
  EnableMenuItem(GetMenu(hWindow), IDM_FILE_OPENWPLRS,
                 MF_BYCOMMAND | MF_ENABLED);
  EnableMenuItem(
      GetMenu(hWindow), IDM_FILE_RECORD,
      MF_BYCOMMAND | ((Game.IsRunning && Game.Control.IsRuntimeRecordPossible())
                          ? MF_ENABLED
                          : MF_GRAYED));
  EnableMenuItem(
      GetMenu(hWindow), IDM_FILE_SAVEGAME,
      MF_BYCOMMAND |
          ((fEnable && Game.Players.GetCount()) ? MF_ENABLED : MF_GRAYED));
  EnableMenuItem(
      GetMenu(hWindow), IDM_FILE_SAVEGAMEAS,
      MF_BYCOMMAND |
          ((fEnable && Game.Players.GetCount()) ? MF_ENABLED : MF_GRAYED));
  EnableMenuItem(GetMenu(hWindow), IDM_FILE_SAVE,
                 MF_BYCOMMAND | (fEnable ? MF_ENABLED : MF_GRAYED));
  EnableMenuItem(GetMenu(hWindow), IDM_FILE_SAVEAS,
                 MF_BYCOMMAND | (fEnable ? MF_ENABLED : MF_GRAYED));
  EnableMenuItem(GetMenu(hWindow), IDM_FILE_CLOSE,
                 MF_BYCOMMAND | (fEnable ? MF_ENABLED : MF_GRAYED));

  // Components menu
  EnableMenuItem(
      GetMenu(hWindow), IDM_COMPONENTS_SCRIPT,
      MF_BYCOMMAND | ((fEnable && Editing) ? MF_ENABLED : MF_GRAYED));
  EnableMenuItem(
      GetMenu(hWindow), IDM_COMPONENTS_INFO,
      MF_BYCOMMAND | ((fEnable && Editing) ? MF_ENABLED : MF_GRAYED));
  EnableMenuItem(
      GetMenu(hWindow), IDM_COMPONENTS_TITLE,
      MF_BYCOMMAND | ((fEnable && Editing) ? MF_ENABLED : MF_GRAYED));

  // Player & viewport menu
  EnableMenuItem(
      GetMenu(hWindow), IDM_PLAYER_JOIN,
      MF_BYCOMMAND | ((fEnable && Editing) ? MF_ENABLED : MF_GRAYED));
  EnableMenuItem(GetMenu(hWindow), IDM_VIEWPORT_NEW,
                 MF_BYCOMMAND | (fEnable ? MF_ENABLED : MF_GRAYED));
#elif WITH_DEVELOPER_MODE
  // Halt controls
  ...
  // Edit modes
  gtk_widget_set_sensitive(btnModePlay,
                           Game.Network.isLobbyActive() || fEnable);
  gtk_widget_set_sensitive(btnModeEdit,
                           Game.Network.isLobbyActive() || fEnable);
  gtk_widget_set_sensitive(btnModeDraw,
                           Game.Network.isLobbyActive() || fEnable);
  ...
  gtk_widget_set_sensitive(
      fileRecord, Game.IsRunning && Game.Control.IsRuntimeRecordPossible());
...
BOOL C4Console::FileOpen() {
  ...
  char cmdline[2000] = "";
  SAppend("\"", cmdline, 1999);
  SAppend(c4sfile, cmdline, 1999);
  SAppend("\" ", cmdline, 1999);
  // Open game
  OpenGame(cmdline);
  return TRUE;
...
BOOL C4Console::FileOpenWPlrs() {
  // Get scenario file name
  char c4sfile[512 + 1] = "";
  if (!FileSelect(c4sfile, 512, "Clonk 4 Scenario\0*.c4s;*.c4f\0\0",
                  OFN_HIDEREADONLY | OFN_FILEMUSTEXIST))
    return FALSE;
  // Get player file name(s)
  char c4pfile[4096 + 1] = "";
  if (!FileSelect(c4pfile, 4096, "Clonk 4 Player\0*.c4p\0\0",
                  OFN_HIDEREADONLY | OFN_ALLOWMULTISELECT | OFN_EXPLORER))
    return FALSE;
  // Compose command line
  char cmdline[6000] = "";
  SAppend("\"", cmdline, 5999);
  SAppend(c4sfile, cmdline, 5999);
  SAppend("\" ", cmdline, 5999);
  if (DirectoryExists(c4pfile))  // Multiplayer
  {
    ...
    while (*cptr) {
      SAppend("\"", cmdline, 5999);
      SAppend(c4pfile, cmdline, 5999);
      SAppend(DirSep, cmdline, 5999);
      SAppend(cptr, cmdline, 5999);
      SAppend("\" ", cmdline, 5999);
      cptr += SLen(cptr) + 1;
    }
  } else  // Single player
...
BOOL C4Console::FileSelect(char* sFilename, int iSize, const char* szFilter,
  ...
  // Reset working directory to exe path as Windows file dialog might have
  // changed it
  SetCurrentDirectory(Config.General.ExePath);
  return fResult;
#elif WITH_DEVELOPER_MODE
  GtkWidget* dialog = gtk_file_chooser_dialog_new(
      fSave ? "Save file..." : "Load file...", GTK_WINDOW(window),
      fSave ? GTK_FILE_CHOOSER_ACTION_SAVE : GTK_FILE_CHOOSER_ACTION_OPEN,
      GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
      fSave ? GTK_STOCK_SAVE : GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
  ...
  // TODO: Not in GTK+ 2.4, we could check GTK+ version at runtime and rely on
  // lazy bindung
  //	gtk_file_chooser_set_do_overwrite_confirmation(GTK_FILE_CHOOSER(dialog),
  //(dwFlags & OFN_OVERWRITEPROMPT) != 0);

  // TODO: Not in GTK+ 2.4, we could check GTK+ version at runtime and rely on
  // lazy binding
  //	gtk_file_chooser_set_show_hidden(GTK_FILE_CHOOSER(dialog), (dwFlags &
  //OFN_HIDEREADONLY) == 0);

  gtk_file_chooser_set_select_multiple(GTK_FILE_CHOOSER(dialog),
                                       (dwFlags & OFN_ALLOWMULTISELECT) != 0);
...
void C4Console::Close() { Application.Quit(); }
...
void C4Console::HelpAbout() {
  ...
  strMessage.Format("%s %s\n\n%s", C4ENGINECAPTION, C4VERSION,
                    strCopyright.getData());
  MessageBox(NULL, strMessage.getData(), C4ENGINECAPTION,
             MB_ICONINFORMATION | MB_TASKMODAL);
#elif WITH_DEVELOPER_MODE
  gtk_show_about_dialog(GTK_WINDOW(window), "name", C4ENGINECAPTION, "version",
                        C4VERSION, "copyright", strCopyright.getData(), NULL);
#endif  // WITH_DEVELOPER_MODE / _WIN32
}

void C4Console::ViewportNew() { Game.CreateViewport(NO_OWNER); }
...
#elif WITH_DEVELOPER_MODE
  gtk_label_set_label(GTK_LABEL(lblCursor),
                      Languages.IconvUtf8(szCursor).getData());
#endif
  return TRUE;
}
...
bool C4Console::UpdateViewportMenu() {
  ...
  for (C4Player* pPlr = Game.Players.First; pPlr; pPlr = pPlr->Next) {
    ...
    GtkWidget* menuItem = gtk_menu_item_new_with_label(
        Languages.IconvUtf8(sText.getData()).getData());
    gtk_menu_shell_append(GTK_MENU_SHELL(menuViewport), menuItem);
    g_signal_connect(G_OBJECT(menuItem), "activate", G_CALLBACK(OnViewNewPlr),
                     GINT_TO_POINTER(pPlr->Number));
...
bool C4Console::UpdateModeCtrls(int iMode) {
...
#ifdef _WIN32
  SendDlgItemMessage(hWindow, IDC_BUTTONMODEPLAY, BM_SETSTATE,
                     (iMode == C4CNS_ModePlay), 0);
  UpdateWindow(GetDlgItem(hWindow, IDC_BUTTONMODEPLAY));
  SendDlgItemMessage(hWindow, IDC_BUTTONMODEEDIT, BM_SETSTATE,
                     (iMode == C4CNS_ModeEdit), 0);
  UpdateWindow(GetDlgItem(hWindow, IDC_BUTTONMODEEDIT));
  SendDlgItemMessage(hWindow, IDC_BUTTONMODEDRAW, BM_SETSTATE,
                     (iMode == C4CNS_ModeDraw), 0);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(btnModePlay),
                               iMode == C4CNS_ModePlay);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(btnModeEdit),
                               iMode == C4CNS_ModeEdit);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(btnModeDraw),
                               iMode == C4CNS_ModeDraw);
...
void C4Console::EditObjects() { ObjectListDlg.Open(); }
...
void C4Console::UpdateInputCtrl() {
  ...
  GtkEntryCompletion* completion =
      gtk_entry_get_completion(GTK_ENTRY(txtScript));
  if (!completion) {
...
#ifdef _WIN32
    AddMenuItem(
        hMenu, IDM_PLAYER_QUIT1 + pPlr->Number, sText.getData(),
        (!Game.Network.isEnabled() || Game.Network.isHost()) && Editing);
#elif WITH_DEVELOPER_MODE
    // TODO: Implement AddMenuItem...
    GtkWidget* menuItem = gtk_menu_item_new_with_label(
        Languages.IconvUtf8(sText.getData()).getData());
    gtk_menu_shell_append(GTK_MENU_SHELL(menuPlayer), menuItem);
    g_signal_connect(G_OBJECT(menuItem), "activate", G_CALLBACK(OnPlrQuit),
                     GINT_TO_POINTER(pPlr->Number));
    gtk_widget_show(menuItem);
...
void C4Console::PlayerJoin() {
  ...
  if (DirectoryExists(c4pfile)) {
    ...
    while (*cptr) {
      ...
      SAppend(DirSep, c4plist);
      SAppend(cptr, c4plist);
      cptr += SLen(cptr) + 1;
    }
  }
  // Single player
  ...
  for (int iPar = 0;
       SCopySegment(c4plist, iPar, szPlayerFilename, ';', _MAX_PATH); iPar++)
    if (szPlayerFilename[0])
      if (Game.Network.isEnabled())
        Game.Network.Players.JoinLocalPlayer(szPlayerFilename, true);
      else
        Game.Players.CtrlJoinLocalNoNetwork(szPlayerFilename,
                                            Game.Clients.getLocalID(),
                                            Game.Clients.getLocalName());
}
...
void C4Console::UpdateMenuText(HMENU hMenu) {
  ...
  if (!Active) return;
  // File
  ModifyMenu(hMenu, MenuIndexFile, MF_BYPOSITION | MF_STRING, 0,
             LoadResStr("IDS_MNU_FILE"));
  hSubMenu = GetSubMenu(hMenu, MenuIndexFile);
  SetMenuItemText(hSubMenu, IDM_FILE_OPEN, LoadResStr("IDS_MNU_OPEN"));
  SetMenuItemText(hSubMenu, IDM_FILE_OPENWPLRS,
                  LoadResStr("IDS_MNU_OPENWPLRS"));
  ...
  SetMenuItemText(hSubMenu, IDM_FILE_SAVEAS,
                  LoadResStr("IDS_MNU_SAVESCENARIOAS"));
  SetMenuItemText(hSubMenu, IDM_FILE_SAVEGAME, LoadResStr("IDS_MNU_SAVEGAME"));
  SetMenuItemText(hSubMenu, IDM_FILE_SAVEGAMEAS,
                  LoadResStr("IDS_MNU_SAVEGAMEAS"));
  ...
  // Components
  ModifyMenu(hMenu, MenuIndexComponents, MF_BYPOSITION | MF_STRING, 0,
             LoadResStr("IDS_MNU_COMPONENTS"));
  hSubMenu = GetSubMenu(hMenu, MenuIndexComponents);
  SetMenuItemText(hSubMenu, IDM_COMPONENTS_SCRIPT,
                  LoadResStr("IDS_MNU_SCRIPT"));
  ...
  // Player
  ModifyMenu(hMenu, MenuIndexPlayer, MF_BYPOSITION | MF_STRING, 0,
             LoadResStr("IDS_MNU_PLAYER"));
  hSubMenu = GetSubMenu(hMenu, MenuIndexPlayer);
  SetMenuItemText(hSubMenu, IDM_PLAYER_JOIN, LoadResStr("IDS_MNU_JOIN"));
  // Viewport
  ModifyMenu(hMenu, MenuIndexViewport, MF_BYPOSITION | MF_STRING, 0,
             LoadResStr("IDS_MNU_VIEWPORT"));
...
// Insert menu
#ifdef _WIN32
  if (!InsertMenu(GetMenu(hWindow), MenuIndexHelp, MF_BYPOSITION | MF_POPUP,
                  (UINT)CreateMenu(), LoadResStr("IDS_MNU_NET")))
    return;
#elif WITH_DEVELOPER_MODE
...
// Host
#ifdef NETWORK
  sprintf(OSTR, LoadResStr("IDS_MNU_NETHOST"), Game.Clients.getLocalName(),
          Game.Clients.getLocalID());
#ifdef _WIN32
  ...
  g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(OnNetClient),
                   GINT_TO_POINTER(Game.Clients.getLocalID()));
#endif
  // Clients
  for (C4Network2Client* pClient = Game.Network.Clients.GetNextClient(NULL);
       pClient; pClient = Game.Network.Clients.GetNextClient(pClient)) {
    sprintf(OSTR, LoadResStr(pClient->isActivated() ? "IDS_MNU_NETCLIENT"
                                                    : "IDS_MNU_NETCLIENTDE"),
...
bool C4Console::OpenGame(const char* szCmdLine) {
  ...
  // Default game - only if open before, because we do not want to default out
  // the GUI
  if (fGameWasOpen) Game.Default();

  // Pretend to be called with a new commandline
  if (szCmdLine) Game.ParseCommandLine(szCmdLine);
...
bool C4Console::TogglePause() { return Game.TogglePause(); }
...
void C4Console::OnNetClient(GtkWidget* item, gpointer data) {
  ...
  Game.Clients.CtrlRemove(Game.Clients.getClientByID(GPOINTER_TO_INT(data)),
                          LoadResStr("IDS_MSG_KICKBYMENU"));
}
#endif  // WITH_DEVELOPER_MODE
