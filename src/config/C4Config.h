...
#define C4CFG_UpdateServer \
  "www.clonkx.de/rage/update"  // for download files, replace 'update' with the
                               // below
...
class C4ConfigGeneral {
 public:
  ...
  char Language[CFG_MaxString + 1];  // entered by user in frontend options (may
                                     // contain comma separated list or long
                                     // language descriptions)
  char LanguageEx[CFG_MaxString + 1];  // full fallback list composed by
                                       // frontend options (condensed comma
                                       // separated list)
  ...
  char UserPath[CFG_MaxString + 1];  // absolute path; environment variables are
                                     // stored and only expanded upon evaluation
  ...
  int32_t FairCrewStrength,
      MaxFairCrewStrength;    // strength of clonks in fair crew mode
  int32_t MouseAScroll;       // auto scroll strength
  int32_t ScrollSmooth;       // view movement smoothing
  int32_t ConfigResetSafety;  // safety value: If this value is screwed, the
                              // config got currupted and must be reset
  ...
  bool FirstStart;
  bool fUTF8;
  bool UserPortraitsWritten;  // set when default portraits have been copied to
                              // the UserPath (this is only done once)
 public:
...
class C4ConfigGraphics {
 public:
  ...
  int32_t
      FireParticles;  // draw extended fire particles if enabled (defualt on)
  int32_t MaxRefreshDelay;  // minimum time after which graphics should be
                            // refreshed (ms)
  int32_t AutoFrameSkip;    // if true, gfx frames are skipped when they would
                            // slow down the game
  void CompileFunc(StdCompiler *pComp);
};
...
class C4ConfigStartup {
 public:
  ...
  int32_t NoSplash;
  int32_t AlphabeticalSorting;  // if set, Folder.txt-sorting is ignored in
                                // scenario selection
...
class C4ConfigLobby {
 public:
  ...
  int32_t AllowPlayerSave;  // whether save-to-disk function is enabled for
                            // player ressources
  void CompileFunc(StdCompiler *pComp);
};
...
class C4ConfigSecurity {
 public:
  char KeyPath[CFG_MaxString + 1];  // absolute path; environment variables are
                                    // stored and only expanded upon evaluation
...
class C4Config : protected CStdConfig {
 ...
 public:
  ...
  StdStrBuf
      ConfigFilename;  // set for configs loaded from a nondefault config file
 public:
  ...
  const char *AtUserPath(const char *szFilename);  // this one will expand
                                                   // environment variables
                                                   // on-the-fly
  void ForceRelativePath(StdStrBuf *sFilename);  // try AtExeRelativePath; force
                                                 // GetC4Filename if not
                                                 // possible
  void CompileFunc(StdCompiler *pComp);
  bool IsCorrupted() {
    return (General.ConfigResetSafety !=
            C4ConfigGeneral::ConfigResetSafetyVal) ||
           !Graphics.ResX;
  }
