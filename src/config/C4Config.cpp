...
// For those without the ability to intuitively guess what the falses and trues
// mean:
// its mkNamingAdapt(field, name, default, fPrefillDefault, fStoreDefault)
// where fStoreDefault writes out the value to the config even if it's the same
// as the default.
#define s mkStringAdaptM
  ...
  pComp->Value(
      mkNamingAdapt(s(LanguageCharset), "LanguageCharset", "", false, true));
  ...
  pComp->Value(
      mkNamingAdapt(s(DefinitionPath), "DefinitionPath", "", false, true));
#ifdef _WIN32
  pComp->Value(mkNamingAdapt(s(UserPath), "UserPath", "%APPDATA%\\Clonk Rage",
                             false, true));
#elif defined(__linux__)
  pComp->Value(
      mkNamingAdapt(s(UserPath), "UserPath", "$HOME/.clonk/rage", false, true));
#elif defined(__APPLE__)
  pComp->Value(mkNamingAdapt(s(UserPath), "UserPath",
                             "$HOME/Library/Application Support/Clonk Rage",
                             false, true));
#endif
  pComp->Value(mkNamingAdapt(SaveGameFolder, "SaveGameFolder", "Savegames.c4f",
                             false, true));
  pComp->Value(mkNamingAdapt(SaveDemoFolder, "SaveDemoFolder", "Records.c4f",
                             false, true));
#ifdef C4ENGINE
  pComp->Value(
      mkNamingAdapt(s(MissionAccess), "MissionAccess", "", false, true));
#endif
  pComp->Value(mkNamingAdapt(FPS, "FPS", 0));
  pComp->Value(mkNamingAdapt(Record, "Record", 0));
  pComp->Value(mkNamingAdapt(DefRec, "DefRec", 0));
  pComp->Value(mkNamingAdapt(ScreenshotFolder, "ScreenshotFolder",
...
#endif
  pComp->Value(
      mkNamingAdapt(s(RXFontName), "FontName", "Endeavour", false, true));
  ...
  pComp->Value(
      mkNamingAdapt(UserPortraitsWritten, "UserPortraitsWritten", false));
  pComp->Value(mkNamingAdapt(ConfigResetSafety, "ConfigResetSafety",
                             static_cast<int32_t>(ConfigResetSafetyVal)));
}
...
void C4ConfigGraphics::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(
      mkNamingAdapt(ShowAllResolutions, "ShowAllResolutions", 0, false, true));
  pComp->Value(mkNamingAdapt(SplitscreenDividers, "SplitscreenDividers", 1));
  pComp->Value(mkNamingAdapt(ShowPlayerInfoAlways, "ShowPlayerInfoAlways", 1,
                             false, true));
  pComp->Value(mkNamingAdapt(ShowPortraits, "ShowPortraits", 1, false, true));
  pComp->Value(mkNamingAdapt(AddNewCrewPortraits, "AddNewCrewPortraits", 1,
                             false, true));
  pComp->Value(mkNamingAdapt(SaveDefaultPortraits, "SaveDefaultPortraits", 1,
                             false, true));
  pComp->Value(mkNamingAdapt(ShowCommands, "ShowCommands", 1, false, true));
  pComp->Value(
      mkNamingAdapt(ShowCommandKeys, "ShowCommandKeys", 1, false, true));
  pComp->Value(mkNamingAdapt(ShowStartupMessages, "ShowStartupMessages", 1,
                             false, true));
  ...
  pComp->Value(
      mkNamingAdapt(MenuTransparency, "MenuTransparency", 1, false, true));
...
void C4ConfigNetwork::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(mkNamingAdapt(MaxResSearchRecursion, "MaxResSearchRecursion", 1,
                             false, true));
  ...
  pComp->Value(mkNamingAdapt(PortDiscovery, "PortDiscovery",
                             C4NetStdPortDiscovery, false, true));
  pComp->Value(mkNamingAdapt(PortRefServer, "PortRefServer",
                             C4NetStdPortRefServer, false, true));
#endif
  ...
  pComp->Value(mkNamingAdapt(MaxLoadFileSize, "MaxLoadFileSize",
                             5 * 1024 * 1024, false, true));
  ...
  pComp->Value(
      mkNamingAdapt(MasterReferencePeriod, "MasterReferencePeriod", 120));
  pComp->Value(mkNamingAdapt(LeagueServerSignUp, "LeagueServerSignUp", 0));
  pComp->Value(
      mkNamingAdapt(s(ServerAddress), "ServerAddress", C4CFG_LeagueServer));
  pComp->Value(mkNamingAdapt(UseAlternateServer, "UseAlternateServer", 0));
  pComp->Value(mkNamingAdapt(s(AlternateServerAddress),
                             "AlternateServerAddress", C4CFG_LeagueServer));
  pComp->Value(mkNamingAdapt(s(UpdateServerAddress), "UpdateServerAddress",
                             C4CFG_UpdateServer));
  pComp->Value(mkNamingAdapt(s(LastPassword), "LastPassword", "Wipf"));
  pComp->Value(
      mkNamingAdapt(AutomaticUpdate, "AutomaticUpdate", 0, false, true));
  pComp->Value(mkNamingAdapt(LastUpdateTime, "LastUpdateTime", 0));
  pComp->Value(mkNamingAdapt(AsyncMaxWait, "AsyncMaxWait", 2));

  pComp->Value(
      mkNamingAdapt(s(PuncherAddress), "PuncherAddress",
                    "clonk.de:11115"));  // maybe store default for this one?
}

void C4ConfigLobby::CompileFunc(StdCompiler *pComp) {
  pComp->Value(
      mkNamingAdapt(AllowPlayerSave, "AllowPlayerSave", 0, false, false));
  pComp->Value(mkNamingAdapt(CountdownTime, "CountdownTime", 5, false, false));
}

void C4ConfigIRC::CompileFunc(StdCompiler *pComp) {
  pComp->Value(
      mkNamingAdapt(s(Server), "Server2", "irc.euirc.net", false, true));
  ...
  pComp->Value(
      mkNamingAdapt(AllowAllChannels, "AllowAllChannels", 0, false, true));
}
...
void C4ConfigExplorer::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(mkNamingAdapt(DontShowLanguageDisclaimer,
                             "DontShowLanguageDisclaimer", 0));
  pComp->Value(mkNamingAdapt(s(Definitions), "Definitions", ""));
  ...
  pComp->Value(
      mkNamingAdapt(s(EditorRichText), "EditorRichText", "Wordpad.exe"));
  ...
  pComp->Value(mkNamingAdapt(s(CommandLine), "CommandLine",
                             "Clonk.exe /console /nonetwork", false, true));
}

void C4ConfigSecurity::CompileFunc(StdCompiler *pComp) {
  pComp->Value(mkNamingAdapt(WasRegistered, "WasRegistered", 0));
#ifdef _WIN32
  pComp->Value(mkNamingAdapt(s(KeyPath), "KeyPath", "%APPDATA%\\Clonk Rage",
                             false, true));
#elif defined(__linux__)
  pComp->Value(
      mkNamingAdapt(s(KeyPath), "KeyPath", "$HOME/.clonk/rage", false, true));
#elif defined(__APPLE__)
  pComp->Value(mkNamingAdapt(s(KeyPath), "KeyPath",
                             "$HOME/Library/Application Support/Clonk Rage",
                             false, true));
#endif
}
...
void C4ConfigGamepad::CompileFunc(StdCompiler *pComp, bool fButtonsOnly) {
  /* The defaults here are for a Logitech Dual Action under Linux-SDL. Better
   * than nothing, I guess. */
  if (!fButtonsOnly) {
    for (int i = 0; i < 6; ++i) {
      pComp->Value(mkNamingAdapt(AxisMin[i],
                                 FormatString("Axis%dMin", i).getData(), 0u));
      pComp->Value(mkNamingAdapt(AxisMax[i],
                                 FormatString("Axis%dMax", i).getData(), 0u));
      pComp->Value(mkNamingAdapt(AxisCalibrated[i],
                                 FormatString("Axis%dCalibrated", i).getData(),
                                 false));
    }
  }
...
void C4ConfigGamepad::ResetButtons() {
  ...
  StdCompilerNull Comp;
  Comp.Compile(mkParAdapt(*this, true));
}

void C4ConfigGamepad::Reset() {
  // loads an empty config for the gamepad config
  StdCompilerNull Comp;
  Comp.Compile(mkParAdapt(*this, false));
}
...
#define KEY(win, x, sdl) sdl
  ...
  pComp->Value(
      mkNamingAdapt(Keyboard[0][0], "Kbd1Key1", KEY('Q', XK_q, SDLK_q)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][1], "Kbd1Key2", KEY('W', XK_w, SDLK_w)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][2], "Kbd1Key3", KEY('E', XK_e, SDLK_e)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][3], "Kbd1Key4", KEY('A', XK_a, SDLK_a)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][4], "Kbd1Key5", KEY('S', XK_s, SDLK_s)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][5], "Kbd1Key6", KEY('D', XK_d, SDLK_d)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][6], "Kbd1Key7",
                    fGer ? KEY('Y', XK_y, SDLK_y) : KEY('Z', XK_z, SDLK_z)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][7], "Kbd1Key8", KEY('X', XK_x, SDLK_x)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][8], "Kbd1Key9", KEY('C', XK_c, SDLK_c)));
  pComp->Value(mkNamingAdapt(
      Keyboard[0][9], "Kbd1Key10",
      fGer ? KEY(226, XK_less, SDLK_LESS) : KEY('R', XK_r, SDLK_r)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][10], "Kbd1Key11", KEY('V', XK_v, SDLK_v)));
  pComp->Value(
      mkNamingAdapt(Keyboard[0][11], "Kbd1Key12", KEY('F', XK_f, SDLK_f)));

  pComp->Value(mkNamingAdapt(Keyboard[1][0], "Kbd2Key1",
                             KEY(103, XK_KP_Home, SDLK_KP7)));
  pComp->Value(
      mkNamingAdapt(Keyboard[1][1], "Kbd2Key2", KEY(104, XK_KP_Up, SDLK_KP8)));
  pComp->Value(mkNamingAdapt(Keyboard[1][2], "Kbd2Key3",
                             KEY(105, XK_KP_Page_Up, SDLK_KP9)));
  pComp->Value(mkNamingAdapt(Keyboard[1][3], "Kbd2Key4",
                             KEY(100, XK_KP_Left, SDLK_KP4)));
  pComp->Value(mkNamingAdapt(Keyboard[1][4], "Kbd2Key5",
                             KEY(101, XK_KP_Begin, SDLK_KP5)));
  pComp->Value(mkNamingAdapt(Keyboard[1][5], "Kbd2Key6",
                             KEY(102, XK_KP_Right, SDLK_KP6)));
  pComp->Value(
      mkNamingAdapt(Keyboard[1][6], "Kbd2Key7", KEY(97, XK_KP_End, SDLK_KP1)));
  pComp->Value(
      mkNamingAdapt(Keyboard[1][7], "Kbd2Key8", KEY(98, XK_KP_Down, SDLK_KP2)));
  pComp->Value(mkNamingAdapt(Keyboard[1][8], "Kbd2Key9",
                             KEY(99, XK_KP_Page_Down, SDLK_KP3)));
  pComp->Value(mkNamingAdapt(Keyboard[1][9], "Kbd2Key10",
                             KEY(96, XK_KP_Insert, SDLK_KP0)));
  pComp->Value(mkNamingAdapt(Keyboard[1][10], "Kbd2Key11",
                             KEY(110, XK_KP_Delete, SDLK_KP_PERIOD)));
  pComp->Value(mkNamingAdapt(Keyboard[1][11], "Kbd2Key12",
                             KEY(107, XK_KP_Add, SDLK_KP_PLUS)));

  pComp->Value(
      mkNamingAdapt(Keyboard[2][0], "Kbd3Key1", KEY('I', XK_i, SDLK_i)));
  pComp->Value(
      mkNamingAdapt(Keyboard[2][1], "Kbd3Key2", KEY('O', XK_o, SDLK_o)));
  pComp->Value(
      mkNamingAdapt(Keyboard[2][2], "Kbd3Key3", KEY('P', XK_p, SDLK_p)));
  pComp->Value(
      mkNamingAdapt(Keyboard[2][3], "Kbd3Key4", KEY('K', XK_k, SDLK_k)));
  pComp->Value(
      mkNamingAdapt(Keyboard[2][4], "Kbd3Key5", KEY('L', XK_l, SDLK_l)));
  pComp->Value(mkNamingAdapt(Keyboard[2][5], "Kbd3Key6",
                             fGer ? KEY(192, XK_odiaeresis, SDLK_WORLD_4)
                                  : KEY(0xBA, XK_semicolon, SDLK_SEMICOLON)));
  pComp->Value(mkNamingAdapt(Keyboard[2][6], "Kbd3Key7",
                             KEY(188, XK_comma, SDLK_COMMA)));
  pComp->Value(mkNamingAdapt(Keyboard[2][7], "Kbd3Key8",
                             KEY(190, XK_period, SDLK_PERIOD)));
  pComp->Value(mkNamingAdapt(
      Keyboard[2][8], "Kbd3Key9",
      fGer ? KEY(189, XK_minus, SDLK_MINUS) : KEY(0xBF, XK_slash, SDLK_SLASH)));
  pComp->Value(
      mkNamingAdapt(Keyboard[2][9], "Kbd3Key10", KEY('M', XK_m, SDLK_m)));
  pComp->Value(mkNamingAdapt(Keyboard[2][10], "Kbd3Key11",
                             KEY(222, XK_adiaeresis, SDLK_WORLD_3)));
  pComp->Value(mkNamingAdapt(Keyboard[2][11], "Kbd3Key12",
                             KEY(186, XK_udiaeresis, SDLK_WORLD_2)));

  pComp->Value(mkNamingAdapt(Keyboard[3][0], "Kbd4Key1",
                             KEY(VK_INSERT, XK_Insert, SDLK_INSERT)));
  pComp->Value(mkNamingAdapt(Keyboard[3][1], "Kbd4Key2",
                             KEY(VK_HOME, XK_Home, SDLK_HOME)));
  pComp->Value(mkNamingAdapt(Keyboard[3][2], "Kbd4Key3",
                             KEY(VK_PRIOR, XK_Page_Up, SDLK_PAGEUP)));
  pComp->Value(mkNamingAdapt(Keyboard[3][3], "Kbd4Key4",
                             KEY(VK_DELETE, XK_Delete, SDLK_DELETE)));
  pComp->Value(
      mkNamingAdapt(Keyboard[3][4], "Kbd4Key5", KEY(VK_UP, XK_Up, SDLK_UP)));
  pComp->Value(mkNamingAdapt(Keyboard[3][5], "Kbd4Key6",
                             KEY(VK_NEXT, XK_Page_Down, SDLK_PAGEDOWN)));
  pComp->Value(mkNamingAdapt(Keyboard[3][6], "Kbd4Key7",
                             KEY(VK_LEFT, XK_Left, SDLK_LEFT)));
  pComp->Value(mkNamingAdapt(Keyboard[3][7], "Kbd4Key8",
                             KEY(VK_DOWN, XK_Down, SDLK_DOWN)));
  pComp->Value(mkNamingAdapt(Keyboard[3][8], "Kbd4Key9",
                             KEY(VK_RIGHT, XK_Right, SDLK_RIGHT)));
  pComp->Value(mkNamingAdapt(Keyboard[3][9], "Kbd4Key10",
                             KEY(VK_END, XK_End, SDLK_END)));
  pComp->Value(mkNamingAdapt(Keyboard[3][10], "Kbd4Key11",
                             KEY(VK_RETURN, XK_Return, SDLK_RETURN)));
  pComp->Value(mkNamingAdapt(Keyboard[3][11], "Kbd4Key12",
                             KEY(VK_BACK, XK_BackSpace, SDLK_BACKSPACE)));

  if (fKeysOnly) return;

  pComp->Value(mkNamingAdapt(MouseAScroll, "MouseAutoScroll", 0));
  pComp->Value(
      mkNamingAdapt(GamepadGuiControl, "GamepadGuiControl", 0, false, true));
...
BOOL C4Config::Load(BOOL forceWorkingDirectory, const char *szConfigFile) {
  try {
#ifdef _WIN32
    // Windows: Default load from registry, if no explicit config file is
    // specified
    if (!szConfigFile) {
      StdCompilerConfigRead CfgRead(
          HKEY_CURRENT_USER, "Software\\" C4CFG_Company "\\" C4CFG_Product);
      CfgRead.Compile(*this);
    } else
#endif
    {
      // Nonwindows or explicit config file: Determine filename to load config
      // from
      StdStrBuf filename;
      if (szConfigFile) {
...
// Config file not present?
        ...
        if (!szConfigFile) {
          ...
          filename += ".clonk";
          // Upgrade from older installations which had a single ~/.clonk config
          // file
...
// Configuration file syntax error?
#ifdef C4ENGINE
    LogF(
        "Error loading configuration: %s" /*LoadResStr("IDS_ERR_CONFREAD") - restbl not yet loaded*/,
        pExc->Msg.getData());
#endif
    delete pExc;
    return FALSE;
  ...
  if (Graphics.Engine == GFXENGN_DIRECTX || Graphics.Engine == GFXENGN_DIRECTXS)
    // Direct3D
    DDrawCfg.Set(Graphics.NewGfxCfg, (float)Graphics.TexIndent / 1000.0f,
                 (float)Graphics.BlitOff / 100.0f);
  else
#endif
    // OpenGL
    DDrawCfg.Set(Graphics.NewGfxCfgGL, (float)Graphics.TexIndentGL / 1000.0f,
                 (float)Graphics.BlitOffGL / 100.0f);
// Warning against invalid ports
#ifdef C4ENGINE
  if (Config.Network.PortTCP > 0 &&
      Config.Network.PortTCP == Config.Network.PortRefServer) {
    Log("Warning: Network TCP port and reference server port both set to same "
        "value - increasing reference server port!");
    ++Config.Network.PortRefServer;
    if (Config.Network.PortRefServer >= 65536)
      Config.Network.PortRefServer = C4NetStdPortRefServer;
  }
  if (Config.Network.PortUDP > 0 &&
      Config.Network.PortUDP == Config.Network.PortDiscovery) {
    Log("Warning: Network UDP port and LAN game discovery port both set to "
        "same value - increasing discovery port!");
  ...
  /*if (!Network.Nick.getLength())
          Network.Nick.Copy(Network.LocalName); // assuming that LocalName will
     always contain some useful value*/
  fConfigLoaded = true;
  if (szConfigFile)
    ConfigFilename.Copy(szConfigFile);
  else
...
BOOL C4Config::Save() {
  /* - deactivated: Graphics.Engine might not be same as last initialized
engine, because it might be changed by C4StartupOptionsDlg
#ifdef USE_DIRECTX
  // set ddraw config
  if (Graphics.Engine == GFXENGN_DIRECTX || Graphics.Engine == GFXENGN_DIRECTXS)
          {
  ...
  try {
    ...
    if (!ConfigFilename.getLength()) {
      ...
      StdCompilerConfigWrite CfgWrite(
          HKEY_CURRENT_USER, "Software\\" C4CFG_Company "\\" C4CFG_Product);
...
void C4ConfigGeneral::DeterminePaths(BOOL forceWorkingDirectory) {
  ...
  if (forceWorkingDirectory) SetWorkingDirectory(ExePath);
  // Log path
  ...
  if (ScreenshotFolder.getLength() + SLen(ScreenshotPath) + 1 <=
      CFG_MaxString) {
  ...
  if (!DirectoryExists(Config.AtUserPath("")))
    CreateDirectory(Config.AtUserPath(""), NULL);  // currently no error
                                                   // handling here; also: no
                                                   // recursive directory
                                                   // creation
#endif
}
...
const char *C4Config::AtScreenshotPath(const char *szFilename) {
  ...
  if (!DirectoryExists(AtPathFilename) &&
      !CreateDirectory(AtPathFilename, NULL)) {
...
BOOL C4ConfigGeneral::CreateSaveFolder(const char *strDirectory,
  ...
  char lang[3];
  SCopy(Config.General.Language, lang, 2);
  StdStrBuf strTitleFile;
  strTitleFile.Format("%s%c%s", strDirectory, DirectorySeparator,
                      C4CFN_WriteTitle);
  StdStrBuf strTitleData;
  strTitleData.Format("%s:%s", lang, strLanguageTitle);
  CStdFile hFile;
  if (!FileExists(strTitleFile.getData()))
    if (!hFile.Create(strTitleFile.getData()) ||
        !hFile.WriteString(strTitleData.getData()) || !hFile.Close())
      return FALSE;
...
const char *C4ConfigNetwork::GetLeagueServerAddress() {
  ...
  if (UseAlternateServer) return AlternateServerAddress;
...
void C4Config::ForceRelativePath(StdStrBuf *sFilename) {
  ...
  const char *szRelative =
      GetRelativePathS(sFilename->getData(), General.ExePath);
  if (szRelative != sFilename->getData()) {
    // return relative path
    StdStrBuf sTemp;
    sTemp.Copy(szRelative);
    sFilename->Take(sTemp);
  } else {
    // not in ExePath: Is it a global path?
    if (IsGlobalPath(sFilename->getData())) {
      // then shorten it (e.g. C:\Temp\Missions.c4f\Goldmine.c4s to
      // Missions.c4f\Goldmine.c4s)
      StdStrBuf sTemp;
      sTemp.Copy(GetC4Filename(sFilename->getData()));
      sFilename->Take(sTemp);
    }
  }
}
...
void C4ConfigGeneral::DefaultLanguage() {
  ...
  if (!LanguageEx[0]) GetLanguageSequence(Language, LanguageEx);
}
...
BOOL C4Config::Init() { return TRUE; }
...
int C4ConfigGeneral::GetLanguageSequence(const char *strSource,
                                         char *strTarget) {
  // Copy a condensed list of language codes from the source list to the target
  // string,
  // skipping any whitespace or long language descriptions. Language sequences
  // are
  // comma separated.
  int iCount = 0;
  char strLang[2 + 1];
...
void C4ConfigStartup::CompileFunc(StdCompiler *pComp) {
  pComp->Value(
      mkNamingAdapt(HideMsgGfxEngineChange, "HideMsgGfxEngineChange", 0));
  pComp->Value(
      mkNamingAdapt(HideMsgGfxBitDepthChange, "HideMsgGfxBitDepthChange", 0));
  pComp->Value(mkNamingAdapt(HideMsgMMTimerChange, "HideMsgMMTimerChange", 0));
  pComp->Value(
      mkNamingAdapt(HideMsgStartDedicated, "HideMsgStartDedicated", 0));
  ...
  pComp->Value(
      mkNamingAdapt(HideMsgNoOfficialLeague, "HideMsgNoOfficialLeague", 0));
  ...
  pComp->Value(
      mkNamingAdapt(LastPortraitFolderIdx, "LastPortraitFolderIdx", 0));
}
...
void C4Config::CompileFunc(StdCompiler *pComp) {
  ...
  for (int i = 0; i < C4ConfigMaxGamepads; ++i)
    pComp->Value(
        mkNamingAdapt(Gamepads[i], FormatString("Gamepad%d", i).getData()));
...
void C4Config::ExpandEnvironmentVariables(char *strPath, int iMaxLen) {
  ...
  StdStrBuf home(getenv("HOME"));
  char *rest;
  if (home && (rest = (char *)SSearch(strPath, "$HOME")) &&
      (SLen(strPath) - 5 + home.getLength() <= iMaxLen)) {
