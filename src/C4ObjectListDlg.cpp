...
/* Some boilerplate GObject defines. 'klass' is used instead of 'class', because
...
#define C4_LIST_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST((klass), C4_TYPE_LIST, C4ListClass))
...
#define C4_LIST_GET_CLASS(obj) \
  (G_TYPE_INSTANCE_GET_CLASS((obj), C4_TYPE_LIST, C4ListClass))
...
static void c4_list_init(C4List *c4_list) {
  c4_list->data = &Game.Objects;

  c4_list->stamp = g_random_int(); /* Random int to check whether iters belong
                                      to this model */
}

// destructor
static void c4_list_finalize(GObject *object) {
  /* must chain up - finalize parent */
  (*parent_class->finalize)(object);
}

static GtkTreeModelFlags c4_list_get_flags(GtkTreeModel *tree_model) {
...
static gboolean c4_list_get_iter(GtkTreeModel *tree_model, GtkTreeIter *iter,
                                 GtkTreePath *path) {
  gint *indices, depth;
...
static GtkTreePath *c4_list_get_path(GtkTreeModel *tree_model,
                                     GtkTreeIter *iter) {
  ...
  int i = 0;
  for (C4ObjectLink *pLnk = ((C4ObjectLink *)iter->user_data)->Prev; pLnk;
       pLnk = pLnk->Prev) {
    // Skip Contained Objects in the main list
  ...
  while (pObj) {
    ...
    if (pObj->Contained) pList = &pObj->Contained->Contents;
    for (C4ObjectLink *pLnk = pList->First; pLnk && pLnk->Obj != pObj;
         pLnk = pLnk->Next) {
      // Skip Contained Objects in the main list
      if (pObj->Contained != pLnk->Obj->Contained) continue;
      ++i;
...
// ++iter
static gboolean c4_list_iter_next(GtkTreeModel *tree_model, GtkTreeIter *iter) {
...
static gboolean c4_list_iter_children(GtkTreeModel *tree_model,
...
static gboolean c4_list_iter_has_child(GtkTreeModel *tree_model,
                                       GtkTreeIter *parent) {
  g_return_val_if_fail(parent == NULL || parent->user_data != NULL, FALSE);
  g_return_val_if_fail(C4_IS_LIST(tree_model), FALSE);
  ...
  C4ObjectList *pList = c4_list->data;
  if (parent) pList = &((C4ObjectLink *)parent->user_data)->Obj->Contents;
  return pList->First != 0;
}

// Counts the children 'parent' has.
static gint c4_list_iter_n_children(GtkTreeModel *tree_model,
                                    GtkTreeIter *parent) {
...
static gboolean c4_list_iter_nth_child(GtkTreeModel *tree_model,
                                       GtkTreeIter *iter, GtkTreeIter *parent,
                                       gint n) {
  g_return_val_if_fail(C4_IS_LIST(tree_model), FALSE);
  g_return_val_if_fail(parent == NULL || parent->user_data != NULL, FALSE);
...
// Helper function.
static gboolean c4_list_iter_for_C4Object(GtkTreeModel *tree_model,
                                          GtkTreeIter *iter,
                                          C4ObjectList *pList, C4Object *pObj) {
...
static gboolean c4_list_iter_parent(GtkTreeModel *tree_model, GtkTreeIter *iter,
                                    GtkTreeIter *child) {
  g_return_val_if_fail(C4_IS_LIST(tree_model), FALSE);
...
static C4Object *c4_list_iter_get_C4Object(GtkTreeModel *tree_model,
                                           GtkTreeIter *iter) {
  g_return_val_if_fail(C4_IS_LIST(tree_model), NULL);
...
static gint c4_list_get_n_columns(GtkTreeModel *tree_model) {
...
static GType c4_list_get_column_type(GtkTreeModel *tree_model, gint index) {
...
// gets the data for the 'column' in the row at 'iter' and stores that in
// 'value'.
static void c4_list_get_value(GtkTreeModel *tree_model, GtkTreeIter *iter,
  ...
  //C4Language::IconvUtf8(pObj->GetName()).getData());
}

// Wrapper around g_object_new.
static C4List *c4_list_new(void) {
  C4List *list;
...
static void c4_list_class_init(C4ListClass *klass) {
...
static void c4_list_tree_model_init(GtkTreeModelIface *iface) {
...
GType c4_list_get_type(void) {
  ...
  if (c4_list_type == 0) {
    ...
    static const GInterfaceInfo tree_model_info = {
        (GInterfaceInitFunc)c4_list_tree_model_init, NULL, NULL};

    g_type_add_interface_static(c4_list_type, GTK_TYPE_TREE_MODEL,
                                &tree_model_info);
  }
...
void C4ObjectListDlg::OnObjectRemove(C4ObjectList *pList, C4ObjectLink *pLnk) {
  ...
  // While pLnk is not in the list anymore, with pLnk->Prev and Contained a path
  // can still be made
  ...
  if (Contained && !Contained->Contents.First) {
    ...
    GtkTreeIter parent;
    C4ObjectList *pList = c4_list->data;
    if (Contained->Contained) pList = &Contained->Contained->Contents;
  ...
  // Cheat: For the signals it must look as if the object had it's parent
  // removed already
...
void C4ObjectListDlg::OnObjectAdded(C4ObjectList *pList, C4ObjectLink *pLnk) {
  ...
  if (pList != c4_list->data) {
    ...
    for (C4ObjectLink *pLnk2 = pList->First; pLnk2 && pLnk2->Obj != pObj;
         pLnk2 = pLnk2->Next) {
      // Skip Contained Objects in the main list
  ...
  if (pLnk->Obj->Contained &&
      pLnk->Obj->Contained->Contents.First ==
          pLnk->Obj->Contained->Contents.Last) {
...
void C4ObjectListDlg::OnSelectionChanged(GtkTreeSelection *selection,
                                         C4ObjectListDlg *dlg) {
  ...
  for (GList *i = list; i; i = i->next) {
    ...
    Console.EditCursor.GetSelection().Add(((C4ObjectLink *)iter.user_data)->Obj,
                                          C4ObjectList::stNone);
  }
...
void C4ObjectListDlg::Update(C4ObjectList &rSelection) {
  ...
  for (C4ObjectLink *pLnk = rSelection.First; pLnk; pLnk = pLnk->Next) {
    ...
    C4List *c4_list = C4_LIST(model);
    C4ObjectList *pList = c4_list->data;
    if (pLnk->Obj->Contained) pList = &pLnk->Obj->Contained->Contents;
    c4_list_iter_for_C4Object(GTK_TREE_MODEL(model), &iter, pList, pLnk->Obj);
...
C4ObjectListDlg::C4ObjectListDlg()
    : updating_selection(false), window(0), treeview(0), model(0) {}

C4ObjectListDlg::~C4ObjectListDlg() {}

void C4ObjectListDlg::Execute() {}

static void name_cell_data_func(GtkTreeViewColumn *column,
  ...
  g_object_set(G_OBJECT(renderer), "text",
               C4Language::IconvUtf8(object->GetName()).getData(),
               (gpointer)NULL);
}

#define ICON_SIZE 24

static void icon_cell_data_func(GtkTreeViewColumn *column,
                                GtkCellRenderer *renderer, GtkTreeModel *model,
                                GtkTreeIter *iter, gpointer data) {
  C4Object *object = c4_list_iter_get_C4Object(model, iter);
  ...
  if (pixbuf == NULL) {
    ...
    if (object->Def->Graphics.BitmapClr)
      surface = object->Def->Graphics.BitmapClr;

    const C4Rect &picture = object->Def->PictureRect;
    pixbuf =
        gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, picture.Wdt, picture.Hgt);
    guchar *pixels = gdk_pixbuf_get_pixels(pixbuf);
    surface->Lock();
    for (int y = 0; y < picture.Hgt; ++y)
      for (int x = 0; x < picture.Wdt; ++x) {
    ...
    if (picture.Wdt >= picture.Hgt) {
      double factor =
          static_cast<double>(picture.Hgt) / static_cast<double>(picture.Wdt);
      dest_width = ICON_SIZE;
      dest_height = dest_width * factor;
    } else {
      double factor =
          static_cast<double>(picture.Wdt) / static_cast<double>(picture.Hgt);
      dest_height = ICON_SIZE;
      dest_width = dest_height * factor;
    }

    GdkPixbuf *scaled = gdk_pixbuf_scale_simple(pixbuf, dest_width, dest_height,
                                                GDK_INTERP_HYPER);
...
void C4ObjectListDlg::Open() {
  ...
  if (window == NULL) {
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolled_wnd),
                                   GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrolled_wnd),
                                        GTK_SHADOW_IN);
    ...
    gtk_tree_view_column_set_cell_data_func(
        col, renderer, icon_cell_data_func,
