...
inline C4String *String(const char *str) {
  return str ? new C4String((str), &Game.ScriptEngine.Strings) : NULL;
}

static StdStrBuf FnStringFormat(C4AulContext *cthr, const char *szFormatPar,
                                C4Value *Par0 = 0, C4Value *Par1 = 0,
                                C4Value *Par2 = 0, C4Value *Par3 = 0,
                                C4Value *Par4 = 0, C4Value *Par5 = 0,
                                C4Value *Par6 = 0, C4Value *Par7 = 0,
                                C4Value *Par8 = 0, C4Value *Par9 = 0) {
  C4Value *Par[11];
  Par[0] = Par0;
  Par[1] = Par1;
  Par[2] = Par2;
  ...
  while (*cpFormat) {
    // Copy normal stuff
    while (*cpFormat && (*cpFormat != '%')) StringBuf.AppendChar(*cpFormat++);
    // Field
    if (*cpFormat == '%') {
      // arrays now valid in %v
      // if (Par[cPar] && Par[cPar]->Type == C4V_Array) throw new
      // C4AulExecError(0, cthr, 0, "array argument to format");
      // Scan field type
      for (cpType = cpFormat + 1;
           *cpType && (*cpType == '.' || Inside(*cpType, '0', '9')); cpType++)
        ;
      // Copy field
      SCopy(cpFormat, szField,
            Min<unsigned int>(sizeof szField - 1, cpType - cpFormat + 1));
      ...
      switch (*cpType) {
        ...
        case 'v': {
          if (!Par[cPar])
            throw new C4AulExecError(cthr->Obj,
                                     "format placeholder without parameter");
          StringBuf.Append(
              static_cast<const StdStrBuf &>(Par[cPar++]->GetDataString()));
          cpFormat += SLen(szField);
          break;
        }
        // String
        case 's': {
          ...
          if (Par[cPar]->GetData()) {
            ...
            if (!pStr)
              throw new C4AulExecError(
                  cthr->Obj, "string format placeholder without string");
...
bool CheckEnergyNeedChain(C4Object *pObj, C4ObjectList &rEnergyChainChecked) {
  ...
  if (pObj->Def->LineConnect & C4D_Power_Consumer)
    if (pObj->NeedEnergy) return TRUE;
...
DWORD StringBitEval(const char *str) {
  ...
  for (int cpos = 0; str && str[cpos]; cpos++)
    if ((str[cpos] != '_') && (str[cpos] != ' ')) rval += 1 << cpos;
  return rval;
}
...
static C4Value FnDeathAnnounce(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (cthr->Obj->Info && *(cthr->Obj->Info->DeathMessage)) {
    ...
    sprintf(idDeathMsg, "IDS_OBJ_DEATH%d", 1 + SafeRandom(MaxDeathMsg));
    GameMsgObject(
        FormatString(LoadResStr(idDeathMsg), cthr->Obj->GetName()).getData(),
        cthr->Obj);
  }
  return C4VTrue;
}
...
static C4Value FnGrabContents(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (!pTo)
    if (!(pTo = cthr->Obj)) return C4VFalse;
...
static C4Value FnKill(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (cthr->Obj && ValidPlr(cthr->Obj->Controller))
    pObj->UpdatLastEnergyLossCause(cthr->Obj->Controller);
  // Do the kill
...
static C4Value FnFling(C4AulContext *cthr, C4Value *pPars) {
  ...
  pObj->Fling(itofix(iXDir, iPrec), itofix(iYDir, iPrec), fAddSpeed,
              cthr->Obj ? cthr->Obj->Controller : NO_OWNER);
  // unstick from ground, because Fling command may be issued in an
  // Action-callback,
...
static C4Value FnExit(C4AulContext *cthr, C4Value *pPars) {
  ...
  return C4VBool(!!pObj->Exit(tx, ty + pObj->Shape.y, tr, itofix(txdir),
                              itofix(tydir), itofix(trdir) / 10));
}

static C4Value FnCollect(C4AulContext *cthr, C4Value *pPars) {
  PAR(object, pItem);
  PAR(object, pCollector);
  bool success = false;

  // local call / safety
  if (!pCollector) pCollector = cthr->Obj;
  if (!pItem || !pCollector) return C4VFalse;
  ...
  if (iOldNoCollectDelay > pCollector->NoCollectDelay)
    pCollector->NoCollectDelay = iOldNoCollectDelay;
  // failure
  ...
  for (cnt = 0; ObjComponents.GetID(cnt); cnt++)
    for (cnt2 = 0; cnt2 < ObjComponents.GetCount(cnt); cnt2++)
      if (pNew = Game.CreateObject(
              ObjComponents.GetID(cnt), pObj, pObj->Owner, pObj->x, pObj->y,
...
static bool FnRemoveObject(C4AulContext *cthr, C4Object *pObj,
                           bool fEjectContents) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnSetPosition(C4AulContext *cthr, long iX, long iY, C4Object *pObj,
                          bool fCheckBounds) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnDoCon(C4AulContext *cthr, long iChange,
                    C4Object *pObj)  // in percent
{
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnDoEnergy(C4AulContext *cthr, long iChange, C4Object *pObj,
                       bool fExact, long iEngType, long iCausedByPlusOne) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  if (!iEngType) iEngType = C4FxCall_EngScript;
  int32_t iCausedBy = iCausedByPlusOne - 1;
  if (!iCausedByPlusOne && cthr->Obj) iCausedBy = cthr->Obj->Controller;
...
static bool FnDoDamage(C4AulContext *cthr, long iChange, C4Object *pObj,
                       long iDmgType, long iCausedByPlusOne) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  int32_t iCausedBy = iCausedByPlusOne - 1;
  if (!iCausedByPlusOne && cthr->Obj) iCausedBy = cthr->Obj->Controller;
...
static bool FnDoMagicEnergy(C4AulContext *cthr, long iChange, C4Object *pObj,
                            bool fAllowPartial) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return false;
  ...
  // Change energy level
  pObj->MagicEnergy =
      BoundBy<long>(pObj->MagicEnergy + iChange, 0, pObj->GetPhysical()->Magic);
...
const int32_t PHYS_Current = 0, PHYS_Permanent = 1, PHYS_Temporary = 2,
              PHYS_StackTemporary = 3;

static bool FnSetPhysical(C4AulContext *cthr, C4String *szPhysical, long iValue,
                          long iMode, C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  // Get physical offset
  C4PhysicalInfo::Offset off;
  if (!C4PhysicalInfo::GetOffsetByName(FnStringPar(szPhysical), &off))
    return FALSE;
  // Set by mode
  switch (iMode) {
    ...
    case PHYS_Current:
      ...
      if (!pObj->PhysicalTemporary)
        if (!pObj->Info || Game.Parameters.UseFairCrew) return FALSE;
      // Set physical
    ...
    case PHYS_Permanent:
      ...
      // In fair crew mode, changing the permanent physicals is only allowed via
      // TrainPhysical
      // Otherwise, stuff like SetPhysical(..., GetPhysical(...)+1, ...) would
      // screw up the crew in fair crew mode
...
static bool FnTrainPhysical(C4AulContext *cthr, C4String *szPhysical,
                            long iTrainBy, long iMaxTrain, C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  // Get physical offset
  C4PhysicalInfo::Offset off;
...
static bool FnResetPhysical(C4AulContext *cthr, C4Object *pObj,
                            C4String *sPhysical) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  ...
  if (szPhysical && *szPhysical) {
    ...
    if (pObj->TemporaryPhysical.HasChanges(pObj->GetPhysical(true)))
      return TRUE;
  }
...
static long FnGetPhysical(C4AulContext *cthr, C4String *szPhysical, long iMode,
  ...
  switch (iMode) {
    ...
    case PHYS_Permanent:
      ...
      // In fair crew mode, scripts may not read permanent physical values -
      // fallback to fair def physical instead!
...
static bool FnSetXDir(C4AulContext *cthr, long nxdir, C4Object *pObj,
                      long iPrec) {
  // safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnSetRDir(C4AulContext *cthr, long nrdir, C4Object *pObj,
                      long iPrec) {
  // safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnSetYDir(C4AulContext *cthr, long nydir, C4Object *pObj,
                      long iPrec) {
  // safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnSetAction(C4AulContext *cthr, C4String *szAction,
  ...
  return !!cthr->Obj->SetActionByName(
      FnStringPar(szAction), pTarget, pTarget2,
      C4Object::SAC_StartCall | C4Object::SAC_AbortCall, !!fDirect);
}

static bool FnSetBridgeActionData(C4AulContext *cthr, long iBridgeLength,
                                  bool fMoveClonk, bool fWall,
                                  long iBridgeMaterial, C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
...
static bool FnSetActionData(C4AulContext *cthr, long iData, C4Object *pObj) {
  ...
  if ((pObj->Action.Act > ActIdle) &&
      (pObj->Def->ActMap[pObj->Action.Act].Procedure == DFA_BRIDGE))
    return FnSetBridgeActionData(cthr, 0, false, false, iData, pObj);
  // attach: check for valid vertex indices
  if ((pObj->Action.Act > ActIdle) &&
      (pObj->Def->ActMap[pObj->Action.Act].Procedure ==
       DFA_ATTACH))  // Fixed Action.Act check here... matthes
...
static bool FnObjectSetAction(C4AulContext *cthr, C4Object *pObj,
                              C4String *szAction, C4Object *pTarget,
                              C4Object *pTarget2, bool fDirect) {
  if (!szAction || !pObj) return FALSE;
  // regular action change
  return !!pObj->SetActionByName(
      FnStringPar(szAction), pTarget, pTarget2,
      C4Object::SAC_StartCall | C4Object::SAC_AbortCall, !!fDirect);
}
...
static bool FnSetCategory(C4AulContext *cthr, long iCategory, C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  if (!(iCategory & C4D_SortLimit))
    iCategory |= (pObj->Category & C4D_SortLimit);
...
static C4Value FnSetCommand(C4AulContext *cthr, C4Value *pPars) {
  ...
  // Object
  if (!pObj) pObj = cthr->Obj;
  if (!pObj || !szCommand) return C4VFalse;
  // Command
  ...
  // Set
  pObj->SetCommand(iCommand, pTarget, Tx, iTy, pTarget2, FALSE, iData, iRetries,
                   szText);
  // Success
  return C4VTrue;
}
...
static C4Value FnAddCommand(C4AulContext *cthr, C4Value *pPars) {
  ...
  // Add
  return C4VBool(pObj->AddCommand(iCommand, pTarget, Tx, iTy, iUpdateInterval,
                                  pTarget2, TRUE, iData, FALSE, iRetries,
                                  szText, iBaseMode));
}
...
static bool FnFinishCommand(C4AulContext *cthr, C4Object *pObj, bool fSuccess,
                            long iCommandNum) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return false;
  C4Command *Command = pObj->Command;
...
static C4Value FnPlayerObjectCommand(C4AulContext *cthr, C4Value *pPars) {
  ...
  // Set
  pPlr->ObjectCommand(iCommand, pTarget, iTx, iTy, pTarget2, iData,
                      C4P_Command_Set);
  // Success
  return C4VTrue;
}
...
static C4String *FnGetName(C4AulContext *cthr, C4Object *pObj,
                           C4ID idDef /*, bool fFilename, long idx*/) {
  // Def name
  C4Def *pDef;
  if (idDef /*|| idx*/) {
    /*if (idx)
...
static bool FnSetName(C4AulContext *cthr, C4String *pNewName, C4Object *pObj,
                      C4ID idDef, bool fSetInInfo, bool fMakeValidIfExists) {
  // safety
  ...
  if (idDef)
    ...
    if (fSetInInfo) {
      ...
      // querying owner info list here isn't 100% accurate, as infos might have
      // been stolen by other players
      ...
      if (pInfoList)
        if (pInfoList->NameExists(szName)) {
      ...
      if (!pNewName)
        ...
        pObj->SetName(pNewName->Data.getData());
    }
  }
  return true;
}
...
static C4String *FnGetTaggedPlayerName(C4AulContext *cthr, long iPlayer) {
  ...
  C4GUI::MakeColorReadableOnBlack(dwClr);
...
static C4Object *FnGetActionTarget(C4AulContext *cthr, long target_index,
                                   C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return NULL;
...
static bool FnSetActionTargets(C4AulContext *cthr, C4Object *pTarget1,
                               C4Object *pTarget2, C4Object *pObj) {
  // safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  // set targets
...
static const int VTX_X = 0,  // vertex data indices
    VTX_Y = 1, VTX_CNAT = 2, VTX_Friction = 3, VTX_SetPermanent = 1,
                 VTX_SetPermanentUpd = 2;

static long FnGetVertex(C4AulContext *cthr, long iIndex, long iValueToGet,
                        C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return 0;
  ...
  switch (iValueToGet) {
    ...
    case VTX_Y:
      ...
      break;
    case VTX_CNAT:
      return pObj->Shape.VtxCNAT[iIndex];
      break;
    case VTX_Friction:
      return pObj->Shape.VtxFriction[iIndex];
      break;
    default:
      // old-style behaviour for any value != 0 (normally not used)
      DebugLog(FormatString(
                   "FnGetVertex: Unknown vertex attribute: %ld; getting VtxY",
                   iValueToGet).getData());
      return pObj->Shape.VtxY[iIndex];
      break;
  }
...
static bool FnSetVertex(C4AulContext *cthr, long iIndex, long iValueToSet,
                        long iValue, C4Object *pObj, long iOwnVertexMode) {
  // local call / safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj || !pObj->Status) return FALSE;
  // own vertex mode?
  ...
  switch (iValueToSet) {
    ...
    case VTX_Y:
      ...
      break;
    case VTX_CNAT:
      pObj->Shape.VtxCNAT[iIndex] = iValue;
      break;
    case VTX_Friction:
      pObj->Shape.VtxFriction[iIndex] = iValue;
      break;
    default:
      // old-style behaviour for any value != 0 (normally not used)
      pObj->Shape.VtxY[iIndex] = iValue;
      DebugLogF("FnSetVertex: Unknown vertex attribute: %ld; setting VtxY",
                iValueToSet);
      break;
  }
...
static bool FnSetContactDensity(C4AulContext *cthr, long iDensity,
                                C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnSetController(C4AulContext *cthr, long iNewController,
                            C4Object *pObj) {
  // validate player
...
static long FnGetCategory(C4AulContext *cthr, C4Object *pObj, C4ID idDef) {
  ...
  C4Def *pDef;
  if (idDef)
    if (pDef = C4Id2Def(idDef)) return pDef->Category;
  // Object category
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return 0;
...
static long FnGetValue(C4AulContext *cthr, C4Object *pObj, C4ID idDef,
                       C4Object *pInBase, long iForPlayer) {
  // Def value
  C4Def *pDef;
  if (idDef)
...
static bool FnCreateMenu(C4AulContext *cthr, C4ID iSymbol, C4Object *pMenuObj,
                         C4Object *pCommandObj, long iExtra,
                         C4String *szCaption, long iExtraData, long iStyle,
                         bool fPermanent, C4ID idMenuID) {
  if (!pMenuObj) {
    pMenuObj = cthr->Obj;
  ...
  if (!pMenuObj->Menu)
    pMenuObj->Menu = new C4ObjectMenu;
  else
    pMenuObj->Menu->ClearItems(true);
  pMenuObj->Menu->Init(fctSymbol, FnStringPar(szCaption), pCommandObj, iExtra,
                       iExtraData, idMenuID ? idMenuID : iSymbol, iStyle, TRUE);
...
const int
    C4MN_Add_ImgRank = 1,
    C4MN_Add_ImgIndexed = 2, C4MN_Add_ImgObjRank = 3, C4MN_Add_ImgObject = 4,
    C4MN_Add_ImgTextSpec = 5, C4MN_Add_ImgColor = 6,
    C4MN_Add_MaxImage =
        127,  // mask for param which decides what to draw as the menu symbol
    C4MN_Add_PassValue = 128,
    C4MN_Add_ForceCount = 256, C4MN_Add_ForceNoDesc = 512;
...
static C4Value FnAddMenuItem(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (!pMenuObj) return C4VBool(FALSE);
  ...
  switch (Parameter.GetType()) {
    ...
    case C4V_String:
      ...
      parameter[0] = '"';
      SCopy(Parameter.getStr()->Data.getData(), parameter + 1,
            sizeof(command) - 3);
      SAppendChar('"', command);
      break;
    case C4V_Any:
  ...
  for (; i < SLen(FnStringPar(szCommand)); i++)
    if (!IsIdentifier(FnStringPar(szCommand)[i])) break;
  if (i < SLen(FnStringPar(szCommand))) {
    // Search for "%d" an replace it by "%s" for insertion of formatted
    // parameter
    SCopy(FnStringPar(szCommand), dummy, 256);
    char *pFound = const_cast<char *>(SSearch(dummy, "%d"));
    if (pFound != 0) *(pFound - 1) = 's';
  ...
  // Old style: function name with id and parameter
    ...
    if (szScriptCom && *szScriptCom) {
      if (iExtra & C4MN_Add_PassValue) {
        // with value
        sprintf(command, "%s(%s,%s,0,%ld)", szScriptCom, C4IdText(idItem),
                parameter, iValue);
        sprintf(command2, "%s(%s,%s,1,%ld)", szScriptCom, C4IdText(idItem),
                parameter, iValue);
      } else {
        // without value
        sprintf(command, "%s(%s,%s)", szScriptCom, C4IdText(idItem), parameter);
        sprintf(command2, "%s(%s,%s,1)", szScriptCom, C4IdText(idItem),
                parameter);
      }
    } else {
      // no command
  ...
  if (!infocaption[0] && !(iExtra & C4MN_Add_ForceNoDesc))
    SCopy(pDef->GetDesc(), infocaption, C4MaxTitle);
  ...
  switch (iExtra & C4MN_Add_MaxImage) {
    case C4MN_Add_ImgRank: {
      ...
      C4RankSystem::DrawRankSymbol(&fctSymbol, iCount, pfctRankSym, iRankSymNum,
                                   true);
      iCount = 0;
      break;
    }
    ...
    case C4MN_Add_ImgObjRank: {
      ...
      if (pGfxObj && pGfxObj->Status) {
        ...
        if (pGfxObj->Info) {
          ...
          C4RankSystem::DrawRankSymbol(&fctSymbol, pGfxObj->Info->Rank,
                                       pRankRes, iRankCnt, true);
    ...
    case C4MN_Add_ImgTextSpec: {
      ...
      if (!szCaption ||
          !Game.DrawTextSpecImage(fctSymSpec, caption, dwClr ? dwClr : 0xff))
        return C4Value();
      fctSymbol.Create(iSymbolSize, iSymbolSize);
  ...
  if (iCount == 0 && !(iExtra & C4MN_Add_ForceCount))
  ...
  // Add menu item
  pMenuObj->Menu->Add(caption, fctSymbol, command, iCount, NULL, infocaption,
                      idItem, command2, fOwnValue, iValue, fIsSelectable);

  return C4VBool(true);
}

static bool FnSelectMenuItem(C4AulContext *cthr, long iItem,
                             C4Object *pMenuObj) {
  if (!pMenuObj) pMenuObj = cthr->Obj;
  if (!pMenuObj) return false;
...
static bool FnSetMenuDecoration(C4AulContext *cthr, C4ID idNewDeco,
                                C4Object *pMenuObj) {
...
static bool FnSetMenuTextProgress(C4AulContext *cthr, long iNewProgress,
                                  C4Object *pMenuObj) {
  if (!pMenuObj || !pMenuObj->Menu) return FALSE;
...
static bool FnShiftContents(C4AulContext *cthr, C4Object *pObj, bool fShiftBack,
                            C4ID idTarget, bool fDoCalls) {
  // local call/safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return false;
  // regular shift
...
static C4Object *FnFindOtherContents(C4AulContext *cthr, C4ID c_id,
                                     C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return 0;
...
static bool FnComponentAll(C4AulContext *cthr, C4Object *pObj, C4ID c_id) {
  ...
  for (cnt = 0; Components.GetID(cnt); cnt++)
    if (Components.GetID(cnt) != c_id)
      if (Components.GetCount(cnt) > 0) return FALSE;
  return TRUE;
}

static C4Object *FnCreateObject(C4AulContext *cthr, C4ID id, long iXOffset,
                                long iYOffset, long iOwner) {
  ...
  C4Object *pNewObj =
      Game.CreateObject(id, cthr->Obj, iOwner, iXOffset, iYOffset);

  // Set initial controller to creating controller, so more complicated
  // cause-effect-chains can be traced back to the causing player
  if (pNewObj && cthr->Obj && cthr->Obj->Controller > NO_OWNER)
    pNewObj->Controller = cthr->Obj->Controller;

  return pNewObj;
}

static C4Object *FnCreateConstruction(C4AulContext *cthr, C4ID id,
                                      long iXOffset, long iYOffset, long iOwner,
                                      long iCompletion, bool fTerrain,
                                      bool fCheckSite) {
  ...
  // Check site
  if (fCheckSite)
    if (!ConstructionCheck(id, iXOffset, iYOffset, cthr->Obj)) return NULL;

  // Create site object
  C4Object *pNewObj =
      Game.CreateObjectConstruction(id, cthr->Obj, iOwner, iXOffset, iYOffset,
...
static C4Object *FnCreateContents(C4AulContext *cthr, C4ID c_id, C4Object *pObj,
                                  long iCount) {
  // local call / safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return NULL;
...
static C4Object *FnComposeContents(C4AulContext *cthr, C4ID c_id,
                                   C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return NULL;
...
/*static C4Value FnFindConstructionSite(C4AulContext *cthr, C4Value *id_C4V,
  C4Value *iVarX_C4V, C4Value *iVarY_C4V)
        {
  ...
  if (!pDef) return C4VBool(FALSE);
        ...
        if(iVarX_C4V->GetType() == C4V_pC4Value)			Sorry,
  reference detection doesn't work...
...
static bool FnFindConstructionSite(C4AulContext *cthr, C4ID id, long iVarX,
                                   long iVarY) {
  // Get def																			Old-style
  ...
  if (!Inside<long>(iVarX, 0, C4AUL_MAX_Par - 1) ||
      !Inside<long>(iVarY, 0, C4AUL_MAX_Par - 1))
    return FALSE;
  // Get thread vars
  ...
  // Construction check at starting position
  if (ConstructionCheck(id, V1.getInt(), V2.getInt())) return TRUE;
  // Search for real
  int32_t v1 = V1.getInt(), v2 = V2.getInt();
  bool result = !!FindConSiteSpot(v1, v2, pDef->Shape.Wdt, pDef->Shape.Hgt,
                                  pDef->Category, 20);
  V1 = C4VInt(v1);
  V2 = C4VInt(v2);
  return result;
...
C4FindObject *CreateCriterionsFromPars(C4Value *pPars, C4FindObject **pFOs,
                                       C4SortObject **pSOs) {
...
static C4Value FnObjectCount2(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (!pFO)
    throw new C4AulExecError(
        cthr->Obj, "ObjectCount: No valid search criterions supplied!");
  // Search
...
static C4Value FnFindObject2(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (!pFO)
    throw new C4AulExecError(
        cthr->Obj, "FindObject: No valid search criterions supplied!");
  // Search
...
static C4Value FnFindObjects(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (!pFO)
    throw new C4AulExecError(
        cthr->Obj, "FindObjects: No valid search criterions supplied!");
  // Search
...
static C4Value FnObjectCount(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (pPars->getArray()) return FnObjectCount2(cthr, pPars);
  // Get paramters
  PAR(id, id);
  PAR(int, x);
  PAR(int, y);
...
static C4Value FnFindObject(C4AulContext *cthr, C4Value *pPars) {
  // Got an array? Use the new-style function.
  if (pPars->getArray()) return FnFindObject2(cthr, pPars);
  // Get parameters
  PAR(id, id);
  PAR(int, x);
  PAR(int, y);
...
static C4Object *FnFindObjectOwner(C4AulContext *cthr, C4ID id, long iOwner,
                                   long x, long y, long wdt, long hgt,
                                   long dwOCF, C4String *szAction,
...
static bool FnGrabObjectInfo(C4AulContext *cthr, C4Object *pFrom,
                             C4Object *pTo) {
  // local call, safety
...
static bool FnSmoke(C4AulContext *cthr, long tx, long ty, long level,
                    long dwClr) {
  if (cthr->Obj) {
    tx += cthr->Obj->x;
    ty += cthr->Obj->y;
...
static bool FnInsertMaterial(C4AulContext *cthr, long mat, long x, long y,
                             long vx, long vy) {
  if (cthr->Obj) {
    x += cthr->Obj->x;
    y += cthr->Obj->y;
...
static long FnExtractMaterialAmount(C4AulContext *cthr, long x, long y,
                                    long mat, long amount) {
  if (cthr->Obj) {
    x += cthr->Obj->x;
    y += cthr->Obj->y;
  }
...
static bool FnBlastObjects(C4AulContext *cthr, long iX, long iY, long iLevel,
                           C4Object *pInObj, long iCausedByPlusOne) {
  long iCausedBy = iCausedByPlusOne - 1;
  if (!iCausedByPlusOne && cthr->Obj) iCausedBy = cthr->Obj->Controller;
  Game.BlastObjects(iX, iY, iLevel, pInObj, iCausedBy, cthr->Obj);
  return true;
}

static bool FnBlastObject(C4AulContext *cthr, long iLevel, C4Object *pObj,
                          long iCausedByPlusOne) {
  long iCausedBy = iCausedByPlusOne - 1;
  if (!iCausedByPlusOne && cthr->Obj) iCausedBy = cthr->Obj->Controller;
  if (!pObj)
    if (!(pObj = cthr->Obj)) return false;
  if (!pObj->Status) return false;
  pObj->Blast(iLevel, iCausedBy);
  return true;
}

static bool FnBlastFree(C4AulContext *cthr, long iX, long iY, long iLevel,
                        long iCausedByPlusOne) {
  int32_t iCausedBy = iCausedByPlusOne - 1;
...
static bool FnSound(C4AulContext *cthr, C4String *szSound, bool fGlobal,
                    C4Object *pObj, long iLevel, long iAtPlayer, long iLoop,
                    bool fMultiple, long iCustomFalloffDistance) {
  // play here?
  if (iAtPlayer) {
  ...
  // default sound level
  if (!iLevel || iLevel > 100) iLevel = 100;
  // target object
  if (fGlobal)
    pObj = NULL;
  else if (!pObj)
  ...
  // try to play effect
  if (iLoop >= 0)
    StartSoundEffect(FnStringPar(szSound), !!iLoop, iLevel, pObj,
...
static long FnSetPlayList(C4AulContext *cth, C4String *szPlayList,
                          bool fRestartMusic) {
  long iFilesInPlayList =
      Application.MusicSystem.SetPlayList(FnStringPar(szPlayList));
...
static bool FnSoundLevel(C4AulContext *cthr, C4String *szSound, long iLevel,
                         C4Object *pObj) {
  SoundLevel(FnStringPar(szSound), pObj, iLevel);
  return TRUE;
}

static bool FnGameOver(
    C4AulContext *cthr,
    long iGameOverValue /* provided for future compatibility */) {
...
static C4Value FnLog_C4V(C4AulContext *cthr, C4Value *szMessage, C4Value *iPar0,
                         C4Value *iPar1, C4Value *iPar2, C4Value *iPar3,
                         C4Value *iPar4, C4Value *iPar5, C4Value *iPar6,
                         C4Value *iPar7, C4Value *iPar8) {
  Log(FnStringFormat(cthr, FnStringPar(szMessage->getStr()), iPar0, iPar1,
                     iPar2, iPar3, iPar4, iPar5, iPar6, iPar7,
                     iPar8).getData());
  return C4VBool(true);
}

static C4Value FnDebugLog_C4V(C4AulContext *cthr, C4Value *szMessage,
                              C4Value *iPar0, C4Value *iPar1, C4Value *iPar2,
                              C4Value *iPar3, C4Value *iPar4, C4Value *iPar5,
                              C4Value *iPar6, C4Value *iPar7, C4Value *iPar8) {
  DebugLog(FnStringFormat(cthr, FnStringPar(szMessage->getStr()), iPar0, iPar1,
                          iPar2, iPar3, iPar4, iPar5, iPar6, iPar7,
                          iPar8).getData());
  return C4VBool(true);
}

static C4Value FnFormat_C4V(C4AulContext *cthr, C4Value *szFormat,
                            C4Value *iPar0, C4Value *iPar1, C4Value *iPar2,
                            C4Value *iPar3, C4Value *iPar4, C4Value *iPar5,
                            C4Value *iPar6, C4Value *iPar7, C4Value *iPar8) {
  return C4VString(FnStringFormat(cthr, FnStringPar(szFormat->getStr()), iPar0,
                                  iPar1, iPar2, iPar3, iPar4, iPar5, iPar6,
                                  iPar7, iPar8));
}
...
static C4Value FnPlayerMessage_C4V(C4AulContext *cthr, C4Value *iPlayer,
                                   C4Value *c4vMessage, C4Value *c4vObj,
                                   C4Value *iPar0, C4Value *iPar1,
                                   C4Value *iPar2, C4Value *iPar3,
                                   C4Value *iPar4, C4Value *iPar5,
                                   C4Value *iPar6) {
  ...
  if (!fSpoken)
    if (SCopySegment(
            FnStringFormat(cthr, FnStringPar(szMessage), iPar0, iPar1, iPar2,
                           iPar3, iPar4, iPar5, iPar6).getData(),
            0, buf, '$', MaxFnStringParLen))
      if (pObj)
        GameMsgObjectPlayer(buf, pObj, iPlayer->getInt());
      else
        GameMsgPlayer(buf, iPlayer->getInt());

  return C4VBool(true);
}

static C4Value FnMessage_C4V(C4AulContext *cthr, C4Value *c4vMessage,
                             C4Value *c4vObj, C4Value *iPar0, C4Value *iPar1,
                             C4Value *iPar2, C4Value *iPar3, C4Value *iPar4,
                             C4Value *iPar5, C4Value *iPar6, C4Value *iPar7) {
  char buf[MaxFnStringParLen + 1];
  C4String *szMessage = c4vMessage->getStr();
  if (!szMessage) return C4VBool(false);
  ...
  bool fSpoken = FALSE;
  if (SCopySegment(FnStringPar(szMessage), 1, buf, '$', MaxFnStringParLen))
    if (StartSoundEffect(buf, false, 100, cthr->Obj)) fSpoken = TRUE;
  ...
  if (!fSpoken)
    if (SCopySegment(
            FnStringFormat(cthr, FnStringPar(szMessage), iPar0, iPar1, iPar2,
                           iPar3, iPar4, iPar5, iPar6, iPar7).getData(),
            0, buf, '$', MaxFnStringParLen))
      if (pObj)
...
static C4Value FnAddMessage_C4V(C4AulContext *cthr, C4Value *c4vMessage,
                                C4Value *c4vObj, C4Value *iPar0, C4Value *iPar1,
                                C4Value *iPar2, C4Value *iPar3, C4Value *iPar4,
                                C4Value *iPar5, C4Value *iPar6,
                                C4Value *iPar7) {
  C4String *szMessage = c4vMessage->getStr();
  if (!szMessage) return C4VBool(false);
  ...
  if (pObj)
    Game.Messages.Append(
        C4GM_Target,
        FnStringFormat(cthr, FnStringPar(szMessage), iPar0, iPar1, iPar2, iPar3,
                       iPar4, iPar5, iPar6, iPar7).getData(),
        pObj, NO_OWNER, 0, 0, FWhite);
  else
    Game.Messages.Append(
        C4GM_Global,
        FnStringFormat(cthr, FnStringPar(szMessage), iPar0, iPar1, iPar2, iPar3,
                       iPar4, iPar5, iPar6, iPar7).getData(),
        0, ANY_OWNER, 0, 0, FWhite);

  return C4VBool(true);
}

static C4Value FnPlrMessage_C4V(C4AulContext *cthr, C4Value *c4vMessage,
                                C4Value *iPlr, C4Value *iPar0, C4Value *iPar1,
                                C4Value *iPar2, C4Value *iPar3, C4Value *iPar4,
                                C4Value *iPar5, C4Value *iPar6,
                                C4Value *iPar7) {
  char buf[MaxFnStringParLen + 1];
  ...
  if (!fSpoken)
    if (SCopySegment(
            FnStringFormat(cthr, FnStringPar(szMessage), iPar0, iPar1, iPar2,
                           iPar3, iPar4, iPar5, iPar6, iPar7).getData(),
            0, buf, '$', MaxFnStringParLen))
      if (ValidPlr(iPlr->getInt()))
        GameMsgPlayer(buf, iPlr->getInt());
      else
        GameMsgGlobal(buf);
...
static bool FnCastPXS(C4AulContext *cthr, C4String *mat_name, long amt,
                      long level, long tx, long ty) {
  if (cthr->Obj) {
    tx += cthr->Obj->x;
    ty += cthr->Obj->y;
  }
...
static bool FnCastObjects(C4AulContext *cthr, C4ID id, long amt, long level,
                          long tx, long ty) {
  if (cthr->Obj) {
    tx += cthr->Obj->x;
    ty += cthr->Obj->y;
  }
  Game.CastObjects(id, cthr->Obj, amt, level, tx, ty,
                   cthr->Obj ? cthr->Obj->Owner : NO_OWNER,
                   cthr->Obj ? cthr->Obj->Controller : NO_OWNER);
  return TRUE;
}
...
C4Object *FnPlaceVegetation(C4AulContext *cthr, C4ID id, long iX, long iY,
...
static bool FnDrawVolcanoBranch(C4AulContext *cthr, long mat, long fx, long fy,
                                long tx, long ty, long size) {
  long cx, cx2, cy;
  for (cy = ty; cy < fy; cy++) {
...
static bool FnHostile(C4AulContext *cthr, long iPlr1, long iPlr2,
                      bool fCheckOneWayOnly) {
  if (fCheckOneWayOnly) {
...
static bool FnSetHostility(C4AulContext *cthr, long iPlr, long iPlr2,
                           bool fHostile, bool fSilent, bool fNoCalls) {
  C4Player *pPlr = Game.Players.Get(iPlr);
  if (!pPlr) return false;
  // do rejection test first
  if (!fNoCalls) {
    if (!!Game.Script.GRBroadcast(
            PSF_RejectHostilityChange,
            &C4AulParSet(C4VInt(iPlr), C4VInt(iPlr2), C4VBool(fHostile)), true,
            true))
      return false;
  }
  // OK; set hostility
  ...
  Game.Script.GRBroadcast(
      ...
      &C4AulParSet(C4VInt(iPlr), C4VInt(iPlr2), C4VBool(fHostile),
                   C4VBool(fOldHostility)),
      true);
  return true;
}
...
static bool FnSetPlrShowControl(C4AulContext *cthr, long iPlr,
                                C4String *defstring) {
  if (!ValidPlr(iPlr)) return false;
...
static C4String *FnGetPlrControlName(C4AulContext *cthr, long iPlr, long iCon,
                                     bool fShort) {
...
static bool FnDoHomebaseMaterial(C4AulContext *cthr, long iPlr, C4ID id,
                                 long iChange) {
  // validity check
  C4Player *pPlr = Game.Players.Get(iPlr);
  if (!pPlr) return FALSE;
  ...
  if (!pPlr->HomeBaseMaterial.SetIDCount(id, iLastcount + iChange, TRUE))
    return false;
  if (Game.Rules & C4RULE_TeamHombase) pPlr->SyncHomebaseMaterialToTeam();
  return true;
}

static bool FnDoHomebaseProduction(C4AulContext *cthr, long iPlr, C4ID id,
                                   long iChange) {
  // validity check
  if (!ValidPlr(iPlr)) return FALSE;
  C4Def *pDef = C4Id2Def(id);
  ...
  return Game.Players.Get(iPlr)
      ->HomeBaseProduction.SetIDCount(id, iLastcount + iChange, TRUE);
}
...
static bool FnSetPlrKnowledge(C4AulContext *cthr, long iPlr, C4ID id,
                              bool fRemove) {
  C4Player *pPlr = Game.Players.Get(iPlr);
  if (!pPlr) return FALSE;
  if (fRemove) {
...
static bool FnSetComponent(C4AulContext *cthr, C4ID idComponent, long iCount,
                           C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static C4Value FnGetPlrKnowledge_C4V(C4AulContext *cthr, C4Value *iPlr_C4V,
                                     C4Value *id_C4V, C4Value *iIndex_C4V,
                                     C4Value *dwCategory_C4V) {
  ...
  if (id)
    return C4VBool(Game.Players.Get(iPlr)->Knowledge.GetIDCount(id, 1) != 0);
  // Search indexed item of given category, return C4ID
  return C4VID(
      Game.Players.Get(iPlr)->Knowledge.GetID(Game.Defs, dwCategory, iIndex));
}
...
static C4Value FnGetComponent_C4V(C4AulContext *cthr, C4Value *idComponent_C4V,
                                  C4Value *iIndex_C4V, C4Value *pObj_C4V,
                                  C4Value *idDef_C4V) {
  ...
  if (idDef) {
    ...
    // Component count
    if (idComponent)
      return C4VInt(pDef->GetComponentCount(idComponent, cthr->Obj));
    // Indexed component
...
static C4Value FnGetHomebaseMaterial_C4V(C4AulContext *cthr, C4Value *iPlr_C4V,
                                         C4Value *id_C4V, C4Value *iIndex_C4V,
                                         C4Value *dwCategory_C4V) {
  long iPlr = iPlr_C4V->getInt();
  C4ID id = id_C4V->getC4ID();
  ...
  if (id)
    return C4VInt(Game.Players.Get(iPlr)->HomeBaseMaterial.GetIDCount(id));
  // Search indexed item of given category, return C4ID
  return C4VID(Game.Players.Get(iPlr)
                   ->HomeBaseMaterial.GetID(Game.Defs, dwCategory, iIndex));
}

static C4Value FnGetHomebaseProduction_C4V(C4AulContext *cthr,
                                           C4Value *iPlr_C4V, C4Value *id_C4V,
                                           C4Value *iIndex_C4V,
                                           C4Value *dwCategory_C4V) {
  long iPlr = iPlr_C4V->getInt();
  C4ID id = id_C4V->getC4ID();
  ...
  // Search by id, return available count
  if (id)
    return C4VInt(Game.Players.Get(iPlr)->HomeBaseProduction.GetIDCount(id));
  // Search indexed item of given category, return C4ID
  return C4VID(Game.Players.Get(iPlr)
                   ->HomeBaseProduction.GetID(Game.Defs, dwCategory, iIndex));
}

static long FnSetPlrMagic(C4AulContext *cthr, long iPlr, C4ID id,
                          bool fRemove) {
  C4Player *pPlr = Game.Players.Get(iPlr);
  if (!pPlr) return FALSE;
  if (fRemove) {
...
static C4Value FnGetPlrMagic_C4V(C4AulContext *cthr, C4Value *iPlr_C4V,
                                 C4Value *id_C4V, C4Value *iIndex_C4V) {
  long iPlr = iPlr_C4V->getInt();
  C4ID id = id_C4V->getC4ID();
  long iIndex = iIndex_C4V->getInt();
  ...
  // Search indexed item of given category, return C4ID
  return C4VID(
      Game.Players.Get(iPlr)->Magic.GetID(Game.Defs, C4D_Magic, iIndex));
}
...
static long FnEliminatePlayer(C4AulContext *cthr, long iPlr,
                              bool fRemoveDirect) {
  C4Player *pPlr = Game.Players.Get(iPlr);
  if (!pPlr) return FALSE;
  // direct removal?
...
static bool FnSetLeagueProgressData(C4AulContext *cthr, C4String *pNewData,
                                    long idPlayer) {
  if (!Game.Parameters.League.getLength()) return NULL;
  C4PlayerInfo *info = Game.PlayerInfos.GetPlayerInfoByID(idPlayer);
  if (!info) return false;
  info->SetLeagueProgressData(pNewData ? pNewData->Data.getData() : NULL);
  return TRUE;
}
...
static const int32_t CSPF_FixedAttributes = 1 << 0,
                     ...
                     CSPF_NoEliminationCheck = 1 << 2, CSPF_Invisible = 1 << 3;

static bool FnCreateScriptPlayer(C4AulContext *cthr, C4String *szName,
                                 long dwColor, long idTeam, long dwFlags,
                                 C4ID idExtra) {
  // safety
  ...
  // processed by control host only - clients/replay/etc. will perform the join
  // via queue
  if (!Game.Control.isCtrlHost()) return true;
  C4PlayerInfo *pScriptPlrInfo = new C4PlayerInfo();
  uint32_t dwInfoFlags = 0u;
  if (dwFlags & CSPF_FixedAttributes)
    dwInfoFlags |= C4PlayerInfo::PIF_AttributesFixed;
  if (dwFlags & CSPF_NoScenarioInit)
    dwInfoFlags |= C4PlayerInfo::PIF_NoScenarioInit;
  if (dwFlags & CSPF_NoEliminationCheck)
    dwInfoFlags |= C4PlayerInfo::PIF_NoEliminationCheck;
  if (dwFlags & CSPF_Invisible) dwInfoFlags |= C4PlayerInfo::PIF_Invisible;
  pScriptPlrInfo->SetAsScriptPlayer(szName->Data.getData(), dwColor,
                                    dwInfoFlags, idExtra);
...
static bool FnSetCursor(C4AulContext *cthr, long iPlr, C4Object *pObj,
                        bool fNoSelectMark, bool fNoSelectArrow,
                        bool fNoSelectCrew) {
  C4Player *pPlr = Game.Players.Get(iPlr);
...
static bool FnSelectCrew(C4AulContext *cthr, long iPlr, C4Object *pObj,
                         bool fSelect, bool fNoCursorAdjust) {
  C4Player *pPlr = Game.Players.Get(iPlr);
...
static long FnSetCrewStatus(C4AulContext *cthr, long iPlr, bool fInCrew,
                            C4Object *pObj) {
  // validate player
  C4Player *pPlr = Game.Players.Get(iPlr);
  if (!pPlr) return FALSE;
...
static long FnGetSeason(C4AulContext *cthr) { return Game.Weather.GetSeason(); }
...
static bool FnSetSkyFade(C4AulContext *cthr, long iFromRed, long iFromGreen,
                         long iFromBlue, long iToRed, long iToGreen,
                         long iToBlue) {
  // newgfx: set modulation
  DWORD dwBack,
      dwMod = GetClrModulation(Game.Landscape.Sky.FadeClr1,
                               C4RGB(iFromRed, iFromGreen, iFromBlue), dwBack);
  Game.Landscape.Sky.SetModulation(dwMod, dwBack);
  return TRUE;
}

static bool FnSetSkyColor(C4AulContext *cthr, long iIndex, long iRed,
                          long iGreen, long iBlue) {
  // set first index only
  if (iIndex) return TRUE;
  // get color difference
  DWORD dwBack, dwMod = GetClrModulation(Game.Landscape.Sky.FadeClr1,
                                         C4RGB(iRed, iGreen, iBlue), dwBack);
  Game.Landscape.Sky.SetModulation(dwMod, dwBack);
  // success
  return TRUE;
}
...
static long FnLandscapeWidth(C4AulContext *cthr) { return GBackWdt; }

static long FnLandscapeHeight(C4AulContext *cthr) { return GBackHgt; }

static long FnLaunchLightning(C4AulContext *cthr, long x, long y, long xdir,
                              long xrange, long ydir, long yrange,
                              bool fDoGamma) {
...
static bool FnDigFree(C4AulContext *cthr, long x, long y, long rad,
                      bool fRequest) {
  Game.Landscape.DigFree(x, y, rad, fRequest, cthr->Obj);
  return 1;
}

static bool FnDigFreeRect(C4AulContext *cthr, long iX, long iY, long iWdt,
                          long iHgt, bool fRequest) {
  Game.Landscape.DigFreeRect(iX, iY, iWdt, iHgt, fRequest, cthr->Obj);
  return TRUE;
}

static bool FnFreeRect(C4AulContext *cthr, long iX, long iY, long iWdt,
                       long iHgt, long iFreeDensity) {
  if (iFreeDensity)
...
static C4Value FnPathFree2_C4V(C4AulContext *cthr, C4Value *X1, C4Value *Y1,
                               C4Value *X2, C4Value *Y2) {
  int32_t x = -1, y = -1;
  // Do not use getInt on the references, because it destroys them.
  bool r = !!PathFree(X1->GetRefVal().getInt(), Y1->GetRefVal().getInt(),
                      X2->getInt(), Y2->getInt(), &x, &y);
  if (!r) {
    *X1 = C4VInt(x);
    *Y1 = C4VInt(y);
  }
...
static long FnSetTransferZone(C4AulContext *cthr, long iX, long iY, long iWdt,
                              long iHgt, C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  iX += pObj->x;
  iY += pObj->y;
...
static bool FnNot(C4AulContext *cthr, bool fCondition) { return !fCondition; }

static bool FnOr(C4AulContext *cthr, bool fCon1, bool fCon2, bool fCon3,
                 bool fCon4, bool fCon5) {
...
static long FnSum(C4AulContext *cthr, long iVal1, long iVal2, long iVal3,
                  long iVal4) {
  return (iVal1 + iVal2 + iVal3 + iVal4);
}

static long FnSub(C4AulContext *cthr, long iVal1, long iVal2, long iVal3,
                  long iVal4) {
  return (iVal1 - iVal2 - iVal3 - iVal4);
}

static long FnAbs(C4AulContext *cthr, long iVal) { return Abs(iVal); }
...
static long FnSin(C4AulContext *cthr, long iAngle, long iRadius, long iPrec) {
  if (!iPrec) iPrec = 1;
  // Precalculate the modulo operation so the C4Fixed argument to Sin does not
  // overflow
  iAngle %= 360 * iPrec;
  // Let itofix and fixtoi handle the division and multiplication because that
  // can handle higher ranges
...
static long FnAngle(C4AulContext *cthr, long iX1, long iY1, long iX2, long iY2,
                    long iPrec) {
  long iAngle;
  ...
  iAngle = static_cast<long>(
      180.0 * iPrec *
      atan2(static_cast<double>(Abs(dy)), static_cast<double>(Abs(dx))) / pi);

  if (iX2 > iX1) {
    if (iY2 < iY1)
      iAngle = (90 * iPrec) - iAngle;
    else
      iAngle = (90 * iPrec) + iAngle;
  } else {
    if (iY2 < iY1)
      iAngle = (270 * iPrec) + iAngle;
    else
      iAngle = (270 * iPrec) - iAngle;
  }
...
static long FnDistance(C4AulContext *cthr, long iX1, long iY1, long iX2,
                       long iY2) {
  return Distance(iX1, iY1, iX2, iY2);
}

static long FnObjectDistance(C4AulContext *cthr, C4Object *pObj2,
                             C4Object *pObj) {
  if (!pObj) pObj = cthr->Obj;
...
static long FnBoundBy(C4AulContext *cthr, long iVal, long iRange1,
                      long iRange2) {
  return BoundBy(iVal, iRange1, iRange2);
}

static bool FnInside(C4AulContext *cthr, long iVal, long iRange1,
                     long iRange2) {
  return Inside(iVal, iRange1, iRange2);
}

static long FnSEqual(C4AulContext *cthr, C4String *szString1,
                     C4String *szString2) {
  if (szString1 == szString2) return TRUE;
  return SEqual(FnStringPar(szString1), FnStringPar(szString2));
}

static long FnRandom(C4AulContext *cthr, long iRange) { return Random(iRange); }

static long FnAsyncRandom(C4AulContext *cthr, long iRange) {
  return SafeRandom(iRange);
}

static C4Value FnSetVar_C4V(C4AulContext *cthr, C4Value *iVarIndex,
                            C4Value *iValue) {
...
static C4Value FnVar_C4V(C4AulContext *cthr, C4Value *iVarIndex) {
  if (!cthr->Caller) return C4VNull;
  // Referenz zurÃ¼ckgeben
  return cthr->Caller->NumVars[iVarIndex->getInt()].GetRef();
}

static C4Value FnSetGlobal_C4V(C4AulContext *cthr, C4Value *iVarIndex,
                               C4Value *iValue) {
...
static C4Value FnSetLocal_C4V(C4AulContext *cthr, C4Value *iVarIndex,
                              C4Value *iValue, C4Value *pObj_C4V) {
  C4Object *pObj = pObj_C4V->getObj();

  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return C4VBool(FALSE);
  pObj->Local[iVarIndex->getInt()] = *iValue;
  return *iValue;
}

static C4Value FnLocal_C4V(C4AulContext *cthr, C4Value *iVarIndex,
                           C4Value *pObj_C4V) {
  C4Object *pObj = pObj_C4V->getObj();
...
static C4Value FnCall_C4V(C4AulContext *cthr, C4Value *szFunction_C4V,
                          C4Value *par0, C4Value *par1, C4Value *par2,
                          C4Value *par3, C4Value *par4, C4Value *par5,
                          C4Value *par6, C4Value *par7,
                          C4Value *par8 /*, C4Value* par9*/) {
  // safety
...
static C4Value FnObjectCall_C4V(
    C4AulContext *cthr, C4Value *pObj_C4V, C4Value *szFunction_C4V,
    C4Value *par0, C4Value *par1, C4Value *par2, C4Value *par3, C4Value *par4,
    C4Value *par5, C4Value *par6,
    C4Value *par7 /*, C4Value* par8, C4Value* par9*/) {
  C4Object *pObj = pObj_C4V->getObj();
  C4String *szFunction = szFunction_C4V->getStr();
  ...
  // get func
  C4AulFunc *f;
  if (!(f = pObj->Def->Script.GetSFunc(FnStringPar(szFunction), AA_PUBLIC,
                                       TRUE)))
    return C4Value();
  // copy pars
  C4AulParSet Pars;
...
static C4Value FnDefinitionCall_C4V(
    C4AulContext *cthr, C4Value *idID_C4V, C4Value *szFunction_C4V,
    C4Value *par0, C4Value *par1, C4Value *par2, C4Value *par3, C4Value *par4,
    C4Value *par5, C4Value *par6,
    C4Value *par7 /*, C4Value* par8, C4Value* par9*/) {
...
static C4Value FnGameCall_C4V(C4AulContext *cthr, C4Value *szFunction_C4V,
                              C4Value *par0, C4Value *par1, C4Value *par2,
                              C4Value *par3, C4Value *par4, C4Value *par5,
                              C4Value *par6, C4Value *par7,
                              C4Value *par8 /*, C4Value* par9*/) {
...
static C4Value FnGameCallEx_C4V(C4AulContext *cthr, C4Value *szFunction_C4V,
                                C4Value *par0, C4Value *par1, C4Value *par2,
                                C4Value *par3, C4Value *par4, C4Value *par5,
                                C4Value *par6, C4Value *par7,
                                C4Value *par8 /*, C4Value* par9*/) {
...
static C4Value FnProtectedCall_C4V(
    C4AulContext *cthr, C4Value *pObj_C4V, C4Value *szFunction_C4V,
    C4Value *par0, C4Value *par1, C4Value *par2, C4Value *par3, C4Value *par4,
    C4Value *par5, C4Value *par6,
    C4Value *par7 /*, C4Value* par8, C4Value* par9*/) {
  ...
  // get func
  C4AulScriptFunc *f;
  if (!(f = pObj->Def->Script.GetSFunc(FnStringPar(szFunction), AA_PROTECTED,
                                       TRUE)))
    return C4Value();
  // copy parameters
  C4AulParSet Pars;
...
static C4Value FnPrivateCall_C4V(
    C4AulContext *cthr, C4Value *pObj_C4V, C4Value *szFunction_C4V,
    C4Value *par0, C4Value *par1, C4Value *par2, C4Value *par3, C4Value *par4,
    C4Value *par5, C4Value *par6,
    C4Value *par7 /*, C4Value* par8, C4Value* par9*/) {
  ...
  // get func
  C4AulScriptFunc *f;
  if (!(f = pObj->Def->Script.GetSFunc(FnStringPar(szFunction), AA_PRIVATE,
                                       TRUE)))
    return C4Value();
  // copy parameters
  C4AulParSet Pars;
...
static bool FnIsNetwork(C4AulContext *cthr) {
  return Game.Parameters.IsNetworkGame;
}
...
static bool FnTestMessageBoard(C4AulContext *cthr, long iForPlr,
                               bool fTestIfInUse) {
...
static bool FnCallMessageBoard(C4AulContext *cthr, C4Object *pObj,
                               bool fUpperCase, C4String *szQueryString,
                               long iForPlr) {
  if (!pObj) pObj = cthr->Obj;
  if (pObj && !pObj->Status) return FALSE;
  // check player
  ...
  if (!pPlr) return FALSE;
  // remove any previous
  pPlr->CallMessageBoard(pObj, StdStrBuf(FnStringPar(szQueryString)),
                         !!fUpperCase);
  return TRUE;
}

static bool FnAbortMessageBoard(C4AulContext *cthr, C4Object *pObj,
                                long iForPlr) {
  if (!pObj) pObj = cthr->Obj;
  // check player
  C4Player *pPlr = Game.Players.Get(iForPlr);
...
static bool FnOnMessageBoardAnswer(C4AulContext *cthr, C4Object *pObj,
                                   long iForPlr, C4String *szAnswerString) {
  // remove query
  ...
  if (pObj)
    ...
    scr = &Game.Script;
  // exec func
  return !!scr->ObjectCall(
      NULL, pObj, PSF_InputCallback,
      &C4AulParSet(C4VString(FnStringPar(szAnswerString)), C4VInt(iForPlr)));
}

static long FnScriptCounter(C4AulContext *cthr) { return Game.Script.Counter; }
...
static long FnSetPlrViewRange(C4AulContext *cthr, long iRange, C4Object *pObj,
                              bool fExact) {
  // local/safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static long FnSetPicture(C4AulContext *cthr, long iX, long iY, long iWdt,
                         long iHgt, C4Object *pObj) {
  // local/safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static C4Object *FnBuy(C4AulContext *cthr, C4ID idBuyObj, long iForPlr,
                       long iPayPlr, C4Object *pToBase, bool fShowErrors) {
  // safety
  if (!ValidPlr(iForPlr) || !ValidPlr(iPayPlr)) return NULL;
  // buy
  C4Object *pThing;
  if (!(pThing = Game.Players.Get(iPayPlr)->Buy(idBuyObj, fShowErrors, iForPlr,
                                                pToBase ? pToBase : cthr->Obj)))
    return NULL;
...
static C4Value FnInc(C4AulContext *cthr, C4Value *Value, C4Value *Diff) {
  if (!Value->GetRefVal().ConvertTo(C4V_Int)) return C4Value();
...
static C4Value FnGetLength(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (pArray) return C4VInt(pArray->GetSize());
  C4String *pStr = pPars->getStr();
  if (pStr) return C4VInt(pStr->Data.getLength());
  throw new C4AulExecError(
      cthr->Obj,
      "func \"GetLength\" par 0 cannot be converted to string or array");
}
...
static C4Value FnGetIndexOf(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (!pArray)
    throw new C4AulExecError(
        cthr->Obj, "func \"GetIndexOf\" par 1 cannot be converted to array");
  // find the element by comparing data only - this may result in bogus results
  // if an object ptr array is searched for an int
...
static C4Value FnSetLength(C4AulContext *cthr, C4Value *pPars) {
  ...
  if (iNewSize < 0 || iNewSize > C4ValueList::MaxSize)
    throw new C4AulExecError(
        cthr->Obj,
        FormatString("SetLength: invalid array size (%d)", iNewSize).getData());
...
static bool FnSetVisibility(C4AulContext *cthr, long iVisibility,
                            C4Object *pObj) {
  // local call/safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static bool FnSetClrModulation(C4AulContext *cthr, long dwClr, C4Object *pObj,
                               long iOverlayID) {
  // local call/safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  ...
  if (iOverlayID) {
    ...
    if (!pOverlay) {
      DebugLogF("SetClrModulation: Overlay %d not defined for object %d (%s)",
                (int)iOverlayID, (int)pObj->Number,
...
static long FnGetClrModulation(C4AulContext *cthr, C4Object *pObj,
                               long iOverlayID) {
  // local call/safety
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
  ...
  if (iOverlayID) {
    C4GraphicsOverlay *pOverlay = pObj->GetGraphicsOverlay(iOverlayID, false);
    if (!pOverlay) {
      DebugLogF("GetClrModulation: Overlay %d not defined for object %d (%s)",
                (int)iOverlayID, (int)pObj->Number,
                (const char *)pObj->GetName());
      return FALSE;
    }
...
static bool FnGetMissionAccess(C4AulContext *cthr, C4String *strMissionAccess) {
  ...
  if (Game.Control.SyncMode())
    Log("Warning: using GetMissionAccess may cause desyncs when playing "
        "records!");
...
class C4ValueCompiler : public StdCompiler {
 ...
 public:
  ...
  virtual void Boolean(bool &rBool) {
    if (haveCompleteMatch())
      if (!iEntryNr--) ProcessBool(rBool);
  }
  virtual void Character(char &rChar) {
    if (haveCompleteMatch())
      if (!iEntryNr--) ProcessChar(rChar);
  }

  // The C4ID-Adaptor will set RCT_ID for it's strings (see C4Id.h), so we don't
  // have to guess the type.
  virtual void String(char *szString, size_t iMaxLength, RawCompileType eType) {
    if (haveCompleteMatch())
      if (!iEntryNr--)
        ProcessString(szString, iMaxLength, eType == StdCompiler::RCT_ID);
  }
  virtual void String(char **pszString, RawCompileType eType) {
    if (haveCompleteMatch())
      if (!iEntryNr--) ProcessString(pszString, eType == StdCompiler::RCT_ID);
  }
  virtual void Raw(
      void *pData, size_t iSize,
      RawCompileType eType = RCT_Escaped) { /* C4Script can't handle this */
  }
 ...
 private:
  // match active?
  bool haveCurrentMatch() const {
    return iDepth + 1 == iMatchStart + iMatchCount;
  }
  // match complete?
  bool haveCompleteMatch() const {
...
class C4ValueGetCompiler : public C4ValueCompiler {
 ...
 protected:
  ...
  virtual void ProcessChar(char &rChar) {
    Res = C4VString(FormatString("%c", rChar));
  }
...
class C4ValueSetCompiler : public C4ValueCompiler {
 ...
 public:
  C4ValueSetCompiler(const char **pszNames, int iNameCnt, int iEntryNr,
                     const C4Value &rSetVal)
      : C4ValueCompiler(pszNames, iNameCnt, iEntryNr),
        Val(rSetVal),
        fSuccess(false),
        iRuntimeWriteAllowed(0) {}

  // Query successful setting
  bool getSuccess() const { return fSuccess; }

  virtual void setRuntimeWritesAllowed(int32_t iChange) {
    iRuntimeWriteAllowed += iChange;
  }

 protected:
  // set values as C4Value, only if type matches or is convertible
  virtual void ProcessInt(int32_t &rInt) {
    if (iRuntimeWriteAllowed > 0 && Val.ConvertTo(C4V_Int)) {
      rInt = Val.getInt();
      fSuccess = true;
    }
  }
  virtual void ProcessBool(bool &rBool) {
    if (iRuntimeWriteAllowed > 0 && Val.ConvertTo(C4V_Bool)) {
      rBool = Val.getBool();
      fSuccess = true;
    }
  }
  virtual void ProcessChar(char &rChar) {
    C4String *s;
    if (iRuntimeWriteAllowed > 0 && (s = Val.getStr())) {
      rChar = s->Data.getData() ? *s->Data.getData() : 0;
      fSuccess = true;
    }
  }

  virtual void ProcessString(char *szString, size_t iMaxLength, bool fIsID) {
    if (iRuntimeWriteAllowed <= 0 ||
        !Val.ConvertTo(fIsID ? C4V_C4ID : C4V_String))
      return;
    if (fIsID) {
      assert(iMaxLength >= 4);  // fields that should carry an ID are guaranteed
                                // to have a buffer that's large enough
      ...
      if (rsBuf.getData())
        SCopy(rsBuf.getData(), szString, iMaxLength);
      else
        *szString = 0;
    }
    fSuccess = true;
  ...
  virtual void ProcessString(char **pszString, bool fIsID) {
    if (iRuntimeWriteAllowed <= 0 ||
        !Val.ConvertTo(fIsID ? C4V_C4ID : C4V_String))
      return;
    // This cannot be allowed, because it is run during decompilation and
    // wouldn't update assiciated length fields in StdStrBuf!
    /*
    assert(pszString);
    if (fIsID)
            {
...
template <class T>
C4Value GetValByStdCompiler(const char *strEntry, const char *strSection,
                            int iEntryNr, const T &rFrom) {
  // Set up name array, create compiler
  const char *szNames[2] = {strSection ? strSection : strEntry,
                            strSection ? strEntry : NULL};
...
template <class T>
bool SetValByStdCompiler(const char *strEntry, const char *strSection,
                         int iEntryNr, const T &rTo, const C4Value &rvNewVal) {
  // Set up name array, create compiler
  const char *szNames[2] = {strSection ? strSection : strEntry,
                            strSection ? strEntry : NULL};
...
static C4Value FnGetDefCoreVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                               C4Value *strSection_C4V, C4Value *idDef_C4V,
                               C4Value *iEntryNr_C4V) {
  ...
  if (!idDef)
    if (cthr->Def) idDef = cthr->Def->id;
  if (!idDef) return C4Value();

  C4Def *pDef = C4Id2Def(idDef);
  if (!pDef) return C4Value();

  return GetValByStdCompiler(strEntry, strSection, iEntryNr,
                             mkNamingAdapt(*pDef, "DefCore"));
}

static C4Value FnGetObjectVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                              C4Value *strSection_C4V, C4Value *pObj_C4V,
                              C4Value *iEntryNr_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
  const char *strSection = FnStringPar(strSection_C4V->getStr());
  ...
  // get value
  return GetValByStdCompiler(strEntry, strSection, iEntryNr,
                             mkNamingAdapt(*pObj, "Object"));
}

static C4Value FnGetObjectInfoCoreVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                                      C4Value *strSection_C4V,
                                      C4Value *pObj_C4V,
                                      C4Value *iEntryNr_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
  const char *strSection = FnStringPar(strSection_C4V->getStr());
  ...
  // get value
  return GetValByStdCompiler(strEntry, strSection, iEntryNr,
                             mkNamingAdapt(*pObjInfoCore, "ObjectInfo"));
}

static C4Value FnGetActMapVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                              C4Value *strAction_C4V, C4Value *idDef_C4V,
                              C4Value *iEntryNr_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
  ...
  for (iAct = 0; iAct < pDef->ActNum; iAct++, pAct++)
    if (SEqual(pAct->Name, strAction)) break;

  // not found?
  if (iAct >= pDef->ActNum) return C4Value();

  // get value
  return GetValByStdCompiler(strEntry, NULL, iEntryNr, *pAct);
}

static C4Value FnGetScenarioVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                                C4Value *strSection_C4V,
                                C4Value *iEntryNr_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
  const char *strSection = FnStringPar(strSection_C4V->getStr());
  ...
  return GetValByStdCompiler(strEntry, strSection, iEntryNr,
                             mkParAdapt(Game.C4S, false));
}

static C4Value FnSetScenarioVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                                C4Value *strSection_C4V, C4Value *iEntryNr_C4V,
                                C4Value *setVal_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
  const char *strSection = FnStringPar(strSection_C4V->getStr());
  long iEntryNr = iEntryNr_C4V->getInt();
  ...
  return C4VBool(SetValByStdCompiler(strEntry, strSection, iEntryNr,
                                     mkParAdapt(Game.C4S, false), setVal));
}

static C4Value FnGetPlayerVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                              C4Value *strSection_C4V, C4Value *iPlayer_C4V,
                              C4Value *iEntryNr_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
  const char *strSection = FnStringPar(strSection_C4V->getStr());
  if (strSection && !*strSection) strSection = NULL;
  ...
  // get value
  return GetValByStdCompiler(strEntry, strSection, iEntryNr,
                             mkNamingAdapt(*pPlayer, "Player"));
}

static C4Value FnGetPlayerInfoCoreVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                                      C4Value *strSection_C4V,
                                      C4Value *iPlayer_C4V,
                                      C4Value *iEntryNr_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
...
static C4Value FnGetMaterialVal(C4AulContext *cthr, C4Value *strEntry_C4V,
                                C4Value *strSection_C4V, C4Value *iMat_C4V,
                                C4Value *iEntryNr_C4V) {
  const char *strEntry = FnStringPar(strEntry_C4V->getStr());
  const char *strSection = FnStringPar(strSection_C4V->getStr());
  if (strSection && !*strSection) strSection = NULL;
...
static bool FnResortObjects(C4AulContext *cthr, C4String *szFunc,
                            long Category) {
  // safety
  ...
  if (!pFn)
    throw new C4AulExecError(
        cthr->Obj, FormatString("ResortObjects: Resort function %s not found",
                                FnStringPar(szFunc)).getData());
...
static bool FnResortObject(C4AulContext *cthr, C4String *szFunc,
                           C4Object *pObj) {
  // safety
  if (!szFunc) return FALSE;
  if (!cthr->Caller) return FALSE;
...
static bool FnSetGraphics(C4AulContext *pCtx, C4String *pGfxName,
                          C4Object *pObj, C4ID idSrcGfx, long iOverlayID,
                          long iOverlayMode, C4String *pAction, long dwBlitMode,
                          C4Object *pOverlayObject) {
  // safety
  if (!pObj)
    if (!(pObj = pCtx->Obj)) return FALSE;
  if (!pObj->Status) return FALSE;
  // get def for source graphics
  C4Def *pSrcDef = NULL;
  if (idSrcGfx)
    if (!(pSrcDef = C4Id2Def(idSrcGfx))) return FALSE;
  // setting overlay?
  if (iOverlayID) {
...
static long FnGetDefBottom(C4AulContext *cthr, C4Object *pObj) {
  if (!pObj)
    if (!(pObj = cthr->Obj)) return 0;
  return pObj->y + pObj->Def->Shape.y + pObj->Def->Shape.Hgt;
}

static bool FnSetMaterialColor(C4AulContext *cthr, long iMat, long iClr1R,
                               long iClr1G, long iClr1B, long iClr2R,
                               long iClr2G, long iClr2B, long iClr3R,
                               long iClr3G, long iClr3B) {
  // get mat
  ...
  DWORD dwBack, dwMod = GetClrModulation(
                    C4RGB(pMat->Color[0], pMat->Color[1], pMat->Color[2]),
                    C4RGB(iClr1R, iClr1G, iClr1B), dwBack);
  dwMod &= 0xffffff;
  if (!dwMod) dwMod = 1;
...
static bool FnSetMaterialColor2(C4AulContext *cthr, long iMat, long iClrR,
                                long iClrG, long iClrB) {
  // get mat
  if (!MatValid(iMat)) return FALSE;
  C4Material *pMat = &Game.Material.Map[iMat];
  // newgfx: emulate by landscape modulation - enlightment not allowed...
  pMat->Color[0 + 0] = iClrR;
  pMat->Color[0 + 1] = iClrG;
  pMat->Color[0 + 2] = iClrB;
  // done
  return TRUE;
}

static long FnGetMaterialColor(C4AulContext *cthr, long iMat, long iNum,
                               long iChannel) {
  // get mat
  if (!MatValid(iMat)) return FALSE;
  C4Material *pMat = &Game.Material.Map[iMat];
...
static bool FnSetMenuSize(C4AulContext *cthr, long iCols, long iRows,
                          C4Object *pObj) {
  // get object
  if (!pObj) pObj = cthr->Obj;
  if (!pObj) return FALSE;
...
static C4Value FnEval(C4AulContext *cthr, C4Value *strScript_C4V) {
  ...
  if (cthr->Caller) Strict = cthr->Caller->Func->pOrgScript->Strict;
  if (cthr->Obj)
    return cthr->Obj->Def->Script.DirectExec(
        cthr->Obj, FnStringPar(strScript_C4V->getStr()), "eval", true, Strict);
  else if (cthr->Def)
    return cthr->Def->Script.DirectExec(0, FnStringPar(strScript_C4V->getStr()),
                                        "eval", true, Strict);
  else
    return Game.Script.DirectExec(0, FnStringPar(strScript_C4V->getStr()),
                                  "eval", true, Strict);
}

static bool FnLocateFunc(C4AulContext *cthr, C4String *funcname, C4Object *pObj,
                         C4ID idDef) {
  // safety
  ...
  if (pObj) {
    ...
    C4Def *pDef = C4Id2Def(idDef);
    if (!pDef) {
      Log("Invalid or unloaded def");
      return false;
    }
    pCheckScript = &pDef->Script;
  } else {
    if (!cthr || !cthr->Caller || !cthr->Caller->Func ||
        !cthr->Caller->Func->Owner) {
  ...
  if (!pFunc) {
    ...
    while (pFunc) {
      ...
      if (!pSFunc) {
        ...
        int32_t iLine =
            SGetLine(pSFunc->pOrgScript->GetScript(), pSFunc->Script);
        LogF("%s%s (%s:%d)", szPrefix, pFunc->Name,
             pSFunc->pOrgScript->ScriptName.getData(), (int)iLine);
      }
...
static C4Value FnSetPlrExtraData(C4AulContext *cthr, C4Value *iPlayer_C4V,
                                 C4Value *strDataName_C4V, C4Value *Data) {
  ...
  if (Data->GetType() != C4V_Any && Data->GetType() != C4V_Int &&
      Data->GetType() != C4V_Bool && Data->GetType() != C4V_C4ID)
    return C4VNull;
...
static C4Value FnGetPlrExtraData(C4AulContext *cthr, C4Value *iPlayer_C4V,
                                 C4Value *strDataName_C4V) {
  long iPlayer = iPlayer_C4V->getInt();
  const char *strDataName = FnStringPar(strDataName_C4V->getStr());
  // valid player?
  if (!ValidPlr(iPlayer)) return C4Value();
...
static C4Value FnSetCrewExtraData(C4AulContext *cthr, C4Value *pCrew_C4V,
                                  C4Value *strDataName_C4V, C4Value *Data) {
...
static C4Value FnGetCrewExtraData(C4AulContext *cthr, C4Value *pCrew_C4V,
                                  C4Value *strDataName_C4V) {
  C4Object *pCrew = pCrew_C4V->getObj();
  if (!pCrew) pCrew = cthr->Obj;
  const char *strDataName = FnStringPar(strDataName_C4V->getStr());
...
static long FnDrawMatChunks(C4AulContext *cctx, long tx, long ty, long twdt,
                            long thgt, long icntx, long icnty,
                            C4String *strMaterial, C4String *strTexture,
                            bool bIFT) {
  return Game.Landscape.DrawChunks(tx, ty, twdt, thgt, icntx, icnty,
                                   FnStringPar(strMaterial),
                                   FnStringPar(strTexture), bIFT != 0);
}
...
static bool FnSetCrewEnabled(C4AulContext *cctx, bool fEnabled,
                             C4Object *pObj) {
  // local/safety
  if (!pObj) pObj = cctx->Obj;
  if (!pObj) return FALSE;
  ...
  if (!fEnabled) {
    ...
    if (pOwner = Game.Players.Get(pObj->Owner)) {
      // if viewed player cursor gets deactivated and no new cursor is found,
      // follow the old in target mode
      bool fWasCursorMode = (pOwner->ViewMode == C4PVM_Cursor);
      if (pOwner->Cursor == pObj) pOwner->AdjustCursorCommand();
...
static long FnDrawMap(C4AulContext *cctx, long iX, long iY, long iWdt,
                      long iHgt, C4String *szMapDef) {
  // draw it!
  return Game.Landscape.DrawMap(iX, iY, iWdt, iHgt, FnStringPar(szMapDef));
}

static long FnDrawDefMap(C4AulContext *cctx, long iX, long iY, long iWdt,
                         long iHgt, C4String *szMapDef) {
  // draw it!
...
DWORD FadeClr(DWORD dwClr1, DWORD dwClr2, BYTE byA1) {
  ...
  DarkenClrBy(dwClr1, 255 - byA1);
  DarkenClrBy(dwClr2, byA1);
...
static bool FnDrawModLandscape(C4AulContext *cctx, long iX, long iY, long iWdt,
                               long iHgt, C4Object *pObj) {
  // hardcoded function for LhÃ»nrim.c4s - make the map look beautiful
  // safety
  if (!pObj) {
    pObj = cctx->Obj;
    if (!pObj) return FALSE;
  ...
  // go through it horizontally and vertically in two directions, building a
  // distance-to-lower-mat-map
  signed char *map = new signed char[iWdt * iHgt];
  ZeroMemory(map, iWdt * iHgt);
  signed char *pZ;
  for (int d = 0; d <= 1; ++d)
    for (int i = 0; i <= 1; ++i)
      for (int x1 = d ? (i ? iX2 - 1 : iY2 - 1) : (i ? iX : iY);
           d ? (x1 >= (i ? iX : iY)) : (x1 < (i ? iX2 : iY2));
           d ? (--x1) : (++x1)) {
        long iLastPlacement = 0, z = 0;  // placement out of map
        for (int x2 = d ? (i ? iY2 - 1 : iX2 - 1) : (i ? iY : iX);
             d ? (x2 >= (i ? iY : iX)) : (x2 < (i ? iY2 : iX2));
             d ? (--x2) : (++x2)) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      for (int y = iY + 1; y < iY2 - 1; ++y) {
        for (int x = iX + 1; x < iX2 - 1; ++x) {
          ...
          if (iPlac1 == MatPlacement(GBackMat(x, y - 1)))
            SmoothChars(*pZ, *(pZ - iWdt));
          if (iPlac1 == MatPlacement(GBackMat(x, y + 1)))
            SmoothChars(*pZ, *(pZ + iWdt));
          if (iPlac1 == MatPlacement(GBackMat(x - 1, y)))
            SmoothChars(*pZ, *(pZ - 1));
          if (iPlac1 == MatPlacement(GBackMat(x + 1, y)))
            SmoothChars(*pZ, *(pZ + 1));
          ++pZ;
...
static bool FnCreateParticle(C4AulContext *cthr, C4String *szName, long iX,
                             long iY, long iXDir, long iYDir, long a, long b,
                             C4Object *pObj, bool fBack) {
  // safety
  if (pObj && !pObj->Status) return false;
  // local offset
  if (cthr->Obj) {
  ...
  Game.Particles.Create(
      pDef, (float)iX, (float)iY, (float)iXDir / 10.0f, (float)iYDir / 10.0f,
      (float)a / 10.0f, b,
      pObj ? (fBack ? &pObj->BackParticles : &pObj->FrontParticles) : NULL,
      pObj);
  // success, even if not created
  return TRUE;
}

static bool FnCastAParticles(C4AulContext *cthr, C4String *szName, long iAmount,
                             long iLevel, long iX, long iY, long a0, long a1,
                             long b0, long b1, C4Object *pObj, bool fBack) {
  // safety
  if (pObj && !pObj->Status) return false;
  // local offset
  if (cthr->Obj) {
  ...
  // cast
  Game.Particles.Cast(
      pDef, iAmount, (float)iX, (float)iY, iLevel, (float)a0 / 10.0f, b0,
      (float)a1 / 10.0f, b1,
      pObj ? (fBack ? &pObj->BackParticles : &pObj->FrontParticles) : NULL,
      pObj);
  // success, even if not created
...
static bool FnCastParticles(C4AulContext *cthr, C4String *szName, long iAmount,
                            long iLevel, long iX, long iY, long a0, long a1,
                            long b0, long b1, C4Object *pObj) {
  return FnCastAParticles(cthr, szName, iAmount, iLevel, iX, iY, a0, a1, b0, b1,
                          pObj, FALSE);
}

static bool FnCastBackParticles(C4AulContext *cthr, C4String *szName,
                                long iAmount, long iLevel, long iX, long iY,
...
static bool FnPushParticles(C4AulContext *cthr, C4String *szName, long iAX,
                            long iAY) {
  // particle given?
...
static bool FnClearParticles(C4AulContext *cthr, C4String *szName,
                             C4Object *pObj) {
  // particle given?
  C4ParticleDef *pDef = NULL;
  if (szName) {
...
static bool FnSetSkyParallax(C4AulContext *ctx, long iMode, long iParX,
                             long iParY, long iXDir, long iYDir, long iX,
                             long iY) {
...
static bool FnSetGamma(C4AulContext *ctx, long dwClr1, long dwClr2, long dwClr3,
                       long iRampIndex) {
...
static bool SumPathLength(int32_t iX, int32_t iY, intptr_t iTransferTarget,
                          intptr_t ipPathInfo) {
...
static long FnGetPathLength(C4AulContext *ctx, long iFromX, long iFromY,
                            long iToX, long iToY) {
  PathInfo PathInfo;
  PathInfo.ilx = iFromX;
  PathInfo.ily = iFromY;
  PathInfo.ilen = 0;
  if (!Game.PathFinder.Find(iFromX, iFromY, iToX, iToY, &SumPathLength,
                            (long)&PathInfo))
    return 0;
  return PathInfo.ilen + Distance(PathInfo.ilx, PathInfo.ily, iToX, iToY);
}

static long FnSetTextureIndex(C4AulContext *ctx, C4String *psMatTex,
                              long iNewIndex, bool fInsert) {
  if (!Inside(iNewIndex, 0l, 255l)) return FALSE;
  return Game.Landscape.SetTextureIndex(FnStringPar(psMatTex), BYTE(iNewIndex),
                                        !!fInsert);
}
...
static bool FnSetLandscapePixel(C4AulContext *ctx, long iX, long iY,
                                long dwValue) {
  // local call
  if (ctx->Obj) {
    iX += ctx->Obj->x;
...
static bool FnSetObjectOrder(C4AulContext *ctx, C4Object *pObjBeforeOrAfter,
                             C4Object *pSortObj, bool fSortAfter) {
  // local call/safety
  if (!pSortObj) pSortObj = ctx->Obj;
  if (!pSortObj) return FALSE;
  if (!pObjBeforeOrAfter) return FALSE;
  // note that no category check is done, so this call might corrupt the main
  // list!
  // the scripter must be wise enough not to call it for objects with different
  // categories
  // create object resort
  C4ObjResort *pObjRes = new C4ObjResort();
...
static bool FnDrawMaterialQuad(C4AulContext *ctx, C4String *szMaterial,
                               long iX1, long iY1, long iX2, long iY2, long iX3,
                               long iY3, long iX4, long iY4, bool fSub) {
  const char *szMat = FnStringPar(szMaterial);
  return !!Game.Landscape.DrawQuad(iX1, iY1, iX2, iY2, iX3, iY3, iX4, iY4,
                                   szMat, fSub);
}

static bool FnFightWith(C4AulContext *ctx, C4Object *pTarget,
                        C4Object *pClonk) {
  // local call/safety
  if (!pTarget) return FALSE;
  if (!pClonk)
    if (!(pClonk = ctx->Obj)) return FALSE;
  // check OCF
...
static bool FnSetFilmView(C4AulContext *ctx, long iToPlr) {
  ...
  // set new target plr
  if (C4Viewport *vp = Game.GraphicsSystem.GetFirstViewport())
    vp->Init(iToPlr, true);
...
static C4Object *FnGetObjectLayer(C4AulContext *ctx, C4Object *pObj) {
  // local call/safety
  if (!pObj)
    if (!(pObj = ctx->Obj)) return NULL;
  // get layer object
  return pObj->pLayer;
}

static bool FnSetObjectLayer(C4AulContext *ctx, C4Object *pNewLayer,
                             C4Object *pObj) {
  // local call/safety
  if (!pObj)
    if (!(pObj = ctx->Obj)) return FALSE;
  // set layer object
  pObj->pLayer = pNewLayer;
  // set for all contents as well
  for (C4ObjectLink *pLnk = pObj->Contents.First; pLnk; pLnk = pLnk->Next)
    if ((pObj = pLnk->Obj) && pObj->Status) pObj->pLayer = pNewLayer;
  // success
  return TRUE;
}

static bool FnSetShape(C4AulContext *ctx, long iX, long iY, long iWdt,
                       long iHgt, C4Object *pObj) {
  // local call / safety
  if (!pObj)
    if (!(pObj = ctx->Obj)) return FALSE;
...
static bool FnAddMsgBoardCmd(C4AulContext *ctx, C4String *pstrCommand,
                             C4String *pstrScript, long iRestriction) {
  // safety
  ...
  if (iRestriction != C4MessageBoardCommand::C4MSGCMDR_Identifier)
    if (!ctx->Caller || !*ctx->Caller->Func->Name) return FALSE;
  C4MessageBoardCommand::Restriction eRestriction;
  switch (iRestriction) {
    case C4MessageBoardCommand::C4MSGCMDR_Escaped:
      eRestriction = C4MessageBoardCommand::C4MSGCMDR_Escaped;
      break;
    case C4MessageBoardCommand::C4MSGCMDR_Plain:
      eRestriction = C4MessageBoardCommand::C4MSGCMDR_Plain;
      break;
    case C4MessageBoardCommand::C4MSGCMDR_Identifier:
      eRestriction = C4MessageBoardCommand::C4MSGCMDR_Identifier;
      break;
    default:
      return FALSE;
  }
  // add command
  Game.MessageInput.AddCommand(FnStringPar(pstrCommand),
                               FnStringPar(pstrScript), eRestriction);
  return TRUE;
}
...
static bool FnSetGameSpeed(C4AulContext *ctx, long iSpeed) {
  ...
  if (Game.Parameters.isLeague())
    if (!ctx->Caller || ctx->Caller->TemporaryScript) return false;
  // safety
  if (iSpeed)
    if (!Inside<long>(iSpeed, 0, 1000)) return FALSE;
  if (!iSpeed) iSpeed = 38;
...
static bool FnSetObjDrawTransform(C4AulContext *ctx, long iA, long iB, long iC,
                                  long iD, long iE, long iF, C4Object *pObj,
                                  long iOverlayID) {
  // local call / safety
  if (!pObj) {
    if (!(pObj = ctx->Obj)) return FALSE;
  ...
  // assign values
  pTransform->Set((float)iA / 1000, (float)iB / 1000, (float)iC / 1000,
                  (float)iD / 1000, (float)iE / 1000, (float)iF / 1000, 0, 0,
                  1);
  // done, success
  return TRUE;
}

static bool FnSetObjDrawTransform2(C4AulContext *ctx, long iA, long iB, long iC,
                                   long iD, long iE, long iF, long iG, long iH,
                                   long iI, long iOverlayID) {
  // local call / safety
...
#define L2F(l) ((float)l / 1000)
  ...
  matrix.Set(L2F(iA), L2F(iB), L2F(iC), L2F(iD), L2F(iE), L2F(iF), L2F(iG),
             L2F(iH), L2F(iI));
  *pTransform *= matrix;
#undef L2F
  // done, success
...
#define COPY_C4V_PAR(Var, Par, ParType, Std) \
  Var = (Par && Par->GetType() == ParType ? Par->GetData().Int : Std)

bool SimFlight(FIXED &x, FIXED &y, FIXED &xdir, FIXED &ydir,
               int32_t iDensityMin, int32_t iDensityMax, int32_t iIter);

static C4Value FnSimFlight(C4AulContext *ctx, C4Value *pvrX, C4Value *pvrY,
                           C4Value *pvrXDir, C4Value *pvrYDir,
                           C4Value *pviDensityMin, C4Value *pviDensityMax,
                           C4Value *pviIter, C4Value *pviPrec) {
  ...
  FIXED x = itofix(pvrX->GetData().Int), y = itofix(pvrY->GetData().Int),
        xdir = itofix(pvrXDir->GetData().Int, iPrec),
        ydir = itofix(pvrYDir->GetData().Int, iPrec);
  ...
  // write results back
  *pvrX = C4VInt(fixtoi(x));
  *pvrY = C4VInt(fixtoi(y));
  *pvrXDir = C4VInt(fixtoi(xdir * iPrec));
  *pvrYDir = C4VInt(fixtoi(ydir * iPrec));

  return C4VBool(true);
}
#undef COPY_C4V_PAR
static bool FnSetPortrait(C4AulContext *ctx, C4String *pstrPortrait,
                          C4Object *pTarget, C4ID idSourceDef, bool fPermanent,
                          bool fCopyGfx) {
  // safety
  const char *szPortrait;
  if (!pstrPortrait || !*(szPortrait = FnStringPar(pstrPortrait))) return FALSE;
  if (!pTarget)
    if (!(pTarget = ctx->Obj)) return FALSE;
  if (!pTarget->Status || !pTarget->Info) return FALSE;
  // special case: clear portrait
  if (SEqual(szPortrait, C4Portrait_None))
    return pTarget->Info->ClearPortrait(!!fPermanent);
  // get source def for portrait
  C4Def *pSourceDef;
  if (idSourceDef)
    pSourceDef = Game.Defs.ID2Def(idSourceDef);
  else
    pSourceDef = pTarget->Def;
  if (!pSourceDef) return FALSE;
  // special case: random portrait
  if (SEqual(szPortrait, C4Portrait_Random))
    return pTarget->Info->SetRandomPortrait(pSourceDef->id, !!fPermanent,
                                            !!fCopyGfx);
  // try to set portrait
  return pTarget->Info->SetPortrait(szPortrait, pSourceDef, !!fPermanent,
                                    !!fCopyGfx);
}

static C4Value FnGetPortrait(C4AulContext *ctx, C4Value *pvpObj,
                             C4Value *pvfGetID, C4Value *pvfGetPermanent) {
  // get parameters
  C4Object *pObj = pvpObj->getObj();
  bool fGetID = pvfGetID->getBool();
  bool fGetPermanent = pvfGetPermanent->getBool();
  // check valid object with info section
  if (!pObj)
    if (!(pObj = ctx->Obj)) return C4Value();
  ...
  if (fGetPermanent) {
    ...
    if (!(pPortrait = pObj->Info->pNewPortrait)) {
      ...
      szPortrait = C4Portrait::EvaluatePortraitString(
          szPortrait, idPortraitSource, pObj->Info->id, NULL);
...
static long FnLoadScenarioSection(C4AulContext *ctx, C4String *pstrSection,
                                  long dwFlags) {
  // safety
...
static bool FnSetObjectStatus(C4AulContext *ctx, long iNewStatus,
                              C4Object *pObj, bool fClearPointers) {
  // local call / safety
  if (!pObj) {
    if (!(pObj = ctx->Obj)) return FALSE;
  }
  ...
  switch (iNewStatus) {
    case C4OS_NORMAL:
      return pObj->StatusActivate();
      break;
    case C4OS_INACTIVE:
      return pObj->StatusDeactivate(!!fClearPointers);
      break;
    default:
...
static bool FnAdjustWalkRotation(C4AulContext *ctx, long iRangeX, long iRangeY,
                                 long iSpeed, C4Object *pObj) {
  // local call / safety
  if (!pObj) {
    if (!(pObj = ctx->Obj)) return FALSE;
  }
  // must be rotateable and attached to solid ground
  if (!pObj->Def->Rotateable || ~pObj->Action.t_attach & CNAT_Bottom ||
      pObj->Shape.AttachMat == MNone)
    return FALSE;
  // adjust rotation
  return pObj->AdjustWalkRotation(iRangeX, iRangeY, iSpeed);
}

static C4Value FnAddEffect_C4V(C4AulContext *ctx, C4Value *pvsEffectName,
                               C4Value *pvpTarget, C4Value *pviPrio,
                               C4Value *pviTimerIntervall,
                               C4Value *pvpCmdTarget, C4Value *pvidCmdTarget,
                               C4Value *pvVal1, C4Value *pvVal2,
                               C4Value *pvVal3, C4Value *pvVal4) {
  // evaluate parameters
  ...
  new C4Effect(pTarget, szEffect, iPrio, iTimerIntervall, pCmdTarget,
               idCmdTarget, *pvVal1, *pvVal2, *pvVal3, *pvVal4, true,
               iEffectNumber);
  // return assigned effect number - may be 0 if he effect has been denied by
  // another effect
  // may also be the number of another effect
  return C4VInt(iEffectNumber);
}

static C4Value FnGetEffect_C4V(C4AulContext *ctx, C4Value *pvsEffectName,
                               C4Value *pvpTarget, C4Value *pviIndex,
                               C4Value *pviQueryValue,
                               C4Value *pviMaxPriority) {
  // evaluate parameters
  C4String *psEffectName = pvsEffectName->getStr();
  C4Object *pTarget = pvpTarget->getObj();
  long iIndex = pviIndex->getInt(), iQueryValue = pviQueryValue->getInt(),
       iMaxPriority = pviMaxPriority->getInt();
  const char *szEffect = FnStringPar(psEffectName);
  // get effects
  ...
  switch (iQueryValue) {
    case 0:
      return C4VInt(pEffect->iNumber);  // 0: number
    case 1:
      return C4VString(pEffect->Name);  // 1: name
    case 2:
      return C4VInt(Abs(pEffect->iPriority));  // 2: priority (may be negative
                                               // for deactivated effects)
    case 3:
      return C4VInt(pEffect->iIntervall);  // 3: timer intervall
    case 4:
      return C4VObj(pEffect->pCommandTarget);  // 4: command target
    case 5:
      return C4VID(pEffect->idCommandTarget);  // 5: command target ID
    case 6:
      return C4VInt(pEffect->iTime);  // 6: effect time
  }
  // invalid data queried
  return C4Value();
}

static bool FnRemoveEffect(C4AulContext *ctx, C4String *psEffectName,
                           C4Object *pTarget, long iIndex, bool fDoNoCalls) {
  // evaluate parameters
  const char *szEffect = FnStringPar(psEffectName);
  // get effects
  C4Effect *pEffect = pTarget ? pTarget->pEffects : Game.pGlobalEffects;
...
static bool FnChangeEffect(C4AulContext *ctx, C4String *psEffectName,
                           C4Object *pTarget, long iIndex,
                           C4String *psNewEffectName, long iNewTimer) {
  // evaluate parameters
  const char *szEffect = FnStringPar(psEffectName);
...
static C4Value FnCheckEffect_C4V(C4AulContext *ctx, C4Value *pvsEffectName,
                                 C4Value *pvpTarget, C4Value *pviPrio,
                                 C4Value *pviTimerIntervall, C4Value *pvVal1,
                                 C4Value *pvVal2, C4Value *pvVal3,
                                 C4Value *pvVal4) {
  // evaluate parameters
  C4String *psEffectName = pvsEffectName->getStr();
  C4Object *pTarget = pvpTarget->getObj();
  ...
  if (!pEffect) return C4Value();
  // let them check
  return C4VInt(pEffect->Check(pTarget, szEffect, iPrio, iTimerIntervall,
                               *pvVal1, *pvVal2, *pvVal3, *pvVal4));
}

static long FnGetEffectCount(C4AulContext *ctx, C4String *psEffectName,
                             C4Object *pTarget, long iMaxPriority) {
  // evaluate parameters
  const char *szEffect = FnStringPar(psEffectName);
  // get effects
  C4Effect *pEffect = pTarget ? pTarget->pEffects : Game.pGlobalEffects;
...
static C4Value FnEffectVar_C4V(C4AulContext *cthr, C4Value *pviVarIndex,
                               C4Value *pvpObj, C4Value *pviEffectNumber) {
  // get parameters
  C4Object *pObj = pvpObj->getObj();
  long iVarIndex = pviVarIndex->getInt(),
       iEffectNumber = pviEffectNumber->getInt();
  // safety
...
static C4Value FnEffectCall_C4V(C4AulContext *ctx, C4Value *pvpTarget,
                                C4Value *pviNumber, C4Value *pvsCallFn,
                                C4Value *pvVal1, C4Value *pvVal2,
                                C4Value *pvVal3, C4Value *pvVal4,
                                C4Value *pvVal5, C4Value *pvVal6,
                                C4Value *pvVal7) {
  // evaluate parameters
  ...
  // do call
  return pEffect->DoCall(pTarget, szCallFn, *pvVal1, *pvVal2, *pvVal3, *pvVal4,
                         *pvVal5, *pvVal6, *pvVal7);
}
...
static long FnModulateColor(C4AulContext *cthr, long iClr1, long iClr2) {
  ...
  DWORD r =
      ((dwClr1 & 0xff) * (dwClr2 & 0xff)) >> 8 |                     // blue
...
static long FnWildcardMatch(C4AulContext *ctx, C4String *psString,
                            C4String *psWildcard) {
  return SWildcardMatchEx(FnStringPar(psString), FnStringPar(psWildcard));
}

static long FnGetContact(C4AulContext *ctx, C4Object *pObj, long iVertex,
                         long dwCheck) {
  // local call / safety
  if (!pObj)
    if (!(pObj = ctx->Obj)) return 0;
...
static long FnSetObjectBlitMode(C4AulContext *ctx, long dwNewBlitMode,
                                C4Object *pObj, long iOverlayID) {
  // local call / safety
  if (!pObj)
    if (!(pObj = ctx->Obj)) return 0;
  // overlay?
  if (iOverlayID) {
    C4GraphicsOverlay *pOverlay = pObj->GetGraphicsOverlay(iOverlayID, false);
    if (!pOverlay) {
      DebugLogF("SetObjectBlitMode: Overlay %d not defined for object %d (%s)",
                (int)iOverlayID, (int)pObj->Number,
                (const char *)pObj->GetName());
      return FALSE;
    }
...
static long FnGetObjectBlitMode(C4AulContext *ctx, C4Object *pObj,
                                long iOverlayID) {
  // local call / safety
  if (!pObj)
    if (!(pObj = ctx->Obj)) return 0;
...
static bool FnSetPreSend(C4AulContext *cthr, long iToVal, C4String *pNewName) {
  ...
  if (!szClient || !*szClient ||
      WildcardMatch(szClient, Game.Clients.getLocalName())) {
...
static bool FnSetPlayerTeam(C4AulContext *cthr, long iPlayer, long idNewTeam,
                            bool fNoCalls) {
  ...
  if (!fNoCalls) {
    if (!!Game.Script.GRBroadcast(
            ...
            &C4AulParSet(C4VInt(iPlayer), C4VInt(idNewTeam)), true, true))
      return false;
  }
  // exit previous team
  ...
  if (idNewTeam) {
    ...
    if (pNewTeam) {
      ...
      if (Game.Rules & C4RULE_TeamHombase && !fNoCalls)
        pPlr->SyncHomebaseMaterialFromTeam();
    } else {
      // unknown error
  ...
  if (!fNoCalls)
    Game.Script.GRBroadcast(
        PSF_OnTeamSwitch,
        &C4AulParSet(C4VInt(iPlayer), C4VInt(idNewTeam), C4VInt(idOldTeam)),
        true);
  return true;
}
...
static long FnGetTeamConfig(C4AulContext *cthr, long iConfigValue) {
  // query value
  switch (iConfigValue) {
    case C4TeamList::TEAM_Custom:
      return Game.Teams.IsCustom();
    case C4TeamList::TEAM_Active:
      return Game.Teams.IsMultiTeams();
    case C4TeamList::TEAM_AllowHostilityChange:
      return Game.Teams.IsHostilityChangeAllowed();
    case C4TeamList::TEAM_Dist:
      return Game.Teams.GetTeamDist();
    case C4TeamList::TEAM_AllowTeamSwitch:
      return Game.Teams.IsTeamSwitchAllowed();
    case C4TeamList::TEAM_AutoGenerateTeams:
      return Game.Teams.IsAutoGenerateTeams();
    case C4TeamList::TEAM_TeamColors:
      return Game.Teams.IsTeamColors();
  }
  // undefined value
...
static bool FnInitScenarioPlayer(C4AulContext *cthr, long iPlayer,
                                 long idTeam) {
  C4Player *pPlr = Game.Players.Get(iPlayer);
  if (!pPlr) return false;
...
static bool FnOnOwnerRemoved(C4AulContext *cthr) {
  ...
  if (!pObj) return false;
  C4Player *pPlr = Game.Players.Get(pObj->Owner);
  if (!pPlr) return false;
  if (pPlr->Crew.IsContained(pObj)) {
    // crew members: Those are removed later (AFTER the player has been removed,
    // for backwards compatiblity with relaunch scripting)
    ...
    // Ignore StaticBack, because this would not be backwards compatible with
    // many internal objects such as team account
    // Do not ignore flags which might be StaticBack if being attached to castle
    // parts
    int32_t iNewOwner = NO_OWNER;
    C4Team *pTeam;
    if (pPlr->Team)
      if (pTeam = Game.Teams.GetTeamByID(pPlr->Team)) {
    ...
    // if noone from the same team was found, try to find another non-hostile
    // player
    // (necessary for cooperative rounds without teams)
    if (iNewOwner == NO_OWNER)
      for (C4Player *pOtherPlr = Game.Players.First; pOtherPlr;
           pOtherPlr = pOtherPlr->Next)
        if (pOtherPlr != pPlr)
...
static bool FnSetScoreboardData(C4AulContext *cthr, long iRowID, long iColID,
                                C4String *pText, long iData) {
  Game.Scoreboard.SetCell(iColID, iRowID, pText ? pText->Data.getData() : NULL,
                          iData);
  return TRUE;
}

static C4String *FnGetScoreboardString(C4AulContext *cthr, long iRowID,
                                       long iColID) {
  return String(Game.Scoreboard.GetCellString(iColID, iRowID));
}

static int32_t FnGetScoreboardData(C4AulContext *cthr, long iRowID,
                                   long iColID) {
...
static bool FnDoScoreboardShow(C4AulContext *cthr, long iChange, long iForPlr) {
  C4Player *pPlr;
  if (iForPlr) {
    // abort if the specified player is not local - but always return if the
    // player exists,
...
static bool FnAddEvaluationData(C4AulContext *cthr, C4String *pText,
                                long idPlayer) {
  // safety
...
static long FnGetUnusedOverlayID(C4AulContext *ctx, long iBaseIndex,
                                 C4Object *pObj) {
  // local call / safety
...
static long FnActivateGameGoalMenu(C4AulContext *ctx, long iPlayer) {
  ...
  if (!pPlr) return FALSE;
  // open menu
  return pPlr->Menu.ActivateGoals(
      pPlr->Number, pPlr->LocalControl && !Game.Control.isReplay());
}

static bool FnFatalError(C4AulContext *ctx, C4String *pErrorMsg) {
  throw new C4AulExecError(
      ctx->Obj,
      FormatString("User error: %s", pErrorMsg ? pErrorMsg->Data.getData()
                                               : "(no error)").getData());
}
...
static bool FnCustomMessage(C4AulContext *ctx, C4String *pMsg, C4Object *pObj,
                            long iOwner, long iOffX, long iOffY, long dwClr,
                            C4ID idDeco, C4String *sPortrait, long dwFlags,
                            long iHSize) {
  // safeties
  ...
  if (((vpos | vpos - 1) + 1) >> 1 != vpos) {
    throw new C4AulExecError(
        ctx->Obj, "CustomMessage: Only one vertical positioning flag allowed!");
  }
  // message color
  ...
  // create it!
  return Game.Messages.New(
      iType, sMsg, pObj, iOwner, iOffX, iOffY, (uint32_t)dwClr, idDeco,
      sPortrait ? sPortrait->Data.getData() : NULL, dwFlags, iHSize);
}
...
static bool FnSetNextMission(C4AulContext *ctx, C4String *szNextMission,
                             C4String *szNextMissionText,
                             C4String *szNextMissionDesc) {
...
class C4AulDefFuncHelper : public C4AulFunc {
 public:
  C4AulDefFuncHelper(C4AulScript *pOwner, const char *pName, bool Public,
                     C4V_Type pt0 = C4V_Any, C4V_Type pt1 = C4V_Any,
                     C4V_Type pt2 = C4V_Any, C4V_Type pt3 = C4V_Any,
                     C4V_Type pt4 = C4V_Any, C4V_Type pt5 = C4V_Any,
                     C4V_Type pt6 = C4V_Any, C4V_Type pt7 = C4V_Any,
                     C4V_Type pt8 = C4V_Any, C4V_Type pt9 = C4V_Any)
      : C4AulFunc(pOwner, pName), Public(Public) {
    ParType[0] = pt0;
    ParType[1] = pt1;
    ParType[2] = pt2;
    ParType[3] = pt3;
...
// The lists are used in a context where a leading comma is needed when the list
// is not empty
#define LIST0(TEXT)
...
// N is the number of parameters pFunc needs. Templates can only have a fixed
// number of arguments,
...
#define TEMPLATE(N)                                                          \
  ...
  class C4AulDefFunc##N : public C4AulDefFuncHelper {                        \
   public:                                                                   \
    ...
    virtual C4V_Type GetRetType() { return C4ValueConv<RType>::Type(); }     \
    /* Constructor, using the base class to create the ParType array */      \
    C4AulDefFunc##N(C4AulScript *pOwner, const char *pName, Func pFunc,      \
                    bool Public)                                             \
        : C4AulDefFuncHelper(pOwner, pName, Public LIST(N, CONV_TYPE)),      \
          pFunc(pFunc) {}                                                    \
    /* Extracts the parameters from C4Values and wraps the return value in a \
     * C4Value */                                                            \
    C4Value Exec(C4AulContext *pContext, C4Value pPars[],                    \
                 bool fPassErrors = false) {                                 \
      return C4ValueConv<RType>::ToC4V(                                      \
          pFunc(pContext LIST(N, CONV_FROM_C4V)));                           \
    }                                                                        \
                                                                             \
   protected:                                                                \
    Func pFunc;                                                              \
  ...
  template <typename RType LIST(N, TYPENAMES)>                               \
  static void AddFunc(C4AulScript *pOwner, const char *Name,                 \
                      RType (*pFunc)(C4AulContext * LIST(N, PARS)),          \
                      bool Public = true) {                                  \
...
class C4AulDefCastFunc : public C4AulDefFuncHelper {
 public:
  C4AulDefCastFunc(C4AulScript *pOwner, const char *pName, C4V_Type ParType,
                   C4V_Type RType)
...
void InitFunctionMap(C4AulScriptEngine *pEngine) {
  ...
  for (C4ScriptConstDef *pCDef = &C4ScriptConstMap[0]; pCDef->Identifier;
       pCDef++)
    Game.ScriptEngine.RegisterGlobalConstant(
        pCDef->Identifier, C4Value(pCDef->Data, pCDef->ValType));
  ...
  AddFunc(pEngine, "SetMaterialColor2", FnSetMaterialColor2);
  ...
  AddFunc(pEngine, "HideSettlementScoreInEvaluation",
          FnHideSettlementScoreInEvaluation, false);
...
C4ScriptConstDef C4ScriptConstMap[] = {
    ...
    {"FX_OK", C4V_Int,
     C4Fx_OK},  // generic standard behaviour for all effect callbacks
    {"FX_Effect_Deny", C4V_Int, C4Fx_Effect_Deny},    // delete effect
    {"FX_Effect_Annul", C4V_Int, C4Fx_Effect_Annul},  // delete effect, because
                                                      // it has annulled a
                                                      // countereffect
    {"FX_Effect_AnnulDoCalls", C4V_Int,
     C4Fx_Effect_AnnulCalls},  // delete effect, because it has annulled a
                               // countereffect; temp readd countereffect
    {"FX_Execute_Kill", C4V_Int,
     C4Fx_Execute_Kill},  // execute callback: Remove effect now
    {"FX_Stop_Deny", C4V_Int, C4Fx_Stop_Deny},    // deny effect removal
    {"FX_Start_Deny", C4V_Int, C4Fx_Start_Deny},  // deny effect start

    {"FX_Call_Normal", C4V_Int,
     C4FxCall_Normal},  // normal call; effect is being added or removed
    {"FX_Call_Temp", C4V_Int, C4FxCall_Temp},  // temp call; effect is being
                                               // added or removed in responce
                                               // to a lower-level effect change
    {"FX_Call_TempAddForRemoval", C4V_Int,
     C4FxCall_TempAddForRemoval},  // temp call; effect is being added because
                                   // it had been temp removed and is now
                                   // removed forever
    {"FX_Call_RemoveClear", C4V_Int,
     C4FxCall_RemoveClear},  // effect is being removed because object is being
                             // removed
    {"FX_Call_RemoveDeath", C4V_Int,
     C4FxCall_RemoveDeath},  // effect is being removed because object died -
                             // return -1 to avoid removal
    {"FX_Call_DmgScript", C4V_Int,
     C4FxCall_DmgScript},  // damage through script call
    ...
    {"FX_Call_EngScript", C4V_Int,
     C4FxCall_EngScript},  // energy loss through script call
    {"FX_Call_EngBlast", C4V_Int,
     C4FxCall_EngBlast},  // energy loss through blast
    {"FX_Call_EngObjHit", C4V_Int,
     C4FxCall_EngObjHit},  // energy loss through object hitting the living
    {"FX_Call_EngFire", C4V_Int, C4FxCall_EngFire},  // energy loss through fire
    {"FX_Call_EngBaseRefresh", C4V_Int,
     C4FxCall_EngBaseRefresh},  // energy reload in base (also by base object,
                                // but that's normally not called)
    {"FX_Call_EngAsphyxiation", C4V_Int,
     C4FxCall_EngAsphyxiation},  // energy loss through asphyxiaction
    {"FX_Call_EngCorrosion", C4V_Int,
     C4FxCall_EngCorrosion},  // energy loss through corrosion (acid)
    {"FX_Call_EngStruct", C4V_Int,
     C4FxCall_EngStruct},  // regular structure energy loss (normally not
                           // called)
    {"FX_Call_EngGetPunched", C4V_Int,
     C4FxCall_EngGetPunched},  // energy loss during fighting

    {"GFXOV_MODE_None", C4V_Int,
     C4GraphicsOverlay::MODE_None},  // gfx overlay modes
    ...
    {"GFXOV_MODE_IngamePicture", C4V_Int,
     C4GraphicsOverlay::MODE_IngamePicture},                         //
    {"GFXOV_MODE_Object", C4V_Int, C4GraphicsOverlay::MODE_Object},  //
    {"GFXOV_MODE_ExtraGraphics", C4V_Int,
     C4GraphicsOverlay::MODE_ExtraGraphics},  //
    {"GFX_Overlay", C4V_Int, 1},              // default overlay index
    {"GFXOV_Clothing", C4V_Int,
     1000},  // overlay indices for clothes on Clonks, etc.
    {"GFXOV_Tools", C4V_Int, 2000},  // overlay indices for tools, weapons, etc.
    {"GFXOV_ProcessTarget", C4V_Int,
     3000},  // overlay indices for objects processed by a Clonk
    ...
    // scoreboard
    {"SBRD_Caption", C4V_Int,
     C4Scoreboard::TitleKey},  // used to set row/coloumn headers

    // teams - constants for GetTeamConfig
    {"TEAM_Custom", C4V_Int, C4TeamList::TEAM_Custom},
    {"TEAM_Active", C4V_Int, C4TeamList::TEAM_Active},
    {"TEAM_AllowHostilityChange", C4V_Int,
     C4TeamList::TEAM_AllowHostilityChange},
    ...
    {"C4MSGCMDR_Identifier", C4V_Int,
     C4MessageBoardCommand::C4MSGCMDR_Identifier},
...
#define MkFnC4V                                                       \
  (C4Value (*)(C4AulContext * cthr, C4Value *, C4Value *, C4Value *,  \
...
C4ScriptFnDef
    C4ScriptFnMap[] = {
        ...
        {"goto",
         1,
         C4V_Any,
         {C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         Fn_goto},
        {"this",
         1,
         C4V_C4Object,
         {C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         Fn_this},
        {"Equal",
        ...
        {"Global",
         ...
         {C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGlobal_C4V,
         0},
        {"SetLocal",
         1,
         C4V_Any,
         {C4V_Int, C4V_Any, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnSetLocal_C4V,
         0},
        {"Local",
         1,
         C4V_Any,
         {C4V_Int, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnLocal_C4V,
         0},
        {"Explode",
         1,
         C4V_Bool,
         {C4V_Int, C4V_C4Object, C4V_C4ID, C4V_String, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any, C4V_Any},
         0,
         FnExplode},
        {"Incinerate",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnIncinerate},
        {"IncinerateLandscape",
         1,
         C4V_Bool,
         {C4V_Int, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnIncinerateLandscape},
        {"Extinguish",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
        ...
        {"GrabContents",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any, C4V_Any},
         0,
         FnGrabContents},
        {"Punch",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnPunch},
        {"Kill",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_Bool, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnKill},
        {"Fling",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_Int, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnFling},
        {"Jump",
        ...
        {"ChangeDef",
         1,
         C4V_Bool,
         {C4V_C4ID, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnChangeDef},
        {"Exit",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_Int, C4V_Int, C4V_Int, C4V_Int, C4V_Int, C4V_Int,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnExit},
        {"Enter",
        ...
        {"PlayerObjectCommand",
         1,
         C4V_Bool,
         {C4V_Int, C4V_String, C4V_C4Object, C4V_Any, C4V_Int, C4V_C4Object,
          C4V_Any, C4V_Int, C4V_Any, C4V_Any},
         0,
         FnPlayerObjectCommand},
        {"SetCommand",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_String, C4V_C4Object, C4V_Any, C4V_Int,
          C4V_C4Object, C4V_Any, C4V_Int, C4V_Any, C4V_Any},
         0,
         FnSetCommand},
        {"AddCommand",
         1,
         C4V_Bool,
         {C4V_C4Object, C4V_String, C4V_C4Object, C4V_Any, C4V_Int,
          C4V_C4Object, C4V_Int, C4V_Any, C4V_Int, C4V_Int},
         0,
         FnAddCommand},
        {"AppendCommand",
         1,
        ...
        {"GetCommand",
         ...
         0,
         FnGetCommand},
        //{ "FindConstructionSite",	0	,C4V_Bool			,{
        //C4V_C4ID		,C4V_Int		,C4V_Int		,C4V_Any
        //,C4V_Any		,C4V_Any		,C4V_Any		,C4V_Any
        //,C4V_Any		,C4V_Any}  ,MkFnC4V FnFindConstructionSite ,    0
        //},
        {"PathFree2",
         1,
         C4V_Bool,
         {C4V_pC4Value, C4V_pC4Value, C4V_Int, C4V_Int, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnPathFree2_C4V,
         0},
        {"DeathAnnounce",
         1,
         C4V_Bool,
        ...
        {"FindObject",
         1,
         C4V_C4Object,
         {C4V_C4ID, C4V_Int, C4V_Int, C4V_Int, C4V_Int, C4V_Int, C4V_String,
          C4V_C4Object, C4V_Any, C4V_C4Object},
         0,
         FnFindObject},
        {"FindObject2",
         1,
         C4V_C4Object,
         {C4V_Array, C4V_Array, C4V_Array, C4V_Array, C4V_Array, C4V_Array,
          C4V_Array, C4V_Array, C4V_Array, C4V_Array},
         0,
         FnFindObject2},
        {"FindObjects",
         1,
         C4V_Array,
         {C4V_Array, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnFindObjects},
        {"ObjectCount",
        ...
        {"ObjectCall",
         1,
         C4V_Any,
         {C4V_C4Object, C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnObjectCall_C4V,
         0},
        {"ProtectedCall",
         1,
         C4V_Any,
        ...
        {"PrivateCall",
         ...
         {C4V_C4Object, C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnPrivateCall_C4V,
         0},
        {"GameCall",
         1,
         C4V_Any,
         {C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGameCall_C4V,
         0},
        {"GameCallEx",
         1,
         C4V_Any,
         {C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGameCallEx_C4V,
         0},
        {"DefinitionCall",
         1,
         C4V_Any,
         {C4V_C4ID, C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnDefinitionCall_C4V,
         0},
        {"Call",
         0,
         C4V_Any,
        ...
        {"GetPlrKnowledge",
         1,
         C4V_Int,
         {C4V_Int, C4V_C4ID, C4V_Int, C4V_Int, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetPlrKnowledge_C4V,
         0},
        {"GetPlrMagic",
         1,
         C4V_Int,
         {C4V_Int, C4V_C4ID, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetPlrMagic_C4V,
         0},
        {"GetComponent",
         1,
         C4V_Int,
         {C4V_C4ID, C4V_Int, C4V_C4Object, C4V_C4ID, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetComponent_C4V,
         0},
        {"PlayerMessage",
         1,
         C4V_Int,
         {C4V_Int, C4V_String, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V & FnPlayerMessage_C4V,
         0},
        {"Message",
         1,
         C4V_Bool,
         {C4V_String, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V & FnMessage_C4V,
         0},
        {"AddMessage",
         1,
         C4V_Bool,
         {C4V_String, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V & FnAddMessage_C4V,
         0},
        {"PlrMessage",
         1,
         C4V_Bool,
         {C4V_String, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V & FnPlrMessage_C4V,
         0},
        {"Log",
         1,
         C4V_Bool,
        ...
        {"AddMenuItem",
         1,
         C4V_Bool,
         {C4V_String, C4V_String, C4V_C4ID, C4V_C4Object, C4V_Int, C4V_Any,
          C4V_String, C4V_Int, C4V_Any, C4V_Any},
         0,
         FnAddMenuItem},
        {"SetSolidMask",
         1,
         C4V_Bool,
         {C4V_Int, C4V_Int, C4V_Int, C4V_Int, C4V_Int, C4V_Int, C4V_C4Object,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnSetSolidMask},
        {"SetGravity",
        ...
        {"GetHomebaseMaterial",
         ...
         {C4V_Int, C4V_C4ID, C4V_Int, C4V_Int, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetHomebaseMaterial_C4V,
         0},
        {"GetHomebaseProduction",
         ...
         {C4V_Int, C4V_C4ID, C4V_Int, C4V_Int, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetHomebaseProduction_C4V,
         0},

        {"Set",
         1,
         C4V_Any,
         {C4V_pC4Value, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnSet,
         0},
        {"Inc",
        ...
        {"GetIndexOf",
         1,
         C4V_Int,
         {C4V_Any, C4V_Array, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnGetIndexOf},
        {"SetLength",
         1,
         C4V_Bool,
         {C4V_pC4Value, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         0,
         FnSetLength},
        ...
        {"GetDefCoreVal",
         1,
         C4V_Any,
         {C4V_String, C4V_String, C4V_C4ID, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetDefCoreVal,
         0},
        {"GetActMapVal",
         1,
         C4V_Any,
         {C4V_String, C4V_String, C4V_C4ID, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetActMapVal,
         0},
        {"GetObjectVal",
         1,
         C4V_Any,
         {C4V_String, C4V_String, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetObjectVal,
         0},
        {"GetObjectInfoCoreVal",
         ...
         {C4V_String, C4V_String, C4V_C4Object, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetObjectInfoCoreVal,
         0},
        {"GetScenarioVal",
         1,
         C4V_Any,
         {C4V_String, C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetScenarioVal,
         0},
        //{ "SetScenarioVal",				1	,C4V_Bool
        //,{ C4V_String	,C4V_String	,C4V_Any		,C4V_Any
        //,C4V_Any		,C4V_Any		,C4V_Any		,C4V_Any
        //,C4V_Any		,C4V_Any}	 ,MkFnC4V FnSetScenarioVal, 0 },
        {"GetPlayerVal",
         1,
         C4V_Any,
         {C4V_String, C4V_String, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetPlayerVal,
         0},
        {"GetPlayerInfoCoreVal",
         ...
         {C4V_String, C4V_String, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetPlayerInfoCoreVal,
         0},
        {"GetMaterialVal",
         1,
         C4V_Any,
         {C4V_String, C4V_String, C4V_Int, C4V_Int, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetMaterialVal,
         0},

        {"SetPlrExtraData",
         1,
         C4V_Any,
         {C4V_Int, C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnSetPlrExtraData,
         0},
        {"GetPlrExtraData",
         1,
         C4V_Any,
        ...
        {"GetCrewExtraData",
         ...
         {C4V_C4Object, C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetCrewExtraData,
         0},
        {"SimFlight",
         1,
         C4V_Bool,
         {C4V_pC4Value, C4V_pC4Value, C4V_pC4Value, C4V_pC4Value, C4V_Int,
          C4V_Int, C4V_Int, C4V_Int, C4V_Any, C4V_Any},
         MkFnC4V FnSimFlight,
         0},
        {"GetPortrait",
         1,
         C4V_Any,
         {C4V_C4Object, C4V_Bool, C4V_Bool, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetPortrait,
         0},
        {"AddEffect",
         1,
         C4V_Int,
         {C4V_String, C4V_C4Object, C4V_Int, C4V_Int, C4V_C4Object, C4V_C4ID,
          C4V_Any, C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnAddEffect_C4V,
         0},
        {"GetEffect",
         1,
         C4V_Any,
         {C4V_String, C4V_C4Object, C4V_Int, C4V_Int, C4V_Int, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnGetEffect_C4V,
         0},
        {"CheckEffect",
         1,
         C4V_Int,
         {C4V_String, C4V_C4Object, C4V_Int, C4V_Int, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnCheckEffect_C4V,
         0},
        {"EffectCall",
         1,
         C4V_Any,
         {C4V_C4Object, C4V_Int, C4V_String, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnEffectCall_C4V,
         0},
        {"EffectVar",
         1,
         C4V_pC4Value,
         {C4V_Int, C4V_C4Object, C4V_Int, C4V_Any, C4V_Any, C4V_Any, C4V_Any,
          C4V_Any, C4V_Any, C4V_Any},
         MkFnC4V FnEffectVar_C4V,
         0},
