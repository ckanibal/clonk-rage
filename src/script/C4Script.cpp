...
static bool FnOnMessageBoardAnswer(C4AulContext *cthr, C4Object *pObj, long iForPlr, C4String *szAnswerString)
	...
	// exec func
	return !!scr->ObjectCall(NULL, pObj, PSF_InputCallback, &C4AulParSet(C4VString(FnStringPar(szAnswerString)), C4VInt(iForPlr)));
	}
...
static bool FnSetMaterialColor2(C4AulContext* cthr, long iMat, long iClrR, long iClrG, long iClrB)
{
	// get mat
	if (!MatValid(iMat)) return FALSE;
	C4Material *pMat = &Game.Material.Map[iMat];
...
static bool FnSetScoreboardData(C4AulContext *cthr, long iRowID, long iColID, C4String *pText, long iData)
	{
	Game.Scoreboard.SetCell(iColID, iRowID, pText ? pText->Data.getData() : NULL, iData);
	return TRUE;
	}
...
static bool FnCustomMessage(C4AulContext *ctx, C4String *pMsg, C4Object *pObj, long iOwner, long iOffX, long iOffY, long dwClr, C4ID idDeco, C4String *sPortrait, long dwFlags, long iHSize)
	...
	// create it!
	return Game.Messages.New(iType,sMsg,pObj,iOwner,iOffX,iOffY,(uint32_t)dwClr, idDeco, sPortrait ? sPortrait->Data.getData() : NULL, dwFlags, iHSize);
	}
