...
C4StringTable::C4StringTable() : First(NULL), Last(NULL) {}
...
C4String *C4StringTable::FindString(const char *strString) {
  for (C4String *pAct = First; pAct; pAct = pAct->Next)
    if (SEqual(pAct->Data.getData(), strString)) return pAct;
  return NULL;
}

C4String *C4StringTable::FindString(C4String *pString) {
  for (C4String *pAct = First; pAct; pAct = pAct->Next)
    if (pAct == pString) return pAct;
  return NULL;
}

C4String *C4StringTable::FindString(int iEnumID) {
  for (C4String *pAct = First; pAct; pAct = pAct->Next)
    if (pAct->iEnumID == iEnumID) return pAct;
  return NULL;
}
...
bool C4StringTable::Load(C4Group &ParentGroup) {
  // read data
  char *pData;
  if (!ParentGroup.LoadEntry(C4CFN_Strings, &pData, NULL, 1)) return false;
  // read all strings
  ...
  for (int i = 0; SCopySegment(pData, i, strBuf, 0x0A, C4AUL_MAX_String); i++) {
    ...
    // add string to list
    C4String *pnString;
    if (!(pnString = FindString(strBuf))) pnString = RegString(strBuf);
    pnString->iEnumID = i;
  }
  // delete data
  delete[] pData;
...
bool C4StringTable::Save(C4Group &ParentGroup) {
  ...
  for (pAct = First; pAct; pAct = pAct->Next)
    if (pAct->iEnumID > -1) iTableSize += SLen(pAct->Data.getData()) + 2;
  ...
  for (pAct = First; pAct; pAct = pAct->Next)
    if (pAct->iEnumID > -1) {
      ...
      if (strchr(pPos, 10) || strchr(pPos, 13)) {
        // delete feeds
        char *pCharPos = pPos;
        while (pCharPos = strchr(pCharPos, 10))
          memmove(pCharPos, pCharPos + 1, SLen(pCharPos + 1) + 1);
