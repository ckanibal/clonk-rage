...
C4AulExecError::C4AulExecError(C4Object *pObj, const char *szError)
    : cObj(pObj) {
...
void C4AulScriptContext::dump(StdStrBuf Dump) {
    ...
    for (int i = 0; i < C4AUL_MAX_Par; i++)
      if (Pars + i < Vars)
        if (!Pars[i])
          ...
          if (i > iNullPars) Dump.AppendChar(',');
  ...
  if (!fDirectExec && Func->Owner)
    Dump.AppendFormat(" (%s:%d)", Func->pOrgScript->ScriptName.getData(),
                      SGetLine(Func->pOrgScript->GetScript(),
                               CPos ? CPos->SPos : Func->Script));
  // Log it
...
class C4AulExec {
 public:
  C4AulExec() : pCurCtx(Contexts - 1), pCurVal(Values - 1), iTraceStart(-1) {}
 ...
 public:
  C4Value Exec(C4AulScriptFunc *pSFunc, C4Object *pObj, C4Value pPars[],
               bool fPassErrors, bool fTemporaryScript = false);
  C4Value Exec(C4AulBCC *pCPos, bool fPassErrors);

  void StartTrace();
  void StartProfiling(C4AulScript *pScript);  // resets profling times and
                                              // starts recording the times
  void StopProfiling();  // stop the profiler and displays results
  void AbortProfiling() { fProfiling = false; }
  inline void StartDirectExec() {
    if (fProfiling) tDirectExecStart = timeGetTime();
  }
  inline void StopDirectExec() {
    if (fProfiling) tDirectExecTotal += timeGetTime() - tDirectExecStart;
  }
 ...
 private:
  ...
  void PopContext() {
    ...
    if (fProfiling) {
      ...
      if (dt && pCurCtx->Func) pCurCtx->Func->tProfileTime += dt;
    }
    // Trace done?
    if (iTraceStart >= 0) {
    ...
    if (pCurCtx->TemporaryScript) delete pCurCtx->Func->Owner;
    pCurCtx--;
  }
  ...
  bool PopValue() {
    if (LocalValueStackSize() < 1)
      throw new C4AulExecError(pCurCtx->Obj,
                               "internal error: value stack underflow!");
    (pCurVal--)->Set0();
    return true;
  }
  ...
  void PopValuesUntil(C4Value *pUntilVal) {
    if (pUntilVal < Values - 1)
      throw new C4AulExecError(pCurCtx->Obj,
                               "internal error: value stack underflow!");
    while (pCurVal > pUntilVal) (pCurVal--)->Set0();
  }

  int ContextStackSize() const { return pCurCtx - Contexts + 1; }

  int ValueStackSize() const { return pCurVal - Values + 1; }
  ...
  void CheckOpPars(int iOpID) {
    ...
    if (!pPar1->ConvertTo(C4ScriptOpMap[iOpID].Type1))
      throw new C4AulExecError(
          pCurCtx->Obj,
          FormatString(
              "operator \"%s\" left side: got \"%s\", but expected \"%s\"!",
              C4ScriptOpMap[iOpID].Identifier, pPar1->GetTypeInfo(),
              GetC4VName(C4ScriptOpMap[iOpID].Type1)).getData());
    if (!pPar2->ConvertTo(C4ScriptOpMap[iOpID].Type2))
      throw new C4AulExecError(
          pCurCtx->Obj,
          FormatString(
              "operator \"%s\" right side: got \"%s\", but expected \"%s\"!",
              C4ScriptOpMap[iOpID].Identifier, pPar2->GetTypeInfo(),
              GetC4VName(C4ScriptOpMap[iOpID].Type2)).getData());
  }
    ...
    if (!pCurVal->ConvertTo(C4ScriptOpMap[iOpID].Type1))
      throw new C4AulExecError(
          ...
          FormatString("operator \"%s\": got \"%s\", but expected \"%s\"!",
                       C4ScriptOpMap[iOpID].Identifier, pCurVal->GetTypeInfo(),
                       GetC4VName(C4ScriptOpMap[iOpID].Type1)).getData());
  }
  C4AulBCC *Call(C4AulFunc *pFunc, C4Value *pReturn, C4Value *pPars,
                 C4Object *pObj = NULL, C4Def *pDef = NULL);
};

C4AulExec AulExec;

C4Value C4AulExec::Exec(C4AulScriptFunc *pSFunc, C4Object *pObj,
                        C4Value *pnPars, bool fPassErrors,
                        bool fTemporaryScript) {
  // Push parameters
  C4Value *pPars = pCurVal + 1;
  if (pnPars)
    for (int i = 0; i < C4AUL_MAX_Par; i++) PushValue(pnPars[i]);
  ...
  // This must hold: The scripter might try to access local variables that don't
  // exist!
...
C4Value C4AulExec::Exec(C4AulBCC *pCPos, bool fPassErrors) {
  ...
  try {
    for (;;) {
      ...
      switch (pCPos->bccType) {
        ...
        case AB_ERR:
          throw new C4AulExecError(
              pCurCtx->Obj,
              "syntax error: see previous parser error for details.");
        ...
        case AB_LOCALN_V:
          if (!pCurCtx->Obj)
            throw new C4AulExecError(
                pCurCtx->Obj,
                "can't access local variables in a definition call!");
          if (pCurCtx->Func->Owner->Def != pCurCtx->Def)
            throw new C4AulExecError(
                pCurCtx->Obj, "can't access local variables after ChangeDef!");
        ...
        case AB_SEqual:  // S=, eq
        {
          ...
          C4Value *pPar1 = pCurVal - 1, *pPar2 = pCurVal;
          pPar1->SetBool(
              SEqual(pPar1->_getStr() ? pPar1->_getStr()->Data.getData() : "",
        ...
        case AB_SNEqual:  // ne
        {
          ...
          pPar1->SetBool(
              !SEqual(
                  pPar1->_getStr() ? pPar1->_getStr()->Data.getData() : "",
                  pPar2->_getStr() ? pPar2->_getStr()->Data.getData() : ""));
          PopValue();
          break;
        }
        ...
        case AB_DivIt:  // /=
        {
          ...
          C4Value *pPar1 = pCurVal - 1, *pPar2 = pCurVal;
          pPar1->GetData().Int =
              pPar2->_getInt() ? pPar1->GetData().Int / pPar2->_getInt() : 0;
          PopValue();
          break;
        }
        case AB_ModIt:  // %=
        ...
        /*				case AB_UNOP:
                                                ...
                                                if(!pCurVal->ConvertTo(C4ScriptOpMap[iOpID].Type1))
                                                        throw new
           C4AulExecError(pCurCtx->Obj,
                                                FormatString("operator \"%s\":
           got \"%s\", but expected \"%s\"!",
                                                        C4ScriptOpMap[iOpID].Identifier,
           pCurVal->GetTypeInfo(),
           GetC4VName(C4ScriptOpMap[iOpID].Type1)).getData());

                                                // Execute operator
                                                if(C4ScriptOpMap[iOpID].Function)
                                                        pCurVal->Set((*C4ScriptOpMap[iOpID].Function)(pCurCtx,
           pCurVal->_getRaw(), 0), C4ScriptOpMap[iOpID].RetType);
                                                else
           if(C4ScriptOpMap[iOpID].FunctionC4V)
                                                        pCurVal->Set((*C4ScriptOpMap[iOpID].FunctionC4V)(pCurCtx,
           pCurVal, NULL));
           ...
           *pPar2 = pCurVal;
           ...
           \"%s\" left side: got \"%s\", but expected \"%s\"!",
                                                                        C4ScriptOpMap[iOpID].Identifier,
           pPar1->GetTypeInfo(),
           GetC4VName(C4ScriptOpMap[iOpID].Type1)).getData());
                                                if(!pPar2->ConvertTo(C4ScriptOpMap[iOpID].Type2))
           ...
           C4AulExecError(pCurCtx->Obj,
                                                                FormatString("operator
           \"%s\" right side: got \"%s\", but expected \"%s\"!",
                                                                        C4ScriptOpMap[iOpID].Identifier,
           pPar2->GetTypeInfo(),
           GetC4VName(C4ScriptOpMap[iOpID].Type2)).getData());
                                                ...
                                                // Execute operator
                                                if(C4ScriptOpMap[iOpID].Function)
                                                        pPar1->Set((*C4ScriptOpMap[iOpID].Function)(pCurCtx,
           pPar1->_getRaw(), pPar2->_getRaw()),C4ScriptOpMap[iOpID].RetType);
                                                else
           if(C4ScriptOpMap[iOpID].FunctionC4V)
                                                        pPar1->Set((*C4ScriptOpMap[iOpID].FunctionC4V)(pCurCtx,
           pPar1, pPar2));
        ...
        */
        ...
        case AB_ARRAYA_V: {
          ...
          if (!Array.ConvertTo(C4V_Array))
            throw new C4AulExecError(
                pCurCtx->Obj,
                FormatString("array access: can't access %s as an array!",
                             Array.GetTypeName()).getData());
          if (!Index.ConvertTo(C4V_Int))
            throw new C4AulExecError(
                pCurCtx->Obj,
                FormatString("array access: index of type %s, int expected!",
                             Index.GetTypeName()).getData());
        ...
        case AB_RETURN: {
          // Resolve reference
          if (!pCurCtx->Func->SFunc()->bReturnRef) pCurVal->Deref();

          // Trace
          if (iTraceStart >= 0) {
            StdStrBuf Buf("T");
            Buf.AppendChars('>', ContextStackSize() - iTraceStart);
            LogF("%s%s returned %s", Buf.getData(), pCurCtx->Func->Name,
                 pCurVal->GetDataString().getData());
          }
          ...
          // Save return value
          if (pCurVal != pReturn) pReturn->Set(*pCurVal);
        ...
        case AB_VAR_V:
          if (!pCurVal->ConvertTo(C4V_Int))
            throw new C4AulExecError(
                pCurCtx->Obj,
                FormatString("Var: index of type %s, int expected!",
                             pCurVal->GetTypeName()).getData());
        ...
        case AB_PAR_V:
          if (!pCurVal->ConvertTo(C4V_Int))
            throw new C4AulExecError(
                pCurCtx->Obj,
                FormatString("Par: index of type %s, int expected!",
                             pCurVal->GetTypeName()).getData());
          // Push reference to parameter on the stack
          if (pCurVal->_getInt() >= 0 &&
              pCurVal->_getInt() < pCurCtx->ParCnt()) {
        ...
        case AB_FOREACH_NEXT: {
          ...
          if (!iItem) {
            if (!pCurVal[-1].ConvertTo(C4V_Array))
              throw new C4AulExecError(
                  pCurCtx->Obj,
                  FormatString("for: array expected, but got %s!",
                               pCurVal[-1].GetTypeName()).getData());
            if (!pCurVal[-1]._getArray())
              throw new C4AulExecError(
                  pCurCtx->Obj,
                  FormatString("for: array expected, but got 0!").getData());
          }
          C4ValueArray *pArray = pCurVal[-1]._getArray();
          // No more entries?
          if (pCurVal->_getInt() >= pArray->GetSize()) break;
          // Get next
        ...
        case AB_CALLFS: {
          ...
          if (pTargetVal->ConvertTo(C4V_C4Object)) {
            ...
            if (!pDestDef)
              throw new C4AulExecError(
                  pCurCtx->Obj,
                  FormatString(
                      "Definition call: Definition for id %s not found!",
                      C4IdText(pTargetVal->_getC4ID())).getData());
          } else
            throw new C4AulExecError(pCurCtx->Obj,
                                     FormatString(
                                         "Object call: Invalid target type %s, "
                                         "expected object or id!",
                                         pTargetVal->GetTypeName()).getData());

          // Resolve overloads
          C4AulFunc *pFunc = reinterpret_cast<C4AulFunc *>(pCPos->bccX);
          while (pFunc->OverloadedBy) pFunc = pFunc->OverloadedBy;
          ...
          if (!pFunc) {
            const char *szFuncName =
                reinterpret_cast<C4AulFunc *>(pCPos->bccX)->Name;
            if (pDestObj)
              throw new C4AulExecError(
                  pCurCtx->Obj,
                  FormatString(
                      "Object call: No function \"%s\" in object \"%s\"!",
                      szFuncName,
                      pTargetVal->GetDataString().getData()).getData());
            else
              throw new C4AulExecError(
                  pCurCtx->Obj,
                  FormatString(
                      "Definition call: No function \"%s\" in definition "
                      "\"%s\"!",
                      szFuncName, pDestDef->Name.getData()).getData());
          }
          ...
          // Call function
          C4AulBCC *pNewCPos =
              Call(pFunc, pTargetVal, pPars, pDestObj, pDestDef);
          if (pNewCPos) {
            // Jump
            pCPos = pNewCPos;
            fJump = true;
    ...
    if (pUntil) PopValuesUntil(pUntil);
  }
...
C4AulBCC *C4AulExec::Call(C4AulFunc *pFunc, C4Value *pReturn, C4Value *pPars,
                          C4Object *pObj, C4Def *pDef) {
  ...
  for (int i = 0; i < pFunc->GetParCount(); i++)
    if (!pPars[i].ConvertTo(pTypes[i]))
      throw new C4AulExecError(
          pCurCtx->Obj,
          FormatString(
              "call to \"%s\" parameter %d: got \"%s\", but expected \"%s\"!",
              pFunc->Name, i + 1, pPars[i].GetTypeName(),
              GetC4VName(pTypes[i])).getData());
  ...
  if (pSFunc) {
    ...
    if (Game.FrameCounter >= DEBUGREC_START_FRAME) {
      StdStrBuf sCallText;
      if (pObj) sCallText.AppendFormat("Object(%d): ", pObj->Number);
...
void C4AulStartTrace() { AulExec.StartTrace(); }

void C4AulExec::StartTrace() {
  if (iTraceStart < 0) iTraceStart = ContextStackSize();
}
...
void C4AulProfiler::StopProfiling() { AulExec.StopProfiling(); }

void C4AulProfiler::Abort() { AulExec.AbortProfiling(); }
...
void C4AulProfiler::Show() {
  ...
  for (EntryList::iterator i = Times.begin(); i != Times.end(); ++i) {
    Entry &e = (*i);
    LogF("%05dms\t%s", (int)e.tProfileTime,
         e.pFunc ? (e.pFunc->GetFullName().getData()) : "Direct exec");
  }
  Log("==============================");
  // done!
}
...
C4Value C4AulScriptFunc::Exec(C4AulContext *pCtx, C4Value pPars[],
                              bool fPassErrors) {
#ifdef C4ENGINE
...
C4Value C4AulScriptFunc::Exec(C4Object *pObj, C4AulParSet *pPars,
                              bool fPassErrors) {
#ifdef C4ENGINE
  ...
  // execute
  return AulExec.Exec(this, pObj, pPars ? pPars->Par : C4AulParSet().Par,
                      fPassErrors);
...
C4Value C4AulDefFunc::Exec(C4AulContext *pCallerCtx, C4Value pPars[],
                           bool fPassErrors) {
  ...
  if (Def->FunctionC4V != 0)
    ...
    // C4V function
    return Def->FunctionC4V(pCallerCtx, &pPars[0], &pPars[1], &pPars[2],
                            &pPars[3], &pPars[4], &pPars[5], &pPars[6],
                            &pPars[7], &pPars[8], &pPars[9]);

  // should never happen...
  return C4VNull;
}

C4Value C4AulScript::DirectExec(C4Object *pObj, const char *szScript,
                                const char *szContext, bool fPassErrors,
                                enum Strict Strict) {
#ifdef DEBUGREC_SCRIPT
  ...
  pScript->ScriptName =
      FormatString("%s in %s", szContext, ScriptName.getData());
  ...
  // Execute. The TemporaryScript-parameter makes sure the script will be
  // deleted later on.
...
void C4AulScript::ResetProfilerTimes() {
  ...
  for (C4AulFunc *pFn = Func0; pFn; pFn = pFn->Next)
    if (pSFunc = pFn->SFunc()) pSFunc->tProfileTime = 0;
  // reset sub-scripts
