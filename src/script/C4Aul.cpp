...
void C4AulError::show() {
  ...
  if (sMessage) DebugLog(sMessage.getData());
#endif
}

C4AulFunc::C4AulFunc(C4AulScript *pOwner, const char *pName, bool bAtEnd)
    : MapNext(NULL), LinkedTo(NULL), OverloadedBy(NULL), NextSNFunc(NULL) {
...
C4AulFunc::~C4AulFunc() {
  ...
  if (LinkedTo && Owner)
    if (LinkedTo->Owner == Owner->Engine) delete LinkedTo;
  // unlink func
  if (LinkedTo) {
    // find prev
    C4AulFunc *pAkt = this;
...
void C4AulFunc::DestroyLinked() {
  ...
  while (LinkedTo) delete LinkedTo;
}
...
C4AulFunc *C4AulFunc::FindSameNameFunc(C4Def *pScope) {
  ...
  do {
    ...
    if (pFunc->Owner == Owner->Engine) pResult = pFunc;
...
C4AulFunc *C4AulScript::GetOverloadedFunc(C4AulFunc *ByFunc) {
  ...
  if (!f && Owner) {
    if (f = Owner->GetFuncRecursive(ByFunc->Name))
      ...
      if (ByFunc && f->LinkedTo == ByFunc) f = Owner->GetOverloadedFunc(f);
  }
  // return found fn
  return f;
}
...
C4AulFunc *C4AulScript::GetFuncRecursive(const char *pIdtf) {
  ...
  // nothing found? then search owner, if existant
    ...
    return Owner->GetFuncRecursive(pIdtf);
  return NULL;
}
...
C4AulScriptFunc *C4AulScript::GetSFuncWarn(const char *pIdtf,
                                           C4AulAccess AccNeeded,
                                           const char *WarnStr) {
  // no identifier
  ...
  if (!pFn)
    Warn(FormatString("Error getting %s function '%s'", WarnStr, pIdtf)
             .getData(),
         NULL);
  return pFn;
}

C4AulScriptFunc *C4AulScript::GetSFunc(const char *pIdtf, C4AulAccess AccNeeded,
                                       BOOL fFailsafe) {
  // failsafe call
  if (*pIdtf == '~') {
    fFailsafe = TRUE;
  ...
  if (pFn->Access < AccNeeded) {
    ...
    err.show();
    // don't even break in strict execution, because the caller might be
    // non-strict
...
void C4AulScriptFunc::CopyBody(C4AulScriptFunc &FromFunc) {
  ...
  for (int i = 0; i < C4AUL_MAX_Par; i++) ParType[i] = FromFunc.ParType[i];
...
void C4AulScriptEngine::UnLink() {
  ...
  Strings.Clear();
  // Do not clear global variables and constants, because they are registered by
  // the
  // preparser. Note that keeping those fields means that you cannot delete a
  // global
...
void C4AulScriptEngine::RegisterGlobalConstant(const char *szName,
                                               const C4Value &rValue) {
  // Register name and set value.
  // AddName returns the index of existing element if the name is assigned
  // already.
  // That is OK, since it will only change the value of the global ("overload"
  // it).
  // A warning would be nice here. However, this warning would show up whenever
  // a script
  // containing globals constants is recompiled.
  GlobalConsts[GlobalConstNames.AddName(szName)] = rValue;
}

bool C4AulScriptEngine::GetGlobalConstant(const char *szName,
...
C4AulFuncMap::C4AulFuncMap()
    : Capacity(CapacityInc), FuncCnt(0), Funcs(new C4AulFunc *[CapacityInc]) {
  memset(Funcs, 0, sizeof(C4AulFunc *) * Capacity);
}

C4AulFuncMap::~C4AulFuncMap() { delete[] Funcs; }

unsigned int C4AulFuncMap::Hash(const char *name) {
  // Fowler/Noll/Vo hash
  unsigned int h = 2166136261u;
  while (*name) h = (h ^ *(name++)) * 16777619;
  return h;
}
...
C4AulFunc *C4AulFuncMap::GetFirstFunc(const char *Name) {
  ...
  while (Func && !SEqual(Name, Func->Name)) Func = Func->MapNext;
  return Func;
}

C4AulFunc *C4AulFuncMap::GetNextSNFunc(const C4AulFunc *After) {
  C4AulFunc *Func = After->MapNext;
  while (Func && !SEqual(After->Name, Func->Name)) Func = Func->MapNext;
  return Func;
}
...
C4AulFunc *C4AulFuncMap::GetFunc(const char *Name, const C4AulScript *Owner,
  ...
  C4AulFunc *Func = Funcs[Hash(Name) % Capacity];
  if (After) {
    while (Func && Func != After) Func = Func->MapNext;
    if (Func) Func = Func->MapNext;
  }
