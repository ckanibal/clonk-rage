...
const int32_t C4SCR_MaxIDLen = 100, C4SCR_MaxDesc = 256;
...
class C4ScriptHost : public C4AulScript, public C4ComponentHost {
 ...
 public:
  ...
  BOOL Load(const char *szName, C4Group &hGroup, const char *szFilename,
            const char *szLanguage /*=NULL*/, C4Def *pDef /*=NULL*/,
            class C4LangStringTable *pLocalTable, bool fLoadTable = false);
  const char *GetControlDesc(const char *szFunctionFormat, int32_t iCom,
                             C4ID *pidImage = NULL,
                             int32_t *piImagePhase = NULL);
  void GetControlMethodMask(const char *szFunctionFormat, int32_t &first,
                            int32_t &second);
  int32_t GetControlMethod(int32_t com, int32_t first, int32_t second);
  C4Value ObjectCall(C4Object *pCaller, C4Object *pObj, const char *szFunction,
                     C4AulParSet *pPars = 0, bool fPassError = false) {
    if (!szFunction) return C4VNull;
    return FunctionCall(pCaller, szFunction, pObj, pPars, false, fPassError);
  }
  C4Value Call(const char *szFunction, C4AulParSet *pPars = 0,
 ...
 protected:
  ...
  void MakeScript();
  C4Value FunctionCall(C4Object *pCaller, const char *szFunction,
                       C4Object *pObj, C4AulParSet *pPars = 0,
                       bool fPrivateCall = false, bool fPassError = false);
...
class C4DefScriptHost : public C4ScriptHost {
 public:
  ...
  BOOL Delete() {
    return FALSE;
  }  // do NOT delete this - it's just a class member!
 protected:
 ...
 public:
  ...
  C4AulScriptFunc *
      SFn_ControlTransfer;  // object par(0) tries to get to par(1)/par(2)
  C4AulScriptFunc *SFn_CustomComponents;  // PSF_GetCustomComponents
  int32_t ControlMethod[2], ContainedControlMethod[2],
      ActivationControlMethod[2];
};
...
class C4GameScriptHost : public C4ScriptHost {
 public:
  ...
  void Default();
  C4Value GRBroadcast(const char *szFunction, C4AulParSet *pPars = 0,
                      bool fPassError = false,
                      bool fRejectTest = false);  // call function in scenario
                                                  // script and all
