...
bool C4ValueMapData::operator==(const C4ValueMapData &Data) const {
  ...
  if (pNames)
    for (int i = 0; i < pNames->iSize; i++)
      if (pData[i] != Data.pData[i]) return false;
  return true;
}

C4ValueMapData::~C4ValueMapData() { Reset(); }
...
void C4ValueMapData::SetNameList(C4ValueMapNames *pnNames) {
  ...
  if (pNames) {
    ...
    if (bTempNameList) delete pNames;
    bTempNameList = false;
...
void C4ValueMapData::OnNameListChanged(const char **pOldNames,
                                       int32_t iOldSize) {
  if (!pNames) {
    Reset();
    return;
...
void C4ValueMapData::DenumeratePointers() {
  if (!pNames) return;
  for (int32_t i = 0; i < pNames->iSize; i++) pData[i].DenumeratePointer();
}
...
void C4ValueMapData::CompileFunc(StdCompiler *pComp) {
  ...
  if (fCompiler)
    for (int32_t i = 0; i < iValueCnt; i++) ppNames[i] = 0;
...
C4ValueMapNames::C4ValueMapNames(C4ValueMapNames &NamesToCopy)
    : pNames(0), pExtra(0), iSize(0), pFirst(0) {
  ChangeNameList(const_cast<const char **>(NamesToCopy.pNames),
                 NamesToCopy.pExtra, NamesToCopy.iSize);
}
...
C4ValueMapNames::~C4ValueMapNames() { Reset(); }

void C4ValueMapNames::Reset() {
  // unreg all data lists
  while (pFirst) UnRegister(pFirst);
  // free name list
  for (int32_t i = 0; i < iSize; i++) delete[] pNames[i];
  delete[] pNames;
  delete[] pExtra;
  pNames = NULL;
  pExtra = NULL;
...
void C4ValueMapNames::ChangeNameList(const char **pnNames, int32_t *pnExtra,
  ...
  if (!pnExtra) ZeroMem(pExtra, sizeof(*pExtra) * nSize);
  ...
  // delete old list
  for (i = 0; i < iOldSize; i++) delete[] pOldNames[i];
  delete[] pOldNames;
  delete[] pOldExtra;
...
int32_t C4ValueMapNames::AddName(const char *pnName, int32_t iExtra) {
  ...
  int32_t iNr;
  if ((iNr = GetItemNr(pnName)) != -1) return iNr;
...
int32_t C4ValueMapNames::GetItemNr(const char *strName) {
  for (int32_t i = 0; i < iSize; i++)
    if (SEqual(pNames[i], strName)) return i;
  return -1;
}
