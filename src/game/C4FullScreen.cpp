...
LRESULT APIENTRY
  ...
  switch (uMsg) {
    ...
    case WM_ACTIVATEAPP:
      ...
      if (Application.Active) Game.GraphicsSystem.Execute();
      // update cursor clip
    ...
    case WM_CHAR: {
      ...
      // GUI: forward
      if (Game.pGUI)
        if (Game.pGUI->CharIn(c)) return 0;
      return FALSE;
    }
    case WM_USER_LOG:
...
bool isSpecialKey(unsigned unicode) {
  if (unicode >= 0xe00) return true;
  if (unicode < 32 || unicode == 127) return true;
  return false;
}
}
...
void C4FullScreen::HandleMessage(SDL_Event &e) {
  switch (e.type) {
    ...
    case SDL_MOUSEBUTTONDOWN:
      ...
      Game.GraphicsSystem.MouseMove(button, e.button.x, e.button.y, flags,
                                    NULL);
      break;
...
BOOL C4FullScreen::ViewportCheck() {
  ...
  switch (Game.GraphicsSystem.GetViewportCount()) {
    ...
    case 0:
      ...
      if (fFilm)
        if (pPlr = Game.Players.First) iPlrNum = pPlr->Number;
      // Create viewport
