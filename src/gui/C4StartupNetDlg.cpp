/* by Sven2, 2006 */
// Startup screen for non-parameterized engine start: Network game selection
// dialog
...
C4StartupNetListEntry::C4StartupNetListEntry(C4GUI::ListBox *pForListBox,
    ...
    : pList(pForListBox),
      ...
      iInfoLink(-1) {
  // calc height
  int32_t iLineHgt = C4GUI::GetRes()->TextFont.GetLineHeight(),
          iHeight = iLineHgt * 2 + 4;
  // add icons - normal icons use small size, only animated netgetref uses full
  // size
  rctIconLarge.Set(0, 0, iHeight, iHeight);
  int32_t iSmallIcon = iHeight * 2 / 3;
  rctIconSmall.Set((iHeight - iSmallIcon) / 2, (iHeight - iSmallIcon) / 2,
                   iSmallIcon, iSmallIcon);
  ...
  for (int32_t iIcon = 0; iIcon < MaxInfoIconCount; ++iIcon) {
    AddElement(pInfoIcons[iIcon] =
                   new C4GUI::Icon(rcIconRect, C4GUI::Ico_None));
  ...
  for (int i = 0; i < InfoLabelCount; ++i) {
    ...
    if (!i)
      rcLabelBounds.Wdt -= iLineHgt;  // leave space for topright extra icon
    AddElement(pLbl = pInfoLbl[i] = new C4GUI::Label("", rcLabelBounds, ALeft,
...
C4StartupNetListEntry::~C4StartupNetListEntry() { ClearRef(); }
...
void C4StartupNetListEntry::DrawElement(C4FacetEx &cgo) {
  ...
  if (fIsImportant && !IsSelectedChild(this)) {
    ...
    lpDDraw->DrawBoxDw(cgo.Surface, x1, y1, x1 + rcBounds.Wdt,
                       y1 + rcBounds.Hgt, C4GUI_ImportantBGColor);
  }
  // inherited
...
const char *C4StartupNetListEntry::GetQueryTypeName(QueryType eQueryType) {
  switch (eQueryType) {
    case NRQT_GameDiscovery:
      return LoadResStr("IDS_NET_QUERY_LOCALNET");
    case NRQT_Masterserver:
      return LoadResStr("IDS_NET_QUERY_MASTERSRV");
    case NRQT_DirectJoin:
      return LoadResStr("IDS_NET_QUERY_DIRECTJOIN");
  };
  return "";
}

void C4StartupNetListEntry::SetRefQuery(const char *szAddress,
                                        enum QueryType eQueryType) {
  // safety: clear previous
  ClearRef();
  // setup layout
  ((C4Facet &)pIcon->GetFacet()) =
  ...
  // set info
  sInfoText[0].Format(LoadResStr("IDS_NET_CLIENTONNET"),
                      GetQueryTypeName(eQueryType),
                      pRefClient->getServerName());
  ...
  if (eQueryType == NRQT_Masterserver) iSortOrder = 100;
  // register proc
  C4InteractiveThread &Thread = Application.InteractiveThread;
...
void C4StartupNetListEntry::OnRequestFailed() {
  ++iNumFails;
  // special clonk.de handling: Alternate between ports 80 and 84 and re-request
  // direcly (since we've already waited for the timeout)
...
bool C4StartupNetListEntry::OnReference() {
  ...
  if (!pRefClient || pRefClient->isBusy()) return true;
  // successful?
  ...
  C4Network2Reference **ppNewRefs = NULL;
  int32_t iNewRefCount;
  ...
  if (!iNewRefCount) {
    ...
    C4StartupNetListEntry *pNewRefEntry = this;
    int iPlayerCount = 0;
    for (int i = 0; i < iNewRefCount; i++) {
      pNewRefEntry = AddReference(
          ppNewRefs[i],
          pNewRefEntry->GetNextLower(ppNewRefs[i]->getSortOrder()));
      iPlayerCount +=
          ppNewRefs[i]->Parameters.PlayerInfos.GetActivePlayerCount(false);
    }
  ...
  if (eQueryType == NRQT_Masterserver) {
    ...
    if (pRefClient->GetMessageOfTheDay() && *pRefClient->GetMessageOfTheDay())
      sInfoText[1 + ++iMasterServerMessages].Format(
          LoadResStr("IDS_NET_MOTD"), pRefClient->GetMessageOfTheDay());
    ...
    if (pRefClient->GetLeagueServerRedirect() &&
        (!Config.Network.UseAlternateServer ||
         SEqual(Config.Network.ServerAddress,
                Config.Network.AlternateServerAddress)) &&
        !pNetDlg->GetIgnoreUpdate()) {
      const char *newLeagueServer = pRefClient->GetLeagueServerRedirect();
      if (newLeagueServer &&
          !SEqual(newLeagueServer, Config.Network.ServerAddress)) {
        // this is a new redirect. Inform the user and auto-change servers if
        // desired
        StdStrBuf sMessage;
        sMessage.Format(LoadResStr("IDS_NET_SERVERREDIRECTMSG"),
                        newLeagueServer);
        if (GetScreen()->ShowMessageModal(
                sMessage.getData(), LoadResStr("IDS_NET_SERVERREDIRECT"),
                C4GUI::MessageDialog::btnYesNo, C4GUI::Ico_OfficialServer)) {
          // apply new server setting
          SCopy(newLeagueServer, Config.Network.ServerAddress, CFG_MaxString);
          Config.Save();
          GetScreen()->ShowMessageModal(
              LoadResStr("IDS_NET_SERVERREDIRECTDONE"),
              LoadResStr("IDS_NET_SERVERREDIRECT"), C4GUI::MessageDialog::btnOK,
              C4GUI::Ico_OfficialServer);
    ...
    // masterserver is official: So check version - league server no longer used
    // for engine update - done via UpdateServer instead
  ...
  // non-masterserver
  if (iNewRefCount) {
    // this item has been "converted" into the references - remove without
    // further feedback
    return false;
  } else
...
C4GUI::Element *C4StartupNetListEntry::GetNextLower(int32_t sortOrder) {
  ...
  for (C4GUI::Element *pElem = pList->GetFirst(); pElem;
       pElem = pElem->GetNext()) {
    C4StartupNetListEntry *pEntry = static_cast<C4StartupNetListEntry *>(pElem);
    if (pEntry->iSortOrder < sortOrder) return pElem;
  }
...
void C4StartupNetListEntry::UpdateEntrySize() {
  ...
  while (iLblCnt > 2 && !sInfoText[iLblCnt - 1]) iLblCnt--;
...
void C4StartupNetListEntry::UpdateText() {
  ...
  for (i = 0; i < InfoLabelCount; ++i) {
    ...
    pUseFont->BreakMessage(sInfoText[i].getData(), iAvailableWdt, &BrokenText,
                           true);
    int32_t iHgt, iWdt;
    if (pUseFont->GetTextExtent(BrokenText.getData(), iWdt, iHgt, true)) {
      if ((pInfoLbl[i]->GetBounds().Hgt != iHgt) ||
          (pInfoLbl[i]->GetBounds().Wdt != iAvailableWdt)) {
        C4Rect rcBounds = pInfoLbl[i]->GetBounds();
    ...
    if (iInfoLink == i)
      ...
      pInfoLbl[i]->SetColor(fIsEnabled ? C4GUI_MessageFontClr
...
void C4StartupNetListEntry::AddStatusIcon(C4GUI::Icons eIcon,
...
void C4StartupNetListEntry::SetReference(C4Network2Reference *pRef) {
  ...
  sInfoText[0].Format(LoadResStr("IDS_NET_REFONCLIENT"), pRef->getTitle(),
                      pHost ? pHost->getName() : "unknown");
  sInfoText[1].Format(
      LoadResStr("IDS_NET_INFOPLRSGOALDESC"), (int)iPlrCnt,
  ...
  if (pRef->getTime() > 0) {
    StdStrBuf strDuration;
    strDuration.Format("%02d:%02d:%02d", pRef->getTime() / 3600,
                       (pRef->getTime() % 3600) / 60, pRef->getTime() % 60);
    sInfoText[1].Append(" - ");
    sInfoText[1].Append(strDuration);
  }
  sInfoText[2].Format(LoadResStr("IDS_DESC_VERSION"),
                      pRef->getGameVersion().GetString().getData());
  sInfoText[3].Format("%s: %s", LoadResStr("IDS_CTL_COMMENT"),
                      pRef->getComment());
  StdStrBuf sAddress;
  ...
  if (pRef->isPasswordNeeded())
    AddStatusIcon(C4GUI::Ico_Ex_LockedFrontal,
                  LoadResStr("IDS_NET_INFOPASSWORD"));
  // reg join only
  ...
  if (pRef->isJoinAllowed() &&
      pRef->getGameStatus()
          .isPastLobby())  // A little workaround to determine RuntimeJoin...
    AddStatusIcon(C4GUI::Ico_RuntimeJoin,
                  LoadResStr("IDS_NET_RUNTIMEJOINFREE"));
  // fair crew
  if (pRef->Parameters.UseFairCrew)
    AddStatusIcon(C4GUI::Ico_Ex_FairCrew, LoadResStr("IDS_CTL_FAIRCREW_DESC"));
  // official server
  if (pRef->isOfficialServer() &&
      !Config.Network.UseAlternateServer)  // Offical server icon is only
                                           // displayed if references are
                                           // obtained from official league
                                           // server
  {
    fIsImportant = true;
    AddStatusIcon(C4GUI::Ico_OfficialServer,
                  LoadResStr("IDS_NET_OFFICIALSERVER"));
  }
  // list participating player names
  sInfoText[4].Format(
      "%s: %s", LoadResStr("IDS_CTL_PLAYER"),
      iPlrCnt
          ? pRef->Parameters.PlayerInfos.GetActivePlayerNames(false).getData()
          : LoadResStr("IDS_CTL_NONE"));
...
void C4StartupNetListEntry::SetError(const char *szErrorText,
...
void C4StartupNetListEntry::SetTimeout(TimeoutType eTimeout) {
  ...
  switch (eTimeout) {
    case TT_RefReqWait:
      iTime = (eQueryType == NRQT_Masterserver) ? C4NetMasterServerQueryInterval
                                                : C4NetErrorRefTimeout;
      break;
    case TT_Reference:
      iTime = C4NetReferenceTimeout;
      break;
    case TT_Masterserver:
      iTime = C4NetMasterServerQueryInterval;
      break;
    case TT_Refresh:
      iTime = 1;
      break;  // refresh ASAP
...
C4StartupNetListEntry *C4StartupNetListEntry::AddReference(
    C4Network2Reference *pAddRef, C4GUI::Element *pInsertBefore) {
  ...
  for (C4GUI::Element *pElem = pList->GetFirst(); pElem;
    ...
    if (pEntry->IsSameHost(pAddRef) && pEntry->IsSameAddress(pAddRef) &&
  ...
  // no update - just add
  C4StartupNetListEntry *pNewRefEntry =
      new C4StartupNetListEntry(pList, pInsertBefore, pNetDlg);
...
bool C4StartupNetListEntry::IsSameAddress(const C4Network2Reference *pRef2) {
  ...
  for (int i = 0; i < pRef->getAddrCnt(); i++)
    ...
    for (int j = 0; j < pRef2->getAddrCnt(); j++)
      // at least one match!
      if (pRef->getAddr(i) == pRef2->getAddr(j)) return true;
  // no match
  return false;
}
...
C4StartupNetDlg::C4StartupNetDlg()
    : C4StartupDlg(LoadResStr("IDS_DLG_NETSTART")),
      iGameDiscoverInterval(0),
      pMasterserverClient(NULL),
      fIsCollapsed(false),
      fUpdatingList(false),
      tLastRefresh(0),
      pChatTitleLabel(NULL),
      fIgnoreUpdate(false) {
  // ctor
  // key bindings
  ...
  pKeyBack = new C4KeyBinding(
      keys, "StartupNetBack", KEYSCOPE_Gui,
      new C4GUI::DlgKeyCB<C4StartupNetDlg>(*this, &C4StartupNetDlg::KeyBack),
      C4CustomKey::PRIO_Dlg);
  pKeyRefresh = new C4KeyBinding(
      C4KeyCodeEx(K_F5), "StartupNetReload", KEYSCOPE_Gui,
      new C4GUI::DlgKeyCB<C4StartupNetDlg>(*this, &C4StartupNetDlg::KeyRefresh),
      C4CustomKey::PRIO_CtrlOverride);
  // pKeyForward = new C4KeyBinding(C4KeyCodeEx(K_RIGHT), "StartupNetNext",
  // KEYSCOPE_Gui, - blocks editbox
  //<	new C4GUI::DlgKeyCB<C4StartupNetDlg>(*this,
  //&C4StartupNetDlg::KeyForward), C4CustomKey::PRIO_CtrlOverride);

  // screen calculations
  UpdateSize();
  int32_t iIconSize = C4GUI_IconExWdt;
  int32_t iButtonWidth, iCaptionFontHgt,
      iSideSize = Max<int32_t>(GetBounds().Wdt / 6, iIconSize);
  ...
  C4GUI::ComponentAligner caButtonArea(
      caMain.GetFromBottom(caMain.GetHeight() / 7), 0, 0);
  int32_t iButtonAreaWdt = caButtonArea.GetWidth() * 7 / 8;
  iButtonWidth =
      Min<int32_t>(iButtonWidth, (iButtonAreaWdt - 8 * iButtonIndent) / 4);
  iButtonIndent = (iButtonAreaWdt - 4 * iButtonWidth) / 8;
  C4GUI::ComponentAligner caButtons(
      caButtonArea.GetCentered(iButtonAreaWdt, iButtonHeight), iButtonIndent,
      0);
  C4GUI::ComponentAligner caLeftBtnArea(
      caMain.GetFromLeft(iSideSize),
      Min<int32_t>(caMain.GetWidth() / 20, (iSideSize - C4GUI_IconExWdt) / 2),
      caMain.GetHeight() / 40);
  C4GUI::ComponentAligner caConfigArea(
      caMain.GetFromRight(iSideSize),
      Min<int32_t>(caMain.GetWidth() / 20, (iSideSize - C4GUI_IconExWdt) / 2),
      caMain.GetHeight() / 40);

  // left button area: Switch between chat and game list
  if (C4ChatDlg::IsChatEnabled()) {
    btnGameList = new C4GUI::CallbackButton<C4StartupNetDlg, C4GUI::IconButton>(
        C4GUI::Ico_Ex_GameList, caLeftBtnArea.GetFromTop(iIconSize, iIconSize),
        '\0', &C4StartupNetDlg::OnBtnGameList);
    ...
    btnChat = new C4GUI::CallbackButton<C4StartupNetDlg, C4GUI::IconButton>(
        C4GUI::Ico_Ex_Chat, caLeftBtnArea.GetFromTop(iIconSize, iIconSize),
        '\0', &C4StartupNetDlg::OnBtnChat);
  ...
  C4GUI::ComponentAligner caGameList(pSheetGameList->GetContainedClientRect(),
                                     0, 0, false);
  C4GUI::WoodenLabel *pGameListLbl;
  int32_t iCaptHgt =
      C4GUI::WoodenLabel::GetDefaultHeight(&C4GUI::GetRes()->TextFont);
  pGameListLbl = new C4GUI::WoodenLabel(
      LoadResStr("IDS_NET_GAMELIST"), caGameList.GetFromTop(iCaptHgt),
      C4GUI_Caption2FontClr, &C4GUI::GetRes()->TextFont, ALeft);
  // const char *szGameSelListTip = LoadResStr("IDS_NET_GAMELIST_INFO");
  // disabled this tooltip, it's mainly disturbing when browsing the list
  // pGameListLbl->SetToolTip(szGameSelListTip);
  pSheetGameList->AddElement(pGameListLbl);
  pGameSelList = new C4GUI::ListBox(
      caGameList.GetFromTop(caGameList.GetHeight() - iCaptHgt));
  ...
  pGameSelList->SetSelectionDblClickFn(
      new C4GUI::CallbackHandler<C4StartupNetDlg>(
          this, &C4StartupNetDlg::OnSelDblClick));
  pGameSelList->SetSelectionChangeCallbackFn(
      new C4GUI::CallbackHandler<C4StartupNetDlg>(
          this, &C4StartupNetDlg::OnSelChange));
  ...
  pIPLbl =
      new C4GUI::WoodenLabel(szIPLblText, caIP.GetFromLeft(iIPWdt + 10),
                             C4GUI_Caption2FontClr, &C4GUI::GetRes()->TextFont);
  const char *szIPTip = LoadResStr("IDS_NET_IP_DESC");
  pIPLbl->SetToolTip(szIPTip);
  pSheetGameList->AddElement(pIPLbl);
  pJoinAddressEdt = new C4GUI::CallbackEdit<C4StartupNetDlg>(
      caIP.GetAll(), this, &C4StartupNetDlg::OnJoinAddressEnter);
  ...
  if (C4ChatDlg::IsChatEnabled()) {
    ...
    C4GUI::ComponentAligner caChat(pSheetChat->GetContainedClientRect(), 0, 0,
                                   false);
    pSheetChat->AddElement(pChatTitleLabel = new C4GUI::WoodenLabel(
                               "", caChat.GetFromTop(iCaptHgt),
                               C4GUI_Caption2FontClr,
                               &C4GUI::GetRes()->TextFont, ALeft, false));
    ...
    pChatCtrl->SetTitleChangeCB(new C4GUI::InputCallback<C4StartupNetDlg>(
        this, &C4StartupNetDlg::OnChatTitleChange));
    StdStrBuf sCurrTitle;
    sCurrTitle.Ref(pChatCtrl->GetTitle());
    OnChatTitleChange(sCurrTitle);
    pChatGroup->AddElement(pChatCtrl);
  }

  // config area
  btnInternet = new C4GUI::CallbackButton<C4StartupNetDlg, C4GUI::IconButton>(
      Config.Network.MasterServerSignUp ? C4GUI::Ico_Ex_InternetOn
                                        : C4GUI::Ico_Ex_InternetOff,
      caConfigArea.GetFromTop(iIconSize, iIconSize), '\0',
      &C4StartupNetDlg::OnBtnInternet);
  ...
  btnRecord = new C4GUI::CallbackButton<C4StartupNetDlg, C4GUI::IconButton>(
      Game.Record ? C4GUI::Ico_Ex_RecordOn : C4GUI::Ico_Ex_RecordOff,
      caConfigArea.GetFromTop(iIconSize, iIconSize), '\0',
      &C4StartupNetDlg::OnBtnRecord);
  btnRecord->SetToolTip(LoadResStr("IDS_DLGTIP_RECORD"));
  btnRecord->SetText(LoadResStr("IDS_CTL_RECORD"));
  AddElement(btnRecord);
  btnUpdate = new C4GUI::CallbackButton<C4StartupNetDlg, C4GUI::IconButton>(
      C4GUI::Ico_Ex_Update, caConfigArea.GetFromTop(iIconSize, iIconSize), '\0',
      &C4StartupNetDlg::OnBtnUpdate);
  btnUpdate->SetVisibility(
      false);  // update only available if masterserver notifies it
  ...
  AddElement(btn = new C4GUI::CallbackButton<C4StartupNetDlg>(
                 LoadResStr("IDS_BTN_BACK"),
                 caButtons.GetFromLeft(iButtonWidth),
                 &C4StartupNetDlg::OnBackBtn));
  btn->SetToolTip(LoadResStr("IDS_DLGTIP_BACKMAIN"));
  AddElement(btnRefresh = new C4GUI::CallbackButton<C4StartupNetDlg>(
                 LoadResStr("IDS_BTN_RELOAD"),
                 caButtons.GetFromLeft(iButtonWidth),
                 &C4StartupNetDlg::OnRefreshBtn));
  btnRefresh->SetToolTip(LoadResStr("IDS_NET_RELOAD_DESC"));
  AddElement(btnJoin = new C4GUI::CallbackButton<C4StartupNetDlg>(
                 LoadResStr("IDS_NET_JOINGAME_BTN"),
                 caButtons.GetFromLeft(iButtonWidth),
                 &C4StartupNetDlg::OnJoinGameBtn));
  btnJoin->SetToolTip(LoadResStr("IDS_NET_JOINGAME_DESC"));
  AddElement(btn = new C4GUI::CallbackButton<C4StartupNetDlg>(
                 LoadResStr("IDS_NET_NEWGAME"),
                 caButtons.GetFromLeft(iButtonWidth),
                 &C4StartupNetDlg::OnCreateGameBtn));
...
C4GUI::Control *C4StartupNetDlg::GetDefaultControl() {
  ...
  if (GetDlgMode() == SNDM_Chat && pChatCtrl)
    ...
    // game list mode: No default control, because it would move focus away from
    // IP input edit
    return NULL;
}
...
void C4StartupNetDlg::OnBtnUpdate(C4GUI::Control *btn) {
  ...
  if (!C4UpdateDlg::DoUpdate(UpdateVersion, GetScreen())) {
    GetScreen()->ShowMessage(LoadResStr("IDS_MSG_UPDATEFAILED"),
...
void C4StartupNetDlg::UpdateMasterserver() {
  ...
  btnInternet->SetIcon(Config.Network.MasterServerSignUp
                           ? C4GUI::Ico_Ex_InternetOn
                           : C4GUI::Ico_Ex_InternetOff);
  ...
  if (!Config.Network.MasterServerSignUp) {
    ...
    pMasterserverClient->SetRefQuery(Config.Network.GetLeagueServerAddress(),
                                     C4StartupNetListEntry::NRQT_Masterserver);
  }
}
...
void C4StartupNetDlg::UpdateList(bool fGotReference) {
  ...
  while (pElem = pNextElem) {
    pNextElem =
        pElem->GetNext();  // determine next exec element now - execution
    C4StartupNetListEntry *pEntry = static_cast<C4StartupNetListEntry *>(pElem);
    // do item updates
    bool fKeepEntry = true;
    if (fGotReference) fKeepEntry = pEntry->OnReference();
    if (fKeepEntry) fKeepEntry = pEntry->Execute();
    // remove?
    if (!fKeepEntry) {
      // entry wishes to be removed
      // if the selected entry is being removed, the next entry should be
      // selected (which might be the ref for a finished refquery)
      if (pGameSelList->GetSelectedItem() == pEntry)
        if (pEntry->GetNext()) {
          pGameSelList->SelectEntry(pEntry->GetNext(), false);
        }
      delete pEntry;
      fAnyRemoval = true;  // setting any removal will also update collapsed
                           // state of all entries; so no need to do updates
                           // because of selection change here
    }
  }
  ...
  while (DiscoverClient.PopDiscover(Discover)) {
    StdStrBuf Address;
    Address.Format("%s:%d", inet_ntoa(Discover.sin_addr),
                   htons(Discover.sin_port));
    AddReferenceQuery(Address.getData(),
                      C4StartupNetListEntry::NRQT_GameDiscovery);
  }
  ...
  if (fIsCollapsed && fAnyRemoval) {
    ...
    fIsCollapsed = false;
    UpdateCollapsed();
    // if scrolling is still necessary, the view will be collapsed again
    // immediately
  }
...
void C4StartupNetDlg::UpdateCollapsed() {
  // Uncollapse one element even if list is collapsed. Choose masterserver if no
  ...
  for (C4GUI::Element *pElem = pGameSelList->GetFirst(); pElem;
       pElem = pElem->GetNext()) {
    C4StartupNetListEntry *pEntry = static_cast<C4StartupNetListEntry *>(pElem);
...
C4StartupNetDlg::DlgMode C4StartupNetDlg::GetDlgMode() {
  ...
  if (pMainTabular->GetActiveSheetIndex() == SNDM_Chat)
    return SNDM_Chat;
  else
    return SNDM_GameList;
}

void C4StartupNetDlg::OnThreadEvent(C4InteractiveEventType eEvent,
                                    void *pEventData) {
  UpdateList(true);
}
...
bool C4StartupNetDlg::DoOK() {
  ...
  // OK on editbox with text enetered: Add the specified IP for reference
  // retrieval
  if (GetFocus() == pJoinAddressEdt) {
    ...
    if (szDirectJoinAddress && *szDirectJoinAddress) {
      AddReferenceQuery(szDirectJoinAddress,
                        C4StartupNetListEntry::NRQT_DirectJoin);
  ...
  if (!pSelection) {
    ...
    Game.pGUI->ShowMessageModal(LoadResStr("IDS_NET_NOJOIN_NOREF"),
                                strNoJoin.getData(),
                                C4GUI::MessageDialog::btnOK, C4GUI::Ico_Error);
    return true;
  }
  C4StartupNetListEntry *pRefEntry =
      static_cast<C4StartupNetListEntry *>(pSelection);
  const char *szError;
  if (szError = pRefEntry->GetError()) {
    ...
    Game.pGUI->ShowMessageModal(
        ...
        strNoJoin.getData(), C4GUI::MessageDialog::btnOK, C4GUI::Ico_Error);
    return true;
  }
  ...
  if (pRef) {
    ...
    if (!pRef->isJoinAllowed()) {
      if (!Game.pGUI->ShowMessageModal(
              LoadResStr("IDS_NET_NOJOIN_NORUNTIME"), strNoJoin.getData(),
  ...
  *Game.ScenarioFilename = '\0';
  if (szDirectJoinAddress)
    SCopy(szDirectJoinAddress, Game.DirectJoinAddress, _MAX_PATH);
  else
    *Game.DirectJoinAddress = '\0';
...
void C4StartupNetDlg::DoRefresh() {
  ...
  if (!DiscoverClient.StartDiscovery()) {
    ...
    Game.pGUI->ShowMessageModal(
        FormatString(LoadResStr("IDS_NET_NODISCOVERY_DESC"),
                     DiscoverClient.GetError()).getData(),
        strNoDiscovery.getData(), C4GUI::MessageDialog::btnAbort,
        C4GUI::Ico_Error);
  }
  iGameDiscoverInterval = C4NetGameDiscoveryInterval;
...
void C4StartupNetDlg::OnSec1Timer() {
  // no updates if dialog is inactive (e.g., because a join password dlg is
  // shown!)
...
void C4StartupNetDlg::AddReferenceQuery(
    const char *szAddress, C4StartupNetListEntry::QueryType eQueryType) {
  ...
  C4StartupNetListEntry *pEntry =
      new C4StartupNetListEntry(pGameSelList, NULL, this);
...
void C4StartupNetDlg::OnChatTitleChange(const StdStrBuf &sNewTitle) {
  // update label
  if (pChatTitleLabel)
    pChatTitleLabel->SetText(FormatString("%s - %s", LoadResStr("IDS_DLG_CHAT"),
                                          sNewTitle.getData()).getData());
}
