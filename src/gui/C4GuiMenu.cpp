...
void ContextMenu::Entry::DrawElement(C4FacetEx &cgo) {
  ...
  if (icoIcon > Ico_None) {
    ...
    if (!iXMax) iXMax = 6;
    // load icon
    const C4Facet &rfctIcon =
        GetRes()->fctIcons.GetPhase(icoIcon % iXMax, icoIcon / iXMax);
    rfctIcon.DrawX(cgo.Surface, rcBounds.x + cgo.TargetX,
                   rcBounds.y + cgo.TargetY, rcBounds.Hgt, rcBounds.Hgt);
  }
  // print out label
  if (!!sText)
    lpDDraw->TextOut(sText.getData(), GetRes()->TextFont, 1.0f, cgo.Surface,
                     cgo.TargetX + rcBounds.x + GetIconIndent(),
                     rcBounds.y + cgo.TargetY, C4GUI_ContextFontClr, ALeft);
  // submenu arrow
  if (pSubmenuHandler) {
    C4Facet &rSubFct = GetRes()->fctSubmenu;
    rSubFct.Draw(cgo.Surface,
                 cgo.TargetX + rcBounds.x + rcBounds.Wdt - rSubFct.Wdt,
                 cgo.TargetY + rcBounds.y + (rcBounds.Hgt - rSubFct.Hgt) / 2);
  }
}

ContextMenu::Entry::Entry(const char *szText, Icons icoIcon,
                          MenuHandler *pMenuHandler,
                          ContextHandler *pSubmenuHandler)
    : Element(),
      cHotkey(0),
      icoIcon(icoIcon),
      pMenuHandler(pMenuHandler),
      pSubmenuHandler(pSubmenuHandler) {
  // set text with hotkey
  if (szText) {
    sText.Copy(szText);
    ExpandHotkeyMarkup(sText, cHotkey);
    // adjust size
    GetRes()->TextFont.GetTextExtent(sText.getData(), rcBounds.Wdt,
                                     rcBounds.Hgt, true);
  } else {
    rcBounds.Wdt = 40;
...
ContextMenu::ContextMenu()
    : Window(), pTarget(NULL), pSelectedItem(NULL), pSubmenu(NULL) {
  ...
  pKeySelUp =
      new C4KeyBinding(Keys, "GUIContextSelUp", KEYSCOPE_Gui,
                       new C4KeyCB<ContextMenu>(*this, &ContextMenu::KeySelUp),
                       C4CustomKey::PRIO_Context);
  ...
  pKeySelDown = new C4KeyBinding(
      Keys, "GUIContextSelDown", KEYSCOPE_Gui,
      new C4KeyCB<ContextMenu>(*this, &ContextMenu::KeySelDown),
      C4CustomKey::PRIO_Context);
  ...
  pKeySubmenu = new C4KeyBinding(
      Keys, "GUIContextSubmenu", KEYSCOPE_Gui,
      new C4KeyCB<ContextMenu>(*this, &ContextMenu::KeySubmenu),
      C4CustomKey::PRIO_Context);
  ...
  pKeyBack =
      new C4KeyBinding(Keys, "GUIContextBack", KEYSCOPE_Gui,
                       new C4KeyCB<ContextMenu>(*this, &ContextMenu::KeyBack),
                       C4CustomKey::PRIO_Context);
  ...
  pKeyAbort =
      new C4KeyBinding(Keys, "GUIContextAbort", KEYSCOPE_Gui,
                       new C4KeyCB<ContextMenu>(*this, &ContextMenu::KeyAbort),
                       C4CustomKey::PRIO_Context);
  ...
  pKeyConfirm = new C4KeyBinding(
      Keys, "GUIContextConfirm", KEYSCOPE_Gui,
      new C4KeyCB<ContextMenu>(*this, &ContextMenu::KeyConfirm),
      C4CustomKey::PRIO_Context);

  pKeyHotkey = new C4KeyBinding(
      C4KeyCodeEx(KEY_Any), "GUIContextHotkey", KEYSCOPE_Gui,
      new C4KeyCBPassKey<ContextMenu>(*this, &ContextMenu::KeyHotkey),
      C4CustomKey::PRIO_Context);
}
...
void ContextMenu::DrawElement(C4FacetEx &cgo) {
  ...
  lpDDraw->DrawBoxDw(
      ...
      rcBounds.y + rcBounds.Hgt + cgo.TargetY - 1, C4GUI_ContextBGColor);
  ...
  if (pSelectedItem) {
    ...
    lpDDraw->DrawBoxDw(
        ...
        rcSelArea.y + rcSelArea.Hgt + cgo.TargetY - 1, C4GUI_ContextSelColor);
  }
  // draw frame
  Draw3DFrame(cgo);
}

void ContextMenu::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
...
void ContextMenu::UpdateElementPositions() {
  ...
  rcBounds.Hgt =
      Max<int32_t>(iOverallHgt, 8) + GetMarginTop() + GetMarginBottom();
  UpdateSize();
}
...
void ContextMenu::RemoveElement(Element *pChild) {
  ...
  // forward to mouse
  if (GetScreen()) GetScreen()->Mouse.RemoveElement(pChild);
  // update positions
...
bool ContextMenu::CtxMouseInput(CMouse &rMouse, int32_t iButton,
                                int32_t iScreenX, int32_t iScreenY,
                                DWORD dwKeyParam) {
  // check submenu
  if (pSubmenu)
    if (pSubmenu->CtxMouseInput(rMouse, iButton, iScreenX, iScreenY,
                                dwKeyParam))
      return true;
  // check bounds
  if (!rcBounds.Contains(iScreenX, iScreenY)) return false;
  // inside menu: do input in local coordinates
  MouseInput(rMouse, iButton, iScreenX - rcBounds.x, iScreenY - rcBounds.y,
             dwKeyParam);
  return true;
}
...
void ContextMenu::CheckOpenSubmenu() {
  ...
  // get open pos
  int32_t iX =
      GetClientRect().x + pSelEntry->GetBounds().x + pSelEntry->GetBounds().Wdt;
  int32_t iY = GetClientRect().y + pSelEntry->GetBounds().y +
               pSelEntry->GetBounds().Hgt / 2;
  int32_t iScreenWdt = GetScreen()->GetBounds().Wdt,
          iScreenHgt = GetScreen()->GetBounds().Hgt;
...
ContextButton::ContextButton(C4Rect &rtBounds)
    : Control(rtBounds), iOpenMenu(0), fMouseOver(false) {
  RegisterContextKey();
}

ContextButton::ContextButton(Element *pForEl, bool fAdd, int32_t iHIndent,
                             int32_t iVIndent)
  ...
  Container *pCont;
  if (fAdd)
    if (pCont = pForEl->GetContainer()) pCont->AddElement(this);
  RegisterContextKey();
}

ContextButton::~ContextButton() { delete pKeyContext; }
...
void ContextButton::RegisterContextKey() {
  ...
  pKeyContext = new C4KeyBinding(
      ContextKeys, "GUIContextButtonPress", KEYSCOPE_Gui,
      new ControlKeyCB<ContextButton>(*this, &ContextButton::KeyContext),
...
void ContextButton::DrawElement(C4FacetEx &cgo) {
  ...
  if (HasDrawFocus() || (fMouseOver && IsInActiveDlg(false)) || iOpenMenu) {
    ...
    GetRes()->fctButtonHighlight.DrawX(cgo.Surface, x0, y0, rcBounds.Wdt,
                                       rcBounds.Hgt);
    lpDDraw->ResetBlitMode();
  }
}

void ContextButton::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
                               int32_t iY, DWORD dwKeyParam) {
