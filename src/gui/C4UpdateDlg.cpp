...
C4UpdateDlg::C4UpdateDlg()
    : C4GUI::InfoDialog(LoadResStr("IDS_TYPE_UPDATE"), 10) {
  // initial text update
  UpdateText();
...
void C4UpdateDlg::UserClose(bool fOK) {
  ...
  if (UpdateRunning) {
    GetScreen()->ShowMessage(LoadResStr("IDS_MSG_UPDATEINPROGRESS"),
...
void C4UpdateDlg::UpdateText() {
  ...
  if (amount_read == -1) {
    ...
    StdStrBuf Errormessage =
        FormatString("read error from c4group: %s", strerror(errno));
  ...
  // EOF: Update done.
    ...
    // If c4group did not exit but something else caused EOF, then that's bad.
    // But don't hang.
    int child_status = 0;
...
inline static bool IsWindowsVista() {
#ifdef _WIN32
...
bool C4UpdateDlg::DoUpdate(const C4GameVersion &rUpdateVersion,
  ...
  // Objects major update: we will update to the first minor of the next major
  // version - we can not skip major versions or jump directly to a higher minor
  // of the next major version.
  if (rUpdateVersion.iVer[2] > C4XVER3)
    strUpdateFile.Format(C4CFG_UpdateMajor, (int)rUpdateVersion.iVer[0],
                         (int)rUpdateVersion.iVer[1], C4XVER3 + 1, 0, C4_OS);
  // Objects version match: engine update only
  else if ((rUpdateVersion.iVer[2] == C4XVER3) &&
           (rUpdateVersion.iVer[3] == C4XVER4))
    strUpdateFile.Format(C4CFG_UpdateEngine, (int)rUpdateVersion.iBuild, C4_OS);
  // Objects version mismatch: full objects update
  else
    strUpdateFile.Format(
        C4CFG_UpdateObjects, (int)rUpdateVersion.iVer[0],
        (int)rUpdateVersion.iVer[1], (int)rUpdateVersion.iVer[2],
        (int)rUpdateVersion.iVer[3], (int)rUpdateVersion.iBuild, C4_OS);
  // Compose full update URL by using update server address and replacing last
  // path element name with the update file
  ...
  StdStrBuf strLocalFilename;
  strLocalFilename.Copy(GetFilename(strUpdateFile.getData()));
  ...
  if (!C4DownloadDlg::DownloadFile(
          LoadResStr("IDS_TYPE_UPDATE"), pScreen, strUpdateURL.getData(),
          strLocalFilename.getData(), LoadResStr("IDS_MSG_UPDATENOTAVAILABLE")))
    // Download failed (return success, because error message has already been
    // shown)
    return true;
...
bool C4UpdateDlg::ApplyUpdate(const char *strUpdateFile, bool fDeleteUpdate,
                              C4GUI::Screen *pScreen) {
  // Determine name of update program
  StdStrBuf strUpdateProg;
  strUpdateProg.Copy(C4CFN_UpdateProgram);
  // Windows: manually append extension because ExtractEntry() cannot properly
  // glob and Extract() doesn't return failure values
  if (SEqual(C4_OS, "win32")) strUpdateProg += ".exe";
  // Determine name of local extract of update program
  StdStrBuf strUpdateProgEx;
  strUpdateProgEx.Copy(strUpdateProg);
  // Windows Vista: rename update program to setup.exe for UAC elevation and in
  // temp path
  ...
  if (!UpdateGroup.ExtractEntry(strUpdateProg.getData(),
                                strUpdateProgEx.getData()))
    // Not found: look for an engine update pack one level down
    if (UpdateGroup.FindEntry(FormatString("cr_*_%s.c4u", C4_OS).getData(),
                              strSubGroup))
      // Extract update program from sub group
      if (SubGroup.OpenAsChild(&UpdateGroup, strSubGroup)) {
        SubGroup.ExtractEntry(strUpdateProg.getData(),
                              strUpdateProgEx.getData());
        SubGroup.Close();
      }
  UpdateGroup.Close();
  ...
  // Notice: even if the update program and update group are in the temp path,
  // they must be executed in our working directory
  StdStrBuf strUpdateArgs;
  strUpdateArgs.Format("\"%s\" /p -w \"" C4ENGINECAPTION
                       "\" -w \"" C4EDITORCAPTION "\" -w 2000 %s",
                       strUpdateFile, fDeleteUpdate ? "-yd" : "-y");
  int iError = (int)ShellExecute(NULL, "open", strUpdateProgEx.getData(),
                                 strUpdateArgs.getData(),
                                 Config.General.ExePath, SW_SHOW);
  ...
  switch (pid = fork()) {
    ...
    case 0:
      ...
      if (c4group_output[1] != STDOUT_FILENO &&
          c4group_output[1] != STDERR_FILENO)
        close(c4group_output[1]);
      execl(C4CFN_UpdateProgram, C4CFN_UpdateProgram, "-v", strUpdateFile,
            (fDeleteUpdate ? "-yd" : "-y"), static_cast<char *>(0));
    ...
    default:
      ...
      // Open the update log dialog (this will update itself automatically from
      // c4group_output)
...
bool C4UpdateDlg::IsValidUpdate(const C4GameVersion &rNewVer) {
  ...
  if ((rNewVer.iVer[0] != C4XVER1) || (rNewVer.iVer[1] != C4XVER2))
    return false;
...
bool C4UpdateDlg::CheckForUpdates(C4GUI::Screen *pScreen, bool fAutomatic) {
  ...
  if (fAutomatic)
    if (time(NULL) - Config.Network.LastUpdateTime < 60 * 60 * 24) return false;
  // Store the time of this update check (whether it's automatic or not or
  // successful or not)
  ...
  if (C4GUI::IsGUIValid()) {
    pWaitDlg = new C4GUI::MessageDialog(
        LoadResStr("IDS_MSG_LOOKINGFORUPDATES"),
        Config.Network.UpdateServerAddress, C4GUI::MessageDialog::btnAbort,
        C4GUI::Ico_Ex_Update, C4GUI::MessageDialog::dsRegular);
  ...
  StdStrBuf strQuery;
  strQuery.Format("%s?action=version", Config.Network.UpdateServerAddress);
  if (VerChecker.Init() && VerChecker.SetServer(strQuery.getData()) &&
      VerChecker.QueryVersion()) {
    ...
    while (VerChecker.isBusy()) {
      ...
      if (pWaitDlg)
        if (!C4GUI::IsGUIValid() || !pWaitDlg->IsShown()) {
          fAborted = true;
          break;
        }
    }
  ...
  if (!fSuccess) {
    StdStrBuf sError;
    sError.Copy(LoadResStr("IDS_MSG_UPDATEFAILED"));
    const char *szErrMsg = VerChecker.GetError();
    if (szErrMsg) {
      sError.Append(": ");
      sError.Append(szErrMsg);
    }
    pScreen->ShowMessage(sError.getData(), Config.Network.UpdateServerAddress,
                         C4GUI::Ico_Ex_Update);
    return false;
  }
  ...
  if ((strUpdateRedirect.getLength() > 0) &&
      !SEqual(strUpdateRedirect.getData(),
              Config.Network.UpdateServerAddress)) {
    ...
    if (pScreen->ShowMessageModal(
      ...
      Config.Save();
      pScreen->ShowMessageModal(LoadResStr("IDS_NET_SERVERREDIRECTDONE"),
                                LoadResStr("IDS_NET_SERVERREDIRECT"),
  ...
  if (C4UpdateDlg::IsValidUpdate(UpdateVersion)) {
    ...
    StdStrBuf strMsg;
    strMsg.Format(LoadResStr("IDS_MSG_ANUPDATETOVERSIONISAVAILA"),
                  UpdateVersion.GetString().getData());
    if (pScreen->ShowMessageModal(
            strMsg.getData(), Config.Network.UpdateServerAddress,
            C4GUI::MessageDialog::btnYesNo, C4GUI::Ico_Ex_Update))
      if (!DoUpdate(UpdateVersion, pScreen))
        pScreen->ShowMessage(LoadResStr("IDS_MSG_UPDATEFAILED"),
                             Config.Network.UpdateServerAddress,
                             C4GUI::Ico_Ex_Update);
      else
        return true;
  ...
  // No applicable update available
    ...
    if (!fAutomatic)
      pScreen->ShowMessage(LoadResStr("IDS_MSG_NOUPDATEAVAILABLEFORTHISV"),
                           Config.Network.UpdateServerAddress,
                           C4GUI::Ico_Ex_Update);
  }
...
bool C4Network2VersionInfoClient::GetVersion(C4GameVersion *piVerOut) {
  ...
  try {
    CompileFromBuf<StdCompilerINIRead>(
        mkNamingAdapt(mkNamingAdapt(mkParAdapt(*piVerOut, false), "Version"),
                      C4ENGINENAME),
        ResultString);
...
bool C4Network2VersionInfoClient::GetRedirect(StdStrBuf &rRedirect) {
  ...
  try {
    CompileFromBuf<StdCompilerINIRead>(
        mkNamingAdapt(
            mkNamingAdapt(mkParAdapt(strUpdateRedirect, StdCompiler::RCT_All),
