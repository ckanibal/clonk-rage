...
class C4StartupPlrSelDlg : public C4StartupDlg {
 private:
  enum Mode {
    PSDM_Player = 0,
    PSDM_Crew
  };                              // player selection list, or crew editing mode
 ...
 private:
  ...
  class ListItem : public C4GUI::Control {
   ...
   private:
    class C4KeyBinding *
        pKeyCheck;  // space activates/deactivates selected player
    C4FacetExSurface fctPortrait, fctPortraitBase;  // big portrait
    StdStrBuf Filename;                             // file info was loaded from

   public:
    ListItem(C4StartupPlrSelDlg *pForDlg, C4GUI::ListBox *pForListBox,
   ...
   protected:
    ...
    C4GUI::ContextMenu *ContextMenu(C4GUI::Element *pEl, int32_t iX,
                                    int32_t iY) {
      return ContextMenu();
    }
    ...
    virtual bool IsFocusOnClick() {
      return false;
    }  // do not focus; keep focus on listbox
   ...
   public:
    ListItem *GetNext() const {
      return static_cast<ListItem *>(BaseClass::GetNext());
    }
    ...
    virtual void SetSelectionInfo(
        C4GUI::TextWindow *pSelectionInfo) = 0;  // clears text field and writes
                                                 // selection info text into it
 ...
 public:
  ...
  class PlayerListItem : public ListItem {
   ...
   public:
    PlayerListItem(C4StartupPlrSelDlg *pForDlg, C4GUI::ListBox *pForListBox,
                   C4GUI::Element *pInsertBeforeElement = NULL,
                   bool fActivated = false);
    ~PlayerListItem() {}
   ...
   public:
    ...
    void UpdateCore(
        C4PlayerInfoCore &NewCore);  // Save Core to disk and update this item
 ...
 private:
  ...
  class CrewListItem : public ListItem {
   ...
   public:
    CrewListItem(C4StartupPlrSelDlg *pForDlg, C4GUI::ListBox *pForListBox,
                 uint32_t dwPlrClr);
    ~CrewListItem() {}

    void Load(C4Group &rGrp,
              const StdStrBuf &rsFilename);  // may throw LoadError
   ...
   private:
    StdStrBuf GetPhysicalTextLine(
        int32_t iPhysValue,
        const char *idsName);  // get string for physical info bar
   ...
   public:
    ...
    virtual uint32_t GetColorDw() const {
      return dwPlrClr;
    };  // get drawing color for portrait
    virtual void SetSelectionInfo(
        C4GUI::TextWindow *pSelectionInfo);  // clears text field and writes
                                             // selection info text into it
    virtual StdStrBuf GetDelWarning();
    const C4ObjectInfoCore &GetCore() const { return Core; }

    CrewListItem *GetNext() const {
      return static_cast<CrewListItem *>(ListItem::GetNext());
    }
 ...
 private:
  class C4KeyBinding *pKeyBack, *pKeyProperties, *pKeyCrew, *pKeyDelete,
      *pKeyRename, *pKeyNew;
 ...
 private:
  C4Rect rcBottomButtons;
  int32_t iBottomButtonWidth;
  class C4GUI::Button *btnActivatePlr, *btnCrew, *btnProperties, *btnDelete,
      *btnBack, *btnNew;

  void UpdateBottomButtons();  // update command button texts and positions
  void UpdatePlayerList();  // refill pPlrListBox with players in player folder,
                            // or with crew in selected player
  void UpdateSelection();
  void OnSelChange(class C4GUI::Element *pEl) { UpdateSelection(); }
  void OnSelDblClick(class C4GUI::Element *pEl) {
    C4GUI::GUISound("Click");
    OnPropertyBtn(NULL);
  }
  void UpdateActivatedPlayers();  // update Config.General.Participants by
                                  // currently activated players
  void SelectItem(const StdStrBuf &Filename,
                  bool fActivate);  // find item by filename and select (and
                                    // activate it, if desired)

  void SetPlayerMode();  // change view to listing players
  void SetCrewMode(
      PlayerListItem *pForPlayer);  // change view to listing crew of a player

  static int32_t CrewSortFunc(const C4GUI::Element *pEl1,
                              const C4GUI::Element *pEl2, void *par);
  void ResortCrew();

 protected:
  void OnItemCheckChange(C4GUI::Element *pCheckBox);
  static bool CheckPlayerName(const StdStrBuf &Playername, StdStrBuf &Filename,
                              const StdStrBuf *pPrevFilename, bool fWarnEmpty);
  ListItem *GetSelection();
  void SetSelection(ListItem *pNewItem);

  C4GUI::RenameEdit *pRenameEdit;  // hack: set by crew list item renaming. Must
                                   // be cleared when something is done in the
                                   // dlg
  void AbortRenaming();
...
class C4StartupPlrPropertiesDlg : public C4GUI::Dialog {
 protected:
  C4StartupPlrSelDlg *
      pMainDlg;  // may be NULL if shown as creation dialog in main menu!
  ...
  C4FacetExSurface fctNewPicture,
      fctNewBigIcon;                  // if assigned, save new picture/bigicon
 ...
 private:
  ...
  bool SetNewPicture(C4Surface &srcSfc, C4FacetExSurface *trgFct,
                     int32_t iMaxSize, bool fColorize);
  void SetNewPicture(const char *szFromFilename, bool fSetPicture,
                     bool fSetBigIcon);  // set new picture/bigicon by loading
                                         // and scaling if necessary. If
                                         // szFromFilename==NULL, clear
                                         // picture/bigicon

 public:
  C4StartupPlrPropertiesDlg(C4StartupPlrSelDlg::PlayerListItem *pForPlayer,
