...
class C4PlayerInfoListBox : public C4GUI::ListBox {
 ...
 private:
  ...
  class ListItem : public C4GUI::Window {
   ...
   public:
    struct ID {
      enum IDType {
        PLI_NONE = 0,
        PLI_SCRIPTPLR,    // script player caption (ID=0) - script players
                          // themselbed are regular PLI_PLAYER
   ...
   protected:
    ...
    bool CanLocalChooseTeams(int32_t idPlayer = 0)
        const;  // whether the local client can change any teams

   public:
    ListItem(C4PlayerInfoListBox *pList)
        : pList(pList), dwBackground(0), C4GUI::Window() {}
  };
  ...
  class PlayerListItem : public ListItem {
   private:
    ...
    C4GUI::Label *
        pTimeLabel;  // evaluation only: label showing total playing time
    C4GUI::Label *
        pExtraLabel;  // evaluation only: label showing extra data set by script
    C4GUI::Icon *pRankIcon;  // league rank icon
    C4GUI::ComboBox *
        pTeamCombo;            // team selection combo - NULL for no-team-scens
   ...
   protected:
    ...
    bool fFreeSavegamePlayer;    // if set, the player is an (unassociated)
                                 // savegame player
   ...
   public:
    PlayerListItem(C4PlayerInfoListBox *pForListBox, int32_t idClient,
                   int32_t idPlayer, bool fSavegamePlayer,
                   C4GUI::Element *pInsertBeforeElement);  // ctor
    ~PlayerListItem(){};                                   // dtor

    void UpdateIcon(C4PlayerInfo *pInfo,
                    C4PlayerInfo *pJoinedInfo);  // update player icon
    void UpdateTeam();
    void UpdateScoreLabel(
        C4PlayerInfo *pInfo);  // update league score labels and icons
    void UpdateCollapsed();

   public:
    C4GUI::ContextMenu *OnContext(C4GUI::Element *pListItem, int32_t iX,
                                  int32_t iY);  // open context menu
    C4GUI::ContextMenu *OnContextTakeOver(
        C4GUI::Element *pListItem, int32_t iX,
        int32_t iY);  // takeover savegame player submenu
    ...
    bool OnTeamComboSelChange(C4GUI::ComboBox *pForCombo,
    ...
    C4PlayerInfo *GetJoinedInfo() const;  // if this player is joined or
                                          // associated to a joined info, return
                                          // the joined info

    bool IsLocalClientPlayer()
        const;  // whether this player is going to join locally
    bool CanLocalChooseTeam()
        const;  // whether the local client can change team for this player
  };
  ...
  class ClientListItem : public ListItem {
   ...
   protected:
    ...
    time_t tLastSoundTime;  // now() when the client last issued a sound
                            // (display as sound icon). 0 for no sound.

   public:
    ClientListItem(C4PlayerInfoListBox *pForListBox,
                   const C4ClientCore &rClientInfo,
    ...
    void SetPing(
        int32_t iToPing);  // update ping label; iToPing=-1 removes the label
    void UpdateCore(const C4ClientCore &rCore);  // core update: re-set name and
                                                 // indicate whether active or
                                                 // not
    void SetSoundIcon();  // sets the sound icon as current icon and schedules
   ...
   public:
    ...
    bool IsLocalClientPlayer()
        const;  // whether this player is going to join locally
    C4GUI::Icons GetCurrentStatusIcon();  // get status icon that shows the
                                          // current client state
    class C4Network2Client *GetNetClient()
        const;  // return assicuated network client; NULL for local
    bool IsLocal() const;
    ...
    C4GUI::ContextMenu *OnContext(C4GUI::Element *pListItem, int32_t iX,
                                  int32_t iY);  // open context menu
    void OnCtxKick(
        C4GUI::Element *pListItem);  // kick item selected in client ctx menu
  ...
  class TeamListItem : public ListItem {
   ...
   public:
    TeamListItem(C4PlayerInfoListBox *pForListBox, int32_t idTeam,
                 ListItem *pInsertBefore);  // ctor

    // spacing inserted between of those list items; usually this is top
    // anyway...
    virtual bool GetListItemTopSpacingBar() { return true; }
    virtual int32_t GetListItemTopSpacing() { return ClientListBoxSpacing; }
   ...
   private:
    void MoveLocalPlayersIntoTeam();  // move all local players into team marked
                                      // by this label
  };
  ...
  class FreeSavegamePlayersListItem : public ListItem {
   ...
   public:
    FreeSavegamePlayersListItem(C4PlayerInfoListBox *pForListBox,
                                ListItem *pInsertBefore);  // ctor
  ...
  class ScriptPlayersListItem : public ListItem {
   ...
   public:
    ScriptPlayersListItem(C4PlayerInfoListBox *pForListBox,
                          ListItem *pInsertBefore);  // ctor
  ...
  // list of players in record (currently not used, because replays in network
  // are not allowed :C)
  class ReplayPlayersListItem : public ListItem {
   ...
   public:
    ReplayPlayersListItem(C4PlayerInfoListBox *pForListBox,
                          ListItem *pInsertBefore);  // ctor
 ...
 private:
  Mode eMode;
  int32_t iMaxUncollapsedPlayers;  // maximum number of players that can be
                                   // displayed without collapse - valid only if
                                   // fIsCollapsed
  bool fIsCollapsed;
  int32_t iTeamFilter;  // if nonzero, only playeers of this team are shown in
                        // the listbox
  ...
  void UpdatePlayersByEvaluation(ListItem **ppCurrInList, C4Team *pTeam,
                                 AddMode eAddMode);

  ListItem *GetPlayerListItem(ListItem::ID::IDType eType,
                              int32_t id);  // search for a player list item
  bool PlrListItemUpdate(ListItem::ID::IDType eType, int32_t id,
                         class ListItem **pEnsurePos);  // search for player
                                                        // list item with given
                                                        // ID in the list
                                                        // starting at
                                                        // ensurepos; ensure
                                                        // it's positioned at
                                                        // given pos; update and
                                                        // return true if found

  bool IsEvaluation() const {
    return eMode == PILBM_Evaluation || eMode == PILBM_EvaluationNoWinners;
  }
  bool IsLobby() const {
    return eMode == PILBM_LobbyClientSort || eMode == PILBM_LobbyTeamSort;
  }
  bool IsTeamFilter() const { return !!iTeamFilter; }

 protected:
  bool IsPlayerItemCollapsed(PlayerListItem *pItem);  // CB from list item:
                                                      // return true if it
 ...
 public:
  C4PlayerInfoListBox(const C4Rect &rcBounds, Mode eMode,
                      int32_t iTeamFilter = 0);
