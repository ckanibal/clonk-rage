...
const int C4NetMasterServerQueryInterval =
    30;  // seconds after last and beforenew game query to master server
const int C4NetRefRequestTimeout =
    12;  // seconds after which the reference request is interrupted
const int C4NetReferenceTimeout =
    42;  // seconds after which references are removed from the list
         // (C4NetRefRequestTimeout + C4NetMasterServerQueryInterval)
const int C4NetErrorRefTimeout =
    10;  // seconds after which failed reference requestsare removed
const int C4NetGameDiscoveryInterval = 30;  // seconds
const int C4NetMinRefreshInterval =
    8;  // seconds; minimum time between refreshes

class C4StartupNetListEntry : public C4GUI::Window {
 public:
  C4StartupNetListEntry(C4GUI::ListBox *pForListBox,
  ...
  enum TimeoutType {
    TT_RefReqWait,    // C4NetMasterServerQueryInterval for masterserver;
                      // C4NetErrorRefTimeout for other ref clients
 ...
 private:
  ...
  C4GUI::ListBox *pList;
  StdStrBuf sRefClientAddress;  // set during reference retrieval: reference
                                // server address
  C4Network2RefClient *
      pRefClient;  // set during reference retrieval: reference request client
  C4Network2Reference *pRef;  // set for retrieved references
  bool fError;  // if set, the label was changed to an error message and no more
                // updates are done
  StdStrBuf sError;
  QueryType eQueryType;  // valid if pRefClient is set: Where the ref query is
                         // originating

  time_t iTimeout;  // lifetime: If set, marks time when the item is removed or
                    // should re-query
  time_t iRequestTimeout;  // if this times out while the request remains busy,
                           // the ref client assumes that the tcp socket blocked
                           // upon request and aborts it
  ...
  C4GUI::Icon *pInfoIcons[MaxInfoIconCount];  // right-aligned status icons at
                                              // topright position
  int32_t iInfoLink;  // if !=-1, index of the label that should be turned into
                      // a clickable link (for MotD-link)
  ...
  bool fIsImportant;  // if set, the item is presented in yellow (lower priority
                      // than fIsEnabled)
  ...
  static const char *GetQueryTypeName(
      QueryType eQueryType);  // name of QueryType values
  void SetError(const char *szErrorText,
                TimeoutType eTimeout);  // change secondary label to error
                                        // label, mark error and set a removal
                                        // timer
  void SetTimeout(TimeoutType eTimeout);
  C4StartupNetListEntry *AddReference(
      C4Network2Reference *pAddRef,
      C4GUI::Element *
          pInsertBefore);  // add a reference list item to the same list
  void InvalidateStatusIcons() {
    iInfoIconCount = 0;
  }  // schedule all current status icons for removal when UpdateText is called
     // next
  void AddStatusIcon(
      C4GUI::Icons eIcon,
      const char *szToolTip);  // add a status icon with the specified tooltip
 ...
 protected:
  ...
  C4GUI::Element *GetNextLower(int32_t sortOrder);  // returns the element
                                                    // before which this element
                                                    // should be inserted

 public:
  void ClearRef();  // del any ref/refclient/error data
  void SetRefQuery(const char *szAddress,
                   QueryType eQueryType);  // start loading references fromt he
                                           // specified address
  void SetReference(C4Network2Reference *pNewRef);  // replace the reference

  bool
  Execute();  // update stuff - if false is returned, the item is to be removed
  void OnRequestFailed();  // called on connection error or timeout
  bool OnReference();  // like Execute(), but only called if some reference was
                       // received
  void UpdateCollapsed(bool fToCollapseValue);

  const char *GetError() {
    return fError ? sError.getData() : NULL;
  }                                      // return error message, if any is set
  C4Network2Reference *GrabReference();  // grab the reference so it won't be
                                         // deleted when this item is removed
  C4Network2Reference *GetReference() const {
    return pRef;
  }  // have a look at the reference
  bool IsSameHost(const C4Network2Reference *pRef2);  // check whether the
                                                      // reference was created
                                                      // by the same host as
                                                      // this one
  bool IsSameAddress(const C4Network2Reference *pRef2);   // check whether there
                                                          // is at least one
                                                          // matching address
                                                          // (address and port)
  bool IsSameRefQueryAddress(const char *szJoinAddress);  // check whether
                                                          // reference query was
                                                          // created with same
                                                          // host address
...
class C4StartupNetDlg : public C4StartupDlg,
                        private C4InteractiveThread::Callback {
 public:
 ...
 private:
  ...
  C4GUI::Tabular *pMainTabular;  // main tabular control: Contains game
                                 // selection list and chat control
  C4GUI::ListBox *pGameSelList;  // game selection listbox
  C4KeyBinding *pKeyRefresh, *pKeyBack, *pKeyForward;
  C4GUI::CallbackButton<C4StartupNetDlg, C4GUI::IconButton> *btnGameList,
      *btnChat;  // left side buttons
  C4GUI::CallbackButton<C4StartupNetDlg, C4GUI::IconButton> *btnInternet,
      *btnRecord, *btnUpdate;  // right side buttons
  C4GUI::Button *btnJoin, *btnRefresh;
  C4GUI::Edit *pJoinAddressEdt;
  class C4ChatControl *pChatCtrl;
  C4GUI::WoodenLabel *pChatTitleLabel;
  C4StartupNetListEntry *pMasterserverClient;  // set if masterserver query is
                                               // enabled: Checks clonk.de for
                                               // new games
  bool fIsCollapsed;  // set if the number of games in the list requires them to
                      // be displayed in a condensed format
  bool
      fUpdatingList;  // set during list update - prevent selection update calls
  C4GameVersion UpdateVersion;  // set if update button is visible: Version to
                                // be updated to

  C4Sec1TimerCallback<C4StartupNetDlg> *
      pSec1Timer;  // engine timer hook for list updates
#ifdef NETWORK
  C4Network2IODiscoverClient
      DiscoverClient;  // disocver client to search for local hosts
#endif
 ...
 protected:
  ...
  C4GUI::Control *GetDlgModeFocusControl();  // get control to be focused when
                                             // main tabular sheet changes
  ...
  virtual void OnClosed(
      bool fOK);  // callback when dlg got closed: Return to main screen
  ...
  C4GUI::Edit::InputResult OnJoinAddressEnter(C4GUI::Edit *edt, bool fPasting,
                                              bool fPastingMore) {
    DoOK();
 ...
 private:
  void UpdateMasterserver();  // creates masterserver object if masterserver is
                              // enabled; destroy otherwise
  void UpdateList(bool fGotReference = false);
  void UpdateCollapsed();
  void UpdateSelection(bool fUpdateCollapsed);
  void UpdateDlgMode();  // update button visibility after switching between
                         // game sel list and chat

  void AddReferenceQuery(
      const char *szAddress,
      C4StartupNetListEntry::QueryType
          eQueryType);  // add a ref searcher entry and start searching
 ...
 public:
  ...
  void CheckVersionUpdate(const C4GameVersion &rNewVer);  // make an update
                                                          // button visible if
                                                          // the passed verionis
                                                          // an update to this
                                                          // version
