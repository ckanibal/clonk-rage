...
Tabular::Sheet::Sheet(const char *szTitle, const C4Rect &rcBounds,
                      int32_t icoTitle, bool fHasCloseButton, bool fTitleMarkup)
    : Window(),
      icoTitle(icoTitle),
      cHotkey(0),
      dwCaptionClr(0u),
      fHasCloseButton(fHasCloseButton),
      fCloseButtonHighlighted(false),
      fTitleMarkup(fTitleMarkup) {
  // store title
  if (szTitle) {
    sTitle.Copy(szTitle);
...
void Tabular::Sheet::DrawCaption(C4FacetEx &cgo, int32_t x, int32_t y,
                                 int32_t iMaxWdt, bool fLarge, bool fActive,
                                 bool fFocus, C4FacetEx *pfctClip,
                                 C4FacetEx *pfctIcon, CStdFont *pUseFont) {
  // calculations
  int32_t iTxtHgt, iTxtWdt;
  GetCaptionSize(&iTxtWdt, &iTxtHgt, fLarge, fActive, pfctClip, pfctIcon,
                 pUseFont);
  if (pfctClip) iMaxWdt = iTxtWdt;
  CStdFont &rUseFont = pUseFont ? *pUseFont : (fLarge ? GetRes()->CaptionFont
                                                      : GetRes()->TextFont);
  if (pfctClip && pfctIcon) {
    ...
    int32_t iLabelHgt = rUseFont.GetLineHeight();
    int32_t iIconLabelSpacing = 2;
    int32_t yTop =
        yCenter - (pfctIcon->Hgt + iIconLabelSpacing + iLabelHgt) / 2;
    pfctIcon->Draw(cgo.Surface, xCenter - pfctIcon->Wdt / 2, yTop, icoTitle);
    lpDDraw->TextOut(
        sTitle.getData(), rUseFont, 1.0f, cgo.Surface, xCenter,
        yTop + pfctIcon->Hgt + iIconLabelSpacing,
        fActive ? C4GUI_GfxTabCaptActiveClr : C4GUI_GfxTabCaptInactiveClr,
        ACenter);
  }
  // focus highlight
  if (fFocus) {
    lpDDraw->SetBlitMode(C4GFXBLIT_ADDITIVE);
    GetRes()->fctButtonHighlight.DrawX(
        cgo.Surface, (fLarge ? x : x - iTxtWdt / 2) + 5, y + 3,
        (fLarge ? iMaxWdt : iTxtWdt) - 10, iTxtHgt - 6);
  ...
  if (!(pfctClip && pfctIcon)) {
    ...
    if (!dwClr)
      dwClr = fActive ? C4GUI_CaptionFontClr : C4GUI_InactCaptionFontClr;
    lpDDraw->TextOut(sTitle.getData(), rUseFont, fLarge ? 1.2f : 1.0f,
                     cgo.Surface, x, y, dwClr, fLarge ? ALeft : ACenter,
                     fTitleMarkup);
    // close button
    if (fHasCloseButton) {
...
void Tabular::Sheet::GetCaptionSize(int32_t *piWdt, int32_t *piHgt, bool fLarge,
                                    bool fActive, C4FacetEx *pfctClip,
                                    C4FacetEx *pfctIcon, CStdFont *pUseFont) {
  // caption by gfx?
  if (pfctClip && pfctIcon) {
...
bool Tabular::Sheet::IsPosOnCloseButton(int32_t x, int32_t y, int32_t iCaptWdt,
                                        int32_t iCaptHgt, bool fLarge) {
  // close button is on right end of tab
  return fHasCloseButton &&
         Inside<int32_t>(x, iCaptWdt - iCaptHgt + 1, iCaptWdt - 2) &&
         Inside<int32_t>(y, 1, iCaptHgt - 2);
}
...
Tabular::Tabular(C4Rect &rtBounds, TabPosition eTabPos)
    : Control(rtBounds),
      pActiveSheet(NULL),
      eTabPos(eTabPos),
      iMaxTabWidth(0),
      ...
      pfctClip(NULL),
      pfctIcons(NULL),
      pSheetCaptionFont(NULL),
      iSheetMargin(4),
      fDrawSelf(true),
      iCaptionLengthTotal(0),
      iCaptionScrollPos(0),
      fScrollingLeft(false),
      fScrollingRight(false),
  ...
  if (eTabPos != tbNone) {
    // Ctrl+(Shift-)Tab works with dialog focus only (assumes max one tabular
    // per dialog)
    ...
    pKeySelUp =
        new C4KeyBinding(Keys, "GUITabularSelUp", KEYSCOPE_Gui,
                         new ControlKeyCB<Tabular>(*this, &Tabular::KeySelUp),
    ...
    pKeySelDown =
        new C4KeyBinding(Keys, "GUITabularSelDown", KEYSCOPE_Gui,
                         new ControlKeyCB<Tabular>(*this, &Tabular::KeySelDown),
                         C4CustomKey::PRIO_Ctrl);

    pKeySelUp2 = new C4KeyBinding(
        C4KeyCodeEx(K_TAB, C4KeyShiftState(KEYS_Shift | KEYS_Control)),
        "GUITabularSelUp2", KEYSCOPE_Gui,
        new DlgKeyCB<Tabular>(*this, &Tabular::KeySelUp),
        C4CustomKey::PRIO_Ctrl);
    pKeySelDown2 = new C4KeyBinding(
        C4KeyCodeEx(K_TAB, KEYS_Control), "GUITabularSelDown2", KEYSCOPE_Gui,
        new DlgKeyCB<Tabular>(*this, &Tabular::KeySelDown),
        C4CustomKey::PRIO_Ctrl);
    pKeyCloseTab = new C4KeyBinding(
        C4KeyCodeEx(K_F4, KEYS_Control), "GUITabularCloseTab", KEYSCOPE_Gui,
        new DlgKeyCB<Tabular>(*this, &Tabular::KeyCloseTab),
        C4CustomKey::PRIO_Ctrl);
  } else {
...
void Tabular::SelectionChanged(bool fByUser) {
  ...
  // if nothing is selected now, but something was selected before, focus new
  // default control
  if (pFocusCtrl && !pDlg->GetFocus())
    pDlg->SetFocus(pDlg->GetDefaultControl(), fByUser);
}
...
void Tabular::SheetsChanged() {
  ...
  if (eTabPos) {
    ...
    for (pSheet = (Sheet *)GetFirst(); pSheet;
         pSheet = (Sheet *)pSheet->GetNext()) {
      int32_t iTabWidth, iTabHeight;
      pSheet->GetCaptionSize(&iTabWidth, &iTabHeight, HasLargeCaptions(),
                             pSheet == pActiveSheet, pfctClip, pfctIcons,
                             pSheetCaptionFont);
    ...
    if (eTabPos == tbLeft && iTotalHgt > rcBounds.Hgt - GetMarginBottom()) {
      ...
      iSheetSpacing -=
          (iTotalHgt - rcBounds.Hgt + GetMarginBottom() - iSheetOff) /
          iSheetNum;
      iSheetOff = 0;
...
void Tabular::UpdateScrolling() {
  ...
  if (eTabPos != tbTop || iCaptionLengthTotal <= iAvailableTabSpace ||
      iAvailableTabSpace <= iScrollPinSize * 2) {
    fScrollingLeft = fScrollingRight = fScrollingLeftDown =
        fScrollingRightDown = false;
    iCaptionScrollPos = 0;
  } else {
    ...
    if (!fScrollingLeft) {
      ...
      fScrollingRight =
          (iCaptionLengthTotal - iCaptionScrollPos > iAvailableTabSpace);
...
void Tabular::DoCaptionScroll(int32_t iDir) {
  ...
  int32_t iAvailableTabSpace = rcBounds.Wdt;
  int32_t iScrollPinSize = GetTopSize();
  iCaptionScrollPos = BoundBy<int32_t>(
      iCaptionScrollPos + iDir * iAvailableTabSpace / 2, 0,
      iCaptionLengthTotal - iAvailableTabSpace + iScrollPinSize);
  UpdateScrolling();
}
...
void Tabular::DrawElement(C4FacetEx &cgo) {
  ...
  if ((fScrollingLeftDown || fScrollingRightDown) &&
      timeGetTime() - tLastScrollTime >= C4GUI_TabCaptionScrollTime)
    DoCaptionScroll(fScrollingRightDown - fScrollingLeftDown);
  // border
  if (!fGfx)
    Draw3DFrame(cgo, false, 1, 0xaf, eTabPos != tbTop, GetTopSize(),
                eTabPos != tbLeft, GetLeftSize());
  // calc positions
  ...
  // main area BG
  if (!fGfx)
    lpDDraw->DrawBoxDw(cgo.Surface, x0, y0, x1, y1, C4GUI_StandardBGColor);
  // no tabs?
  if (!eTabPos) return;
  ...
  int32_t d =
      (fLeft ? y0 : x0) +
      iSheetOff;  // current tab position (leave some space to the left/top)
  ...
  // DrawBar(cgo, GetRes()->fctCaptionLeft, GetRes()->fctCaptionBar,
  // GetRes()->fctCaptionRight);
  // tabs
  for (Sheet *pSheet = (Sheet *)GetFirst(); pSheet;
       pSheet = (Sheet *)pSheet->GetNext()) {
    // get tab size
    int32_t iTabWidth, iTabHeight;
    pSheet->GetCaptionSize(&iTabWidth, &iTabHeight, HasLargeCaptions(),
    ...
    if (!fGfx) {
      ...
      if (fLeft) {
        vtx[0] = x0;
        vtx[1] = d;
        vtx[2] = x0 - GetLeftSize();
        vtx[3] = d;
        vtx[4] = x0 - GetLeftSize();
        vtx[5] = d + iTabHeight;
        vtx[6] = x0;
        vtx[7] = d + iTabHeight;
      } else {
      ...
      DWORD dwClr = (pSheet == pActiveSheet) ? C4GUI_ActiveTabBGColor
                                             : C4GUI_StandardBGColor;
      lpDDraw->DrawQuadDw(cgo.Surface, vtx, dwClr, dwClr, dwClr, dwClr);
      // draw caption frame
      lpDDraw->DrawLineDw(cgo.Surface, (float)vtx[0] - 1, (float)vtx[1],
                          (float)vtx[2] - 1, (float)vtx[3],
                          C4GUI_BorderColorA1);
      lpDDraw->DrawLineDw(cgo.Surface, (float)vtx[2] - 1, (float)vtx[3],
                          (float)vtx[4] - fLeft, (float)vtx[5],
                          C4GUI_BorderColorA1);
      lpDDraw->DrawLineDw(cgo.Surface, (float)vtx[4], (float)vtx[5],
                          (float)vtx[6], (float)vtx[7], C4GUI_BorderColorA1);
      lpDDraw->DrawLineDw(cgo.Surface, (float)vtx[0], (float)vtx[1] + fLeft,
                          (float)vtx[2], (float)vtx[3] + fLeft,
                          C4GUI_BorderColorA2);
      lpDDraw->DrawLineDw(cgo.Surface, (float)vtx[2] - !fLeft,
                          (float)vtx[3] + 1, (float)vtx[4],
                          (float)vtx[5] + !fLeft, C4GUI_BorderColorA2);
      lpDDraw->DrawLineDw(cgo.Surface, (float)vtx[4] + 1, (float)vtx[5] + fLeft,
                          (float)vtx[6], (float)vtx[7] + fLeft,
                          C4GUI_BorderColorA2);
    }
    // draw caption text
    int32_t iCptTextX = fLeft ? (x0 - GetLeftSize() + 10) : (d + iTabWidth / 2);
    int32_t iCptTextY =
        fLeft ? (d + iSheetSpacing / 2) : (y0 - GetTopSize() + 2);
    if (pSheet == pActiveSheet) {
      // store active sheet pos for border line or later drawing
      ad0 = d;
      ad1 = d + (fLeft ? iTabHeight : iTabWidth);
      aCptTxX = iCptTextX;
      aCptTxY = iCptTextY;
      // draw active caption
      if (!fGfx)
        pSheet->DrawCaption(cgo, iCptTextX, iCptTextY, iMaxTabWidth, fLeft,
                            true, HasDrawFocus(), NULL, NULL, NULL);
    } else {
      // draw inactive caption
      pSheet->DrawCaption(cgo, iCptTextX, iCptTextY, iMaxTabWidth, fLeft, false,
  ...
  if (!fGfx)
    if (ad0 || ad1) {
      lpDDraw->DrawLineDw(cgo.Surface, (float)x0, (float)y0,
                          (float)(fLeft ? x0 : ad0), (float)(fLeft ? ad0 : y0),
                          C4GUI_BorderColorA1);
      lpDDraw->DrawLineDw(cgo.Surface, (float)(x0 + 1), (float)(y0 + 1),
                          (float)((fLeft ? x0 : ad0) + 1),
                          (float)((fLeft ? ad0 : y0) + 1), C4GUI_BorderColorA2);
      lpDDraw->DrawLineDw(cgo.Surface, (float)(fLeft ? x0 : ad1),
                          (float)(fLeft ? ad1 : y0), (float)(fLeft ? x0 : x1),
                          (float)(fLeft ? y1 : y0), C4GUI_BorderColorA1);
      lpDDraw->DrawLineDw(cgo.Surface, (float)((fLeft ? x0 : ad1) + 1),
                          (float)((fLeft ? ad1 : y0) + 1),
                          (float)((fLeft ? x0 : x1) + 1),
                          (float)((fLeft ? y1 : y0) + 1), C4GUI_BorderColorA2);
    }
  ...
  if (fGfx) {
    ...
    if (pActiveSheet)
      pActiveSheet->DrawCaption(cgo, aCptTxX, aCptTxY, iMaxTabWidth, fLeft,
                                true, HasDrawFocus(), pfctClip, pfctIcons,
                                pSheetCaptionFont);
  }
  // scrolling
  if (fScrollingLeft)
    GetRes()->fctBigArrows.DrawX(cgo.Surface, x0 + iSheetOff, y0 - iScrollSize,
                                 iScrollSize, iScrollSize,
                                 fScrollingLeftDown * 2);
  if (fScrollingRight)
    GetRes()->fctBigArrows.DrawX(cgo.Surface, x1 - iScrollSize,
                                 y0 - iScrollSize, iScrollSize, iScrollSize,
                                 1 + fScrollingRightDown * 2);
}

void Tabular::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
                         int32_t iY, DWORD dwKeyParam) {
  // tabular contains controls?
  if (eTabPos) {
    bool fLeft = (eTabPos == tbLeft);
    bool fInCaptionArea = ((!fLeft && Inside<int32_t>(iY, 0, GetTopSize())) ||
                           (fLeft && Inside<int32_t>(iX, 0, GetLeftSize())));
    if (!fInCaptionArea || iButton == C4MC_Button_LeftUp) {
      MouseLeaveCaptionArea();
    }
    // then check for mousedown in coloumn area
    else if ((iButton == C4MC_Button_LeftDown || iButton == C4MC_Button_None) &&
             fInCaptionArea) {
      int32_t d = iSheetOff;
      ...
      if (fScrollingLeft || fScrollingRight) {
        int32_t iScrollSize = GetTopSize();
        if (iButton == C4MC_Button_LeftDown && fScrollingRight &&
            Inside(iX, rcBounds.Wdt - iScrollSize, rcBounds.Wdt)) {
          fProcessed = fScrollingRightDown = true;
          GUISound("Command");
          DoCaptionScroll(+1);
        } else if (fScrollingLeft) {
          if (iButton == C4MC_Button_LeftDown &&
              Inside(iX, d, d + iScrollSize)) {
      ...
      if (!fProcessed)
        for (Sheet *pSheet = (Sheet *)GetFirst(); pSheet;
          ...
          pSheet->GetCaptionSize(&iCaptWidth, &iCaptHeight, HasLargeCaptions(),
                                 pSheet == pActiveSheet, pfctClip, pfctIcons,
                                 pSheetCaptionFont);
          iTabWidth = iCaptWidth + (fLeft ? 20 : iSheetSpacing);
          iTabHeight = iCaptHeight + (fLeft ? iSheetSpacing : 10);
          // check containment in this tab (check rect only, may catch some
          // side-clicks...)
          if ((!fLeft && Inside(iX, d, d + iTabWidth)) ||
              (fLeft && Inside(iY, d, d + iTabHeight))) {
            // close button
            if (pSheet->IsPosOnCloseButton(
                    iX - d * !fLeft - (iTabWidth - iCaptWidth) / 2,
                    iY - d * fLeft, iCaptWidth, iCaptHeight,
                    HasLargeCaptions())) {
              if (iButton == C4MC_Button_LeftDown) {
...
int32_t Tabular::GetActiveSheetIndex() {
  int32_t i = -1;
  Sheet *pSheet;
  while (pSheet = GetSheet(++i))
    if (pSheet == pActiveSheet) return i;
  return -1;
}

void Tabular::SetGfx(C4FacetEx *pafctBack, C4FacetEx *pafctClip,
                     C4FacetEx *pafctIcons, CStdFont *paSheetCaptionFont,
  ...
  if (pfctBack && fResizeByAspect) {
    int32_t iEffWdt = rcBounds.Wdt - GetLeftSize(),
            iEffHgt = rcBounds.Hgt - GetTopSize();
...
void Tabular::UpdateSize() {
  Control::UpdateSize();
  // update all sheets
  for (Sheet *pSheet = static_cast<Sheet *>(GetFirst()); pSheet;
       pSheet = static_cast<Sheet *>(pSheet->GetNext()))
