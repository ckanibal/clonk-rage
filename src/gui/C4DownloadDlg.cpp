...
C4DownloadDlg::C4DownloadDlg(const char *szDLType)
    : C4GUI::Dialog(
          C4GUI_ProgressDlgWdt, 100,
          FormatString(LoadResStr("IDS_CTL_DL_TITLE"), szDLType).getData(),
          false),
      szError(NULL) {
#ifdef HAVE_WINSOCK
  ...
  AddElement(pStatusLabel =
                 new C4GUI::Label("", C4Rect(), ACenter, C4GUI_MessageFontClr,
                                  &C4GUI::GetRes()->TextFont, false));
...
void C4DownloadDlg::SetStatus(const char *szNewText, int32_t iProgressPercent) {
  ...
  int iMsgHeight = C4GUI::GetRes()->TextFont.BreakMessage(
      szNewText, caMain.GetInnerWidth(), &sMsgBroken, true);
  pStatusLabel->SetBounds(caMain.GetFromTop(iMsgHeight));
  pStatusLabel->SetText(sMsgBroken.getData());
  // place progress bar
  if (iProgressPercent >= 0) {
    if (!pProgressBar) {
      AddElement(pProgressBar = new C4GUI::ProgressBar(
                     caMain.GetFromTop(C4GUI_ProgressDlgPBHgt)));
    } else {
...
void C4DownloadDlg::OnIdle() {
  ...
  SetStatus(
      FormatString(
          szStatusString,
          GetFilename(
              HTTPClient.getRequest()) /*, sSize.getData(), sStatus.getData()*/)
          .getData(),
      iProgress);
}
...
bool C4DownloadDlg::ShowModal(C4GUI::Screen *pScreen, const char *szURL,
                              const char *szSaveAsFilename) {
  // reset error
  szError = NULL;
  // initial text
...
bool C4DownloadDlg::DownloadFile(const char *szDLType, C4GUI::Screen *pScreen,
  ...
  if (!pDlg->ShowModal(pScreen, szURL, szSaveAsFilename)) {
    ...
    StdStrBuf sError;
    sError.Format(LoadResStr("IDS_PRC_DOWNLOADERROR"), GetFilename(szURL),
                  szError);
    // it's a 404: display extended message
    if (SSearch(szError, "404") && szNotFoundMessage) {
      sError.Append("|");
      sError.Append(szNotFoundMessage);
    }
    // display message
    pScreen->ShowMessageModal(
        sError.getData(),
        FormatString(LoadResStr("IDS_CTL_DL_TITLE"), szDLType).getData(),
        C4GUI::MessageDialog::btnOK, C4GUI::Ico_Error, NULL);
    delete pDlg;
    return false;
  }
