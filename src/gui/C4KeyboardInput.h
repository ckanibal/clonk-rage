...
enum C4KeyScope {
  KEYSCOPE_None = 0,
  KEYSCOPE_Control =
      1,  // player control (e.g. NUM1 to move left on keypad control)
  KEYSCOPE_Gui = 2,  // keys used to manipulate GUI elements (e.g. Tab to cycle
                     // through controls)
  KEYSCOPE_Fullscreen =
      4,  // generic fullscreen-only keys (e.g. F9 for screenshot)
  KEYSCOPE_Console =
      8,  // generic console-mode only keys (e.g. Space to switch edit cursor)
  KEYSCOPE_Generic = 16,    // generic keys available in fullscreen and console
                            // mode outside GUI (e.g. F1 for music on/off)
  KEYSCOPE_FullSMenu = 32,  // fullscreen menu control. If fullscreen menu is
                            // active, this disables viewport controls (e.g.
                            // Return to close player join menu)
  KEYSCOPE_FilmView = 64,   // ownerless viewport scrolling in film mode, player
                            // switching, etc. (e.g. Enter to switch to next
                            // player)
  KEYSCOPE_FreeView = 128,  // ownerless viewport scrolling, player switching,
                            // etc. (e.g. arrow left to scroll left in view)
...
const C4KeyCode KEY_Default = 0,  // no key
    ...
    KEY_JOY_Button1 =
        10,  // key index of joypad buttons + button index for more buttons
    KEY_JOY_ButtonMax = KEY_JOY_Button1 +
                        31,  // maximum number of supported buttons on a gamepad
...
inline uint8_t KEY_JOY_Axis(uint8_t idx, bool fMax) {
  return KEY_JOY_Axis1Min + 2 * idx + fMax;
}

inline C4KeyCode KEY_Gamepad(
    uint8_t idGamepad,
    uint8_t idButton)  // convert gamepad key to Clonk-gamepad-keycode
{
...
inline bool Key_IsGamepadButton(C4KeyCode key) {
  ...
  return Key_IsGamepad(key) &&
         Inside<uint8_t>(Key_GetGamepadButton(key), KEY_JOY_Button1,
                         KEY_JOY_ButtonMax);
}
...
inline bool Key_IsGamepadAxis(C4KeyCode key) {
  // whether this is a unique button event (AnyButton not included)
  return Key_IsGamepad(key) &&
         Inside<uint8_t>(Key_GetGamepadButton(key), KEY_JOY_Axis1Min,
                         KEY_JOY_AxisMax);
}
...
inline bool Key_IsGamepadAxisHigh(C4KeyCode key) { return !!(key & 1); }
...
struct C4KeyCodeEx {
  ...
  static StdStrBuf KeyCode2String(C4KeyCode wCode, bool fHumanReadable,
                                  bool fShort);
  ...
  C4KeyCodeEx(C4KeyCode Key = KEY_Default, C4KeyShiftState Shift = KEYS_None,
...
class C4KeyboardCallbackInterface {
 ...
 public:
  virtual bool OnKeyEvent(C4KeyCodeEx key,
                          C4KeyEventType eEv) = 0;  // return true if processed
  ...
  bool IsOriginalKey(const class C4CustomKey *pCheckKey) const {
    return pCheckKey == pOriginalKey;
  }
};

// callback interface
template <class TargetClass>
class C4KeyCB : public C4KeyboardCallbackInterface {
 public:
 ...
 protected:
  virtual bool OnKeyEvent(C4KeyCodeEx key, C4KeyEventType eEv) {
    ...
    switch (eEv) {
      case KEYEV_Down:
        return pFuncDown ? (rTarget.*pFuncDown)() : false;
      case KEYEV_Up:
        return pFuncUp ? (rTarget.*pFuncUp)() : false;
      case KEYEV_Pressed:
        return pFuncPressed ? (rTarget.*pFuncPressed)() : false;
    }
    return false;
  }
 ...
 public:
  C4KeyCB(TargetClass &rTarget, CallbackFunc pFuncDown,
          CallbackFunc pFuncUp = NULL, CallbackFunc pFuncPressed = NULL)
      : rTarget(rTarget),
        pFuncDown(pFuncDown),
        pFuncUp(pFuncUp),
        pFuncPressed(pFuncPressed) {}
};

// callback interface that passes the pressed key as a parameter
template <class TargetClass>
class C4KeyCBPassKey : public C4KeyboardCallbackInterface {
 public:
 ...
 protected:
  virtual bool OnKeyEvent(C4KeyCodeEx key, C4KeyEventType eEv) {
    ...
    switch (eEv) {
      case KEYEV_Down:
        return pFuncDown ? (rTarget.*pFuncDown)(key) : false;
      case KEYEV_Up:
        return pFuncUp ? (rTarget.*pFuncUp)(key) : false;
      case KEYEV_Pressed:
        return pFuncPressed ? (rTarget.*pFuncPressed)(key) : false;
    }
    return false;
  }
 ...
 public:
  C4KeyCBPassKey(TargetClass &rTarget, CallbackFunc pFuncDown,
                 CallbackFunc pFuncUp = NULL, CallbackFunc pFuncPressed = NULL)
      : rTarget(rTarget),
        pFuncDown(pFuncDown),
        pFuncUp(pFuncUp),
...
class C4KeyCBEx : public C4KeyboardCallbackInterface {
 public:
 ...
 protected:
  virtual bool OnKeyEvent(C4KeyCodeEx key, C4KeyEventType eEv) {
    ...
    switch (eEv) {
      case KEYEV_Down:
        return pFuncDown ? (rTarget.*pFuncDown)(par) : false;
      case KEYEV_Up:
        return pFuncUp ? (rTarget.*pFuncUp)(par) : false;
      case KEYEV_Pressed:
        return pFuncPressed ? (rTarget.*pFuncPressed)(par) : false;
    }
    return false;
  }
 ...
 public:
  C4KeyCBEx(TargetClass &rTarget, const ParameterType &par,
...
template <class TargetClass, class ParameterType>
class C4KeyCBExPassKey : public C4KeyboardCallbackInterface {
 public:
 ...
 protected:
  virtual bool OnKeyEvent(C4KeyCodeEx key, C4KeyEventType eEv) {
    ...
    switch (eEv) {
      case KEYEV_Down:
        return pFuncDown ? (rTarget.*pFuncDown)(key, par) : false;
      case KEYEV_Up:
        return pFuncUp ? (rTarget.*pFuncUp)(key, par) : false;
      case KEYEV_Pressed:
        return pFuncPressed ? (rTarget.*pFuncPressed)(key, par) : false;
    }
    return false;
  }
 ...
 public:
  C4KeyCBExPassKey(TargetClass &rTarget, const ParameterType &par,
                   CallbackFunc pFuncDown, CallbackFunc pFuncUp = NULL,
                   CallbackFunc pFuncPressed = NULL)
      : rTarget(rTarget),
        pFuncDown(pFuncDown),
...
class C4CustomKey {
 ...
 private:
  ...
  unsigned int uiPriority;  // key priority: If multiple keys of same code are
                            // defined, high prio overwrites low prio keys
 ...
 public:
  ...
  enum Priority {
    ...
    PRIO_CtrlOverride = 4u,  // dialog handlings of keys that overwrite regular
                             // control handlings
    PRIO_FocusCtrl = 5u,     // controls override special dialog handling keys
                             // (e.g., RenameEdit)
    PRIO_Context = 6u,       // context menus above controls
    PRIO_PlrControl = 7u,    // player controls overwrite any other controls
    PRIO_MoreThanMax =
        100u,  // must be larger than otherwise largest used priority
  };
 ...
 public:
  C4CustomKey(C4KeyCodeEx DefCode, const char *szName, C4KeyScope Scope,
              C4KeyboardCallbackInterface *pCallback,
              unsigned int uiPriority = PRIO_Base);  // ctor for default key
  C4CustomKey(const CodeList &rDefCodes, const char *szName, C4KeyScope Scope,
              C4KeyboardCallbackInterface *pCallback,
              unsigned int uiPriority = PRIO_Base);  // ctor for default key
                                                     // with multiple possible
                                                     // keys assigned
  C4CustomKey(C4KeyCodeEx Code,
              const StdStrBuf &rName);  // ctor for single custom key override
  ...
  const CodeList &GetCodes() const {
    return Codes.size() ? Codes : DefaultCodes;
  }  // return assigned codes; default if no custom has been assigned
  ...
  void KillCallbacks(const C4CustomKey *pOfKey);  // remove any callbacks that
                                                  // were created by given key

  void CompileFunc(StdCompiler *pComp);
};

// a key that auto-registers itself into main game keyboard input class and does
// dereg when deleted
class C4KeyBinding : protected C4CustomKey {
 public:
  C4KeyBinding(C4KeyCodeEx DefCode, const char *szName, C4KeyScope Scope,
               C4KeyboardCallbackInterface *pCallback,
               unsigned int uiPriority = PRIO_Base);  // ctor for default key
  C4KeyBinding(const CodeList &rDefCodes, const char *szName, C4KeyScope Scope,
               C4KeyboardCallbackInterface *pCallback,
               unsigned int uiPriority = PRIO_Base);  // ctor for default key
  ~C4KeyBinding();
};
...
class C4KeyboardInput {
 ...
 public:
  static bool IsValid;  // global var to fix any deinitialization orders of key
                        // map and static keys
 ...
 private:
  ...
  void UpdateKeyCodes(C4CustomKey *pKey, const C4CustomKey::CodeList &rOldCodes,
                      const C4CustomKey::CodeList &rNewCodes);

 public:
  void RegisterKey(C4CustomKey *pRegKey);  // register key into code and name
                                           // maps, or update specific key
  void UnregisterKey(const StdStrBuf &rsName);  // remove key from all maps
  void UnregisterKeyBinding(
      C4CustomKey *pKey);  // just remove callbacks from a key
  ...
  StdStrBuf GetKeyCodeNameByKeyName(const char *szKeyName, bool fShort = false,
                                    int32_t iIndex = 0);
};
