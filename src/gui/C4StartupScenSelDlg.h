...
const int32_t C4StartupScenSel_DefaultIcon_Scenario = 14,
              ...
              C4StartupScenSel_TitleOverlayMargin =
                  10;  // number of pixels to each side of title overlay picture
...
class C4ScenarioListLoader {
 public:
  ...
  class Entry {
   ...
   public:
    ...
    bool Load(C4Group *pFromGrp, const StdStrBuf *psFilename,
              bool fLoadEx);  // load as child if pFromGrp, else directly from
                              // filename
    virtual bool LoadCustom(C4Group &rGrp, bool fNameLoaded, bool fIconLoaded) {
      return true;
    }  // load custom data for entry type (e.g. scenario title fallback in
       // Scenario.txt)
    virtual bool LoadCustomPre(C4Group &rGrp) {
      return true;
    }  // preload stuff that's early in the group (Scenario.txt)
    virtual bool Start() = 0;  // start/open entry
    virtual Folder *GetIsFolder() {
      return NULL;
    }  // return this if this is a folder
    ...
    static Entry *CreateEntryForFile(
        const StdStrBuf &sFilename,
        Folder *pParent);  // create correct entry type based on file extension

    virtual bool CanOpen(StdStrBuf &sError) {
      return true;
    }  // whether item can be started/opened (e.g. mission access, unregistered)
    virtual bool IsGrayed() {
      return false;
    }  // additional condition for graying out - notice unreg folders are grayed
    ...
    virtual C4SForceFairCrew GetFairCrewAllowed() const {
      return C4SFairCrew_Free;
    }

    virtual const char *GetDefaultExtension() {
      return NULL;
    }  // extension to be added when item is renamed
  ...
  class Scenario : public Entry {
   ...
   public:
    Scenario(class Folder *pParent)
        : Entry(pParent),
          fNoMissionAccess(false),
          fUnregisteredAccess(false),
          iMinPlrCount(0) {}
    virtual ~Scenario() {}

    virtual bool LoadCustom(C4Group &rGrp, bool fNameLoaded,
                            bool fIconLoaded);  // do fallbacks for title and
                                                // icon; check whether scenario
                                                // is valid
    virtual bool LoadCustomPre(C4Group &rGrp);  // load scenario core
    virtual bool Start();                       // launch scenario!

    virtual bool CanOpen(
        StdStrBuf &sError);  // check mission access, player count, etc.
    virtual bool IsGrayed() {
      return false;
    }  // additional option for graying out
    virtual bool HasMissionAccess() const {
      return !fNoMissionAccess;
    };  // check mission access only
    ...
    virtual C4SForceFairCrew GetFairCrewAllowed() const {
      return static_cast<C4SForceFairCrew>(C4S.Head.ForcedFairCrew);
    }

    virtual StdStrBuf GetTypeName() {
      return StdCopyStrBuf(LoadResStr("IDS_TYPE_SCENARIO"), true);
    }
  ...
  class Folder : public Entry {
   protected:
    ...
    class C4MapFolderData *pMapData;  // if set, contains gfx and data for
                                      // special map-style folders
    friend class Entry;

   public:
    Folder(Folder *pParent)
        : Entry(pParent),
    ...
    bool LoadContents(C4ScenarioListLoader *pLoader, C4Group *pFromGrp,
                      const StdStrBuf *psFilename, bool fLoadEx,
                      bool fReload);  // load folder contents as child if
                                      // pFromGrp, else directly from filename
    uint32_t GetEntryCount() const;

   protected:
    void ClearChildren();
    void Sort();
    virtual bool DoLoadContents(C4ScenarioListLoader *pLoader,
                                C4Group *pFromGrp, const StdStrBuf &sFilename,
                                bool fLoadEx) = 0;  // load folder contents as
                                                    // child if pFromGrp, else
                                                    // directly from filename
   ...
   public:
    ...
    Entry *FindEntryByName(
        const char *szFilename) const;  // find entry by filename comparison

    virtual bool CanOpen(StdStrBuf &sError) {
      return true;
    }                         // can always open folders
    virtual bool IsGrayed();  // unreg folders can be opened to view stuff but
                              // they are still grayed out for clarity
    virtual StdStrBuf GetOpenText();  // get open button text
    virtual StdStrBuf GetOpenTooltip();
  ...
  class SubFolder : public Folder {
   ...
   protected:
    virtual bool LoadCustom(C4Group &rGrp, bool fNameLoaded,
                            bool fIconLoaded);  // load custom data for entry
                                                // type - icon fallback to
                                                // folder icon
    virtual bool DoLoadContents(C4ScenarioListLoader *pLoader,
                                C4Group *pFromGrp, const StdStrBuf &sFilename,
                                bool fLoadEx);  // load folder contents as child
                                                // if pFromGrp, else directly
                                                // from filename
  };
  ...
  class RegularFolder : public Folder {
   public:
    ...
    virtual StdStrBuf GetTypeName() {
      return StdCopyStrBuf(LoadResStr("IDS_TYPE_DIRECTORY"), true);
    }
 ...
 public:
  bool DoProcessCallback(
      int32_t iProgress,
      int32_t iMaxProgress);  // returns false if the activity was aborted

 public:
  bool Load(const StdStrBuf &sRootFolder);  // (unthreaded) loading of all
                                            // entries in root folder
  bool Load(Folder *pSpecifiedFolder,
            bool fReload);  // (unthreaded) loading of all entries in subfolder
  bool LoadExtended(Entry *pEntry);  // (unthreaded) loading of desc and title
                                     // image of specified entry
  ...
  Entry *GetFirstEntry() const {
    return pCurrFolder ? pCurrFolder->GetFirstEntry() : NULL;
  }
  ...
  int32_t GetProgressPercent() const {
    return iProgress * 100 / Max<int32_t>(iMaxProgress, 1);
  }
};
...
class C4MapFolderData {
 private:
  struct Scenario {
    ...
    // parameters for title as drawn on the map (if desired; otherwise sTitle
    // empty)
    StdStrBuf sTitle;
  ...
  class MapPic : public C4GUI::Picture {
   ...
   protected:
    virtual void MouseInput(C4GUI::CMouse &rMouse, int32_t iButton, int32_t iX,
                            int32_t iY,
                            DWORD dwKeyParam);  // input: mouse movement or
                                                // buttons - deselect everything
                                                // if clicked
    virtual void DrawElement(C4FacetEx &cgo);   // draw the image
  };

 private:
  C4FacetExSurface fctBackgroundPicture;
  FLOAT_RECT rcfBG;
  ...
  Scenario **ppScenList;
  int32_t iScenCount;
  AccessGfx **ppAccessGfxList;
  int32_t iAccessGfxCount;
 ...
 private:
  void ConvertFacet2ScreenCoord(const C4Rect &rc, FLOAT_RECT *pfrc,
                                float fBGZoomX, float fBGZoomY, int iOffX,
                                int iOffY);
  void ConvertFacet2ScreenCoord(int32_t *piValue, float fBGZoom, int iOff);
  void ConvertFacet2ScreenCoord(C4Rect &rcMapArea,
                                bool fAspect);  // adjust coordinates of loaded
                                                // facets so they match given
                                                // area
 ...
 public:
  ...
  void CompileFunc(StdCompiler *pComp);
  void CreateGUIElements(C4StartupScenSelDlg *pMainDlg,
...
class C4StartupScenSelDlg : public C4StartupDlg {
 public:
  ...
  class ScenListItem : public C4GUI::Window {
   private:
    ...
    C4ScenarioListLoader::Entry *
        pScenListEntry;  // associated, loaded item info

   public:
    ScenListItem(C4GUI::ListBox *pForListBox,
                 C4ScenarioListLoader::Entry *pForEntry,
                 C4GUI::Element *pInsertBeforeElement = NULL);

   protected:
    struct RenameParams {};
    void AbortRenaming(RenameParams par);
    C4GUI::RenameEdit::RenameResult DoRenaming(RenameParams par,
   ...
   public:
    ...
    ScenListItem *GetNext() {
      return static_cast<ScenListItem *>(BaseClass::GetNext());
    }

    virtual bool CheckNameHotkey(const char *c);  // return whether this item
 ...
 private:
  ...
  C4GUI::Label *pScenSelCaption;        // caption label atop scenario list;
                                        // indicating current folder
  C4GUI::ListBox *pScenSelList;         // left page of book: Scenario selection
  C4GUI::Label *pScenSelProgressLabel;  // progress label shown while scenario
                                        // list is being generated
  C4GUI::TextWindow *pSelectionInfo;  // used to display the description of the
                                      // current selection

  C4KeyBinding *pKeyRefresh, *pKeyBack, *pKeyForward, *pKeyRename, *pKeyDelete,
      *pKeyCheat;
