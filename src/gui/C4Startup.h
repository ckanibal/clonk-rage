...
class C4StartupGraphics {
 ...
 public:
  ...
  C4GUI::ScrollBarFacets sfctBookScroll, sfctBookScrollR, sfctBookScrollG,
      sfctBookScrollB;
  ...
  CStdFont &GetBlackFontByHeight(
      int32_t iHgt, float *pfZoom);  // get optimal font for given control size
};
...
class C4Startup {
 ...
 private:
  ...
  C4StartupDlg *pLastDlg, *pCurrDlg;  // startup dlg that is currently shown,
                                      // and dialog that was last shown
 ...
 protected:
  ...
  class C4StartupDlg *SwitchDialog(
      DialogID eToDlg, bool fFade = true);  // do transition to another dialog
 ...
 public:
  static C4Startup *
  EnsureLoaded();         // create and load startup data if not done yet
  ...
  static bool SetStartScreen(const char *szScreen);  // set screen that is shown
                                                     // first by case
