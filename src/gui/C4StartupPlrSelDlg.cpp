...
StdStrBuf DateString(int iTime) {
  ...
  return FormatString("%02d.%02d.%d %02d:%02d", pLocalTime->tm_mday,
                      pLocalTime->tm_mon + 1, pLocalTime->tm_year + 1900,
...
C4StartupPlrSelDlg::ListItem::ListItem(C4StartupPlrSelDlg *pForDlg,
                                       C4GUI::ListBox *pForListBox,
                                       C4GUI::Element *pInsertBeforeElement,
                                       bool fActivated)
    : Control(C4Rect(0, 0, 0, 0)),
      pCheck(NULL),
      pIcon(NULL),
      pNameLabel(NULL),
      pPlrSelDlg(pForDlg) {
  ...
  pCheck =
      new C4GUI::CheckBox(C4Rect(0, 0, iHeight, iHeight), NULL, fActivated);
  pCheck->SetOnChecked(new C4GUI::CallbackHandler<C4StartupPlrSelDlg>(
      pForDlg, &C4StartupPlrSelDlg::OnItemCheckChange));
  pKeyCheck = new C4KeyBinding(
      C4KeyCodeEx(K_SPACE), "StartupPlrSelTogglePlayerActive", KEYSCOPE_Gui,
      new C4GUI::ControlKeyCB<ListItem>(*this, &ListItem::KeyCheck),
      C4CustomKey::PRIO_Ctrl);
  pIcon =
      new C4GUI::Icon(C4Rect(iHeight + IconLabelSpacing, 0, iHeight, iHeight),
                      C4GUI::Ico_Player);
  pNameLabel =
      new C4GUI::Label("Q", (iHeight + IconLabelSpacing) * 2, IconLabelSpacing,
                       ALeft, ClrPlayerItem, &rUseFont, false, false);
  pNameLabel->SetAutosize(false);
  // calc own bounds - use icon bounds only, because only the height is used
  // when the item is added
  SetBounds(pIcon->GetBounds());
  // add components
  AddElement(pCheck);
  AddElement(pIcon);
  AddElement(pNameLabel);
  // add to listbox (will get resized horizontally and moved) - zero indent; no
  // tree structure in this dialog
  ...
  rcNameLabelBounds.Wdt =
      GetClientRect().Wdt - rcNameLabelBounds.x - IconLabelSpacing;
  pNameLabel->SetBounds(rcNameLabelBounds);
  // context menu
  SetContextHandler(new C4GUI::CBContextHandler<C4StartupPlrSelDlg::ListItem>(
      this, &C4StartupPlrSelDlg::ListItem::ContextMenu));
}

C4StartupPlrSelDlg::ListItem::~ListItem() { delete pKeyCheck; }
...
void C4StartupPlrSelDlg::ListItem::LoadPortrait(C4Group &rGrp,
                                                bool fUseDefault) {
  bool fPortraitLinked = false;
  if (!rGrp.FindEntry(C4CFN_Portrait) ||
      !fctPortraitBase.Load(rGrp, C4CFN_Portrait))
    if (!rGrp.FindEntry(C4CFN_Portrait_Old) ||
        !fctPortraitBase.Load(rGrp, C4CFN_Portrait_Old)) {
...
void C4StartupPlrSelDlg::ListItem::UpdateOwnPos() {
  ...
  // reposition items
  C4GUI::ComponentAligner caBounds(GetContainedClientRect(), IconLabelSpacing,
                                   IconLabelSpacing);
...
C4StartupPlrSelDlg::PlayerListItem::PlayerListItem(
    C4StartupPlrSelDlg *pForDlg, C4GUI::ListBox *pForListBox,
    C4GUI::Element *pInsertBeforeElement, bool fActivated)
    : ListItem(pForDlg, pForListBox, pInsertBeforeElement, fActivated),
...
void C4StartupPlrSelDlg::PlayerListItem::Load(const StdStrBuf &rsFilename) {
  ...
  if (!PlrGroup.Open(rsFilename.getData()))
    throw LoadError(FormatString(
        "Error loading player file from %s: Error opening group: %s",
        rsFilename.getData(), PlrGroup.GetError()));
  if (!Core.Load(PlrGroup))
    throw LoadError(FormatString(
        "Error loading player file from %s: Core data invalid or missing "
        "(Group: %s)!",
        rsFilename.getData(), PlrGroup.GetError()));
  // load icon
  C4FacetExSurface fctIcon;
  if (PlrGroup.FindEntry(C4CFN_BigIcon) &&
      fctIcon.Load(PlrGroup, C4CFN_BigIcon))
    fHasCustomIcon = true;
  else {
  ...
  if (!PlrGroup.Close())
    throw LoadError(FormatString(
        "Error loading player file from %s: Error closing group: %s",
        rsFilename.getData(), PlrGroup.GetError()));
  // default name
  if (!*Core.PrefName)
    SCopy(GetFilenameOnly(rsFilename.getData()), Core.PrefName,
          sizeof(Core.PrefName) - 1);
...
C4GUI::ContextMenu *C4StartupPlrSelDlg::PlayerListItem::ContextMenu() {
  ...
  pCtx->AddItem(LoadResStr("IDS_BTN_PROPERTIES"),
                LoadResStr("IDS_DLGTIP_PLAYERPROPERTIES"), C4GUI::Ico_None,
                new C4GUI::CBMenuHandler<C4StartupPlrSelDlg>(
                    pPlrSelDlg, &C4StartupPlrSelDlg::OnPropertyCtx));
  pCtx->AddItem(LoadResStr("IDS_BTN_DELETE"),
                LoadResStr("IDS_DLGTIP_PLAYERDELETE"), C4GUI::Ico_None,
                new C4GUI::CBMenuHandler<C4StartupPlrSelDlg>(
                    pPlrSelDlg, &C4StartupPlrSelDlg::OnDelCtx));
  return pCtx;
}

void C4StartupPlrSelDlg::PlayerListItem::GrabCustomIcon(
    C4FacetExSurface &fctGrabFrom) {
  // set flag
...
void C4StartupPlrSelDlg::PlayerListItem::UpdateCore(C4PlayerInfoCore &NewCore) {
  C4Group PlrGroup;
  if (!PlrGroup.Open(GetFilename().getData()) || !NewCore.Save(PlrGroup) ||
      !PlrGroup.Close()) {
    GetScreen()->ShowMessage(LoadResStr("IDS_FAIL_MODIFY"), "",
                             C4GUI::Ico_Error);
    return;
  }
  Core = NewCore;
...
void C4StartupPlrSelDlg::PlayerListItem::SetSelectionInfo(
    C4GUI::TextWindow *pSelectionInfo) {
  // write info text for player
  pSelectionInfo->ClearText(false);
  // pSelectionInfo->AddTextLine(FormatString("%s %s", Core.RankName,
  // Core.PrefName).getData(), &C4Startup::Get()->Graphics.BookFontCapt,
  // ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(FormatString("%s", Core.PrefName).getData(),
                              &C4Startup::Get()->Graphics.BookFontCapt,
                              ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      FormatString(LoadResStr("IDS_DESC_PLAYER"), (int)Core.Score,
                   (int)Core.Rounds, (int)Core.RoundsWon, (int)Core.RoundsLost,
                   TimeString(Core.TotalPlayingTime).getData(),
                   Core.Comment).getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  if (Core.LastRound.Title[0])
    pSelectionInfo->AddTextLine(
        FormatString(LoadResStr("IDS_DESC_LASTGAME"),
                     Core.LastRound.Title.getData(),
                     DateString(Core.LastRound.Date).getData(),
                     TimeString(Core.LastRound.Duration).getData(),
                     (int)Core.LastRound.FinalScore).getData(),
        &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
...
bool C4StartupPlrSelDlg::PlayerListItem::MoveFilename(
    const char *szToFilename) {
  // anything to do?
...
C4StartupPlrSelDlg::CrewListItem::CrewListItem(C4StartupPlrSelDlg *pForDlg,
                                               C4GUI::ListBox *pForListBox,
                                               uint32_t dwPlrClr)
    : ListItem(pForDlg, pForListBox, NULL, false),
      fLoaded(false),
      dwPlrClr(dwPlrClr),
      pParentGrp(NULL) {
...
void C4StartupPlrSelDlg::CrewListItem::Load(C4Group &rGrp,
  ...
  if (!CrewGroup.OpenAsChild(&rGrp, rsFilename.getData()))
    throw LoadError(FormatString(
        ...
        rsFilename.getData(), rGrp.GetFullName().getData(),
        CrewGroup.GetError()));
  if (!Core.Load(CrewGroup))
    throw LoadError(FormatString(
        "Error loading crew file from %s: Core data invalid or missing (Group: "
        "%s)!",
  ...
  // load rank as icon
  C4FacetExSurface fctIcon;
  if (fctIcon.Load(CrewGroup, C4CFN_ClonkRank, C4FCT_Full, C4FCT_Full, false,
                   true)) {
    GrabIcon(fctIcon);
  } else {
    // no custom icon: create default by rank system
    if (C4RankSystem::DrawRankSymbol(&fctIcon, Core.Rank,
                                     &Game.GraphicsResource.fctRank,
                                     Game.GraphicsResource.iNumRanks, true))
      GrabIcon(fctIcon);
  }
...
C4GUI::ContextMenu *C4StartupPlrSelDlg::CrewListItem::ContextMenu() {
  ...
  // context menu operations
  C4GUI::ContextMenu *pCtx = new C4GUI::ContextMenu();
  pCtx->AddItem(LoadResStr("IDS_BTN_RENAME"), LoadResStr("IDS_DESC_CREWRENAME"),
                C4GUI::Ico_None,
                new C4GUI::CBMenuHandler<C4StartupPlrSelDlg>(
                    pPlrSelDlg, &C4StartupPlrSelDlg::OnPropertyCtx));
  pCtx->AddItem(LoadResStr("IDS_BTN_DELETE"),
                LoadResStr("IDS_MSG_DELETECLONK_DESC"), C4GUI::Ico_None,
                new C4GUI::CBMenuHandler<C4StartupPlrSelDlg>(
                    pPlrSelDlg, &C4StartupPlrSelDlg::OnDelCtx));
  pCtx->AddItem(
      LoadResStr("IDS_MSG_SETDEATHMESSAGE"),
      LoadResStr("IDS_MSG_SETTHEMESSAGETHATAPPEARWH"), C4GUI::Ico_None,
      new C4GUI::CBMenuHandler<C4StartupPlrSelDlg::CrewListItem>(
          this, &C4StartupPlrSelDlg::CrewListItem::OnDeathMessageCtx));
  return pCtx;
}
...
void C4StartupPlrSelDlg::CrewListItem::OnDeathMessageCtx(C4GUI::Element *el) {
  ...
  GetScreen()->ShowRemoveDlg(
      pDlg = new C4GUI::InputDialog(
          LoadResStr("IDS_MSG_ENTERNEWDEATHMESSAGE"),
          LoadResStr("IDS_MSG_SETDEATHMESSAGE"), C4GUI::Ico_Ex_Comment,
          new C4GUI::InputCallback<C4StartupPlrSelDlg::CrewListItem>(
              this, &C4StartupPlrSelDlg::CrewListItem::OnDeathMessageSet),
          false));
  pDlg->SetMaxText(C4MaxDeathMsg);
  pDlg->SetInputText(Core.DeathMessage);
}

void C4StartupPlrSelDlg::CrewListItem::OnDeathMessageSet(
    const StdStrBuf &rsNewMessage) {
  // copy msg
  if (!rsNewMessage)
    *Core.DeathMessage = '\0';
  else
    SCopy(rsNewMessage.getData(), Core.DeathMessage, C4MaxDeathMsg);
  // save
  RewriteCore();
  // acoustic feedback
...
void C4StartupPlrSelDlg::CrewListItem::RewriteCore() {
  ...
  if (!CrewGroup.OpenAsChild(pParentGrp, GetFilename().getData()) ||
      !Core.Save(CrewGroup, NULL) || !CrewGroup.Close() ||
      !pParentGrp->Save(TRUE)) {
    GetScreen()->ShowMessage(LoadResStr("IDS_FAIL_MODIFY"), "",
                             C4GUI::Ico_Error);
    return;
  }
...
bool C4StartupPlrSelDlg::CrewListItem::SetName(const char *szNewName) {
  ...
  if (!ItemIdentical(fn, GetFilename().getData())) {
    ...
    if (pParentGrp->FindEntry(fn)) {
      StdStrBuf sMsg;
      sMsg.Format(LoadResStr("IDS_ERR_CLONKCOLLISION"), fn);
      Game.pGUI->ShowMessageModal(sMsg.getData(), LoadResStr("IDS_FAIL_RENAME"),
                                  C4GUI::MessageDialog::btnOK,
                                  C4GUI::Ico_Error);
      return false;
    }
    // OK; then rename
    if (!pParentGrp->Rename(GetFilename().getData(), fn) ||
        !pParentGrp->Save(TRUE)) {
      StdStrBuf sMsg;
      sMsg.Format(LoadResStr("IDS_ERR_RENAMEFILE"), GetFilename().getData(),
                  fn);
      Game.pGUI->ShowMessageModal(sMsg.getData(), LoadResStr("IDS_FAIL_RENAME"),
                                  C4GUI::MessageDialog::btnOK,
                                  C4GUI::Ico_Error);
...
StdStrBuf C4StartupPlrSelDlg::CrewListItem::GetPhysicalTextLine(
    int32_t iPhysValue, const char *idsName) {
...
void C4StartupPlrSelDlg::CrewListItem::SetSelectionInfo(
    C4GUI::TextWindow *pSelectionInfo) {
  // write info text for player
  pSelectionInfo->ClearText(false);
  pSelectionInfo->AddTextLine(
      FormatString("%s %s", Core.sRankName.getData(), Core.Name).getData(),
      &C4Startup::Get()->Graphics.BookFontCapt, ClrPlayerItem, false, false);
  StdStrBuf sPromo;
  int32_t iNextRankExp;
  StdStrBuf sNextRankName;
  if (Core.GetNextRankInfo(Game.Rank, &iNextRankExp, &sNextRankName))
    sPromo.Format(LoadResStr("IDS_DESC_PROMO"), sNextRankName.getData(),
                  (int)iNextRankExp);
  else
    sPromo.Copy(LoadResStr("IDS_DESC_NOPROMO"));
  pSelectionInfo->AddTextLine(
      FormatString(LoadResStr("IDS_DESC_OBJECT"), Core.TypeName,
                   Core.Experience, Core.Rounds, Core.DeathCount,
                   sPromo.getData(),
                   TimeString(Core.TotalPlayingTime).getData(),
                   DateString(Core.Birthday).getData()).getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Energy, "IDS_DESC_ENERGY").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Breath, "IDS_DESC_BREATH").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Walk, "IDS_DESC_WALK").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Jump, "IDS_DESC_JUMP").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  if (Core.Physical.CanScale)
    pSelectionInfo->AddTextLine(
        GetPhysicalTextLine(Core.Physical.Scale, "IDS_DESC_SCALE").getData(),
        &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  if (Core.Physical.CanHangle)
    pSelectionInfo->AddTextLine(
        GetPhysicalTextLine(Core.Physical.Hangle, "IDS_DESC_HANGLE").getData(),
        &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Dig, "IDS_DESC_DIG").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Swim, "IDS_DESC_SWIM").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Throw, "IDS_DESC_THROW").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Push, "IDS_DESC_PUSH").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->AddTextLine(
      GetPhysicalTextLine(Core.Physical.Fight, "IDS_DESC_FIGHT").getData(),
      &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  if (Core.Physical.Magic)
    pSelectionInfo->AddTextLine(
        GetPhysicalTextLine(Core.Physical.Magic, "IDS_DESC_MAGIC").getData(),
        &C4Startup::Get()->Graphics.BookFont, ClrPlayerItem, false, false);
  pSelectionInfo->UpdateHeight();
}
...
StdStrBuf C4StartupPlrSelDlg::CrewListItem::GetDelWarning() {
  StdStrBuf sWarning;
  sWarning.Format(LoadResStr("IDS_MSG_DELETECLONK"), Core.sRankName.getData(),
                  Core.Name, GetFilename().getData());
  int32_t iPlrTime = Core.TotalPlayingTime;
  if (iPlrTime > 60 * 60 * 10)
    sWarning.Append(static_cast<const StdStrBuf &>(
        FormatString(LoadResStr("IDS_MSG_DELETECLONK_PLAYTIME"),
...
void C4StartupPlrSelDlg::CrewListItem::CrewRename() {
  ...
  // rename this entry
  pPlrSelDlg->pRenameEdit = new C4GUI::CallbackRenameEdit<
      C4StartupPlrSelDlg::CrewListItem, RenameParams>(
      pNameLabel, this, RenameParams(),
      &C4StartupPlrSelDlg::CrewListItem::DoRenaming,
      &C4StartupPlrSelDlg::CrewListItem::AbortRenaming);
}
...
C4GUI::RenameEdit::RenameResult C4StartupPlrSelDlg::CrewListItem::DoRenaming(
    RenameParams par, const char *szNewName) {
  // accept if name can be set; will fail if name is invalid or already given to
  // another Crew member
...
C4StartupPlrSelDlg::C4StartupPlrSelDlg()
    : C4StartupDlg("W"), eMode(PSDM_Player), pRenameEdit(NULL) {
  // ctor
  ...
  int iButtonXSpacing =
      (GetClientRect().Wdt > 700) ? GetClientRect().Wdt / 58 : 2;
  int iButtonCount = 6;
  C4GUI::ComponentAligner caMain(GetClientRect(), 0, 0, true);
  C4GUI::ComponentAligner caButtonArea(
      caMain.GetFromBottom(Max(caMain.GetHeight() / 15, iButtonHeight)), 0, 0);
  rcBottomButtons = caButtonArea.GetCentered(caMain.GetWidth(), iButtonHeight);
  iBottomButtonWidth =
      (caButtonArea.GetWidth() - iButtonXSpacing * (iButtonCount - 1)) /
      iButtonCount;
  C4Rect rcMain = caMain.GetAll();
  C4Rect rcPlrList = C4Rect(rcMain.Wdt / 10, rcMain.Hgt * 10 / 36,
                            rcMain.Wdt * 25 / 81, rcMain.Hgt * 2 / 3);
  C4Rect rcInfoWindow = C4Rect(rcMain.Wdt * 371 / 768, rcMain.Hgt * 197 / 451,
                               rcMain.Wdt * 121 / 384, rcMain.Hgt * 242 / 451);
  int iPictureWidth = Min(rcMain.Wdt * 121 / 384, 200);
  int iPictureHeight = iPictureWidth * 3 / 4;
  C4Rect rcPictureArea = C4Rect(rcMain.Wdt * 613 / 768 - iPictureWidth,
                                rcMain.Hgt * 197 / 451 - iPictureHeight,
  ...
  pPlrListBox->SetDecoration(false, &C4Startup::Get()->Graphics.sfctBookScroll,
                             true);
  pPlrListBox->UpdateElementPositions();
  pPlrListBox->SetSelectionChangeCallbackFn(
      new C4GUI::CallbackHandler<C4StartupPlrSelDlg>(
          this, &C4StartupPlrSelDlg::OnSelChange));
  pPlrListBox->SetSelectionDblClickFn(
      new C4GUI::CallbackHandler<C4StartupPlrSelDlg>(
          this, &C4StartupPlrSelDlg::OnSelDblClick));
  AddElement(pSelectionInfo = new C4GUI::TextWindow(rcInfoWindow));
  pSelectionInfo->SetDecoration(
      false, false, &C4Startup::Get()->Graphics.sfctBookScroll, true);
  pSelectionInfo->UpdateHeight();
  AddElement(pPortraitPict = new C4GUI::Picture(rcPictureArea, true));

  // bottom line buttons - positioning done in UpdateBottomButtons by
  // UpdatePlayerList
  C4Rect rcDefault(0, 0, 10, 10);
  AddElement(btnBack = new C4GUI::CallbackButton<C4StartupPlrSelDlg>(
                 LoadResStr("IDS_BTN_BACK"), rcDefault,
                 &C4StartupPlrSelDlg::OnBackBtn));
  AddElement(
      btnNew = new C4GUI::CallbackButton<C4StartupPlrSelDlg>(
          LoadResStr("IDS_BTN_NEW"), rcDefault, &C4StartupPlrSelDlg::OnNewBtn));
  btnNew->SetToolTip(LoadResStr("IDS_DLGTIP_NEWPLAYER"));
  AddElement(btnActivatePlr = new C4GUI::CallbackButton<C4StartupPlrSelDlg>(
                 NULL, rcDefault, &C4StartupPlrSelDlg::OnActivateBtn));
  AddElement(btnDelete = new C4GUI::CallbackButton<C4StartupPlrSelDlg>(
                 LoadResStr("IDS_BTN_DELETE"), rcDefault,
                 &C4StartupPlrSelDlg::OnDelBtn));
  AddElement(btnProperties = new C4GUI::CallbackButton<C4StartupPlrSelDlg>(
                 NULL, rcDefault, &C4StartupPlrSelDlg::OnPropertyBtn));
  AddElement(btnCrew = new C4GUI::CallbackButton<C4StartupPlrSelDlg>(
                 LoadResStr("IDS_SELECT_CREW"), rcDefault,
                 &C4StartupPlrSelDlg::OnCrewBtn));
  btnCrew->SetToolTip(LoadResStr("IDS_DLGTIP_PLAYERCREW"));

  // refill listboxes
  UpdatePlayerList();
  // Just safety incase listbox was empty, in which case no selection change
  // callback will have been done:
  ...
  pKeyBack = new C4KeyBinding(keys, "StartupPlrSelBack", KEYSCOPE_Gui,
                              new C4GUI::DlgKeyCB<C4StartupPlrSelDlg>(
                                  *this, &C4StartupPlrSelDlg::KeyBack),
                              C4CustomKey::PRIO_CtrlOverride);
  pKeyProperties =
      new C4KeyBinding(C4KeyCodeEx(K_F2), "StartupPlrSelProp", KEYSCOPE_Gui,
                       new C4GUI::DlgKeyCB<C4StartupPlrSelDlg>(
                           *this, &C4StartupPlrSelDlg::KeyProperties),
                       C4CustomKey::PRIO_CtrlOverride);
  pKeyCrew =
      new C4KeyBinding(C4KeyCodeEx(K_RIGHT), "StartupPlrSelCrew", KEYSCOPE_Gui,
                       new C4GUI::ControlKeyDlgCB<C4StartupPlrSelDlg>(
                           pPlrListBox, *this, &C4StartupPlrSelDlg::KeyCrew),
                       C4CustomKey::PRIO_CtrlOverride);
  pKeyDelete =
      new C4KeyBinding(C4KeyCodeEx(K_DELETE), "StartupPlrSelDelete",
                       KEYSCOPE_Gui, new C4GUI::DlgKeyCB<C4StartupPlrSelDlg>(
                                         *this, &C4StartupPlrSelDlg::KeyDelete),
                       C4CustomKey::PRIO_CtrlOverride);
  pKeyNew =
      new C4KeyBinding(C4KeyCodeEx(K_INSERT), "StartupPlrSelNew", KEYSCOPE_Gui,
                       new C4GUI::DlgKeyCB<C4StartupPlrSelDlg>(
                           *this, &C4StartupPlrSelDlg::KeyNew),
                       C4CustomKey::PRIO_CtrlOverride);
}
...
void C4StartupPlrSelDlg::UpdateBottomButtons() {
  ...
  switch (eMode) {
    case PSDM_Player: {
      ...
      btnBack->SetBounds(caBottomButtons.GetGridCell(
          0, 6, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnNew->SetBounds(caBottomButtons.GetGridCell(
          1, 6, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnActivatePlr->SetBounds(caBottomButtons.GetGridCell(
          2, 6, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnDelete->SetBounds(caBottomButtons.GetGridCell(
          3, 6, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnProperties->SetBounds(caBottomButtons.GetGridCell(
          4, 6, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnCrew->SetBounds(caBottomButtons.GetGridCell(
          5, 6, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
    } break;
    ...
    case PSDM_Crew: {
      ...
      btnBack->SetBounds(caBottomButtons.GetGridCell(
          0, 4, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnActivatePlr->SetBounds(caBottomButtons.GetGridCell(
          1, 4, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnDelete->SetBounds(caBottomButtons.GetGridCell(
          2, 4, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
      btnProperties->SetBounds(caBottomButtons.GetGridCell(
          3, 4, 0, 1, iBottomButtonWidth, C4GUI_ButtonHgt, true));
    } break;
  };
}
...
void C4StartupPlrSelDlg::UpdatePlayerList() {
  ...
  switch (eMode) {
    case PSDM_Player: {
      ...
      PlayerListItem *pFirstActivatedPlrItem = NULL,
                     *pFirstDeactivatedPlrItem = NULL, *pPlrItem = NULL;
      for (DirectoryIterator i(sSearchPath.getData()); szFn = *i; i++) {
        ...
        bool fIsParticipating =
            !!SIsModule(Config.General.Participants, szFn, NULL, false);
        pPlrItem =
            new PlayerListItem(this, pPlrListBox, NULL, fIsParticipating);
        try {
        ...
        } else if (!pFirstDeactivatedPlrItem)
          pFirstDeactivatedPlrItem = pPlrItem;
      }
      // select first element; prefer activated player
      if (!(pPlrItem = pFirstActivatedPlrItem))
        pPlrItem = pFirstDeactivatedPlrItem;
      if (pPlrItem) pPlrListBox->SelectEntry(pPlrItem, false);
    ...
    case PSDM_Crew: {
      SetTitle(FormatString("%s %s", LoadResStrNoAmp("IDS_CTL_CREW"),
                            CurrPlayer.Core.PrefName).getData());
      // crew mode: Insert complete crew of player (2do: sort)
      bool fSucc;
      char szFn[_MAX_PATH + 1];
      for (fSucc = CurrPlayer.Grp.FindEntry(C4CFN_ObjectInfoFiles, szFn); fSucc;
           fSucc = CurrPlayer.Grp.FindNextEntry(C4CFN_ObjectInfoFiles, szFn,
                                                NULL, NULL, true)) {
        CrewListItem *pCrewItem =
            new CrewListItem(this, pPlrListBox, CurrPlayer.Core.PrefColorDw);
        try {
...
void C4StartupPlrSelDlg::UpdateSelection() {
  ...
  // button text 'activate' if current player is deactivated; 'deactivate'
  // otherwise
  if (pSel && pSel->IsActivated()) {
    btnActivatePlr->SetText(LoadResStr("IDS_BTN_DEACTIVATE"));
    btnActivatePlr->SetToolTip(
        FormatString(LoadResStr("IDS_MSG_NOPARTICIPATE_DESC"), pSel->GetName())
            .getData());
  } else {
    btnActivatePlr->SetText(LoadResStr("IDS_BTN_ACTIVATE"));
    btnActivatePlr->SetToolTip(
        FormatString(LoadResStr("IDS_MSG_PARTICIPATE_DESC"),
                     pSel ? pSel->GetName() : "").getData());
  }
  // no item selected?
  if (!pSel) {
...
void C4StartupPlrSelDlg::OnItemCheckChange(C4GUI::Element *pCheckBox) {
  switch (eMode) {
    ...
    case PSDM_Crew:
      ...
      if (pCheckBox)
        static_cast<CrewListItem *>(pCheckBox->GetParent())
            ->UpdateClonkEnabled();
      break;
  }
...
void C4StartupPlrSelDlg::UpdateActivatedPlayers() {
  ...
  *Config.General.Participants = '\0';
  for (ListItem *pPlrItem = static_cast<ListItem *>(pPlrListBox->GetFirst());
       pPlrItem; pPlrItem = pPlrItem->GetNext())
    if (pPlrItem->IsActivated()) {
      const char *szAddFilename = pPlrItem->GetFilename().getData();
      if (SLen(Config.General.Participants) + 1 + SLen(szAddFilename) <
          sizeof(Config.General.Participants))
        SAddModule(Config.General.Participants, szAddFilename);
      else {
        pPlrItem->SetActivated(false);
        GetScreen()->ShowMessage(
            FormatString(LoadResStr("IDS_ERR_PLAYERSTOOLONG"),
                         pPlrItem->GetName()).getData(),
            LoadResStr("IDS_ERR_TITLE"), C4GUI::Ico_Error);
      }
    }
}
...
void C4StartupPlrSelDlg::OnNewBtn(C4GUI::Control *btn) {
  ...
  pDlg->SetPos(Min<int32_t>(GetBounds().Wdt / 10,
                            GetBounds().Wdt - pDlg->GetBounds().Wdt),
               Min<int32_t>(GetBounds().Hgt / 4,
                            GetBounds().Hgt - pDlg->GetBounds().Hgt));
}

bool C4StartupPlrSelDlg::CheckPlayerName(const StdStrBuf &Playername,
                                         StdStrBuf &Filename,
                                         const StdStrBuf *pPrevFilename,
                                         bool fWarnEmpty) {
  // must not be empty
  if (!Playername.getLength()) {
    if (fWarnEmpty)
      C4GUI::Screen::GetScreenS()->ShowMessage(
          LoadResStr("IDS_ERR_PLRNAME_EMPTY"), "", C4GUI::Ico_Error);
    return false;
  }
  ...
  if (!pPrevFilename ||
      !ItemIdentical(Path.getData(), pPrevFilename->getData()))
    if (ItemExists(Path.getData())) {
      C4GUI::Screen::GetScreenS()->ShowMessage(
          FormatString(LoadResStr("IDS_ERR_PLRNAME_TAKEN"),
                       Playername.getData()).getData(),
          "", C4GUI::Ico_Error);
      return false;
    }
  Filename.Take(Path);
...
void C4StartupPlrSelDlg::SetPlayerMode() {
  ...
  LastPlrFilename.Copy(
      static_cast<const StdStrBuf &>(CurrPlayer.Grp.GetFullName()));
...
void C4StartupPlrSelDlg::SetCrewMode(PlayerListItem *pSel) {
  ...
  if (!CurrPlayer.Grp.FindEntry(C4CFN_ObjectInfoFiles)) {
    StdCopyStrBuf strCrew(FormatString("%s %s", LoadResStrNoAmp("IDS_CTL_CREW"),
                                       CurrPlayer.Core.PrefName));
    // player has no crew!
...
void C4StartupPlrSelDlg::OnDelBtn(C4GUI::Control *btn) {
  ...
  if (!pSel) return;
  StdStrBuf sWarning;
  sWarning.Take(pSel->GetDelWarning());
  GetScreen()->ShowRemoveDlg(new C4GUI::ConfirmationDialog(
      sWarning.getData(), LoadResStr("IDS_BTN_DELETE"),
      new C4GUI::CallbackHandlerExPar<C4StartupPlrSelDlg, ListItem *>(
          this, &C4StartupPlrSelDlg::OnDelBtnConfirm, pSel),
  ...
  switch (eMode) {
    case PSDM_Player:
      if (!C4Group_DeleteItem(pSel->GetFilename().getData())) {
        StdStrBuf sMsg;
        sMsg.Copy(LoadResStr("IDS_FAIL_DELETE"));
        GetScreen()->ShowMessage(sMsg.getData(), LoadResStr("IDS_DLG_CLEAR"),
                                 C4GUI::Ico_Error);
      }
      break;
...
void C4StartupPlrSelDlg::OnPropertyBtn(C4GUI::Control *btn) {
  ...
  switch (eMode) {
    case PSDM_Player: {
      ...
      if (!pSel) return;
      C4GUI::Dialog *pDlg;
      GetScreen()->ShowRemoveDlg(pDlg =
                                     new C4StartupPlrPropertiesDlg(pSel, this));
      pDlg->SetPos(Min<int32_t>(GetBounds().Wdt / 10,
                                GetBounds().Wdt - pDlg->GetBounds().Wdt),
...
class C4StartupPlrSelDlg_CrewSortDataMatchType {
 ...
 public:
  ...
  bool operator()(C4StartupPlrSelDlg_CrewSortDataEntry Check) {
    return Check.idType == idType;
  }
};

typedef std::vector<C4StartupPlrSelDlg_CrewSortDataEntry>
    C4StartupPlrSelDlg_CrewSortData;

int32_t C4StartupPlrSelDlg::CrewSortFunc(const C4GUI::Element *pEl1,
                                         const C4GUI::Element *pEl2,
                                         void *par) {
  ...
  C4StartupPlrSelDlg_CrewSortData &rSortData =
      *static_cast<C4StartupPlrSelDlg_CrewSortData *>(par);
  C4StartupPlrSelDlg_CrewSortData::iterator
      i = std::find_if(
          rSortData.begin(), rSortData.end(),
          C4StartupPlrSelDlg_CrewSortDataMatchType(pItem1->GetCore().id)),
      j = std::find_if(
          rSortData.begin(), rSortData.end(),
          C4StartupPlrSelDlg_CrewSortDataMatchType(pItem2->GetCore().id));
  // primary sort: By Clonk type, where high exp Clonk types are sorted atop low
  // exp Clonk types
  if (i != j) {
    if (i == rSortData.end())
      return -1;
...
void C4StartupPlrSelDlg::ResortCrew() {
  ...
  for (CrewListItem *pCrewItem =
           static_cast<CrewListItem *>(pPlrListBox->GetFirst());
       pCrewItem; pCrewItem = pCrewItem->GetNext()) {
    C4StartupPlrSelDlg_CrewSortData::iterator i = std::find_if(
        SortData.begin(), SortData.end(),
        C4StartupPlrSelDlg_CrewSortDataMatchType(pCrewItem->GetCore().id));
    if (i == SortData.end())
      SortData.push_back(C4StartupPlrSelDlg_CrewSortDataEntry(
          pCrewItem->GetCore().Experience, pCrewItem->GetCore().id));
    else
      (*i).iMaxExp =
          Max<int32_t>((*i).iMaxExp, pCrewItem->GetCore().Experience);
  }
...
C4StartupPlrPropertiesDlg::C4StartupPlrPropertiesDlg(
    C4StartupPlrSelDlg::PlayerListItem *pForPlayer,
    C4StartupPlrSelDlg *pParentDlg)
    : Dialog(C4Startup::Get()->Graphics.fctPlrPropBG.Wdt,
             C4Startup::Get()->Graphics.fctPlrPropBG.Hgt, "", false),
      pForPlayer(pForPlayer),
      pMainDlg(pParentDlg),
      fClearPicture(false),
      fClearBigIcon(false) {
  if (pForPlayer) {
  ...
  C4GUI::ComponentAligner caButtonArea(
      caMain.GetFromBottom(C4GUI_ButtonAreaHgt), 0, 0);
  // dlg title
  const char *szTitle;
  if (pForPlayer) {
  ...
  C4GUI::Label *pLbl =
      new C4GUI::Label(szTitle, caMain.GetFromTop(pUseFont->GetLineHeight()),
                       ALeft, C4StartupFontClr, pUseFont, false);
  AddElement(pLbl);
  caMain.ExpandTop(-BetweenElementDist);
  // place name label
  AddElement(new C4GUI::Label(LoadResStr("IDS_CTL_NAME2"),
                              caMain.GetFromTop(pSmallFont->GetLineHeight()),
                              ALeft, C4StartupFontClr, pSmallFont, false));
  // place name edit
  pNameEdit = new C4GUI::Edit(
      caMain.GetFromTop(C4GUI::Edit::GetCustomEditHeight(pUseFont)));
  pNameEdit->SetFont(pUseFont);
  pNameEdit->SetColors(C4StartupEditBGColor, C4StartupFontClr,
  ...
  AddElement(new C4GUI::Label(
      FormatString("%s:", LoadResStr("IDS_CTL_COLOR")).getData(),
      caMain.GetFromTop(pSmallFont->GetLineHeight()), ALeft, C4StartupFontClr,
      pSmallFont, false));
  // place color controls
  C4GUI::ComponentAligner caColorArea(
      caMain.GetFromTop(C4GUI::ArrowButton::GetDefaultHeight()), 2, 0);
  caColorArea.ExpandLeft(2);
  C4GUI::Button *pBtn;
  const char *szTip;
  szTip = LoadResStr("IDS_DLGTIP_PLAYERCOLORS");
  AddElement(pBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                                              C4GUI::ArrowButton>(
                 C4GUI::ArrowButton::Left,
                 caColorArea.GetFromLeft(C4GUI::ArrowButton::GetDefaultWidth()),
                 &C4StartupPlrPropertiesDlg::OnClrChangeLeft));
  pBtn->SetToolTip(szTip);
  C4Facet &rfctClrPreviewPic =
      Game.GraphicsResource
          .fctFlagClr;  // C4Startup::Get()->Graphics.fctCrewClr;
                        // //Game.GraphicsResource.fctCrewClr;
  pClrPreview = new C4GUI::Picture(
      caColorArea.GetFromLeft(
          rfctClrPreviewPic.GetWidthByHeight(caColorArea.GetHeight())),
      true);
  pClrPreview->SetFacet(rfctClrPreviewPic);
  AddElement(pClrPreview);
  AddElement(pBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                                              C4GUI::ArrowButton>(
                 C4GUI::ArrowButton::Right,
                 caColorArea.GetFromLeft(C4GUI::ArrowButton::GetDefaultWidth()),
                 &C4StartupPlrPropertiesDlg::OnClrChangeRight));
  pBtn->SetToolTip(szTip);
  szTip = LoadResStr("IDS_DLGTIP_PLAYERCOLORSTGB");
  int32_t iSliderYDiff = (caColorArea.GetHeight() - 3 * C4GUI_ScrollBarHgt) / 2;
  pClrSliderR = new C4GUI::ScrollBar(
      caColorArea.GetFromTop(C4GUI_ScrollBarHgt), true,
      new C4GUI::ParCallbackHandler<C4StartupPlrPropertiesDlg, int32_t>(
          this, &C4StartupPlrPropertiesDlg::OnClrSliderRChange));
  pClrSliderR->SetDecoration(&C4Startup::Get()->Graphics.sfctBookScrollR,
                             false);
  pClrSliderR->SetToolTip(szTip);
  caColorArea.ExpandTop(-iSliderYDiff);
  pClrSliderG = new C4GUI::ScrollBar(
      caColorArea.GetFromTop(C4GUI_ScrollBarHgt), true,
      new C4GUI::ParCallbackHandler<C4StartupPlrPropertiesDlg, int32_t>(
          this, &C4StartupPlrPropertiesDlg::OnClrSliderGChange));
  pClrSliderG->SetDecoration(&C4Startup::Get()->Graphics.sfctBookScrollG,
                             false);
  pClrSliderG->SetToolTip(szTip);
  caColorArea.ExpandTop(-iSliderYDiff);
  pClrSliderB = new C4GUI::ScrollBar(
      caColorArea.GetFromTop(C4GUI_ScrollBarHgt), true,
      new C4GUI::ParCallbackHandler<C4StartupPlrPropertiesDlg, int32_t>(
          this, &C4StartupPlrPropertiesDlg::OnClrSliderBChange));
  pClrSliderB->SetDecoration(&C4Startup::Get()->Graphics.sfctBookScrollB,
                             false);
  ...
  int32_t iControlPicSize = C4GUI::ArrowButton::
      GetDefaultHeight();  // GetGridCell(0,3,0,1,-1,-1,false,2)
  C4GUI::ComponentAligner caControlArea(
      caMain.GetFromTop(iControlPicSize + pSmallFont->GetLineHeight() +
                        BetweenElementDist),
      0, 0, false);
  C4GUI::ComponentAligner caPictureArea(
      caControlArea.GetFromRight(iControlPicSize), 0, 0, false);
  AddElement(new C4GUI::Label(
      FormatString("%s:", LoadResStr("IDS_CTL_CONTROL")).getData(),
      caControlArea.GetFromTop(pSmallFont->GetLineHeight()), ALeft,
      C4StartupFontClr, pSmallFont, false));
  AddElement(
      new C4GUI::Label(LoadResStr("IDS_CTL_PICTURE"),
                       caPictureArea.GetFromTop(pSmallFont->GetLineHeight()),
                       ACenter, C4StartupFontClr, pSmallFont, false));
  caControlArea.ExpandTop(-BetweenElementDist);
  caPictureArea.ExpandTop(-BetweenElementDist);
  // place control controls
  C4GUI::ComponentAligner caControl(caControlArea.GetFromTop(iControlPicSize),
                                    2, 0);
  szTip = LoadResStr("IDS_DLGTIP_PLAYERCONTROL");
  AddElement(pBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                                              C4GUI::ArrowButton>(
                 C4GUI::ArrowButton::Left,
                 caControl.GetFromLeft(C4GUI::ArrowButton::GetDefaultWidth()),
                 &C4StartupPlrPropertiesDlg::OnCtrlChangeLeft));
  pBtn->SetToolTip(szTip);
  C4Facet &rfctCtrlPic =
      Game.GraphicsResource.fctKeyboard;  // UpdatePlayerControl() will
                                          // alternatively set fctGamepad
  AddElement(pCtrlImg = new C4GUI::Picture(
                 caControl.GetFromLeft(
                     rfctCtrlPic.GetWidthByHeight(caControl.GetHeight())),
                 true));
  ...
  AddElement(pBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                 ...
                 C4GUI::ArrowButton::Right,
                 caControl.GetFromLeft(C4GUI::ArrowButton::GetDefaultWidth()),
                 &C4StartupPlrPropertiesDlg::OnCtrlChangeRight));
  pBtn->SetToolTip(szTip);
  caControl.ExpandLeft(-10);
  AddElement(pMouseBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                                                   C4GUI::IconButton>(
                 C4GUI::Ico_MouseOff,
                 caControl.GetFromLeft(caControl.GetHeight()), 'M' /* 2do */,
                 &C4StartupPlrPropertiesDlg::OnCtrlChangeMouse));
  ...
  AddElement(pPictureBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                                                     C4GUI::IconButton>(
                 C4GUI::Ico_Player, caPictureArea.GetAll(), 'P' /* 2do */,
                 &C4StartupPlrPropertiesDlg::OnPictureBtn));
  ...
  AddElement(new C4GUI::Label(
      FormatString("%s:", LoadResStr("IDS_DLG_MOVEMENT")).getData(),
      caMain.GetFromTop(pSmallFont->GetLineHeight()), ALeft, C4StartupFontClr,
      pSmallFont, false));
  // place AutoStopControl controls
  C4Facet &rfctMovementIcons = C4Startup::Get()->Graphics.fctPlrCtrlType;
  C4GUI::ComponentAligner caMovement(caMain.GetFromTop(rfctMovementIcons.Hgt),
                                     5, 0);
  C4Rect rcBtn = caMovement.GetFromLeft(
      rfctMovementIcons.GetWidthByHeight(caMovement.GetHeight()));
  AddElement(pLbl = new C4GUI::Label(LoadResStr("IDS_DLG_JUMPANDRUN"),
                                     rcBtn.x + rcBtn.Wdt / 2,
                                     rcBtn.y + rcBtn.Hgt - 6, ACenter,
                                     C4StartupFontClr, pSmallFont, false));
  szTip = LoadResStr("IDS_DLGTIP_JUMPANDRUN");
  pLbl->SetToolTip(szTip);
  AddElement(pJumpNRunBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                                                      C4GUI::IconButton>(
                 C4GUI::Ico_None, rcBtn, 'J' /* 2do */,
                 &C4StartupPlrPropertiesDlg::OnMovementBtn));
  pJumpNRunBtn->SetToolTip(szTip);
  rcBtn = caMovement.GetFromRight(
      rfctMovementIcons.GetWidthByHeight(caMovement.GetHeight()));
  AddElement(pLbl = new C4GUI::Label(LoadResStr("IDS_DLG_CLASSIC"),
                                     rcBtn.x + rcBtn.Wdt / 2,
                                     rcBtn.y + rcBtn.Hgt - 6, ACenter,
                                     C4StartupFontClr, pSmallFont, false));
  szTip = LoadResStr("IDS_DLGTIP_CLASSIC");
  pLbl->SetToolTip(szTip);
  AddElement(pClassicBtn = new C4GUI::CallbackButton<C4StartupPlrPropertiesDlg,
                                                     C4GUI::IconButton>(
                 C4GUI::Ico_None, rcBtn, 'C' /* 2do */,
                 &C4StartupPlrPropertiesDlg::OnMovementBtn));
  pClassicBtn->SetToolTip(szTip);
  UpdatePlayerMovement();
  // place buttons
  // OK
  C4GUI::Button *pBtnOK = new C4GUI::OKIconButton(
      C4Rect(147 - GetMarginLeft(), 295 + 35 - GetMarginTop(), 54, 33),
      C4GUI::Ico_None);
  AddElement(pBtnOK);  // pBtnOK->SetToolTip(LoadResStr("IDS_DLGTIP_OK"));
  // Cancel
  C4GUI::Button *pBtnAbort = new C4GUI::CancelIconButton(
      C4Rect(317 - GetMarginLeft(), 16 - GetMarginTop(), 21, 21),
      C4GUI::Ico_None);
  AddElement(
  ...
  if (!pForPlayer) {
    ...
    C4Group hGroup;
    StdStrBuf strPortrait;
    strPortrait.Format("Portrait%d.png", 1 + Random(5));
  ...
  // when called from player selection screen: input dlg always closed in the
  // end
...
void C4StartupPlrPropertiesDlg::DrawElement(C4FacetEx &cgo) {
  C4Startup::Get()->Graphics.fctPlrPropBG.Draw(
      cgo.Surface, rcBounds.x + cgo.TargetX, rcBounds.y + cgo.TargetY);
}
...
void C4StartupPlrPropertiesDlg::UpdatePlayerColor(bool fUpdateSliders) {
  if (!C4P.PrefColorDw)
    C4P.PrefColorDw = 1;  // no black! Would turn to blue in some instances
...
void C4StartupPlrPropertiesDlg::UpdatePlayerControl() {
  ...
  C4Facet &rfctCtrlPic = (C4P.PrefControl < C4P_Control_GamePad1)
                             ? Game.GraphicsResource.fctKeyboard
                             : Game.GraphicsResource.fctGamepad;
  pCtrlImg->SetFacet(rfctCtrlPic);
  pCtrlImg->GetMFacet().X +=
      rfctCtrlPic.Wdt *
      (C4P.PrefControl -
       ((C4P.PrefControl < C4P_Control_GamePad1) ? 0 : C4P_Control_GamePad1));
  // update mouse image
...
void C4StartupPlrPropertiesDlg::UpdatePlayerMovement() {
  ...
  pJumpNRunBtn->SetFacet(C4Startup::Get()->Graphics.fctPlrCtrlType.GetPhase(
      C4P.PrefControlStyle ? 1 : 0, 1));
  pClassicBtn->SetFacet(C4Startup::Get()->Graphics.fctPlrCtrlType.GetPhase(
      C4P.PrefControlStyle ? 0 : 1, 0));
}
...
void C4StartupPlrPropertiesDlg::UserClose(bool fOK) {
  ...
  if (fOK) {
    ...
    if (!C4StartupPlrSelDlg::CheckPlayerName(
            PlrName, Filename, pForPlayer ? &pForPlayer->GetFilename() : NULL,
            true))
      return;
  }
  Close(fOK);
    ...
    if (C4StartupPlrSelDlg::CheckPlayerName(
      ...
      if (pForPlayer) {
        if (!pForPlayer->MoveFilename(Filename.getData()))
          GetScreen()->ShowMessage(LoadResStr("IDS_FAIL_RENAME"), "",
                                   C4GUI::Ico_Error);
        // update picture/bigicon
        if (fClearPicture || fClearBigIcon || fctNewPicture.Surface ||
            fctNewBigIcon.Surface) {
          C4Group PlrGroup;
          if (PlrGroup.Open(Filename.getData())) {
            if (fClearPicture || fctNewPicture.Surface)
              PlrGroup.Delete(C4CFN_Portrait);
            if (fClearBigIcon || fctNewBigIcon.Surface)
              PlrGroup.Delete(C4CFN_BigIcon);
            ...
            if (fClearBigIcon || fctNewBigIcon.Surface)
              pForPlayer->GrabCustomIcon(fctNewBigIcon);
            if (fClearPicture || fctNewPicture.Surface)
              pForPlayer->GrabPortrait(&fctNewPicture);
          }
        } else {
          fSucc = true;
        }
        pForPlayer->UpdateCore(C4P);
        // player may have been activated: Make sure any new filename is
        // reflected in participants list
        ...
        if (PlrGroup.Open(Filename.getData(), true)) {
          ...
          if (pMainDlg) {
            ...
            // no main player selection dialog: This means that this dlg was
            // shown as a creation dialog from the main startup dlg
            // Just set the newly created player as current selection
            SCopy(Config.AtExeRelativePath(Filename.getData()),
...
bool C4StartupPlrPropertiesDlg::SetNewPicture(C4Surface &srcSfc,
                                              C4FacetExSurface *trgFct,
                                              int32_t iMaxSize,
                                              bool fColorize) {
  if (fColorize) {
    C4Surface srcSfcClr;
  ...
  if (!szFromFilename) {
    ...
    if (SrcGrp.Open(sParentPath.getData())) {
      if (sfcNewPic.Load(SrcGrp, GetFilename(szFromFilename))) {
        fSucc = true;
        if (fSetPicture)
          if (!SetNewPicture(sfcNewPic, &fctNewPicture, C4MaxPictureSize,
                             false))
            fSucc = false;
        if (fSetBigIcon)
          if (!SetNewPicture(sfcNewPic, &fctNewBigIcon, C4MaxBigIconSize, true))
            fSucc = false;
      }
    }
    if (!fSucc) {
      ...
      GetScreen()->ShowErrorMessage(
          FormatString(LoadResStr("IDS_PRC_NOGFXFILE"), szFromFilename,
                       SrcGrp.GetError()).getData());
    }
  }
  // update icon
...
void C4StartupPlrPropertiesDlg::OnPictureBtn(C4GUI::Control *pBtn) {
  StdStrBuf sNewPic;
  bool fSetPicture = true, fSetBigIcon = true;
  if (C4PortraitSelDlg::SelectPortrait(GetScreen(), &sNewPic, &fSetPicture,
                                       &fSetBigIcon)) {
