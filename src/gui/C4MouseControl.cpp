...
const int32_t C4MC_Drag_None = 0, C4MC_Drag_Selecting = 1, C4MC_Drag_Moving = 2,
              // C4MC_Drag_Menu					= 3,
...
const int32_t C4MC_Cursor_Region = 0, C4MC_Cursor_Crosshair = 1,
              C4MC_Cursor_Enter = 2, C4MC_Cursor_Grab = 3, C4MC_Cursor_Chop = 4,
              C4MC_Cursor_Dig = 5, C4MC_Cursor_Build = 6,
              C4MC_Cursor_Select = 7, C4MC_Cursor_Object = 8,
              C4MC_Cursor_Ungrab = 9, C4MC_Cursor_Up = 10,
              C4MC_Cursor_Down = 11, C4MC_Cursor_Left = 12,
              C4MC_Cursor_Right = 13, C4MC_Cursor_UpLeft = 14,
              C4MC_Cursor_UpRight = 15, C4MC_Cursor_DownLeft = 16,
              C4MC_Cursor_DownRight = 17, C4MC_Cursor_JumpLeft = 18,
              C4MC_Cursor_JumpRight = 19, C4MC_Cursor_Drop = 20,
              C4MC_Cursor_ThrowRight = 21, C4MC_Cursor_Put = 22,
...
void C4MouseControl::Default() {
  ...
  IsHelpCaption = false;
  CaptionBottomY = 0;
  VpX = VpY = X = Y = DownX = DownY = DownOffsetX = DownOffsetY = ViewX =
      ViewY = 0;
...
void C4MouseControl::UpdateClip() {
  ...
  if (!Active || !Application.Active ||
      (Game.pGUI && Game.pGUI->HasMouseFocus())) {
    ClipCursor(NULL);
    return;
  }
  ...
  vpRct.left = pVP->OutX;
  vpRct.top = pVP->OutY;
  vpRct.right = pVP->OutX + pVP->ViewWdt;
  vpRct.bottom = pVP->OutY + pVP->ViewHgt;
  // adjust by window pos
  RECT rtWindow;
  if (GetWindowRect(FullScreen.hWindow, &rtWindow)) {
    vpRct.left += rtWindow.left;
    vpRct.top += rtWindow.top;
    vpRct.right += rtWindow.left;
    vpRct.bottom += rtWindow.top;
  }
  ClipCursor(&vpRct);
  // and inform GUI
  if (Game.pGUI)
    Game.pGUI->SetPreferredDlgRect(
        C4Rect(pVP->OutX, pVP->OutY, pVP->ViewWdt, pVP->ViewHgt));
#endif
  // StdWindow manages this.
}

void C4MouseControl::Move(int32_t iButton, int32_t iX, int32_t iY,
                          DWORD dwKeyFlags, BOOL fCenter) {
  // Active
  if (!Active || !fMouseOwned) return;
  // Execute caption
  if (KeepCaption)
  ...
  // Check player
  if ((Player > NO_OWNER) && !(pPlayer = Game.Players.Get(Player))) {
    Active = FALSE;
    return;
  }
  // Check viewport
  ...
  fctViewport.Set(NULL, rcViewport.x, rcViewport.y, rcViewport.Wdt,
                  rcViewport.Hgt);
...
// First time viewport attachment: center mouse
  ...
  if (iButton != C4MC_Button_Wheel) {
    ...
    // Control state
    ControlDown = false;
    if (dwKeyFlags & MK_CONTROL) ControlDown = true;
    ShiftDown = false;
    if (dwKeyFlags & MK_SHIFT) ShiftDown = true;
    // Target region
    UpdateTargetRegion();
    // Scrolling
    UpdateScrolling();
    ...
    // Blocked by fog of war: evaluate button up, dragging and region controls
    // only
    if (FogOfWar && Drag == C4MC_Drag_None && !TargetRegion) {
      ...
      if (iButton == C4MC_Button_RightUp) {
        ...
        if (Drag == C4MC_Drag_None) SendPlayerSelectNext();
      }
    }
  }
...
void C4MouseControl::Draw(C4FacetEx &cgo) {
  ...
  switch (Drag) {
    ...
    case C4MC_Drag_Construct:
      ...
      // Previously, there used to be custom-defined hotspots for all cursors.
      // Calc them in.
      if (GfxR->fOldStyleCursor) {
        switch (Cursor) {
          case C4MC_Cursor_Select:
          case C4MC_Cursor_Region:  // case C4MC_Cursor_DragMenu:
            iOffsetX = iOffsetY = 0;
            break;
        ...
        switch (Cursor) {
          case C4MC_Cursor_Up:
            iOffsetY += -GfxR->fctMouseCursor.Hgt / 2;
            break;
          case C4MC_Cursor_Down:
            iOffsetY += +GfxR->fctMouseCursor.Hgt / 2;
            break;
          case C4MC_Cursor_Left:
            iOffsetX += -GfxR->fctMouseCursor.Wdt / 2;
            break;
          case C4MC_Cursor_Right:
            iOffsetX += +GfxR->fctMouseCursor.Wdt / 2;
            break;
          case C4MC_Cursor_UpLeft:
            iOffsetX += -GfxR->fctMouseCursor.Wdt / 2;
            iOffsetY += -GfxR->fctMouseCursor.Hgt / 2;
            break;
          case C4MC_Cursor_UpRight:
            iOffsetX += +GfxR->fctMouseCursor.Wdt / 2;
            iOffsetY += -GfxR->fctMouseCursor.Hgt / 2;
            break;
          case C4MC_Cursor_DownLeft:
            iOffsetX += -GfxR->fctMouseCursor.Wdt / 2;
            iOffsetY += +GfxR->fctMouseCursor.Hgt / 2;
            break;
          case C4MC_Cursor_DownRight:
            iOffsetX += +GfxR->fctMouseCursor.Wdt / 2;
            iOffsetY += +GfxR->fctMouseCursor.Hgt / 2;
            break;
        }
      ...
      if (ShiftDown)
        if ((Cursor != C4MC_Cursor_Region) &&
            (Cursor != C4MC_Cursor_Select)  //&& (Cursor!=C4MC_Cursor_DragMenu)
            && (Cursor != C4MC_Cursor_JumpLeft) &&
            (Cursor != C4MC_Cursor_JumpRight))
          if (!IsPassive()) fAddMark = true;
      // Drag image
      if (DragImage.Surface) {
        ...
        DragImage.DrawClrMod(cgo.Surface, cgo.X + VpX - iOffsetX,
                             cgo.Y + VpY - iOffsetY, 0, 0,
                             DragImagePhase ? 0x8f7f0000 : 0x1f007f00);
    ...
    case C4MC_Drag_Selecting:
      // Draw frame
      Application.DDraw->DrawFrame(cgo.Surface, cgo.X + VpX, cgo.Y + VpY,
  ...
  if (Caption) {
    if (IsHelpCaption && Game.pGUI) {
      ...
      C4GUI::Screen::DrawToolTip(Caption.getData(), cgoTip, cgo.X + VpX,
                                 cgo.Y + VpY);
    } else {
      // Otherwise red mouse control style
      int32_t iWdt, iHgt;
      Game.GraphicsResource.FontRegular.GetTextExtent(Caption.getData(), iWdt,
                                                      iHgt, true);
      Application.DDraw->TextOut(
          Caption.getData(), Game.GraphicsResource.FontRegular, 1.0,
          cgo.Surface,
          cgo.X + BoundBy<int32_t>(VpX, iWdt / 2 + 1, cgo.Wdt - iWdt / 2 - 1),
          cgo.Y + Min<int32_t>(
                      CaptionBottomY ? CaptionBottomY - iHgt - 1 : VpY + 13,
                      cgo.Hgt - iHgt),
          0xfaFF0000, ACenter);
    }
  }
...
void C4MouseControl::UpdateCursorTarget() {
  ...
  DWORD ocf = OCF_Grab | OCF_Chop | OCF_Container | OCF_Construct | OCF_Living |
              OCF_Carryable | OCF_Container | OCF_Exclusive;
  if (Help) ocf |= OCF_All;
  TargetObject = GetTargetObject(X, Y, ocf);
  if (TargetObject && FogOfWar && !(TargetObject->Category & C4D_IgnoreFoW))
    TargetObject = NULL;
  ...
  if (TargetObject && !IsPassive()) {
    ...
    // get position
    int32_t iObjX, iObjY;
    TargetObject->GetViewPos(iObjX, iObjY, ViewX, ViewY, fctViewport);
    // Enter (containers)
    if (ocf & OCF_Container)
      if (TargetObject->OCF & OCF_Entrance) Cursor = C4MC_Cursor_Enter;
    // Grab / Ungrab
    if (ocf & OCF_Grab) {
    ...
    // Chop (reduced range)
    if (ocf & OCF_Chop)
      if (Inside<int32_t>(X - iObjX, -TargetObject->Shape.Wdt / 3,
                          +TargetObject->Shape.Wdt / 3))
        if (Inside<int32_t>(Y - iObjY, -TargetObject->Shape.Wdt / 2,
                            +TargetObject->Shape.Wdt / 3))
          Cursor = C4MC_Cursor_Chop;
    // Enter
    if (ocf & OCF_Entrance) Cursor = C4MC_Cursor_Enter;
    // Build
    ...
    if (TargetObject->Category & C4D_MouseSelect) Cursor = C4MC_Cursor_Select;
    // Attack
    if (ocf & OCF_Alive)
      if (TargetObject->GetAlive())
        if (Hostile(Player, TargetObject->Owner)) Cursor = C4MC_Cursor_Attack;
  }
    ...
    if (!pPlrCursor->Contained)
      if (pPlrCursor->GetProcedure() == DFA_WALK)
        if (Inside<int32_t>(Y - pPlrCursor->y, -25, -10)) {
          if (Inside<int32_t>(X - pPlrCursor->x, -15, -1))
            Cursor = C4MC_Cursor_JumpLeft;
          if (Inside<int32_t>(X - pPlrCursor->x, +1, +15))
            Cursor = C4MC_Cursor_JumpRight;
        }
    ...
    if (TimeOnTargetObject >= C4MC_Time_on_Target) {
      ...
      if (idCaption)
        if (!KeepCaption) {
          ...
          if (fDouble) {
            Caption.AppendChar('|');
            Caption.Append(LoadResStr("IDS_CON_DOUBLECLICK"));
          }
          IsHelpCaption = false;
        }
    }
...
int32_t C4MouseControl::UpdateCrewSelection() {
  ...
  for (cLnk = pPlayer->Crew.First; cLnk && (cObj = cLnk->Obj);
    ...
    if (!cObj->CrewDisabled) {
      int32_t iObjX, iObjY;
      cObj->GetViewPos(iObjX, iObjY, ViewX, ViewY, fctViewport);
...
int32_t C4MouseControl::UpdateSingleSelection() {
  ...
  if (TargetObject && (Cursor == C4MC_Cursor_Select)) {
    Selection.Clear();
    Selection.Add(TargetObject, C4ObjectList::stNone);
  }

  // Cursor has moved off single crew (or target object) selection: clear
  // selection
...
void C4MouseControl::UpdateScrolling() {
  ...
  if (VpX == 0) {
    ...
    ScrollView(-ScrollSpeed, 0, Viewport->ViewWdt, Viewport->ViewHgt);
    Scrolling = true;
  }
  if (VpY == 0) {
    Cursor = C4MC_Cursor_Up;
    ScrollView(0, -ScrollSpeed, Viewport->ViewWdt, Viewport->ViewHgt);
    Scrolling = true;
  }
...
void C4MouseControl::UpdateTargetRegion() {
  ...
  KeepCaption = 0;
  // Help region caption by region target object; not in menu, because this
  // would be the cursor object
  if (Help)
    if (TargetRegion->Target /*&& Cursor!=C4MC_Cursor_DragMenu*/) {
      if (TargetRegion->Target->Def->GetDesc())
        Caption.Format("%s: %s", TargetRegion->Target->GetName(),
                       TargetRegion->Target->Def->GetDesc());
      else
  ...
  if (TargetRegion->MoveOverCom) {
    if ((TargetRegion->X != iLastRegionX) ||
      ...
      // Control queue
      Game.Input.Add(CID_PlrControl, new C4ControlPlayerControl(
                                         Player, TargetRegion->MoveOverCom,
                                         TargetRegion->Data));
    }
  } else {
...
BOOL C4MouseControl::UpdatePutTarget(BOOL fVehicle) {
  ...
  if (TargetObject = GetTargetObject(X, Y, ocf)) {
    ...
    if (Selection.GetObject()) {
      ...
      // Set caption
      Caption.Format(
          LoadResStr(fVehicle ? "IDS_CON_VEHICLEPUT" : "IDS_CON_PUT"),
          sName.getData(), TargetObject->GetName());
      IsHelpCaption = false;
    }
    // Put target found
    return TRUE;
...
void C4MouseControl::LeftDown() {
  ...
  if (TargetRegion) {
    ...
    if (!Help && pPlayer && pPlayer->ControlStyle &&
        (DownRegion.Com & (COM_Double | COM_Single)) == 0)
...
void C4MouseControl::DragMoving() {
  ...
  if (Selection.GetObject() && (Selection.GetObject()->OCF & OCF_Carryable)) {
    ...
    if (ControlDown)
      if (UpdatePutTarget(FALSE)) return;
    // In liquid: drop
    if (GBackLiquid(X, Y)) {
    ...
    if (!GBackSolid(X, Y)) {
      ...
      if (pPlayer->Cursor)
        fixThrow = ValByPhysical(400, pPlayer->Cursor->GetPhysical()->Throw);
      // Preferred throwing direction
      int32_t iDir = +1;
      if (pPlayer->Cursor)
        if (pPlayer->Cursor->x > X) iDir = -1;
      // Throwing height
      int32_t iHeight = 20;
      if (pPlayer->Cursor) iHeight = pPlayer->Cursor->Shape.Hgt;
      // Check throw
      if (FindThrowingPosition(X, Y, fixThrow * iDir, -fixThrow, iHeight, iX,
                               iY) ||
          FindThrowingPosition(X, Y, fixThrow *(iDir *= -1), -fixThrow, iHeight,
                               iX, iY)) {
  ...
  // Vehicle
    ...
    // Check for put target
    if (ControlDown) UpdatePutTarget(TRUE);
  }
}
  ...
  if (LeftButtonDown) {
    switch (Cursor) {
      ...
      case C4MC_Cursor_Region:
        if (!Tick5)
          if (DownRegion.HoldCom) SendControl(DownRegion.HoldCom);
        break;
    }
  }
      ...
      case C4MC_Cursor_DigObject:
        if (DownTarget) {
          ...
          if (!Selection.GetLink(DownTarget)) {
            Selection.Clear();
            Selection.Add(DownTarget, C4ObjectList::stNone);
          }
        }
        break;
      ...
      case C4MC_Cursor_Region:
        ...
        if (DownRegion.id)
          if ((pDef = C4Id2Def(DownRegion.id)) && pDef->Constructable) {
            StartConstructionDrag(DownRegion.id);
            break;
          }
        break;
      // Help: no dragging
    ...
    case C4MC_Drag_None:
      ...
      switch (Cursor) {
        case C4MC_Cursor_Attack:
          ...
          break;
        case C4MC_Cursor_Grab:
          SendCommand(C4CMD_Grab, 0, 0, TargetObject);
          break;  // grab at zero-offset!
        case C4MC_Cursor_Ungrab:
        ...
        case C4MC_Cursor_Object:
        case C4MC_Cursor_DigObject:
          SendCommand(C4CMD_Get, 0, 0, TargetObject);
          break;
        case C4MC_Cursor_Dig:
...
void C4MouseControl::CreateDragImage(C4ID id) {
  ...
  if (pDef->DragImagePicture)
    DragImage.Set(pDef->Graphics.GetBitmap(), pDef->PictureRect.x,
...
void C4MouseControl::LeftUpDragNone() {
  ...
  // been moved.
  if (DownCursor == C4MC_Cursor_Region && !Help && pPlayer &&
      pPlayer->ControlStyle &&
      (DownRegion.Com & (COM_Double | COM_Single)) == 0) {
    ...
    case C4MC_Cursor_Help:
      if (DownTarget) {
        if (DownTarget->Def->GetDesc())
          Caption.Format("%s: %s", DownTarget->GetName(),
                         DownTarget->Def->GetDesc());
        else
...
void C4MouseControl::ButtonUpDragSelecting() {
  ...
  if (DragSelecting == C4MC_Selecting_Crew) {
    Game.Input.Add(CID_PlrSelect, new C4ControlPlayerSelect(Player, Selection));
    Selection.Clear();
  }
...
void C4MouseControl::SendCommand(int32_t iCommand, int32_t iX, int32_t iY,
  ...
  Game.Input.Add(CID_PlrCommand,
                 new C4ControlPlayerCommand(Player, iCommand, iX, iY, pTarget,
                                            pTarget2, iData, iAddMode));
}
...
void C4MouseControl::UpdateFogOfWar() {
  ...
  if ((pPlayer->fFogOfWar && !pPlayer->FoWIsVisible(X, Y)) || X < 0 || Y < 0 ||
      X >= GBackWdt || Y >= GBackHgt) {
    FogOfWar = true;
    // allow dragging, scrolling, region selection and manipulations of objects
    // not affected by FoW
    if (!TargetRegion && !Scrolling &&
        (!TargetObject || !(TargetObject->Category & C4D_IgnoreFoW))) {
      Cursor = C4MC_Cursor_Nothing;
      ShowPointX = ShowPointY = -1;
...
const char *C4MouseControl::GetCaption() { return Caption.getData(); }

C4Object *C4MouseControl::GetTargetObject(int32_t iX, int32_t iY, DWORD &dwOCF,
                                          C4Object *pExclude) {
  // find object
  C4Object *pObj = Game.FindVisObject(ViewX, ViewY, Player, fctViewport, iX, iY,
                                      0, 0, dwOCF, pExclude);
  if (!pObj) return NULL;
  // adjust OCF
...
void C4MouseControl::ScrollView(int32_t iX, int32_t iY, int32_t ViewWdt,
                                int32_t ViewHgt) {
