...
const int32_t C4MN_DefInfoWdt = 270,  // default width of info windows
    ...
    C4MN_DlgLines =
        5,  // default number of text lines visible in a dialog window
...
#include "object/C4ObjectMenu.h"  // only needed for menu symbol constants below

void DrawMenuSymbol(int32_t iMenu, C4Facet &cgo, int32_t iOwner,
                    C4Object *cObj) {
  C4Facet ccgo;
  ...
  switch (iMenu) {
    ...
    case C4MN_Buy:
      Game.GraphicsResource.fctFlagClr.DrawClr(ccgo = cgo.GetFraction(75, 75),
                                               TRUE, dwColor);
      Game.GraphicsResource.fctWealth.Draw(
          ccgo = cgo.GetFraction(100, 50, C4FCT_Left, C4FCT_Bottom));
      Game.GraphicsResource.fctArrow.Draw(
          ccgo = cgo.GetFraction(70, 70, C4FCT_Right, C4FCT_Center), FALSE, 0);
      break;
    case C4MN_Sell:
      Game.GraphicsResource.fctFlagClr.DrawClr(ccgo = cgo.GetFraction(75, 75),
                                               TRUE, dwColor);
      Game.GraphicsResource.fctWealth.Draw(
          ccgo = cgo.GetFraction(100, 50, C4FCT_Left, C4FCT_Bottom));
      Game.GraphicsResource.fctArrow.Draw(
          ccgo = cgo.GetFraction(70, 70, C4FCT_Right, C4FCT_Center), FALSE, 1);
      break;
      /*case C4MN_Main:
...
C4MenuItem::C4MenuItem(C4Menu *pMenu, int32_t iIndex, const char *szCaption,
                       const char *szCommand, int32_t iCount, C4Object *pObject,
                       const char *szInfoCaption, C4ID idID,
                       const char *szCommand2, bool fOwnValue, int32_t iValue,
                       int32_t iStyle, bool fIsSelectable)
    : C4GUI::Element(),
      Count(iCount),
      id(idID),
      Object(pObject),
  ...
  *Caption = *Command = *Command2 = *InfoCaption = 0;
  ...
  SReplaceChar(InfoCaption, 10, ' ');
  SReplaceChar(InfoCaption, 13, '|');
  SetToolTip(InfoCaption);
  // components initialization
  if (idID) {
    C4Def *pDef = C4Id2Def(idID);
    if (pDef)
      pDef->GetComponents(&Components, NULL,
                          pMenu ? pMenu->GetParentObject() : NULL);
  }
}

C4MenuItem::~C4MenuItem() { Symbol.Clear(); }
...
void C4MenuItem::DoTextProgress(int32_t &riByVal) {
  ...
  const char *szPos =
      sText.getPtr(Min<int>(TextDisplayProgress, sText.getLength()));
  while (riByVal && *szPos) {
...
int32_t C4MenuItem::GetSymbolWidth(int32_t iForHeight) {
  ...
  if (iStyle == C4MN_Style_Context ||
      (iStyle == C4MN_Style_Dialog && Symbol.Surface))
    return Max(Symbol.Wdt * iForHeight / Max<int32_t>(Symbol.Hgt, 1),
               iForHeight);
  // Info menus
...
void C4MenuItem::DrawElement(C4FacetEx &cgo) {
  // get target pos
  C4Facet cgoOut(cgo.Surface, cgo.TargetX + rcBounds.x,
                 cgo.TargetY + rcBounds.y, rcBounds.Wdt, rcBounds.Hgt);
  // Select mark
  if (iStyle != C4MN_Style_Info)
    if (fSelected && TextDisplayProgress)
      Application.DDraw->DrawBox(cgo.Surface, cgoOut.X, cgoOut.Y,
                                 cgoOut.X + cgoOut.Wdt - 1,
                                 cgoOut.Y + cgoOut.Hgt - 1, CRed);
  // Symbol/text areas
  ...
  // Draw item symbol:
  // Draw if there is no text progression at all (TextDisplayProgress==-1, or if
  // it's progressed far enough already (TextDisplayProgress>0)
  if (Symbol.Surface && TextDisplayProgress)
    Symbol.DrawClr(cgoItemSymbol, TRUE, dwSymbolClr);
  // Draw item text
  Application.DDraw->StorePrimaryClipper();
  Application.DDraw->SubPrimaryClipper(cgoItemText.X, cgoItemText.Y,
                                       cgoItemText.X + cgoItemText.Wdt - 1,
                                       cgoItemText.Y + cgoItemText.Hgt - 1);
  switch (iStyle) {
    case C4MN_Style_Context:
      Application.DDraw->TextOut(Caption, Game.GraphicsResource.FontRegular,
                                 1.0, cgoItemText.Surface, cgoItemText.X,
                                 cgoItemText.Y,
    ...
    case C4MN_Style_Info: {
      ...
      Game.GraphicsResource.FontRegular.BreakMessage(
          InfoCaption, cgoItemText.Wdt, &sText, true);
      Application.DDraw->TextOut(
          sText.getData(), Game.GraphicsResource.FontRegular, 1.0,
          cgoItemText.Surface, cgoItemText.X, cgoItemText.Y);
      break;
    }
    case C4MN_Style_Dialog: {
      ...
      // display broken text
      StdStrBuf sText;
      Game.GraphicsResource.FontRegular.BreakMessage(Caption, cgoItemText.Wdt,
                                                     &sText, true);
      Application.DDraw->TextOut(
          sText.getData(), Game.GraphicsResource.FontRegular, 1.0,
          cgoItemText.Surface, cgoItemText.X, cgoItemText.Y);
  ...
  if (Count != C4MN_Item_NoCount) {
    ...
    Application.DDraw->TextOut(szCount, Game.GraphicsResource.FontRegular, 1.0,
                               cgoItemText.Surface,
                               cgoItemText.X + cgoItemText.Wdt - 1,
                               cgoItemText.Y + cgoItemText.Hgt - 1 -
...
void C4MenuItem::MouseInput(C4GUI::CMouse &rMouse, int32_t iButton, int32_t iX,
                            int32_t iY, DWORD dwKeyParam) {
  // clicky clicky!
  if (iButton == C4MC_Button_LeftDown) {
    // button down: Init drag only; Enter selection only by button up
    if (IsDragElement()) StartDragging(rMouse, iX, iY, dwKeyParam);
...
void C4MenuItem::DoDragging(C4GUI::CMouse &rMouse, int32_t iX, int32_t iY,
                            DWORD dwKeyParam) {
...
void C4MenuItem::StopDragging(C4GUI::CMouse &rMouse, int32_t iX, int32_t iY,
                              DWORD dwKeyParam) {
...
C4Menu::C4Menu()
    : C4GUI::Dialog(100, 100, NULL, true)  // will be re-adjusted later
{
  Default();
...
bool C4Menu::DoInit(C4FacetExSurface &fctSymbol, const char *szEmpty,
                    int32_t iExtra, int32_t iExtraData, int32_t iId,
                    int32_t iStyle) {
  Clear();
  Default();
...
bool C4Menu::DoInitRefSym(const C4FacetEx &fctSymbol, const char *szEmpty,
                          int32_t iExtra, int32_t iExtraData, int32_t iId,
                          int32_t iStyle) {
  Clear();
  Default();
...
bool C4Menu::InitMenu(const char *szEmpty, int32_t iExtra, int32_t iExtraData,
                      int32_t iId, int32_t iStyle) {
  SCopy(szEmpty, Caption, C4MaxTitle);
  Extra = iExtra;
  ExtraData = iExtraData;
  Identification = iId;
  if (*Caption || iStyle == C4MN_Style_Dialog)
    SetTitle(Caption, HasMouse());
  else
    SetTitle(" ", HasMouse());
  if (pTitle) pTitle->SetIcon(Symbol);
  Style = iStyle & C4MN_Style_BaseMask;
  // Menus are synchronous to allow COM_MenuUp/Down to be converted to movements
  // at the clients
  if (Style == C4MN_Style_Normal)
    Columns = 5;
  else
    // in reality, Dialog menus may have two coloumns (first for the portrait)
    // however, they are not uniformly spaced and stuff; so they are better just
    // ignored and handled by the drawing routine
    Columns = 1;
...
BOOL C4Menu::AddRefSym(const char *szCaption, const C4FacetEx &fctSymbol,
                       const char *szCommand, int32_t iCount, C4Object *pObject,
                       const char *szInfoCaption, C4ID idID,
                       const char *szCommand2, bool fOwnValue, int32_t iValue,
                       bool fIsSelectable) {
  ...
  C4MenuItem *pNew = new C4MenuItem(
      this, ItemCount, szCaption, szCommand, iCount, pObject, szInfoCaption,
      idID, szCommand2, fOwnValue, iValue, Style, fIsSelectable);
  // Ref Symbol
  pNew->RefSymbol(fctSymbol);
  // Add
  return AddItem(pNew, szCaption, szCommand, iCount, pObject, szInfoCaption,
                 idID, szCommand2, fOwnValue, iValue, fIsSelectable);
}

BOOL C4Menu::Add(const char *szCaption, C4FacetExSurface &fctSymbol,
                 const char *szCommand, int32_t iCount, C4Object *pObject,
                 const char *szInfoCaption, C4ID idID, const char *szCommand2,
                 bool fOwnValue, int32_t iValue, bool fIsSelectable) {
  if (!IsActive()) return FALSE;
  // Create new menu item
  C4MenuItem *pNew = new C4MenuItem(
      this, ItemCount, szCaption, szCommand, iCount, pObject, szInfoCaption,
...
BOOL C4Menu::AddItem(C4MenuItem *pNew, const char *szCaption,
                     const char *szCommand, int32_t iCount, C4Object *pObject,
                     const char *szInfoCaption, C4ID idID,
                     const char *szCommand2, bool fOwnValue, int32_t iValue,
                     bool fIsSelectable) {
#ifdef DEBUGREC_MENU
  if (pObject) {
    C4RCMenuAdd rc = {pObject ? pObject->Number : -1, iCount, idID,
                      fOwnValue,                      iValue, fIsSelectable};
  ...
  if (!ItemCount && (!szCaption || !*szCaption)) fHasPortrait = true;
  // Item count
  ItemCount++;
  // set new item size
  if (!pClientWindow->IsFrozen()) UpdateElementPositions();
  // Init selection if not frozen
  if (Selection == -1 && fIsSelectable && !pClientWindow->IsFrozen())
    SetSelection(ItemCount - 1, false, false);
  // initial progress
  ...
  switch (byCom) {
    ...
    case COM_MenuUp:
      ...
      if (Selection + iData < 0)
        while (Selection + iData + Columns < ItemCount) iData += Columns;
    ...
    case COM_MenuDown:
      ...
      if (Selection + iData >= ItemCount)
        while (Selection + iData - Columns >= 0) iData -= Columns;
      MoveSelection(iData, true, true);
      break;
    case COM_MenuSelect:
      if (ItemCount)
        SetSelection(iData & (~C4MN_AdjustPosition),
                     !!(iData & C4MN_AdjustPosition), true);
      break;
...
BOOL C4Menu::IsActive() { return fActive; }
...
int32_t C4Menu::GetItemCount() { return ItemCount; }
...
BOOL C4Menu::SetSelection(int32_t iSelection, bool fAdjustPosition,
                          bool fDoCalls) {
  // Not active
  if (!IsActive()) return FALSE;
...
C4MenuItem *C4Menu::GetSelectedItem() { return GetItem(Selection); }
...
void C4Menu::AdjustPosition() {
  ...
  if ((Lines > 1) && Columns) {
    C4MenuItem *pSel = GetSelectedItem();
    if (pSel)
      pClientWindow->ScrollRangeInView(pSel->GetBounds().y,
                                       pSel->GetBounds().Hgt);
  }
}

int32_t C4Menu::GetSelection() { return Selection; }

BOOL C4Menu::SetPosition(int32_t iPosition) {
  if (!IsActive()) return FALSE;
  // update scroll pos, if location is initialized
  if (IsVisible() && pClientWindow)
    pClientWindow->SetScroll((iPosition / Columns) * ItemHeight);
  return TRUE;
}

int32_t C4Menu::GetIdentification() { return Identification; }
...
void C4Menu::InitLocation(C4Facet &cgoArea) {
  ...
  switch (Style) {
    case C4MN_Style_Normal:
      ...
      break;
    case C4MN_Style_Context: {
      ItemHeight = Max<int32_t>(
          C4MN_SymbolSize, Game.GraphicsResource.FontRegular.GetLineHeight());
      int32_t iWdt, iHgt;
      Game.GraphicsResource.FontRegular.GetTextExtent(Caption, ItemWidth, iHgt,
                                                      true);
      // FIXME: Blah. This stuff should be calculated correctly by pTitle.
      ItemWidth += ItemHeight + 16;
      C4MenuItem *pItem;
      for (int i = 0; pItem = GetItem(i); ++i) {
        Game.GraphicsResource.FontRegular.GetTextExtent(pItem->Caption, iWdt,
                                                        iHgt, true);
        ItemWidth = Max(ItemWidth, iWdt + pItem->GetSymbolWidth(ItemHeight));
      }
      ItemWidth += 3;  // Add some extra space so text doesn't touch right
                       // border frame...
      break;
    }
    case C4MN_Style_Info: {
      // calculate size from a default size determined by a window width of
      // C4MN_DefInfoWdt
      int32_t iWdt, iHgt, iLargestTextWdt;
      Game.GraphicsResource.FontRegular.GetTextExtent(Caption, iWdt, iHgt,
                                                      true);
      iLargestTextWdt = iWdt + 2 * C4MN_SymbolSize + C4MN_FrameWidth;
      ItemWidth = Min<int>(cgoArea.Wdt - 2 * C4MN_FrameWidth,
                           Max(iLargestTextWdt, C4MN_DefInfoWdt));
      ItemHeight = 0;
      StdStrBuf sText;
      C4MenuItem *pItem;
      for (int32_t i = 0; pItem = GetItem(i); ++i) {
        Game.GraphicsResource.FontRegular.BreakMessage(pItem->InfoCaption,
                                                       ItemWidth, &sText, true);
        Game.GraphicsResource.FontRegular.GetTextExtent(sText.getData(), iWdt,
                                                        iHgt, true);
      ...
      // although width calculation is done from C4MN_DefInfoWdt, this may be
      // too large for some very tiny info windows
    ...
    case C4MN_Style_Dialog: {
      ...
      Game.GraphicsResource.FontRegular.GetTextExtent(Caption, iWdt, iHgt,
                                                      true);
      ItemWidth = Min<int>(
          cgoArea.Wdt - 2 * C4MN_FrameWidth,
          Max<int>(iWdt + 2 * C4MN_SymbolSize + C4MN_FrameWidth, C4MN_DlgWdt));
      ...
      if (HasPortrait()) {
        ...
        if (ItemWidth > C4MN_DlgPortraitWdt * 2 && cgoArea.Hgt > cgoArea.Wdt)
          ItemWidth = Max<int>(
              ItemWidth - C4MN_DlgPortraitWdt - C4MN_DlgPortraitIndent, 40);
      }
    }
  }
  ...
  if (Style == C4MN_Style_Dialog)
    ...
    Lines = DisplayedItemCount / Columns +
            Min<int32_t>(DisplayedItemCount % Columns, 1);
  // adjust by max. height
  Lines = Max<int32_t>(
      Min<int32_t>((cgoArea.Hgt - 100) / Max<int32_t>(ItemHeight, 1), Lines),
      1);
  ...
  if (Alignment & C4MN_Align_Right)
    X = cgoArea.Wdt - 2 * C4SymbolSize - rcBounds.Wdt;
  if (Alignment & C4MN_Align_Top) Y = C4SymbolSize;
  if (Alignment & C4MN_Align_Bottom)
    Y = cgoArea.Hgt - C4SymbolSize - rcBounds.Hgt;
  if (Alignment & C4MN_Align_Free) {
    X = BoundBy<int32_t>(X, 0, cgoArea.Wdt - rcBounds.Wdt);
    Y = BoundBy<int32_t>(Y, 0, cgoArea.Hgt - rcBounds.Hgt);
  }
  // Centered (due to small viewport size)
  if (rcBounds.Wdt > cgoArea.Wdt - 2 * C4SymbolSize)
    X = (cgoArea.Wdt - rcBounds.Wdt) / 2;
  if (rcBounds.Hgt > cgoArea.Hgt - 2 * C4SymbolSize)
    Y = (cgoArea.Hgt - rcBounds.Hgt) / 2;
  SetPos(X, Y);
...
void C4Menu::InitSize() {
  ...
  if (pLast && Style == C4MN_Style_Dialog) {
    Height = Max<int>(Height, pLast->GetBounds().y + pLast->GetBounds().Hgt +
                                  C4MN_DlgLineMargin);
    fBarNeeded = false;
  } else
    fBarNeeded = pLast &&
                 pLast->GetBounds().y + pLast->GetBounds().Hgt >
                     pClientWindow->GetBounds().Hgt;
  // add dlg margins
  Width += GetMarginLeft() + GetMarginRight() + pClientWindow->GetMarginLeft() +
           pClientWindow->GetMarginRight();
  Height += GetMarginTop() + GetMarginBottom() + pClientWindow->GetMarginTop() +
            pClientWindow->GetMarginBottom();
...
void C4Menu::Draw(C4FacetEx &cgo) {
  ...
  if (TimeOnSelection >= C4MN_InfoCaption_Delay)
    if (Style != C4MN_Style_Info)  // No tooltips in info menus - doesn't make
                                   // any sense...
...
void C4Menu::DrawElement(C4FacetEx &cgo) {
  ...
  //	Application.DDraw->SubPrimaryClipper(rcBounds.x, rcBounds.y,
  //rcBounds.x+rcBounds.Wdt-1, rcBounds.y+rcBounds.Hgt-1);

  C4Facet cgoExtra(
      cgo.Surface, cgo.TargetX + rcBounds.x + 1,
      cgo.TargetY + rcBounds.y + rcBounds.Hgt - C4MN_SymbolSize - 1,
      rcBounds.Wdt - 2, C4MN_SymbolSize);

  // Draw bar divider
  if (Extra || DrawMenuControls) {
    DrawFrame(cgoExtra.Surface, cgoExtra.X - 1, cgoExtra.Y - 1,
              cgoExtra.Wdt + 1, cgoExtra.Hgt + 1);
  }
  ...
  if (DrawMenuControls) {
    ...
    if (Style != C4MN_Style_Info) {
      ...
      DrawCommandKey(
          cgoControl, COM_Throw, FALSE,
          PlrControlKeyName(iPlayer, Com2Control(COM_Throw), true).getData());
      cgoControl = cgoExtra.TruncateSection(C4FCT_Left);
      GfxR->fctOKCancel.Draw(cgoControl, TRUE, 0, 0);
      // Enter-all on Special2
      if (pItem && pItem->Command2[0]) {
        cgoControl = cgoExtra.TruncateSection(C4FCT_Left);
        DrawCommandKey(cgoControl, COM_Special2, FALSE,
                       PlrControlKeyName(iPlayer, Com2Control(COM_Special2),
                                         true).getData());
        cgoControl = cgoExtra.TruncateSection(C4FCT_Left);
    ...
    cgoControl = cgoExtra.TruncateSection(C4FCT_Left);
    DrawCommandKey(
        cgoControl, COM_Dig, FALSE,
        PlrControlKeyName(iPlayer, Com2Control(COM_Dig), true).getData());
    cgoControl = cgoExtra.TruncateSection(C4FCT_Left);
    // Close command contains "Exit"? Show an exit symbol in the status bar.
    if (SSearch(CloseCommand.getData(), "\"Exit\""))
      GfxR->fctExit.Draw(cgoControl);
    else
      GfxR->fctOKCancel.Draw(cgoControl, TRUE, 1, 0);
  }

  // live max magic
  int32_t iUseExtraData = 0;
  if (Extra == C4MN_Extra_LiveMagicValue ||
      Extra == C4MN_Extra_ComponentsLiveMagic) {
    C4Object *pMagicSourceObj = Game.Objects.SafeObjectPointer(ExtraData);
    if (pMagicSourceObj)
      iUseExtraData = pMagicSourceObj->MagicEnergy / MagicPhysicalFactor;
  } else {
  ...
  switch (Extra) {
    case C4MN_Extra_Components:
      if (pItem)
        pItem->Components.Draw(cgoExtra, -1, Game.Defs, C4D_All, TRUE,
                               C4FCT_Right | C4FCT_Triple | C4FCT_Half);
      break;
    case C4MN_Extra_Value: {
      if (pDef)
        Game.GraphicsResource.fctWealth.DrawValue(cgoExtra, iValue, 0, 0,
                                                  C4FCT_Right);
    ...
    case C4MN_Extra_LiveMagicValue:
      if (pDef) {
        Game.GraphicsResource.fctMagic.DrawValue2(
            cgoExtra, iValue, iUseExtraData, 0, 0, C4FCT_Right);
      }
      break;
...
void C4Menu::DrawFrame(SURFACE sfcSurface, int32_t iX, int32_t iY, int32_t iWdt,
                       int32_t iHgt) {
...
void C4Menu::SetAlignment(int32_t iAlignment) { Alignment = iAlignment; }
...
void C4Menu::Execute() {
  ...
  if (!Game.iTick35 || NeedRefill)
    if (!RefillInternal()) Close(false);
  // text progress
  if (fTextProgressing /*&& !Game.iTick2*/) SetTextProgress(+1, true);
}
...
BOOL C4Menu::ConvertCom(int32_t &rCom, int32_t &rData, bool fAsyncConversion) {
  // This function converts normal Coms to menu Coms before they are send to the
  // queue
  ...
  // Note: conversion to COM_MenuShowText is not synchronized because text
  // lengths may vary
  // between clients. The above switch is used to determine whether the com was
  // a menu com
  if (fTextProgressing && fAsyncConversion) rCom = COM_MenuShowText;
...
C4Viewport *C4Menu::GetViewport() {
  // ask all viewports
  for (C4Viewport *pVP = Game.GraphicsSystem.GetFirstViewport(); pVP;
       pVP = pVP->GetNext())
    if (pVP->IsViewportMenu(this)) return pVP;
  // none matching
  return NULL;
}
...
void C4Menu::UpdateElementPositions() {
  ...
  if (HasPortrait() && pCurr) {
    ...
    C4Rect rcPortraitBounds(
        0, 0, C4MN_DlgPortraitWdt + C4MN_DlgPortraitIndent,
        fctPortrait.Hgt * C4MN_DlgPortraitWdt / Max<int>(fctPortrait.Wdt, 1));
  ...
  int32_t iIndex = 0;
  C4Rect rcNewBounds(0, 0, ItemWidth, ItemHeight);
  ...
  while (pCurr = pNext) {
    ...
    if (Style == C4MN_Style_Dialog) {
      ...
      if (!pPrev || (!pPrev->IsSelectable || !pCurr->IsSelectable))
        yOff += C4MN_DlgLineMargin;
      else
        yOff += C4MN_DlgOptionLineMargin;
      // determine item height.
      StdStrBuf sText;
      int32_t iAssumedItemHeight =
          Game.GraphicsResource.FontRegular.GetLineHeight();
      int32_t iWdt, iAvailWdt = ItemWidth, iSymWdt;
      for (;;) {
        iSymWdt = Min<int32_t>(pCurr->GetSymbolWidth(iAssumedItemHeight),
                               iAvailWdt / 2);
        iAvailWdt = ItemWidth - iSymWdt;
        Game.GraphicsResource.FontRegular.BreakMessage(pCurr->Caption,
                                                       iAvailWdt, &sText, true);
        Game.GraphicsResource.FontRegular.GetTextExtent(sText.getData(), iWdt,
                                                        rcNewBounds.Hgt, true);
        if (!iSymWdt || rcNewBounds.Hgt <= iAssumedItemHeight) break;
        // If there is a symbol, the symbol grows as more lines become available
        // Thus, less space is available for the text, and it might become
        // larger
...
void C4Menu::ClearPointers(C4Object *pObj) {
  C4MenuItem *pItem;
  for (int32_t i = 0; pItem = GetItem(i); ++i)
    if (pItem->GetObject() == pObj) pItem->ClearObject();
}
