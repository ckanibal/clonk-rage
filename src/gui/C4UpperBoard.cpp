...
C4UpperBoard::~C4UpperBoard() { Clear(); }
...
void C4UpperBoard::Execute() {
  ...
  sprintf(cTimeString, "%02d:%02d:%02d", Game.Time / 3600,
          (Game.Time % 3600) / 60, Game.Time % 60);
  time_t t = time(0);
  strftime(cTimeString2, sizeof(cTimeString2), "[%H:%M:%S]", localtime(&t));
  Draw(Output);
}
...
void C4UpperBoard::Draw(C4Facet &cgo) {
  ...
  Application.DDraw->BlitSurfaceTile(
      Game.GraphicsResource.fctUpperBoard.Surface, Output.Surface, 0, 0,
      Output.Wdt, Output.Hgt);
  // Logo
  C4Facet cgo2;
  float fLogoZoom = 0.75f;
  cgo2.Set(cgo.Surface,
           (int32_t)(cgo.Wdt / 2 -
                     (Game.GraphicsResource.fctLogo.Wdt / 2) * fLogoZoom),
           0, (int32_t)(Game.GraphicsResource.fctLogo.Wdt * fLogoZoom),
           (int32_t)(Game.GraphicsResource.fctLogo.Hgt * fLogoZoom));
  Game.GraphicsResource.fctLogo.Draw(cgo2);
  // Right text sections
  int32_t iRightOff = 1;
  // Playing time
  Application.DDraw->TextOut(
      cTimeString, Game.GraphicsResource.FontRegular, 1.0, cgo.Surface,
      Config.Graphics.ResX - (iRightOff++) * TextWidth - 10, TextYPosition,
      0xFFFFFFFF);
  // Clock
  if (Config.Graphics.ShowClock)
    Application.DDraw->TextOut(
        cTimeString2, Game.GraphicsResource.FontRegular, 1.0, cgo.Surface,
        Config.Graphics.ResX - (iRightOff++) * TextWidth - 30, TextYPosition,
        0xFFFFFFFF);
  // FPS
  ...
  // Scenario title
  Application.DDraw->TextOut(
      Game.ScenarioTitle.getData(), Game.GraphicsResource.FontRegular, 1.0,
      cgo.Surface, 10,
      cgo.Hgt / 2 - Game.GraphicsResource.FontRegular.GetLineHeight() / 2,
      0xFFFFFFFF);
}
