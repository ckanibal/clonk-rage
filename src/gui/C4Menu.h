...
class C4MenuItem : public C4GUI::Element {
 ...
 protected:
  ...
  int32_t iIndex;
  bool IsSelectable;
  int32_t TextDisplayProgress;  // dialog menus only: Amount of text which is to
                                // be displayed already (-1 for everything)
  C4IDList Components;  // components to be displayed in info line if item is
                        // selected
 ...
 protected:
  ...
  // ctor
  C4MenuItem(C4Menu *pMenu, int32_t iIndex, const char *szCaption,
             ...
             bool fIsSelectable);
  void GrabSymbol(C4FacetExSurface &fctSymbol) {
    Symbol.GrabFrom(fctSymbol);
    if (Symbol.Surface) dwSymbolClr = Symbol.Surface->GetClr();
  }
  void RefSymbol(const C4FacetEx &fctSymbol) {
    Symbol.Set(fctSymbol);
    if (Symbol.Surface) dwSymbolClr = Symbol.Surface->GetClr();
  }
  void SetSelected(bool fToVal) { fSelected = fToVal; }
  void DoTextProgress(
      int32_t &riByVal);  // progress number of shown characters by given amount

  // GUI calls
  virtual void MouseInput(
      class C4GUI::CMouse &rMouse, int32_t iButton, int32_t iX, int32_t iY,
      DWORD dwKeyParam);  // input: mouse movement or buttons
  virtual void MouseEnter(class C4GUI::CMouse &rMouse);  // called when mouse
                                                         // cursor enters
                                                         // element region:
                                                         // Select this item
                                                         // (deselects any
  ...
  virtual void DoDragging(
      class C4GUI::CMouse &rMouse, int32_t iX, int32_t iY,
      DWORD dwKeyParam);  // called by mouse: dragging process
  virtual void StopDragging(class C4GUI::CMouse &rMouse, int32_t iX, int32_t iY,
...
class C4Menu : public C4GUI::Dialog {
 ...
 protected:
  ...
  bool fHasPortrait;      // if set, first menu item is used at a portrait at
                          // topleft of menu
  bool fTextProgressing;  // if true, text is being shown progressively (dialog
                          // menus)
  bool fEqualIconItemHeight;  // for dialog menus only: If set, all options with
                              // an icon are forced to have the same height
  bool fActive;  // set if menu is shown - independant of GUI to keep
                 // synchronized when there's no GUI
 public:
  ...
  int GetSymbolSize() {
    return (Style == C4MN_Style_Dialog) ? 64 : C4SymbolSize;
  }
  ...
  BOOL AddRefSym(const char *szCaption, const C4FacetEx &fctSymbol,
                 const char *szCommand, int32_t iCount = C4MN_Item_NoCount,
                 C4Object *pObject = NULL, const char *szInfoCaption = NULL,
                 C4ID idID = C4ID_None, const char *szCommand2 = NULL,
                 bool fOwnValue = false, int32_t iValue = 0,
                 bool fIsSelectable = true);
  BOOL Add(const char *szCaption, C4FacetExSurface &fctSymbol,
           const char *szCommand, int32_t iCount = C4MN_Item_NoCount,
           C4Object *pObject = NULL, const char *szInfoCaption = NULL,
           C4ID idID = C4ID_None, const char *szCommand2 = NULL,
           bool fOwnValue = false, int32_t iValue = 0,
  ...
  BOOL SetLocation(int32_t iX,
                   int32_t iY);  // set location relative to user viewport
  bool SetTextProgress(int32_t iToProgress, bool fAdd);  // enable/disable
                                                         // progressive text
                                                         // display and set
                                                         // starting pos
  void SetEqualItemHeight(bool fToVal) {
    fEqualIconItemHeight = fToVal;
  }  // enable/disable equal item heights
 ...
 private:
  BOOL AddItem(C4MenuItem *pNew, const char *szCaption, const char *szCommand,
               ...
               int32_t iValue, bool fIsSelectable);
  bool InitMenu(const char *szEmpty, int32_t iExtra, int32_t iExtraData,
                int32_t iId, int32_t iStyle);

 protected:
  bool DoInitRefSym(const C4FacetEx &fctSymbol, const char *szEmpty,
                    int32_t iExtra = C4MN_Extra_None, int32_t iExtraData = 0,
                    int32_t iId = 0, int32_t iStyle = C4MN_Style_Normal);
  bool DoInit(C4FacetExSurface &fctSymbol, const char *szEmpty,
              int32_t iExtra = C4MN_Extra_None, int32_t iExtraData = 0,
              int32_t iId = 0, int32_t iStyle = C4MN_Style_Normal);
  ...
  void DrawScrollbar(C4Facet &cgo, int32_t iTotal, int32_t iVisible,
                     int32_t iPosition);
  void DrawFrame(C4Facet &cgo, int32_t iAlign);
  void DrawFrame(SURFACE sfcSurface, int32_t iX, int32_t iY, int32_t iWdt,
                 int32_t iHgt);
  void InitLocation(C4Facet &cgo);
  void InitSize();
  void UpdateScrollBar();  // call InitSize if a scroll bar is needed but not
                           // present or vice vera
  void UserSelectItem(
      int32_t Player,
      C4MenuItem *pItem);  // select item (direct) or do control (object menus)
  void UserEnter(int32_t Player, C4MenuItem *pItem,
                 bool fRight);  // enter on an item
  ...
  virtual bool IsReadOnly() {
    return false;
  }  // determine whether the menu is just viewed by an observer, and should not
     // issue any calls
  virtual int32_t GetControllingPlayer() { return NO_OWNER; }

  virtual const char *GetID() {
    return 0;
  }  // no ID needed, because it's a viewport dlg

  bool HasPortrait() {
    return fHasPortrait;
  }  // dialog menus only: Whether a portrait is shown in the topleft
 ...
 protected:
  ...
  virtual bool IsExternalDrawDialog() {
    return true;
  }  // drawn by viewport drawing proc
  ...
  virtual void DrawElement(C4FacetEx &cgo);  // draw menu
  ...
  virtual int32_t GetMarginBottom() {
    return ((Extra || DrawMenuControls) ? C4MN_SymbolSize : 0) +
           C4MN_FrameWidth + BaseClass::GetMarginBottom();
  }
  virtual int32_t GetMarginLeft() {
    return C4MN_FrameWidth + BaseClass::GetMarginLeft();
  }
  virtual int32_t GetMarginRight() {
    return C4MN_FrameWidth + BaseClass::GetMarginRight();
  }
