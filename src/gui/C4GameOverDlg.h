...
class C4GoalDisplay : public C4GUI::Window {
 ...
 public:
  C4GoalDisplay(const C4Rect &rcBounds) : C4GUI::Window() {
    SetBounds(rcBounds);
  }
  virtual ~C4GoalDisplay() {}

  void SetGoals(const C4IDList &rAllGoals, const C4IDList &rFulfilledGoals,
...
class C4GameOverDlg : public C4GUI::Dialog {
 private:
  ...
  C4Sec1TimerCallback<C4GameOverDlg> *
      pSec1Timer;  // engine timer hook for player list update
  int32_t iPlrListCount;
  class C4PlayerInfoListBox **ppPlayerLists;
  C4GoalDisplay *pGoalDisplay;
  C4GUI::Label *
      pNetResultLabel;  // label showing league result, disconnect, etc.
  C4GUI::Button *pBtnExit, *pBtnContinue;
  bool fIsNetDone;  // set if league is evaluated and round results arrived
  bool fIsQuitBtnVisible;      // quit button available? set if not host or when
                               // fIsNetDone
  bool fHasNextMissionButton;  // continue button replaced by "next
                               // mission"-button?
 ...
 private:
  ...
  void Update();
  void SetNetResult(const char *szResultString,
                    C4RoundResults::NetResult eResultType,
                    size_t iPendingStreamingData, bool fIsStreaming);
 ...
 protected:
  ...
  virtual bool OnEnter() {
    if (fIsQuitBtnVisible) OnExitBtn(NULL);
    return true;
  }  // enter on non-button: Always quit
  virtual bool OnEscape() {
    if (fIsQuitBtnVisible) UserClose(false);
    return true;
  }  // escape ignored if still streaming
