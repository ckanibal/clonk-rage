...
StdStrBuf C4PacketCountdown::GetCountdownMsg(bool fInitialMsg) const {
  ...
  if (iCountdown < AlmostStartCountdownTime && !fInitialMsg)
    ...
    szCountdownMsg = LoadResStr("IDS_PRC_COUNTDOWN");
  return FormatString(szCountdownMsg, (int)iCountdown);
}

// ----------- ScenDescs ---------------------------------------------

ScenDesc::ScenDesc(const C4Rect &rcBounds, bool fActive)
    : C4GUI::Window(), fDescFinished(false), pSec1Timer(NULL) {
  // build components
  // CStdFont &rTitleFont = C4GUI::GetRes()->CaptionFont;
  SetBounds(rcBounds);
  C4GUI::ComponentAligner caMain(GetClientRect(), 0, 0, true);
  // AddElement(pTitle = new C4GUI::Label("",
  // caMain.GetFromTop(rTitleFont.GetLineHeight()), ALeft, C4GUI_CaptionFontClr,
  // &rTitleFont, true));
  AddElement(pDescBox = new C4GUI::TextWindow(caMain.GetAll(), 0, 0, 0, 100,
                                              4096, "", true));
  pDescBox->SetDecoration(false, false, NULL, true);
  // pDescBox->SetToolTip(LoadResStr("IDS_MSG_SCENARIODESC")); annoying when
  // scrolling through desc...
...
void ScenDesc::Update() {
  ...
  if (pRes->isComplete()) {
    ...
    if (!ScenarioFile.Open(pRes->getFile())) {
      pDescBox->AddTextLine("scenario file load error", &rTextFont,
                            C4GUI_MessageFontClr, false, true);
    } else {
      StdStrBuf sDesc;
      // load desc
      C4ComponentHost DefDesc;
      if (DefDesc.LoadEx("Desc", ScenarioFile, C4CFN_ScenarioDesc,
      ...
      DefDesc.Close();
      if (!!sDesc)
        pDescBox->AddTextLine(sDesc.getData(), &rTextFont, C4GUI_MessageFontClr,
                              false, true, &rTitleFont);
      else
        pDescBox->AddTextLine(Game.ScenarioTitle.getData(), &rTitleFont,
                              C4GUI_CaptionFontClr, false, true);
    }
    // okay, done loading. No more updates.
    fDescFinished = true;
    Deactivate();
  } else {
    pDescBox->AddTextLine(
        FormatString(LoadResStr("IDS_MSG_SCENARIODESC_LOADING"),
                     (int)pRes->getPresentPercent()).getData(),
...
MainDlg::MainDlg(bool fHost)
    : C4GUI::FullscreenDialog(
          !Game.ScenarioTitle
              ...
              : FormatString("%s - %s", Game.ScenarioTitle.getData(),
                             LoadResStr("IDS_DLG_LOBBY")).getData(),
  ...
  // key bindings
  pKeyHistoryUp =
      new C4KeyBinding(C4KeyCodeEx(K_UP), "LobbyChatHistoryUp", KEYSCOPE_Gui,
                       new C4GUI::DlgKeyCBEx<MainDlg, bool>(
                           *this, true, &MainDlg::KeyHistoryUpDown),
                       C4CustomKey::PRIO_CtrlOverride);
  pKeyHistoryDown = new C4KeyBinding(
      C4KeyCodeEx(K_DOWN), "LobbyChatHistoryDown", KEYSCOPE_Gui,
      new C4GUI::DlgKeyCBEx<MainDlg, bool>(*this, false,
                                           &MainDlg::KeyHistoryUpDown),
      C4CustomKey::PRIO_CtrlOverride);
  // timer
  ...
  if (pSubTitle) pSubTitle->SetToolTip(LoadResStr("IDS_DLG_SCENARIOTITLE"));
  C4GUI::Label *pLbl;
  ...
  C4GUI::ComponentAligner caBottom(
      caMain.GetFromBottom(iDefBtnHeight + iIndentY1 * 2), iIndentX1,
      iIndentY1);
  // add buttons
  C4GUI::CallbackButton<MainDlg> *btnExit;
  btnExit = new C4GUI::CallbackButton<MainDlg>(LoadResStr("IDS_DLG_EXIT"),
                                               caBottom.GetFromLeft(100),
                                               &MainDlg::OnExitBtn);
  if (fHost)
    btnRun = new C4GUI::CallbackButton<MainDlg>(LoadResStr("IDS_DLG_GAMEGO"),
                                                caBottom.GetFromRight(100),
                                                &MainDlg::OnRunBtn);
  else
    // 2do: Ready-checkbox
    caBottom.GetFromRight(90);
  // C4GUI::CallbackButton<MainDlg> *btnTest = new
  // C4GUI::CallbackButton<MainDlg>("test", caBottom.GetFromRight(90),
  // &MainDlg::OnTestBtn);
  pGameOptionButtons = new C4GameOptionButtons(
      caBottom.GetCentered(caBottom.GetInnerWidth(),
                           Min<int32_t>(C4GUI_IconExHgt, caBottom.GetHeight())),
      true, fHost, true);

  // players / ressources sidebar
  C4GUI::ComponentAligner caRight(caMain.GetFromRight(iClientListWdt),
                                  iIndentX3, iIndentY4);
  pRightTabLbl = new C4GUI::WoodenLabel(
      "", caRight.GetFromTop(C4GUI::WoodenLabel::GetDefaultHeight(
              &(C4GUI::GetRes()->TextFont))),
      C4GUI_CaptionFontClr, &C4GUI::GetRes()->TextFont, ALeft);
  caRight.ExpandTop(
      iIndentY4 * 2 +
      1);  // undo margin, so client list is located directly under label
  pRightTab = new C4GUI::Tabular(caRight.GetAll(), C4GUI::Tabular::tbNone);
  C4GUI::Tabular::Sheet *pPlayerSheet =
      pRightTab->AddSheet(LoadResStr("IDS_DLG_PLAYERS"));
  C4GUI::Tabular::Sheet *pResSheet =
      pRightTab->AddSheet(LoadResStr("IDS_DLG_RESOURCES"));
  C4GUI::Tabular::Sheet *pOptionsSheet =
      pRightTab->AddSheet(LoadResStr("IDS_DLG_OPTIONS"));
  C4GUI::Tabular::Sheet *pScenarioSheet =
      pRightTab->AddSheet(LoadResStr("IDS_DLG_SCENARIO"));
  pPlayerList =
      new C4PlayerInfoListBox(pPlayerSheet->GetContainedClientRect(),
                              C4PlayerInfoListBox::PILBM_LobbyClientSort);
  ...
  pOptionsList =
      new C4GameOptionsList(pResSheet->GetContainedClientRect(), false, false);
  ...
  pRightTabLbl->SetContextHandler(
      new C4GUI::CBContextHandler<C4GameLobby::MainDlg>(
          this, &MainDlg::OnRightTabContext));
  ...
  // btnAddPlayer = new C4GUI::CallbackButton<MainDlg,
  // C4GUI::IconButton>(C4GUI::Ico_AddPlr,
  // pRightTabLbl->GetToprightCornerRect(16,16,4,4,2+fHasTeams),
  // pPlayerSheet->GetHotkey(), &MainDlg::OnTabTeams);
  if (fHasTeams)
    btnTeams = new C4GUI::CallbackButton<MainDlg, C4GUI::IconButton>(
        C4GUI::Ico_Team,
        pRightTabLbl->GetToprightCornerRect(16, 16, 4, 4, --iBtnNum),
        pPlayerSheet->GetHotkey(), &MainDlg::OnTabTeams);
  btnPlayers = new C4GUI::CallbackButton<MainDlg, C4GUI::IconButton>(
      C4GUI::Ico_Player,
      pRightTabLbl->GetToprightCornerRect(16, 16, 4, 4, --iBtnNum),
      pPlayerSheet->GetHotkey(), &MainDlg::OnTabPlayers);
  btnResources = new C4GUI::CallbackButton<MainDlg, C4GUI::IconButton>(
      C4GUI::Ico_Resource,
      pRightTabLbl->GetToprightCornerRect(16, 16, 4, 4, --iBtnNum),
      pResSheet->GetHotkey(), &MainDlg::OnTabRes);
  btnOptions = new C4GUI::CallbackButton<MainDlg, C4GUI::IconButton>(
      C4GUI::Ico_Options,
      pRightTabLbl->GetToprightCornerRect(16, 16, 4, 4, --iBtnNum),
      pOptionsSheet->GetHotkey(), &MainDlg::OnTabOptions);
  btnScenario = new C4GUI::CallbackButton<MainDlg, C4GUI::IconButton>(
      C4GUI::Ico_Gfx,
      pRightTabLbl->GetToprightCornerRect(16, 16, 4, 4, --iBtnNum),
      pOptionsSheet->GetHotkey(), &MainDlg::OnTabScenario);
  if (fHasChat)
    btnChat = new C4GUI::CallbackButton<MainDlg, C4GUI::IconButton>(
        C4GUI::Ico_Ex_Chat,
        pRightTabLbl->GetToprightCornerRect(16, 16, 4, 4, --iBtnNum),
        0 /* 2do*/, &MainDlg::OnBtnChat);
  ...
  C4GUI::ComponentAligner caChat(
      caCenter.GetFromBottom(C4GUI::Edit::GetDefaultEditHeight()), 0, 0);
  pLbl =
      new C4GUI::WoodenLabel(LoadResStr("IDS_CTL_CHAT"), caChat.GetFromLeft(40),
                             C4GUI_CaptionFontClr, &C4GUI::GetRes()->TextFont);
  pEdt = new C4GUI::CallbackEdit<MainDlg>(caChat.GetAll(), this,
                                          &MainDlg::OnChatInput);
  pEdt->SetToolTip(LoadResStr("IDS_DLGTIP_CHAT"));
  pLbl->SetToolTip(LoadResStr("IDS_DLGTIP_CHAT"));
  ...
  AddElement(pChatBox);
  AddElement(pLbl);
  AddElement(pEdt);  // chat

  AddElement(pRightTabLbl);
  //(new C4GUI::ContextButton(pRightTabLbl, true))->SetToolTip("[.!] Switches
  //between player and ressource view"); // right tab label+ctx menu
  ...
  AddElement(btnExit);
  btnExit->SetToolTip(LoadResStr("IDS_DLGTIP_EXIT"));
...
void MainDlg::OnClosed(bool fOK) {
  ...
  if (!fOK) C4GameLobby::UserAbort = true;
  // finish countdown if running
  // (may not be finished if status change packet from host is faster than the
  // countdown-initiate)
...
void MainDlg::Start(int32_t iCountdownTime) {
  // check league rules
  if (!Game.Parameters.CheckLeagueRulesStart(true)) return;
  ...
  if (Game.C4S.Head.SaveGame)
    if (Game.PlayerInfos.FindUnassociatedRestoreInfo(Game.RestorePlayerInfos)) {
      StdStrBuf sMsg;
      sMsg.Ref(LoadResStr("IDS_MSG_NOTALLSAVEGAMEPLAYERSHAVE"));
      if (!GetScreen()->ShowMessageModal(
              sMsg.getData(), LoadResStr("IDS_MSG_FREESAVEGAMEPLRS"),
              C4GUI::MessageDialog::btnYesNo, C4GUI::Ico_SavegamePlayer,
              &Config.Startup.HideMsgPlrNoTakeOver))
        return;
    }
...
C4GUI::Edit::InputResult MainDlg::OnChatInput(C4GUI::Edit *edt, bool fPasting,
                                              bool fPastingMore) {
  // get edit text
  ...
  if (!szInputText || !*szInputText) {
    ...
    // those must not be executed when text is pasted, because that could crash
    // the GUI system
    // when there are additional lines to paste, but the edit field is
    // destructed by the command
    if (!fProcessed && *szInputText == '/') {
      // must be 1 char longer than the longest command only. If given commands
      // are longer, they will be truncated, and such a command won't exist
      // anyway
      ...
      if (SEqualNoCase(Command, "/joinplr")) {
        ...
        if (!ItemExists(plrPath.getData())) {
          sprintf(OSTR, LoadResStr("IDS_MSG_CMD_JOINPLR_NOFILE"),
                  plrPath.getData());
          LobbyError(OSTR);
        } else
      ...
      // ------------------------------------------------------
        ...
        if (Game.Network.Clients.GetLocal())
          idLocalClient = Game.Network.Clients.GetLocal()->getID();
        if (iSepPos > 0) {
        ...
        if (pNfo)
          pCltNfo = Game.PlayerInfos.GetClientInfoByPlayerID(pNfo->GetID());
        if (!pCltNfo) {
          LobbyError(LoadResStr("IDS_MSG_CMD_PLRCLR_NOPLAYER"));
        } else {
          // may color of this client be set?
          if (pCltNfo->GetClientID() != idLocalClient &&
              !Game.Network.isHost()) {
            ...
            if (sscanf(szPar, "%x", &dwNewClr) != 1) {
              ...
              C4PlayerInfo *pPlrInfo =
                  LocalInfoRequest.GetPlayerInfoByID(pNfo->GetID());
              assert(pPlrInfo);
              if (pPlrInfo) {
                pPlrInfo->SetOriginalColor(
                    dwNewClr);  // set this as a new color wish
      ...
      // ------------------------------------------------------
        ...
        if (!Game.Network.isHost())
          LobbyError(LoadResStr("IDS_MSG_CMD_HOSTONLY"));
                 ...
                 (!sscanf(szPar, "%d", &iTimeout) || iTimeout < 0))
      ...
      // ------------------------------------------------------
        ...
        LogF("/start [time] - %s",
             LoadResStr("IDS_TEXT_STARTTHEROUNDWITHSPECIFIE"));
        LogF("/abort - %s", LoadResStr("IDS_TEXT_ABORTSTARTCOUNTDOWN"));
        LogF("/alert - %s", LoadResStr("IDS_TEXT_ALERTTHEHOSTIFTHEHOSTISAW"));
        LogF("/joinplr [filename] - %s",
             LoadResStr("IDS_TEXT_JOINALOCALPLAYERFROMTHESP"));
        LogF("/kick [client] - %s",
             LoadResStr("IDS_TEXT_KICKTHESPECIFIEDCLIENT"));
        LogF("/observer [client] - %s",
             LoadResStr("IDS_TEXT_SETTHESPECIFIEDCLIENTTOOB"));
        LogF("/me [action] - %s",
             LoadResStr("IDS_TEXT_PERFORMANACTIONINYOURNAME"));
        LogF("/sound [sound] - %s",
             LoadResStr("IDS_TEXT_PLAYASOUNDFROMTHEGLOBALSO"));
        LogF("/team [message] - %s",
             LoadResStr("IDS_MSG_SENDAPRIVATEMESSAGETOYOUR"));
        LogF("/plrclr [player] [RGB] - %s",
             LoadResStr("IDS_TEXT_CHANGETHECOLOROFTHESPECIF"));
        LogF("/plrclr [RGB] - %s",
             LoadResStr("IDS_TEXT_CHANGEYOUROWNPLAYERCOLOR"));
        ...
        LogF("/set maxplayer [number] - %s",
             LoadResStr("IDS_TEXT_SETANEWMAXIMUMNUMBEROFPLA"));
        LogF("/clear - %s", LoadResStr("IDS_MSG_CLEARTHEMESSAGEBOARD"));
      }
      // ------------------------------------------------------
      else {
        // command not known or not a specific lobby command - forward to
        // messageinput
        fProcessed = false;
      }
    }
    // not processed? Then forward to messageinput, which parses additional
    // commands and sends regular messages
    if (!fProcessed) Game.MessageInput.ProcessInput(szInputText);
  }
  // clear edit field after text has been processed
  pEdt->SelectAll();
  pEdt->DeleteSelection();
...
void MainDlg::OnClientConnect(C4Client *pClient,
...
void MainDlg::HandlePacket(char cStatus, const C4PacketBase *pPacket,
...
bool MainDlg::OnMessage(C4Client *pOfClient, const char *szMessage) {
  ...
  if (pChatBox && C4GUI::GetRes()) {
    pChatBox->AddTextLine(
        szMsgBuf, &C4GUI::GetRes()->TextFont,
        Game.Network.Players.GetClientChatColor(
            pOfClient ? pOfClient->getID() : Game.Clients.getLocalID(), true) |
...
void MainDlg::OnLog(const char *szLogMsg, DWORD dwClr) {
  if (pChatBox && C4GUI::GetRes()) {
    pChatBox->AddTextLine(szLogMsg, &C4GUI::GetRes()->TextFont, dwClr, true,
                          true);
    pChatBox->ScrollToBottom();
  }
}
...
void MainDlg::OnError(const char *szErrMsg) {
  if (pChatBox && C4GUI::GetRes()) {
    ...
    pChatBox->AddTextLine(szErrMsg, &C4GUI::GetRes()->TextFont,
                          C4GUI_ErrorFontClr, true, true);
    pChatBox->ScrollToBottom();
  }
}

void MainDlg::OnSec1Timer() { UpdatePlayerList(); }
...
C4GUI::ContextMenu *MainDlg::OnRightTabContext(C4GUI::Element *pLabel,
  ...
  pMenu->AddItem(
      pPlayerSheet->GetTitle(), pPlayerSheet->GetToolTip(), C4GUI::Ico_Player,
  ...
  if (Game.Teams.IsMultiTeams()) {
    ...
    pMenu->AddItem(LoadResStr("IDS_MSG_SHOWTEAMS"), strShowTeamsDesc.getData(),
                   C4GUI::Ico_Team, new C4GUI::CBMenuHandler<MainDlg>(
                                        this, &MainDlg::OnCtxTabTeams));
  }
  pMenu->AddItem(
      pResSheet->GetTitle(), pResSheet->GetToolTip(), C4GUI::Ico_Resource,
      new C4GUI::CBMenuHandler<MainDlg>(this, &MainDlg::OnCtxTabRes));
  pMenu->AddItem(pOptionsSheet->GetTitle(), pOptionsSheet->GetToolTip(),
                 C4GUI::Ico_Options, new C4GUI::CBMenuHandler<MainDlg>(
                                         this, &MainDlg::OnCtxTabOptions));
  // open it
  return pMenu;
}
...
void MainDlg::OnClientAddPlayer(const char *szFilename, int32_t idClient) {
  ...
  if (idClient != Game.Clients.getLocalID()) {
    LobbyError(FormatString(LoadResStr("IDS_ERR_JOINPLR_NOLOCALCLIENT"),
                            szFilename, idClient).getData());
    return;
  }
  // player join - check filename
  if (!ItemExists(szFilename)) {
    LobbyError(FormatString(LoadResStr("IDS_ERR_JOINPLR_NOFILE"), szFilename)
                   .getData());
    return;
  }
  ...
  // join!
  Game.Network.Players.JoinLocalPlayer(Config.AtExeRelativePath(szFilename),
                                       true);
}

void MainDlg::OnTabPlayers(C4GUI::Control *btn) {
  if (pPlayerList)
    pPlayerList->SetMode(C4PlayerInfoListBox::PILBM_LobbyClientSort);
  if (pRightTab) {
...
void MainDlg::OnTabTeams(C4GUI::Control *btn) {
  if (pPlayerList)
    pPlayerList->SetMode(C4PlayerInfoListBox::PILBM_LobbyTeamSort);
  if (pRightTab) {
    pRightTab->SelectSheet(SheetIdx_PlayerList, true);
    UpdateRightTab();
  }
...
void MainDlg::UpdateRightTab() {
  if (!pRightTabLbl || !pRightTab || !pRightTab->GetActiveSheet() ||
      !pPlayerList)
    return;
  ...
  if (pRightTab->GetActiveSheetIndex() == SheetIdx_PlayerList)
    UpdatePlayerList();
  if (pRightTab->GetActiveSheetIndex() == SheetIdx_Res)
    pResList->Activate();
  else
    pResList->Deactivate();
  if (pRightTab->GetActiveSheetIndex() == SheetIdx_Options)
    pOptionsList->Activate();
  else
    pOptionsList->Deactivate();
  if (pRightTab->GetActiveSheetIndex() == SheetIdx_Scenario)
    pScenarioInfo->Activate();
  else
    pScenarioInfo->Deactivate();
  // update selection buttons
  if (btnPlayers)
    btnPlayers->SetHighlight(
        pRightTab->GetActiveSheetIndex() == SheetIdx_PlayerList &&
        pPlayerList->GetMode() == C4PlayerInfoListBox::PILBM_LobbyClientSort);
  if (btnResources)
    btnResources->SetHighlight(pRightTab->GetActiveSheetIndex() ==
                               SheetIdx_Res);
  if (btnTeams)
    btnTeams->SetHighlight(
        pRightTab->GetActiveSheetIndex() == SheetIdx_PlayerList &&
        pPlayerList->GetMode() == C4PlayerInfoListBox::PILBM_LobbyTeamSort);
  if (btnOptions)
    btnOptions->SetHighlight(pRightTab->GetActiveSheetIndex() ==
                             SheetIdx_Options);
  if (btnScenario)
    btnScenario->SetHighlight(pRightTab->GetActiveSheetIndex() ==
                              SheetIdx_Scenario);
}
...
bool MainDlg::KeyHistoryUpDown(bool fUp) {
  ...
  pEdt->SelectAll();
  pEdt->DeleteSelection();
  const char *szPrevInput = Game.MessageInput.GetBackBuffer(
      fUp ? (++iBackBufferIndex) : (--iBackBufferIndex));
...
void MainDlg::UpdateFairCrew() {
  // if the fair crew setting has changed, make sure the buttons reflect this
  // change
  pGameOptionButtons->UpdateFairCrewBtn();
}

void MainDlg::UpdatePassword() {
  // if the password setting has changed, make sure the buttons reflect this
  // change
...
void MainDlg::ClearLog() { pChatBox->ClearText(true); }
...
Countdown::Countdown(int32_t iStartTimer)
    : iStartTimer(iStartTimer), pSec1Timer(NULL) {
  // only on network hosts
  assert(Game.Network.isHost());
  // ctor: Init; sends initial countdown packet
  C4PacketCountdown pck(iStartTimer);
  Game.Network.Clients.BroadcastMsgToClients(
      MkC4NetIOPacket(PID_LobbyCountdown, pck));
...
void Countdown::OnSec1Timer() {
  ...
  if (iStartTimer <= AlmostStartCountdownTime ||  // last seconds
      (iStartTimer <= 600 &&
       !(iStartTimer % 10)) ||  // last minute: 10s interval
  ...
  // countdown done
  if (!iStartTimer) {
    // Dedicated server: if there are not enough players for this game, abort
    // and quit the application
    if (!Game.Network.GetLobby() &&
        (Game.PlayerInfos.GetPlayerCount() < Game.C4S.GetMinPlayer())) {
      Log(LoadResStr("IDS_MSG_NOTENOUGHPLAYERSFORTHISRO"));  // it would also be
                                                             // nice to send
                                                             // this message to
                                                             // all clients...
      Application.Quit();
