...
bool FrameDecoration::SetFacetByAction(C4Def *pOfDef, C4FacetEx &rfctTarget,
                                       const char *szFacetName) {
  // get action
  StdStrBuf sActName;
  sActName.Format("FrameDeco%s", szFacetName);
  int cnt;
  C4ActionDef *pAct = pOfDef->ActMap;
  for (cnt = pOfDef->ActNum; cnt; --cnt, ++pAct)
    if (sActName == pAct->Name) break;
  if (!cnt) return false;
  // set facet by it
  rfctTarget.Set(pOfDef->Graphics.GetBitmap(), pAct->Facet.x, pAct->Facet.y,
                 pAct->Facet.Wdt, pAct->Facet.Hgt, pAct->Facet.tx,
                 pAct->Facet.ty);
  return true;
}
...
bool FrameDecoration::SetByDef(C4ID idSourceDef) {
  ...
  // query values
  dwBackClr = pSrcDef->Script.Call(FormatString(PSF_FrameDecoration, "BackClr")
                                       .getData()).getInt();
  iBorderTop =
      pSrcDef->Script.Call(FormatString(PSF_FrameDecoration, "BorderTop")
                               .getData()).getInt();
  iBorderLeft =
      pSrcDef->Script.Call(FormatString(PSF_FrameDecoration, "BorderLeft")
                               .getData()).getInt();
  iBorderRight =
      pSrcDef->Script.Call(FormatString(PSF_FrameDecoration, "BorderRight")
                               .getData()).getInt();
  iBorderBottom =
      pSrcDef->Script.Call(FormatString(PSF_FrameDecoration, "BorderBottom")
                               .getData()).getInt();
  // get gfx
  ...
  fHasGfxOutsideClientArea =
      (fctTopLeft.TargetY < 0) || (fctTop.TargetY < 0) ||
      (fctTopRight.TargetY < 0) || (fctTopLeft.TargetX < 0) ||
      (fctLeft.TargetX < 0) || (fctBottomLeft.TargetX < 0) ||
      (fctTopRight.TargetX + fctTopRight.Wdt > iBorderRight) ||
      (fctRight.TargetX + fctRight.Wdt > iBorderRight) ||
      (fctBottomRight.TargetX + fctBottomRight.Wdt > iBorderRight) ||
      (fctBottomLeft.TargetY + fctBottomLeft.Hgt > iBorderBottom) ||
      (fctBottom.TargetY + fctBottom.Hgt > iBorderBottom) ||
      (fctBottomRight.TargetY + fctBottomRight.Hgt > iBorderBottom);
  // k, done
  return true;
}
...
void FrameDecoration::Draw(C4FacetEx &cgo, C4Rect &rcBounds) {
  ...
  lpDDraw->DrawBoxDw(cgo.Surface, ox, oy, ox + rcBounds.Wdt - 1,
                     oy + rcBounds.Hgt - 1, dwBackClr);
  // draw borders
  int x, y, Q;
  // top
  if (Q = fctTop.Wdt) {
  ...
  if (Q = fctRight.Hgt) {
    for (y = iBorderTop; y < rcBounds.Hgt - iBorderBottom; y += fctRight.Hgt) {
      ...
      fctRight.Hgt = h;
      fctRight.Draw(cgo.Surface,
                    ox + rcBounds.Wdt - iBorderRight + fctRight.TargetX,
                    oy + y);
    }
    fctRight.Hgt = Q;
  }
  ...
  if (Q = fctBottom.Wdt) {
    for (x = iBorderLeft; x < rcBounds.Wdt - iBorderRight; x += fctBottom.Wdt) {
      ...
      fctBottom.Wdt = w;
      fctBottom.Draw(cgo.Surface, ox + x,
                     oy + rcBounds.Hgt - iBorderBottom + fctBottom.TargetY);
    }
    fctBottom.Wdt = Q;
  }
  // draw edges
  fctTopLeft.Draw(cgo.Surface, ox + fctTopLeft.TargetX,
                  oy + fctTopLeft.TargetY);
  fctTopRight.Draw(cgo.Surface,
                   ox + rcBounds.Wdt - iBorderRight + fctTopRight.TargetX,
                   oy + fctTopRight.TargetY);
  fctBottomLeft.Draw(cgo.Surface, ox + fctBottomLeft.TargetX,
                     oy + rcBounds.Hgt - iBorderBottom + fctBottomLeft.TargetY);
  fctBottomRight.Draw(
      cgo.Surface, ox + rcBounds.Wdt - iBorderRight + fctBottomRight.TargetX,
      oy + rcBounds.Hgt - iBorderBottom + fctBottomRight.TargetY);
}
...
#ifdef _WIN32
CStdWindow *DialogWindow::Init(CStdApp *pApp, const char *Title,
                               CStdWindow *pParent, const C4Rect &rcBounds,
                               const char *szID) {
  Active = true;
  ...
  hWindow = ::CreateWindowEx(
      0, ConsoleDlgClassName, Title, ConsoleDlgWindowStyle, CW_USEDEFAULT,
      CW_USEDEFAULT, rtSize.right - rtSize.left, rtSize.bottom - rtSize.top,
      pParent->hWindow, NULL, pApp->hInstance, NULL);
  if (hWindow) {
    // update pos
    if (szID && *szID)
      RestoreWindowPosition(hWindow,
                            FormatString("ConsoleGUI_%s", szID).getData(),
                            Config.GetSubkeyPath("Console"), FALSE);
    // and show
  ...
  switch (uMsg) {
    ...
    case WM_KEYUP:
      if (Game.DoKeyboardInput(wParam, KEYEV_Up, !!(lParam & 0x20000000),
                               Application.IsControlDown(),
                               Application.IsShiftDown(), false, pDlg))
        return 0;
      break;
    //------------------------------------------------------------------------------------------------------------
    case WM_SYSKEYDOWN:
    ...
    case WM_DESTROY: {
      ...
      if (szID && *szID)
        StoreWindowPosition(hwnd, FormatString("ConsoleGUI_%s", szID).getData(),
                            Config.GetSubkeyPath("Console"), FALSE);
    } break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_CLOSE:
    ...
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_LBUTTONDOWN:
      Game.pGUI->MouseInput(C4MC_Button_LeftDown, LOWORD(lParam),
                            HIWORD(lParam), wParam, pDlg, NULL);
      break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_LBUTTONUP:
      Game.pGUI->MouseInput(C4MC_Button_LeftUp, LOWORD(lParam), HIWORD(lParam),
                            wParam, pDlg, NULL);
      break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_RBUTTONDOWN:
      Game.pGUI->MouseInput(C4MC_Button_RightDown, LOWORD(lParam),
                            HIWORD(lParam), wParam, pDlg, NULL);
      break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_RBUTTONUP:
      Game.pGUI->MouseInput(C4MC_Button_RightUp, LOWORD(lParam), HIWORD(lParam),
                            wParam, pDlg, NULL);
      break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_LBUTTONDBLCLK:
      Game.pGUI->MouseInput(C4MC_Button_LeftDouble, LOWORD(lParam),
                            HIWORD(lParam), wParam, pDlg, NULL);
      break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_RBUTTONDBLCLK:
      Game.pGUI->MouseInput(C4MC_Button_RightDouble, LOWORD(lParam),
                            HIWORD(lParam), wParam, pDlg, NULL);
      break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_MOUSEMOVE:
      // SetCursor(NULL);
      Game.pGUI->MouseInput(C4MC_Button_None, LOWORD(lParam), HIWORD(lParam),
                            wParam, pDlg, NULL);
      break;
    //----------------------------------------------------------------------------------------------------------------------------------
    case WM_MOUSEWHEEL:
      Game.pGUI->MouseInput(C4MC_Button_Wheel, LOWORD(lParam), HIWORD(lParam),
                            wParam, pDlg, NULL);
      break;
      //----------------------------------------------------------------------------------------------------------------------------------
  }
...
bool Dialog::RegisterWindowClass(HINSTANCE hInst) {
  ...
  WndClass.hCursor =
      LoadCursor(NULL, IDC_ARROW);  // - always use normal hw cursor
...
CStdWindow *DialogWindow::Init(CStdApp *pApp, const char *Title,
  ...
  if (CStdWindow::Init(pApp, Title, pParent, false)) {
    // update pos
    if (szID && *szID)
      RestorePosition(FormatString("ConsoleGUI_%s", szID).getData(),
                      Config.GetSubkeyPath("Console"), FALSE);
    else
...
bool Dialog::CreateConsoleWindow() {
  ...
  if (!pWindow->Init(&Application, TitleString.getData(), &Console, rcBounds,
                     GetID())) {
    delete pWindow;
    pWindow = NULL;
    return false;
...
Dialog::Dialog(int32_t iWdt, int32_t iHgt, const char *szTitle,
               bool fViewportDlg)
    : Window(),
      pTitle(NULL),
      pCloseBtn(NULL),
      fDelOnClose(false),
      fViewportDlg(fViewportDlg),
      pWindow(NULL),
      pCtx(NULL),
      pFrameDeco(NULL) {
  // zero fields
  ...
  pKeyAdvanceControl = new C4KeyBinding(
      Keys, "GUIAdvanceFocus", KEYSCOPE_Gui,
      new DlgKeyCBEx<Dialog, bool>(*this, false, &Dialog::KeyAdvanceFocus),
  ...
  pKeyAdvanceControlB = new C4KeyBinding(
      Keys, "GUIAdvanceFocusBack", KEYSCOPE_Gui,
      new DlgKeyCBEx<Dialog, bool>(*this, true, &Dialog::KeyAdvanceFocus),
      C4CustomKey::PRIO_Dlg);
  Keys.clear();
  Keys.push_back(C4KeyCodeEx(KEY_Any, KEYS_Alt));
  Keys.push_back(C4KeyCodeEx(KEY_Any, C4KeyShiftState(KEYS_Alt | KEYS_Shift)));
  pKeyHotkey =
      new C4KeyBinding(Keys, "GUIHotkey", KEYSCOPE_Gui,
                       new DlgKeyCBPassKey<Dialog>(*this, &Dialog::KeyHotkey),
  ...
  pKeyEnter = new C4KeyBinding(Keys, "GUIDialogOkay", KEYSCOPE_Gui,
                               new DlgKeyCB<Dialog>(*this, &Dialog::KeyEnter),
                               C4CustomKey::PRIO_Dlg);
  Keys.clear();
  ...
  pKeyEscape = new C4KeyBinding(Keys, "GUIDialogAbort", KEYSCOPE_Gui,
                                new DlgKeyCB<Dialog>(*this, &Dialog::KeyEscape),
                                C4CustomKey::PRIO_Dlg);
  Keys.clear();
  Keys.push_back(C4KeyCodeEx(KEY_Any));
  Keys.push_back(C4KeyCodeEx(KEY_Any, KEYS_Shift));
  pKeyFocusDefControl =
      new C4KeyBinding(Keys, "GUIFocusDefault", KEYSCOPE_Gui,
                       new DlgKeyCB<Dialog>(*this, &Dialog::KeyFocusDefault),
                       C4CustomKey::PRIO_Dlg);
}
...
void Dialog::SetTitle(const char *szTitle, bool fShowCloseButton) {
  ...
  if (szTitle && *szTitle) {
    ...
    if (pTitle) {
      pTitle->GetBounds() =
          C4Rect(-GetMarginLeft(), -iTextHgt, rcBounds.Wdt, iTextHgt);
      // noupdate if title is same - this is necessary to prevent scrolling
      // reset when refilling internal menus
      if (SEqual(pTitle->GetText(), szTitle)) return;
      pTitle->SetText(szTitle);
    } else
      AddElement(pTitle = new WoodenLabel(
                     szTitle, C4Rect(-GetMarginLeft(), -iTextHgt, rcBounds.Wdt,
                                     iTextHgt),
                     C4GUI_CaptionFontClr, &GetRes()->TextFont, ALeft, false));
    ...
    if (fShowCloseButton) {
      ...
      if (!pCloseBtn) {
        AddElement(pCloseBtn = new CallbackButton<Dialog, IconButton>(
                       Ico_Close,
                       pTitle->GetToprightCornerRect(16, 16, 4, 4, 0), 0,
                       &Dialog::OnUserClose));
...
void Dialog::UpdateSize() {
  ...
  if (pTitle) {
    ...
    pTitle->SetBounds(
        C4Rect(-GetMarginLeft(), -iTextHgt, rcBounds.Wdt, iTextHgt));
    if (pCloseBtn)
      pCloseBtn->SetBounds(pTitle->GetToprightCornerRect(16, 16, 4, 4, 0));
  }
  // inherited
  Window::UpdateSize();
...
void Dialog::Draw(C4FacetEx &cgo) {
  ...
  switch (eFade) {
    ...
    case eFadeIn:
      ...
      if ((iFade += 10) >= 100) {
        if (pScreen = GetScreen()) {
          if (pScreen->GetTopDialog() == this) pScreen->ActivateDialog(this);
        }
        eFade = eFadeNone;
      }
      break;
    case eFadeOut:
      ...
      if ((iFade -= 10) <= 0) {
        ...
        if (pScreen = GetScreen()) pScreen->RecheckActiveDialog();
        eFade = eFadeNone;
      }
  }
  // set fade
  if (iFade < 100) {
    if (iFade <= 0) return;
    lpDDraw->ActivateBlitModulation(((100 - iFade) * 255 / 100) << 24 |
                                    0xffffff);
  }
  // seperate window: Clear background
  if (pWindow)
    lpDDraw->DrawBoxDw(cgo.Surface, rcBounds.x, rcBounds.y, rcBounds.Wdt - 1,
                       rcBounds.Hgt - 1, C4GUI_StandardBGColor & 0xffffff);
  ...
  if (pWindow) {
    RECT rtSrc, rtDst;
    rtSrc.left = rcBounds.x;
    rtSrc.top = rcBounds.y;
    rtSrc.right = rcBounds.x + rcBounds.Wdt;
    rtSrc.bottom = rcBounds.y + rcBounds.Hgt;
    rtDst.left = 0;
    rtDst.top = 0;
    rtDst.right = rcBounds.Wdt;
    rtDst.bottom = rcBounds.Hgt;
...
void Dialog::DrawElement(C4FacetEx &cgo) {
  ...
  if (pFrameDeco)
    ...
    lpDDraw->DrawBoxDw(
        ...
        rcBounds.y + rcBounds.Hgt - 1 + cgo.TargetY, C4GUI_StandardBGColor);
    // draw frame
    Draw3DFrame(cgo);
  }
}
...
BOOL Dialog::CharIn(const char *c) {
  ...
  // (changing focus here would render buttons unusable, because they switch on
  // KeyUp)
...
bool Dialog::KeyHotkey(C4KeyCodeEx key) {
  ...
  if (Inside<WORD>(TOUPPERIFX11(wKey), 'A', 'Z'))
    if (OnHotkey(char(TOUPPERIFX11(wKey)))) return true;
  if (Inside<WORD>(TOUPPERIFX11(wKey), '0', '9'))
    if (OnHotkey(char(TOUPPERIFX11(wKey)))) return true;
  return false;
}
...
bool Dialog::KeyFocusDefault() {
  ...
  Control *pDefCtrl = GetDefaultControl();
  if (pDefCtrl && pDefCtrl != pActiveCtrl) SetFocus(pDefCtrl, false);
  // never mark this as processed, so a later char message to the control may be
  // sent (for deselected chat)
  return false;
}

void Dialog::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX, int32_t iY,
                        DWORD dwKeyParam) {
  // inherited will do...
...
void Dialog::SetFocus(Control *pCtrl, bool fByMouse) {
  ...
  if (pActiveCtrl) {
    ...
    pActiveCtrl = NULL;
    pC->OnLooseFocus();
    // if leaving the old focus set a new one, abort here because it looks like
    // the control didn't want to lose focus
...
void Dialog::AdvanceFocus(bool fBackwards) {
  ...
  for (;;) {
    ...
    if (!pCurrElement && pActiveCtrl)
      if (!(pCurrElement = GetNextNestedElement(NULL, fBackwards))) return;
    // cycled?
    if (pCurrElement == pActiveCtrl) {
    ...
    if (pCurrElement->GetParent() &&
        !pCurrElement->GetParent()->IsSelectedChild(pCurrElement))
      continue;
...
bool Dialog::Show(Screen *pOnScreen, bool fCB) {
  ...
  // default screen
  if (!pOnScreen)
    if (!(pOnScreen = Screen::GetScreenS())) return false;
  // show there
...
void Dialog::Close(bool fOK) {
  ...
  if (pScreen)
    pScreen->CloseDialog(this, false);
  else
    fShow = false;
...
bool Dialog::DoModal() {
  ...
  while (fShow) {
    ...
    while ((iResult != HR_Timer) && fShow) {
      ...
      // quit
      if (iResult == HR_Failure || !IsGUIValid())
...
FullscreenDialog::FullscreenDialog(const char *szTitle, const char *szSubtitle)
    : Dialog(Screen::GetScreenS()->GetClientRect().Wdt,
             Screen::GetScreenS()->GetClientRect().Hgt,
             NULL /* create own title */, false),
      pFullscreenTitle(NULL),
      pBtnHelp(NULL) {
  // set margins
  ...
  if (iScreenX < 500)
    ...
    iDlgMarginX = iScreenX / 50;
  if (iScreenY < 320)
    iDlgMarginY = 2;
  else
    iDlgMarginY = iScreenY * 2 / 75;
  // set size - calcs client rect as well
  ...
  if (szSubtitle && *szSubtitle && HasUpperBoard()) {
    AddElement(pSubTitle = new Label(
                   szSubtitle, rcClientRect.Wdt,
                   C4UpperBoardHeight -
                       GetRes()->CaptionFont.GetLineHeight() / 2 - 25 -
                       GetMarginTop(),
                   ARight, C4GUI_CaptionFontClr, &GetRes()->TextFont));
  ...
  if (szTitle && *szTitle) {
    ...
    // pBtnHelp = new CallbackButton<FullscreenDialog,
    // IconButton>(Ico_UnknownClient /* 2do: Help icon */, C4Rect(0,0,32,32),
    // 'H' /* 2do */, &FullscreenDialog::OnHelpBtn, this);
    // C4Facet fctHelp = Game.GraphicsResource.fctOKCancel;
    // fctHelp.Y += fctHelp.Hgt;
    // pBtnHelp->SetFacet(fctHelp);
    // pBtnHelp->SetToolTip("[.!]Help button: Press this button and hover the
    // element you want help for!");
    // UpdateHelpButtonPos();
    // AddElement(pBtnHelp);
  }
}
...
void FullscreenDialog::SetTitle(const char *szTitle) {
  ...
  if (szTitle && *szTitle) {
    ...
    if (HasUpperBoard())
      pFullscreenTitle =
          new Label(szTitle, 0, C4UpperBoardHeight / 2 -
                                    GetRes()->TitleFont.GetLineHeight() / 2 -
                                    GetMarginTop(),
                    ALeft, C4GUI_CaptionFontClr, &GetRes()->TitleFont);
    else
      // non-woodbar: Title is centered and in big font
      pFullscreenTitle = new Label(
          szTitle, GetClientRect().Wdt / 2,
          C4UpperBoardHeight / 2 - GetRes()->TitleFont.GetLineHeight() / 2 -
              GetMarginTop(),
          ACenter, C4GUI_FullscreenCaptionFontClr, &GetRes()->TitleFont);
...
void FullscreenDialog::DrawElement(C4FacetEx &cgo) {
  // draw upper board
  if (HasUpperBoard())
    lpDDraw->BlitSurfaceTile(
        Game.GraphicsResource.fctUpperBoard.Surface, cgo.Surface, 0,
        Min<int32_t>(iFade - Game.GraphicsResource.fctUpperBoard.Hgt, 0),
        cgo.Wdt, Game.GraphicsResource.fctUpperBoard.Hgt);
}
...
void FullscreenDialog::UpdateHelpButtonPos() {
  // reposition help button
  if (pBtnHelp)
    pBtnHelp->SetBounds(C4Rect(GetBounds().Wdt - 4 - 32 - GetMarginLeft(),
                               4 - GetMarginTop(), 32, 32));
}

void FullscreenDialog::DrawBackground(C4FacetEx &cgo, C4Facet &rFromFct) {
  // draw across fullscreen bounds - zoom 1px border to prevent flashing borders
  // by blit offsets
  Screen *pScr = GetScreen();
  C4Facet cgoScreen = cgo;
  C4Rect &rcScreenBounds = pScr ? pScr->GetBounds() : GetBounds();
  cgoScreen.X = rcScreenBounds.x - 1;
  cgoScreen.Y = rcScreenBounds.y - 1;
  cgoScreen.Wdt = rcScreenBounds.Wdt + 2;
  cgoScreen.Hgt = rcScreenBounds.Hgt + 2;
...
MessageDialog::MessageDialog(const char *szMessage, const char *szCaption,
                             DWORD dwButtons, Icons icoIcon, DlgSize eSize,
                             int32_t *piConfigDontShowAgainSetting,
                             bool fDefaultNo)
    : Dialog(eSize, 100 /* will be resized */, szCaption, false),
      piConfigDontShowAgainSetting(piConfigDontShowAgainSetting) {
  CStdFont &rUseFont = GetRes()->TextFont;
  // get positions
  ComponentAligner caMain(GetClientRect(), C4GUI_DefDlgIndent,
                          C4GUI_DefDlgIndent, true);
  // place icon
  C4Rect rcIcon = caMain.GetFromLeft(C4GUI_IconWdt);
  rcIcon.Hgt = C4GUI_IconHgt;
  Icon *pIcon = new Icon(rcIcon, icoIcon);
  AddElement(pIcon);
  // centered text for small dialogs and/or dialogs w/o much text (i.e.: no
  // linebreaks)
  bool fTextCentered;
  if (eSize != dsRegular)
    fTextCentered = true;
    ...
    fTextCentered = ((iMsgWdt <= caMain.GetInnerWidth() - C4GUI_IconWdt -
                                     C4GUI_DefDlgIndent * 2) &&
                     iMsgHgt <= rUseFont.GetLineHeight());
  }
  ...
  int iMsgHeight = rUseFont.BreakMessage(szMessage, caMain.GetInnerWidth(),
                                         &sMsgBroken, true);
  Label *pLblMessage = new Label("", caMain.GetFromTop(iMsgHeight),
                                 fTextCentered ? ACenter : ALeft,
                                 C4GUI_MessageFontClr, &rUseFont, false);
  ...
  if (piConfigDontShowAgainSetting) {
    ...
    CheckBox *pCheck = new C4GUI::CheckBox(caMain.GetFromTop(h, w), szCheckText,
                                           !!*piConfigDontShowAgainSetting);
    pCheck->SetOnChecked(new C4GUI::CallbackHandler<MessageDialog>(
        this, &MessageDialog::OnDontShowAgainCheck));
    AddElement(pCheck);
  }
  ...
  int32_t i = 1;
  while (i) {
    if (dwButtons & i) ++iButtonCount;
    i = i << 1;
  }
  fHasOK = !!(dwButtons & btnOK) || !!(dwButtons & btnYes);
  Button *btnFocus = NULL;
  if (iButtonCount) {
    C4Rect rcBtn = caButtonArea.GetCentered(
        iButtonCount * C4GUI_DefButton2Wdt +
    ...
    if (dwButtons & btnOK) {
      ...
      AddElement(pBtnOK);  // pBtnOK->SetToolTip((dwButtons & btnAbort) ?
                           // LoadResStr("IDS_DLGTIP_OK2") :
                           // LoadResStr("IDS_DLGTIP_OK"));
    ...
    if (dwButtons & btnRetry) {
      ...
      AddElement(
          pBtnRetry);  // pBtnAbort->SetToolTip(LoadResStr("IDS_DLGTIP_CANCEL"));
      rcBtn.x += C4GUI_DefButton2Wdt + C4GUI_DefButton2HSpace;
    ...
    if (dwButtons & btnNo) {
      ...
      AddElement(
          pBtnNo);  // pBtnNo->SetToolTip(LoadResStr("IDS_DLGTIP_CANCEL"));
      // rcBtn.x += C4GUI_DefButton2Wdt+C4GUI_DefButton2HSpace;
...
ConfirmationDialog::ConfirmationDialog(const char *szMessage,
                                       const char *szCaption,
                                       BaseCallbackHandler *pCB,
                                       DWORD dwButtons, bool fSmall,
                                       Icons icoIcon)
    : MessageDialog(
          szMessage, szCaption, dwButtons, icoIcon,
          fSmall ? MessageDialog::dsSmall : MessageDialog::dsRegular) {
...
ProgressDialog::ProgressDialog(const char *szMessage, const char *szCaption,
                               int32_t iMaxProgress, int32_t iInitialProgress,
                               Icons icoIcon)
    : Dialog(C4GUI_ProgressDlgWdt,
             Max(GetRes()->TextFont.BreakMessage(
                     szMessage, C4GUI_ProgressDlgWdt - 3 * C4GUI_DefDlgIndent -
                                    C4GUI_IconWdt,
                     OSTR, 4096, true),
                 C4GUI_IconHgt) +
  ...
  ComponentAligner caMain(GetClientRect(), C4GUI_DefDlgIndent,
                          C4GUI_DefDlgIndent, true);
  ComponentAligner caButtonArea(caMain.GetFromBottom(C4GUI_ButtonAreaHgt), 0,
                                0);
  ...
  // use text with line breaks
  Label *pLblMessage =
      new Label(OSTR, caMain.GetAll().GetMiddleX(), caMain.GetAll().y, ACenter,
                C4GUI_MessageFontClr, &GetRes()->TextFont);
  AddElement(pLblMessage);
  // place progress bar
  ...
  AddElement(pBar);
  // place abort button
  Button *pBtnAbort = new CancelButton(
...
bool Screen::ShowMessage(const char *szMessage, const char *szCaption,
  ...
  return true;
#endif
  return ShowRemoveDlg(new MessageDialog(
      szMessage, szCaption, MessageDialog::btnOK, icoIcon,
      MessageDialog::dsRegular, piConfigDontShowAgainSetting));
}
...
bool Screen::ShowMessageModal(const char *szMessage, const char *szCaption,
                              DWORD dwButtons, Icons icoIcon,
                              int32_t *piConfigDontShowAgainSetting) {
  // always log messages
  ...
  return ShowModalDlg(new MessageDialog(szMessage, szCaption, dwButtons,
                                        icoIcon, MessageDialog::dsRegular,
                                        piConfigDontShowAgainSetting));
}

ProgressDialog *Screen::ShowProgressDlg(const char *szMessage,
                                        const char *szCaption,
                                        int32_t iMaxProgress,
                                        int32_t iInitialProgress,
                                        Icons icoIcon) {
  // create progress dlg
  ProgressDialog *pDlg = new ProgressDialog(szMessage, szCaption, iMaxProgress,
                                            iInitialProgress, icoIcon);
  // show it
  if (!pDlg->Show(this, true)) {
    delete pDlg;
    return NULL;
  }
  // do not return invalid pointer if GUI got deleted (whil eshowing the
  // progress bar Dlg; maybe some stupid stuff in OnShow)
...
InputDialog::InputDialog(const char *szMessage, const char *szCaption,
                         Icons icoIcon, BaseInputCallback *pCB,
                         bool fChatLayout)
    : Dialog(fChatLayout ? Config.Graphics.ResX * 4 / 5 : C4GUI_InputDlgWdt,
             fChatLayout ? C4GUI::Edit::GetDefaultEditHeight() + 2
                         : Max(GetRes()->TextFont.BreakMessage(
                                   szMessage,
                                   C4GUI_InputDlgWdt - 3 * C4GUI_DefDlgIndent -
                                       C4GUI_IconWdt,
                                   OSTR, 4096, true),
                               C4GUI_IconHgt) +
                               C4GUI_InputDlgVRoom,
  ...
  if (fChatLayout) {
    ...
    pChatLbl = new C4GUI::WoodenLabel(szMessage, caChat.GetFromLeft(w + 4),
    ...
    // use text with line breaks
    Label *pLblMessage =
        new Label(OSTR, caMain.GetAll().GetMiddleX(), caMain.GetAll().y,
                  ACenter, C4GUI_MessageFontClr, &GetRes()->TextFont);
    AddElement(pLblMessage);
    // place input edit
    SetCustomEdit(new Edit(rcEditBounds));
    // place buttons
    C4Rect rcBtn = caButtonArea.GetCentered(
        2 * C4GUI_DefButton2Wdt + C4GUI_DefButton2HSpace, C4GUI_ButtonHgt);
    rcBtn.Wdt = C4GUI_DefButton2Wdt;
    // OK
    Button *pBtnOK = new OKButton(rcBtn);
...
InfoDialog::InfoDialog(const char *szCaption, int32_t iLineCount)
    : Dialog(C4GUI_InfoDlgWdt, GetRes()->TextFont.GetLineHeight() * iLineCount +
                                   C4GUI_InfoDlgVRoom,
             szCaption, false),
      iScroll(0),
      pSec1Timer(NULL) {
...
InfoDialog::InfoDialog(const char *szCaption, int iLineCount,
  ...
  for (size_t i = 0; i < sText.getLength(); ++i) {
    ...
    pTextWin->AddTextLine(sLine.getData(), &GetRes()->TextFont,
                          C4GUI_MessageFontClr, false, true);
  }
...
void InfoDialog::CreateSubComponents() {
  ...
  // place info box
  pTextWin =
      new TextWindow(caMain.GetAll(), 0, 0, 0, 100, 4096, "  ", true, NULL, 0);
  AddElement(pTextWin);
  // place close button
  Button *pBtnClose = new DlgCloseButton(
      caButtonArea.GetCentered(C4GUI_DefButtonWdt, C4GUI_ButtonHgt));
  AddElement(pBtnClose);
  pBtnClose->SetToolTip(LoadResStr("IDS_MNU_CLOSE"));
}

void InfoDialog::AddLine(const char *szText) {
  // add line to text window
  if (!pTextWin) return;
  pTextWin->AddTextLine(szText, &GetRes()->TextFont, C4GUI_MessageFontClr,
                        false, true);
}
