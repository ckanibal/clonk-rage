...
bool C4StartupGraphics::LoadFile(C4FacetExID &rToFct, const char *szFilename) {
  return Game.GraphicsResource.LoadFile(rToFct, szFilename,
...
bool C4StartupGraphics::Init() {
  ...
  /*	if (!LoadFile(fctCrew, "StartupCrew")) return false; - currently unused
     ...
     LogFatal("ClrByOwner error! (11)"); return FALSE; }
  ...
  fctScenSelIcons.Wdt =
      fctScenSelIcons.Hgt;  // icon width is determined by icon height
  ...
  fctOptionsIcons.Set(fctOptionsIcons.Surface, 0, 0, fctOptionsIcons.Hgt,
                      fctOptionsIcons.Hgt);
...
bool C4StartupGraphics::InitFonts() {
  ...
  if (!Game.FontLoader.InitFont(
          BookFontCapt, szFont, C4FontLoader::C4FT_Caption,
          Config.General.RXFontSize, &Game.GraphicsResource.Files, false)) {
    LogFatal("Font Error (1)");
    return false;
  }
  Game.SetInitProgress(85);
  if (!Game.FontLoader.InitFont(BookFont, szFont, C4FontLoader::C4FT_Main,
                                Config.General.RXFontSize,
                                &Game.GraphicsResource.Files, false)) {
    LogFatal("Font Error (2)");
    return false;
  }
  Game.SetInitProgress(90);
  if (!Game.FontLoader.InitFont(BookFontTitle, szFont, C4FontLoader::C4FT_Title,
                                Config.General.RXFontSize,
                                &Game.GraphicsResource.Files, false)) {
    LogFatal("Font Error (3)");
    return false;
  }
  Game.SetInitProgress(95);
  if (!Game.FontLoader.InitFont(
          BookSmallFont, szFont, C4FontLoader::C4FT_MainSmall,
          Config.General.RXFontSize, &Game.GraphicsResource.Files, false)) {
    LogFatal("Font Error (4)");
    return false;
  }
  return true;
}
...
CStdFont &C4StartupGraphics::GetBlackFontByHeight(int32_t iHgt, float *pfZoom) {
  ...
  CStdFont *pUseFont;
  if (iHgt <= BookSmallFont.GetLineHeight())
    pUseFont = &BookSmallFont;
  else if (iHgt <= BookFont.GetLineHeight())
    pUseFont = &BookFont;
  else if (iHgt <= BookFontCapt.GetLineHeight())
    pUseFont = &BookFontCapt;
  else
    pUseFont = &BookFontTitle;
  // determine zoom
  if (pfZoom) {
...
C4Startup::C4Startup()
    : fInStartup(false), fAborted(false), pLastDlg(NULL), pCurrDlg(NULL) {
  // must be single!
  assert(!pInstance);
  pInstance = this;
}
...
C4StartupDlg *C4Startup::SwitchDialog(DialogID eToDlg, bool fFade) {
  // can't go back twice, because dialog is not remembered: Always go back to
  // main in this case
...
// first run: Splash video
  ...
  // Play some music!
  if (Config.Sound.FEMusic) Application.MusicSystem.Play();
  ...
  if (Game.fQuitWithError || GetFatalError()) {
    ...
    if (szErr) {
      Game.pGUI->ShowMessage(szErr, LoadResStr("IDS_DLG_LOG"),
                             C4GUI::Ico_Error);
    } else {
      // fallback to showing complete log
      StdStrBuf sLastLog;
      if (GetLogSection(Game.StartupLogPos,
                        Game.QuitLogPos - Game.StartupLogPos, sLastLog))
        if (!sLastLog.isNull())
          Game.pGUI->ShowRemoveDlg(
              new C4GUI::InfoDialog(LoadResStr("IDS_DLG_LOG"), 10, sLastLog));
    }
    ResetFatalError();
  }
...
bool C4Startup::SetStartScreen(const char *szScreen) {
  ...
  if (SEqualNoCase(szScreen, "main")) eLastDlgID = SDID_Main;
  if (SEqualNoCase(szScreen, "scen")) eLastDlgID = SDID_ScenSel;
