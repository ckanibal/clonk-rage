...
bool C4PlayerInfoListBox::ListItem::CanLocalChooseTeams(
    int32_t idPlayer) const {
...
void C4PlayerInfoListBox::ListItem::DrawElement(C4FacetEx &cgo) {
  if (dwBackground)
    lpDDraw->DrawBoxDw(
        cgo.Surface, cgo.TargetX + rcBounds.x, cgo.TargetY + rcBounds.y,
        cgo.TargetX + rcBounds.x + rcBounds.Wdt - 1,
        cgo.TargetY + rcBounds.y + rcBounds.Hgt - 1, dwBackground);
...
C4PlayerInfoListBox::PlayerListItem::PlayerListItem(
    C4PlayerInfoListBox *pForListBox, int32_t idClient, int32_t idPlayer,
    bool fSavegamePlayer, C4GUI::Element *pInsertBeforeElement)
    : ListItem(pForListBox),
      pTeamCombo(NULL),
      fIconSet(false),
      fJoinedInfoSet(false),
      dwJoinClr(0),
      dwPlrClr(0),
      idClient(idClient),
      idPlayer(idPlayer),
      fFreeSavegamePlayer(fSavegamePlayer),
      pScoreLabel(NULL),
      pRankIcon(NULL),
      pTimeLabel(NULL),
      pTeamPic(NULL),
...
{
  bool fIsEvaluation = pForListBox->IsEvaluation(),
       fIsLobby = pForListBox->IsLobby();
  ...
  // calc height
  int32_t iHeight = C4GUI::GetRes()->TextFont.GetLineHeight() +
                    C4GUI::ComboBox::GetDefaultHeight() + 3 * IconLabelSpacing;
  // create subcomponents
  pIcon =
      new C4GUI::Icon(C4Rect(0, 0, iHeight, iHeight), C4GUI::Ico_UnknownPlayer);
  if (Game.Parameters.isLeague())
    pRankIcon =
        new C4GUI::Icon(C4Rect(0, 0, C4GUI::ComboBox::GetDefaultHeight(),
                               C4GUI::ComboBox::GetDefaultHeight()),
                        C4GUI::Ico_None);
  if (Game.Teams.IsMultiTeams() && !(fIsEvaluation && pList->IsTeamFilter())) {
    ...
    pTeamCombo->SetComboCB(new C4GUI::ComboBox_FillCallback<PlayerListItem>(
        this, &PlayerListItem::OnTeamComboFill,
        &PlayerListItem::OnTeamComboSelChange));
  ...
  if (fIsEvaluation) {
    ...
    if (!pList->IsTeamFilter()) {
      ...
      if (pTeam && pTeam->GetIconSpec() && *pTeam->GetIconSpec()) {
        pTeamPic = new C4GUI::Picture(
            C4Rect(iHeight + IconLabelSpacing, 0, iHeight, iHeight), true);
        Game.DrawTextSpecImage(pTeamPic->GetMFacet(), pTeam->GetIconSpec(),
                               pTeam->GetColor());
        pTeamPic->SetDrawColor(pTeam->GetColor());
      }
    }
    // Total playing time (not in team filter because then the second line is
    // taken by the score label)
    if (!pList->IsTeamFilter()) {
      StdStrBuf sTimeLabelText;
      C4RoundResultsPlayer *pRoundResultsPlr =
          Game.RoundResults.GetPlayers().GetByID(idPlayer);
      uint32_t iTimeTotal = pRoundResultsPlr
                                ? pRoundResultsPlr->GetTotalPlayingTime()
                                : 0 /* unknown - should not happen */;
      sTimeLabelText.Format(LoadResStr("IDS_CTL_TOTALPLAYINGTIME"),
                            iTimeTotal / 3600, (iTimeTotal / 60) % 60,
                            iTimeTotal % 60);
      pTimeLabel =
          new C4GUI::Label(sTimeLabelText.getData(), 0, 0, ARight, dwTextColor,
                           pForListBox->GetCustomFont(), false, true);
    }
    // Extra info set by script
    C4RoundResultsPlayer *pEvaluationPlayer =
        Game.RoundResults.GetPlayers().GetByID(idPlayer);
    ;
    if (pEvaluationPlayer) {
      const char *szCustomEval =
          pEvaluationPlayer->GetCustomEvaluationStrings();
      if (szCustomEval && *szCustomEval) {
        pExtraLabel =
            new C4GUI::Label(szCustomEval, 0, 0, ARight);  // positioned later
        iHeight += C4GUI::GetRes()->TextFont.GetLineHeight();
      }
    }
  }
  pNameLabel = new C4GUI::Label(sPlayerName.getData(),
                                (iHeight + IconLabelSpacing) * (1 + !!pTeamPic),
                                IconLabelSpacing, ALeft, dwPlayerColor,
                                pCustomFont, !fIsEvaluation, true);
  // calc own bounds - list box needs height only; width and pos will be moved
  // by list
  ...
  // tooltip? (same for all components for now. seperate tooltip for status icon
  // later?)
  // SetToolTip(FormatString("%s %s", LoadResStr("IDS_CTL_PLAYER"),
  // sPlayerName.getData()).getData()); hat so keine nennenswerte Aussage...
  ...
  if (fIsLobby)
    SetContextHandler(new C4GUI::CBContextHandler<PlayerListItem>(
        this, &PlayerListItem::OnContext));
...
void C4PlayerInfoListBox::PlayerListItem::UpdateOwnPos() {
  ...
  if (pTeamPic)
    caBounds.GetFromLeft(pTeamPic->GetBounds().Wdt - IconLabelSpacing);
  C4Rect rcExtraDataRect;
  // extra data label area
  if (pExtraLabel)
    rcExtraDataRect =
        caBounds.GetFromBottom(C4GUI::GetRes()->TextFont.GetLineHeight());
  // second line (team+rank)
  C4GUI::ComponentAligner caTeamArea(
      caBounds.GetFromBottom(C4GUI::ComboBox::GetDefaultHeight()), 0, 0);
  C4Rect rcRankIcon;
  if (pList->IsEvaluation()) {
    if (pRankIcon) {
      rcRankIcon = caBounds.GetFromRight(caBounds.GetInnerHeight());
      if (pExtraLabel)
        rcExtraDataRect.Wdt = caBounds.GetInnerWidth();  // In evaluation view,
                                                         // rank icon has its
                                                         // own coloumn
    }
  } else {
    if (pRankIcon)
      rcRankIcon = caTeamArea.GetFromRight(caTeamArea.GetInnerHeight());
  }
...
int32_t C4PlayerInfoListBox::PlayerListItem::GetListItemTopSpacing() {
  ...
  if (pList->IsEvaluation()) {
    ...
    if (pPrevItem) {
      C4PlayerInfoListBox::ListItem *pPrevListItem =
          static_cast<C4PlayerInfoListBox::ListItem *>(pPrevItem);
      if (pPrevListItem->idListItemID.idType == ListItem::ID::PLI_PLAYER) {
        PlayerListItem *pPrevPlayerListItem =
            static_cast<C4PlayerInfoListBox::PlayerListItem *>(pPrevListItem);
...
void C4PlayerInfoListBox::PlayerListItem::UpdateIcon(
    C4PlayerInfo *pInfo, C4PlayerInfo *pJoinedInfo) {
  // check whether icon is known
  bool fResPresent = false;
  C4Network2Res *pRes = NULL;
  if (pInfo)
    if (pRes = pInfo->GetRes()) fResPresent = pRes->isComplete();
  C4RoundResultsPlayer *pEvaluationPlayer = NULL;
  if (pList->IsEvaluation())
    pEvaluationPlayer = Game.RoundResults.GetPlayers().GetByID(idPlayer);
  bool fHasIcon =
      fResPresent || pEvaluationPlayer || (!Game.Network.isEnabled() && pInfo);
  ...
  // already up-to-date?
  if (fHasIcon == fIconSet && fJoinedInfoSet == fHasJoinedInfo &&
      dwJoinedInfoClr == dwJoinClr && dwPlayerClr == dwPlrClr)
    return;
  // update then
  // redraw player icon
  if (fHasIcon) {
    ...
    if (!fIconSet) {
      ...
      Game.GraphicsResource.fctPlayerClr.DrawClr(pIcon->GetMFacet(), TRUE,
                                                 dwPlayerClr);
    }
    fIconSet = true;
    // unreg icon? Grayscale and stamp!
    /*bool fUnreg = false;
    C4ClientPlayerInfos *pClientPlrInfos =
    Game.PlayerInfos.GetClientInfoByPlayerID(pInfo->GetID());
    if (pClientPlrInfos)
            {
    ...
    if (fUnreg)
            {
            C4FacetExSurface &rPlrImg = pIcon->GetMFacet();
    ...
    pIcon->SetIcon((pInfo && pInfo->GetType() == C4PT_Script)
                       ? C4GUI::Ico_Host
                       : C4GUI::Ico_UnknownPlayer);
  // join
  if (fHasJoinedInfo) {
    ...
    fctDraw.Hgt -= iCrewClrHgt;
    fctDraw.Y += iCrewClrHgt;
    fctDraw.Wdt = iSizeMax / 2;
    fctDraw.X = 2;
    // shadow
    DWORD dwPrevMod;
    bool fPrevMod = lpDDraw->GetBlitModulation(dwPrevMod);
    Application.DDraw->ActivateBlitModulation(1);
    Game.GraphicsResource.fctCrewClr.DrawClr(fctDraw, TRUE, dwJoinedInfoClr);
    if (fPrevMod)
      lpDDraw->ActivateBlitModulation(dwPrevMod);
    else
...
void C4PlayerInfoListBox::PlayerListItem::UpdateTeam() {
  ...
  if (!Game.Teams.CanLocalSeeTeam())
    ...
    if (idTeam = pInfo->GetTeam())
      if (pTeam = Game.Teams.GetTeamByID(idTeam)) szTeamName = pTeam->GetName();
  pTeamCombo->SetText(szTeamName);
  pTeamCombo->SetReadOnly(fReadOnly);
}

void C4PlayerInfoListBox::PlayerListItem::UpdateScoreLabel(
    C4PlayerInfo *pInfo) {
  assert(pInfo);
  C4RoundResultsPlayer *pRoundResultsPlr = NULL;
  if (pList->IsEvaluation())
    pRoundResultsPlr = Game.RoundResults.GetPlayers().GetByID(idPlayer);

  if (pInfo->getLeagueScore() || pInfo->IsLeagueProjectedGainValid() ||
      pRoundResultsPlr) {
    int32_t iScoreRightPos =
        ((pRankIcon && pList->IsEvaluation()) ? pRankIcon->GetBounds().x
                                              : GetBounds().Wdt) -
        IconLabelSpacing;
    int32_t iScoreYPos = IconLabelSpacing;
    // if evaluation and team lists, move score label into second line - TODO:
    // some hack only, still needs to be done right

    C4RoundResultsPlayer *pEvaluationPlayer =
        Game.RoundResults.GetPlayers().GetByID(pInfo->GetID());
    ;
    bool fPlayerHasEvaluationData = false;
    if (pEvaluationPlayer) {
      const char *szCustomEval =
          pEvaluationPlayer->GetCustomEvaluationStrings();
      if (szCustomEval && *szCustomEval) fPlayerHasEvaluationData = true;
    }
    if (pList->IsEvaluation() && pList->IsTeamFilter())
      iScoreYPos = GetBounds().Hgt - (C4GUI::ComboBox::GetDefaultHeight() *
                                      (1 + (int32_t)fPlayerHasEvaluationData)) -
                   IconLabelSpacing;

    // score label visible
    if (!pScoreLabel) {
      AddElement(pScoreLabel = new C4GUI::Label("", iScoreRightPos, iScoreYPos,
                                                ARight, pList->GetTextColor(),
                                                pList->GetCustomFont(), false));
      if (pList->IsEvaluation())
        pScoreLabel->SetToolTip(LoadResStr("IDS_DESC_OLDANDNEWSCORE"));
      else
        pScoreLabel->SetToolTip(
            LoadResStr("IDS_DESC_LEAGUESCOREANDPROJECTEDGA"));
    }
    StdStrBuf sText;
    // Evaluation (GameOver)
    if (pList->IsEvaluation()) {
      if (pInfo->getLeagueScore() || pInfo->IsLeagueProjectedGainValid() ||
          (pRoundResultsPlr && pRoundResultsPlr->IsLeagueScoreNewValid())) {
        if (pRoundResultsPlr && pRoundResultsPlr->IsLeagueScoreNewValid()) {
          // Show old league score, gain, and new league score
          // Normally, the league server should make sure that [Old score] +
          // [Gain] == [New score]
          int32_t iOldScore = pInfo->getLeagueScore(),
                  iScoreGain = pRoundResultsPlr->GetLeagueScoreGain(),
                  iNewScore = pRoundResultsPlr->GetLeagueScoreNew();
          int32_t iDiscrepancy = iNewScore - (iOldScore + iScoreGain);
          if (!iDiscrepancy) {
            sText.Format("{{Ico:League}}<c afafaf>%d (%+d)</c> %d %s",
                         (int)iOldScore, (int)iScoreGain, (int)iNewScore,
                         LoadResStr("IDS_TEXT_SCORE"));
          } else {
            // If there's a discrepancy, there must have been some kind of admin
            // intervention during the game - display it in red!
            sText.Format(
                "{{Ico:League}}<c afafaf>%d (%+d)</c><c ff0000>(%+d)</c> %d %s",
                (int)iOldScore, (int)iScoreGain, (int)iDiscrepancy,
                (int)iNewScore, LoadResStr("IDS_TEXT_SCORE"));
          }
        }
        // Show old league score only
        else {
          sText.Format("{{Ico:League}}<c afafaf>(%d)</c> %s",
                       (int)pInfo->getLeagueScore(),
                       LoadResStr("IDS_TEXT_SCORE"));
        }
      } else if (pRoundResultsPlr && pRoundResultsPlr->IsScoreNewValid() &&
                 !Game.RoundResults.SettlementScoreIsHidden()) {
        // new score known
        sText.Format("{{Ico:Settlement}}<c afafaf>%d (%+d)</c> %d %s",
                     (int)pRoundResultsPlr->GetScoreOld(),
                     (int)(pRoundResultsPlr->GetScoreNew() -
                           pRoundResultsPlr->GetScoreOld()),
                     (int)pRoundResultsPlr->GetScoreNew(),
                     LoadResStr("IDS_TEXT_SCORE"));
      } else if (pRoundResultsPlr && !pRoundResultsPlr->IsScoreNewValid() &&
                 !Game.RoundResults.SettlementScoreIsHidden()) {
        // only old score known (e.g., player disconnected)
        sText.Format("{{Ico:Settlement}}<c afafaf>(%d)</c> %s",
                     (int)pRoundResultsPlr->GetScoreOld(),
                     LoadResStr("IDS_TEXT_SCORE"));
      } else {
    ...
    // Pre-evaluation (Lobby)
      ...
      // Don't show if team invisible, so random surprise teams don't get
      // spoiled
      if (pInfo->IsLeagueProjectedGainValid() && Game.Teams.IsTeamVisible())
        sText.Format("%d (%+d)", (int)pInfo->getLeagueScore(),
                     (int)pInfo->GetLeagueProjectedGain());
  ...
  if (pRankIcon) {
    ...
    if (!iSym) iSym = pInfo->getLeagueRankSymbol();
    if (iSym && !fShownCollapsed) {
      C4GUI::Icons eRankIcon = (C4GUI::Icons)(
          C4GUI::Ico_Rank1 +
          BoundBy<int32_t>(iSym - 1, 0, C4GUI::Ico_Rank9 - C4GUI::Ico_Rank1));
...
C4GUI::ContextMenu *C4PlayerInfoListBox::PlayerListItem::OnContext(
    C4GUI::Element *pListItem, int32_t iX, int32_t iY) {
  C4PlayerInfo *pInfo = GetPlayerInfo();
  assert(pInfo);
  ...
  if (fFreeSavegamePlayer) {
    if (pInfo->GetType() != C4PT_Script) {
      ...
      pMenu->AddItem(strTakeOver.getData(),
                     LoadResStr("IDS_MSG_TAKEOVERPLR_DESC"), C4GUI::Ico_Player,
                     NULL, new C4GUI::CBContextHandler<PlayerListItem>(
                               this, &PlayerListItem::OnContextTakeOver));
    }
  } else {
    ...
    if (Game.Network.isHost() || IsLocalClientPlayer()) {
      ...
      if (pInfo->GetType() != C4PT_Script ||
          !pInfo->GetAssociatedSavegamePlayerID()) {
        StdCopyStrBuf strRemove(LoadResStr("IDS_MSG_REMOVEPLR"));
        pMenu->AddItem(strRemove.getData(),
                       LoadResStr("IDS_MSG_REMOVEPLR_DESC"), C4GUI::Ico_Close,
                       new C4GUI::CBMenuHandler<PlayerListItem>(
                           this, &PlayerListItem::OnCtxRemove),
                       NULL);
      }
      // color was changed: Add option to assign a new color
      C4PlayerInfo *pInfo = GetPlayerInfo();
      assert(pInfo);
      if (pInfo && pInfo->HasAutoGeneratedColor() &&
          (!Game.Teams.IsTeamColors() || !pInfo->GetTeam())) {
        StdCopyStrBuf strNewColor(LoadResStr("IDS_MSG_NEWPLRCOLOR"));
        pMenu->AddItem(
            strNewColor.getData(), LoadResStr("IDS_MSG_NEWPLRCOLOR_DESC"),
            C4GUI::Ico_Player, new C4GUI::CBMenuHandler<PlayerListItem>(
                                   this, &PlayerListItem::OnCtxNewColor),
            NULL);
      }
    }
  }
...
C4GUI::ContextMenu *C4PlayerInfoListBox::PlayerListItem::OnContextTakeOver(
    C4GUI::Element *pListItem, int32_t iX, int32_t iY) {
  // create context menu
  C4GUI::ContextMenu *pMenu = new C4GUI::ContextMenu();
    ...
    while (pInfo = pkInfo->GetPlayerInfo(i++))
      if (!pInfo->HasJoinIssued())
        if (!pInfo->GetAssociatedSavegamePlayerID()) {
          ...
          pMenu->AddItem(
              OSTR, LoadResStr("IDS_MSG_USINGPLR_DESC"), C4GUI::Ico_Player,
              new C4GUI::CBMenuHandlerEx<PlayerListItem, int32_t>(
                  this, &PlayerListItem::OnCtxTakeOver, pInfo->GetID()));
        }
  }
  // add option to use a new one... 2do
  // pMenu->AddItem("[.!]From &File...", "Select another player file",
  // C4GUI::Ico_Player, new C4GUI::CBMenuHandler<PlayerListItem>(this,
  // OnCtxTest2));
  // add option to take over from savegame player 2do
  // pMenu->AddItem("[.!]From &Savegame", "Use savegame player file",
  // C4GUI::Ico_Player, new C4GUI::CBMenuHandler<PlayerListItem>(this,
  // OnCtxTest2));
  // open it
  return pMenu;
...
void C4PlayerInfoListBox::PlayerListItem::OnCtxTakeOver(
    C4GUI::Element *pListItem, const int32_t &idPlayer) {
  // use player idPlayer to take over this one
  // this must be processed as a request by the host
  // some safety first...
  C4ClientPlayerInfos *pLocalInfo =
      Game.Network.Players.GetLocalPlayerInfoPacket();
...
void C4PlayerInfoListBox::PlayerListItem::OnCtxRemove(
    C4GUI::Element *pListItem) {
  // only host or own player
  if (!Game.Network.isEnabled() ||
      (!Game.Network.isHost() && !IsLocalClientPlayer()))
    return;
  ...
  C4ClientPlayerInfos *pChangeInfo =
      Game.PlayerInfos.GetInfoByClientID(idClient);
...
void C4PlayerInfoListBox::PlayerListItem::OnCtxNewColor(
    C4GUI::Element *pListItem) {
  // only host or own player
  if (!Game.Network.isEnabled() ||
      (!Game.Network.isHost() && !IsLocalClientPlayer()))
...
void C4PlayerInfoListBox::PlayerListItem::OnTeamComboFill(
...
bool C4PlayerInfoListBox::PlayerListItem::OnTeamComboSelChange(
    C4GUI::ComboBox *pForCombo, int32_t idNewSelection) {
  // always return true to mark combo sel as processed, so the GUI won't change
  // the team text
...
void C4PlayerInfoListBox::PlayerListItem::Update() {
  ...
  if (pNfo) {
    ...
    if (pList->IsEvaluation()) {
      ...
      if (fShowWinners) {
        sShowName.Take(
            FormatString("%s (%s)", sShowName.getData(),
                         LoadResStr(fHasWon ? "IDS_CTL_WON" : "IDS_CTL_LOST")));
      }
      // evaluation: Golden color+background for winners; gray for losers or no
      // winner show
      if (fHasWon) {
...
C4PlayerInfo *C4PlayerInfoListBox::PlayerListItem::GetPlayerInfo() const {
  return fFreeSavegamePlayer
             ? Game.RestorePlayerInfos.GetPlayerInfoByID(idPlayer)
             : Game.PlayerInfos.GetPlayerInfoByID(idPlayer);
}
...
bool C4PlayerInfoListBox::PlayerListItem::IsLocalClientPlayer() const {
  ...
  // if no client can be found, assume network disconnect and everythign local
  // then
...
C4PlayerInfoListBox::ClientListItem::ClientListItem(
    C4PlayerInfoListBox *pForListBox, const C4ClientCore &rClientInfo,
    ListItem *pInsertBefore)  // ctor
    : ListItem(pForListBox),
      idClient(rClientInfo.getID()),
      dwClientClr(0xffffff),
      tLastSoundTime(0) {
  ...
  // create subcomponents
  pStatusIcon = new C4GUI::Icon(C4Rect(0, 0, iIconSize, iIconSize),
                                GetCurrentStatusIcon());
  pNameLabel = new C4GUI::Label(
      rClientInfo.getName(), iIconSize + IconLabelSpacing, 0, ALeft,
      dwClientClr | C4GUI_MessageFontAlpha, NULL, true, false);
  pPingLabel = NULL;
  ...
  if (IsLocalClientPlayer()) {
    ...
    btnAddPlayer = new C4GUI::CallbackButton<ClientListItem, C4GUI::IconButton>(
        C4GUI::Ico_AddPlr, C4Rect(0, 0, iIconSize, iIconSize),
        'P' /* 2do TODO */, &ClientListItem::OnBtnAddPlr, this);
  }
  // calc own bounds
  C4Rect rcOwnBounds = pNameLabel->GetBounds();
  rcOwnBounds.Wdt += rcOwnBounds.x;
  rcOwnBounds.x = 0;
  rcOwnBounds.Hgt += rcOwnBounds.y;
  rcOwnBounds.y = 0;
  ...
  // context menu for list item
  SetContextHandler(new C4GUI::CBContextHandler<ClientListItem>(
      this, &ClientListItem::OnContext));
...
void C4PlayerInfoListBox::ClientListItem::SetPing(int32_t iToPing) {
  ...
  if (!pPingLabel) {
    pPingLabel = new C4GUI::Label(OSTR, GetBounds().Wdt, 0, ARight,
...
bool C4PlayerInfoListBox::ClientListItem::IsLocalClientPlayer() const {
  // check whether client is local
  // if no client can be found, something is wrong - assume network disconnect
  // and everything local then
  C4Network2Client *pClient = GetNetClient();
  assert(pClient);
  return !pClient || pClient->isLocal();
}
...
C4GUI::Icons C4PlayerInfoListBox::ClientListItem::GetCurrentStatusIcon() {
  ...
  // info present?
  C4ClientPlayerInfos *pInfoPacket =
      Game.PlayerInfos.GetInfoByClientID(idClient);
...
void C4PlayerInfoListBox::ClientListItem::UpdatePing() {
  ...
  if (pClient && !pClient->isLocal()) {
    ...
    if (iPing <= 0)
      if (pConn = pClient->getDataConn()) iPing = pConn->getLag();
  }
...
C4GUI::ContextMenu *C4PlayerInfoListBox::ClientListItem::OnContext(
    C4GUI::Element *pListItem, int32_t iX, int32_t iY) {
  // safety
  ...
  if (Game.Network.isHost() && GetNetClient()) {
    ...
    pMenu->AddItem(LoadResStr("IDS_NET_KICKCLIENT"), strKickDesc.getData(),
                   C4GUI::Ico_None, new C4GUI::CBMenuHandler<ClientListItem>(
                                        this, &ClientListItem::OnCtxKick));
    StdCopyStrBuf strActivateDesc(LoadResStr("IDS_NET_ACTIVATECLIENT_DESC"));
    pMenu->AddItem(
        LoadResStr(pClient->isActivated() ? "IDS_NET_DEACTIVATECLIENT"
                                          : "IDS_NET_ACTIVATECLIENT"),
        strActivateDesc.getData(), C4GUI::Ico_None,
        new C4GUI::CBMenuHandler<ClientListItem>(
            this, &ClientListItem::OnCtxActivate));
  }
  // info
  StdCopyStrBuf strClientInfoDesc(LoadResStr("IDS_NET_CLIENTINFO_DESC"));
  pMenu->AddItem(LoadResStr("IDS_NET_CLIENTINFO"), strClientInfoDesc.getData(),
                 C4GUI::Ico_None, new C4GUI::CBMenuHandler<ClientListItem>(
                                      this, &ClientListItem::OnCtxInfo));
  // open it
  return pMenu;
}
...
void C4PlayerInfoListBox::ClientListItem::OnCtxActivate(
    C4GUI::Element *pListItem) {
  // host only
  C4Client *pClient = GetClient();
  ...
  Game.Control.DoInput(CID_ClientUpdate,
                       new C4ControlClientUpdate(idClient, CUT_Activate,
                                                 !pClient->isActivated()),
                       CDT_Sync);
}
...
void C4PlayerInfoListBox::ClientListItem::OnBtnAddPlr(C4GUI::Control *btn) {
  ...
  GetScreen()->ShowRemoveDlg(
      new C4PlayerSelDlg(new C4FileSel_CBEx<C4GameLobby::MainDlg>(
          GetLobby(), &C4GameLobby::MainDlg::OnClientAddPlayer, idClient)));
}

// ----------- TeamListItem ---------------------------------------------

C4PlayerInfoListBox::TeamListItem::TeamListItem(
  ...
  if (idTeam == TEAMID_Unknown)
    ...
    if (pTeam)
      ...
      szTeamName = "INTERNAL TEAM ERROR";
  }
  // set ID
  ...
  pNameLabel = new C4GUI::Label(szTeamName, iIconSize + IconLabelSpacing,
                                (iIconSize - pFont->GetLineHeight()) / 2, ALeft,
                                pList->GetTextColor(), pFont, false);
  if (fEvaluation && pTeam && pTeam->GetIconSpec() && *pTeam->GetIconSpec()) {
    Game.DrawTextSpecImage(pIcon->GetMFacet(), pTeam->GetIconSpec(),
                           pTeam->GetColor());
  }
  // calc own bounds
  C4Rect rcOwnBounds = pNameLabel->GetBounds();
...
void C4PlayerInfoListBox::TeamListItem::MouseInput(C4GUI::CMouse &rMouse,
...
void C4PlayerInfoListBox::TeamListItem::UpdateOwnPos() {
  ...
  if (pList->IsEvaluation()) {
    int32_t iTotalWdt =
        pIcon->GetBounds().Wdt + IconLabelSpacing +
        C4GUI::GetRes()->TitleFont.GetTextWidth(pNameLabel->GetText());
    C4GUI::ComponentAligner caAll(GetContainedClientRect(), 0, 0);
    C4GUI::ComponentAligner caBounds(
        caAll.GetCentered(iTotalWdt, caAll.GetInnerHeight()), 0, 0);
    pIcon->SetBounds(
        caBounds.GetFromLeft(pIcon->GetBounds().Wdt, pIcon->GetBounds().Hgt));
    pNameLabel->SetBounds(caBounds.GetCentered(
        caBounds.GetInnerWidth(), C4GUI::GetRes()->TitleFont.GetLineHeight()));
  }
}
...
C4PlayerInfoListBox::FreeSavegamePlayersListItem::FreeSavegamePlayersListItem(
  ...
  int32_t iIconSize = C4GUI::GetRes()->TextFont.GetLineHeight();
  // create subcomponents
  pIcon = new C4GUI::Icon(C4Rect(0, 0, iIconSize, iIconSize),
                          C4GUI::Ico_SavegamePlayer);
  pNameLabel = new C4GUI::Label(LoadResStr("IDS_MSG_FREESAVEGAMEPLRS"),
                                iIconSize + IconLabelSpacing, 0, ALeft);
  // calc own bounds
  C4Rect rcOwnBounds = pNameLabel->GetBounds();
  rcOwnBounds.Wdt += rcOwnBounds.x;
  rcOwnBounds.x = 0;
...
C4PlayerInfoListBox::ScriptPlayersListItem::ScriptPlayersListItem(
    C4PlayerInfoListBox *pForListBox, ListItem *pInsertBefore)
    : ListItem(pForListBox) {
  // set ID
  ...
  // create subcomponents
  pIcon =
      new C4GUI::Icon(C4Rect(0, 0, iIconSize, iIconSize), C4GUI::Ico_Record);
  pNameLabel = new C4GUI::Label(LoadResStr("IDS_CTL_SCRIPTPLAYERS"),
                                iIconSize + IconLabelSpacing, 0, ALeft);
  btnAddPlayer = NULL;
  if (Game.Control.isCtrlHost()) {
    btnAddPlayer =
        new C4GUI::CallbackButton<ScriptPlayersListItem, C4GUI::IconButton>(
            C4GUI::Ico_AddPlr, C4Rect(0, 0, iIconSize, iIconSize),
            'A' /* 2do TODO */, &ScriptPlayersListItem::OnBtnAddPlr, this);
  }
  // calc own bounds
  C4Rect rcOwnBounds = pNameLabel->GetBounds();
  rcOwnBounds.Wdt += rcOwnBounds.x;
...
void C4PlayerInfoListBox::ScriptPlayersListItem::Update() {
  ...
  if (btnAddPlayer) {
    bool fCanJoinScriptPlayers =
        (Game.Teams.GetMaxScriptPlayers() -
             Game.PlayerInfos.GetActiveScriptPlayerCount(true, true) >
         0);
    btnAddPlayer->SetVisibility(fCanJoinScriptPlayers);
  }
}

void C4PlayerInfoListBox::ScriptPlayersListItem::OnBtnAddPlr(
    C4GUI::Control *btn) {
  // safety
  int32_t iCurrScriptPlrCount =
      Game.PlayerInfos.GetActiveScriptPlayerCount(true, true);
  bool fCanJoinScriptPlayers =
      (Game.Teams.GetMaxScriptPlayers() - iCurrScriptPlrCount > 0);
  ...
  pScriptPlrInfo->SetAsScriptPlayer(
      Game.Teams.GetScriptPlayerName().getData(),
      GenerateRandomPlayerColor(iCurrScriptPlrCount), 0, C4ID_None);
...
C4PlayerInfoListBox::ReplayPlayersListItem::ReplayPlayersListItem(
    C4PlayerInfoListBox *pForListBox, ListItem *pInsertBefore)
    : ListItem(pForListBox) {
  // set ID
  ...
  pIcon =
      new C4GUI::Icon(C4Rect(0, 0, iIconSize, iIconSize), C4GUI::Ico_Record);
  pNameLabel = new C4GUI::Label(LoadResStr("IDS_MSG_REPLAYPLRS"),
                                iIconSize + IconLabelSpacing, 0, ALeft);
  // calc own bounds
  C4Rect rcOwnBounds = pNameLabel->GetBounds();
  rcOwnBounds.Wdt += rcOwnBounds.x;
...
C4PlayerInfoListBox::C4PlayerInfoListBox(const C4Rect &rcBounds, Mode eMode,
                                         int32_t iTeamFilter)
    : C4GUI::ListBox(rcBounds),
      fIsCollapsed(false),
      iMaxUncollapsedPlayers(10),
      eMode(eMode),
      iTeamFilter(iTeamFilter),
      dwTextColor(C4GUI_MessageFontClr),
      pCustomFont(NULL) {
  // update if client listbox selection changes
  SetSelectionChangeCallbackFn(new C4GUI::CallbackHandler<C4PlayerInfoListBox>(
      this, &C4PlayerInfoListBox::OnPlrListSelChange));
  // initial update
  Update();
}
...
C4PlayerInfoListBox::ListItem *C4PlayerInfoListBox::GetPlayerListItem(
    ListItem::ID::IDType eType, int32_t id) {
  // safety
  if (!C4GUI::IsGUIValid()) return NULL;
  ListItem::ID idSearch(eType, id);
  // search through listbox
  for (C4GUI::Element *pEItem = GetFirst(); pEItem;
       pEItem = pEItem->GetNext()) {
...
bool C4PlayerInfoListBox::PlrListItemUpdate(ListItem::ID::IDType eType,
                                            int32_t id,
...
void C4PlayerInfoListBox::Update() {
  ...
  ListItem *pCurrInList = static_cast<ListItem *>(
      GetFirst());  // list item being compared with the searched item

  // free savegame players first
  UpdateSavegamePlayers(&pCurrInList);

  // next comes the regular players, sorted either by clients or teams, or
  // special sort for evaluation mode
  switch (eMode) {
  ...
  if (!fIsCollapsed && IsScrollingActive() && !IsEvaluation()) {
    ...
    Update();  // recursive call!
             ...
             Game.PlayerInfos.GetPlayerCount() <= iMaxUncollapsedPlayers) {
    // player count dropped below collapse-limit: uncollapse
    // note that this may again cause a collapse after that update, if scrolling
    // was still necessary
    // however, it will then not recurse any further, because
    // iMaxUncollapsedPlayers will have been updated
    fIsCollapsed = false;
    Update();
  }
}
...
void C4PlayerInfoListBox::UpdateSavegamePlayers(ListItem **ppCurrInList) {
  ...
  if (Game.RestorePlayerInfos.GetActivePlayerCount(true) -
      Game.RestorePlayerInfos.GetActiveScriptPlayerCount(true, true)) {
    // caption
    ...
    while (pInfo = Game.RestorePlayerInfos.GetNextPlayerInfoByID(iInfoID)) {
      ...
      // skip script controlled - those are put into the script controlled
      // player list
      if (pInfo->GetType() == C4PT_Script) continue;
      // players are in the list
      fAnyPlayers = true;
      // show them
      if (!PlrListItemUpdate(ListItem::ID::PLI_SAVEGAMEPLR, iInfoID,
                             ppCurrInList))
...
void C4PlayerInfoListBox::UpdateScriptPlayers(ListItem **ppCurrInList) {
  // script controlled players from the main list
  // processing the restore list would be redundant, because all script players
  // should have been taken over by a new script player join automatically
  // also show the label if script players can be joined
  if (Game.Teams.GetMaxScriptPlayers() ||
      Game.PlayerInfos.GetActiveScriptPlayerCount(true, false)) {
    // header
    if (!PlrListItemUpdate(ListItem::ID::PLI_SCRIPTPLR, 0, ppCurrInList))
      new ScriptPlayersListItem(this, *ppCurrInList);
    // players
    C4PlayerInfo *pInfo;
    int32_t iClientIdx = 0;
    ...
    while (pInfos = Game.PlayerInfos.GetIndexedInfo(iClientIdx++)) {
      ...
      while (pInfo = pInfos->GetPlayerInfo(iInfoIdx++)) {
        ...
        if (!PlrListItemUpdate(ListItem::ID::PLI_PLAYER, iInfoID, ppCurrInList))
          new PlayerListItem(this, pInfos->GetClientID(), iInfoID, false,
                             *ppCurrInList);
      }
    }
  }
...
void C4PlayerInfoListBox::UpdatePlayersByTeam(ListItem **ppCurrInList) {
  ...
  while (pTeam = Game.Teams.GetTeamByIndex(i++)) {
    ...
    // the team label
    if (!PlrListItemUpdate(ListItem::ID::PLI_TEAM, pTeam->GetID(),
                           ppCurrInList))
    ...
    while (idPlr = pTeam->GetIndexedPlayer(j++))
      if (pPlrInfo = Game.PlayerInfos.GetPlayerInfoByID(idPlr, &idClient))
        if (!pPlrInfo->IsInvisible())
          if ((pClient = Game.Clients.getClientByID(idClient)) &&
              pClient->isActivated())
            if (!PlrListItemUpdate(ListItem::ID::PLI_PLAYER, idPlr,
                                   ppCurrInList))
...
void C4PlayerInfoListBox::UpdatePlayersByRandomTeam(ListItem **ppCurrInList) {
  // team sort but teams set to random and invisible: Show all players within
  // one "Random Team"-label
  ...
  while (pClient = Game.Clients.getClient(pClient)) {
    ...
    C4ClientPlayerInfos *pInfoPacket =
        Game.PlayerInfos.GetInfoByClientID(pClient->getID());
    if (pInfoPacket) {
      C4PlayerInfo *pPlrInfo;
      int32_t i = 0;
      while (pPlrInfo = pInfoPacket->GetPlayerInfo(i++)) {
        ...
        if (!fTeamLabelPut) {
          if (!PlrListItemUpdate(ListItem::ID::PLI_TEAM, TEAMID_Unknown,
                                 ppCurrInList))
            new TeamListItem(this, TEAMID_Unknown, *ppCurrInList);
          fTeamLabelPut = true;
        }
        if (!PlrListItemUpdate(ListItem::ID::PLI_PLAYER, pPlrInfo->GetID(),
                               ppCurrInList))
          new PlayerListItem(this, pClient->getID(), pPlrInfo->GetID(), false,
                             *ppCurrInList);
      }
    }
  }
...
void C4PlayerInfoListBox::UpdatePlayersByClient(ListItem **ppCurrInList) {
  ...
  while (pClient = Game.Clients.getClient(pClient)) {
    // the client label
    if (!PlrListItemUpdate(ListItem::ID::PLI_CLIENT, pClient->getID(),
                           ppCurrInList))
      new ClientListItem(this, pClient->getCore(), *ppCurrInList);
    // player infos for this client - not for observers, and never in replays
    // could also check for activated here. However, non-observers will usually
    // be activated later and thus be using their players
...
void C4PlayerInfoListBox::UpdatePlayersByEvaluation(ListItem **ppCurrInList,
                                                    bool fShowWinners) {
  ...
  AddMode *pAddModes;
  int32_t iAddModeCount;
  if (fShowWinners) {
    pAddModes = pShowWinnersAddModes;
    iAddModeCount = 2;
  } else {
    pAddModes = pHideWinnersAddModes;
    iAddModeCount = 1;
  }
  for (int32_t iAddMode = 0; iAddMode < iAddModeCount; ++iAddMode) {
    ...
    if (iTeamFilter) {
      ...
      UpdatePlayersByEvaluation(ppCurrInList,
                                Game.Teams.GetTeamByID(iTeamFilter), eAddMode);
    } else {
...
void C4PlayerInfoListBox::UpdatePlayersByEvaluation(
    ListItem **ppCurrInList, C4Team *pTeam,
    C4PlayerInfoListBox::AddMode eWinMode) {
  // check winning status of team first
  if (pTeam && eWinMode != AM_All)
    if (pTeam->HasWon() != (eWinMode == AM_Winners)) return;
  ...
  while (pInfoPacket = Game.PlayerInfos.GetIndexedInfo(iClient++)) {
    ...
    while (pPlrInfo = pInfoPacket->GetPlayerInfo(i++)) {
      ...
      if (pPlrInfo->IsInvisible()) continue;
      if (!pTeam && eWinMode != AM_All &&
          pPlrInfo->HasWon() != (eWinMode == AM_Winners))
        continue;
      if (!PlrListItemUpdate(ListItem::ID::PLI_PLAYER, pPlrInfo->GetID(),
                             ppCurrInList))
        new PlayerListItem(this, pInfoPacket->GetClientID(), pPlrInfo->GetID(),
                           false, *ppCurrInList);
    }
  }
}
...
void C4PlayerInfoListBox::SetCustomFont(CStdFont *pNewFont,
