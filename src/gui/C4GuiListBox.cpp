...
ListBox::ListBox(const C4Rect &rtBounds, int32_t iMultiColItemWidth)
    : Control(rtBounds),
      iMultiColItemWidth(iMultiColItemWidth),
      iColCount(1),
      pSelectedItem(NULL),
      pSelectionChangeHandler(NULL),
      pSelectionDblClickHandler(NULL),
  ...
  // create key bindings
  pKeyContext =
      new C4KeyBinding(C4KeyCodeEx(K_MENU), "GUIListBoxContext", KEYSCOPE_Gui,
                       new ControlKeyCB<ListBox>(*this, &ListBox::KeyContext),
  ...
  pKeyUp = new C4KeyBinding(keys, "GUIListBoxUp", KEYSCOPE_Gui,
                            new ControlKeyCB<ListBox>(*this, &ListBox::KeyUp),
                            C4CustomKey::PRIO_Ctrl);
  keys.clear();
  ...
  pKeyDown =
      new C4KeyBinding(keys, "GUIListBoxDown", KEYSCOPE_Gui,
                       new ControlKeyCB<ListBox>(*this, &ListBox::KeyDown),
                       C4CustomKey::PRIO_Ctrl);
  keys.clear();
  ...
  pKeyLeft =
      new C4KeyBinding(keys, "GUIListBoxLeft", KEYSCOPE_Gui,
                       new ControlKeyCB<ListBox>(*this, &ListBox::KeyLeft),
                       C4CustomKey::PRIO_Ctrl);
  keys.clear();
  ...
  pKeyRight =
      new C4KeyBinding(keys, "GUIListBoxRight", KEYSCOPE_Gui,
                       new ControlKeyCB<ListBox>(*this, &ListBox::KeyRight),
                       C4CustomKey::PRIO_Ctrl);
  pKeyPageUp =
      new C4KeyBinding(C4KeyCodeEx(K_PAGEUP), "GUIListBoxPageUp", KEYSCOPE_Gui,
                       new ControlKeyCB<ListBox>(*this, &ListBox::KeyPageUp),
                       C4CustomKey::PRIO_Ctrl);
  pKeyPageDown = new C4KeyBinding(
      C4KeyCodeEx(K_PAGEDOWN), "GUIListBoxPageDown", KEYSCOPE_Gui,
      new ControlKeyCB<ListBox>(*this, &ListBox::KeyPageDown),
      C4CustomKey::PRIO_Ctrl);
  pKeyHome =
      new C4KeyBinding(C4KeyCodeEx(K_HOME), "GUIListBoxHome", KEYSCOPE_Gui,
                       new ControlKeyCB<ListBox>(*this, &ListBox::KeyHome),
                       C4CustomKey::PRIO_Ctrl);
  pKeyEnd = new C4KeyBinding(C4KeyCodeEx(K_END), "GUIListBoxEnd", KEYSCOPE_Gui,
                             new ControlKeyCB<ListBox>(*this, &ListBox::KeyEnd),
                             C4CustomKey::PRIO_Ctrl);
  ...
  pKeyActivate =
      new C4KeyBinding(keys, "GUIListActivate", KEYSCOPE_Gui,
                       new ControlKeyCB<ListBox>(*this, &ListBox::KeyActivate),
                       C4CustomKey::PRIO_Ctrl);
}
...
void ListBox::DrawElement(C4FacetEx &cgo) {
  ...
  if (pSelectedItem) {
    ...
    rcSelArea.Hgt = Min(rcSelArea.Hgt,
                        GetClientRect().y + GetClientRect().Hgt - rcSelArea.y);
    // draw
    if (rcSelArea.Hgt >= 0)
      lpDDraw->DrawBoxDw(
          cgo.Surface, rcSelArea.x + cgo.TargetX, rcSelArea.y + cgo.TargetY,
          rcSelArea.x + rcSelArea.Wdt + cgo.TargetX - 1,
          rcSelArea.y + rcSelArea.Hgt + cgo.TargetY - 1,
  ...
  while (pCurr = pCurr->GetNext())
    if (pCurr->GetListItemTopSpacingBar()) {
      ...
      int32_t iY = pCurr->GetBounds().y + GetClientRect().y +
                   pClientWindow->GetClientRect().y - iYSpace / 2;
      int32_t iX0 =
          pCurr->GetBounds().x + GetClientRect().x + C4GUI_ListBoxBarIndent;
      int32_t iX1 =
          iX0 + pClientWindow->GetClientRect().Wdt - 2 * C4GUI_ListBoxBarIndent;
      // clip
      if (iY < GetClientRect().y ||
          iY >= GetClientRect().y + GetClientRect().Hgt)
        continue;
      // draw
      lpDDraw->DrawLineDw(cgo.Surface, (float)(iX0 + cgo.TargetX),
                          (float)(iY + cgo.TargetY), (float)(iX1 + cgo.TargetX),
                          (float)(iY + cgo.TargetY), C4GUI_ListBoxBarColor);
    }
}

void ListBox::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
  ...
  if (pClientWindow) {
    ...
    if (pClientWindow->GetBounds().Contains(iX, iY))
      ...
      if (iButton == C4MC_Button_LeftDown ||
          iButton == C4MC_Button_LeftDouble) {
        // reset selection
        ...
        for (Element *pCurr = pClientWindow->GetFirst(); pCurr;
             pCurr = pCurr->GetNext())
          if (pCurr->GetBounds().Contains(iX, iY)) pSelectedItem = pCurr;
...
void ListBox::UpdateColumnCount() {
  if (iMultiColItemWidth && pClientWindow) {
    ...
    iColCount = Max<int32_t>(
        pClientWindow->GetClientRect().Wdt / iMultiColItemWidth, 1);
  } else {
...
int32_t ListBox::ContractToElementHeight() {
  ...
  int32_t iExtraSpace =
      pClientWindow->GetBounds().Hgt - pClientWindow->GetClientRect().Hgt;
...
void ListBox::OnGetFocus(bool fByMouse) {
  ...
  // select list item if none is selected (only for keyboard; mouse will select
  // with left-click anyway)
...
bool ListBox::KeyPageDown() {
  ...
  Element *pNextSelectedItem = pSelectedItem
                                   ? pSelectedItem
                                   : pClientWindow->GetFirstContained(),
          *pNext;
  if (!pNextSelectedItem) return false;
  if (pNext = pNextSelectedItem->GetNext()) {
    pNextSelectedItem = pNext;
    // if this is not the last, visible item in the list: go down until item is
    // no longer fully in view
    if (pClientWindow->IsRangeInView(pNextSelectedItem->GetBounds().y,
                                     pNextSelectedItem->GetBounds().Hgt)) {
      while (pNext = pNextSelectedItem->GetNext())
        if (pClientWindow->IsRangeInView(pNext->GetBounds().y,
                                         pNext->GetBounds().Hgt))
          pNextSelectedItem = pNext;
        else
          break;
    } else {
      // selected item was last visible: Just scroll one page down and select
      // last visible
      pClientWindow->ScrollPages(+1);
      pNextSelectedItem = pClientWindow->GetLastContained();
      while (!pClientWindow->IsRangeInView(pNextSelectedItem->GetBounds().y,
                                           pNextSelectedItem->GetBounds().Hgt))
...
bool ListBox::KeyPageUp() {
  ...
  Element *pNextSelectedItem =
              pSelectedItem ? pSelectedItem : pClientWindow->GetLastContained(),
          *pNext;
  if (!pNextSelectedItem) return false;
  if (pNext = pNextSelectedItem->GetPrev()) {
    pNextSelectedItem = pNext;
    // if this is not the first, visible item in the list: go up until item is
    // no longer fully in view
    if (pClientWindow->IsRangeInView(pNextSelectedItem->GetBounds().y,
                                     pNextSelectedItem->GetBounds().Hgt)) {
      ...
      // selected item was last visible: Just scroll one page up and select
      // first visible
      pClientWindow->ScrollPages(-1);
...
void ListBox::ScrollItemInView(Element *pItem) {
  ...
  pClientWindow->ScrollRangeInView(pItem->GetBounds().y,
                                   pItem->GetBounds().Hgt);
}
  ...
  if (pCurr) {
    if (!iMultiColItemWidth) {
      ...
      for (; pCurr; pCurr = pCurr->GetNext()) {
        ...
        if (rcCurrBounds.x != x || rcCurrBounds.y != y ||
            rcCurrBounds.Wdt != iMultiColItemWidth)
    ...
    if (iMultiColItemWidth) {
      ...
      if (col >= iColCount) {
        ...
        while (cnt-- && pPrevChild) {
          iPrevLineHgt =
              Max<int32_t>(iPrevLineHgt, pPrevChild->GetBounds().Hgt);
      ...
      rcChildBounds.y = pChild->GetPrev()->GetBounds().y +
                        pChild->GetPrev()->GetBounds().Hgt +
                        pChild->GetListItemTopSpacing();
    }
  } else
    rcChildBounds.y = 0;
  pChild->UpdateOwnPos();
...
bool ListBox::InsertElement(Element *pChild, Element *pInsertBefore,
                            int32_t iIndent) {
  // fail if no client window is present
  if (!pClientWindow) return false;
  // add to scroll window
...
void ListBox::SelectionChanged(bool fByUser) {
  ...
  // because scrolling the item into view must be done AFTER the callback, as
  // the callback might resize
...
BOOL ListBox::CharIn(const char *c) {
  ...
  while (pSel != pStartCheck && !pSel->CheckNameHotkey(c))
    if (!(pSel = pSel->GetNext()))
      if (pStartCheck)
        // list end reached while another entry had been selected before:
        // Re-check start of list
        pSel = GetFirst();
...
void ListBox::SortElements(SortFunction SortFunc, void *par) {
  ...
  try {
    ...
    for (Element *pEl = pClientWindow->GetFirst(); pEl; pEl = pEl->GetNext())
      ppElements[i++] = pEl;
    std::sort(ppElements, ppElements + iElemCount,
              SortCompareElements(SortFunc, par));
    for (i = 0; i < iElemCount; ++i) pClientWindow->ReaddElement(ppElements[i]);
  } catch (...) {
    delete[] ppElements;
    throw;
  }
