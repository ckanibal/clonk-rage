...
Button::Button(const char *szBtnText, const C4Rect &rtBounds)
  ...
  pKeyButton =
      new C4KeyBinding(keys, "GUIButtonPress", KEYSCOPE_Gui,
                       new ControlKeyCB<Button>(*this, &Button::KeyButtonDown,
                                                &Button::KeyButtonUp),
...
Button::~Button() { delete pKeyButton; }
...
void Button::DrawElement(C4FacetEx &cgo) {
  ...
  if (fEnabled)
    if (HasDrawFocus() || (fMouseOver && IsInActiveDlg(false))) {
      ...
      GetRes()->fctButtonHighlight.DrawX(cgo.Surface, x0 + 5, y0 + 3,
                                         rcBounds.Wdt - 10, rcBounds.Hgt - 6);
      lpDDraw->ResetBlitMode();
    }
  // draw text
  int32_t iTextHgt = rcBounds.Hgt - 2;
  CStdFont &rUseFont = (GetRes()->TitleFont.GetLineHeight() > iTextHgt
  ...
  // lpDDraw->TextOut(Text, rShadowFont, (float)
  // iTextHgt/rShadowFont.GetLineHeight(), cgo.Surface, (x0+x1)/2 + iTxtOff,
  // (y0+y1-iTextHgt)/2 + iTxtOff, C4GUI_ButtonFontShadowClr, ACenter, true);
  lpDDraw->TextOut(sText.getData(), rUseFont, 1.0f, cgo.Surface,
                   (x0 + x1) / 2 + iTxtOff, (y0 + y1 - iTextHgt) / 2 + iTxtOff,
                   C4GUI_ButtonFontClr, ACenter, true);
}
...
void Button::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX, int32_t iY,
                        DWORD dwKeyParam) {
  // inherited
  ...
  if (fEnabled) switch (iButton) {
      case C4MC_Button_LeftDown:
        ...
        // but it should be remembered if the user leaves the button with the
        // mouse down
...
void IconButton::DrawElement(C4FacetEx &cgo) {
  ...
  // draw selection highlight
  if (fEnabled)
    if (fHighlight || HasDrawFocus() || (fMouseOver && IsInActiveDlg(false))) {
      lpDDraw->SetBlitMode(C4GFXBLIT_ADDITIVE);
  ...
  if (sText.getLength()) {
    ...
    lpDDraw->TextOut(sText.getData(), rUseFont, 1.0f, cgo.Surface,
                     x0 + rcBounds.Wdt / 2,
                     y0 + rcBounds.Hgt - rUseFont.GetLineHeight() * 4 / 5,
                     C4GUI_CaptionFontClr, ACenter);
  }
}
...
void IconButton::SetIcon(Icons eUseIcon) {
  if (eUseIcon >= 0)
    fctIcon = Icon::GetIconFacet(eUseIcon);
  else
    fctIcon.Surface = NULL;
}
...
void ArrowButton::DrawElement(C4FacetEx &cgo) {
  ...
  GetRes()->fctBigArrows.GetPhase(iFctIdx).DrawX(cgo.Surface, x0, y0,
                                                 rcBounds.Wdt, rcBounds.Hgt);
}
...
FacetButton::FacetButton(const C4Facet &rBaseFct, const C4Facet &rHighlightFct,
                         const FLOAT_RECT &rtfBounds, char cHotkey)
    : Button("", C4Rect(rtfBounds)),
      fctBase(rBaseFct),
      fctHighlight(rHighlightFct),
      rcfDrawBounds(rtfBounds),
      dwTextClrInact(0x7f000000),
      dwTextClrAct(0xff000000),
      pFont(NULL),
      fFontZoom(1.0f) {
  // ctor
  this->cHotkey = cHotkey;
...
void FacetButton::DrawElement(C4FacetEx &cgo) {
  // get drawing bounds
  float x0 = rcfDrawBounds.left + cgo.TargetX,
        y0 = rcfDrawBounds.top + cgo.TargetY;
  // draw button or highlight facet
  uint32_t dwTextClr;
  if ((HasDrawFocus() || (fMouseOver && IsInActiveDlg(false))) && fEnabled) {
    fctHighlight.DrawXFloat(cgo.Surface, x0, y0,
                            rcfDrawBounds.right - rcfDrawBounds.left,
                            rcfDrawBounds.bottom - rcfDrawBounds.top);
    dwTextClr = dwTextClrAct;
  } else {
    fctBase.DrawXFloat(cgo.Surface, x0, y0,
                       rcfDrawBounds.right - rcfDrawBounds.left,
                       rcfDrawBounds.bottom - rcfDrawBounds.top);
  ...
  if (sText.getLength() > 0) {
    CStdFont *pUseFont =
        pFont ? pFont : &(GetRes()->GetFontByHeight(rcBounds.Hgt, &fFontZoom));
    lpDDraw->TextOut(sText.getData(), *pUseFont, fFontZoom, cgo.Surface,
                     (int)(x0 + iTxtOffX), (int)(y0 + iTxtOffY), dwTextClr,
                     byTxtAlign, true);
  }
  /*
  if (fEnabled) if (fMouseOver && IsInActiveDlg(false))
          {
          lpDDraw->SetBlitMode(C4GFXBLIT_ADDITIVE);
          GetRes()->fctButtonHighlight.DrawXFloat(cgo.Surface, x0, y0,
  rcfDrawBounds.right-rcfDrawBounds.left,
  rcfDrawBounds.bottom-rcfDrawBounds.top);
          lpDDraw->ResetBlitMode();
          }*/
