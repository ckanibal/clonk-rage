...
void C4Scoreboard::Entry::SwapWith(Entry *pSwap) {
  ...
  swp.Text.Take(Text);
  swp.iVal = iVal;
  Text.Take(pSwap->Text);
  iVal = pSwap->iVal;
  pSwap->Text.Take(swp.Text);
  pSwap->iVal = swp.iVal;
}
...
void C4Scoreboard::DelCol(int32_t iDelIndex) {
  ...
  Entry *pCpy = pNewEntries;
  for (int32_t iRow = 0; iRow < iRows; ++iRow)
    for (int32_t iCol = 0, iFromCol = 0; iCol < (iCols - 1);
         ++iCol, ++iFromCol) {
...
void C4Scoreboard::SetCell(int32_t iColKey, int32_t iRowKey,
  ...
  if (!szValue || !*szValue) {
    ...
    if (iRow) {
      for (i = 1; i < iCols; ++i)
        if (GetCell(i, iRow)->Text) break;
      if (i == iCols) DelRow(iRow);
    }
    // prune empty col (not label col)
    if (iCol) {
      for (i = 1; i < iRows; ++i)
        if (GetCell(iCol, i)->Text) break;
...
bool C4Scoreboard::SortBy(int32_t iColKey, bool fReverse) {
  ...
  while (iSortBegin < iSortEnd) {
    int32_t iNewBorder = iSortBegin;
    int32_t i;
    for (i = iSortBegin; i < iSortEnd; ++i)
      if (GetCell(iCol, i)->iVal * iSortDir >
    ...
    for (i = iSortEnd; i > iSortBegin; --i)
      if (GetCell(iCol, i - 1)->iVal * iSortDir >
...
void C4Scoreboard::DoDlgShow(int32_t iChange, bool fUserToggle) {
  ...
  if (!fUserToggle)
    ...
    fUserToggle = (ShouldBeShown() == !pDlg);
      ...
      // user pressed Tab: Always toggle except if the scoreboard cannot be
      // shown at all
      if (!CanBeShown() && !pDlg)
    fUserToggle = false;
  if (fUserToggle) {
    ...
    for (int32_t iRow = 0; iRow < iRows; ++iRow)
      for (int32_t iCol = 0; iCol < iCols; ++iCol) {
        ...
        pComp->Value(mkNamingAdapt(
            pEnt->Text, FormatString("Cell%i_%iString", iCol, iRow).getData()));
        pComp->Value(mkNamingAdapt(
            pEnt->iVal, FormatString("Cell%i_%iValue", iCol, iRow).getData()));
      }
    // recheck dlg show in read mode
    // will usually not do anything, because reading is done before enetering
    // shared mode
...
C4ScoreboardDlg::C4ScoreboardDlg(C4Scoreboard *pForScoreboard)
    : C4GUI::Dialog(100, 100, "nops", false),
      piColWidths(NULL),
      pBrd(pForScoreboard) {
  Update();
}
...
void C4ScoreboardDlg::Update() {
  // counts
  int32_t iRowCount = pBrd->iRows;
  int32_t iColCount = pBrd->iCols;
  delete[] piColWidths;
  piColWidths = NULL;
  // invalid board - scipters can create those, but there's no reason why the
  // engine
  ...
  for (int32_t iCol = 0; iCol < iColCount; ++iCol) {
    ...
    for (int32_t iRow = 0; iRow < iRowCount; ++iRow) {
      ...
      if ((iRow || iCol) && !pCell->Text.isNull())
        piColWidths[iCol] = Max<int32_t>(
            piColWidths[iCol], Game.GraphicsResource.FontRegular.GetTextWidth(
                                   pCell->Text.getData()) +
                                   XIndent);
    }
  ...
  iHgt +=
      iRowCount * (Game.GraphicsResource.FontRegular.GetLineHeight() + YIndent);
  const char *szTitle = pBrd->GetCell(0, 0)->Text.getData();
  if (szTitle)
    iWdt = Max<int32_t>(
        iWdt, Game.GraphicsResource.FontRegular.GetTextWidth(szTitle) + 40);
  ...
  SetTitle(szTitle);
  if (szTitle && pTitle)
    pTitle->SetIcon(C4GUI::Icon::GetIconFacet(C4GUI::Ico_Player));
  // realign
  C4GUI::Screen *pScr = GetScreen();
  if (pScr) DoPlacement(pScr, pScr->GetPreferredDlgRect());
}

bool C4ScoreboardDlg::DoPlacement(C4GUI::Screen *pOnScreen,
                                  const C4Rect &rPreferredDlgRect) {
  // align topright
  SetPos(rPreferredDlgRect.x + rPreferredDlgRect.Wdt - rcBounds.Wdt - 20,
         rPreferredDlgRect.y + 38);
  return true;
}
  ...
  for (int32_t iRow = 0; iRow < iRowCount; ++iRow) {
    ...
    for (int32_t iCol = 0; iCol < iColCount; ++iCol) {
      ...
      if (szText && *szText && (iRow || iCol))
        lpDDraw->TextOut(szText, Game.GraphicsResource.FontRegular, 1.0,
                         cgo.Surface, iCol ? iX + piColWidths[iCol] / 2 : iX,
                         iY, 0xffffffff, iCol ? ACenter : ALeft);
      iX += piColWidths[iCol];
    }
