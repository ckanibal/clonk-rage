...
template <class CB>
class C4FileSel_CBEx : public C4FileSel_BaseCB {
 public:
 ...
 public:
  C4FileSel_CBEx(CB *pCBClass, FileSelFunc SelFunc, int32_t idToken)
      : pCBClass(pCBClass), SelFunc(SelFunc), idToken(idToken) {}
};
...
class C4FileSelDlg : public C4GUI::Dialog {
 public:
  ...
  class DefaultListItem : public ListItem {
   ...
   public:
    DefaultListItem(const char *szFilename, bool fTruncateExtension,
 ...
 protected:
  ...
  virtual void UserClose(
      bool fOK);  // allow OK only if something is sth is selected
  ...
  virtual int32_t GetFileSelColWidth() const {
    return 0;
  }  // width of each file selection element; 0 for default all listbox
  ...
  bool OnLocationComboSelChange(C4GUI::ComboBox *pForCombo,
                                int32_t idNewSelection);

  void AddLocation(
      const char *szName,
      const char *szPath);  // add location to be shown in dropdown list
  void AddCheckedLocation(const char *szName,
                          const char *szPath);  // add location to be shown in
                                                // dropdown list, only if path
                                                // exists and isn't added yet
 ...
 public:
  C4FileSelDlg(const char *szRootPath, const char *szTitle,
               C4FileSel_BaseCB *pSelCallback, bool fInitElements = true);
  ...
  StdStrBuf GetSelection(const char *szFixedSelection,
                         bool fFilenameOnly) const;  // get single selected file
                                                     // for single selection dlg
                                                     // ';'-seperated list for
                                                     // multi selection dlg
};
...
class C4DefinitionSelDlg : public C4FileSelDlg {
 ...
 public:
  C4DefinitionSelDlg(C4FileSel_BaseCB *pSelCallback,
                     const char *szFixedSelection);

  static bool SelectDefinitions(C4GUI::Screen *pOnScreen,
...
class C4PortraitSelDlg : public C4FileSelDlg {
 ...
 private:
  class ListItem : public C4FileSelDlg::ListItem {
   private:
    ...
    C4FacetExSurface fctLoadedImage;  // image as loaded by background thread.
                                      // Must be scaled by main thread
  ...
  class LoaderThread : public StdThread {
   ...
   public:
    ...
    void AddLoadItem(
        ListItem *pItem);  // not to be called when thread is running!
 ...
 protected:
  ...
  virtual bool HasNoneItem() const {
    return true;
  }  // if true, a special <none> item can be selected
  virtual bool HasPreviewArea() const {
    return false;
  }  // no preview area. Preview images directly
  virtual bool HasExtraOptions() const { return true; }
  virtual void AddExtraOptions(const C4Rect &rcOptionsRect);
  virtual int32_t GetFileSelColWidth() const {
    return ImagePreviewSize;
  }  // width of each file selection element
 ...
 public:
  C4PortraitSelDlg(C4FileSel_BaseCB *pSelCallback, bool fSetPicture,
                   bool fSetBigIcon);

  bool IsSetPicture() const {
    return pCheckSetPicture ? pCheckSetPicture->GetChecked() : fDefSetPicture;
  }
  bool IsSetBigIcon() const {
    return pCheckSetBigIcon ? pCheckSetBigIcon->GetChecked() : fDefSetBigIcon;
  }

  static bool SelectPortrait(C4GUI::Screen *pOnScreen, StdStrBuf *pSelection,
                             bool *pfSetPicture, bool *pfSetBigIcon);
};
