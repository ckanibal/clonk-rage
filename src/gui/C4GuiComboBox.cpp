...
void ComboBox_FillCB::AddEntry(const char *szText, int32_t id) {
  ...
  pDrop->AddItem(szText,
                 FormatString(LoadResStr("IDS_MSG_SELECT"), szText).getData(),
                 Ico_Empty, pHandler);
}

bool ComboBox_FillCB::FindEntry(const char *szText) {
  // check for entry with same name
  ContextMenu::Entry *pEntry;
  int32_t idx = 0;
  while (pEntry = pDrop->GetIndexedEntry(idx++))
    if (SEqual(pEntry->GetText(), szText)) return true;
  return false;
}

void ComboBox_FillCB::SelectEntry(int32_t iEntry) { pDrop->SelectItem(iEntry); }

void ComboBox_FillCB::ClearEntries() { pDrop->Clear(); }
...
ComboBox::ComboBox(const C4Rect &rtBounds)
    : Control(rtBounds),
      ...
      fMouseOver(false),
      pUseFont(NULL),
      dwFontClr(C4GUI_ComboFontClr),
      dwBGClr(C4GUI_StandardBGColor),
      dwBorderClr(0),
      pFctSideArrow(NULL) {
  *Text = 0;
  ...
  pKeyOpenCombo = new C4KeyBinding(
      cbKeys, "GUIComboOpen", KEYSCOPE_Gui,
      new ControlKeyCB<ComboBox>(*this, &ComboBox::KeyDropDown),
  ...
  pKeyCloseCombo = new C4KeyBinding(
      cbKeys, "GUIComboClose", KEYSCOPE_Gui,
      new ControlKeyCB<ComboBox>(*this, &ComboBox::KeyAbortDropDown),
      C4CustomKey::PRIO_Ctrl);
}
...
void ComboBox::DrawElement(C4FacetEx &cgo) {
  ...
  if (!fReadOnly && !fSimple) {
    // draw background
    lpDDraw->DrawBoxDw(cgo.Surface, x0, y0, x0 + rcBounds.Wdt - 1,
                       y0 + rcBounds.Hgt - 1, dwBGClr);
    // draw frame
    if (dwBorderClr) {
    ...
    (pFctSideArrow ? pFctSideArrow : &(GetRes()->fctContext))->Draw(
        cgo.Surface, iRightTextEnd,
        y0 + (rcBounds.Hgt - GetRes()->fctContext.Hgt) / 2, iOpenMenu ? 1 : 0);
  } else if (!fReadOnly) {
    // draw button in simple mode: Left of text
    (pFctSideArrow ? pFctSideArrow : &(GetRes()->fctContext))->Draw(
        cgo.Surface, x0, y0 + (rcBounds.Hgt - GetRes()->fctContext.Hgt) / 2,
        iOpenMenu ? 1 : 0);
  }
  // draw text
  if (*Text) {
    ...
    lpDDraw->SubPrimaryClipper(x0, y0, iRightTextEnd - 1,
                               y0 + rcBounds.Hgt - 1);
    lpDDraw->TextOut(
        Text, *pUseFont, 1.0f, cgo.Surface, x0 + GetRes()->fctContext.Wdt + 2,
        y0 + (rcBounds.Hgt - pUseFont->GetLineHeight()) / 2, dwFontClr, ALeft);
...
void ComboBox::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
  ...
  if (!fReadOnly)
    if (iButton == C4MC_Button_LeftDown) {
      // recheck open menu
      Screen *pScr = GetScreen();
      if (!pScr || (iOpenMenu != pScr->GetLastContextMenuIndex()))
        iOpenMenu = 0;
      if (iOpenMenu)
...
void ComboBox::SetText(const char *szToText) {
  if (szToText)
    SCopy(szToText, Text, C4MaxTitle);
  else
    *Text = 0;
}

void ComboBox::OnCtxComboSelect(C4GUI::Element *pListItem,
