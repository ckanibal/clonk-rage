...
#include "game/C4Game.h"  // only for single use of Game.GraphicsResource.fctOKCancel below...
#endif
...
// So for now, the loading will be done in 1/10 of the frames in OnIdle of the
// dialog
...
bool C4FileSelDlg::OnLocationComboSelChange(C4GUI::ComboBox *pForCombo,
...
void C4FileSelDlg::UpdateSelection() {
  ...
  if (pSelectionInfoBox) {
    ...
    if (!pSelection) {
      pSelectionInfoBox->UpdateHeight();
      return;
    }
    // add selection description
    if (pSelection->GetFilename())
      pSelectionInfoBox->AddTextLine(pSelection->GetFilename(),
                                     &C4GUI::GetRes()->TextFont,
                                     C4GUI_MessageFontClr, true, false);
  }
}
...
void C4FileSelDlg::AddLocation(const char *szName, const char *szPath) {
  ...
  delete[] pLocations;
  pLocations = pNewLocations;
  iLocationCount = iNewLocCount;
...
void C4FileSelDlg::AddCheckedLocation(const char *szName, const char *szPath) {
  ...
  for (int32_t i = 0; i < iLocationCount; ++i)
    if (ItemIdentical(szPath, pLocations[i].sPath.getData())) return;
  // OK; add it!
...
void C4FileSelDlg::SetCurrentLocation(int32_t idx, bool fRefresh) {
  ...
  if (pLocationComboBox)
    pLocationComboBox->SetText(pLocations[idx].sName.getData());
  // set new path
...
C4PlayerSelDlg::C4PlayerSelDlg(C4FileSel_BaseCB *pSelCallback)
    : C4FileSelDlg(Config.AtExePath(Config.General.PlayerPath),
                   LoadResStr("IDS_MSG_SELECTPLR"), pSelCallback) {}
...
bool C4DefinitionSelDlg::SelectDefinitions(C4GUI::Screen *pOnScreen,
                                           StdStrBuf *pSelection) {
  // let the user select definitions by showing a modal selection dialog
  C4DefinitionSelDlg *pDlg =
      new C4DefinitionSelDlg(NULL, pSelection->getData());
  bool fResult;
...
void C4PortraitSelDlg::ListItem::DrawElement(C4FacetEx &cgo) {
  ...
  if (fLoaded) {
    fLoaded = false;
    if (!fctImage.CopyFromSfcMaxSize(fctLoadedImage.GetFace(),
                                     ImagePreviewSize))
      fError = true;
  ...
  // Draw picture
  CStdFont *pUseFont = &(C4GUI::GetRes()->MiniFont);
  C4Facet cgoPicture(cgo.Surface, cgo.TargetX + rcBounds.x,
                     cgo.TargetY + rcBounds.y, ImagePreviewSize,
                     ImagePreviewSize);
  if (fError || !sFilename) {
    C4Facet &fctNoneImg = Game.GraphicsResource.fctOKCancel;
    fctNoneImg.Draw(cgoPicture.Surface,
                    cgoPicture.X + (cgoPicture.Wdt - fctNoneImg.Wdt) / 2,
                    cgoPicture.Y + (cgoPicture.Hgt - fctNoneImg.Hgt) / 2, 1, 0);
  } else {
    if (!fctImage.Surface) {
      // not loaded yet
      lpDDraw->TextOut(
          LoadResStr("IDS_PRC_INITIALIZE"), C4GUI::GetRes()->MiniFont, 1.0f,
          cgo.Surface, cgoPicture.X + cgoPicture.Wdt / 2,
          cgoPicture.Y +
              (cgoPicture.Hgt - C4GUI::GetRes()->MiniFont.GetLineHeight()) / 2,
          C4GUI_StatusFontClr, ACenter, false);
    } else {
      fctImage.Draw(cgoPicture);
    }
  }
  // draw filename
  lpDDraw->TextOut(sFilenameLabelText.getData(), *pUseFont, 1.0f, cgo.Surface,
                   cgoPicture.X + rcBounds.Wdt / 2,
                   cgoPicture.Y + cgoPicture.Hgt, C4GUI_MessageFontClr, ACenter,
                   false);
}
...
void C4PortraitSelDlg::AddExtraOptions(const C4Rect &rcOptionsRect) {
  C4GUI::ComponentAligner caOptions(rcOptionsRect, C4GUI_DefDlgIndent,
                                    C4GUI_DefDlgSmallIndent, false);
  CStdFont *pUseFont = &(C4GUI::GetRes()->TextFont);
  AddElement(new C4GUI::Label(
      LoadResStr("IDS_CTL_IMPORTIMAGEAS"),
      caOptions.GetGridCell(0, 3, 0, 1, -1, pUseFont->GetLineHeight(), true),
      ALeft));
  AddElement(pCheckSetPicture = new C4GUI::CheckBox(
                 caOptions.GetGridCell(1, 3, 0, 1, -1,
                                       pUseFont->GetLineHeight(), true),
                 LoadResStr("IDS_TEXT_PLAYERIMAGE"), fDefSetPicture));
  pCheckSetPicture->SetToolTip(
      LoadResStr("IDS_DESC_CHANGESTHEIMAGEYOUSEEINTH"));
  AddElement(pCheckSetBigIcon = new C4GUI::CheckBox(
                 caOptions.GetGridCell(2, 3, 0, 1, -1,
                                       pUseFont->GetLineHeight(), true),
                 LoadResStr("IDS_TEXT_LOBBYICON"), fDefSetPicture));
  pCheckSetBigIcon->SetToolTip(
      LoadResStr("IDS_DESC_CHANGESTHEIMAGEYOUSEEINTH2"));
}
...
C4FileSelDlg::ListItem *C4PortraitSelDlg::CreateListItem(
