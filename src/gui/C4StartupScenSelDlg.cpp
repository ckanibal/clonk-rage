...
void C4MapFolderData::Scenario::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(
      mkNamingAdapt(dwTitleInactClr, "TitleColorInactive", 0x7fffffffu));
...
C4MapFolderData::MapPic::MapPic(const FLOAT_RECT &rcfBounds,
                                const C4Facet &rfct)
    : C4GUI::Picture(C4Rect(rcfBounds), false), rcfBounds(rcfBounds) {
  // ctor
  SetFacet(rfct);
  SetToolTip(LoadResStr("IDS_MSG_MAP_DESC"));
}

void C4MapFolderData::MapPic::MouseInput(C4GUI::CMouse &rMouse, int32_t iButton,
...
void C4MapFolderData::MapPic::DrawElement(C4FacetEx &cgo) {
  ...
  // draw the image
  GetFacet().DrawXFloat(cgo.Surface, x0, y0, rcfBounds.right - rcfBounds.left,
                        rcfBounds.bottom - rcfBounds.top);
}
...
bool C4MapFolderData::Load(C4Group &hGroup,
  ...
  bool fHasLangTable = !!LangTable.LoadEx(
      "StringTbl", hGroup, C4CFN_ScriptStringTbl, Config.General.LanguageEx);
  // load core data
  StdStrBuf Buf;
  if (!hGroup.LoadEntryString(C4CFN_MapFolderData, Buf)) return false;
  if (fHasLangTable) LangTable.ReplaceStrings(Buf);
  if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(
          mkNamingAdapt(*this, "FolderMap"), Buf, C4CFN_MapFolderData))
    return false;
  ...
  if (!fctBackgroundPicture.Load(hGroup, C4CFN_MapFolderBG)) {
    DebugLogF(
        "C4MapFolderData::Load(%s): Could not load background graphic \"%s\"",
        hGroup.GetName(), C4CFN_MapFolderBG);
    return false;
  }
  int i;
  for (i = 0; i < iScenCount; ++i) {
    ...
    pScen->pScenEntry =
        pScenLoaderFolder->FindEntryByName(pScen->sFilename.getData());
    pScen->pBtn = NULL;
    pScen->sTitle.Replace(
        "TITLE",
        pScen->pScenEntry
            ? pScen->pScenEntry->GetName().getData()
            : "<c ff0000>ERROR</c>" /* scenario not loaded; title cannot be referenced */);
    // developer image dump
    if (pScen->fImgDump) {
      C4FacetExSurface fctDump;
      bool fSuccess = false;
      if (fctDump.Create(pScen->rcOverlayPos.Wdt, pScen->rcOverlayPos.Hgt,
                         C4FCT_Full, C4FCT_Full)) {
        lpDDraw->Blit(
            fctBackgroundPicture.Surface, (float)pScen->rcOverlayPos.x,
            (float)pScen->rcOverlayPos.y, (float)pScen->rcOverlayPos.Wdt,
            (float)pScen->rcOverlayPos.Hgt, fctDump.Surface, 0, 0, fctDump.Wdt,
            fctDump.Hgt);
        fSuccess = fctDump.Surface->SavePNG(pScen->sBaseImage.getData(), true,
                                            false, false);
      }
      if (!fSuccess)
        DebugLogF("C4MapFolderData::Load(%s): Could not dump graphic \"%s\"",
                  hGroup.GetName(), pScen->sBaseImage.getData());
      continue;
    }
    // load images
    if (pScen->sBaseImage.getLength() > 0)
      if (!pScen->fctBase.Load(hGroup, pScen->sBaseImage.getData())) {
        DebugLogF(
            "C4MapFolderData::Load(%s): Could not load base graphic \"%s\"",
            hGroup.GetName(), pScen->sBaseImage.getData());
        return false;
      }
    if (pScen->sOverlayImage.getLength() > 0)
      if (!pScen->fctOverlay.Load(hGroup, pScen->sOverlayImage.getData())) {
        DebugLogF("C4MapFolderData::Load(%s): Could not load graphic \"%s\"",
                  hGroup.GetName(), pScen->sOverlayImage.getData());
        return false;
      }
  }
  for (i = 0; i < iAccessGfxCount; ++i) {
    AccessGfx *pGfx = ppAccessGfxList[i];
    if (pGfx->sOverlayImage.getLength() > 0)
      if (!pGfx->fctOverlay.Load(hGroup, pGfx->sOverlayImage.getData())) {
        DebugLogF("C4MapFolderData::Load(%s): Could not load graphic \"%s\"",
                  hGroup.GetName(), pGfx->sOverlayImage.getData());
        return false;
      }
  }
  // all loaded
...
void C4MapFolderData::CompileFunc(StdCompiler *pComp) {
  // core values
  pComp->Value(
      mkNamingAdapt(rcScenInfoArea, "ScenInfoArea", C4Rect(0, 0, 0, 0)));
  ...
  if (iScenCount) {
    ...
    pComp->Value(mkNamingAdapt(
        mkArrayAdaptMap(ppScenList, iScenCount, mkPtrAdaptNoNull<Scenario>),
        "Scenario"));
  }
  ...
  if (iAccessGfxCount) {
    ...
    pComp->Value(mkNamingAdapt(mkArrayAdaptMap(ppAccessGfxList, iAccessGfxCount,
                                               mkPtrAdaptNoNull<AccessGfx>),
                               "AccessGfx"));
  }
}

void C4MapFolderData::ConvertFacet2ScreenCoord(const C4Rect &rc,
                                               FLOAT_RECT *pfrc, float fBGZoomX,
                                               float fBGZoomY, int iOffX,
                                               int iOffY) {
...
void C4MapFolderData::ConvertFacet2ScreenCoord(int32_t *piValue, float fBGZoom,
                                               int iOff) {
  *piValue = int32_t(floorf(fBGZoom * *piValue + 0.5f)) + iOff;
}

void C4MapFolderData::ConvertFacet2ScreenCoord(C4Rect &rcMapArea,
                                               bool fAspect) {
  if (!fctBackgroundPicture.Wdt || !fctBackgroundPicture.Hgt)
    return;  // invalid BG - should not happen
  // get zoom of background image
  float fBGZoomX = 1.0f, fBGZoomY = 1.0f;
  int iOffX = 0, iOffY = 0;
  if (fAspect) {
    if (fctBackgroundPicture.Wdt * rcMapArea.Hgt >
        rcMapArea.Wdt * fctBackgroundPicture.Hgt) {
      // background image is limited by width
      fBGZoomX = fBGZoomY = (float)rcMapArea.Wdt / fctBackgroundPicture.Wdt;
      iOffY = Max<int>(0, (int)(rcMapArea.Hgt -
                                (fBGZoomX * fctBackgroundPicture.Hgt))) /
              2;
    } else {
      // background image is limited by height
      fBGZoomX = fBGZoomY = (float)rcMapArea.Hgt / fctBackgroundPicture.Hgt;
      iOffX = Max<int>(0, (int)(rcMapArea.Wdt -
                                (fBGZoomY * fctBackgroundPicture.Wdt))) /
              2;
    }
  } else {
  ...
  iOffY -= rcMapArea.y;
  C4Rect rcBG;
  rcBG.Set(0, 0, fctBackgroundPicture.Wdt, fctBackgroundPicture.Hgt);
  ...
  if (!rcScenInfoArea.Wdt)
    rcScenInfoArea.Set((int32_t)(fctBackgroundPicture.Wdt * 2 / 3),
                       (int32_t)(fctBackgroundPicture.Hgt / 16),
                       (int32_t)(fctBackgroundPicture.Wdt / 3),
                       (int32_t)(fctBackgroundPicture.Hgt * 7 / 8));
  // assume all facet coordinates are referring to background image zoom;
  // convert them to screen coordinates by applying zoom and offset
  FLOAT_RECT rcfScenInfoArea;
  ConvertFacet2ScreenCoord(rcScenInfoArea, &rcfScenInfoArea, fBGZoomX, fBGZoomY,
                           iOffX, iOffY);
  rcScenInfoArea.x = (int32_t)rcfScenInfoArea.left;
  rcScenInfoArea.y = (int32_t)rcfScenInfoArea.top;
  ...
  for (i = 0; i < iScenCount; ++i) {
    Scenario *pScen = ppScenList[i];
    ConvertFacet2ScreenCoord(pScen->rcOverlayPos, &(pScen->rcfOverlayPos),
                             fBGZoomX, fBGZoomY, iOffX, iOffY);
    // title sizes
  ...
  for (i = 0; i < iAccessGfxCount; ++i)
    ConvertFacet2ScreenCoord(ppAccessGfxList[i]->rcOverlayPos,
                             &(ppAccessGfxList[i]->rcfOverlayPos), fBGZoomX,
                             fBGZoomY, iOffX, iOffY);
  // done
...
void C4MapFolderData::CreateGUIElements(C4StartupScenSelDlg *pMainDlg,
  ...
  for (i = 0; i < iAccessGfxCount; ++i) {
    ...
    if (!szPassword || !*szPassword ||
        SIsModule(Config.General.MissionAccess, szPassword)) {
  ...
  for (i = 0; i < iScenCount; ++i) {
    ...
    if (pScen->pScenEntry && !pScen->pScenEntry->HasMissionAccess()) {
      // no access to this scenario: Do not create a button at all; not even
      // base image. The scenario is "invisible".
    } else {
      C4GUI::CallbackButtonEx<C4StartupScenSelDlg, C4GUI::FacetButton> *pBtn =
          new C4GUI::CallbackButtonEx<C4StartupScenSelDlg, C4GUI::FacetButton>(
              pScen->fctBase, pScen->fctOverlay, pScen->rcfOverlayPos, 0,
              pMainDlg, &C4StartupScenSelDlg::OnButtonScenario);
      ppScenList[i]->pBtn = pBtn;
      if (pScen->pScenEntry)
        pBtn->SetToolTip(
            FormatString(LoadResStr("IDS_MSG_MAP_STARTSCEN"),
                         pScen->pScenEntry->GetName().getData()).getData());
      if (pScen->sTitle.getLength() > 0) {
        pBtn->SetText(pScen->sTitle.getData());
        pBtn->SetTextColors(InvertRGBAAlpha(pScen->dwTitleInactClr),
                            InvertRGBAAlpha(pScen->dwTitleActClr));
        pBtn->SetTextPos(pScen->iTitleOffX, pScen->iTitleOffY,
                         pScen->byTitleAlign);
        CStdFont *pUseFont;
        float fFontZoom = 1.0f;
        if (pScen->fTitleBookFont)
          pUseFont = &(C4Startup::Get()->Graphics.GetBlackFontByHeight(
              pScen->iTitleFontSize, &fFontZoom));
        else
          pUseFont = &(C4GUI::GetRes()->GetFontByHeight(pScen->iTitleFontSize,
                                                        &fFontZoom));
        if (Inside<float>(fFontZoom, 0.8f, 1.25f))
          fFontZoom = 1.0f;  // some tolerance for font zoom
  ...
  pSelectionInfoBox = new C4GUI::TextWindow(
      rcScenInfoArea, C4StartupScenSel_TitlePictureWdt +
                          2 * C4StartupScenSel_TitleOverlayMargin,
      C4StartupScenSel_TitlePictureHgt +
          2 * C4StartupScenSel_TitleOverlayMargin,
      C4StartupScenSel_TitlePicturePadding, 100, 4096, NULL, true,
      &C4Startup::Get()->Graphics.fctScenSelTitleOverlay,
      C4StartupScenSel_TitleOverlayMargin);
  pSelectionInfoBox->SetDecoration(
...
C4ScenarioListLoader::Entry::Entry(Folder *pParent)
    : pNext(NULL), pParent(pParent), fBaseLoaded(false), fExLoaded(false) {
...
bool C4ScenarioListLoader::Entry::Load(C4Group *pFromGrp,
  ...
  if (fBaseLoaded) {
    ...
    if (!LoadCustomPre(Group)) return false;
    // Load entry name
    ...
    if (Group.FindEntry(C4CFN_IconPNG) && fctIcon.Load(Group, C4CFN_IconPNG))
      ...
      if (Group.FindEntry(C4CFN_ScenarioIcon) &&
          fctTemp.Load(Group, C4CFN_ScenarioIcon, C4FCT_Full, C4FCT_Full,
                       true)) {
        ...
        C4Startup::Get()
            ->Graphics.fctScenSelIcons
            .GetPhase(C4StartupScenSel_DefaultIcon_OldIconBG)
            .Draw(fctIcon);
        fctTemp.Draw(fctIcon.Surface, (fctIcon.Wdt - fctTemp.Wdt) / 2,
                     (fctIcon.Hgt - fctTemp.Hgt) / 2);
        fctTemp.Clear();
        fIconLoaded = true;
      }
    }
    // load any entryx-type-specific custom data (e.g. fallbacks for scenario
    // title, and icon)
  ...
  if (fLoadEx && !fExLoaded) {
    ...
    if (DefDesc.LoadEx("Desc", Group, C4CFN_ScenarioDesc,
    ...
    DefDesc.Close();
    // load title
    if (!fctTitle.Load(Group, C4CFN_ScenarioTitlePNG, C4FCT_Full, C4FCT_Full,
                       false, true))
      fctTitle.Load(Group, C4CFN_ScenarioTitle, C4FCT_Full, C4FCT_Full, true,
                    true);
    fExLoaded = true;
    // load author
    if (Group.IsPacked()) {
      const char *strSecAuthors =
          "RedWolf Design;Clonk History Project;GWE-Team";  // Now hardcoded...
      if (SIsModule(strSecAuthors, Group.GetMaker()) &&
          Group.LoadEntryString(C4CFN_Author, sAuthor)) {
  ...
  // LogF("dbg: Loaded \"%s\" as \"%s\". (%s)", (const char *) sFilename, (const
  // char *) sName, GetIsFolder() ? "Folder" : "Scenario");
  // done, success
  return true;
}
...
bool DirContainsScenarios(const char *szDir) {
  // create iterator on free store to avoid stack overflow with deeply recursed
  // folders
  ...
  for (; szChildFilename = **pIter; ++*pIter) {
    ...
    if (WildcardMatch(C4CFN_ScenarioFiles, szChildFilename) ||
        WildcardMatch(C4CFN_FolderFiles, szChildFilename))
      break;
    if (DirectoryExists(szChildFilename))
      if (DirContainsScenarios(szChildFilename)) break;
  }
  delete pIter;
...
C4ScenarioListLoader::Entry *C4ScenarioListLoader::Entry::CreateEntryForFile(
    const StdStrBuf &sFilename, Folder *pParent) {
  // determine entry type by file type
  const char *szFilename = sFilename.getData();
  if (!szFilename || !*szFilename) return NULL;
  if (WildcardMatch(C4CFN_ScenarioFiles, sFilename.getData()))
    return new Scenario(pParent);
  if (WildcardMatch(C4CFN_FolderFiles, sFilename.getData()))
    return new SubFolder(pParent);
  // regular, open folder (C4Group-packed folders without extensions are not
  // regarded, because they could contain anything!)
  ...
  if ((!szExt || !*szExt) && DirectoryExists(sFilename.getData())) {
    ...
    if (DirContainsScenarios(szFilename)) return new RegularFolder(pParent);
  }
  // type not recognized
  return NULL;
}
...
bool C4ScenarioListLoader::Entry::RenameTo(const char *szNewName) {
  ...
  if (szExt) {
    SAppend(".", fn, _MAX_PATH);
    SAppend(szExt, fn, _MAX_PATH);
  }
  ...
  if (!ItemIdentical(sFilename.getData(), fullfn)) {
    ...
    if (ItemExists(fullfn)) {
      StdStrBuf sMsg;
      sMsg.Format(LoadResStr("IDS_ERR_FILEEXISTS"), fullfn);
      Game.pGUI->ShowMessageModal(sMsg.getData(), strErr.getData(),
    ...
    if (!C4Group_MoveItem(sFilename.getData(), fullfn, true)) {
      StdStrBuf sMsg;
      sMsg.Format(LoadResStr("IDS_ERR_RENAMEFILE"), sFilename.getData(),
                  fullfn);
      Game.pGUI->ShowMessageModal(sMsg.getData(), strErr.getData(),
                                  C4GUI::MessageDialog::btnOK,
                                  C4GUI::Ico_Error);
  ...
  if (C4Group_IsGroup(fullfn)) {
    ...
    if (!Grp.Open(fullfn)) {
      StdStrBuf sMsg;
      sMsg.Format(LoadResStr("IDS_ERR_OPENFILE"), sFilename.getData(),
                  Grp.GetError());
      Game.pGUI->ShowMessageModal(sMsg.getData(), strErr.getData(),
                                  C4GUI::MessageDialog::btnOK,
                                  C4GUI::Ico_Error);
    ...
    if (!Grp.Delete(C4CFN_Title)) {
      StdStrBuf sMsg;
      sMsg.Format(LoadResStr("IDS_ERR_DELOLDTITLE"), sFilename.getData(),
                  Grp.GetError());
      Game.pGUI->ShowMessageModal(sMsg.getData(), strErr.getData(),
                                  C4GUI::MessageDialog::btnOK,
                                  C4GUI::Ico_Error);
    ...
    if (!Grp.Close()) {
      StdStrBuf sMsg;
      sMsg.Format(LoadResStr("IDS_ERR_WRITENEWTITLE"), sFilename.getData(),
                  Grp.GetError());
      Game.pGUI->ShowMessageModal(sMsg.getData(), strErr.getData(),
                                  C4GUI::MessageDialog::btnOK,
                                  C4GUI::Ico_Error);
...
bool C4ScenarioListLoader::Entry::SetTitleInGroup(C4Group &rGrp,
  ...
  // default for group files: Create a title text file and set the title in
  // there
  // no title needed if filename is sufficient - except for scenarios, where a
  // Scenario.txt could overwrite the title
  if (!IsScenario()) {
    StdStrBuf sNameByFile;
    sNameByFile.Copy(GetFilename(sFilename.getData()));
  ...
  // okay, make a title
  StdStrBuf sTitle;
  sTitle.Format("%s:%s", Config.General.Language, szNewTitle);
  if (!rGrp.Add(C4CFN_WriteTitle, sTitle, FALSE, TRUE)) {
    StdStrBuf sMsg;
    sMsg.Format(LoadResStr("IDS_ERR_ERRORADDINGNEWTITLEFORFIL"),
                sFilename.getData(), rGrp.GetError());
...
bool C4ScenarioListLoader::Scenario::LoadCustomPre(C4Group &rGrp) {
  ...
  if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(
          mkParAdapt(C4S, false), sFileContents,
          (rGrp.GetFullName() + DirSep C4CFN_ScenarioCore).getData()))
    return false;
  return true;
}

bool C4ScenarioListLoader::Scenario::LoadCustom(C4Group &rGrp, bool fNameLoaded,
                                                bool fIconLoaded) {
  // icon fallback: Standard scenario icon
  if (!fIconLoaded) {
    iIconIndex = C4S.Head.Icon;
    int32_t iIcon = C4S.Head.Icon;
    if (!Inside<int32_t>(iIcon, 0, C4StartupScenSel_IconCount - 1))
      iIcon = C4StartupScenSel_DefaultIcon_Scenario;
    fctIcon.Set(C4Startup::Get()->Graphics.fctScenSelIcons.GetSection(iIcon));
  }
  // scenario name fallback to core
  if (!fNameLoaded) sName.Copy(C4S.Head.Title);
  // unregistered access
  C4Group *pFolder;
  ...
  // difficulty: Set only for regular rounds (not savegame or record) to avoid
  // bogus sorting
...
bool C4ScenarioListLoader::Scenario::CanOpen(StdStrBuf &sErrOut) {
  ...
  if (C4S.Head.MissionAccess[0] &&
      !SIsModule(Config.General.MissionAccess, C4S.Head.MissionAccess)) {
  ...
  // regular game
    ...
    if (C4S.Head.SaveGame) {
      // Some scenarios have adjusted MaxPlayerCount to 0 after starting to
    ...
    // normal scenarios: At least one player except in network mode, where it is
    // possible to wait for the additional players
    // Melees need at least two
    if ((iPlrCount < iMinPlrCount)) {
      if (pDlg->IsNetworkStart()) {
        // network game: Players may yet join in lobby
        // only issue a warning for too few players (by setting the error but
        // not returning false here)
        sErrOut.Format(LoadResStr("IDS_MSG_TOOFEWPLAYERSNET"),
                       (int)iMinPlrCount);
      } else {
    ...
    // scenarios (both normal and savegame) may also impose a maximum player
    // restriction
    if (iPlrCount > iMaxPlrCount) {
      sErrOut.Format(LoadResStr("IDS_MSG_TOOMANYPLAYERS"),
...
int
  ...
  C4ScenarioListLoader::Entry *pEntry1 =
                                  *(C4ScenarioListLoader::Entry * const *)pEl1,
                              *pEntry2 =
                                  *(C4ScenarioListLoader::Entry * const *)pEl2;
  // sort folders before scenarios
  bool fS1, fS2;
  if (!(fS1 = !pEntry1->GetIsFolder()) != !true !=
      !(fS2 = !pEntry2->GetIsFolder()))
    return fS1 - fS2;
  // sort by folder index (undefined index 0 goes to the end)
  if (!Config.Startup.AlphabeticalSorting)
    if (pEntry1->GetFolderIndex() || pEntry2->GetFolderIndex()) {
  ...
  if (!Config.Startup.AlphabeticalSorting)
    if (pEntry1->GetDifficulty() || pEntry2->GetDifficulty()) {
...
void C4ScenarioListLoader::Folder::Sort() {
  ...
  if (!pFirst) return;
  uint32_t iCount, i;
  Entry **ppEntries = new Entry *[i = iCount = GetEntryCount()], **ppI,
        *pI = pFirst, **ppIThis;
  for (ppI = ppEntries; i--; pI = pI->pNext) *ppI++ = pI;
  qsort(ppEntries, iCount, sizeof(Entry *), &EntrySortFunc);
  ppIThis = &pFirst;
  for (ppI = ppEntries; iCount--; ppIThis = &((*ppIThis)->pNext))
    *ppIThis = *ppI++;
  *ppIThis = NULL;
  delete[] ppEntries;
}
...
C4ScenarioListLoader::Entry *C4ScenarioListLoader::Folder::FindEntryByName(
    const char *szFilename) const {
  // do a case-insensitive filename comparison
  for (Entry *pEntry = pFirst; pEntry; pEntry = pEntry->GetNext())
    if (SEqualNoCase(szFilename,
                     GetFilename(pEntry->GetEntryFilename().getData())))
      return pEntry;
  // nothing found
  return NULL;
}
...
bool C4ScenarioListLoader::Folder::IsGrayed() {
  return false; /*!(Config.Registered() ||
                   Config.IsFreeFolder(sFilename.getData(), sMaker.getData()));
                   FREEWARE */
}
...
bool C4ScenarioListLoader::Folder::LoadCustomPre(C4Group &rGrp) {
  ...
  if (rGrp.LoadEntryString(C4CFN_FolderCore, sFileContents))
    if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(
            C4F, sFileContents,
            (rGrp.GetFullName() + DirSep C4CFN_FolderCore).getData()))
      return false;
  return true;
}
...
bool C4ScenarioListLoader::SubFolder::LoadCustom(C4Group &rGrp,
  ...
  if (!fIconLoaded)
    fctIcon.Set(C4Startup::Get()->Graphics.fctScenSelIcons.GetSection(
        C4StartupScenSel_DefaultIcon_Folder));
  // folder index
  iFolderIndex = C4F.Head.Index;
  return true;
}

bool C4ScenarioListLoader::SubFolder::DoLoadContents(
    C4ScenarioListLoader *pLoader, C4Group *pFromGrp,
    const StdStrBuf &sFilename, bool fLoadEx) {
  assert(pLoader);
  // clear any previous
  ClearChildren();
  // group specified: Load as child
  ...
  if (pFromGrp) {
    if (!Group.OpenAsChild(pFromGrp, sFilename.getData())) return false;
      ...
      // no parent group: Direct load from filename
      if (!Group.Open(sFilename.getData()))
    return false;
  // get number of entries, to estimate progress
  const char *szC4CFN_ScenarioFiles =
      C4CFN_ScenarioFiles;  // assign values for constant comparison
  const char *szSearchMask;
  int32_t iEntryCount = 0;
  ...
  char ChildFilename[_MAX_FNAME + 1];
  StdStrBuf sChildFilename;
  int32_t iLoadCount = 0;
  for (szSearchMask = szC4CFN_ScenarioFiles; szSearchMask;) {
    // LogF("SubFolder \"%s\" loading mask \"%s\"", (const char *)
    // Group.GetFullName(), (const char *) szSearchMask);
    Group.ResetSearch();
    while (Group.FindNextEntry(szSearchMask, ChildFilename)) {
      sChildFilename.Ref(ChildFilename);
      // okay; create this item
      // LogF("SubFolder \"%s\" loading \"%s\"", (const char *) sFilename,
      // (const char *) sChildFilename);
      Entry *pNewEntry = Entry::CreateEntryForFile(sChildFilename, this);
      if (pNewEntry) {
        // ...and load it
        if (!pNewEntry->Load(&Group, &sChildFilename, fLoadEx)) {
          DebugLogF("Error loading entry \"%s\" in SubFolder \"%s\"!",
                    sChildFilename.getData(), Group.GetFullName().getData());
          delete pNewEntry;
        }
      }
      // mark progress
...
bool C4ScenarioListLoader::RegularFolder::LoadCustom(C4Group &rGrp,
  ...
  if (!fIconLoaded)
    fctIcon.Set(C4Startup::Get()->Graphics.fctScenSelIcons.GetSection(
        C4StartupScenSel_DefaultIcon_WinFolder));
  // folder index
  iFolderIndex = C4F.Head.Index;
  return true;
}

bool C4ScenarioListLoader::RegularFolder::DoLoadContents(
    C4ScenarioListLoader *pLoader, C4Group *pFromGrp,
    const StdStrBuf &sFilename, bool fLoadEx) {
  // clear any previous
  ClearChildren();
  ...
  for (DirIter.Reset(sFilename.getData()); szChildFilename = *DirIter;
       ++DirIter) {
    // Ignore directory navigation entries and CVS folders
    if (!*szChildFilename || *GetFilename(szChildFilename) == '.') continue;
    if (SEqualNoCase(GetFilename(szChildFilename), "CVS")) continue;
    ...
    if (pNewEntry) {
      ...
      if (!pNewEntry->Load(NULL, &sChildFilename, fLoadEx)) {
        DebugLogF("Error loading entry \"%s\" in Folder \"%s\"!",
                  sChildFilename.getData(), sFilename.getData());
        delete pNewEntry;
      }
    }
    // progress callback
...
bool C4ScenarioListLoader::BeginActivity(bool fAbortPrevious) {
  ...
  if (iLoading && fAbortPrevious) this->fAbortPrevious = true;
  // mark this activity
  ++iLoading;
...
bool C4ScenarioListLoader::DoProcessCallback(int32_t iProgress,
  ...
  // callback to dialog
  if (C4StartupScenSelDlg::pInstance)
    C4StartupScenSelDlg::pInstance->ProcessCallback();
...
bool C4ScenarioListLoader::Load(const StdStrBuf &sRootFolder) {
  ...
  bool fSuccess =
      pRootFolder->LoadContents(this, NULL, &sRootFolder, false, false);
  EndActivity();
  return fSuccess;
};
...
// font clrs
const uint32_t ClrScenarioItem = 0xff000000, ClrScenarioItemXtra = 0x7f000000,
               ClrScenarioItemDisabled = 0x7f000000;

// ------------------------------------------------
// --- C4StartupScenSelDlg::ScenListItem
C4StartupScenSelDlg::ScenListItem::ScenListItem(
    C4GUI::ListBox *pForListBox, C4ScenarioListLoader::Entry *pForEntry,
  ...
  StdStrBuf sIgnore;
  bool fEnabled =
      pScenListEntry->CanOpen(sIgnore) && !pScenListEntry->IsGrayed();
  // calc height
  ...
  pNameLabel =
      new C4GUI::Label(pScenListEntry->GetName().getData(),
                       iHeight + IconLabelSpacing, IconLabelSpacing, ALeft,
                       fEnabled ? ClrScenarioItem : ClrScenarioItemDisabled,
                       &rUseFont, false, false);
  ...
  C4Rect rcNLB = pNameLabel->GetBounds();
  rcNLB.Wdt = GetClientRect().Wdt - rcNLB.x - IconLabelSpacing;
...
void C4StartupScenSelDlg::ScenListItem::MouseInput(C4GUI::CMouse &rMouse,
                                                   int32_t iButton, int32_t iX,
                                                   int32_t iY,
                                                   DWORD dwKeyParam) {
...
bool C4StartupScenSelDlg::ScenListItem::KeyRename() {
  // rename this entry
  C4StartupScenSelDlg::pInstance->StartRenaming(new C4GUI::CallbackRenameEdit<
      C4StartupScenSelDlg::ScenListItem, RenameParams>(
      pNameLabel, this, RenameParams(),
      &C4StartupScenSelDlg::ScenListItem::DoRenaming,
      &C4StartupScenSelDlg::ScenListItem::AbortRenaming));
  return true;
}
...
C4GUI::RenameEdit::RenameResult C4StartupScenSelDlg::ScenListItem::DoRenaming(
...
C4StartupScenSelDlg::C4StartupScenSelDlg(bool fNetwork)
    : C4StartupDlg(
          LoadResStrNoAmp(fNetwork ? "IDS_DLG_NETSTART" : "IDS_DLG_STARTGAME")),
      pScenLoader(NULL),
      fIsInitialLoading(false),
      fStartNetworkGame(fNetwork),
      pMapData(NULL),
      pRenameEdit(NULL),
      pfctBackground(NULL) {
  // ctor
  ...
  C4GUI::ComponentAligner caButtonArea(
      caMain.GetFromBottom(caMain.GetHeight() / 8),
      rcBounds.Wdt / (rcBounds.Wdt >= 700 ? 128 : 256), 0);
  C4Rect rcMap = caMain.GetCentered(caMain.GetWidth(), caMain.GetHeight());
  int iYOversize = caMain.GetHeight() / 10;  // overlap of map to top
  rcMap.y -= iYOversize;
  rcMap.Hgt += iYOversize;
  C4GUI::ComponentAligner caMap(rcMap, 0, 0, true);
  caMap.ExpandTop(-iYOversize);
  C4GUI::ComponentAligner caBook(
      caMap.GetCentered(caMap.GetWidth() * 11 / 12 - 4 * iExtraHPadding,
                        caMap.GetHeight()),
      rcBounds.Wdt / 30, iExtraVPadding, false);
  C4GUI::ComponentAligner caBookLeft(
      caBook.GetFromLeft(iBookPageWidth = caBook.GetWidth() * 4 / 9 + 4 -
                                          iExtraHPadding * 2),
      0, 5);
  ...
  pScenSelCaption = new C4GUI::Label(
      "", caBookLeft.GetFromTop(rScenSelCaptionFont.GetLineHeight()), ACenter,
      ClrScenarioItem, &rScenSelCaptionFont, false);
  ...
  pScenSelList->SetDecoration(false, &C4Startup::Get()->Graphics.sfctBookScroll,
                              true);
  pSheetBook->AddElement(pScenSelList);
  pScenSelList->SetSelectionChangeCallbackFn(
      new C4GUI::CallbackHandler<C4StartupScenSelDlg>(
          this, &C4StartupScenSelDlg::OnSelChange));
  pScenSelList->SetSelectionDblClickFn(
      new C4GUI::CallbackHandler<C4StartupScenSelDlg>(
          this, &C4StartupScenSelDlg::OnSelDblClick));
  // scenario selection list progress label
  pScenSelProgressLabel = new C4GUI::Label(
      "", pScenSelList->GetBounds().GetMiddleX(),
      pScenSelList->GetBounds().GetMiddleX() - iCaptionFontHgt / 2, ACenter,
      ClrScenarioItem, &(C4Startup::Get()->Graphics.BookFontCapt), false);
  pSheetBook->AddElement(pScenSelProgressLabel);

  // right side of book: Displaying current selection
  pSelectionInfo = new C4GUI::TextWindow(
      caBook.GetFromRight(iBookPageWidth),
      C4StartupScenSel_TitlePictureWdt +
          2 * C4StartupScenSel_TitleOverlayMargin,
      C4StartupScenSel_TitlePictureHgt +
          2 * C4StartupScenSel_TitleOverlayMargin,
      C4StartupScenSel_TitlePicturePadding, 100, 4096, NULL, true,
  ...
  AddElement(btn = new C4GUI::CallbackButton<C4StartupScenSelDlg>(
                 LoadResStr("IDS_BTN_BACK"),
                 caButtonArea.GetFromLeft(iButtonWidth, iButtonHeight),
                 &C4StartupScenSelDlg::OnBackBtn));
  btn->SetToolTip(LoadResStr("IDS_DLGTIP_BACKMAIN"));
  AddElement(btn);
  // LogF("BackBtn bounds: (%d,%d)+(%d,%d)", btn->GetBounds().x,
  // btn->GetBounds().y, btn->GetBounds().Wdt, btn->GetBounds().Hgt);
  // next button
  pOpenBtn = new C4GUI::CallbackButton<C4StartupScenSelDlg>(
      LoadResStr("IDS_BTN_OPEN"),
      caButtonArea.GetFromRight(iButtonWidth, iButtonHeight),
      &C4StartupScenSelDlg::OnNextBtn);
  pOpenBtn->SetToolTip(LoadResStr("IDS_DLGTIP_SCENSELNEXT"));
  // game options boxes
  pGameOptionButtons =
      new C4GameOptionButtons(caButtonArea.GetAll(), fNetwork, true, false);
  ...
  pKeyBack = new C4KeyBinding(keys, "StartupScenSelFolderUp", KEYSCOPE_Gui,
                              new C4GUI::DlgKeyCB<C4StartupScenSelDlg>(
                                  *this, &C4StartupScenSelDlg::KeyBack),
                              C4CustomKey::PRIO_CtrlOverride);
  pKeyRefresh =
      new C4KeyBinding(C4KeyCodeEx(K_F5), "StartupScenSelReload", KEYSCOPE_Gui,
                       new C4GUI::DlgKeyCB<C4StartupScenSelDlg>(
                           *this, &C4StartupScenSelDlg::KeyRefresh),
                       C4CustomKey::PRIO_CtrlOverride);
  pKeyForward =
      new C4KeyBinding(C4KeyCodeEx(K_RIGHT), "StartupScenSelNext", KEYSCOPE_Gui,
                       new C4GUI::DlgKeyCB<C4StartupScenSelDlg>(
                           *this, &C4StartupScenSelDlg::KeyForward),
                       C4CustomKey::PRIO_CtrlOverride);
  pKeyRename = new C4KeyBinding(
      C4KeyCodeEx(K_F2), "StartupScenSelRename", KEYSCOPE_Gui,
      new C4GUI::ControlKeyDlgCB<C4StartupScenSelDlg>(
          pScenSelList, *this, &C4StartupScenSelDlg::KeyRename),
      C4CustomKey::PRIO_CtrlOverride);
  pKeyDelete = new C4KeyBinding(
      C4KeyCodeEx(K_DELETE), "StartupScenSelDelete", KEYSCOPE_Gui,
      new C4GUI::ControlKeyDlgCB<C4StartupScenSelDlg>(
          pScenSelList, *this, &C4StartupScenSelDlg::KeyDelete),
      C4CustomKey::PRIO_CtrlOverride);
  pKeyCheat = new C4KeyBinding(
      C4KeyCodeEx(KEY_M, KEYS_Alt), "StartupScenSelCheat", KEYSCOPE_Gui,
      new C4GUI::ControlKeyDlgCB<C4StartupScenSelDlg>(
          pScenSelList, *this, &C4StartupScenSelDlg::KeyCheat),
      C4CustomKey::PRIO_CtrlOverride);
}
...
void C4StartupScenSelDlg::OnClosed(bool fOK) {
  ...
  if (pScenLoader) {
    delete pScenLoader;
    pScenLoader = NULL;
    UpdateList();  // must clear scenario list, because it points to deleted
                   // stuff
...
void C4StartupScenSelDlg::UpdateList() {
  ...
  C4GUI::Tabular::Sheet *pMapSheet =
      pScenSelStyleTabular->GetSheet(ShowStyle_Map);
  ...
  if (pScenLoader->IsLoading()) {
    ...
    sProgressText.Format(LoadResStr("IDS_MSG_SCENARIODESC_LOADING"),
                         (int32_t)pScenLoader->GetProgressPercent());
  ...
  C4ScenarioListLoader::Folder *pFolder =
      static_cast<C4ScenarioListLoader::Folder *>(pScenLoader->GetCurrFolder());
  if (pMapData = pFolder->GetMapData()) {
    pMapData->ResetSelection();
    pMapData->CreateGUIElements(this,
                                *pScenSelStyleTabular->GetSheet(ShowStyle_Map));
    pScenSelStyleTabular->SelectSheet(ShowStyle_Map, false);
  } else {
    // book style selection
    // add what has been loaded
    for (C4ScenarioListLoader::Entry *pEnt = pScenLoader->GetFirstEntry(); pEnt;
         pEnt = pEnt->GetNext()) {
    ...
    // new list has been loaded: Select first entry if nothing else had been
    // selected
...
void C4StartupScenSelDlg::UpdateSelection() {
  ...
  C4GUI::TextWindow *pSelectionInfo =
      pMapData ? pMapData->GetSelectionInfoBox() : this->pSelectionInfo;
  // get selected entry
  C4ScenarioListLoader::Entry *pSel = GetSelectedEntry();
  if (!pSel) {
  ...
  C4Facet fctTitle;
  StdStrBuf sTitle, sDesc, sVersion, sAuthor;
  if (pSel) {
    ...
    // image
    if (!fctTitle.Surface && (!sDesc || !*sDesc.getData())) sTitle.Clear();
  ...
  if (!!sTitle && (!sDesc || !*sDesc.getData()))
    pSelectionInfo->AddTextLine(sTitle.getData(),
                                &C4Startup::Get()->Graphics.BookFontCapt,
                                ClrScenarioItem, false, false);
  if (!!sDesc)
    pSelectionInfo->AddTextLine(
        sDesc.getData(), &C4Startup::Get()->Graphics.BookFont, ClrScenarioItem,
        false, false, &C4Startup::Get()->Graphics.BookFontCapt);
  if (!!sAuthor)
    pSelectionInfo->AddTextLine(
        FormatString(LoadResStr("IDS_CTL_AUTHOR"), sAuthor.getData()).getData(),
        &C4Startup::Get()->Graphics.BookFont, ClrScenarioItemXtra, false,
        false);
  if (!!sVersion)
    pSelectionInfo->AddTextLine(FormatString(LoadResStr("IDS_DLG_VERSION"),
                                             sVersion.getData()).getData(),
                                &C4Startup::Get()->Graphics.BookFont,
                                ClrScenarioItemXtra, false, false);
...
C4ScenarioListLoader::Entry *C4StartupScenSelDlg::GetSelectedEntry() {
  ...
  ScenListItem *pSel =
      static_cast<ScenListItem *>(pScenSelList->GetSelectedItem());
  return pSel ? pSel->GetEntry() : NULL;
}

bool C4StartupScenSelDlg::StartScenario(
    C4ScenarioListLoader::Scenario *pStartScen) {
  assert(pStartScen);
  ...
  if (pStartScen->GetC4S().Definitions.AllowUserChange) {
    ...
    if (!pStartScen->GetC4S().Definitions.GetModules(&sDefinitions))
      sDefinitions.Copy("Objects.c4d");
    if (!C4DefinitionSelDlg::SelectDefinitions(GetScreen(), &sDefinitions))
      // user aborted during definition selection
      return false;
    SCopy(sDefinitions.getData(), Game.DefinitionFilenames,
          (sizeof Game.DefinitionFilenames) - 1);
  } else
    // for no user change, just set default objects. Custom settings will
...
bool C4StartupScenSelDlg::DoOK() {
  ...
  if (!pSel->CanOpen(sError)) {
    GetScreen()->ShowMessage(sError.getData(),
                             LoadResStr("IDS_MSG_CANNOTSTARTSCENARIO"),
                             C4GUI::Ico_Error);
    return false;
  }
  // if CanOpen returned true but set an error message, that means it's a
  // warning. Display it!
  if (sError.getLength()) {
    if (!GetScreen()->ShowMessageModal(
            sError.getData(), LoadResStrNoAmp("IDS_DLG_STARTGAME"),
            C4GUI::MessageDialog::btnOKAbort, C4GUI::Ico_Notify,
            &Config.Startup.HideMsgStartDedicated))
...
bool C4StartupScenSelDlg::DoBack(bool fAllowClose) {
  ...
  // while this isn't multithreaded, the dialog must not be aborted while
  // initial load...
...
bool C4StartupScenSelDlg::KeyDelete() {
  ...
  sWarning.Format(
      LoadResStr(fOriginal ? "IDS_MSG_DELETEORIGINAL" : "IDS_MSG_PROMPTDELETE"),
      FormatString("%s %s", pEnt->GetTypeName().getData(),
                   pEnt->GetName().getData()).getData());
  GetScreen()->ShowRemoveDlg(new C4GUI::ConfirmationDialog(
      sWarning.getData(), LoadResStr("IDS_MNU_DELETE"),
      new C4GUI::CallbackHandlerExPar<C4StartupScenSelDlg, ScenListItem *>(
          this, &C4StartupScenSelDlg::DeleteConfirm, pSel),
...
void C4StartupScenSelDlg::DeleteConfirm(ScenListItem *pSel) {
  ...
  if (!C4Group_DeleteItem(pEnt->GetEntryFilename().getData(), true)) {
    StdStrBuf sMsg;
    sMsg.Format("%s", LoadResStr("IDS_FAIL_DELETE"));
    Game.pGUI->ShowMessageModal(sMsg.getData(), LoadResStr("IDS_MNU_DELETE"),
                                C4GUI::MessageDialog::btnOK, C4GUI::Ico_Error);
    return;
  }
...
bool C4StartupScenSelDlg::KeyCheat() {
  return Game.pGUI->ShowRemoveDlg(new C4GUI::InputDialog(
      LoadResStr("IDS_TEXT_ENTERMISSIONPASSWORD"),
      LoadResStr("IDS_DLG_MISSIONACCESS"), C4GUI::Ico_Options,
      new C4GUI::InputCallback<C4StartupScenSelDlg>(
          this, &C4StartupScenSelDlg::KeyCheat2),
      false));
}
...
void C4StartupScenSelDlg::FocusScenList() { SetFocus(pScenSelList, false); }
