...
CheckBox::CheckBox(const C4Rect &rtBounds, const char *szCaption, bool fChecked)
    : Control(rtBounds),
      ...
      fEnabled(true),
      pFont(NULL),
      dwEnabledClr(C4GUI_CheckboxFontClr),
      dwDisabledClr(C4GUI_CheckboxDisabledFontClr),
      cHotkey(0) {
  if (szCaption) {
  ...
  pKeyCheck =
      new C4KeyBinding(Keys, "GUICheckboxToggle", KEYSCOPE_Gui,
                       new ControlKeyCB<CheckBox>(*this, &CheckBox::KeyCheck),
...
void CheckBox::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
                          int32_t iY, DWORD dwKeyParam) {
  if (fEnabled) {
    // set mouse-on flag depending on whether mouse is over box area
    fMouseOn = Inside<int32_t>(iX, 0, rcBounds.Hgt) &&
               Inside<int32_t>(iY, 0, rcBounds.Hgt);
...
void CheckBox::MouseEnter(CMouse &rMouse) { Control::MouseEnter(rMouse); }
...
void CheckBox::DrawElement(C4FacetEx &cgo) {
  ...
  GetRes()->fctCheckbox.GetPhase(fChecked + 2 * !fEnabled).DrawX(
      cgo.Surface, x0, y0, rcBounds.Hgt, rcBounds.Hgt);
  ...
  lpDDraw->TextOut(sCaption.getData(), *pUseFont, fZoom, cgo.Surface,
                   x0 + rcBounds.Hgt + C4GUI_CheckBoxLabelSpacing, y0 + yOff,
                   fEnabled ? dwEnabledClr : dwDisabledClr, ALeft, true);
  // selection marker
  if ((fMouseOn && IsInActiveDlg(false)) || HasDrawFocus()) {
    lpDDraw->SetBlitMode(C4GFXBLIT_ADDITIVE);
    GetRes()->fctButtonHighlight.DrawX(
        cgo.Surface, x0 + rcBounds.Hgt * 1 / 4, y0 + rcBounds.Hgt * 1 / 4,
        rcBounds.Hgt * 1 / 2, rcBounds.Hgt * 1 / 2);
...
bool CheckBox::GetStandardCheckBoxSize(int *piWdt, int *piHgt,
                                       const char *szForCaptionText,
                                       CStdFont *pUseFont) {
