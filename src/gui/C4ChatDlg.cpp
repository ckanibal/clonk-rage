...
C4ChatControl::ChatSheet::NickItem::NickItem(class C4Network2IRCUser *pByUser)
    : pStatusIcon(NULL), pNameLabel(NULL), fFlaggedExisting(false), iStatus(0) {
  ...
  AddElement(pNameLabel =
                 new C4GUI::Label("", rcDefault, ALeft, C4GUI_CaptionFontClr,
...
void C4ChatControl::ChatSheet::NickItem::Update(
    class C4Network2IRCUser *pByUser) {
  // set status icon
  ...
  switch (*szPrefix) {
    case '!':
      eStatusIcon = C4GUI::Ico_Rank1;
      iStatus = 4;
      break;
    case '@':
      eStatusIcon = C4GUI::Ico_Rank2;
      iStatus = 3;
      break;
    case '%':
      eStatusIcon = C4GUI::Ico_Rank3;
      iStatus = 2;
      break;
    case '+':
      eStatusIcon = C4GUI::Ico_AddPlr;
      iStatus = 1;
      break;
    case '\0':
      eStatusIcon = C4GUI::Ico_Player;
      iStatus = 0;
      break;
    default:
      eStatusIcon = C4GUI::Ico_Player;
...
int32_t C4ChatControl::ChatSheet::NickItem::SortFunc(const C4GUI::Element *pEl1,
...
C4GUI::Edit::InputResult C4ChatControl::ChatSheet::OnChatInput(
  ...
  if (!szInputText || !*szInputText) {
    ...
    if (!pChatControl->ProcessInput(szInputText, this))
      eResult = C4GUI::Edit::IR_Abort;
  }
  // clear edit field after text has been processed
  pInputEdit->SelectAll();
  pInputEdit->DeleteSelection();
...
C4ChatControl::C4ChatControl(C4Network2IRCClient *pnIRCClient)
    : C4GUI::Window(),
      pTitleChangeBC(NULL),
      pIRCClient(pnIRCClient),
      fInitialMessagesReceived(false) {
  // create elements - positioned later
  C4Rect rcDefault(0, 0, 10, 10);
  ...
  // login sheet
  CStdFont *pUseFont = &C4GUI::GetRes()->TextFont;
  pSheetLogin->AddElement(pLblLoginNick = new C4GUI::Label(
                              LoadResStr("IDS_CTL_NICK"), rcDefault, ALeft,
                              C4GUI_CaptionFontClr, pUseFont, false, true));
  pSheetLogin->AddElement(
      pEdtLoginNick = new C4GUI::CallbackEdit<C4ChatControl>(
          rcDefault, this, &C4ChatControl::OnLoginDataEnter));
  pSheetLogin->AddElement(
      pLblLoginPass =
          new C4GUI::Label(LoadResStr("IDS_CTL_PASSWORDOPTIONAL"), rcDefault,
                           ALeft, C4GUI_CaptionFontClr, pUseFont, false, true));
  pSheetLogin->AddElement(
      pEdtLoginPass = new C4GUI::CallbackEdit<C4ChatControl>(
          rcDefault, this, &C4ChatControl::OnLoginDataEnter));
  pEdtLoginPass->SetPasswordMask('*');
  pSheetLogin->AddElement(pLblLoginRealName = new C4GUI::Label(
                              LoadResStr("IDS_CTL_REALNAME"), rcDefault, ALeft,
                              C4GUI_CaptionFontClr, pUseFont, false, true));
  pSheetLogin->AddElement(
      pEdtLoginRealName = new C4GUI::CallbackEdit<C4ChatControl>(
          rcDefault, this, &C4ChatControl::OnLoginDataEnter));
  pSheetLogin->AddElement(pLblLoginChannel = new C4GUI::Label(
                              LoadResStr("IDS_CTL_CHANNEL"), rcDefault, ALeft,
                              C4GUI_CaptionFontClr, pUseFont, false, true));
  pSheetLogin->AddElement(
      pEdtLoginChannel = new C4GUI::CallbackEdit<C4ChatControl>(
          rcDefault, this, &C4ChatControl::OnLoginDataEnter));
  pSheetLogin->AddElement(pBtnLogin =
                              new C4GUI::CallbackButtonEx<C4ChatControl>(
                                  LoadResStr("IDS_BTN_CONNECT"), rcDefault,
                                  this, &C4ChatControl::OnConnectBtn));
...
void C4ChatControl::OnShown() { UpdateShownPage(); }
...
C4ChatControl::ChatSheet *C4ChatControl::OpenQuery(
    const char *szForNick, bool fSelect, const char *szIdentFallback) {
  ...
  if (!pChatSheet && szIdentFallback)
    pChatSheet = GetSheetByIdent(szIdentFallback, ChatSheet::CS_Query);
  // auto-open query if not found
  if (!pChatSheet) {
    pTabChats->AddCustomSheet(pChatSheet = new ChatSheet(this, szForNick,
                                                         szIdentFallback,
                                                         ChatSheet::CS_Query));
    // initial chat title just user name; changed to user name+ident if a
    // message from the nick arrives
...
void C4ChatControl::ClearChatSheets() {
  ...
  // add server sheet
  pTabChats->AddCustomSheet(new ChatSheet(this, LoadResStr("IDS_CHAT_SERVER"),
                                          NULL, ChatSheet::CS_Server));
}
...
C4ChatDlg::C4ChatDlg() : C4GUI::Dialog(100, 100, "IRC", false) {
  ...
  pChatCtrl->SetTitleChangeCB(
      new C4GUI::InputCallback<C4ChatDlg>(this, &C4ChatDlg::OnChatTitleChange));
  AddElement(pChatCtrl);
  C4Rect rcDefault(0, 0, 10, 10);
  // pBtnClose = new
  // C4GUI::CallbackButton<C4ChatDlg>(LoadResStr("IDS_DLG_EXIT"), rcDefault,
  // &C4ChatDlg::OnExitBtn);
...
bool C4ChatDlg::IsChatEnabled() { return true; }

C4GUI::Control *C4ChatDlg::GetDefaultControl() {
  return pChatCtrl->GetDefaultControl();
}

bool C4ChatDlg::DoPlacement(C4GUI::Screen *pOnScreen,
  ...
  rcPos.x += rcPos.Wdt / 10;
  rcPos.Wdt -= rcPos.Wdt / 5;
  rcPos.y += rcPos.Hgt / 10;
  rcPos.Hgt -= rcPos.Hgt / 5;
  SetBounds(rcPos);
  return true;
}
...
void C4ChatDlg::OnExitBtn(C4GUI::Control *btn) { UserClose(false); }

void C4ChatDlg::OnChatTitleChange(const StdStrBuf &sNewTitle) {
  SetTitle(FormatString("%s - %s", LoadResStr("IDS_DLG_CHAT"),
