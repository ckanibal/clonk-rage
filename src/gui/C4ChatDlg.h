...
class C4ChatControl : public C4GUI::Window,
  ...
  class ChatSheet : public C4GUI::Tabular::Sheet {
   public:
    ...
    class NickItem : public C4GUI::Window {
     private:
      C4GUI::Icon *
          pStatusIcon;  // status icon indicating channel status (e.g. operator)
     ...
     public:
      ...
      static int32_t SortFunc(const C4GUI::Element *pEl1,
                              const C4GUI::Element *pEl2, void *);
    };
   ...
   private:
    ...
    StdStrBuf sIdent;
    bool fHasUnread;
    StdStrBuf sChatTitle;  // topic for channels; name+ident for queries; server
                           // name for server sheet

    C4KeyBinding *pKeyHistoryUp,
        *pKeyHistoryDown;  // keys used to scroll through chat history

   public:
    ChatSheet(C4ChatControl *pChatControl, const char *szTitle,
   ...
   protected:
    ...
    virtual void UserClose();  // user pressed close button: Close queries, part
                               // channels, etc.
   ...
   private:
    ...
    NickItem *GetFirstNickItem() {
      return pNickList ? static_cast<NickItem *>(pNickList->GetFirst()) : NULL;
    }
    NickItem *GetNextNickItem(NickItem *pPrev) {
      return static_cast<NickItem *>(pPrev->GetNext());
    }
  };
 ...
 private:
  ...
  // login controls
  C4GUI::Label *pLblLoginNick, *pLblLoginPass, *pLblLoginRealName,
      *pLblLoginChannel;
  C4GUI::Edit *pEdtLoginNick, *pEdtLoginPass, *pEdtLoginRealName,
      *pEdtLoginChannel;
  ...
  bool fInitialMessagesReceived;  // set after initial update call, which
                                  // fetches all messages. Subsequent calls
 ...
 protected:
  virtual void UpdateSize();
  C4GUI::Edit::InputResult OnLoginDataEnter(C4GUI::Edit *edt, bool fPasting,
                                            bool fPastingMore);  // advance
                                                                 // focus when
                                                                 // user presses
                                                                 // enter in one
                                                                 // of the login
 ...
 public:
  ...
  bool ProcessInput(const char *szInput,
                    ChatSheet *pChatSheet);  // process chat input - return
                                             // false if no more pasting is to
                                             // be done (i.e., on /quit or /part
                                             // in channel)
  const char *GetTitle() { return sTitle.getData(); }
  void UserQueryQuit();
  ChatSheet *OpenQuery(const char *szForNick, bool fSelect,
                       const char *szIdentFallback);

 private:
  static bool IsServiceName(const char *szName);
  ChatSheet *GetActiveChatSheet();
  ChatSheet *GetSheetByTitle(const char *szTitle,
                             C4ChatControl::ChatSheet::SheetType eType);
  ChatSheet *GetSheetByIdent(const char *szIdent,
                             C4ChatControl::ChatSheet::SheetType eType);
  ...
  virtual void OnThreadEvent(C4InteractiveEventType eEvent, void *pEventData) {
    if (pEventData == pIRCClient) Update();
  }
};
