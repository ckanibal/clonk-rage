...
C4MainMenu::C4MainMenu()
    : C4Menu()  // will be re-adjusted later
{
  Clear();
}
...
bool C4MainMenu::Init(C4FacetExSurface &fctSymbol, const char *szEmpty,
                      int32_t iPlayer, int32_t iExtra, int32_t iExtraData,
                      int32_t iId, int32_t iStyle) {
...
bool C4MainMenu::InitRefSym(const C4FacetEx &fctSymbol, const char *szEmpty,
                            int32_t iPlayer, int32_t iExtra, int32_t iExtraData,
                            int32_t iId, int32_t iStyle) {
...
bool C4MainMenu::ActivateNewPlayer(int32_t iPlayer) {
  ...
  // Menu symbol/init
  if (GfxR->fctPlayerClr.Surface) GfxR->fctPlayerClr.Surface->SetClr(0xff);
  ...
  for (DirectoryIterator iter(Config.General.PlayerPath); *iter; ++iter)
    if (WildcardMatch("*.c4p", *iter)) {
      ...
      if (Config.Graphics.ShowPortraits)
        if (!fctPortrait.Load(hGroup, C4CFN_BigIcon, C4FCT_Full, C4FCT_Full,
                              false, true))
          if (!fctPortrait.Load(hGroup, C4CFN_Portrait, C4FCT_Full, C4FCT_Full,
                                false, true))
            fctPortrait.Load(hGroup, C4CFN_Portrait_Old, C4FCT_Full, C4FCT_Full,
                             false, true);
      // Close group
      hGroup.Close();
      // Add player item
...
bool C4MainMenu::DoRefillInternal(bool &rfRefilled) {
  ...
  switch (Identification) {
      ...
      for (iIndex = 0; pPlr = Game.Players.GetByIndex(iIndex); iIndex++)
        ...
        if (pPlr != pPlayer)
          if (!pPlr->IsInvisible()) {
            ...
            sprintf(szInfoCaption, LoadResStr("IDS_MENU_ATTACKINFO"),
                    pPlr->GetName(), szFriendly, szNot);
            if (iIndex == pPlayer->Number)
              SCopy(LoadResStr("IDS_MENU_ATTACKSELF"), szInfoCaption);
            // Add item
            Add(sMsg.getData(), fctSymbol, szCommand, C4MN_Item_NoCount, NULL,
                szInfoCaption);
            fctSymbol.Default();
          }
      break;
    ...
    case C4MN_TeamSwitch: {
      ...
      C4Team *pTeam;
      int32_t i = 0;
      bool fAddNewTeam = Game.Teams.IsAutoGenerateTeams();
      for (;;) {
        ...
        // create team symbol: Icon spec if specified; otherwise flag for empty
        // and crew for nonempty team
        fctSymbol.Create(C4SymbolSize, C4SymbolSize);
        ...
        if (szIconSpec && *szIconSpec) {
          ...
          fHasIcon =
              Game.DrawTextSpecImage(fctSrc, szIconSpec, pTeam->GetColor());
          fctSrc.Draw(fctSymbol);
        }
        if (!fHasIcon) {
          if (pTeam && pTeam->GetPlayerCount())
            Game.GraphicsResource.fctCrewClr.DrawClr(fctSymbol, TRUE,
                                                     pTeam->GetColor());
          else
        ...
        const char *szOperation =
            (Identification == C4MN_TeamSwitch) ? "TeamSwitch" : "TeamSel";
        Add(sTeamName.getData(), fctSymbol,
            FormatString("%s:%d", szOperation,
                         pTeam ? pTeam->GetID() : TEAMID_New).getData(),
            C4MN_Item_NoCount, NULL,
            FormatString(LoadResStr("IDS_MSG_JOINTEAM"), sTeamName.getData())
                .getData(),
            C4ID(pTeam ? pTeam->GetID() : 0));
        fctSymbol.Default();
      }
      break;
    }
    ...
    case C4MN_Observer:  // observer menu
    {
      ...
      // Add free view
      AddRefSym(LoadResStr("IDS_MSG_FREEVIEW"),
                C4GUI::Icon::GetIconFacet(C4GUI::Ico_Star), "Observe:Free",
                C4MN_Item_NoCount, NULL,
                LoadResStr("IDS_MSG_FREELYSCROLLAROUNDTHEMAP"));
      // Add players
      C4Player *pPlr;
      int32_t iIndex;
      for (iIndex = 0; pPlr = Game.Players.GetByIndex(iIndex); iIndex++) {
        ...
        if (!pPlr->IsInvisible()) {
          // Symbol
          fctSymbol.Create(C4SymbolSize, C4SymbolSize);
          Game.GraphicsResource.fctPlayerClr.DrawClr(fctSymbol, TRUE,
                                                     pPlr->ColorDw);
          // Message
          StdStrBuf sMsg;
          DWORD dwClr = pPlr->ColorDw;
          sMsg.Format("<c %x>%s</c>", C4GUI::MakeColorReadableOnBlack(dwClr),
                      pPlr->GetName());
          // Command
          StdStrBuf sCommand;
          ...
          // Info caption
          StdStrBuf sInfo;
          sInfo.Format(LoadResStr("IDS_TEXT_FOLLOWVIEWOFPLAYER"),
                       pPlr->GetName());
          // Add item
          Add(sMsg.getData(), fctSymbol, sCommand.getData(), C4MN_Item_NoCount,
              NULL, sInfo.getData());
          fctSymbol.Default();
          // check if this is the currently selected player
          if (pVP->GetPlayer() == pPlr->Number)
            iInitialSelection = GetItemCount() - 1;
        }
...
bool C4MainMenu::ActivateGoals(int32_t iPlayer, bool fDoActivate) {
  ...
  if (fDoActivate) {
    // Menu symbol/init
    InitRefSym(GfxR->fctMenu.GetPhase(4), LoadResStr("IDS_MENU_CPGOALS"),
               iPlayer);
    SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
    SetPermanent(false);
    fctGF.Set(NULL, C4SymbolSize - Game.GraphicsResource.fctCaptain.Wdt - 2, 2,
              Game.GraphicsResource.fctCaptain.Wdt,
              Game.GraphicsResource.fctCaptain.Hgt);
  }
  // determine if the goals are fulfilled - do the calls even if the menu is not
  // to be opened to ensure synchronization
    ...
    for (int32_t i = 0; i < iNumGoals; ++i)
      if (idGoal = GoalList.GetID(i, &cnt))
        if (pDef = C4Id2Def(idGoal)) {
          ...
          // this would allow us to do dynamic pictures and overlays; e.g. draw
          // the actual, required settlement score
          ...
          Add(pDef->GetName(), fctSymbol, Command.getData(), C4MN_Item_NoCount,
              NULL, pDef->GetDesc());
        }
    // Go back to options menu on close
    SetCloseCommand("ActivateMenu:Main");
...
bool C4MainMenu::ActivateRules(int32_t iPlayer) {
  ...
  char Command[256];
  C4FacetExSurface fctSymbol;
  InitRefSym(GfxR->fctMenu.GetPhase(5), LoadResStr("IDS_MENU_CPRULES"),
             iPlayer);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  SetPermanent(false);
  // Items
  ...
  C4ID idGoal;
  C4Def *pDef;
  for (cnt = 0; idGoal = Game.Objects.ObjectsInt().GetListID(C4D_Rule, cnt);
       cnt++)
    if (pDef = C4Id2Def(idGoal)) {
      fctSymbol.Create(C4SymbolSize, C4SymbolSize);
      pDef->Draw(fctSymbol);
...
bool LooksLikeInteger(const char *szInt) {
  ...
  char c;
  while (c = *(szInt++))
    if (!Inside<char>(c, '0', '9')) return false;
  // it's an int32_t
  return true;
}
...
bool C4MainMenu::ActivateSavegame(int32_t iPlayer) {
  ...
  char ScenName[_MAX_PATH + 1];
  *ScenName = 0;

  InitRefSym(GfxR->fctMenu.GetPhase(0), LoadResStr("IDS_MENU_CPSAVEGAME"),
             iPlayer);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  SetPermanent(true);
  ...
  // some magic is needed to ensure savegames are stored properly into their
  // folders
  ...
  if (LooksLikeInteger(DirPath)) {
    ...
    while (Inside<char>(*--pScenNameEnd, '0', '9'))
      if (pScenNameEnd == ScenName) {
        // should not happen: digit-only-filenames should have been caught
        // earlier
  ...
  // New Style 2007:
  // * scenarios are saved into ScenName.c4f/ScenName123.c4s to keep umlauts out
  // of filenames
  // * language titles are stored in folders as title component
  StdStrBuf strFilename, strTitle;
  strFilename.Format("%s.c4f%c%s%%d.c4s", ScenName, DirectorySeparator,
                     ScenName);
  ...
  for (int32_t i = 1; i <= 10; i++) {
    ...
    // Compose commmand
    strCommand.Format("Save:Game:%s:%s", strFilenameIndexed.getData(),
                      strTitle.getData());  // Notice: the language title might
                                            // contain ':' and thus confuse the
                                            // segment list - but
                                            // C4Menu::MenuCommand will try to
                                            // handle this...
    // Check free slot
    strSavePath.Format("%s%c%s", Config.General.SaveGameFolder.getData(),
                       DirectorySeparator, strFilenameIndexed.getData());
    bool fFree = !C4Group_IsGroup(strSavePath.getData());
    // Item caption
    strCaption = LoadResStr("IDS_MENU_CPSAVEGAME");
    // add menu item
    AddRefSym(strCaption.getData(),
              GfxR->fctMenu.GetPhase(i - 1, fFree ? 2 : 1),
              strCommand.getData(), C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MENU_CPSAVEGAMEINFO"));
  }
...
bool C4MainMenu::ActivateHost(int32_t iPlayer) {
  // Menu symbol/init
  InitRefSym(C4GUI::Icon::GetIconFacet(C4GUI::Ico_Disconnect),
             LoadResStr("IDS_MENU_DISCONNECTCLIENT"), iPlayer, C4MN_Extra_None,
             0, 0, C4MN_Style_Context);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  SetPermanent(true);
  // Clients
  for (C4Network2Client *pClient = Game.Network.Clients.GetNextClient(NULL);
    ...
    C4GUI::Icons iIcon =
        fHost ? C4GUI::Ico_Host
              : (pClient->isActivated() ? C4GUI::Ico_Client
                                        : C4GUI::Ico_ObserverClient);
    AddRefSym(strText.getData(), C4GUI::Icon::GetIconFacet(iIcon),
              strCommand.getData());
  }
  // Go back to options menu on close
  SetCloseCommand("ActivateMenu:Main");
...
bool C4MainMenu::ActivateClient(int32_t iPlayer) {
  ...
  C4FacetExSurface fctSymbol;
  InitRefSym(C4GUI::Icon::GetIconFacet(C4GUI::Ico_Disconnect),
             LoadResStr("IDS_MENU_DISCONNECTFROMSERVER"), iPlayer,
             C4MN_Extra_None, 0, 0, C4MN_Style_Context);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  fctSymbol.Create(C4SymbolSize, C4SymbolSize);
  GfxR->fctOKCancel.Draw(fctSymbol, TRUE, 3, 0);
  Add(LoadResStr("IDS_BTN_YES"), fctSymbol, "Part");
  fctSymbol.Create(C4SymbolSize, C4SymbolSize);
  GfxR->fctOKCancel.Draw(fctSymbol, TRUE, 1, 0);
...
bool C4MainMenu::ActivateSurrender(int32_t iPlayer) {
  C4FacetExSurface fctSymbol;
  InitRefSym(C4GUI::Icon::GetIconFacet(C4GUI::Ico_Surrender),
             LoadResStr("IDS_MENU_SURRENDER"), iPlayer, C4MN_Extra_None, 0, 0,
             C4MN_Style_Context);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  fctSymbol.Create(C4SymbolSize, C4SymbolSize);
  GfxR->fctOKCancel.Draw(fctSymbol, TRUE, 3, 0);
...
bool C4MainMenu::ActivateOptions(int32_t iPlayer, int32_t selection) {
  // Menu symbol/init
  InitRefSym(GfxR->fctOptions.GetPhase(0), LoadResStr("IDS_MNU_OPTIONS"),
             iPlayer, C4MN_Extra_None, 0, 0, C4MN_Style_Context);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  SetPermanent(true);
  // Sound
  AddRefSym(LoadResStr("IDS_DLG_SOUND"),
            GfxR->fctOptions.GetPhase(17 + Config.Sound.RXSound),
            "Options:Sound", C4MN_Item_NoCount);
  // Music
  AddRefSym(LoadResStr("IDS_MNU_MUSIC"),
            GfxR->fctOptions.GetPhase(1 + Config.Sound.RXMusic),
            "Options:Music", C4MN_Item_NoCount);
  // Mouse control
  C4Player *pPlr = Game.Players.Get(iPlayer);
  if (pPlr && !Game.C4S.Head.DisableMouse) {
    if (pPlr->MouseControl)
      AddRefSym(LoadResStr("IDS_MNU_MOUSECONTROL"),
                GfxR->fctOptions.GetPhase(11 + 1), "Options:Mouse");
    else if (!Game.Players.MouseControlTaken())
      AddRefSym(LoadResStr("IDS_MNU_MOUSECONTROL"),
                GfxR->fctOptions.GetPhase(11), "Options:Mouse");
  }
  // Music
  AddRefSym(LoadResStr("IDS_MENU_DISPLAY"), GfxR->fctMenu.GetPhase(8),
...
bool C4MainMenu::ActivateDisplay(int32_t iPlayer, int32_t selection) {
  // Menu symbol/init
  InitRefSym(GfxR->fctMenu.GetPhase(8), LoadResStr("IDS_MENU_DISPLAY"), iPlayer,
             C4MN_Extra_None, 0, 0, C4MN_Style_Context);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  SetPermanent(true);
  // Crew player names
  AddRefSym(LoadResStr("IDS_MNU_PLAYERNAMES"),
            GfxR->fctOptions.GetPhase(7 + Config.Graphics.ShowCrewNames),
            "Display:PlayerNames", C4MN_Item_NoCount, NULL,
            LoadResStr("IDS_MENU_PLAYERNAMES_DESC"));
  // Crew clonk names
  AddRefSym(LoadResStr("IDS_MNU_CLONKNAMES"),
            GfxR->fctOptions.GetPhase(9 + Config.Graphics.ShowCrewCNames),
            "Display:ClonkNames", C4MN_Item_NoCount, NULL,
            LoadResStr("IDS_MENU_CLONKNAMES_DESC"));
  // Portraits
  AddRefSym(LoadResStr("IDS_MNU_PORTRAITS"),
            GfxR->fctOptions.GetPhase(13 + Config.Graphics.ShowPortraits),
            "Display:Portraits", C4MN_Item_NoCount);
  // ShowCommands
  AddRefSym(LoadResStr("IDS_MENU_SHOWCOMMANDS"),
            GfxR->fctOptions.GetPhase(19 + Config.Graphics.ShowCommands),
            "Display:ShowCommands", C4MN_Item_NoCount);
  // ShowCommandKeys
  AddRefSym(LoadResStr("IDS_MENU_SHOWCOMMANDKEYS"),
            GfxR->fctOptions.GetPhase(21 + Config.Graphics.ShowCommandKeys),
            "Display:ShowCommandKeys", C4MN_Item_NoCount);
  // Upper Board
  if (Application.isFullScreen) {
    AddRefSym(LoadResStr("IDS_MNU_UPPERBOARD"),
              GfxR->fctOptions.GetPhase(3 + Config.Graphics.UpperBoard),
              "Display:UpperBoard", C4MN_Item_NoCount);
  }
  // FPS
  if (Application.isFullScreen)
    AddRefSym(LoadResStr("IDS_MNU_FPS"),
              GfxR->fctOptions.GetPhase(5 + Config.General.FPS), "Display:FPS",
              C4MN_Item_NoCount);
  // Clock
  if (Application.isFullScreen)
    AddRefSym(LoadResStr("IDS_MNU_CLOCK"),
              GfxR->fctOptions.GetPhase(15 + Config.Graphics.ShowClock),
              "Display:Clock", C4MN_Item_NoCount);
  // Restore selection
  SetSelection(selection, false, true);
  // Go back to options menu on close
...
bool C4MainMenu::ActivateMain(int32_t iPlayer) {
  ...
  Init(fctSymbol, LoadResStr(pPlr ? "IDS_MENU_CPMAIN" : "IDS_MENU_OBSERVER"),
       iPlayer, C4MN_Extra_None, 0, 0, C4MN_Style_Context);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  // Goals+Rules (player menu only)
  // Goal menu can't be shown because of script callbacks
  // Rule menu could be shown, but rule activation would issue script callbacks
  // and trigger client activation
  // Showing rules but not showing goals would be strange anyway
  if (pPlr) {
    // Goals
    AddRefSym(LoadResStr("IDS_MENU_CPGOALS"), GfxR->fctMenu.GetPhase(4),
              "ActivateMenu:Goals", C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MENU_CPGOALSINFO"));
    // Rules
    AddRefSym(LoadResStr("IDS_MENU_CPRULES"), GfxR->fctMenu.GetPhase(5),
              "ActivateMenu:Rules", C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MENU_CPRULESINFO"));
  }
  // Observer menu in free viewport
  if (!pPlr) {
    AddRefSym(LoadResStr("IDS_TEXT_VIEW"),
              C4GUI::Icon::GetIconFacet(C4GUI::Ico_View),
              "ActivateMenu:Observer", C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_TEXT_DETERMINEPLAYERVIEWTOFOLL"));
  }
  // Hostility (player menu only)
  if (pPlr && (Game.Players.GetCount() > 1)) {
    GfxR->fctFlagClr.Surface->SetClr(0xff0000);
    AddRefSym(LoadResStr("IDS_MENU_CPATTACK"), GfxR->fctMenu.GetPhase(7),
              "ActivateMenu:Hostility", C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MENU_CPATTACKINFO"));
  }
  // Team change
  if (pPlr && Game.Teams.IsTeamSwitchAllowed()) {
    C4FacetEx fctTeams;
    fctTeams = C4GUI::Icon::GetIconFacet(C4GUI::Ico_Team);
    AddRefSym(LoadResStr("IDS_MSG_SELTEAM"), fctTeams, "ActivateMenu:TeamSel",
              C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MSG_ALLOWSYOUTOJOINADIFFERENT"));
  }
  // Player join
  if ((Game.Players.GetCount() < Game.Parameters.MaxPlayers) &&
      !Game.Parameters.isLeague()) {
    AddRefSym(LoadResStr("IDS_MENU_CPNEWPLAYER"), GfxR->fctPlayerClr.GetPhase(),
              "ActivateMenu:NewPlayer", C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MENU_CPNEWPLAYERINFO"));
  }
  // Save game (player menu only - should we allow saving games with no players
  // in it?)
  if (pPlr && (!Game.Network.isEnabled() || Game.Network.isHost())) {
    AddRefSym(LoadResStr("IDS_MENU_CPSAVEGAME"), GfxR->fctMenu.GetPhase(0),
              "ActivateMenu:Save:Game", C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MENU_CPSAVEGAMEINFO"));
  }
  // Options
  AddRefSym(LoadResStr("IDS_MNU_OPTIONS"), GfxR->fctOptions.GetPhase(0),
            "ActivateMenu:Options", C4MN_Item_NoCount, NULL,
            LoadResStr("IDS_MNU_OPTIONSINFO"));
  // Disconnect
  if (Game.Network.isEnabled()) {
    // Host
    if (Game.Network.isHost() && Game.Clients.getClient(NULL))
      AddRefSym(LoadResStr("IDS_MENU_DISCONNECT"),
                C4GUI::Icon::GetIconFacet(C4GUI::Ico_Disconnect),
                "ActivateMenu:Host", C4MN_Item_NoCount, NULL,
                LoadResStr("IDS_TEXT_KICKCERTAINCLIENTSFROMTHE"));
    // Client
    if (!Game.Network.isHost())
      AddRefSym(LoadResStr("IDS_MENU_DISCONNECT"),
                C4GUI::Icon::GetIconFacet(C4GUI::Ico_Disconnect),
                "ActivateMenu:Client", C4MN_Item_NoCount, NULL,
                LoadResStr("IDS_TEXT_DISCONNECTTHEGAMEFROMTHES"));
  }
  // Surrender (player menu only)
  if (pPlr)
    AddRefSym(LoadResStr("IDS_MENU_CPSURRENDER"),
              C4GUI::Icon::GetIconFacet(C4GUI::Ico_Surrender),
              "ActivateMenu:Surrender", C4MN_Item_NoCount, NULL,
              LoadResStr("IDS_MENU_CPSURRENDERINFO"));
  // Abort
  if (Application.isFullScreen)
    AddRefSym(LoadResStr("IDS_MENU_ABORT"),
              C4GUI::Icon::GetIconFacet(C4GUI::Ico_Exit), "Abort",
              C4MN_Item_NoCount, NULL, LoadResStr("IDS_MENU_ABORT_DESC"));
  // No empty menus
...
bool C4MainMenu::ActivateHostility(int32_t iPlayer) {
  ...
  Init(fctSymbol, LoadResStr("IDS_MENU_CPATTACK"), iPlayer, C4MN_Extra_None, 0,
       C4MN_Hostility);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
  SetPermanent(true);
  Refill();
...
bool C4MainMenu::MenuCommand(const char *szCommand, bool fIsCloseCommand) {
  ...
  if (SEqual2(szCommand, "ActivateMenu:")) {
    if (C4GameOverDlg::IsShown())
      return false;  // no new menus during game over dlg
    ...
    if (SEqual(szCommand + 13, "Goals")) {
      Game.Control.DoInput(
          CID_Script,
          new C4ControlScript(
              FormatString("ActivateGameGoalMenu(%d)", Player).getData()),
          CDT_Queue);
      return true;
    }
    ...
    if (SEqual(szCommand + 13, "TeamSel"))
      return pPlr ? pPlr->ActivateMenuTeamSelection(true) : false;
  ...
  if (SEqual2(szCommand, "JoinPlayer:")) {
    ...
    if (Game.Network.isEnabled())
      ...
      Game.Players.CtrlJoinLocalNoNetwork(szCommand + 11,
  ...
  if (SEqual2(szCommand, "SetHostility:")) {
    ...
    Game.Input.Add(
        CID_Script,
        new C4ControlScript(
            FormatString("SetHostility(%d, %d, !Hostile(%d, %d, true))", Player,
                         iOpponent, Player, iOpponent).getData(),
            C4ControlScript::SCOPE_Global, true));
    return true;
  }
  // Abort
  ...
  if (SEqual2(szCommand, "Surrender")) {
    Game.Control.DoInput(CID_Script,
                         new C4ControlScript(FormatString("SurrenderPlayer(%d)",
                                                          Player).getData()),
                         CDT_Queue);
    return true;
  }
  ...
  if (SEqual2(szCommand, "Save:Game:")) {
    ...
    SCopySegment(szCommand, 2, strFilename, ':', _MAX_PATH);
    char strTitle[_MAX_PATH + 1];
    SCopy(szCommand + SCharPos(':', szCommand, 2) + 1, strTitle, _MAX_PATH);
  ...
  if (SEqual2(szCommand, "Host:Kick:")) {
    ...
    if (iClientID && Game.Network.isEnabled())
      if (Game.Parameters.isLeague() && Game.Players.GetAtClient(iClientID))
        ...
        if (pClient)
          Game.Clients.CtrlRemove(pClient, LoadResStr("IDS_MSG_KICKBYMENU"));
        Close(true);
      }
    return true;
  }
    ...
    if (Game.Network.isEnabled())
      if (Game.Parameters.isLeague() && Game.Players.GetLocalByIndex(0))
        ...
        Game.RoundResults.EvaluateNetwork(
            ...
            LoadResStr("IDS_ERR_GAMELEFTVIAPLAYERMENU"));
        Game.Network.Clear();
      }
    return true;
  }
  ...
  if (SEqual2(szCommand, "Options:")) {
    ...
    if (SEqual(szCommand + 8, "Mouse"))
      if (pPlr) pPlr->ToggleMouseControl();
  ...
  if (SEqual2(szCommand, "Display:")) {
    ...
    // Player names
    if (SEqual(szCommand + 8, "PlayerNames"))
      Config.Graphics.ShowCrewNames = !Config.Graphics.ShowCrewNames;
    // Clonk names
    if (SEqual(szCommand + 8, "ClonkNames"))
      Config.Graphics.ShowCrewCNames = !Config.Graphics.ShowCrewCNames;
    // Portraits
    if (SEqual(szCommand + 8, "Portraits"))
      Config.Graphics.ShowPortraits = !Config.Graphics.ShowPortraits;
    // ShowCommands
    if (SEqual(szCommand + 8, "ShowCommands"))
      Config.Graphics.ShowCommands = !Config.Graphics.ShowCommands;
    // ShowCommandKeys
    if (SEqual(szCommand + 8, "ShowCommandKeys"))
      Config.Graphics.ShowCommandKeys = !Config.Graphics.ShowCommandKeys;
    // Clock
    if (SEqual(szCommand + 8, "Clock"))
      Config.Graphics.ShowClock = !Config.Graphics.ShowClock;
    // Reopen with updated options
    ActivateDisplay(Player, GetSelection());
    return true;
  }
  // Goal info
  if (SEqual2(szCommand, "Player:Goal:") ||
      SEqual2(szCommand, "Player:Rule:")) {
    if (!ValidPlr(Player))
      return false;  // observers may not look at goal/rule info, because it
                     // requires queue activation
    Close(true);
    // TODO!
    C4Object *pObj;
    C4ID idItem = C4Id(szCommand + 12);
    if (pObj = Game.Objects.FindInternal(idItem))
      Game.Control.DoInput(
          CID_Script,
          new C4ControlScript(FormatString("Activate(%d)", Player).getData(),
                              pObj->Number),
          CDT_Queue);
    else
      return false;
  ...
  if (SEqual2(szCommand, "TeamSwitch:")) {
    ...
    Game.Control.DoInput(
        CID_Script,
        new C4ControlScript(FormatString("SetPlayerTeam(%d,%d)", (int)Player,
                                         (int)idTeam).getData()),
        CDT_Queue);
    return true;
  }
...
bool C4MainMenu::ActivateObserver() {
  ...
  // Menu symbol/init
  InitRefSym(C4GUI::Icon::GetIconFacet(C4GUI::Ico_View),
             LoadResStr("IDS_TEXT_VIEW"), NO_OWNER, C4MN_Extra_None, 0,
             C4MN_Observer, C4MN_Style_Context);
  SetAlignment(C4MN_Align_Left | C4MN_Align_Bottom);
