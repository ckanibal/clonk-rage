...
void Container::Draw(C4FacetEx &cgo) {
  // self visible?
  if (!IsVisible()) return;
  ...
  for (Element *pEl = pFirst; pEl; pEl = pEl->pNext)
    if (pEl->fVisible) {
      ...
      if (!pEl->IsExternalDrawDialog()) pEl->Draw(cgo);
    }
}
...
void Container::Clear() { ClearChildren(); }
...
void Container::ClearChildren() {
  ...
  while (pFirst) {
    if (pFirst->IsOwnPtrElement()) {
      ...
      pFirst->pParent = NULL;
      if (pFirst = pANext) pFirst->pPrev = NULL;
    } else
      delete pFirst;
  }
}
...
void Container::InsertElement(Element *pChild, Element *pInsertBefore) {
  ...
  if (pChild->pPrev = pInsertBefore->pPrev)
    ...
    pFirst = pChild;
  pChild->pNext = pInsertBefore;
  pInsertBefore->pPrev = pChild;
  pChild->pParent = this;
}

Element *Container::GetNextNestedElement(Element *pPrevElement,
                                         bool fBackwards) {
  if (fBackwards) {
    // this is last
    ...
    if (pPrevElement->pPrev)
      return pPrevElement->pPrev->GetFirstNestedElement(true);
...
Element *Container::GetFirstNestedElement(bool fBackwards) {
  ...
  if (pFirst)
    return (fBackwards ? pLast : pFirst)->GetFirstNestedElement(fBackwards);
...
Element *Container::GetElementByIndex(int32_t i) {
  ...
  // if i is negative or equal or larger than childcount, the loop will never
  // break and NULL returned
  Element *pEl;
...
bool Container::IsParentOf(Element *pEl) {
  // return whether this is the parent container (directly or recursively) of
  // the passed element
...
void Window::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX, int32_t iY,
                        DWORD dwKeyParam) {
  // invisible?
  if (!IsVisible()) return;
  // inherited
  ...
  iX -= rcClientRect.x - rcBounds.x;
  iY -= rcClientRect.y - rcBounds.y;
  ...
  for (Element *pChild = pLast; pChild; pChild = pChild->GetPrev())
    if (pChild->fVisible && pChild->GetBounds().Contains(iX, iY)) {
      // forward
      pChild->MouseInput(rMouse, iButton, iX - pChild->GetBounds().x,
                         iY - pChild->GetBounds().y, dwKeyParam);
      // forward to one control only
      break;
    }
}

void Window::Draw(C4FacetEx &cgo) {
  // invisible?
  if (!IsVisible()) return;
  // draw window itself
  DrawElement(cgo);
  ...
  C4Rect &rcClipArea =
      (IsComponentOutsideClientArea() ? GetBounds() : GetClientRect());
  // clip to window area
  int clx1, cly1, clx2, cly2;
  lpDDraw->GetPrimaryClipper(clx1, cly1, clx2, cly2);
  lpDDraw->SubPrimaryClipper(cgo.TargetX + rcClipArea.x,
                             cgo.TargetY + rcClipArea.y,
                             cgo.TargetX + rcClipArea.x + rcClipArea.Wdt - 1,
                             cgo.TargetY + rcClipArea.y + rcClipArea.Hgt - 1);
  // update target area
  cgo.TargetX += rcClientRect.x;
  cgo.TargetY += rcClientRect.y;
  // draw contents
  Container::Draw(cgo);
  // reset target area
  cgo.TargetX -= rcClientRect.x;
...
void Window::UpdateOwnPos() {
  ...
  rcClientRect.Set(
      rcBounds.x + iMarginL, rcBounds.y + iMarginT,
      Max<int32_t>(rcBounds.Wdt - iMarginL - GetMarginRight(), 0),
      Max<int32_t>(rcBounds.Hgt - iMarginT - GetMarginBottom(), 0));
}

// --------------------------------------------------
// ScrollBar

ScrollBar::ScrollBar(C4Rect &rcBounds, ScrollWindow *pWin)
    : fAutoHide(false),
      pCustomGfx(NULL),
      fHorizontal(false),
      pScrollCallback(NULL),
      iCBMaxRange(100) {
  // set bounds
...
ScrollBar::ScrollBar(C4Rect &rcBounds, bool fHorizontal,
                     BaseParCallbackHandler<int32_t> *pCB, int32_t iCBMaxRange)
    : fAutoHide(false),
      pCustomGfx(NULL),
      fHorizontal(fHorizontal),
      pScrollWindow(NULL),
      iCBMaxRange(iCBMaxRange) {
  // set bounds
  this->rcBounds = rcBounds;
  // set initial values
...
void ScrollBar::Update() {
  ...
  if (pScrollWindow) {
    ...
    if (fScrolling = (iVisHgt < iClientHgt)) {
      ...
      // calc scroll pos
      iScrollPos = BoundBy<int32_t>(iMaxBarScroll * iWinScroll / iMaxWinScroll,
                                    0, iMaxBarScroll);
    }
  } else
    fScrolling = !!pScrollCallback;
  // reset buttons
  if (!fScrolling) fTopDown = fBottomDown = false;
...
void ScrollBar::OnPosChanged() {
  ...
  if (pScrollCallback)
    pScrollCallback->DoCall(BoundBy<int32_t>(
        iScrollPos * (iCBMaxRange - 1) / iMaxBarScroll, 0, (iCBMaxRange - 1)));
  // safety
  ...
  int32_t iNewWinScroll = BoundBy<int32_t>(
      iMaxWinScroll * iScrollPos / iMaxBarScroll, 0, iMaxWinScroll);
  // apply it, if it is different
  if (iWinScroll != iNewWinScroll) pScrollWindow->SetScroll(iNewWinScroll);
}

void ScrollBar::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
  ...
  if (rMouse.IsLDown())
    ...
    if (fHorizontal ? Inside<int32_t>(iY, 0, C4GUI_ScrollBarHgt)
                    : Inside<int32_t>(iX, 0, C4GUI_ScrollBarWdt)) {
      // scroll-direction area check: up/left arrow
      if (fHorizontal ? Inside<int32_t>(iX, 0, C4GUI_ScrollArrowWdt - 1)
                      : Inside<int32_t>(iY, 0, C4GUI_ScrollArrowHgt - 1))
        fTopDown = true;
      // check down arrow
      else if (fHorizontal
                   ? Inside<int32_t>(iX, GetBounds().Wdt - C4GUI_ScrollArrowWdt,
                                     GetBounds().Wdt - 1)
                   : Inside<int32_t>(iY, GetBounds().Hgt - C4GUI_ScrollArrowHgt,
                                     GetBounds().Hgt - 1))
        fBottomDown = true;
      else if (HasPin() &&
               (fHorizontal ? Inside<int32_t>(
                                  iX, C4GUI_ScrollArrowWdt,
                                  GetBounds().Wdt - C4GUI_ScrollArrowWdt - 1)
                            : Inside<int32_t>(iY, C4GUI_ScrollArrowHgt,
                                              GetBounds().Hgt -
                                                  C4GUI_ScrollArrowHgt - 1))) {
        // move thumb here
...
void ScrollBar::DoDragging(CMouse &rMouse, int32_t iX, int32_t iY,
                           DWORD dwKeyParam) {
  // move thumb
  iScrollPos = GetScrollByPos(iX, iY);
...
void ScrollBar::DrawElement(C4FacetEx &cgo) {
  // do scrolling
  // not quite perfect; but there's no OnIdle, and it's be a bit of overkill
  // starting a timer
  ...
  if (fScrolling && HasPin())
    if (fHorizontal)
      rUseGfx.fctScrollPin.Draw(
          cgo.Surface,
          cgo.TargetX + rcBounds.x + C4GUI_ScrollArrowWdt + iScrollPos,
          cgo.TargetY + rcBounds.y);
    else
      rUseGfx.fctScrollPin.Draw(
          cgo.Surface, cgo.TargetX + rcBounds.x,
          cgo.TargetY + rcBounds.y + C4GUI_ScrollArrowHgt + iScrollPos);
}
...
void ScrollWindow::Update() {
  ...
  iScrollY = BoundBy<int32_t>(iScrollY, 0,
                              Max<int32_t>(iClientHeight - GetBounds().Hgt, 0));
  // update client rect
  rcClientRect.x = 0;
...
void ScrollWindow::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
                              int32_t iY, DWORD dwKeyParam) {
...
void GroupBox::DrawElement(C4FacetEx &cgo) {
  ...
  if (HasTitle()) {
    ...
    iTitleGapX = 7;
    iTitleGapWdt += 4;
    lpDDraw->TextOut(sTitle.getData(), *pTitleFont, 1.0f, cgo.Surface,
                     cgo.TargetX + rcBounds.x + iTitleGapX + 2,
                     cgo.TargetY + rcBounds.y, dwTitleClr);
  }
  // draw frame
  if (dwFrameClr) {
    int32_t x1 = cgo.TargetX + rcBounds.x,
            y1 = cgo.TargetY + rcBounds.y + iBorderYOff, x2 = x1 + rcBounds.Wdt,
            y2 = y1 + rcBounds.Hgt - iBorderYOff;
    if (iTitleGapWdt) {
      for (int i = 0; i < 2; ++i) {
        lpDDraw->DrawLineDw(cgo.Surface, (float)x1 + i, (float)y1,
                            (float)(x1 + i), (float)(y2 - 1),
                            dwFrameClr);  // left
        lpDDraw->DrawLineDw(cgo.Surface, (float)(x1 + 2), (float)(y1 + i),
                            (float)(x1 + iTitleGapX), (float)(y1 + i),
                            dwFrameClr);  // top - left side
        lpDDraw->DrawLineDw(cgo.Surface,
                            (float)(x1 + iTitleGapX + iTitleGapWdt),
                            (float)(y1 + i), (float)(x2 - 3), (float)(y1 + i),
                            dwFrameClr);  // top - right side
        lpDDraw->DrawLineDw(cgo.Surface, (float)(x2 - 1 - i), (float)y1,
                            (float)(x2 - 1 - i), (float)(y2 - 1),
                            dwFrameClr);  // right
        lpDDraw->DrawLineDw(cgo.Surface, (float)(x1 + 2), (float)(y2 - 1 - i),
                            (float)(x2 - 3), (float)(y2 - 1 - i),
                            dwFrameClr);  // bottom
      }
    } else {
...
Control::Control(const C4Rect &rtBounds) : Window() {
  ...
  pKeyContext =
      new C4KeyBinding(C4KeyCodeEx(K_MENU), "GUIContext", KEYSCOPE_Gui,
                       new ControlKeyCB<Control>(*this, &Control::KeyContext),
                       C4CustomKey::PRIO_Ctrl);
}

Control::~Control() { delete pKeyContext; }

void Control::MouseInput(CMouse &rMouse, int32_t iButton, int32_t iX,
                         int32_t iY, DWORD dwKeyParam) {
  if (!IsVisible()) return;
  // inherited
  Window::MouseInput(rMouse, iButton, iX, iY, dwKeyParam);
  // left down on click=focus-components?
  if (IsFocusOnClick() && IsFocusElement())
    if (iButton == C4MC_Button_LeftDown && !HasFocus()) {
      // then set focus
