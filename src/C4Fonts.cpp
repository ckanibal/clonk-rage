...
bool C4VectorFont::Init(C4Group &hGrp, const char *szFilename, C4Config &rCfg) {
  ...
  if (!hGrp.IsPacked()) {
    ...
    // make sure the filename is not in use, in case multiple instances of the
    // engine are run
    if (FileExists(FileName)) {
    ...
    if (!hGrp.ExtractEntry(szFilename, FileName)) {
      *FileName = 0;
      return false;
    }
    fIsTempFile = true;
...
bool C4VectorFont::Init(const char *szFacename, int32_t iSize,
                        uint32_t dwWeight, const char *szCharSet) {
  // name by face
  Name.Copy(szFacename);
#if defined(_WIN32) && defined(HAVE_FREETYPE)
  // Win32 using freetype: Load TrueType-data from WinGDI into Data-buffer to be
  // used by FreeType
  bool fSuccess = false;
  HDC hDC = ::CreateCompatibleDC(NULL);
  if (hDC) {
    HFONT hFont =
        ::CreateFont(iSize, 0, 0, 0, dwWeight, FALSE, FALSE, FALSE,
                     GetCharsetCode(szCharSet), OUT_DEFAULT_PRECIS,
                     CLIP_DEFAULT_PRECIS, 5, VARIABLE_PITCH, szFacename);
    if (hFont) {
      SelectObject(hDC, hFont);
      uint32_t dwTTFSize = ::GetFontData(hDC, 0, 0, NULL, 0);
      if (dwTTFSize && dwTTFSize != GDI_ERROR) {
        Data.SetSize(dwTTFSize);
        uint32_t dwRealTTFSize =
            ::GetFontData(hDC, 0, 0, Data.getMData(), dwTTFSize);
...
bool C4FontLoader::IsFontLoaded(const char *szFontName) {
  ...
  // check all fonts
  for (std::vector<C4FontDef>::iterator i = FontDefs.begin();
       i != FontDefs.end(); ++i)
    if (i->Name == szFontName) return true;
  // not found
  return false;
}
...
int32_t C4FontLoader::GetClosestAvailableSize(const char *szFontName,
                                              int32_t iWantedSize) {
  // safety
...
bool C4FontLoader::InitFont(CStdFont &rFont, C4VectorFont *pFont, int32_t iSize,
                            uint32_t dwWeight, bool fDoShadow) {
  if (!pFont->pFont) {
#ifdef HAVE_FREETYPE
    ...
    if (!pFont->pFont)
      pFont->pFont = CStdFont::CreateFont(pFont->Name.getData());
    if (!pFont->pFont) return false;  // this font can't be used
  }
  rFont.Init(*(pFont->pFont), iSize, dwWeight, Config.General.LanguageCharset,
             fDoShadow);  // throws exception on error
  return true;
}

bool C4FontLoader::InitFont(CStdFont &rFont, const char *szFontName,
                            FontType eType, int32_t iSize,
                            C4GroupSet *pGfxGroups, bool fDoShadow) {
  // safety
  ...
  // get font to load
  // pFontDefs may be NULL if no fonts are loaded; but then iFontDefCount is
  // zero as well
  // the function must not be aborted, because a standard windows font may be
  // loaded
  std::vector<C4FontDef>::iterator pFontDefC = FontDefs.begin(),
                                   pFontDef = FontDefs.end();
  while (pFontDefC != FontDefs.end()) {
    ...
    if (pFontDefC->Name == szFontName) {
      ...
      if (pFontDef == FontDefs.end() ||
          Abs(pFontDef->iSize - iSize) >= iSizeDiff)
        pFontDef = pFontDefC;
    }
    // check next one
    ++pFontDefC;
  ...
  if (pFontDef != FontDefs.end()) switch (eType) {
      case C4FT_Log:
        szFontString = pFontDef->LogFont.getData();
        break;
      case C4FT_MainSmall:
        szFontString = pFontDef->SmallFont.getData();
        break;
      case C4FT_Main:
        szFontString = pFontDef->Font.getData();
        break;
      case C4FT_Caption:
        szFontString = pFontDef->CaptionFont.getData();
        break;
      case C4FT_Title:
        szFontString = pFontDef->TitleFont.getData();
        break;
      default:
        LogFatal(LoadResStr("IDS_ERR_INITFONTS"));
  ...
  if (SEqualNoCase(szExt, "png") || SEqualNoCase(szExt, "bmp")) {
    ...
    int32_t iDefFontSize;
    DWORD dwDefWeight = FW_NORMAL;
#if defined(_WIN32) && !defined(HAVE_FREETYPE)
    switch (eType) {
      case C4FT_Log:
        iDefFontSize = 8;
        break;
      case C4FT_MainSmall:
        iDefFontSize = iSize + 1;
        break;
      case C4FT_Main:
        iDefFontSize = iSize + 4;
        break;
      case C4FT_Caption:
        iDefFontSize = iSize + 6;
        dwDefWeight = FW_BOLD;
        break;
      case C4FT_Title:
        iDefFontSize = iSize * 3;
        break;
      default:
        LogFatal(LoadResStr("IDS_ERR_INITFONTS"));
        return false;  // invalid call
    ...
    switch (eType) {
      case C4FT_Log:
        iDefFontSize = iSize * 12 / 14;
        break;
      case C4FT_MainSmall:
        iDefFontSize = iSize * 13 / 14;
        break;
      case C4FT_Main:
        iDefFontSize = iSize;
        break;
      case C4FT_Caption:
        iDefFontSize = iSize * 16 / 14; /*dwDefWeight = FW_MEDIUM;*/
        break;
      case C4FT_Title:
        iDefFontSize = iSize * 22 / 14; /*dwDefWeight = FW_MEDIUM;*/
        break;
      default:
        LogFatal(LoadResStr("IDS_ERR_INITFONTS"));
        return false;  // invalid call
...
#endif
    // regular font name: let WinGDI or Freetype draw a font with the given
    // parameters
    // font size given?
    if (SCopySegment(szFontString, 1, FontParam, ',', C4MaxName))
    ...
    if (!rFont.IsSameAs(FontFaceName, iDefFontSize, dwDefWeight)) {
      ...
      if (rFont.IsInitialized()) {
        // reloading
        rFont.Clear();
        LogF(LoadResStr("IDS_PRC_UPDATEFONT"), FontFaceName, iDefFontSize,
             dwDefWeight);
      }
      ...
      try {
        ...
        // no internal font matching? Then create one using the given
        // face/filename (using a system font)
        if (!pFont) {
          pFont = new C4VectorFont();
          if (pFont->Init(FontFaceName, iDefFontSize, dwDefWeight,
                          Config.General.LanguageCharset)) {
            AddVectorFont(pFont);
            if (!InitFont(rFont, pFont, iDefFontSize, dwDefWeight, fDoShadow))
              throw std::runtime_error(
                  FormatString("Error initializing font %s", FontFaceName)
                      .getData());
          } else {
            delete pFont;
            // no match for font face found
            throw std::runtime_error(
                FormatString("Font face %s undefined", FontFaceName).getData());
          }
        }
      } catch (std::runtime_error &e) {
        LogFatal(e.what());
