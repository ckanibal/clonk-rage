    ...
    case WM_COMMAND:
      ...
      switch (LOWORD(wParam)) {
        ...
        case IDOK:
          ...
          if (buffer[0]) Console.EditCursor.In(buffer);
          return TRUE;
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // - - - - - - - - - - - - - - - - -
...
BOOL C4PropertyDlg::Open() {
  ...
  if (vbox == NULL) {
    ...
    C4DevmodeDlg::AddPage(vbox, GTK_WINDOW(Console.window),
                          LoadResStrUtf8("IDS_DLG_PROPERTIES").getData());

    g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(OnScriptActivate),
                     this);
...
BOOL IsObjectPointer(int iValue) {
  for (C4ObjectLink *cLnk = Game.Objects.First; cLnk; cLnk = cLnk->Next)
    if (cLnk->Obj == (C4Object *)iValue) return TRUE;
  return FALSE;
}
...
void C4PropertyDlg::OnScriptActivate(GtkWidget *widget, gpointer data) {
  ...
  if (text && text[0]) Console.EditCursor.In(text);
}
