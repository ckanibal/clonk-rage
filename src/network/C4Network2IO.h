...
class C4Network2IO : protected C4InteractiveThread::Callback,
 ...
 protected:
  ...
  int iTCPIRate, iTCPORate, iTCPBCRate, iUDPIRate, iUDPORate, iUDPBCRate;
 ...
 public:
  ...
  // initialization
  bool Init(int16_t iPortTCP, int16_t iPortUDP, int16_t iPortDiscovery = -1,
            int16_t iPortRefServer = -1,
            bool fBroadcast = false);             // by main thread
  ...
  // connections
  bool Connect(const C4NetIO::addr_t &addr, C4Network2IOProtocol eProt,
               const C4ClientCore &nCCore,
               const char *szPassword = NULL);     // by main thread
  ...
  C4Network2IOConnection *GetMsgConnection(
      int iClientID);  // by both (returns referenced connection!)
  C4Network2IOConnection *GetDataConnection(
      int iClientID);  // by both (returns referenced connection!)
  ...
  int getProtIRate(C4Network2IOProtocol eProt) const {
    return eProt == P_TCP ? iTCPIRate : iUDPIRate;
  }
  int getProtORate(C4Network2IOProtocol eProt) const {
    return eProt == P_TCP ? iTCPORate : iUDPORate;
  }
  int getProtBCRate(C4Network2IOProtocol eProt) const {
    return eProt == P_TCP ? iTCPBCRate : iUDPBCRate;
  }
 ...
 protected:
  // *** callbacks
  // C4NetIO-Callbacks
  virtual bool OnConn(const C4NetIO::addr_t &addr,
                      const C4NetIO::addr_t &AddrConnect,
                      const C4NetIO::addr_t *pOwnAddr, C4NetIO *pNetIO);
  virtual void OnDisconn(const C4NetIO::addr_t &addr, C4NetIO *pNetIO,
                         const char *szReason);
  ...
  void OnThreadEvent(C4InteractiveEventType eEvent,
  ...
  C4Network2IOConnection *GetConnection(const C4NetIO::addr_t &addr,
                                        C4NetIO *pNetIO);  // by both
  C4Network2IOConnection *GetConnectionByConnAddr(const C4NetIO::addr_t &addr,
                                                  C4NetIO *pNetIO);  // by both
  ...
  // connection acceptance
  bool doAutoAccept(const C4ClientCore &CCore,
                    const C4Network2IOConnection &Conn);

  // general packet handling (= forward in most cases)
  bool HandlePacket(const C4NetIOPacket &rPacket, C4Network2IOConnection *pConn,
                    bool fThread);  // by both
  void CallHandlers(int iHandlers, const class C4IDPacket *pPacket,
                    C4Network2IOConnection *pConn, bool fThread);  // by both

  // packet handling (some are really handled here)
  void HandlePacket(char cStatus, const C4PacketBase *pPacket,
...
enum C4Network2IOConnStatus {
  ...
  CS_HalfAccepted,  // got Conn (peer identified, client class created if
                    // neccessary)
...
class C4Network2IOConnection  // shared
 ...
 protected:
  ...
  int iPingTime;            // ping
  unsigned long iLastPing;  // if > iLastPong, it's the first ping that hasn't
                            // been answered yet
 ...
 public:
  ...
  const C4NetIO::addr_t &getPeerAddr() const {
    return PeerAddr.sin_port ? PeerAddr : ConnectAddr;
  }
  ...
  bool isOpen() const {
    return Status != CS_Connect && Status != CS_Closed &&
           Status != CS_ConnectFail;
  }
  bool isHalfAccepted() const {
    return Status == CS_HalfAccepted || Status == CS_Accepted;
  }
  bool isAccepted() const { return Status == CS_Accepted; }
  bool isClosed() const {
    return Status == CS_Closed || Status == CS_ConnectFail;
  }
 ...
 protected:
  ...
  void Set(C4NetIO *pnNetClass, C4Network2IOProtocol eProt,
           const C4NetIO::addr_t &nPeerAddr,
           const C4NetIO::addr_t &nConnectAddr, C4Network2IOConnStatus nStatus,
           const char *szPassword, uint32_t iID);
 ...
 public:
  ...
  void SetBroadcastTarget(
      bool fSet);  // (only call after C4Network2IO::BeginBroadcast!)
...
class C4PacketConn : public C4PacketBase {
 public:
  C4PacketConn();
  C4PacketConn(const class C4ClientCore &nCCore, uint32_t iConnID,
