...
#include "gui/C4Gui.h"  // for clearly visible error message
...
StdStrBuf ircExtractPar(const char **ppPar) {
  // No parameter left?
  if (!ppPar || !*ppPar || !**ppPar) return StdStrBuf("");
  // Last parameter?
  StdStrBuf Result;
  if (**ppPar == ':') {
...
C4Network2IRCUser::C4Network2IRCUser(const char *szName) : Name(szName) {}
...
C4Network2IRCChannel::~C4Network2IRCChannel() { ClearUsers(); }
...
C4Network2IRCUser *C4Network2IRCChannel::getUser(const char *szName) const {
  for (C4Network2IRCUser *pUser = pUsers; pUser; pUser = pUser->Next)
    if (SEqual(pUser->getName(), szName)) return pUser;
  return NULL;
}
...
void C4Network2IRCChannel::OnUsers(const char *szUsers,
  ...
  while (szUsers && *szUsers) {
    ...
    if (szPrefixes)
      while (strchr(szPrefixes, *szName)) szName++;
    // Copy prefix
    StdStrBuf Prefix;
...
void C4Network2IRCChannel::OnTopic(const char *szTopic) { Topic = szTopic; }
...
void C4Network2IRCChannel::OnJoin(const char *szUser) {
  ...
  if (!getUser(szUser)) AddUser(szUser);
}
...
void C4Network2IRCChannel::DeleteUser(C4Network2IRCUser *pUser) {
  ...
  if (pUser == pUsers)
    ...
    while (pPrev && pPrev->Next != pUser) pPrev = pPrev->Next;
    if (pPrev) pPrev->Next = pUser->Next;
  }
  // Delete
  delete pUser;
}
...
void C4Network2IRCChannel::ClearUsers() {
  while (pUsers) DeleteUser(pUsers);
}
...
C4Network2IRCClient::~C4Network2IRCClient() { Close(); }

void C4Network2IRCClient::PackPacket(const C4NetIOPacket &rPacket,
                                     StdBuf &rOutBuf) {
  // Enlarge buffer
  int iSize = rPacket.getSize(), iPos = rOutBuf.getSize();
...
size_t C4Network2IRCClient::UnpackPacket(const StdBuf &rInBuf,
                                         const C4NetIO::addr_t &addr) {
  // Find line seperation
  const char *pSep = reinterpret_cast<const char *>(
      memchr(rInBuf.getData(), '\n', rInBuf.getSize()));
  if (!pSep) return 0;
  // Check if it's actually correct seperation (rarely the case)
  int iSize = pSep - getBufPtr<char>(rInBuf) + 1, iLength = iSize - 1;
  if (iLength && *(pSep - 1) == '\r') iLength--;
  // Copy the line
  StdStrBuf Buf;
  Buf.Copy(getBufPtr<char>(rInBuf), iLength);
  // Ignore prefix
...
bool C4Network2IRCClient::OnConn(const C4NetIO::addr_t &AddrPeer,
                                 const C4NetIO::addr_t &AddrConnect,
                                 const addr_t *pOwnAddr, C4NetIO *pNetIO) {
  // Security checks
  if (!fConnecting || fConnected || !AddrEqual(AddrConnect, ServerAddr))
    return false;
  CStdLock Lock(&CSec);
  ...
  if (!Password.isNull()) Send("PASS", Password.getData());
...
void C4Network2IRCClient::OnDisconn(const C4NetIO::addr_t &AddrPeer,
                                    C4NetIO *pNetIO, const char *szReason) {
  fConnected = false;
  // Show a message with the reason
  PushMessage(MSG_Status, "", Nick.getData(),
              FormatString(LoadResStr("IDS_MSG_DISCONNECTEDFROMSERVER"),
                           szReason).getData());
}

void C4Network2IRCClient::OnPacket(const class C4NetIOPacket &rPacket,
                                   C4NetIO *pNetIO) {
  // Won't get called
}
...
C4Network2IRCChannel *C4Network2IRCClient::getNextChannel(
    C4Network2IRCChannel *pPrevChan) const {
  return pPrevChan ? pPrevChan->Next : pChannels;
}

C4Network2IRCChannel *C4Network2IRCClient::getChannel(
    const char *szName) const {
  for (C4Network2IRCChannel *pChan = pChannels; pChan; pChan = pChan->Next)
    if (SEqualNoCase(pChan->getName(), szName)) return pChan;
  return NULL;
}

void C4Network2IRCClient::ClearMessageLog() {
  // Clear log
  while (iLogLength) PopMessage();
}
...
void C4Network2IRCClient::MarkMessageLogRead() {
  ...
  while (iLogLength > C4NetIRCMaxReadLogLength) PopMessage();
}

bool C4Network2IRCClient::Connect(const char *szServer, const char *szNick,
                                  const char *szRealName,
                                  const char *szPassword,
                                  const char *szAutoJoin) {
  // Already connected? Close connection
  if (fConnecting || fConnected) Close();
  // Initialize
  C4NetIOTCP::SetCallback(this);
  if (!Init()) return false;
  // Resolve address
  if (!ResolveAddress(szServer, &ServerAddr, 6666)) {
    SetError("Could no resolve server address!");
    return false;
  }
  // Set connection data
  Nick = szNick;
  ...
  AutoJoin = szAutoJoin;
  // Truncate password
  if (Password.getLength() > 31) Password.SetLength(31);
  // Start connecting
  if (!C4NetIOTCP::Connect(ServerAddr)) return false;
  // Reset status data
  Prefixes = "(ov)@+";
...
bool C4Network2IRCClient::Close() {
  ...
  C4NetIOTCP::Close();
  // Clear channels
  while (pChannels) DeleteChannel(pChannels);
  // Clear log
  ClearMessageLog();
  // Reset flags
...
bool C4Network2IRCClient::Send(const char *szCommand,
  ...
  if (szParameters)
    ...
    Msg.Ref(szCommand);
  // Send
  return C4NetIOTCP::Send(
      C4NetIOPacket(Msg.getData(), Msg.getLength(), false, PeerAddr));
}

bool C4Network2IRCClient::Quit(const char *szReason) {
  if (!Send("QUIT", FormatString(":%s", szReason).getData())) return false;
...
bool C4Network2IRCClient::Join(const char *szChannel) {
  ...
  if (!Config.IRC.AllowAllChannels)
    if (!SEqual2NoCase(szChannel, "#clonk")) {
      PushMessage(MSG_Status, "", "",
                  "Can only join channels beginning with #clonk.");
      SetError("Joining this channel not allowed");
      if (C4GUI::Screen::GetScreenS())
        C4GUI::Screen::GetScreenS()->ShowMessage(
            LoadResStr("IDS_CHAT_CHANNELNOTALLOWED"),
            LoadResStr("IDS_DLG_CHAT"), C4GUI::Ico_Error);
      return false;
    }
...
bool C4Network2IRCClient::Action(const char *szTarget, const char *szText) {
  if (!Send("PRIVMSG",
            FormatString("%s :\1ACTION %s\1", szTarget, szText).getData()))
    return false;
...
bool C4Network2IRCClient::RegisterNick(const char *szPassword,
                                       const char *szMail) {
  return Send("PRIVMSG", FormatString("NickServ :REGISTER %s %s", szPassword,
                                      szMail).getData());
}

void C4Network2IRCClient::OnCommand(const char *szSender, const char *szCommand,
                                    const char *szParameters) {
  CStdLock Lock(&CSec);
  // Numeric command?
  ...
  // Ping?
  if (SEqualNoCase(szCommand, "PING")) Send("PONG", szParameters);
  // Message?
  if (SEqualNoCase(szCommand, "NOTICE") || SEqualNoCase(szCommand, "PRIVMSG")) {
    ...
    // Process message
    OnMessage(SEqualNoCase(szCommand, "NOTICE"), szSender, Target.getData(),
              Text.getData());
  }
  // Channel join?
  if (SEqualNoCase(szCommand, "JOIN")) {
    ...
    // Myself?
    if (SenderNick == Nick)
      PushMessage(MSG_Status, szSender, Channel.getData(),
                  FormatString(LoadResStr("IDS_MSG_YOUHAVEJOINEDCHANNEL"),
                               Channel.getData()).getData());
    else
      PushMessage(MSG_Status, szSender, Channel.getData(),
                  FormatString(LoadResStr("IDS_MSG_HASJOINEDTHECHANNEL"),
                               SenderNick.getData()).getData());
  }
  // Channel part?
  if (SEqualNoCase(szCommand, "PART")) {
    ...
    if (SenderNick == Nick) {
      DeleteChannel(pChan);
      PushMessage(MSG_Status, szSender, Nick.getData(),
                  FormatString(LoadResStr("IDS_MSG_YOUHAVELEFTCHANNEL"),
                               Channel.getData(), Comment.getData()).getData());
    } else
      PushMessage(
          MSG_Status, szSender, Channel.getData(),
          FormatString(LoadResStr("IDS_MSG_HASLEFTTHECHANNEL"),
                       SenderNick.getData(), Comment.getData()).getData());
  }
  // Kick?
  if (SEqualNoCase(szCommand, "KICK")) {
    ...
    if (Kicked == Nick) {
      DeleteChannel(pChan);
      PushMessage(MSG_Status, szSender, Nick.getData(),
                  FormatString(LoadResStr("IDS_MSG_YOUWEREKICKEDFROMCHANNEL"),
                               Channel.getData(), Comment.getData()).getData());
    } else
      PushMessage(MSG_Status, szSender, Channel.getData(),
                  FormatString(LoadResStr("IDS_MSG_WASKICKEDFROMTHECHANNEL"),
                               Kicked.getData(), Comment.getData()).getData());
  }
  // Quit?
  if (SEqualNoCase(szCommand, "QUIT")) {
    ...
    StdStrBuf Message = FormatString(LoadResStr("IDS_MSG_HASDISCONNECTED"),
                                     SenderNick.getData(), Comment.getData());
  ...
  if (SEqualNoCase(szCommand, "TOPIC")) {
    ...
    // Message
    PushMessage(MSG_Status, szSender, Channel.getData(),
                FormatString(LoadResStr("IDS_MSG_CHANGESTHETOPICTO"),
                             SenderNick.getData(), Topic.getData()).getData());
  }
  // Mode?
  if (SEqualNoCase(szCommand, "MODE")) {
    ...
    PushMessage(
        MSG_Status, szSender, Channel.getData(),
        FormatString(LoadResStr("IDS_MSG_SETSMODE"), SenderNick.getData(),
                     Flags.getData(), What.getData()).getData());
  }
  // Error?
  ...
  if (SEqualNoCase(szCommand, "NICK")) {
    ...
    // Format status message
    StdStrBuf Message = FormatString(LoadResStr("IDS_MSG_ISNOWKNOWNAS"),
                                     SenderNick.getData(), NewNick.getData());
    ...
    // Self?
    if (SenderNick == Nick) Nick = NewNick;
  }
}

void C4Network2IRCClient::OnNumericCommand(const char *szSender, int iCommand,
                                           const char *szParameters) {
  ...
  switch (iCommand) {
    ...
    case 332:  // Topic notify / change
    {
      ...
      StdStrBuf Channel = ircExtractPar(&szParameters);
      StdStrBuf Topic =
          (iCommand == 332 ? ircExtractPar(&szParameters) : StdStrBuf(""));
      // Set it
      AddChannel(Channel.getData())->OnTopic(Topic.getData());
      // Log
      if (Topic.getLength())
        PushMessage(MSG_Status, szSender, Channel.getData(),
                    FormatString(LoadResStr("IDS_MSG_TOPICIN"),
                                 Channel.getData(), Topic.getData()).getData());
    } break;
    ...
    case 353:  // Names in channel
    {
      ...
      // Set it
      AddChannel(Channel.getData())
          ->OnUsers(Names.getData(), Prefixes.getData());
      fShowMessage = false;
    } break;
    ...
    case 5:  // Server support string
    {
      while (szParameters && *szParameters) {
        ...
        StdStrBuf Parameter;
        Parameter.CopyUntil(Token.getData(), '=');
  ...
  if (fShowMessage) {
    ...
    while (pMsg && *pMsg && *pMsg != ':') pMsg = SSearch(pMsg, " ");
    // Show it
    if (pMsg && *pMsg)
      if (!pChannel)
...
void C4Network2IRCClient::OnConnected() {
  ...
  if (AutoJoin.getLength()) Join(AutoJoin.getData());
}

void C4Network2IRCClient::OnMessage(bool fNotice, const char *szSender,
                                    const char *szTarget, const char *szText) {
  // Find channel, if not private.
  C4Network2IRCChannel *pChan = NULL;
  if (!SEqualNoCase(szTarget, Nick.getData())) pChan = getChannel(szTarget);
  ...
  if (szText[0] == X_DELIM) {
    ...
    while (*pMsg) {
      ...
      StdStrBuf CTCP;
      CTCP.Copy(pMsg, pEnd - pMsg);
      StdStrBuf Tag;
      Tag.CopyUntil(CTCP.getData(), ' ');
      const char *szData = SSearch(CTCP.getData(), " ");
      StdStrBuf Sender;
      Sender.CopyUntil(szSender, '!');
      // Process
      ...
      if (SEqualNoCase(Tag.getData(), "VERSION") && !fNotice)
        Send("NOTICE",
             FormatString("%s :%cVERSION " C4ENGINECAPTION ":" C4VERSION
                          ":" C4_OS "%c",
                          Sender.getData(), X_DELIM, X_DELIM).getData());
      if (SEqualNoCase(Tag.getData(), "PING") && !fNotice)
        Send("NOTICE", FormatString("%s :%cPING %s%c", Sender.getData(),
                                    X_DELIM, szData, X_DELIM).getData());
      // Get next message
      pMsg = pEnd;
...
void C4Network2IRCClient::PushMessage(C4Network2IRCMessageType eType,
                                      const char *szSource,
                                      const char *szTarget,
                                      const char *szText) {
  // Create message
  C4Network2IRCMessage *pMsg =
      new C4Network2IRCMessage(eType, szSource, szTarget, szText);
  // Add to list
  if (pLogEnd) {
    pLogEnd->Next = pMsg;
  } else {
  ...
  iLogLength++;
  iUnreadLogLength++;
  while (iLogLength > C4NetIRCMaxLogLength) PopMessage();
  // Notify
  if (pNotify) pNotify->PushEvent(Ev_IRC_Message, this);
}
...
void C4Network2IRCClient::DeleteChannel(C4Network2IRCChannel *pChannel) {
  ...
  if (pChannel == pChannels)
    ...
    while (pPrev && pPrev->Next != pChannel) pPrev = pPrev->Next;
    if (pPrev) pPrev->Next = pChannel->Next;
  }
  // Delete
  delete pChannel;
}
