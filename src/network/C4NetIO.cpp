...
const char *GetSocketErrorMsg() { return GetSocketErrorMsg(WSAGetLastError()); }
bool HaveSocketError() { return !!WSAGetLastError(); }
bool HaveWouldBlockError() { return WSAGetLastError() == WSAEWOULDBLOCK; }
bool HaveConnResetError() { return WSAGetLastError() == WSAECONNRESET; }
void ResetSocketError() { WSASetLastError(0); }
...
void ReleaseWinSock() {
  iWSockUseCounter--;
  // last use?
  if (!iWSockUseCounter) WSACleanup();
}

#else

const char *GetSocketErrorMsg(int iError) { return strerror(iError); }
const char *GetSocketErrorMsg() { return GetSocketErrorMsg(errno); }

bool HaveSocketError() { return !!errno; }
bool HaveWouldBlockError() {
  return errno == EINPROGRESS || errno == EWOULDBLOCK;
}
bool HaveConnResetError() { return errno == ECONNRESET; }
void ResetSocketError() { errno = 0; }
...
C4NetIOPacket::C4NetIOPacket(const void *pnData, size_t inSize, bool fCopy,
...
C4NetIOPacket::C4NetIOPacket(uint8_t cStatusByte, const char *pnData,
                             size_t inSize, const C4NetIO::addr_t &naddr) {
  // Create buffer
...
C4NetIOPacket::~C4NetIOPacket() { Clear(); }
...
bool C4NetIOTCP::Init(uint16_t iPort) {
  ...
  if ((Event = WSACreateEvent()) == WSA_INVALID_EVENT) {
    SetError("could not create socket event",
             true);  // to do: more error information
    return false;
  }
#else
  // create pipe
...
bool C4NetIOTCP::CloseBroadcast() { return true; }
...
bool C4NetIOTCP::Execute(int iMaxTime)  // (mt-safe)
{
  ...
  if (WaitForSingleObject(Event,
                          iMaxTime == C4NetIO::TO_INF ? INFINITE : iMaxTime) ==
      WAIT_TIMEOUT)
  ...
  // wait for something to happen
  int ret = select(iMax + 1, &fds[0], &fds[1], NULL,
                   (iMaxTime == C4NetIO::TO_INF ? NULL : &to));
...
#endif
      {
        ...
        if (wsaEvents.iErrorCode[FD_CONNECT_BIT]) {
          ...
          if (pCB)
            pCB->OnDisconn(
                pWait->addr, this,
                GetSocketErrorMsg(wsaEvents.iErrorCode[FD_CONNECT_BIT]));
        } else
#else
        // get error code
        int iErrCode;
        socklen_t iErrCodeLen = sizeof(iErrCode);
        if (getsockopt(sock, SOL_SOCKET, SO_ERROR,
                       reinterpret_cast<char *>(&iErrCode),
                       &iErrCodeLen) != 0) {
          close(sock);
        ...
        // error?
          ...
          close(sock);
          if (pCB)
            pCB->OnDisconn(pWait->addr, this, GetSocketErrorMsg(iErrCode));
        } else
#endif
...
// how much?
          ...
          int iBytesToRead;
#endif
          if (::ioctlsocket(pPeer->GetSocket(), FIONREAD, &iBytesToRead) ==
              SOCKET_ERROR) {
            pPeer->Close();
            if (pCB)
              pCB->OnDisconn(pPeer->GetAddr(), this, GetSocketErrorMsg());
            break;
          }
          ...
          int iBytesRead;
          if ((iBytesRead = ::recv(sock, reinterpret_cast<char *>(pBuf),
                                   iBytesToRead, 0)) == SOCKET_ERROR) {
          ...
          // nothing? this means the conection was closed, if you trust in linux
          // manpages.
          if (!iBytesRead) {
            pPeer->Close();
            if (pCB)
              pCB->OnDisconn(pPeer->GetAddr(), this, "connection closed");
            break;
          }
          // pass to Peer::OnRecv
...
bool C4NetIOTCP::Connect(const C4NetIO::addr_t &addr)  // (mt-safe)
{
...
#else
  // disable blocking
  if (::fcntl(nsock, F_SETFL, fcntl(nsock, F_GETFL) | O_NONBLOCK) ==
      SOCKET_ERROR) {
    // set error
    SetError("connect failed: could not disable blocking", true);
    close(nsock);
  ...
  // connect (async)
  if (::connect(nsock, reinterpret_cast<const sockaddr *>(&addr),
...
void C4NetIOTCP::GetFDs(fd_set *pFDs, int *pMaxFD) {
  ...
  FD_SET(Pipe[0], &pFDs[0]);
  if (pMaxFD) *pMaxFD = Max<SOCKET>(*pMaxFD, Pipe[0]);
  // add listener
  if (lsock != INVALID_SOCKET) {
    assert(!FD_ISSET(lsock, &pFDs[0]));
    FD_SET(lsock, &pFDs[0]);
    if (pMaxFD) *pMaxFD = Max<int>(*pMaxFD, lsock);
  }
  ...
  for (ConnectWait *pWait = pConnectWaits; pWait; pWait = pWait->Next) {
    ...
    FD_SET(pWait->sock, &pFDs[1]);
    if (pMaxFD) *pMaxFD = Max<int>(*pMaxFD, pWait->sock);
  }
  // add sockets
  for (Peer *pPeer = pPeerList; pPeer; pPeer = pPeer->Next)
    if (pPeer->GetSocket()) {
...
bool C4NetIOTCP::GetConnStatistic(const addr_t &addr, int *pIRate, int *pORate,
                                  int *pLoss)  // (mt-safe)
{
  CStdShareLock PeerListLock(&PeerListCSec);
  // find peer
...
C4NetIOTCP::Peer *C4NetIOTCP::Accept(SOCKET nsock,
                                     const addr_t &ConnectAddr)  // (mt-safe)
{
  ...
  if (nsock == INVALID_SOCKET) {
    ...
    if ((nsock = ::accept(lsock, reinterpret_cast<sockaddr *>(&addr),
                          &iAddrSize)) == INVALID_SOCKET) {
      // set error
    ...
    // get peer address
    if (::getpeername(nsock, reinterpret_cast<sockaddr *>(&addr), &iAddrSize) ==
        SOCKET_ERROR) {
#ifndef HAVE_WINSOCK
  ...
  int iNoDelay = 1;
  ::setsockopt(nsock, IPPROTO_TCP, TCP_NODELAY,
               reinterpret_cast<const char *>(&iNoDelay), sizeof(iNoDelay));

#ifdef STDSCHEDULER_USE_EVENTS
  // set event
  if (::WSAEventSelect(nsock, Event, FD_READ | FD_WRITE | FD_CLOSE) ==
      SOCKET_ERROR) {
    // set error
...
C4NetIOTCP::Peer *C4NetIOTCP::GetPeer(const addr_t &addr)  // (mt-safe)
{
  ...
  for (Peer *pPeer = pPeerList; pPeer; pPeer = pPeer->Next)
    if (pPeer->Open())
      if (AddrEqual(pPeer->GetAddr(), addr)) return pPeer;
  return NULL;
}
...
C4NetIOTCP::ConnectWait *C4NetIOTCP::GetConnectWait(
    const addr_t &addr)  // (mt-safe)
{
  CStdShareLock PeerListLock(&PeerListCSec);
  // search
  for (ConnectWait *pWait = pConnectWaits; pWait; pWait = pWait->Next)
    if (AddrEqual(pWait->addr, addr)) return pWait;
  return NULL;
}
...
void C4NetIOTCP::PackPacket(const C4NetIOPacket &rPacket, StdBuf &rOutBuf) {
  ...
  *getMBufPtr<uint8_t>(rOutBuf, iPos) = cFirstByte;
  iPos += sizeof(uint8_t);
  *getMBufPtr<uint32_t>(rOutBuf, iPos) = iSize;
  iPos += sizeof(uint32_t);
  rOutBuf.Write(rPacket, iPos);
}

size_t C4NetIOTCP::UnpackPacket(const StdBuf &IBuf,
  ...
  if (iPos + sizeof(uint32_t) > IBuf.getSize()) return 0;
  ...
  // ok, call back
  if (pCB)
    pCB->OnPacket(C4NetIOPacket(IBuf.getPart(iPos, iPacketSize), addr), this);
  // absorbed
...
C4NetIOTCP::Peer::Peer(const C4NetIO::addr_t &naddr, SOCKET nsock,
                       C4NetIOTCP *pnParent)
    : pParent(pnParent),
      addr(naddr),
      sock(nsock),
...
bool C4NetIOTCP::Peer::Send()  // (mt-safe)
{
  ...
  int iBytesSent;
  if ((iBytesSent = ::send(sock, getBufPtr<char>(OBuf), OBuf.getSize(), 0)) ==
      SOCKET_ERROR)
    if (!HaveWouldBlockError()) {
...
void *C4NetIOTCP::Peer::GetRecvBuf(int iSize)  // (mt-safe)
{
  ...
  while ((size_t)(iIBufUsage + iSize) > iIBufSize) iIBufSize *= 2;
  if (iIBufSize != IBuf.getSize()) IBuf.SetSize(iIBufSize);
...
void C4NetIOTCP::Peer::OnRecv(int iSize)  // (mt-safe)
{
  ...
  if (iPacketPos < (size_t)iIBufUsage) {
    ...
    while (iIBufUsage <= iIBufSize / 2) iIBufSize /= 2;
    if (iIBufSize != IBuf.getSize()) IBuf.Shrink(iPacketPos);
  } else {
    // the buffer is empty
    iIBufUsage = 0;
    // shrink buffer to minimum
    if (IBuf.getSize() > iMinIBufSize) IBuf.SetSize(iMinIBufSize);
  }
}
...
C4NetIOSimpleUDP::~C4NetIOSimpleUDP() { Close(); }
...
bool C4NetIOSimpleUDP::Init(uint16_t inPort) {
  ...
  if (::setsockopt(sock, SOL_SOCKET, SO_REUSEADDR,
                   reinterpret_cast<char *>(&fAllowReUse),
                   sizeof fAllowReUse) == SOCKET_ERROR) {
  ...
  if (::bind(sock, reinterpret_cast<sockaddr *>(&naddr), sizeof naddr) ==
      SOCKET_ERROR) {
...
bool C4NetIOSimpleUDP::InitBroadcast(addr_t *pBroadcastAddr) {
  ...
  if (setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                 reinterpret_cast<const char *>(&MCGrpInfo),
                 sizeof(MCGrpInfo)) == SOCKET_ERROR) {
    SetError(
        "could not join multicast group");  // to do: more error information
    return false;
  }
...
// how much can be read?
    ...
    // nothing?
    if (!iMaxMsgSize) break;
    // alloc buffer
    C4NetIOPacket Pkt;
    Pkt.New(iMaxMsgSize);
    // read data (note: it is _not_ garantueed that iMaxMsgSize bytes are
    // available)
    addr_t SrcAddr;
    socklen_t iSrcAddrLen = sizeof(SrcAddr);
    int iMsgSize =
        ::recvfrom(sock, getMBufPtr<char>(Pkt), iMaxMsgSize, 0,
                   reinterpret_cast<sockaddr *>(&SrcAddr), &iSrcAddrLen);
    // error?
    ...
    // again: nothing?
    if (!iMsgSize)
      // docs say that the connection has been closed (whatever that means for a
...
bool C4NetIOSimpleUDP::Send(const C4NetIOPacket &rPacket) {
  ...
  if (::sendto(sock, getBufPtr<char>(rPacket), rPacket.getSize(), 0,
               ...
               sizeof(addr)) != int(rPacket.getSize()) &&
      !HaveWouldBlockError()) {
...
enum C4NetIOSimpleUDP::WaitResult C4NetIOSimpleUDP::WaitForSocket(
    int iTimeout) {
  // wait for anything to happen
  DWORD ret =
      WaitForSingleObject(hEvent, iTimeout == TO_INF ? INFINITE : iTimeout);
  if (ret == WAIT_TIMEOUT) return WR_Timeout;
  if (ret == WAIT_FAILED) {
    SetError("Wait for Event failed");
    return WR_Error;
  }
  // get socket events (and reset the event)
  WSANETWORKEVENTS wsaEvents;
  if (WSAEnumNetworkEvents(sock, hEvent, &wsaEvents) == SOCKET_ERROR) {
    SetError("could not enumerate network events!");
    return WR_Error;
  }
  // socket readable?
  if (wsaEvents.lNetworkEvents | FD_READ) return WR_Readable;
...
void C4NetIOSimpleUDP::GetFDs(fd_set *pFDs, int *pMaxFD) {
  ...
  assert(!FD_ISSET(Pipe[0], &pFDs[0]));
  FD_SET(Pipe[0], &pFDs[0]);
  if (pMaxFD) *pMaxFD = Max(*pMaxFD, Pipe[0]);
  // add socket
  if (sock != INVALID_SOCKET) {
    assert(!FD_ISSET(sock, &pFDs[0]));
    FD_SET(sock, &pFDs[0]);
    if (pMaxFD) *pMaxFD = Max<int>(*pMaxFD, sock);
  }
}
...
enum C4NetIOSimpleUDP::WaitResult C4NetIOSimpleUDP::WaitForSocket(
  ...
  // wait for anything to happen
  int ret = ::select(iMaxFD + 1, &fds[0], &fds[1], NULL,
                     (iTimeout == C4NetIO::TO_INF ? NULL : &to));
  // catch simple cases
  if (ret < 0) {
    SetError("select failed", true);
    return WR_Error;
  }
  if (!ret) return WR_Timeout;
...
int C4NetIOSimpleUDP::GetTimeout() { return C4NetIO::TO_INF; }

bool C4NetIOSimpleUDP::SetMCLoopback(int fLoopback) {
  // enable/disable MC loopback
  setsockopt(sock, IPPROTO_IP, IP_MULTICAST_LOOP,
             reinterpret_cast<char *>(&fLoopback), sizeof fLoopback);
  // read result
  socklen_t iSize = sizeof(fLoopback);
  if (getsockopt(sock, IPPROTO_IP, IP_MULTICAST_LOOP,
                 reinterpret_cast<char *>(&fLoopback), &iSize) == SOCKET_ERROR)
    return false;
  fMCLoopback = !!fLoopback;
  return true;
}

void C4NetIOSimpleUDP::SetReUseAddress(bool fAllow) { fAllowReUse = fAllow; }
...
bool C4NetIOUDP::Init(uint16_t inPort) {
  ...
  // Initialize UDP
  if (!C4NetIOSimpleUDP::Init(inPort)) return false;
  iPort = inPort;
...
bool C4NetIOUDP::InitBroadcast(addr_t *pBroadcastAddr) {
  ...
  // broadcast addr valid?
  if (MCAddr.sin_family != AF_INET || in_addr_b(MCAddr.sin_addr, 0) != 239) {
    ...
    for (int iRetries = 1000; iRetries; iRetries--) {
      ...
      MCAddr.sin_addr.s_addr = MCAddr.sin_addr.s_addr =
          0x000000ef | ((rand() & 0xff) << 24) | ((rand() & 0xff) << 16) |
          ((rand() & 0xff) << 8);
      // init broadcast
      if (!C4NetIOSimpleUDP::InitBroadcast(&MCAddr)) return false;
      ...
      if (!C4NetIOSimpleUDP::Broadcast(
              C4NetIOPacket(&PingPacket, sizeof(PingPacket)))) {
        C4NetIOSimpleUDP::CloseBroadcast();
        return false;
      }
      bool fSuccess = false;
      for (;;) {
        ...
        // looped back?
        if (C4NetIOSimpleUDP::getMCLoopback() &&
            AddrEqual(LastPacket.getAddr(), MCLoopbackAddr))
          // ignore this one
          continue;
...
bool C4NetIOUDP::Execute(int iMaxTime)  // (mt-safe)
{
  ...
  // execute subclass
  if (!C4NetIOSimpleUDP::Execute(iMaxBlock)) return false;

  // connection check needed?
  if (iNextCheck <= timeGetTime()) DoCheck();
  // client timeout?
  for (Peer *pPeer = pPeerList; pPeer; pPeer = pPeer->Next)
    if (!pPeer->Closed()) pPeer->CheckTimeout();

  // do a delayed loopback test once the incoming buffer is empty
  if (fDelayedLoopbackTest) {
    if (fMultiCast) fMultiCast = DoLoopbackTest();
    fDelayedLoopbackTest = false;
  }
...
bool C4NetIOUDP::Broadcast(const C4NetIOPacket &rPacket)  // (mt-safe)
{
  ...
  for (pPeer = pPeerList; pPeer; pPeer = pPeer->Next)
    if (pPeer->Open() && pPeer->MultiCast() && pPeer->doBroadcast()) break;
  bool fSuccess = true;
  if (pPeer) {
...
int C4NetIOUDP::GetTimeout()  // (mt-safe)
{
  ...
  // check timeout
  if (iNextCheck) iTiming = Max<int>(int(iNextCheck) - timeGetTime(), 0);
  ...
  for (Peer *pPeer = pPeerList; pPeer; pPeer = pPeer->Next)
    if (!pPeer->Closed())
      if (pPeer->GetTimeout())
        iTiming =
            Max(Min<int>(iTiming, int(pPeer->GetTimeout() - timeGetTime())), 0);
  // return timing value
  return iTiming;
}
...
bool C4NetIOUDP::GetConnStatistic(const addr_t &addr, int *pIRate, int *pORate,
                                  int *pLoss)  // (mt-safe)
{
  CStdShareLock PeerListLock(&PeerListCSec);
  // find peer
...
void C4NetIOUDP::OnPacket(const C4NetIOPacket &Packet, C4NetIO *pNetIO) {
  ...
  if (fMultiCast && !fDelayedLoopbackTest)
    if (AddrEqual(Packet.getAddr(), MCLoopbackAddr)) return;
  ...
  if (!pPeer) {
    ...
    if ((Packet.getStatus() & 0x7F) == IPID_Ping) {
      PacketHdr PingPacket = {
          static_cast<int8_t>(IPID_Ping | (Packet.getStatus() & 0x80)), 0};
      SendDirect(C4NetIOPacket(static_cast<void *>(&PingPacket),
                               sizeof(PingPacket), false, Packet.getAddr()));
      return;
    }
  ...
  } else /*if(pPeer)*/
  {
    ...
    if (Packet.getStatus() == IPID_AddAddr) {
      OnAddAddress(Packet.getAddr(), *getBufPtr<AddAddrPacket>(Packet));
      return;
    }
...
bool C4NetIOUDP::OnConn(const addr_t &AddrPeer, const addr_t &AddrConnect,
                        const addr_t *pOwnAddr, C4NetIO *pNetIO) {
  // ignore
  return true;
}

void C4NetIOUDP::OnDisconn(const addr_t &AddrPeer, C4NetIO *pNetIO,
                           const char *szReason) {
  assert(pNetIO == this);
...
void C4NetIOUDP::OnAddAddress(const addr_t &FromAddr,
                              const AddAddrPacket &Packet) {
  // Security (this would be strange behavior indeed...)
  if (!AddrEqual(FromAddr, Packet.Addr) && !AddrEqual(FromAddr, Packet.NewAddr))
    return;
  // Search peer(s)
...
C4NetIOUDP::Packet::Packet() : iNr(~0), Data(), pFragmentGot(NULL) {}

C4NetIOUDP::Packet::Packet(C4NetIOPacket rnData, nr_t inNr)
    : iNr(inNr), Data(rnData), pFragmentGot(NULL) {}
...
C4NetIOPacket C4NetIOUDP::Packet::GetFragment(nr_t iFNr,
                                              bool fBroadcastFlag) const {
  assert(iFNr < FragmentCnt());
  // create buffer
  uint16_t iFragmentSize = FragmentSize(iFNr);
  StdBuf Packet;
  Packet.New(sizeof(DataPacketHdr) + iFragmentSize);
  // set up header
...
bool C4NetIOUDP::Packet::Complete() const {
  ...
  for (unsigned int i = 0; i < FragmentCnt(); i++)
    if (!FragmentPresent(i)) return false;
  return true;
}

bool C4NetIOUDP::Packet::FragmentPresent(uint32_t iFNr) const {
  return !Empty() && iFNr < FragmentCnt() &&
         (!pFragmentGot || pFragmentGot[iFNr]);
}
...
bool C4NetIOUDP::Packet::AddFragment(const C4NetIOPacket &Packet,
  ...
  if (fFirstFragment) {
    ...
    // check header
    if (pHdr->Nr < iNr || pHdr->Nr >= iNr + FragmentCnt()) {
      Data.Clear();
      return false;
    }
  } else {
  ...
  // already got this fragment? (needs check for first packet as FragmentPresent
  // always assumes true if pFragmentGot is NULL)
  StdBuf PacketData = Packet.getPart(sizeof(DataPacketHdr), iPacketDataSize);
  if (!fFirstFragment && FragmentPresent(iFNr)) {
    // compare
    if (Data.Compare(PacketData, iFNr * MaxDataSize)) return false;
  } else {
    // otherwise: copy data
    Data.Write(PacketData, iFNr * MaxDataSize);
    // set flag (if fragmented)
    if (pFragmentGot) pFragmentGot[iFNr] = true;
    // shouldn't happen
    else
      assert(Complete());
  }
...
C4NetIOUDP::PacketList::PacketList(unsigned int inMaxPacketCnt)
    : pFront(NULL), iMaxPacketCnt(inMaxPacketCnt), iPacketCnt(0), pBack(NULL) {}

C4NetIOUDP::PacketList::~PacketList() { Clear(); }
...
bool C4NetIOUDP::PacketList::AddPacket(Packet *pPacket) {
  ...
  for (; pInsertAfter;
       pInsertBefore = pInsertAfter, pInsertAfter = pInsertAfter->Prev)
    if (pInsertAfter->GetNr() + pInsertAfter->FragmentCnt() <= pPacket->GetNr())
      break;
  // check: enough space?
  if (pInsertBefore &&
      pInsertBefore->GetNr() < pPacket->GetNr() + pPacket->FragmentCnt())
    return false;
  // insert
  ...
  ++iPacketCnt;
  while (iPacketCnt > iMaxPacketCnt) DeletePacket(pFront);
  // ok
  return true;
}
...
void C4NetIOUDP::PacketList::ClearPackets(unsigned int iUntil) {
  CStdLock ListLock(&ListCSec);
  while (pFront && pFront->GetNr() < iUntil) DeletePacket(pFront);
}

void C4NetIOUDP::PacketList::Clear() {
  CStdLock ListLock(&ListCSec);
  while (iPacketCnt) DeletePacket(pFront);
}
...
bool C4NetIOUDP::Peer::Connect(bool fFailCallback)  // (mt-safe)
{
  ...
  fMultiCast = false;
  fConnFailCallback = fFailCallback;
  return DoConn(false);
}
...
bool C4NetIOUDP::Peer::Send(const C4NetIOPacket &rPacket)  // (mt-safe)
{
  ...
  if (!OPackets.AddPacket(pnPacket)) return false;
...
bool C4NetIOUDP::Peer::Check(bool fForceCheck) {
  ...
  unsigned int iStartAt =
      fNoReCheck ? Max(iLastPacketAsked + 1, iIPacketCounter) : iIPacketCounter;
  unsigned int iStartAtMC = fNoReCheck
                                ? Max(iLastMCPacketAsked + 1, iIMCPacketCounter)
                                : iIMCPacketCounter;
  ...
  for (i = iStartAt; i < iRIPacketCounter; i++)
    if (!IPackets.FragmentPresent(i))
      if (iAskCnt < iMaxAskCnt) iLastPacketAsked = iAskList[iAskCnt++] = i;
  ...
  if (iEAskCnt || fForceCheck) return DoCheck(iAskCnt, iMCAskCnt, iAskList);
  return true;
}
...
void C4NetIOUDP::Peer::OnRecv(const C4NetIOPacket &rPacket)  // (mt-safe)
{
  ...
  {
    CStdLock StatLock(&StatCSec);
    iIRate += rPacket.getSize() + iUDPHeaderSize;
  }
  // get packet header
  ...
  // save packet nr
  (fBroadcasted ? iRIMCPacketCounter : iRIPacketCounter) = Max<unsigned int>(
      (fBroadcasted ? iRIMCPacketCounter : iRIPacketCounter), pHdr->Nr);
#ifdef C4NETIOUDP_OPT_RECV_CHECK_IMMEDIATE
  // do check
  if (eStatus == CS_Works) Check(false);
#endif
  ...
  switch (pHdr->StatusByte & 0x7f) {
    case IPID_Conn: {
      ...
      if (!fBroadcasted) {
        ...
        if (PeerAddr.sin_addr.s_addr && !AddrEqual(PeerAddr, pPkt->Addr)) {
          ...
          SendDirect(C4NetIOPacket(static_cast<void *>(&Pkt), sizeof(Pkt),
                                   false, addr));
      ...
      // send it
      SendDirect(C4NetIOPacket(static_cast<void *>(&nPack), sizeof(nPack),
                               false, addr));
    } break;
      ...
      switch (pPkt->MCMode) {
        case ConnOKPacket::MCM_MC:
          ...
          if (pParent->fMultiCast) {
            ...
            if (fMultiCast) break;
            // Send another Conn packet back (this time broadcasted to check if
            // multicast works)
            fMultiCast = true;
            DoConn(true);
            break;
    ...
    case IPID_Data: {
      ...
      // already complet?
      if (pHdr->Nr < (fBroadcasted ? iIMCPacketCounter : iIPacketCounter))
        break;
    ...
    case IPID_Check: {
      ...
      if (rPacket.getSize() <
          sizeof(CheckPacketHdr) +
              (pPkt->AskCount + pPkt->MCAskCount) * sizeof(int))
        break;
      ...
      for (i = 0; i < pPkt->AskCount + pPkt->MCAskCount; i++) {
        ...
        Packet *pPkt2Send = (fMCPacket ? pParent->OPackets : OPackets)
                                .GetPacketFrgm(pAskList[i]);
        if (!pPkt2Send) {
          Close("starvation");
          break;
        }
    ...
    case IPID_Close: {
      ...
      if (PeerAddr.sin_addr.s_addr && !AddrEqual(PeerAddr, pPkt->Addr)) break;
      // close
...
void C4NetIOUDP::Peer::Close(const char *szReason)  // (mt-safe)
{
  ...
  Pkt.Addr = addr;
  SendDirect(
      C4NetIOPacket(static_cast<void *>(&Pkt), sizeof(Pkt), false, addr));
  // callback
  OnClose(szReason);
}
...
void C4NetIOUDP::Peer::CheckTimeout() {
  ...
  if (!iTimeout) return;
  // check
  if (timeGetTime() > iTimeout) OnTimeout();
}
...
bool C4NetIOUDP::Peer::DoCheck(int iAskCnt, int iMCAskCnt,
  ...
  {
    CStdLock StatLock(&StatCSec);
    iLoss += iAskCnt + iMCAskCnt;
  }
  // alloc data
  int iAskListSize = (iAskCnt + iMCAskCnt) * sizeof(*pAskList);
  StdBuf Packet;
  Packet.New(sizeof(CheckPacketHdr) + iAskListSize);
  CheckPacketHdr *pChkPkt = getMBufPtr<CheckPacketHdr>(Packet);
  // set up header
  pChkPkt->StatusByte =
      IPID_Check;  // (note: always du here, see C4NetIOUDP::DoCheck)
  ...
  if (pAskList) Packet.Write(pAskList, iAskListSize, sizeof(CheckPacketHdr));
  // send packet
...
bool C4NetIOUDP::Peer::SendDirect(const Packet rPacket, unsigned int iNr) {
  ...
  if (iNr + 1) return SendDirect(rPacket.GetFragment(iNr - rPacket.GetNr()));
...
bool C4NetIOUDP::Peer::SendDirect(C4NetIOPacket rPacket)  // (mt-safe)
{
  ...
  {
    CStdLock StatLock(&StatCSec);
    iORate += rPacket.getSize() + iUDPHeaderSize;
  }
  // forward call
...
void C4NetIOUDP::Peer::OnConn() {
  ...
  // do packet callback (in case the peer sent data while the connection was in
  // progress)
  CheckCompleteIPackets();
}
...
void C4NetIOUDP::Peer::OnClose(const char *szReason)  // (mt-safe)
{
  ...
  if (eStatus == CS_Works || (eStatus == CS_Conn && fConnFailCallback))
    if (pCB) pCB->OnDisconn(addr, pParent, szReason);
...
void C4NetIOUDP::Peer::CheckCompleteIPackets() {
  ...
  //  complete packets here, but the one using the interface may get very
  //  confused
  ...
  while (pPkt = IPackets.GetFirstPacketComplete()) {
    ...
    // do callback
    if (pParent->pCB) pParent->pCB->OnPacket(pPkt->GetData(), pParent);
...
#ifdef C4NETIO_DEBUG
  {
    C4NetIOPacket Pkt2 = rPacket;
    Pkt2.SetAddr(toaddr);
    DebugLogPkt(true, Pkt2);
  }
#endif
...
bool C4NetIOUDP::DoLoopbackTest() {
  ...
  // send test packet
  const PacketHdr TestPacket = {IPID_Test | char(0x80),
                                static_cast<uint32_t>(rand())};
  if (!C4NetIOSimpleUDP::Broadcast(
          C4NetIOPacket(&TestPacket, sizeof(TestPacket))))
    return false;

  // wait for socket to become readable (should happen immediatly, do not expect
  // packet loss)
  fSavePacket = true;
  if (!C4NetIOSimpleUDP::Execute(iStdTimeout)) {
    fSavePacket = false;
                                          ...
                                          bool fFailCallback)  // (mt-safe)
{
  ...
  if (!pnPeer->Connect(fFailCallback)) {
    pnPeer->Close("connect failed");
    return NULL;
  }
...
void C4NetIOUDP::DoCheck()  // (mt-safe)
{
  ...
  if (fMultiCast) {
    ...
    Peer *pPeer;
    for (pPeer = pPeerList; pPeer; pPeer = pPeer->Next)
      if (pPeer->Open() && pPeer->MultiCast()) break;
    if (pPeer) {
      // set up packet
      CheckPacketHdr Pkt;
...
void C4NetIOUDP::OpenDebugLog() {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    hDebugLog = open(szFilePath, O_CREAT | O_EXCL | O_TRUNC | _O_SEQUENTIAL |
                                     _O_TEXT | O_WRONLY,
                     S_IREAD | S_IWRITE);
  ...
  if (hDebugLog != -1) {
    ...
    if (pLocalTime)
      sprintf(O,
              "C4NetIOUDP debuglog starting at %d/%d/%d  %d:%2d:%2d - "
              "(Daylight %d)\n",
              pLocalTime->tm_mon + 1, pLocalTime->tm_mday,
              pLocalTime->tm_year + 1900, pLocalTime->tm_hour,
              pLocalTime->tm_min, pLocalTime->tm_sec, pLocalTime->tm_isdst);
    else
      sprintf(O, "C4NetIOUDP debuglog; time not available\n");
    write(hDebugLog, O, strlen(O));
  }
}

void C4NetIOUDP::CloseDebugLog() { close(hDebugLog); }
...
void C4NetIOUDP::DebugLogPkt(bool fOut, const C4NetIOPacket &Pkt) {
  ...
  O.Format("%s %d:%02d:%02d:%03d %s:%d:", fOut ? "out" : "in ",
           ...
           (iTime / 1000) % 60, iTime % 1000, inet_ntoa(Pkt.getAddr().sin_addr),
           htons(Pkt.getAddr().sin_port));
...
#define UPACK(type) const type &P = *getBufPtr<type>(Pkt);

    switch (Hdr.StatusByte) {
      case IPID_Test: {
        UPACK(TestPacket);
        O.AppendFormat(" (%d)", P.TestNr);
        break;
      }
      case IPID_Conn: {
        UPACK(ConnPacket);
        O.AppendFormat(" (Ver %d, MC: %s:%d)", P.ProtocolVer,
                       inet_ntoa(P.MCAddr.sin_addr), htons(P.MCAddr.sin_port));
        break;
      }
      case IPID_ConnOK: {
        UPACK(ConnOKPacket);
      ...
      case IPID_Data: {
        UPACK(DataPacketHdr);
        O.AppendFormat(" (f: %d s: %d)", P.FNr, P.Size);
        for (int iPos = sizeof(DataPacketHdr);
             iPos < Min<int>(Pkt.getSize(), sizeof(DataPacketHdr) + 16); iPos++)
          O.AppendFormat(" %02x", *getBufPtr<unsigned char>(Pkt, iPos));
        break;
      }
      case IPID_Check: {
        UPACK(CheckPacketHdr);
        O.AppendFormat(" (ack: %d, mcack: %d, ask: %d mcask: %d, ", P.AckNr,
                       P.MCAckNr, P.AskCount, P.MCAskCount);
        if (Pkt.getSize() <
            sizeof(CheckPacketHdr) +
                sizeof(unsigned int) * (P.AskCount + P.MCAskCount))
          ...
          for (unsigned int i = 0; i < P.AskCount + P.MCAskCount; i++)
            O.AppendFormat(
                "%s%d", i ? ", " : "",
                *getBufPtr<unsigned int>(
                    Pkt, sizeof(CheckPacketHdr) + i * sizeof(unsigned int)));
          O.Append("])");
        }
        break;
      }
...
void C4NetIOMan::AddIO(C4NetIO *pNetIO, bool fSetCallback) {
  // Set callback
  if (fSetCallback) pNetIO->SetCallback(this);
  // Add to i/o list
  if (iNetIOCnt + 1 > iNetIOCapacity) EnlargeIO(1);
...
void C4NetIOMan::RemoveIO(C4NetIO *pNetIO) {
  ...
  for (i = 0; i < iNetIOCnt; i++)
    if (ppNetIO[i] == pNetIO) break;
  // Not found?
  if (i >= iNetIOCnt) return;
  // Remove
  for (i++; i < iNetIOCnt; i++) ppNetIO[i - 1] = ppNetIO[i];
  iNetIOCnt--;
}

void C4NetIOMan::OnError(StdSchedulerProc *pProc) {
  for (int i = 0; i < iNetIOCnt; i++)
    if (pProc == ppNetIO[i]) OnError(ppNetIO[i]->GetError(), ppNetIO[i]);
}
...
void C4NetIOMan::EnlargeIO(int iBy) {
  ...
  // Set data
  for (int i = 0; i < iNetIOCnt; i++) ppnNetIO[i] = ppNetIO[i];
  delete[] ppNetIO;
  ppNetIO = ppnNetIO;
}
...
bool ResolveAddress(const char *szAddress, C4NetIO::addr_t *paddr,
                    uint16_t iPort) {
