...
C4Network2Status::C4Network2Status() : eState(GS_None), iTargetCtrlTick(-1) {}
...
const char *C4Network2Status::getDescription() const {
  switch (eState) {
    case GS_None:
      return LoadResStr("IDS_DESC_NOTINITED");
    case GS_Init:
      return LoadResStr("IDS_DESC_WAITFORHOST");
    case GS_Lobby:
      return LoadResStr("IDS_DESC_EXPECTING");
    case GS_Pause:
      return LoadResStr("IDS_DESC_GAMEPAUSED");
    case GS_Go:
      return LoadResStr("IDS_DESC_GAMERUNNING");
  }
...
void C4Network2Status::CompileFunc(StdCompiler *pComp, bool fReference) {
  ...
  pComp->Value(mkNamingAdapt(mkEnumAdaptT<uint8_t>(eState, GameStates), "State",
                             GS_None));
  pComp->Value(mkNamingAdapt(mkIntPackAdapt(iCtrlMode), "CtrlMode", -1));

  if (!fReference)
    pComp->Value(
        mkNamingAdapt(mkIntPackAdapt(iTargetCtrlTick), "TargetTick", -1));
}
...
bool C4Network2::InitHost(bool fLobby) {
  ...
  if (!ResList.Init(Game.Clients.getLocalID(), &NetIO)) {
    LogFatal("Network: failed to initialize resource list!");
    Clear();
    return false;
  }
  if (!Game.Parameters.InitNetwork(&ResList)) return false;
  // create initial dynamic
  if (!CreateDynamic(true)) return false;
  // initialize net i/o
  ...
  pControl->Init(C4ClientIDHost, true, Game.Control.getNextControlTick(), true,
                 this);
  // init league
  bool fCancel = true;
...
C4Network2::InitResult C4Network2::InitClient(const C4Network2Reference &Ref,
                                              bool fObserver) {
  if (isEnabled()) Clear();
  // Get host core
  ...
  for (;;) {
    ...
    if (fWrongPassword) {
      ...
      if (!Password.getLength()) return IR_Error;
      fWrongPassword = false;
    }
    // copy addresses
    C4Network2Address Addrs[C4ClientMaxAddr];
    for (int i = 0; i < Ref.getAddrCnt(); i++) Addrs[i] = Ref.getAddr(i);
    // Try to connect to host
    if (InitClient(Addrs, Ref.getAddrCnt(), HostCore, Password.getData()) ==
        IR_Fatal)
      return IR_Fatal;
    // success?
    if (isEnabled()) break;
  ...
  if (!Game.Parameters.InitNetwork(&ResList)) return IR_Fatal;
  // init league
  if (!InitLeague(NULL)) {
    // deinit league
    DeinitLeague();
...
C4Network2::InitResult C4Network2::InitClient(
    const class C4Network2Address *pAddrs, int iAddrCount,
    const C4ClientCore &HostCore, const char *szPassword) {
  // initialization
  ...
  // initialize resource list
  if (!ResList.Init(Game.Clients.getLocalID(), &NetIO)) {
    LogFatal(LoadResStr("IDS_NET_ERR_INITRESLIST"));
    Clear();
    return IR_Fatal;
  }
  // initialize net i/o
  ...
  // try to connect host
  StdStrBuf strAddresses;
  int iSuccesses = 0;
  for (int i = 0; i < iAddrCount; i++)
    if (!pAddrs[i].isIPNull()) {
      // connection
      if (!NetIO.Connect(pAddrs[i].getAddr(), pAddrs[i].getProtocol(), HostCore,
                         szPassword))
        continue;
      // format for message
      if (strAddresses.getLength()) strAddresses.Append(", ");
  ...
  StdStrBuf strMessage =
      FormatString(LoadResStr("IDS_NET_CONNECTHOST"), strAddresses.getData());
  ...
  if (Game.pGUI && !Console.Active) {
    ...
    if (!pDlg->Show(Game.pGUI, true)) {
      Clear();
      return IR_Fatal;
    }
  }
  // wait for connect / timeout / abort by user (host will change status on
  // succesful connect)
...
bool C4Network2::DoLobby() {
  // shouldn't do lobby?
  if (!isEnabled() || (!isHost() && !isLobbyActive())) return true;
  ...
  if (!isHost()) CheckStatusReached();
  // host: set lobby mode
  else
    ChangeGameStatus(GS_Lobby, 0);

  // determine lobby type
  bool fFullscreenLobby =
...
bool C4Network2::FinalInit() {
  ...
  if (fStatusReached && !fStatusAck) {
    ...
    // any pending keyboard commands should not be routed to cancel the wait
    // dialog - flish the message queue!
    ...
    if (Game.pGUI && !Console.Active) {
      ...
      if (isHost())
        ...
        pDlg = new C4GUI::MessageDialog(
            LoadResStr("IDS_NET_WAITFORSTART"), LoadResStr("IDS_NET_CAPTION"),
            C4GUI::MessageDialog::btnAbort, C4GUI::Ico_NetWait,
            C4GUI::MessageDialog::dsSmall);
      // show it
...
bool C4Network2::RetrieveScenario(char *szScenario) {
  ...
  C4Network2Res::Ref pScenario =
      RetrieveRes(*Game.Parameters.Scenario.getResCore(),
                  C4NetResRetrieveTimeout, LoadResStr("IDS_NET_RES_SCENARIO"));
  if (!pScenario) return false;

  // wait for dynamic data
  C4Network2Res::Ref pDynamic = RetrieveRes(ResDynamic, C4NetResRetrieveTimeout,
                                            LoadResStr("IDS_NET_RES_DYNAMIC"));
  if (!pDynamic) return false;

  // create unpacked copy of scenario
  if (!ResList.FindTempResFileName(
          FormatString("Combined%d.c4s", Game.Clients.getLocalID()).getData(),
          szScenario) ||
  ...
  if (FileExists(MaterialScenario.getData()) &&
      FileExists(MaterialDynamic.getData()))
  ...
  C4Group ScenGrp;
  if (!ScenGrp.Open(szScenario) || !ScenGrp.Merge(szTempDynamic)) return false;
  ScenGrp.Close();
...
void C4Network2::OnSec1Timer() { Execute(); }

void C4Network2::Execute() {
  if (pLeagueClient) pLeagueClient->Execute(0);
  if (pStreamer) pStreamer->Execute(0);
  ...
  if (isHost()) {
    ...
    // reference
    if (!iLastReferenceUpdate ||
        time(NULL) >
            (time_t)(iLastReferenceUpdate + C4NetReferenceUpdateInterval))
    ...
    if (!iLastLeagueUpdate ||
        time(NULL) > (time_t)(iLastLeagueUpdate + iLeagueUpdateDelay)) {
      LeagueUpdate();
    }
    // league update reply receive
    if (pLeagueClient && fHost && !pLeagueClient->isBusy() &&
        pLeagueClient->getCurrentAction() == C4LA_Update) {
      LeagueUpdateProcessReply();
    }
    // voting timeout
    if (Votes.firstPkt() &&
        time(NULL) > (time_t)(iVoteStartTime + C4NetVotingTimeout)) {
      C4ControlVote *pVote =
          static_cast<C4ControlVote *>(Votes.firstPkt()->getPkt());
...
void C4Network2::Clear() {
  ...
  if (Game.Control.isNetwork()) Game.Control.ChangeToLocal();
  ...
  iDynamicTick = -1;
  fDynamicNeeded = false;
  iLastActivateRequest = iLastChaseTargetUpdate = iLastReferenceUpdate =
      iLastLeagueUpdate = 0;
  fDelayedActivateReq = false;
...
StdStrBuf C4Network2::QueryClientPassword() {
  ...
  StdStrBuf sCaption;
  sCaption.Copy(LoadResStr("IDS_MSG_ENTERPASSWORD"));
  C4GUI::InputDialog *pInputDlg = new C4GUI::InputDialog(
      LoadResStr("IDS_MSG_ENTERPASSWORD"), sCaption.getData(),
      C4GUI::Ico_Ex_Locked, NULL, false);
  ...
  // copy to buffer
  StdStrBuf Buf;
  Buf.Copy(pInputDlg->GetInputText());
  delete pInputDlg;
  return Buf;
}
...
void C4Network2::AllowJoin(bool fAllow) {
  ...
  fAllowJoin = fAllow;
  if (Game.IsRunning) {
    Game.GraphicsSystem.FlashMessage(LoadResStr(
        fAllowJoin ? "IDS_NET_RUNTIMEJOINFREE" : "IDS_NET_RUNTIMEJOINBARRED"));
...
void C4Network2::OnConn(C4Network2IOConnection *pConn) {
  // Nothing to do atm... New pending connections are managed mainly by
  // C4Network2IO
  // until they are accepted, see PID_Conn/PID_ConnRe handlers in HandlePacket.

  // Note this won't get called anymore because of this (see
  // C4Network2IO::OnConn)
}
...
void C4Network2::OnDisconn(C4Network2IOConnection *pConn) {
  ...
  if (Status.getState() == GS_Init && !isHost()) {
    if (!NetIO.getConnectionCount()) Clear();
    return;
  }
  ...
  if (pConn->CreatePostMortem(&PostMortem)) {
    LogSilentF("Network: Sending %d packets for recovery (%d-%d)",
               PostMortem.getPacketCount(),
               pConn->getOutPacketCounter() - PostMortem.getPacketCount(),
               pConn->getOutPacketCounter() - 1);
    // This might fail because of this disconnect
    // (If it's the only host connection. We're toast then anyway.)
    if (!Clients.SendMsgToClient(pConn->getClientID(),
                                 MkC4NetIOPacket(PID_PostMortem, PostMortem)))
...
void C4Network2::HandlePacket(char cStatus, const C4PacketBase *pPacket,
...
void C4Network2::HandleLobbyPacket(char cStatus, const C4PacketBase *pBasePkt,
                                   C4Network2IOConnection *pConn) {
  // find associated client
  C4Network2Client *pClient = Clients.GetClient(pConn);
  if (!pClient) pClient = Clients.GetClientByID(pConn->getClientID());
      ...
      if (!pClient->hasJoinData())
        if (fSuccess)
          ...
          Game.Clients.CtrlRemove(
              pClient->getClient(),
              LoadResStr("IDS_ERR_ERRORWHILECREATINGJOINDAT"));
  }
}
...
void C4Network2::DrawStatus(C4FacetEx &cgo) {
  ...
  Stat.AppendFormat(
      "Local: %s %s %s (ID %d)",
      pLocal->isObserver() ? "Observing" : pLocal->isActivated() ? "Active"
                                                                 : "Inactive",
      pLocal->isHost() ? "host" : "client", pLocal->getName(), pLocal->getID());

  // game status
  Stat.AppendFormat("|Game Status: %s (tick %d)%s%s", Status.getStateName(),
                    Status.getTargetCtrlTick(),
  ...
  if (pMsgIO && pDataIO) {
    ...
    switch (eMsgProt) {
      case P_TCP:
        iMsgPort = Config.Network.PortTCP;
        break;
      case P_UDP:
        iMsgPort = Config.Network.PortUDP;
        break;
    }
    switch (eDataProt) {
      case P_TCP:
        iDataPort = Config.Network.PortTCP;
        break;
      case P_UDP:
        iDataPort = Config.Network.PortUDP;
        break;
    }
    Stat.AppendFormat(
        "|Protocols: %s: %s (%d i%d o%d bc%d)",
        pMsgIO != pDataIO ? "Msg" : "Msg/Data", NetIO.getNetIOName(pMsgIO),
        iMsgPort, NetIO.getProtIRate(eMsgProt), NetIO.getProtORate(eMsgProt),
        NetIO.getProtBCRate(eMsgProt));
    if (pMsgIO != pDataIO)
      Stat.AppendFormat(
          ", Data: %s (%d i%d o%d bc%d)", NetIO.getNetIOName(pDataIO),
          iDataPort, NetIO.getProtIRate(eDataProt),
          NetIO.getProtORate(eDataProt), NetIO.getProtBCRate(eDataProt));
  } else
  ...
  Stat.AppendFormat(
      ...
      Status.getCtrlMode() == CNM_Decentral
          ? "Decentral"
          : Status.getCtrlMode() == CNM_Central ? "Central" : "Async",
      Game.Control.ControlTick, pControl->GetBehind(Game.Control.ControlTick),
      Game.Control.ControlRate, pControl->getControlPreSend(),
      pControl->getAvgControlSendTime());

  // Streaming statistics
  if (fStreaming)
    Stat.AppendFormat(
        "|Streaming: %d waiting, %d in, %d out, %d sent",
        pStreamedRecord ? pStreamedRecord->GetStreamingBuf().getSize() : 0,
        pStreamedRecord ? pStreamedRecord->GetStreamingPos() : 0,
        getPendingStreamData(), iCurrentStreamPosition);

  // clients
  Stat.Append("|Clients:");
  for (C4Network2Client *pClient = Clients.GetNextClient(NULL); pClient;
       pClient = Clients.GetNextClient(pClient)) {
    // ignore local
    ...
    switch (pClient->getStatus()) {
      case NCS_Joining:
        szClientStatus = " (joining)";
        break;
      case NCS_Chasing:
        szClientStatus = " (chasing)";
        break;
      case NCS_NotReady:
        szClientStatus = " (!rdy)";
        break;
      case NCS_Remove:
        szClientStatus = " (removed)";
        break;
    }
    Stat.AppendFormat(
        "|- %s %s %s (ID %d) (wait %d ms, behind %d)%s%s",
        Core.isObserver() ? "Observing" : Core.isActivated() ? "Active"
                                                             : "Inactive",
        Core.isHost() ? "host" : "client", Core.getName(), Core.getID(),
        pControl->ClientPerfStat(pClient->getID()),
        Game.Control.ControlTick -
            pControl->ClientNextControl(pClient->getID()),
        szClientStatus, pClient->isActivated() &&
                                !pControl->ClientReady(pClient->getID(),
                                                       Game.Control.ControlTick)
                            ? " (!ctrl)"
                            : "");
    // connections
  ...
  if (!Clients.GetNextClient(NULL)) Stat.Append("| - none -");

  // draw
  Application.DDraw->TextOut(Stat.getData(), Game.GraphicsResource.FontRegular,
                             1.0, cgo.Surface, cgo.X + 20, cgo.Y + 50);
}
...
bool C4Network2::InitNetIO(bool fNoClientID, bool fHost) {
  ...
  if (Config.Network.PortTCP != -1 &&
      Config.Network.PortTCP == Config.Network.PortRefServer) {
  ...
  if (Config.Network.PortUDP != -1 &&
      Config.Network.PortUDP == Config.Network.PortDiscovery) {
  ...
  // init subclass
  if (!NetIO.Init(Config.Network.PortTCP, Config.Network.PortUDP,
                  iPortDiscovery, iPortRefServer, fHost))
    return false;
...
void C4Network2::HandleConn(const C4PacketConn &Pkt,
                            C4Network2IOConnection *pConn,
  ...
  // Check if this peer should be allowed to connect, make space for the new
  // connection.

  // connection is closed?
  if (pConn->isClosed()) return;
  ...
  if (Pkt.getVer() != C4XVERBUILD) {
    ...
    if (!fOK && isHost() && !pClient) {
      // check password
      if (!sPassword.isNull() &&
          !SEqual(Pkt.getPassword(), sPassword.getData())) {
  ...
  if (fOK && pConn->isHalfAccepted()) return;

  // send answer
  C4PacketConnRe pcr(fOK, fWrongPassword, szReply);
  if (!pConn->Send(MkC4NetIOPacket(PID_ConnRe, pcr))) return;

  // accepted?
  if (fOK) {
    // set status
    if (!pConn->isClosed()) pConn->SetHalfAccepted();
  }
  // denied? close
  else {
    // log & close
    LogSilentF("Network: connection by %s (%s:%d) blocked: %s", CCore.getName(),
               inet_ntoa(pConn->getPeerAddr().sin_addr),
               htons(pConn->getPeerAddr().sin_port), szReply ? szReply : "");
    pConn->Close();
  }
}
...
bool C4Network2::HostConnect(const C4ClientCore &CCore,
  ...
  // (core is unofficial, see InitClient() -  will be overwritten later in
  // HandleJoinData)
...
bool C4Network2::Join(C4ClientCore &CCore, C4Network2IOConnection *pConn,
                      const char *&szReply) {
  if (!pConn) return false;
  // security
  if (!isHost()) {
  ...
  if (!fAllowJoin && !fAllowObserve) {
    szReply = "join denied";
    return false;
  }
  if (CCore.getID() != C4ClientIDUnknown) {
    szReply = "join with set id not allowed";
    return false;
  }
  // find free client id
  ...
  if (Clients.GetClient(CCore.getName())) {
    ...
    SCopy(CCore.getName(), szNameTmpl, 254);
    SAppend("%d", szNameTmpl, 256);
    int32_t i = 1;
    do
  ...
  // join client
  Game.Control.DoInput(CID_ClientJoin, new C4ControlClientJoin(CCore),
                       CDT_Direct);
  ...
  // Somewhat ironically, the connection may still timeout (resulting in an
  // instant
  // removal and maybe some funny message sequences).
  // The final client initialization will be done at OnClientConnect.
}

void C4Network2::HandleConnRe(const C4PacketConnRe &Pkt,
                              C4Network2IOConnection *pConn,
                              C4Network2Client *pClient) {
  // Handle the connection request reply. After this handling, the connection
  // should
  ...
  // client must get associated with the connection. After doing so, the
  // connection
  ...
  if (!Pkt.isOK()) {
    ...
    // show message
    LogSilentF("Network: connection to %s (%s:%d) refused: %s",
               pClient->getName(), inet_ntoa(pConn->getPeerAddr().sin_addr),
               htons(pConn->getPeerAddr().sin_port), Pkt.getMsg());
    // close connection
    pConn->Close();
    return;
  }
...
void C4Network2::HandleStatus(const C4Network2Status &nStatus) {
  ...
  Status = nStatus;
  // log
  LogSilentF("Network: going into status %s (tick %d)", Status.getStateName(),
             nStatus.getTargetCtrlTick());
  // reset flags
...
void C4Network2::HandleStatusAck(const C4Network2Status &nStatus,
  ...
  if (nStatus.getState() != Status.getState() ||
      nStatus.getTargetCtrlTick() < Status.getTargetCtrlTick())
    return;
  ...
  if (isHost()) {
    ...
    if (fStatusAck) pClient->SendMsg(MkC4NetIOPacket(PID_StatusAck, nStatus));
    // mark as ready (will clear chase-flag)
    pClient->SetStatus(NCS_Ready);
    // check: everyone ready?
    if (!fStatusAck && fStatusReached) CheckStatusAck();
  } else {
    // target tick doesn't match? ignore
    if (nStatus.getTargetCtrlTick() != Status.getTargetCtrlTick()) return;
    // reached?
    // can be ignored safely otherwise - when the status is reached, we will
    // send
    // status ack on which the host should generate another status ack (see
    // above)
    if (fStatusReached) {
...
void C4Network2::HandleActivateReq(int32_t iTick, C4Network2Client *pByClient) {
  ...
  if (isRunning()) {
    ...
    int32_t iLagFrames = BoundBy(
        pByClient->getMsgConn()->getPingTime() * Game.FPS / 500, 0, 100);
...
void C4Network2::HandleJoinData(const C4PacketJoinData &rPkt) {
  ...
  if (Status.getState() != GS_Init) {
    LogSilentF("Network: unexpected join data received!");
    return;
  }
  // get client ID
  if (rPkt.getClientID() == C4ClientIDUnknown) {
    LogSilentF("Network: host didn't set client ID!");
    Clear();
    return;
  }
  // set local ID
  ...
  if (Status.getState() != GS_Lobby && Status.getState() != GS_Pause &&
      Status.getState() != GS_Go) {
    LogSilentF("Network: join data has bad game status: %s",
               Status.getStateName());
    Clear();
    return;
  }
  ...
  if (!pLocalClient) {
    LogSilentF("Network: Could not find local client in join data!");
    Clear();
    return;
  }
  ...
  pControl->Init(rPkt.getClientID(), false, rPkt.getStartCtrlTick(),
                 pLocalClient->isActivated(), this);
  ...
  if (!ResList.AddByCore(ResDynamic)) {
    LogFatal("Network: can not not retrieve dynamic!");
    Clear();
    return;
  }
...
void C4Network2::OnConnect(C4Network2Client *pClient,
                           C4Network2IOConnection *pConn, const char *szMsg,
                           bool fFirstConnection) {
  // log
  LogSilentF("Network: %s %s connected (%s:%d/%s) (%s)",
             pClient->isHost() ? "host" : "client", pClient->getName(),
             inet_ntoa(pConn->getPeerAddr().sin_addr),
...
void C4Network2::OnConnectFail(C4Network2IOConnection *pConn) {
  LogSilentF("Network: %s connection to %s:%d failed!",
             NetIO.getNetIOName(pConn->getNetClass()),
             inet_ntoa(pConn->getPeerAddr().sin_addr),
             htons(pConn->getPeerAddr().sin_port));

  // maybe client connection failure
  // (happens if the connection is not fully accepted and the client
  // disconnects.
  //  See C4Network2::Join)
  C4Network2Client *pClient = Clients.GetClientByID(pConn->getClientID());
  if (pClient && !pClient->isConnected()) OnClientDisconnect(pClient);
}

void C4Network2::OnDisconnect(C4Network2Client *pClient,
                              C4Network2IOConnection *pConn) {
  LogSilentF("Network: %s connection to %s (%s:%d) lost!",
             NetIO.getNetIOName(pConn->getNetClass()), pClient->getName(),
             inet_ntoa(pConn->getPeerAddr().sin_addr),
             htons(pConn->getPeerAddr().sin_port));

  // connection lost?
  if (!pClient->isConnected()) OnClientDisconnect(pClient);
}
...
void C4Network2::OnClientConnect(C4Network2Client *pClient,
  ...
  if (isHost()) {
    // dynamic available?
    if (!pClient->hasJoinData()) SendJoinData(pClient);
...
void C4Network2::OnClientDisconnect(C4Network2Client *pClient) {
  ...
  if (pLeagueClient && (isHost() || pClient->isHost()))
    LeagueNotifyDisconnect(pClient->getID(), C4LDR_ConnectionFailed);
  // host? Remove this client from the game.
  if (isHost()) {
    bool fHadPlayers =
        !!Game.PlayerInfos.GetPrimaryInfoByClientID(pClient->getID());
    // log
    LogSilentF(LoadResStr("IDS_NET_CLIENTDISCONNECTED"),
               pClient->getName());  // silent, because a duplicate message with
                                     // disconnect reason will follow
    // remove the client
    Game.Clients.CtrlRemove(pClient->getClient(),
                            LoadResStr("IDS_MSG_DISCONNECTED"));
    // stop lobby countdown if players where removed
    if (fHadPlayers && GetLobby() && GetLobby()->IsCountdown())
      AbortLobbyCountdown();
    // check status ack (disconnected client might be the last that was waited
    // for)
    CheckStatusAck();
  ...
  if (!isHost() && pClient->isHost()) {
    StdStrBuf sMsg;
    sMsg.Format(LoadResStr("IDS_NET_HOSTDISCONNECTED"), pClient->getName());
    Log(sMsg.getData());
    // host connection lost: clear up everything
    Game.RoundResults.EvaluateNetwork(C4RoundResults::NR_NetError,
                                      sMsg.getData());
    Clear();
  }
}
...
void C4Network2::SendJoinData(C4Network2Client *pClient) {
  ...
  if (ResDynamic.isNull() || iDynamicTick < Game.Control.ControlTick) {
    ...
    Game.Control.DoInput(CID_Synchronize, new C4ControlSynchronize(false, true),
                         CDT_Sync);
    return;
  }
  // save his client ID
...
C4Network2Res::Ref C4Network2::RetrieveRes(const C4Network2ResCore &Core,
  ...
  bool fLog = false;
  int32_t iProcess = -1;
  uint32_t iTimeout = timeGetTime() + iTimeoutLen;
  // wait for ressource
  while (isEnabled()) {
    // find ressource
    ...
    // res found and loaded completely
      ...
      // log
      if (fLog)
        LogF(LoadResStr("IDS_NET_RECEIVED"), szResName,
             pRes->getCore().getFileName());
      // return
      if (pDlg) delete pDlg;
      return pRes;
    }
    ...
    if (pRes && pRes->getPresentPercent() != iProcess) {
      ...
      if (timeGetTime() > iTimeout) {
        LogFatal(FormatString(LoadResStr("IDS_NET_ERR_RESTIMEOUT"), szResName)
                     .getData());
        if (pDlg) delete pDlg;
        return NULL;
      }
    ...
    if (!pDlg && !Console.Active && Game.pGUI) {
      // create
      pDlg = new C4GUI::ProgressDialog(
          FormatString(LoadResStr("IDS_NET_WAITFORRES"), szResName).getData(),
...
bool C4Network2::CreateDynamic(bool fInit) {
  ...
  sprintf(szDynamicBase, Config.AtNetworkPath("Dyn%s"),
          GetFilename(Game.ScenarioFilename), _MAX_PATH);
  ...
  if (!SaveGame.Save(szDynamicFilename) || !SaveGame.Close()) {
    Log(LoadResStr("IDS_NET_SAVE_ERR_SAVEDYNFILE"));
    return false;
  }
  // add ressource
  C4Network2Res::Ref pRes =
      ResList.AddByFile(szDynamicFilename, true, NRT_Dynamic);
  if (!pRes) {
    Log(LoadResStr("IDS_NET_SAVE_ERR_ADDDYNDATARES"));
    return false;
  }
  // save
...
bool C4Network2::ChangeGameStatus(C4NetGameState enState,
                                  int32_t iTargetCtrlTick, int32_t iCtrlMode) {
...
void C4Network2::CheckStatusReached(bool fFromFinalInit) {
  ...
  if (Status.getState() == GS_Lobby) fStatusReached = fLobbyRunning;
  // game go / pause: control must be initialized and target tick reached
    ...
    if (Game.IsRunning || fFromFinalInit) {
      // Make sure we have reached the tick and the control queue is empty
      // (except for chasing)
...
void C4Network2::OnStatusAck() {
  // log it
  LogSilentF("Network: status %s (tick %d) reached", Status.getStateName(),
             Status.getTargetCtrlTick());
  // pause?
  ...
  if (Status.getState() == GS_Go) {
    // set mode
    pControl->SetCtrlMode(
        static_cast<C4GameControlNetworkMode>(Status.getCtrlMode()));
...
void C4Network2::RequestActivate() {
  ...
  if (Game.Clients.getLocal()->isObserver() ||
      Game.Clients.getLocal()->isActivated()) {
    iLastActivateRequest = 0;
    return;
  }
  // host? just do it
  ...
  // ensure interval
  if (iLastActivateRequest &&
      timeGetTime() < iLastActivateRequest + C4NetActivationReqInterval)
    return;
  ...
  // request
  Clients.SendMsgToHost(MkC4NetIOPacket(
      PID_ClientActReq, C4PacketActivateReq(Game.FrameCounter)));
  // store time
...
void C4Network2::DeactivateInactiveClients() {
  ...
  for (C4Network2Client *pClient = Clients.GetNextClient(NULL); pClient;
    ...
    if (!pClient->isLocal() && pClient->isActivated())
      if (pClient->getLastActivity() + C4NetDeactivationDelay <
          Game.FrameCounter)
        Game.Control.DoInput(
            CID_ClientUpdate,
            new C4ControlClientUpdate(pClient->getID(), CUT_Activate, false),
            CDT_Sync);
}
...
void C4Network2::UpdateChaseTarget() {
  // no chasing clients?
  C4Network2Client *pClient;
  for (pClient = Clients.GetNextClient(NULL); pClient;
       pClient = Clients.GetNextClient(pClient))
    if (pClient->isChasing()) break;
  if (!pClient) {
    iLastChaseTargetUpdate = 0;
    return;
  }
  // not time for an update?
  if (!iLastChaseTargetUpdate ||
      long(iLastChaseTargetUpdate + C4NetChaseTargetUpdateInterval) >
          time(NULL))
    return;
...
void C4Network2::LeagueGameEvaluate(const char *szRecordName,
...
bool C4Network2::InitLeague(bool *pCancel) {
  if (fHost) {
    ...
    if (pLeagueClient) delete pLeagueClient;
    pLeagueClient = NULL;
    ...
    // Not needed?
    if (!MasterServerAddress.getLength()) return true;
  }
  ...
  if (!pLeagueClient->Init() ||
    ...
    // Log message
    StdStrBuf Message = FormatString(LoadResStr("IDS_NET_ERR_LEAGUEINIT"),
                                     pLeagueClient->GetError());
    LogFatal(Message.getData());
    // Clear league
    delete pLeagueClient;
    pLeagueClient = NULL;
    if (fHost) Game.Parameters.LeagueAddress.Clear();
    // Show message, allow abort
    bool fResult = true;
    if (Game.pGUI && !Console.Active)
      fResult = Game.pGUI->ShowMessageModal(
          Message.getData(), LoadResStr("IDS_NET_ERR_LEAGUE"),
          (pCancel ? C4GUI::MessageDialog::btnOK : 0) |
...
bool C4Network2::LeagueStart(bool *pCancel) {
  // Not needed?
  if (!pLeagueClient || !fHost) return true;
  ...
  if (!pLeagueClient->Start(Ref)) {
    // Log message
    StdStrBuf Message = FormatString(LoadResStr("IDS_NET_ERR_LEAGUE_STARTGAME"),
                                     pLeagueClient->GetError());
    LogFatal(Message.getData());
    // Show message
    if (Game.pGUI && !Console.Active) {
      ...
      bool fResult = Game.pGUI->ShowMessageModal(
          Message.getData(), LoadResStr("IDS_NET_ERR_LEAGUE"),
          pCancel
              ? (C4GUI::MessageDialog::btnOK | C4GUI::MessageDialog::btnAbort)
  ...
  // We have an internet connection, so let's punch the master server here in
  // order to open an udp port
  C4NetIO::addr_t PuncherAddr;
  if (ResolveAddress(Config.Network.PuncherAddress, &PuncherAddr,
                     C4NetStdPortPuncher))
    NetIO.Punch(PuncherAddr);

  // Let's wait for response
  StdStrBuf Message = FormatString(LoadResStr("IDS_NET_LEAGUE_REGGAME"),
                                   pLeagueClient->getServerName());
  Log(Message.getData());
  // Set up a dialog
  C4GUI::MessageDialog *pDlg = NULL;
  if (Game.pGUI && !Console.Active) {
    // create & show
    pDlg = new C4GUI::MessageDialog(
        Message.getData(), LoadResStr("IDS_NET_LEAGUE_STARTGAME"),
        C4GUI::MessageDialog::btnAbort, C4GUI::Ico_NetWait,
  ...
  while (pLeagueClient->isBusy()) {
    ...
    if (!pLeagueClient->Execute(0)) break;
  }
  // Close dialog
  if (Game.pGUI && pDlg) {
    pDlg->Close(true);
  ...
  if (!pLeagueClient->isSuccess() ||
      !pLeagueClient->GetStartReply(&LeagueServerMessage, &League,
                                    &StreamingAddr, &Seed, &MaxPlayersLeague)) {
    const char *pError =
        pLeagueClient->GetError()
            ? pLeagueClient->GetError()
            : LeagueServerMessage.getLength()
                  ? LeagueServerMessage.getData()
                  : LoadResStr("IDS_NET_ERR_LEAGUE_EMPTYREPLY");
    StdStrBuf Message =
        FormatString(LoadResStr("IDS_NET_ERR_LEAGUE_REGGAME"), pError);
    // Log message
    Log(Message.getData());
    // Show message
    if (Game.pGUI && !Console.Active) {
  ...
  if (LeagueServerMessage.getLength()) {
    StdStrBuf Message = FormatString(LoadResStr("IDS_MSG_LEAGUEGAMESIGNUP"),
                                     pLeagueClient->getServerName(),
                                     LeagueServerMessage.getData());
    // Log message
    Log(Message.getData());
  ...
  if (MaxPlayersLeague) Game.Parameters.MaxPlayers = MaxPlayersLeague;
  if (!League.getLength()) {
    Game.Parameters.LeagueAddress.Clear();
    ...
    if (Status.getCtrlMode() == CNM_Async) Status.SetCtrlMode(CNM_Central);
#endif
  }
...
bool C4Network2::LeagueUpdate() {
  ...
  if (pLeagueClient->isBusy()) return true;
  ...
  if (!pLeagueClient->Update(Ref)) {
    // Log
    LogF(LoadResStr("IDS_NET_ERR_LEAGUE_UPDATEGAME"),
         pLeagueClient->GetError());
    return false;
  }
...
bool C4Network2::LeagueUpdateProcessReply() {
  ...
  bool fSucc =
      pLeagueClient->isSuccess() &&
      pLeagueClient->GetUpdateReply(&LeagueServerMessage, &PlayerLeagueInfos);
    ...
    const char *pError =
        pLeagueClient->GetError()
            ...
            : LeagueServerMessage.getLength()
                  ...
                  : LoadResStr("IDS_NET_ERR_LEAGUE_EMPTYREPLY");
    StdStrBuf Message =
        FormatString(LoadResStr("IDS_NET_ERR_LEAGUE_UPDATEGAME"), pError);
    // Show message - no dialog, because it's not really fatal and might happen
    // in the running game
    Log(Message.getData());
    return false;
  }
  ...
  C4ClientPlayerInfos *pInfos;
  C4PlayerInfo *pInfo, *pResultInfo;
...
bool C4Network2::LeagueEnd(const char *szRecordName, const BYTE *pRecordSHA) {
  ...
  // Not needed?
  if (!pLeagueClient || !fHost || fLeagueEndSent) return true;
  ...
  for (int iRetry = 0; iRetry < MAX_RETRIES; iRetry++) {
    ...
    if (!pLeagueClient->End(Ref, szRecordName, pRecordSHA)) {
      // Log message
      sResultMessage = FormatString(LoadResStr("IDS_NET_ERR_LEAGUE_FINISHGAME"),
                                    pLeagueClient->GetError());
      Log(sResultMessage.getData());
      // Show message, allow retry
      if (!Game.pGUI || Console.Active) break;
      bool fRetry = Game.pGUI->ShowMessageModal(
          sResultMessage.getData(), LoadResStr("IDS_NET_ERR_LEAGUE"),
          C4GUI::MessageDialog::btnRetryAbort, C4GUI::Ico_Error);
      if (fRetry) continue;
      break;
    }
    // Let's wait for response
    StdStrBuf Message = FormatString(LoadResStr("IDS_NET_LEAGUE_SENDRESULT"),
                                     pLeagueClient->getServerName());
    Log(Message.getData());
    // Wait for response
    while (pLeagueClient->isBusy()) {
      // Check if league server has responded
      if (!pLeagueClient->Execute(100)) break;
    }
    // Error?
    StdStrBuf LeagueServerMessage;
    if (!pLeagueClient->isSuccess() ||
        !pLeagueClient->GetEndReply(&LeagueServerMessage, &RoundResults)) {
      const char *pError =
          pLeagueClient->GetError()
              ? pLeagueClient->GetError()
              : LeagueServerMessage.getLength()
                    ...
                    : LoadResStr("IDS_NET_ERR_LEAGUE_EMPTYREPLY");
      sResultMessage.Take(
          FormatString(LoadResStr("IDS_NET_ERR_LEAGUE_SENDRESULT"), pError));
      ...
      fRetry = Game.pGUI->ShowMessageModal(
          sResultMessage.getData(), LoadResStr("IDS_NET_ERR_LEAGUE"),
          fRetry ? C4GUI::MessageDialog::btnRetryAbort
                 : C4GUI::MessageDialog::btnAbort,
          C4GUI::Ico_Error);
      if (fRetry) continue;
    } else {
      // All OK!
      sResultMessage.Copy(LoadResStr(Game.Parameters.isLeague()
                                         ? "IDS_MSG_LEAGUEEVALUATIONSUCCESSFU"
                                         : "IDS_MSG_INTERNETGAMEEVALUATED"));
      fIsError = false;
    }
    // Done
    break;
  ...
  // Take round results
  Game.RoundResults.EvaluateLeague(sResultMessage.getData(), !fIsError,
                                   RoundResults);

  // Send round results to other clients
  C4PacketLeagueRoundResults LeagueUpdatePacket(sResultMessage.getData(),
                                                !fIsError, RoundResults);
  Clients.BroadcastMsgToClients(
      MkC4NetIOPacket(PID_LeagueRoundResults, LeagueUpdatePacket));
...
bool C4Network2::LeaguePlrAuth(C4PlayerInfo *pInfo) {
  ...
  // Try to auth with WebCode, if it's an official league server and we have
  ...
  for (;;) {
    ...
    if (fRegister) {
      ...
      if (!C4LeagueSignupDialog::ShowModal(
              pInfo->GetName(), "", pLeagueClient->getServerName(), &NewAccount,
              &NewPassword, !fOfficialLeague, true))
        return false;
      if (!NewPassword.getLength()) NewPassword.Copy(Password);
    } else if (!fWebCode) {
      // username is default for account, no default password
      Password.Clear();

      // ask for account
      if (!C4LeagueSignupDialog::ShowModal(
              pInfo->GetName(), "", pLeagueClient->getServerName(), &Account,
              &Password, !fOfficialLeague, false))
        return false;
    }
    ...
    if (!pLeagueClient->Auth(
            *pInfo, Account.getData(), Password.getData(),
            NewAccount.getLength() ? NewAccount.getData() : NULL,
            NewPassword.getLength() ? NewPassword.getData() : NULL))
      return false;
    ...
    // Wait for a response
    StdStrBuf Message =
        FormatString(LoadResStr("IDS_MSG_TRYLEAGUESIGNUP"), pInfo->GetName(),
                     Account.getData(), pLeagueClient->getServerName());
    Log(Message.getData());
    // Set up a dialog
    C4GUI::MessageDialog *pDlg = NULL;
    if (Game.pGUI && !Console.Active) {
      // create & show
      pDlg = new C4GUI::MessageDialog(
          Message.getData(), LoadResStr("IDS_DLG_LEAGUESIGNUP"),
          C4GUI::MessageDialog::btnAbort, C4GUI::Ico_NetWait,
          C4GUI::MessageDialog::dsRegular);
      if (!pDlg || !pDlg->Show(Game.pGUI, true)) return false;
    }
    ...
    // Success?
    StdStrBuf AUID, AccountMaster;
    bool fUnregistered = false;
    if (pLeagueClient->GetAuthReply(&Message, &AUID, &AccountMaster,
                                    &fUnregistered)) {
      // Set AUID
      ...
      if (Message.getLength())
        ...
        fSuccess = Game.pGUI->ShowMessageModal(
            FormatString(LoadResStr("IDS_MSG_LEAGUEPLAYERSIGNUPAS"),
                         pInfo->GetName(), AccountMaster.getData(),
                         pLeagueClient->getServerName()).getData(),
            LoadResStr("IDS_DLG_LEAGUESIGNUPCONFIRM"),
            C4GUI::MessageDialog::btnOKAbort, C4GUI::Ico_Ex_League);
      else
        fSuccess = Game.pGUI->ShowMessageModal(
            FormatString(LoadResStr("IDS_MSG_LEAGUEPLAYERSIGNUP"),
                         pInfo->GetName(),
                         pLeagueClient->getServerName()).getData(),
            LoadResStr("IDS_DLG_LEAGUESIGNUPCONFIRM"),
            C4GUI::MessageDialog::btnOKAbort, C4GUI::Ico_Ex_League);
      ...
      if (fSuccess)
        ...
        Game.pGUI->ShowMessageModal(
            FormatString(LoadResStr("IDS_MSG_LEAGUESIGNUPCANCELLED"),
                         pInfo->GetName()).getData(),
            LoadResStr("IDS_DLG_LEAGUESIGNUP"), C4GUI::MessageDialog::btnOK,
            C4GUI::Ico_Notify);
      ...
      if ((!fWebCode && !fUnregistered) || fRegister) {
        ...
        Game.pGUI->ShowMessageModal(
            FormatString(LoadResStr("IDS_MSG_LEAGUESERVERMSG"),
                         Message.getData()).getData(),
            LoadResStr("IDS_DLG_LEAGUESIGNUPFAILED"),
    ...
    if (AccountMaster.getLength()) Account.Take(AccountMaster);
...
bool C4Network2::LeaguePlrAuthCheck(C4PlayerInfo *pInfo) {
  ...
  if (!pLeagueClient->AuthCheck(*pInfo)) return false;

  // Log
  StdStrBuf Message =
      FormatString(LoadResStr("IDS_MSG_LEAGUEJOINING"), pInfo->GetName());
  Log(Message.getData());
  ...
  // Check if league server approves. pInfo will have league info if this call
  // is successful.
  if (!pLeagueClient->GetAuthCheckReply(
          &Message, Game.Parameters.League.getData(), pInfo)) {
    LeagueShowError(FormatString(LoadResStr("IDS_MSG_LEAGUEJOINREFUSED"),
                                 pInfo->GetName(),
                                 Message.getData()).getData());
    return false;
  }
...
void C4Network2::LeagueNotifyDisconnect(int32_t iClientID,
  ...
  // clients send notifications for their own players; host sends for the
  // affected client players
  if (!isHost()) {
    if (!Clients.GetLocal()) return;
    iClientID = Clients.GetLocal()->getID();
  }
  // clients only need notifications if they have players in the game
  const C4ClientPlayerInfos *pInfos =
      Game.PlayerInfos.GetInfoByClientID(iClientID);
  if (!pInfos) return;
  int32_t i = 0;
  C4PlayerInfo *pInfo;
  while (pInfo = pInfos->GetPlayerInfo(i++))
    if (pInfo->IsJoined() && !pInfo->IsRemoved()) break;
  if (!pInfo) return;
  // Make sure league client is avilable
  LeagueWaitNotBusy();
...
void C4Network2::Vote(C4ControlVoteType eType, bool fApprove, int32_t iData) {
  ...
  if (!GetVote(C4ClientIDUnknown, eType, iData)) {
    ...
    if (time(NULL) < (time_t)(iLastOwnVoting + C4NetMinVotingInterval)) {
      ...
      if ((eType == VT_Kick && iData == Game.Clients.getLocalID()) ||
          eType == VT_Cancel)
  ...
  if (GetVote(Game.Control.ClientID(), eType, iData)) return;
  ...
  // send vote control
  Game.Control.DoInput(CID_Vote, new C4ControlVote(eType, fApprove, iData),
                       CDT_Direct);
}

void C4Network2::AddVote(const C4ControlVote &Vote) {
  // Save back timestamp
  if (!Votes.firstPkt()) iVoteStartTime = time(NULL);
  // Save vote back
...
C4IDPacket *C4Network2::GetVote(int32_t iClientID, C4ControlVoteType eType,
                                int32_t iData) {
  C4ControlVote *pVote;
...
void C4Network2::EndVote(C4ControlVoteType eType, bool fApprove,
                         int32_t iData) {
  // Remove all vote packets
  C4IDPacket *pPkt;
  int32_t iOrigin = C4ClientIDUnknown;
  ...
  if (fApprove && iOrigin == Game.Clients.getLocalID()) iLastOwnVoting = 0;
  // Dialog open?
  if (pVoteDialog)
    if (pVoteDialog->getVoteType() == eType &&
        pVoteDialog->getVoteData() == iData) {
      // close
      delete pVoteDialog;
      pVoteDialog = NULL;
    }
  // Did we try to kick ourself? Ask if we'd like to surrender
  bool fCancelVote = (eType == VT_Kick && iData == Game.Clients.getLocalID()) ||
                     eType == VT_Cancel;
  ...
  if (fApprove && eType == VT_Pause) fPausedForVote = !iData;
...
void C4Network2::OpenVoteDialog() {
  ...
  // Can't vote?
  C4ClientPlayerInfos *pPlayerInfos =
      Game.PlayerInfos.GetInfoByClientID(Game.Clients.getLocalID());
  if (!pPlayerInfos || !pPlayerInfos->GetPlayerCount() ||
      !pPlayerInfos->GetJoinedPlayerCount())
    return;
  ...
  for (C4IDPacket *pPkt = Votes.firstPkt(); pPkt; pPkt = Votes.nextPkt(pPkt)) {
    ...
    if (!GetVote(Game.Control.ClientID(), pVote->getType(), pVote->getData())) {
      ...
      StdStrBuf Msg;
      Msg.Format(LoadResStr("IDS_VOTE_WANTSTOALLOW"),
                 pSrcClient ? pSrcClient->getName() : "???",
                 pVote->getDesc().getData());
      Msg.AppendChar('|');
      Msg.Append(pVote->getDescWarning());

      // Open dialog
      pVoteDialog = new C4VoteDialog(Msg.getData(), pVote->getType(),
                                     pVote->getData(), false);
...
void C4Network2::OpenSurrenderDialog(C4ControlVoteType eType, int32_t iData) {
  if (!pVoteDialog) {
    pVoteDialog = new C4VoteDialog(LoadResStr("IDS_VOTE_SURRENDERWARNING"),
                                   eType, iData, true);
    pVoteDialog->SetDelOnClose();
    pVoteDialog->Show(Game.pGUI, true);
  }
...
void C4Network2::OnVoteDialogClosed() { pVoteDialog = NULL; }

// *** C4VoteDialog

C4VoteDialog::C4VoteDialog(const char *szText, C4ControlVoteType eVoteType,
                           int32_t iVoteData, bool fSurrender)
    : eVoteType(eVoteType),
      iVoteData(iVoteData),
      fSurrender(fSurrender),
      MessageDialog(szText, LoadResStr("IDS_DLG_VOTING"),
                    C4GUI::MessageDialog::btnYesNo, C4GUI::Ico_Confirm,
                    C4GUI::MessageDialog::dsRegular, NULL, true) {}
...
void C4VoteDialog::OnClosed(bool fOK) {
  ...
  if (fSurrender) {
    ...
    if (fOK) {
      // set game leave reason, although round results dialog isn't showing it
      // ATM
      Game.RoundResults.EvaluateNetwork(
          C4RoundResults::NR_NetError,
          LoadResStr("IDS_ERR_YOUSURRENDEREDTHELEAGUEGA"));
      // leave game
      Game.Network.LeagueSurrender();
      Game.Network.Clear();
      // We have just league-surrendered. Abort the game - that is what we
      // originally wanted.
      // Note: as we are losing league points and this is a relevant game, it
      // would actually be
      // nice to show an evaluation dialog which tells us that we have lost and
      // how many league
      // points we have lost. But until the evaluation dialog can actually do
      // that, it is better
      // to abort completely.
      // Note2: The league dialog will never know that, because the game will
      // usually not be over yet.
...
bool C4Network2::StartStreaming(C4Record *pRecord) {
  ...
  if (deflateInit(&StreamCompressor, 9) != Z_OK) return false;
  ...
  if (!pStreamer->Init()) return false;
...
bool C4Network2::StreamIn(bool fFinish) {
  ...
  if (!fFinish)
    if (!Data.getSize() || !StreamCompressor.avail_out) return false;
  ...
  do {
    ...
    if (iInAmount > 0) pStreamedRecord->ClearStreamingBuf(iInAmount);

    // Done?
    if (!fFinish || ret == Z_STREAM_END) break;
...
bool C4Network2::StreamOut() {
  // Streamer busy?
  if (!pStreamer || pStreamer->isBusy()) return false;

  // Streamer done?
  if (pStreamer->isSuccess()) {
    // Move new data to front of buffer
    if (getPendingStreamData() != iCurrentStreamAmount)
      StreamingBuf.Move(iCurrentStreamAmount,
                        getPendingStreamData() - iCurrentStreamAmount);
  ...
  if (pStreamedRecord) {
    ...
    if (getPendingStreamData() < C4NetStreamingMinBlockSize) return false;

    // Overflow protection
    if (iLastStreamAttempt &&
        iLastStreamAttempt + C4NetStreamingInterval >= time(NULL))
      return false;
  ...
  StreamAddr.AppendFormat("pos=%d&end=%d", iCurrentStreamPosition,
                          !pStreamedRecord);
