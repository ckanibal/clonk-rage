...
// standard ports
const int16_t C4NetStdPortTCP = 11112, C4NetStdPortUDP = 11113,
              C4NetStdPortDiscovery = 11114, C4NetStdPortRefServer = 11111,
              C4NetStdPortPuncher = 11115, C4NetStdPortHTTP = 80;
...
class C4Network2 {
 ...
 protected:
  ...
  C4Network2HTTPClient *pStreamer;
...
// data access
  ...
  const char *GetPassword() const {
    return sPassword.getData();
  }  // Oh noez, now the password is public!
  ...
  InitResult InitClient(const class C4Network2Address *pAddrs, int iAddrCount,
  ...
  C4Network2Res::Ref RetrieveRes(const C4Network2ResCore &Core,
                                 int32_t iTimeout, const char *szResName,
  ...
  StdStrBuf
  QueryClientPassword();  // ask client for a password; empty if user canceled
  ...
  void HandleLobbyPacket(char cStatus, const C4PacketBase *pBasePkt,
                         C4Network2IOConnection *pConn);
  ...
  // league
  void LeagueGameEvaluate(const char *szRecordName = NULL,
                          const BYTE *pRecordSHA = NULL);
  void LeagueSignupDisable();  // if "internet game" button is switched off in
                               // lobby: Remove from league server
  bool LeagueSignupEnable();   // if "internet game" button is switched on in
                               // lobby: (re)Add to league server
  void InvalidateReference();  // forces a recreation and re-send of the game
                               // reference in the next execution cycle
  bool LeaguePlrAuth(C4PlayerInfo *pInfo);  // client: get authentication for a
                                            // player from the league server
  bool LeaguePlrAuthCheck(C4PlayerInfo *pInfo);  // host: check AUID of player
                                                 // info with league server
  void LeagueNotifyDisconnect(int32_t iClientID,
                              enum C4LeagueDisconnectReason eReason);  //
  void LeagueWaitNotBusy();  // block until league serveris no longer busy.
                             // Process update reply if last message was an
                             // update
  void LeagueSurrender();    // forfeit in league - just fake a disconnect
  void LeagueShowError(const char *szMsg);  // show league error msg box in
                                            // fullscreen; just log in console

  // voting
  void Vote(C4ControlVoteType eType, bool fApprove = true, int32_t iData = 0);
  void AddVote(const C4ControlVote &Vote);
  C4IDPacket *GetVote(int32_t iClientID, C4ControlVoteType eType,
                      int32_t iData);
  ...
  size_t getPendingStreamData() const {
    return StreamingBuf.getSize() - StreamCompressor.avail_out;
  }
 ...
 protected:
  ...
  void HandleConn(const class C4PacketConn &Pkt, C4Network2IOConnection *pConn,
                  C4Network2Client *pClient);
  bool CheckConn(const C4ClientCore &CCore, C4Network2IOConnection *pConn,
                 C4Network2Client *pClient, const char *&szReply);
  bool HostConnect(const C4ClientCore &CCore, C4Network2IOConnection *pConn,
                   const char *&szReply);
  bool Join(C4ClientCore &CCore, C4Network2IOConnection *pConn,
            const char *&szReply);
  void HandleConnRe(const class C4PacketConnRe &Pkt,
                    C4Network2IOConnection *pConn, C4Network2Client *pClient);
  void HandleStatus(const C4Network2Status &nStatus);
  void HandleStatusAck(const C4Network2Status &nStatus,
                       C4Network2Client *pClient);
  void HandleActivateReq(int32_t iTick, C4Network2Client *pClient);
  void HandleJoinData(const class C4PacketJoinData &rPkt);

  // events
  void OnConnect(C4Network2Client *pClient, C4Network2IOConnection *pConn,
                 const char *szMsg, bool fFirstConnection);
  void OnConnectFail(C4Network2IOConnection *pConn);
  void OnDisconnect(C4Network2Client *pClient, C4Network2IOConnection *pConn);
  void OnClientConnect(C4Network2Client *pClient,
                       C4Network2IOConnection *pConn);
  ...
  bool ChangeGameStatus(C4NetGameState enState, int32_t iTargetCtrlTick,
                        int32_t iCtrlMode = -1);
  ...
  bool LeagueEnd(const char *szRecordName = NULL,
                 const BYTE *pRecordSHA = NULL);
...
class C4VoteDialog : public C4GUI::MessageDialog {
 public:
  C4VoteDialog(const char *szText, C4ControlVoteType eVoteType,
 ...
 private:
  virtual bool OnEnter() {
    UserClose(false);
    return true;
  }  // the vote dialog defaults to "No" [MarkFra]
