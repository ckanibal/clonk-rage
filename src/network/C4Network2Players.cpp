...
C4Network2Players::C4Network2Players()
    : rInfoList(Game.Parameters.PlayerInfos) {
...
void C4Network2Players::Init() {
  ...
  if (Game.Network.isHost())
    Game.PlayerInfos.CreateRestoreInfosForJoinedScriptPlayers(
        Game.RestorePlayerInfos);
}
...
bool C4Network2Players::JoinLocalPlayer(const char *szLocalPlayerFilename,
  ...
  // host or client?
  if (Game.Network.isHost()) {
    // error joining players? Zero players is OK for initial packet; marks host
    // as observer
    ...
    // any players to join? Zero players is OK for initial packet; marks client
    // as observer
    // it's also necessary to send the empty player info packet, so the host
    // will answer
    // with infos of all other clients
    if (fAdd && !JoinRequest.Info.GetPlayerCount()) return false;
    Game.Network.Clients.SendMsgToHost(
        MkC4NetIOPacket(PID_PlayerInfoUpdReq, JoinRequest));
    // request activation
    if (JoinRequest.Info.GetPlayerCount() &&
...
void C4Network2Players::RequestPlayerInfoUpdate(
    const class C4ClientPlayerInfos &rRequest) {
  // network only
  assert(Game.Network.isEnabled());
  // host or client?
  if (Game.Network.isHost()) {
    ...
    Game.Network.Clients.SendMsgToHost(
        MkC4NetIOPacket(PID_PlayerInfoUpdReq, UpdateRequest));
  }
}

void C4Network2Players::HandlePlayerInfoUpdRequest(
    const class C4ClientPlayerInfos *pInfoPacket, bool fByHost) {
  // network host only
  assert(Game.Network.isEnabled());
  ...
  // check those with unassigned IDs only, so update packets won't be rejected
  // by this
  if (!OwnInfoPacket.IsInitialPacket()) {
    C4ClientPlayerInfos *pExistingClientInfo =
        rInfoList.GetInfoByClientID(OwnInfoPacket.GetClientID());
    if (pExistingClientInfo) {
      int iCnt = OwnInfoPacket.GetPlayerCount();
      C4PlayerInfo *pPlrInfo;
      C4Network2Res *pRes;
      while (iCnt--)
        if (pPlrInfo = OwnInfoPacket.GetPlayerInfo(iCnt))
          if (!pPlrInfo->GetID())
            if (pRes = pPlrInfo->GetRes())
...
// player join request without players: probably all removed because doubled
  ...
  // assign player IDs
  if (!rInfoList.AssignPlayerIDs(&OwnInfoPacket) &&
      OwnInfoPacket.IsAddPacket()) {
    // no players could be joined in an add request: probably because the
    // maximum player limit has been reached
    return;
  }
  ...
  // this may only change colors and names of all unjoined players (which is all
  ...
  int32_t iPlrInfo = 0;
  C4PlayerInfo *pPlrInfo;
  while (pPlrInfo = OwnInfoPacket.GetPlayerInfo(iPlrInfo++))
    pPlrInfo->ResetLeagueProjectedGain();
  if (Game.Parameters.isLeague()) {
    // lobby only
    if (!Game.Network.isLobbyActive()) return;
    // check league authentication for new players
    for (int i = 0; i < OwnInfoPacket.GetPlayerCount(); i++)
      if (!rInfoList.GetPlayerInfoByID(
              OwnInfoPacket.GetPlayerInfo(i)->GetID())) {
        ...
        if (!Game.Network.LeaguePlrAuthCheck(pInfo)) {
          ...
          // always reset authentication ID after check - it's not needed
          // anymore
          pInfo->SetAuthID("");
      }
  }
  ...
  // this will add the player infos directly on host side (DirectExec as a
  // subcall),
  // so future player join request will take the other joined  clients into
  // consideration
  ...
  // queue
  Game.Control.DoInput(CID_PlrInfo, new C4ControlPlayerInfo(OwnInfoPacket),
                       CDT_Direct);
...
void C4Network2Players::HandlePlayerInfo(
    const class C4ClientPlayerInfos &rInfoPacket) {
  // network only
  assert(Game.Network.isEnabled());
  ...
  // add client info  to local player info list - eventually deleting
  // pClientInfo and
  // returning a pointer to the new info structure when multiple player infos
  // are merged
  ...
  Game.Teams.RecheckTeams();  // recheck random teams - if a player left, teams
                              // may need to be rebalanced
  // make sure resources are loaded for those players
  rInfoList.LoadResources();
  // get associated client - note that pClientInfo might be NULL for empty
  // packets that got discarded
  if (pClientInfo) {
    const C4Client *pClient =
        Game.Clients.getClientByID(pClientInfo->GetClientID());
    // host, game running and client active already?
    if (Game.Network.isHost() && Game.Network.isRunning() && pClient &&
        pClient->isActivated()) {
      // then join the players immediately
      JoinUnjoinedPlayersInControlQueue(pClientInfo);
    }
  }
  // adding the player may have invalidated other players (through team
  // settings). Send them.
  SendUpdatedPlayers();
...
void C4Network2Players::UpdateSavegameAssignments(
    C4ClientPlayerInfos *pNewInfo) {
  // safety
  if (!pNewInfo) return;
  // check all joins of new info; backwards so they can be deleted
  C4PlayerInfo *pInfo, *pInfo2, *pSaveInfo;
  int i = pNewInfo->GetPlayerCount(), j, id;
  while (i--)
    if (pInfo = pNewInfo->GetPlayerInfo(i))
      if (id = pInfo->GetAssociatedSavegamePlayerID()) {
        ...
        // if the player joined just for the savegame assignment, and that
        // failed, delete it
...
void C4Network2Players::JoinUnjoinedPlayersInControlQueue(
    C4ClientPlayerInfos *pNewPacket) {
  ...
  while (pInfo = pNewPacket->GetPlayerInfo(i++))
    ...
    if (!pInfo->HasJoinIssued())
      if (!pInfo->GetAssociatedSavegamePlayerID()) {
        ...
        // but better mark join now already to prevent permanent sending
        // overkill
        pInfo->SetJoinIssued();
        // do so!
        C4Network2Res *pPlrRes = pInfo->GetRes();
        C4Network2Client *pClient =
            Game.Network.Clients.GetClientByID(pNewPacket->GetClientID());
        if (!pPlrRes ||
          ...
          if (pInfo->GetType() != C4PT_Script) {
            ...
            LogF(
                "Network: C4Network2Players::JoinUnjoinedPlayersInControlQueue "
                "failed to join player %s!",
                szPlrName);
            continue;
          }
        if (pPlrRes) {
          ...
          Game.Input.Add(CID_JoinPlr,
                         new C4ControlJoinPlayer(
                             pPlrRes->getFile(), pNewPacket->GetClientID(),
                             pInfo->GetID(), pPlrRes->getCore()));
        } else {
          // join without resource (script player)
          Game.Input.Add(CID_JoinPlr,
                         new C4ControlJoinPlayer(
                             NULL, pNewPacket->GetClientID(), pInfo->GetID()));
        }
      }
}

void C4Network2Players::HandlePacket(char cStatus, const C4PacketBase *pPacket,
  ...
  if (!pClient)
    pClient = Game.Network.Clients.GetClientByID(pConn->getClientID());
...
#define GETPKT(type, name) \
  ...
  if (cStatus == PID_PlayerInfoUpdReq) {
    ...
    // this packet is sent to the host only, and thus cannot have been sent from
    // the host
    ...
    // process
    Game.RoundResults.EvaluateLeague(pkLeagueInfo.sResultsString.getData(),
                                     pkLeagueInfo.fSuccess,
                                     pkLeagueInfo.Players);
  }
...
void C4Network2Players::OnClientPart(C4Client *pPartClient) {
  // lobby could be notified about the removal - but this would be redundant,
  // because
  // client leave notification is already done directly; this will delete any
  // associated players
  C4ClientPlayerInfos **ppCltInfo =
      rInfoList.GetInfoPtrByClientID(pPartClient->getID());
  // abort here if no info is registered - client seems to have had a short life
  // only, anyway...
  if (!ppCltInfo) return;
  ...
  // empty? remove
  if (!(*ppCltInfo)->GetPlayerCount()) rInfoList.RemoveInfo(ppCltInfo);
  ...
  // invalidate reference
  if (Game.Network.isHost()) Game.Network.InvalidateReference();
}
