...
class DirSizeHelper {
  ...
  static bool Helper(const char *szPath) {
    if (szPath[SLen(szPath) - 1] == '.') return false;
    if (iSize > iMaxSize) return false;
 ...
 public:
  static bool GetDirSize(const char *szPath, uint32_t *pSize,
...
void C4Network2ResCore::Set(C4Network2ResType enType, int32_t iResID,
                            const char *strFileName, uint32_t inContentsCRC,
                            const char *strAuthor) {
  // Initialize base data
  eType = enType;
  iID = iResID;
  iDerID = -1;
  fLoadable = false;
  iFileSize = iFileCRC = ~0;
  iContentsCRC = inContentsCRC;
  iChunkSize = C4NetResChunkSize;
...
void C4Network2ResCore::CompileFunc(StdCompiler *pComp) {
  pComp->Value(
      mkNamingAdapt(mkEnumAdaptT<uint8_t>(eType, C4Network2ResType_EnumMap),
                    "Type", NRT_Null));
  ...
  if (fHasFileSHA) pComp->Value(mkNamingAdapt(mkHexAdapt(FileSHA), "FileSHA"));
...
C4Network2ResChunkData::C4Network2ResChunkData(
    const C4Network2ResChunkData &Data2)
    : C4PacketBase(Data2),
...
C4Network2ResChunkData::~C4Network2ResChunkData() { Clear(); }

C4Network2ResChunkData &C4Network2ResChunkData::operator=(
    const C4Network2ResChunkData &Data2) {
  // clear, merge
...
void C4Network2ResChunkData::AddChunkRange(int32_t iStart, int32_t iLength) {
  ...
  for (pRange = pChunkRanges, pPrev = NULL; pRange;
       pPrev = pRange, pRange = pRange->Next)
    if (pRange->Start >= iStart) break;
  // create new
...
int32_t C4Network2ResChunkData::GetChunkToRetrieve(
    const C4Network2ResChunkData &Available, int32_t iLoadingCnt,
    int32_t *pLoading) const {
  // (this version is highly calculation-intensitive, yet the most satisfactory
  //  solution I could find)

  // find everything that should not be retrieved
  C4Network2ResChunkData ChData;
  Available.GetNegative(ChData);
  ChData.Merge(*this);
  for (int32_t i = 0; i < iLoadingCnt; i++) ChData.AddChunk(pLoading[i]);
  // nothing to retrieve?
  if (ChData.isComplete()) return -1;
  // invert to get everything that should be retrieved
  C4Network2ResChunkData ChData2;
  ChData.GetNegative(ChData2);
  // select chunk (random)
...
bool C4Network2ResChunkData::MergeRanges(ChunkRange *pRange) {
  ...
  // get overlap
  int32_t iOverlap =
      Min((pRange->Start + pRange->Length) - pNext->Start, pNext->Length);
  // set new chunk range
...
void C4Network2ResChunkData::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(
      mkNamingAdapt(mkIntPackAdapt(iChunkRangeCnt), "ChunkRangeCnt", 0));
  // Ranges
  if (!pComp->Name("Ranges")) pComp->excCorrupt("ResChunk ranges expected!");
  ...
  // Terminate list
  if (fCompiler) (pRange ? pRange->Next : pChunkRanges) = NULL;
  pComp->NameEnd();
}
...
bool C4Network2Res::SetByFile(const char *strFilePath, bool fTemp,
                              C4Network2ResType eType, int32_t iResID,
                              const char *szResName, bool fSilent) {
  ...
  if (!FileExists(szFile)) {
    if (!fSilent) LogF("SetByFile: file %s not found!", strFilePath);
    return false;
  }
  // calc checksum
  uint32_t iCRC32;
  ...
  // log
  LogSilentF("Network: Resource: complete %d:%s is file %s (%s)", iResID,
             szResName, szFile, fTemp ? "temp" : "static");
#endif
  // set core
...
bool C4Network2Res::SetByGroup(C4Group *pGrp, bool fTemp,
                               C4Network2ResType eType, int32_t iResID,
                               const char *szResName,
                               bool fSilent)  // by main thread
{
  Clear();
  CStdLock FileLock(&FileCSec);
  // default ressource name: relative path
  ...
  // log
  LogSilentF("Network: Resource: complete %d:%s is file %s (%s)", iResID,
             sResName.getData(), szFile, fTemp ? "temp" : "static");
#endif
  // set data
  fDirty = true;
  fTempFile = fTemp;
...
bool C4Network2Res::SetByCore(const C4Network2ResCore &nCore, bool fSilent,
                              const char *szAsFilename,
                              int32_t iRecursion)  // by main thread
{
  StdStrBuf sFilename;
  // try open local file
  const char *szFilename =
      szAsFilename ? szAsFilename : GetC4Filename(nCore.getFileName());
  if (SetByFile(szFilename, false, nCore.getType(), nCore.getID(),
  ...
  // get and search for filename without specified folder (e.g., Castle.c4s when
  // the opened game is Easy.c4f\Castle.c4s)
  ...
  if (szFilenameOnly != szFilenameC4) {
    ...
    if (SetByCore(nCore, fSilent, szFilenameOnly,
                  Config.Network.MaxResSearchRecursion))
      return true;
  }
  // if it could still not be set, try within all folders of root (ignoring
  // special folders), and try as file outside the folder
  ...
  StdStrBuf sNetPath;
  sNetPath.Copy(Config.Network.WorkPath);
  ...
  for (DirectoryIterator i(szSearchPath); *i; ++i)
    if (DirectoryExists(*i))
      if (!*GetExtension(*i))  // directories without extension only
        if (!szNetPath || !*szNetPath ||
            !ItemIdentical(*i, szNetPath))  // ignore network path
        {
          // search for complete name at subpath (e.g.
          // MyFolder\Easy.c4f\Castle.c4s)
...
bool C4Network2Res::SetLoad(const C4Network2ResCore &nCore)  // by main thread
{
  ...
  if (!pParent->FindTempResFileName(Core.getFileName(), szFile)) return false;
#ifdef C4NET2RES_DEBUG_LOG
  // log
  Application.InteractiveThread.ThreadLogS(
      "Network: Resource: loading %d:%s to file %s", Core.getID(),
      Core.getFileName(), szFile);
#endif
...
bool C4Network2Res::SetDerived(const char *strName, const char *strFilePath,
                               bool fTemp, C4Network2ResType eType,
                               int32_t iDResID) {
  Clear();
  CStdLock FileLock(&FileCSec);
  // set core
...
void C4Network2Res::ChangeID(int32_t inID) { Core.SetID(inID); }
...
bool C4Network2Res::GetStandalone(char *pTo, int32_t iMaxL, bool fSetOfficial,
                                  bool fAllowUnloadable, bool fSilent) {
  // already set?
  if (szStandalone[0]) {
  ...
  // not loadable? Wo won't be able to check the standalone as the core will
  // lack the needed information.
  ...
  if (DirectoryExists(szFile)) {
    ...
    if (fAllowUnloadable) {
      uint32_t iDirSize;
      if (!DirSizeHelper::GetDirSize(szFile, &iDirSize,
                                     Config.Network.MaxLoadFileSize)) {
        if (!fSilent)
          LogF("Network: could not get directory size of %s!", szFile);
        szStandalone[0] = '\0';
        return false;
      }
      if (iDirSize > uint32_t(Config.Network.MaxLoadFileSize)) {
        if (!fSilent)
          LogSilentF("Network: %s over size limit, will be marked unloadable!",
                     szFile);
        szStandalone[0] = '\0';
        return false;
      }
    ...
    if (!fTempFile) {
      if (!pParent->FindTempResFileName(szFile, szStandalone)) {
        if (!fSilent)
          Log("GetStandalone: could not find free name for temporary file!");
        szStandalone[0] = '\0';
        return false;
      }
      if (!C4Group_PackDirectoryTo(szFile, szStandalone)) {
        if (!fSilent) Log("GetStandalone: could not pack directory!");
        szStandalone[0] = '\0';
        return false;
      }
    ...
    if (DirectoryExists(szStandalone)) {
      if (!fSilent) Log("GetStandalone: directory hasn't been packed!");
      if (!SEqual(szFile, szStandalone)) EraseDirectory(szStandalone);
      szStandalone[0] = '\0';
      return false;
    }
  ...
  if (!FileExists(szStandalone)) {
    ...
    if (!C4Group_CopyItem(szFile, szStandalone)) {
      if (!fSilent) Log("GetStandalone: could not copy to temporary file!");
      szStandalone[0] = '\0';
      return false;
    }
  }
  ...
  if (!FileExists(szStandalone)) {
    if (!fSilent) Log("GetStandalone: file not found!");
    szStandalone[0] = '\0';
    return false;
  }
  ...
  if (!C4Group_GetFileCRC(szStandalone, &iCRC32)) {
    if (!fSilent) Log("GetStandalone: could not calculate checksum!");
    return false;
  }
  // set / check
...
bool C4Network2Res::CalculateSHA() {
  ...
  if (!C4Group_GetFileSHA1(szStandalone, hash)) return false;
  // save it back
...
C4Network2Res::Ref C4Network2Res::Derive() {
  ...
  // For security: This doesn't make much sense if the resource is currently
  // being
  ...
  if (!*szStandalone || SEqual(szStandalone, szFile)) {
    if (!pParent->FindTempResFileName(szOrgFile, szFile)) {
      Log("Derive: could not find free name for temporary file!");
      return NULL;
    }
    if (!C4Group_CopyItem(szOrgFile, szFile)) {
      Log("Derive: could not copy to temporary file!");
      return NULL;
    }
    // set standalone
    if (*szStandalone) SCopy(szFile, szStandalone, _MAX_PATH);
    fTempFile = true;
  } else {
  ...
  Application.InteractiveThread.ThreadLogS(
      "Network: Ressource: deriving from %d:%s, original at %s", getResID(),
      Core.getFileName(), szFile);
  ...
  // initialize
  if (!pDRes->SetDerived(Core.getFileName(), szOrgFile, fOrgTempFile, getType(),
                         getResID()))
    return NULL;
...
bool C4Network2Res::FinishDerive()  // by main thread
{
  ...
  SCopy(Core.getFileName(), szName, _MAX_PATH);
  char szFileC[_MAX_PATH + 1];
  SCopy(szFile, szFileC, _MAX_PATH);
  // Set by file
  if (!SetByFile(szFileC, fTempFile, getType(), pParent->nextResID(), szName))
    return false;
  // create standalone
  if (!GetStandalone(NULL, 0, true)) return false;
  // Set ID
...
bool C4Network2Res::SendStatus(C4Network2IOConnection *pTo) {
  // pack status
  C4NetIOPacket Pkt =
      MkC4NetIOPacket(PID_NetResStat, C4PacketResStatus(Core.getID(), Chunks));
  // to one client?
  if (pTo)
    return pTo->Send(Pkt);
  else {
...
bool C4Network2Res::SendChunk(uint32_t iChunk, int32_t iToClient) {
  ...
  // find connection for given client (one of the rare uses of the data
  // connection)
...
void C4Network2Res::AddRef() { InterlockedIncrement(&iRefCnt); }
...
void C4Network2Res::OnStatus(const C4Network2ResChunkData &rChunkData,
  ...
  if (rChunkData.getChunkCnt() != Chunks.getChunkCnt()) return;
  // add chunk data
  ClientChunks *pChunks;
  for (pChunks = pCChunks; pChunks; pChunks = pChunks->Next)
    if (pChunks->ClientID == pBy->getClientID()) break;
  // not found? add
  if (!pChunks) {
  ...
  // check load
  if (!StartLoad(pChunks->ClientID, pChunks->Chunks)) RemoveCChunks(pCChunks);
}
...
void C4Network2Res::OnChunk(const C4Network2ResChunk &rChunk) {
  ...
  // log
  Application.InteractiveThread.ThreadLogS(
      "Network: Res: %s chunk %d to ressource %s (%s)%s",
      fSuccess ? "added" : "could not add", rChunk.getChunkNr(),
      Core.getFileName(), szFile, fSuccess ? "" : "!");
#endif
  if (fSuccess) {
    // status changed
    fDirty = true;
    // remove load waits
    for (C4Network2ResLoad *pLoad = pLoads, *pNext, *pPrev = NULL; pLoad;
         pPrev = pLoad, pLoad = pNext) {
      pNext = pLoad->Next();
      if (pLoad->getChunk() == rChunk.getChunkNr()) RemoveLoad(pLoad);
    }
  }
  // complete?
  if (Chunks.isComplete()) EndLoad();
...
bool C4Network2Res::NeedsDiscover() {
  ...
  if (fLoading && !iLoadCnt) {
    ...
    if (!iDiscoverStartTime) iDiscoverStartTime = time(NULL);
    // do discover
    return true;
  }
  return false;
  ...
  if (fTempFile)
    if (FileExists(szFile))
      if (remove(szFile))
        ...
        LogSilentF("Network: Could not delete temporary resource file (%s)",
                   strerror(errno));
...
void C4Network2Res::StartNewLoads() {
  if (!pCChunks) return;
  // count clients
  int32_t iCChunkCnt = 0;
  ClientChunks *pChunks;
  for (pChunks = pCChunks; pChunks; pChunks = pChunks->Next) iCChunkCnt++;
  // create array
  ...
  for (pChunks = pCChunks, i = 0; i < iCChunkCnt;
       i++, pChunks = pChunks->Next) {
    // determine position
...
bool C4Network2Res::StartLoad(int32_t iFromClient,
                              const C4Network2ResChunkData &Available) {
  assert(pParent && pParent->getIOClass());
  ...
  for (C4Network2ResLoad *pPos = pLoads; pPos; pPos = pPos->Next())
    if (pPos->getByClient() == iFromClient) return true;
  // find chunk to retrieve
  int32_t iLoads[C4NetResMaxLoad];
  int32_t i = 0;
  ...
  C4Network2IOConnection *pConn =
      pParent->getIOClass()->GetMsgConnection(iFromClient);
  if (!pConn) return false;
  // send request
  if (!pConn->Send(MkC4NetIOPacket(
          PID_NetResReq, C4PacketResRequest(Core.getID(), iRetrieveChunk)))) {
    pConn->DelRef();
    return false;
  }
  pConn->DelRef();
  ...
  // log
  Application.InteractiveThread.ThreadLogS(
      "Network: Res: requesting chunk %d of %d:%s (%s) from client %d",
      iRetrieveChunk, Core.getID(), Core.getFileName(), szFile, iFromClient);
#endif
  // create load class
  C4Network2ResLoad *pnLoad =
      new C4Network2ResLoad(iRetrieveChunk, iFromClient);
  // add to list
  pnLoad->pNext = pLoads;
  pLoads = pnLoad;
  iLoadCnt++;
...
void C4Network2Res::RemoveLoad(C4Network2ResLoad *pLoad) {
  if (pLoad == pLoads)
    ...
    for (pPrev = pLoads; pPrev && pPrev->Next() != pLoad; pPrev = pPrev->Next())
      ;
    // remove
    if (pPrev) pPrev->pNext = pLoad->Next();
  }
  // delete
  delete pLoad;
  iLoadCnt--;
...
void C4Network2Res::RemoveCChunks(ClientChunks *pChunks) {
  if (pChunks == pCChunks)
    ...
    for (pPrev = pCChunks; pPrev && pPrev->Next != pChunks; pPrev = pPrev->Next)
      ;
    // remove
    if (pPrev) pPrev->Next = pChunks->Next;
  }
  // delete
  delete pChunks;
}
  ...
  if (Core.getType() == NRT_Player) {
    ...
    if (!fTempFile && SEqual(szFile, szStandalone)) {
      ...
      if (!pParent->FindTempResFileName(szStandalone, szNewStandalone)) {
        if (!fSilent)
          Log("OptimizeStandalone: could not find free name for temporary "
              "file!");
        return false;
      }
      if (!C4Group_CopyItem(szStandalone, szNewStandalone)) {
        if (!fSilent)
          Log("OptimizeStandalone: could not copy to temporary file!");
        return false;
      } /* TODO: Test failure */
    ...
    if (!Grp.Open(szStandalone)) {
      if (!fSilent) Log("OptimizeStandalone: could not open player file!");
      return false;
    }
    // remove portrais
...
bool C4Network2ResChunk::Set(C4Network2Res *pRes, uint32_t inChunk) {
  ...
  if (iSize < 0) {
    LogF(
        "Network: could not get chunk from offset %d from resource file %s: "
        "File size is only %d!",
        iOffset, pRes->getFile(), Core.getFileSize());
    return false;
  }
  // open file
  int32_t f = pRes->OpenFileRead();
  if (f == -1) {
    LogF("Network: could not open resource file %s!", pRes->getFile());
    return false;
  }
  // seek
  if (iOffset)
    if (lseek(f, iOffset, SEEK_SET) != iOffset) {
      close(f);
      LogF("Network: could not read resource file %s!", pRes->getFile());
      return false;
    }
  // read chunk of data
...
bool C4Network2ResChunk::AddTo(C4Network2Res *pRes, C4Network2IO *pIO) const {
  ...
  if (iResID != pRes->getResID()) {
    ...
    Application.InteractiveThread.ThreadLogS(
        FormatString(
            "C4Network2ResChunk(%d)::AddTo(%s [%d]): Ressource ID mismatch!",
            (int)iResID, (const char *)Core.getFileName(),
            (int)pRes->getResID()).getData());
#endif
    return false;
  }
  // calculate offset and size
  ...
  if (iOffset + Data.getSize() > Core.getFileSize()) {
    ...
    Application.InteractiveThread.ThreadLogS(
        FormatString(
            "C4Network2ResChunk(%d)::AddTo(%s [%d]): Adding %d bytes at offset "
            "%d exceeds expected file size of %d!",
            (int)iResID, (const char *)Core.getFileName(),
            (int)pRes->getResID(), (int)Data.getSize(), (int)iOffset,
            (int)Core.getFileSize()).getData());
#endif
    return false;
  }
  // open file
  ...
  if (f == -1) {
    ...
    Application.InteractiveThread.ThreadLogS(
        FormatString(
            "C4Network2ResChunk(%d)::AddTo(%s [%d]): Open write file error: "
            "%s!",
            (int)iResID, (const char *)Core.getFileName(),
            (int)pRes->getResID(), strerror(errno)).getData());
#endif
    return false;
  }
  // seek
  if (iOffset)
    if (lseek(f, iOffset, SEEK_SET) != iOffset) {
      ...
      Application.InteractiveThread.ThreadLogS(
          FormatString(
              "C4Network2ResChunk(%d)::AddTo(%s [%d]): lseek file error: %s!",
              (int)iResID, (const char *)Core.getFileName(),
              (int)pRes->getResID(), strerror(errno)).getData());
#endif
      close(f);
...
#ifdef C4NET2RES_DEBUG_LOG
    Application.InteractiveThread.ThreadLogS(
        FormatString("C4Network2ResChunk(%d)::AddTo(%s [%d]): write error: %s!",
                     (int)iResID, (const char *)Core.getFileName(),
                     (int)pRes->getResID(), strerror(errno)).getData());
#endif
    close(f);
...
C4Network2ResList::~C4Network2ResList() { Clear(); }

bool C4Network2ResList::Init(int32_t inClientID,
                             C4Network2IO *pIOClass)  // by main thread
{
  // clear old list
  Clear();
  // safe IO class
...
int32_t C4Network2ResList::nextResID()  // by main thread
{
  ...
  // find free
  while (getRes(iNextResID)) iNextResID++;
  return iNextResID++;
}
...
C4Network2Res *C4Network2ResList::getRes(int32_t iResID) {
  ...
  for (C4Network2Res *pCur = pFirst; pCur; pCur = pCur->pNext)
    if (pCur->getResID() == iResID) return pCur;
  return NULL;
}
  ...
  for (C4Network2Res *pCur = pFirst; pCur; pCur = pCur->pNext)
    if (!pCur->isAnonymous())
      if (SEqual(pCur->getFile(), szFile))
        if (!fLocalOnly || pCur->getResClient() == iClientID) return pCur;
  return NULL;
}
...
C4Network2Res::Ref C4Network2ResList::getRefNextRes(int32_t iResID) {
  ...
  for (C4Network2Res *pCur = pFirst; pCur; pCur = pCur->pNext)
    if (!pCur->isRemoved() && pCur->getResID() >= iResID)
      if (!pRes || pRes->getResID() > pCur->getResID()) pRes = pCur;
  return pRes;
}
...
C4Network2Res::Ref C4Network2ResList::AddByFile(
    const char *strFilePath, bool fTemp, C4Network2ResType eType,
    int32_t iResID, const char *szResName, bool fAllowUnloadable) {
  // already in list?
  ...
  if (iResID < 0) {
    Log("AddByFile: no more ressource IDs available!");
    return NULL;
  }
  // create new
  pRes = new C4Network2Res(this);
  // initialize
  if (!pRes->SetByFile(strFilePath, fTemp, eType, iResID, szResName)) {
    return NULL;
  }
  // create standalone for non-system files
  // system files shouldn't create a standalone; they should never be marked
  // loadable!
  if (eType != NRT_System)
...
C4Network2Res::Ref C4Network2ResList::AddByGroup(C4Group *pGrp, bool fTemp,
  ...
  if (iResID < 0) iResID = nextResID();
  if (iResID < 0) {
    Log("AddByGroup: no more ressource IDs available!");
    return NULL;
  }
  // create new
                                                ...
                                                bool fLoad)  // by main thread
{
  ...
  // log
  Application.InteractiveThread.ThreadLogS(
      "Network: Found identical %s. Not loading.",
...
C4Network2Res::Ref C4Network2ResList::AddLoad(
    const C4Network2ResCore &Core)  // by main thread
{
  // marked unloadable by creator?
  if (!Core.isLoadable()) {
    // show error msg
    Application.InteractiveThread.ThreadLog(
        "Network: Cannot load %s (marked unloadable)", Core.getFileName());
    return NULL;
  }
  // create new
  C4Network2Res::Ref pRes = new C4Network2Res(this);
  ...
  pRes->SetLoad(Core);
  // log
  Application.InteractiveThread.ThreadLogS("Network: loading %s...",
                                           Core.getFileName());
  // add to list
  Add(pRes);
  return pRes;
...
void C4Network2ResList::RemoveAtClient(int32_t iClientID)  // by main thread
{
  CStdShareLock ResListLock(&ResListCSec);
  for (C4Network2Res *pRes = pFirst; pRes; pRes = pRes->pNext)
    if (pRes->getResClient() == iClientID) pRes->Remove();
}
...
void C4Network2ResList::OnClientConnect(
    C4Network2IOConnection *pConn)  // by main thread
{
  // discover ressources
  SendDiscover(pConn);
}

void C4Network2ResList::HandlePacket(char cStatus, const C4PacketBase *pPacket,
                                     C4Network2IOConnection *pConn) {
  // security
  if (!pConn) return;
...
#define GETPKT(type, name) \
  ...
  switch (cStatus) {
    case PID_NetResDis:  // ressource discover
    {
      ...
      for (C4Network2Res *pRes = pFirst; pRes; pRes = pRes->pNext)
        if (Pkt.isIDPresent(pRes->getResID()))
          ...
          if (pRes->IsBinaryCompatible()) pRes->OnDiscover(pConn);
    } break;
    ...
    case PID_NetResStat:  // ressource status
    {
      ...
      if (pRes) pRes->OnStatus(Pkt.getChunks(), pConn);
    } break;
    ...
    case PID_NetResDerive:  // ressource derive
    {
      ...
      for (C4Network2Res *pRes = pFirst; pRes; pRes = pRes->pNext)
        if (pRes->isAnonymous() &&
            pRes->getCore().getDerID() == Core.getDerID())
    ...
    case PID_NetResReq:  // ressource request
    {
      ...
      if (pRes && pRes->IsBinaryCompatible())
        pRes->SendChunk(Pkt.getReqChunk(), pConn->getClientID());
    } break;
...
void C4Network2ResList::OnTimer() {
  ...
  for (pRes = pFirst; pRes; pRes = pRes->pNext)
    if (pRes->isLoading() && !pRes->isRemoved())
      if (!pRes->DoLoad()) pRes->Remove();
  // discovery time?
  if (!iLastDiscover ||
      difftime(time(NULL), iLastDiscover) >= C4NetResDiscoverInterval) {
    // needed?
    ...
    // send
    if (fSendDiscover) SendDiscover();
  }
  // status update?
  if (!iLastStatus ||
      difftime(time(NULL), iLastStatus) >= C4NetResStatusInterval) {
    // any?
...
void C4Network2ResList::OnShareFree(CStdCSecEx *pCSec) {
  if (pCSec == &ResListCSec) {
    // remove entries
    for (C4Network2Res *pRes = pFirst, *pNext, *pPrev = NULL; pRes;
         pRes = pNext) {
      pNext = pRes->pNext;
      if (pRes->isRemoved() &&
          (!pRes->getLastReqTime() ||
           difftime(time(NULL), pRes->getLastReqTime()) > C4NetResDeleteTime)) {
        // unlink
...
bool C4Network2ResList::SendDiscover(C4Network2IOConnection *pTo)  // by both
{
  ...
  for (C4Network2Res *pRes = pFirst; pRes; pRes = pRes->pNext)
    if (!pRes->isRemoved())
      if (pRes->isLoading()) Pkt.AddDisID(pRes->getResID());
  ResListLock.Clear();
  // empty?
...
void C4Network2ResList::OnResComplete(C4Network2Res *pRes) {
  ...
  Application.InteractiveThread.ThreadLogS("Network: %s received.",
                                           pRes->getCore().getFileName());
...
bool C4Network2ResList::CreateNetworkFolder() {
  ...
  if (access(szNetworkPath, 00)) {
    if (!CreateDirectory(szNetworkPath, 0)) {
      LogFatal("Network: could not create network path!");
      return false;
    }
    return true;
  }
  ...
  if (stat(szNetworkPath, &s)) {
    LogFatal("Network: could not stat network path!");
    return false;
  }
  // not a subdir?
  if (!(s.st_mode & S_IFDIR)) {
    LogFatal("Network: could not create network path: blocked by a file!");
    return false;
  }
  // ok
  return true;
...
bool C4Network2ResList::FindTempResFileName(const char *szFilename,
  ...
  while (*szFilename) {
    if ((*szFilename >= 'a' && *szFilename <= 'z') ||
        ...
        (*szFilename >= '0' && *szFilename <= '9') || (*szFilename == '.') ||
        (*szFilename == '/'))
      *safePos = *szFilename;
    else
      *safePos = '_';
