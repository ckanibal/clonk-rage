...
C4TableGraph::C4TableGraph(int iBackLogLength, int iStartTime)
    : iBackLogLength(iBackLogLength),
      pValues(NULL),
      iBackLogPos(0),
      fWrapped(false),
      iTime(iStartTime),
      iInitialStartTime(iStartTime),
      pAveragedValues(NULL),
      iAveragedTime(iStartTime),
      iAvgRange(1),
      fMultiplier(1) {
  // create value buffer
...
C4Graph::ValueType C4TableGraph::GetMedianValue(TimeType iStartTime,
                                                TimeType iEndTime) const {
  assert(iStartTime < iEndTime);
  // safety: Never build median if no values are recorded
  if (!iBackLogPos && !fWrapped) return 0;
  // sum up and divide in the end - let's hope this will never be called for
  // really large values that could overflow ValueType
...
void C4TableGraph::Update() const {
  ...
  int iAvgFwRange = 0;
#endif
  for (int iUpdateTime = Max(iAveragedTime - iAvgFwRange - 1, iStartTime);
       iUpdateTime < iTime; ++iUpdateTime) {
    ValueType iSum = 0, iSumWeight = 0, iWeight;
    for (int iSumTime = Max(iUpdateTime - iAvgRange, iStartTime);
         iSumTime < Min(iUpdateTime + iAvgFwRange + 1, iTime); ++iSumTime) {
...
void C4Network2Stats::ExecuteControlFrame() {
  ...
  statControls.SetMultiplier((C4Graph::ValueType)1000 / 38 /
                             Game.Control.ControlRate);
  statActions.SetMultiplier((C4Graph::ValueType)1000 / 38 * 60 /
                            Game.Control.ControlRate);
