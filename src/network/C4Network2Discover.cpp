...
void C4Network2IODiscover::OnPacket(const class C4NetIOPacket &rPacket,
                                    C4NetIO *pNetIO) {
  // discovery?
...
bool C4Network2IODiscover::Init(uint16_t iPort) {
  ...
  if (!C4NetIOSimpleUDP::Init(iPort)) return false;
  // Set callback
  ...
  if (!C4NetIOSimpleUDP::InitBroadcast(&DiscoveryAddr)) return false;
...
void C4Network2IODiscoverClient::OnPacket(const class C4NetIOPacket &rPacket,
                                          C4NetIO *pNetIO) {
  // discovery?
  if (rPacket.getSize() == sizeof(C4Network2IODiscoverReply) &&
      rPacket.getStatus() == 4) {
    // save discovered address
    if (iDiscoverCount < C4NetMaxDiscover) {
      const C4Network2IODiscoverReply *pReply =
          reinterpret_cast<const C4Network2IODiscoverReply *>(
              rPacket.getData());
...
bool C4Network2IODiscoverClient::PopDiscover(C4NetIO::addr_t &Discover) {
  // Discovers left?
  if (!getDiscoverCount()) return false;
  // Return one
