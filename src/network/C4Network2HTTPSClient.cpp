...
C4Network2HTTPSClient::C4Network2HTTPSClient() : C4Network2HTTPClient() {
	...
	x509_crt_parse(&cacert, (const unsigned char *)test_ca_list, strlen(test_ca_list));
...
bool C4Network2HTTPSClient::Connect(const addr_t &addr) {
	int ret = net_connect(&server_fd, inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
	...
	if ((ret = ssl_get_verify_result(&ssl)) != 0)
	{
		...
		SetError(FormatString("SSL verify failed: bad certificate (%s)!", reason).getData());
		return true;
//		return false;
	}
...
bool C4Network2HTTPSClient::Send(const C4NetIOPacket &rPacket) {
	return (ssl_write(&ssl, (const unsigned char*)rPacket.getPData(), rPacket.getSize()) == 0);
}
...
int C4Network2HTTPSClient::my_verify(void *ctx, x509_crt *cert, int depth, int *flags) {
	CERT_CHAIN_PARA ChainPara;
	PCCERT_CHAIN_CONTEXT pChainContext = NULL;

	PCCERT_CONTEXT pccert_ctx = CertCreateCertificateContext(X509_ASN_ENCODING, cert->raw.p, cert->raw.len);
