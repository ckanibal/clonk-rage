...
#define C4League_Name_Valid_Characters                                         \
  "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ\xC0\xC1\xC2" \
  "\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF\xD0\xD1\xD2\xD3\xD4"   \
  "\xD5\xD6\xD9\xDA\xDB\xDC\xDD\xDF\xE0\xE1\xE2\xE3\xE4\xE5\xE6\xE7\xE8\xE9"   \
  "\xEA\xEB\xEC\xED\xEE\xEF\xF0\xF1\xF2\xF3\xF4\xF5\xF6\xF8\xF9\xFA\xFB\xFC"   \
  "\xFD\xFE\xFF\x20\x2E\x2D\x5F"
...
enum C4LeagueAction {
  ...
  C4LA_ReportDisconnect,  // [both] Sent by host and client when a client is
                          // disconnected irregularly
};

class C4LeagueRequestHead {
 public:
  C4LeagueRequestHead(C4LeagueAction eAction, const char *szCSID = "",
                      const char *szAUID = "")
...
class C4LeagueReportDisconnectHead : public C4LeagueRequestHead {
 ...
 public:
  C4LeagueReportDisconnectHead(const char *szCSID,
                               C4LeagueDisconnectReason eReason)
      : eReason(eReason),
        C4LeagueRequestHead(C4LA_ReportDisconnect, szCSID, NULL) {}
...
class C4LeagueRequestHeadEnd : public C4LeagueRequestHead {
 public:
  C4LeagueRequestHeadEnd(C4LeagueAction eAction, const char *szCSID,
                         const char *szRecordName = NULL,
                         const BYTE *pRecordSHA = NULL)
      : C4LeagueRequestHead(eAction, szCSID), RecordName(szRecordName) {
    if (pRecordSHA) memcpy(RecordSHA, pRecordSHA, CryptoPP::SHA1::DIGESTSIZE);
  }

 private:
  StdCopyStrBuf RecordName;
  uint8_t RecordSHA[CryptoPP::SHA1::DIGESTSIZE];
...
class C4LeagueResponseHead {
 ...
 public:
  ...
  bool isStatusRegister() const {
    return SEqualNoCase(Status.getData(), "Register");
  }
...
class C4LeagueClient : public C4Network2RefClient {
 ...
 public:
  ...
  bool GetStartReply(StdStrBuf *pMessage, StdStrBuf *pLeague,
                     StdStrBuf *pStreamingAddr, int32_t *pSeed,
                     int32_t *pMaxPlayers);

  // Action "Update"
  bool Update(const C4Network2Reference &Ref);
  bool GetUpdateReply(StdStrBuf *pMessage,
                      C4ClientPlayerInfos *pPlayerLeagueInfos);

  // Action "End"
  bool End(const C4Network2Reference &Ref, const char *szRecordName,
           const BYTE *pRecordSHA);
  bool GetEndReply(StdStrBuf *pMessage,
                   class C4RoundResultsPlayers *pRoundResults);

  // Action "Auth"
  bool Auth(const C4PlayerInfo &PlrInfo, const char *szAccount,
            const char *szPassword, const char *szNewAccount = NULL,
            const char *szNewPassword = NULL);
  bool GetAuthReply(StdStrBuf *pMessage, StdStrBuf *pAUID, StdStrBuf *pAccount,
                    bool *pRegister);

  // Action "Join"
  bool AuthCheck(const C4PlayerInfo &PlrInfo);
  bool GetAuthCheckReply(StdStrBuf *pMessage, const char *szLeague,
                         class C4PlayerInfo *pPlrInfo);

  // Action "Disconnect" - sent by host and client when one or more clients are
  // disconnected irregularly
  bool ReportDisconnect(const C4ClientPlayerInfos &rSendPlayerFBIDs,
                        C4LeagueDisconnectReason eReason);
  bool GetReportDisconnectReply(StdStrBuf *pMessage);

 private:
  void ModifyForChecksum(StdStrBuf *pData, const char *pReplace);
  void ModifyForChecksum(const void *pData, size_t iDataSize, char *pReplace,
                         uint32_t iChecksum, uint32_t iCheckMask);
};

// dialog shown for each participating player to enter league authentication
// data
class C4LeagueSignupDialog : public C4GUI::Dialog {
 ...
 public:
  C4LeagueSignupDialog(const char *szPlayerName, const char *szLeagueName,
                       const char *szLeagueServerName,
                       const char *szAccountPref, const char *szPassPref,
  ...
  static bool ShowModal(const char *szPlayerName, const char *szLeagueName,
                        const char *szLeagueServerName, StdStrBuf *psAccount,
                        StdStrBuf *psPass, bool fWarnThirdParty,
                        bool fRegister);
