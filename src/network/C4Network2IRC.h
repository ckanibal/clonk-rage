...
const int C4NetIRCMaxLogLength =
    300000;  // Maximum total length of log - if log gets longer, it will be
             // cleared even if messages have never been read
const int C4NetIRCMaxReadLogLength =
    1000;  // Maximum log messages kept even after they've been read

class C4Network2IRCMessage {
  friend class C4Network2IRCClient;

 public:
  C4Network2IRCMessage(C4Network2IRCMessageType enType, const char *szSource,
                       const char *szTarget, const char *szData)
      : eType(enType),
        Source(szSource),
        Target(szTarget),
        Data(szData),
...
class C4Network2IRCClient : public C4NetIOTCP, private C4NetIO::CBClass {
 ...
 private:
  ...
  bool OnConn(const C4NetIO::addr_t &AddrPeer,
              const C4NetIO::addr_t &AddrConnect, const addr_t *pOwnAddr,
              C4NetIO *pNetIO);
 ...
 public:
  ...
  C4Network2IRCMessage *getUnreadMessageLog() const {
    return pLogLastRead ? pLogLastRead->getNext() : pLog;
  }
  ...
  bool Connect(const char *szServer, const char *szNick, const char *szRealName,
               const char *szPassword = NULL, const char *szChannel = NULL);
  bool Close();
 ...
 private:
  void OnCommand(const char *szSender, const char *szCommand,
                 const char *szParameters);
  void OnNumericCommand(const char *szSender, int iCommand,
                        const char *szParameters);
  void OnConnected();
  void OnMessage(bool fNotice, const char *szSource, const char *szTarget,
                 const char *szText);

  void PopMessage();
  void PushMessage(C4Network2IRCMessageType eType, const char *szSource,
