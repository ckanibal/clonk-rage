...
StdStrBuf C4Network2Address::toString() const {
  switch (eProtocol) {
    case P_UDP:
      return FormatString("UDP:%s:%d", inet_ntoa(addr.sin_addr),
                          htons(addr.sin_port));
    case P_TCP:
      return FormatString("TCP:%s:%d", inet_ntoa(addr.sin_addr),
                          htons(addr.sin_port));
  }
...
bool C4Network2Client::DoConnectAttempt(C4Network2IO *pIO) {
  ...
  if (getMsgConn() != getDataConn()) {
    iNextConnAttempt = time(NULL) + 10;
    return true;
  }
  // too early?
  ...
  if (iBestAddress < 0 ||
      AddrAttempts[iBestAddress] > C4NetClientConnectAttempts) {
    iNextConnAttempt = time(NULL) + 10;
    return true;
  }
  // save attempt
  AddrAttempts[iBestAddress]++;
  iNextConnAttempt = time(NULL) + C4NetClientConnectInterval;
  // log
  LogSilentF("Network: connecting client %s on %s...", getName(),
             Addr[iBestAddress].toString().getData());
  // connect
  return pIO->Connect(Addr[iBestAddress].getAddr(),
                      Addr[iBestAddress].getProtocol(), pClient->getCore());
}

bool C4Network2Client::hasAddr(const C4Network2Address &addr) const {
  for (int32_t i = 0; i < iAddrCnt; i++)
    if (Addr[i] == addr) return true;
  return false;
}

void C4Network2Client::ClearAddr() { iAddrCnt = 0; }
...
bool C4Network2Client::AddAddr(const C4Network2Address &addr, bool fAnnounce) {
  ...
  // add
  Addr[iAddrCnt] = addr;
  AddrAttempts[iAddrCnt] = 0;
  iAddrCnt++;
  // attempt to use this one
  if (!iNextConnAttempt) iNextConnAttempt = time(NULL);
  // announce
  if (fAnnounce)
    if (!pParent->BroadcastMsgToConnClients(
            MkC4NetIOPacket(PID_Addr, C4PacketAddr(getID(), addr))))
      return false;
  // done
  return true;
}
...
void C4Network2Client::CreateGraphs() {
  ...
  ClearGraphs();
  // get client color
  static const DWORD ClientDefColors[] = {
      0xff0000, 0x00ff00, 0xffff00, 0x7f7fff, 0xffffff, 0x00ffff,
      0xff00ff, 0x7f7f7f, 0xff7f7f, 0x7fff7f, 0x0000ff};
  int32_t iClientColorNum = sizeof(ClientDefColors) / sizeof(DWORD);
  DWORD dwClientClr =
      ClientDefColors[Max<int32_t>(getID(), 0) % iClientColorNum];
  // create graphs
  pstatPing = new C4TableGraph(
      C4TableGraph::DefaultBlockLength,
      Game.pNetworkStatistics ? Game.pNetworkStatistics->SecondCounter : 0);
  pstatPing->SetColorDw(dwClientClr);
  pstatPing->SetTitle(getName());
  // register into stat module
  if (Game.pNetworkStatistics)
    Game.pNetworkStatistics->statPings.AddGraph(pstatPing);
}
...
C4Network2ClientList::~C4Network2ClientList() { Clear(); }
...
C4Network2Client *C4Network2ClientList::GetClientByID(int32_t iID) const {
  for (C4Network2Client *pClient = pFirst; pClient; pClient = pClient->pNext)
    if (pClient->getID() == iID) return pClient;
  return NULL;
}
...
C4Network2Client *C4Network2ClientList::GetClient(const char *szName) const {
  for (C4Network2Client *pClient = pFirst; pClient; pClient = pClient->pNext)
    if (SEqual(pClient->getName(), szName)) return pClient;
  return NULL;
}

C4Network2Client *C4Network2ClientList::GetClient(
    C4Network2IOConnection *pConn) const {
  for (C4Network2Client *pClient = pFirst; pClient; pClient = pClient->pNext)
    if (pClient->hasConn(pConn)) return pClient;
  return NULL;
}

C4Network2Client *C4Network2ClientList::GetClient(const C4ClientCore &CCore,
                                                  int32_t iMaxDiffLevel) {
  for (C4Network2Client *pClient = pFirst; pClient; pClient = pClient->pNext)
    if (pClient->getCore().getDiffLevel(CCore) <= iMaxDiffLevel) return pClient;
  return NULL;
}
...
C4Network2Client *C4Network2ClientList::GetNextClient(
...
C4Network2Client *C4Network2ClientList::RegClient(C4Client *pClient) {
  // security
  if (pClient->getNetClient()) return pClient->getNetClient();
  // find insert position
  C4Network2Client *pPos = pFirst, *pLast = NULL;
  for (; pPos; pLast = pPos, pPos = pPos->getNext())
    if (pPos->getID() > pClient->getID()) break;
...
bool C4Network2ClientList::BroadcastMsgToConnClients(
    const C4NetIOPacket &rPkt) {
  ...
  for (C4Network2Client *pClient = pFirst; pClient;
    ...
    if (pClient->isConnected()) pClient->getMsgConn()->SetBroadcastTarget(true);
  // broadcast
...
bool C4Network2ClientList::SendMsgToClient(int32_t iClient,
  ...
  if (pClient->isConnected()) return pClient->SendMsg(rPkt);
  // forward
  C4PacketFwd Fwd;
  Fwd.SetListType(false);
...
void C4Network2ClientList::ResetReady() {
  for (C4Network2Client *pClient = pFirst; pClient;
       pClient = pClient->getNext())
    if (pClient->isWaitedFor()) pClient->SetStatus(NCS_NotReady);
}
