...
class C4Graph {
 ...
 public:
  ...
  // retrieve values within backlog timeframe - guarantueed to be working inside
  // [GetStartTime(), GetEndTime()[
  virtual ValueType GetValue(TimeType iAtTime) const = 0;
  virtual ValueType GetMedianValue(TimeType iStartTime, TimeType iEndTime)
      const = 0;  // median within [iStartTime, iEndTime[
...
class C4TableGraph : public C4Graph {
 private:
  ...
  mutable ValueType *
      pAveragedValues;  // equals pValues if no average is being calculated
  ...
  TimeType iInitialStartTime;  // initial table start time; used to calc offset
                               // in buffer
  TimeType iTime;              // next timeframe to be recorded
  mutable TimeType
      iAveragedTime;     // last timeframe for which average has been calculated
 ...
 public:
  ...
  C4TableGraph(int iBackLogLength = DefaultBlockLength,
  ...
  virtual ValueType GetValue(
      TimeType iAtTime) const;                   // retrieve averaged value!
  ValueType GetAtValue(TimeType iAtTime) const;  // retrieve not-averaged value
  void SetAvgValue(TimeType iAtTime,
                   ValueType iValue) const;  // overwrite avg value at time -
                                             // considered const because it
                                             // modifies mutable only
  virtual ValueType GetMedianValue(TimeType iStartTime, TimeType iEndTime)
      const;  // median within [iStartTime, iEndTime[
  ...
  virtual const C4Graph *GetSeries(int iIndex) const {
    return iIndex ? NULL : this;
  }

  virtual void SetAverageTime(int iToTime);
  virtual void Update()
      const;  // make sure average times are correctly calculated
...
class C4GraphCollection : public C4Graph, private std::vector<C4Graph *> {
 ...
 public:
  ...
  virtual C4Graph::ValueType GetMedianValue(C4Graph::TimeType iStartTime,
                                            C4Graph::TimeType iEndTime) const {
    assert(0);
    return 0;
  }
  ...
  void AddGraph(C4Graph *pAdd) {
    push_back(pAdd);
    if (iCommonAvgTime) pAdd->SetAverageTime(iCommonAvgTime);
    if (fMultiplier) pAdd->SetMultiplier(fMultiplier);
  }
  void RemoveGraph(const C4Graph *pRemove) {
    iterator i = std::find(begin(), end(), pRemove);
    if (i != end()) erase(i);
  }
...
class C4Network2Stats : private C4Sec1Callback {
 ...
 protected:
  ...
  int ControlCounter;  // control frames passed in measured time by network
                       // stats module
