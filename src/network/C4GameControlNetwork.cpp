...
C4GameControlNetwork::~C4GameControlNetwork() { Clear(); }

bool C4GameControlNetwork::Init(int32_t inClientID, bool fnHost,
                                int32_t iStartTick, bool fnActivated,
                                C4Network2 *pnNetwork)  // by main thread
{
  if (IsEnabled()) Clear();
  // init
  ...
  pNetwork->Clients.BroadcastMsgToConnClients(
      MkC4NetIOPacket(PID_ControlReq, C4PacketControlReq(iControlReady + 1)));
  // ok
  fEnabled = true;
  fRunning = false;
  iTargetFPS = 38;
...
{
  // Control ticks only
  if (Game.FrameCounter % Game.Control.ControlRate) return;

  // Save time the control tick was reached
  if (iWaitStart == -1) iWaitStart = timeGetTime();
...
int32_t C4GameControlNetwork::ClientPerfStat(
    int32_t iClientID)  // by main thread
{
  if (eMode == CNM_Central && !fHost) return true;
  // get client
...
int32_t C4GameControlNetwork::ClientNextControl(
    int32_t iClientID)  // by main thread
{
  // get client
  CStdLock ClientsLock(&ClientsCSec);
...
void C4GameControlNetwork::DoInput(const C4Control &Input)  // by main thread
{
  ...
  // client in central or async mode: send to host (will resend it to the other
  // clients)
    ...
    if (!fHost)
      if (!pNetwork->Clients.SendMsgToHost(CtrlPkt))
        Application.InteractiveThread.ThreadLog(
            "Failed to send control to host!");
  }
...
void C4GameControlNetwork::DoInput(
    C4PacketType eCtrlType, C4ControlPacket *pCtrl,
    C4ControlDeliveryType eDelivery)  // by main thread
{
  if (!fEnabled) return;
  ...
  switch (eDelivery) {
    ...
    case CDT_Sync: {
      if (!fHost) {
        // Client: send to host
        if (!Game.Network.Clients.SendMsgToHost(
                MkC4NetIOPacket(PID_ControlPkt, CtrlPkt))) {
          LogFatal("Network: could not send direct control packet!");
          break;
        }
        delete pCtrl;
      } else {
        ...
        if (Game.Network.isFrozen()) {
          ...
          Game.Network.Clients.BroadcastMsgToClients(
              MkC4NetIOPacket(PID_ExecSyncCtrl, Pkt));
        } else {
          // Safe back otherwise
    ...
    case CDT_Private: {
      // Send to all clients
      if (!Game.Network.Clients.BroadcastMsgToClients(
              MkC4NetIOPacket(PID_ControlPkt, CtrlPkt))) {
        LogFatal("Network: could not send direct control packet!");
        break;
      }
    ...
    // Only some delivery types support single packets (queue control must be
    // tick-stamped)
    default:
      assert(false);
  }
...
C4ControlDeliveryType C4GameControlNetwork::DecideControlDelivery() const {
  ...
  if (!fHost) return CDT_Queue;
  // Decide the fastest control delivery type atm. Note this is a guess.
  // Control sent with the returned delivery type may in theory be delayed
  // infinitely.
  if (Game.Network.isFrozen()) return CDT_Sync;
  if (!Game.Clients.getLocal()->isActivated()) return CDT_Sync;
  return CDT_Queue;
}
...
void C4GameControlNetwork::ExecSyncControl()  // by main thread
{
  ...
  // This is a callback from C4Network informing that a point where accumulated
  // sync control
  // can be executed has been reached (it's "momentarily" safe to execute)

  // Nothing to do? Save some sweat.
  if (!SyncControl.firstPkt()) return;
...
void C4GameControlNetwork::ExecSyncControl(
    int32_t iControlTick)  // by main thread
{
  // Nothing to do?
  if (!SyncControl.firstPkt()) return;
  ...
  if (pParent->ControlTick == iControlTick)
    ...
    LogF(
        "Network: Fatal: got sync control to execute for ctrl tick %d, but "
        "already in ctrl tick %d!",
        iControlTick, pParent->ControlTick);
...
void C4GameControlNetwork::AddClient(int32_t iClientID,
                                     const char *szName)  // by main thread
{
  // security
...
void C4GameControlNetwork::ClearClients() {
  CStdLock ClientsLock(&ClientsCSec);
  while (pClients) {
    C4GameControlClient *pClient = pClients;
...
void C4GameControlNetwork::CopyClientList(const C4ClientList &rClients) {
  ...
  while (pClient = rClients.getClient(pClient))
    if (pClient->isActivated()) AddClient(pClient->getID(), pClient->getName());
}

void C4GameControlNetwork::SetRunning(bool fnRunning,
                                      int32_t inTargetTick)  // by main thread
{
  assert(fEnabled);
...
void C4GameControlNetwork::SetActivated(bool fnActivated)  // by main thread
{
  ...
  if (fActivated) iControlSent = Game.Control.getNextControlTick() - 1;
}

void C4GameControlNetwork::SetCtrlMode(
    C4GameControlNetworkMode enMode)  // by main thread
{
  assert(fEnabled);
  // no change?
  ...
  if (enMode == CNM_Decentral) {
    CStdLock CtrlLock(&CtrlCSec);
    C4GameControlPacket *pPkt;
    for (int32_t iCtrlTick = Game.Control.ControlTick;
         pPkt = getCtrl(iClientID, iCtrlTick); iCtrlTick++)
      Game.Network.Clients.BroadcastMsgToClients(
          MkC4NetIOPacket(PID_Control, *pPkt));
  } else if (enMode == CNM_Central && fHost) {
    CStdLock CtrlLock(&CtrlCSec);
    C4GameControlPacket *pPkt;
    for (int32_t iCtrlTick = Game.Control.ControlTick;
         pPkt = getCtrl(C4ClientIDAll, iCtrlTick); iCtrlTick++)
      Game.Network.Clients.BroadcastMsgToClients(
          MkC4NetIOPacket(PID_Control, *pPkt));
  }
}
...
void C4GameControlNetwork::CalcPerformance(int32_t iCtrlTick) {
  ...
  int32_t iClientsPing = 0;
  int32_t iPingClientCount = 0;
  int32_t iNumTunnels = 0;
  int32_t iHostPing = 0;
  for (C4GameControlClient *pClient = pClients; pClient;
       pClient = pClient->pNext) {
    // Some rudimentary PreSend-calculation
    // get associated connection - NULL for self
    C4Network2Client *pNetClt =
        Game.Network.Clients.GetClientByID(pClient->getClientID());
  ...
  if (eMode == CNM_Decentral) {
    // average ping time
    iControlSendTime = (iClientsPing + iHostPing * (iNumTunnels + 1)) /
                       (iPingClientCount + iNumTunnels + 1);
  ...
  if (iControlSendTime) {
    iAvgControlSendTime =
        (iAvgControlSendTime * 149 + iControlSendTime * 1000) / 150;
    // now calculate the all-time optimum PreSend there is
    int32_t iBestPreSend =
        BoundBy((iTargetFPS * iAvgControlSendTime) / 1000000 + 1, 1, 15);
    // fixed PreSend?
...
void C4GameControlNetwork::HandleControl(int32_t iByClientID,
  ...
  if (IsEnabled()) CheckCompleteCtrl(true);
...
void C4GameControlNetwork::HandleControlReq(const C4PacketControlReq &rPkt,
  ...
  for (int iTick = rPkt.getCtrlTick();; iTick++) {
    ...
    // send everything we have for this tick (this is an emergency case, so
    // efficiency
...
void C4GameControlNetwork::HandleControlPkt(
    C4PacketType eCtrlType, C4ControlPacket *pCtrl,
    C4ControlDeliveryType eType)  // main thread
{
...
C4GameControlClient *C4GameControlNetwork::getClient(int32_t iID)  // by both
{
  ...
  for (C4GameControlClient *pClient = pClients; pClient;
       pClient = pClient->pNext)
    if (pClient->getClientID() == iID) return pClient;
  return NULL;
}

void C4GameControlNetwork::AddClient(
    C4GameControlClient *pClient)  // by main thread
{
  if (!pClient) return;
  // lock
  CStdLock ClientsLock(&ClientsCSec);
  ...
  for (; pPos; pPrev = pPos, pPos = pPos->pNext)
    if (pPos->getClientID() > pClient->getClientID()) break;
  // insert
  (pPrev ? pPrev->pNext : pClients) = pClient;
  pClient->pNext = pPos;
}

void C4GameControlNetwork::RemoveClient(
    C4GameControlClient *pClient)  // by main thread
{
  // obtain lock
  CStdLock ClientsLock(&ClientsCSec);
  ...
  if (pClient == pClients)
    ...
    for (pPrev = pClients; pPrev && pPrev->pNext; pPrev = pPrev->pNext)
      if (pPrev->pNext == pClient) break;
    if (pPrev && pPrev->pNext == pClient) pPrev->pNext = pClient->pNext;
  }
}

C4GameControlPacket *C4GameControlNetwork::getCtrl(
    int32_t iClientID, int32_t iCtrlTick)  // by both
{
  // lock
  CStdLock CtrlLock(&CtrlCSec);
  // search
...
void C4GameControlNetwork::CheckCompleteCtrl(bool fSetEvent)  // by both
{
  ...
  for (;;) {
    ...
    if (!pComplete) {
      ...
      // (try to) pack
      if (!(pComplete = PackCompleteCtrl(iControlReady + 1))) break;
    }
    // preexecute to check if it's ready for execute
    if (!pComplete->getControl().PreExecute()) break;
  ...
  if (iControlReady < iTargetTick &&
    ...
    Application.InteractiveThread.ThreadLogS(
        "Network: Recovering: Requesting control for tick %d...",
        iControlReady + 1);
    // make request
    C4NetIOPacket Pkt =
        MkC4NetIOPacket(PID_ControlReq, C4PacketControlReq(iControlReady + 1));
...
C4GameControlPacket *C4GameControlNetwork::PackCompleteCtrl(int32_t iTick) {
  ...
  for (pClient = pClients; pClient; pClient = pClient->pNext)
    if (!getCtrl(pClient->getClientID(), iTick)) break;
  if (pClient) {
    // async mode: wait n extra frames for slow clients
    const int iMaxWait = Game.Control.ControlRate *
                         (Config.Network.AsyncMaxWait * 1000) / iTargetFPS;
    if (eMode != CNM_Async || iWaitStart == -1 ||
        timeGetTime() <= iWaitStart + iMaxWait)
      return NULL;
  }
  ...
  if (eMode != CNM_Decentral)
    Game.Network.Clients.BroadcastMsgToConnClients(
        MkC4NetIOPacket(PID_Control, *pComplete));

  // advance control request time
  iNextControlReqeust = Max<uint32_t>(iNextControlReqeust,
                                      timeGetTime() + C4ControlRequestInterval);
                                              ...
                                              int32_t iTick)  // by main thread
{
  ...
  // behind packets for the same tick, so they will be executed in the right
  // order.
...
void C4GameControlNetwork::ExecQueuedSyncCtrl()  // by main thread
{
  // security
  while (pSyncCtrlQueue &&
         pSyncCtrlQueue->getCtrlTick() < pParent->ControlTick) {
    LogF(
        "Network: Fatal: got sync control to execute for ctrl tick %d, but "
        "already in ctrl tick %d!",
        pSyncCtrlQueue->getCtrlTick(), pParent->ControlTick);
...
C4GameControlPacket &C4GameControlPacket::operator=(
    const C4GameControlPacket &Pkt2) {
...
int32_t C4GameControlClient::getPerfStat() const { return iPerformance / 100; }
