...
class C4Network2Address {
 public:
  C4Network2Address() : eProtocol(P_NONE) { ZeroMem(&addr, sizeof(addr)); }
...
class C4Network2Client {
 ...
 public:
  ...
  bool isWaitedFor() const {
    return getStatus() == NCS_NotReady || getStatus() == NCS_Ready;
  }
...
class C4Network2ClientList {
 ...
 public:
  ...
  C4Network2Client *GetClient(const C4ClientCore &CCore,
