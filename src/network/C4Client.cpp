...
C4ClientCore::C4ClientCore() : iID(-1), fActivated(false), fObserver(false) {
  Name.Ref("");
  CUID.Ref(""), Nick.Ref("");
}
...
C4Client::C4Client() : pNetClient(NULL) {}
...
void C4Client::SetActivated(bool fnActivated) {
  ...
  // activity
  if (fnActivated && pNetClient) pNetClient->SetLastActivity(Game.FrameCounter);
}
...
void C4Client::CompileFunc(StdCompiler *pComp) {
  ...
  if (pComp->isCompiler()) fLocal = false;
}

// *** C4ClientList

C4ClientList::C4ClientList() : pFirst(NULL), pLocal(NULL), pNetClients(NULL) {}

C4ClientList::~C4ClientList() { Clear(); }
...
void C4ClientList::Add(C4Client *pClient) {
  ...
  // register to network
  if (pNetClients) pClient->pNetClient = pNetClients->RegClient(pClient);
}
...
C4Client *C4ClientList::getNextClientByID(int32_t iAfterID) const {
  for (C4Client *pClient = pFirst; pClient; pClient = pClient->pNext)
    if (pClient->getID() > iAfterID) return pClient;
  return NULL;
}
...
int32_t C4ClientList::getClientCnt() const {
  int32_t iCnt = 0;
  for (C4Client *pClient = pFirst; pClient; pClient = pClient->pNext) iCnt++;
  return iCnt;
}
...
C4Client *C4ClientList::Add(const C4ClientCore &Core) {
  ...
  if (getClientByID(Core.getID())) {
    Log("ClientList: Duplicated client ID!");
    return NULL;
  }
  // create, add
...
void C4ClientList::CtrlRemove(const C4Client *pClient, const char *szReason) {
  ...
  // Net client? flag
  if (pClient->getNetClient()) pClient->getNetClient()->SetStatus(NCS_Remove);
  // add control
