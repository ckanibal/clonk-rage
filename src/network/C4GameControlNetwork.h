...
class C4GameControlNetwork  // run by network thread
 ...
 public:
  ...
  void setControlPreSend(int32_t iToVal) {
    iControlPreSend = Min(iToVal, C4MaxPreSend);
  }
  int32_t getAvgControlSendTime() const { return iAvgControlSendTime; }
  void setTargetFPS(int32_t iToVal) { iTargetFPS = iToVal; }

  // main thread communication
  bool Init(int32_t iClientID, bool fHost, int32_t iStartTick, bool fActivated,
            C4Network2 *pNetwork);  // by main thread
  ...
  bool CtrlOverflow(int32_t iTick) const {
    return fRunning && iControlReady >= iTick + C4ControlOverflowLimit;
  }  // by main thread
  int32_t GetBehind(int32_t iTick) const {
    return iControlReady - iTick + 1;
  }                                                    // by main thread
  ...
  void DoInput(C4PacketType eCtrlType, C4ControlPacket *pPkt,
               enum C4ControlDeliveryType eType);  // by main thread
 ...
 protected:
  ...
  void HandleControlReq(const C4PacketControlReq &rPkt,
                        C4Network2IOConnection *pConn);
  void HandleControlPkt(C4PacketType eCtrlType, C4ControlPacket *pPkt,
  ...
  // control stack
  C4GameControlPacket *getCtrl(int32_t iClientID,
                               int32_t iCtrlTick);  // by both
  ...
  // sync control
  void AddSyncCtrlToQueue(const C4Control &Ctrl,
...
class C4PacketControlPkt : public C4PacketBase {
 public:
  C4PacketControlPkt() {}
  C4PacketControlPkt(enum C4ControlDeliveryType eDelivery,
                     const C4IDPacket &Ctrl)
...
class C4PacketExecSyncCtrl : public C4PacketBase {
 ...
 public:
  ...
  virtual void CompileFunc(StdCompiler *pComp) {
    pComp->Value(mkNamingAdapt(iControlTick, "ControlTick", -1));
  }
};
