...
// network ressource: data needed for the game (scenario, plr files,
// definitions...)
...
class C4Network2ResCore : public C4PacketBase {
 ...
 protected:
  ...
  uint8_t fHasFileSHA;
  uint8_t FileSHA[CryptoPP::SHA1::DIGESTSIZE];
  uint32_t iChunkSize;
 ...
 public:
  ...
  uint32_t getChunkCnt() const {
    return iFileSize && iChunkSize ? (iFileSize - 1) / iChunkSize + 1 : 0;
  }

  void Set(C4Network2ResType eType, int32_t iResID, const char *strFileName,
           uint32_t iContentsCRC, const char *szAutor);
  ...
  void SetFileSHA(BYTE *pSHA) {
    memcpy(FileSHA, pSHA, CryptoPP::SHA1::DIGESTSIZE);
    fHasFileSHA = true;
  }
  void Clear();
...
class C4Network2ResChunkData : public C4PacketBase {
 ...
 public:
  ...
  int32_t getPresentPercent() const {
    return iPresentChunkCnt * 100 / iChunkCnt;
  }
  ...
  int32_t GetChunkToRetrieve(const C4Network2ResChunkData &Available,
                             int32_t iLoadingCnt, int32_t *pLoading) const;
...
class C4Network2Res {
 ...
 public:
  ...
  class Ref {
   public:
    Ref() : pRes(NULL) {}
    Ref(C4Network2Res *pRes) : pRes(pRes) {
      if (pRes) pRes->AddRef();
    }
    Ref(const Ref &rCopy) : pRes(rCopy.pRes) {
      if (pRes) pRes->AddRef();
    }
    ~Ref() { Clear(); }
 ...
 public:
  ...
  int32_t getPresentPercent() const {
    return fLoading ? Chunks.getPresentPercent() : 100;
  }

  bool SetByFile(const char *strFilePath, bool fTemp, C4Network2ResType eType,
                 int32_t iResID, const char *szResName = NULL,
                 bool fSilent = false);
  bool SetByGroup(C4Group *pGrp, bool fTemp, C4Network2ResType eType,
                  int32_t iResID, const char *szResName = NULL,
                  bool fSilent = false);
  bool SetByCore(const C4Network2ResCore &nCore, bool fSilent = false,
                 const char *szAsFilename = NULL, int32_t iRecursion = 0);
  bool SetLoad(const C4Network2ResCore &nCore);

  bool SetDerived(const char *strName, const char *strFilePath, bool fTemp,
                  C4Network2ResType eType, int32_t iDResID);

  void ChangeID(int32_t inID);

  bool IsBinaryCompatible();
  bool GetStandalone(char *pTo, int32_t iMaxL, bool fSetOfficial,
                     bool fAllowUnloadable = false, bool fSilent = false);
  bool CalculateSHA();
  ...
  void OnStatus(const C4Network2ResChunkData &rChunkData,
...
class C4Network2ResList : protected CStdCSecExCallback  // run by network thread
 ...
 public:
  ...
  C4Network2Res::Ref getRefRes(const char *szFile,
                               bool fLocalOnly = false);  // by both
  C4Network2Res::Ref getRefNextRes(int32_t iResID);       // by both

  void Add(C4Network2Res *pRes);  // by both
  C4Network2Res::Ref AddByFile(const char *strFilePath, bool fTemp,
                               C4Network2ResType eType, int32_t iResID = -1,
                               const char *szResName = NULL,
                               bool fAllowUnloadable = false);  // by both
  C4Network2Res::Ref AddByGroup(C4Group *pGrp, bool fTemp,
                                C4Network2ResType eType, int32_t iResID = -1,
                                const char *szResName = NULL,
                                bool fAllowUnloadable = false);  // by both
  C4Network2Res::Ref AddByCore(const C4Network2ResCore &Core,
                               bool fLoad = true);            // by main thread
