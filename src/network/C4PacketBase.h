...
class C4PacketBase {
 ...
 public:
  ...
  C4NetIOPacket pack(uint8_t cStatus,
                     const C4NetIO::addr_t &addr = C4NetIO::addr_t()) const;
  void unpack(const C4NetIOPacket &Pkt, char *pStatus = NULL);
};

inline C4NetIOPacket MkC4NetIOPacket(
    char cStatus, const class C4PacketBase &Pkt,
...
inline C4NetFilenameAdapt mkNetFilenameAdapt(StdStrBuf &FileName) {
  return C4NetFilenameAdapt(FileName);
}
