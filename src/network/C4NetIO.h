...
class C4NetIO : public StdSchedulerProc {
 public:
  ...
  class CBClass {
   public:
    virtual bool OnConn(const addr_t &AddrPeer, const addr_t &AddrConnect,
                        const addr_t *pOwnAddr, C4NetIO *pNetIO) {
      return true;
    }
    virtual void OnDisconn(const addr_t &AddrPeer, C4NetIO *pNetIO,
                           const char *szReason) {}
    virtual void OnPacket(const class C4NetIOPacket &rPacket,
                          C4NetIO *pNetIO) = 0;
    virtual ~CBClass() {}
  };
...
#define NETIO_CREATE_CALLBACK_PROXY(ForClass, ProxyName) \
  ...
  virtual bool GetConnStatistic(const addr_t &addr, int *pIRate, int *pORate,
                                int *pLoss) = 0;
...
class C4NetIOPacket : public StdCopyBuf {
 public:
  ...
  C4NetIOPacket(const void *pnData, size_t inSize, bool fCopy = false,
                const C4NetIO::addr_t &naddr = C4NetIO::addr_t());
  // construct from buffer (takes data, if possible)
  explicit C4NetIOPacket(StdBuf Buf,
                         const C4NetIO::addr_t &naddr = C4NetIO::addr_t());
  // construct from status byte + buffer (copies data)
  C4NetIOPacket(uint8_t cStatusByte, const char *pnData, size_t inSize,
                const C4NetIO::addr_t &naddr = C4NetIO::addr_t());
...
class C4NetIOTCP : public C4NetIO, protected CStdCSecExCallback {
 ...
 protected:
  ...
  // Extract a packet from the start of the input buffer (if possible) and call
  // OnPacket.
  ...
  class Peer {
   ...
   public:
    ...
    bool Send();
    // request buffer space for new input. Must call OnRecv or NoRecv
    // afterwards!
    void *GetRecvBuf(int iSize);
    // called after the buffer returned by GetRecvBuf has been filled with fresh
    // data
  ...
  Peer *Accept(SOCKET nsock = INVALID_SOCKET,
...
// - Broadcast will multicast the packet to all clients with the same broadcast
// address.
...
// - Delivery garantueed
// - Broadcast will automatically be activated on one side if it's active on the
// other side.
//   If the peer can't be reached through broadcasting, packets will be sent
//   directly.
 ...
 protected:
  ...
  class Peer {
   ...
   protected:
    ...
    addr_t PeerAddr;
    // connection status
    enum ConnStatus { CS_None, CS_Conn, CS_Works, CS_Closed } eStatus;
    // does multicast work?
    bool fMultiCast;
    // selected for broadcast?
    bool fDoBroadcast;
   ...
   protected:
    ...
    bool DoCheck(int iAskCnt = 0, int iMCAskCnt = 0,
                 unsigned int *pAskList = NULL);
  ...
  // sending
  bool BroadcastDirect(const Packet &rPacket,
                       unsigned int iNr = ~0u);  // (mt-safe)
...
// helpers
inline bool AddrEqual(const C4NetIO::addr_t addr1,
                      const C4NetIO::addr_t addr2) {
  return addr1.sin_addr.s_addr == addr2.sin_addr.s_addr &&
         addr1.sin_family == addr2.sin_family &&
         addr1.sin_port == addr2.sin_port;
}
inline bool operator==(const C4NetIO::addr_t addr1,
                       const C4NetIO::addr_t addr2) {
...
// there seems to be no standard way to get these numbers, so let's do it the
// dirty way...
...
inline void CompileFunc(in_addr &ip, StdCompiler *pComp) {
  pComp->Value(in_addr_b(ip, 0));
  pComp->Seperator(StdCompiler::SEP_PART);
  pComp->Value(in_addr_b(ip, 1));
  pComp->Seperator(StdCompiler::SEP_PART);
  pComp->Value(in_addr_b(ip, 2));
  pComp->Seperator(StdCompiler::SEP_PART);
  pComp->Value(in_addr_b(ip, 3));
}

inline void CompileFunc(C4NetIO::addr_t &addr, StdCompiler *pComp) {
  pComp->Value(addr.sin_addr);
