...
C4MCCallbackArray::C4MCCallbackArray(C4AulFunc *pSFunc,
  ...
  if (this->pMapCreator = pMapCreator) pMapCreator->CallbackArrays.Add(this);
  // done
}
...
C4MCOverlay *C4MCNode::OwnerOverlay() {
  for (C4MCNode *pOwnr = Owner; pOwnr; pOwnr = pOwnr->Owner)
    if (C4MCOverlay *pOwnrOvrl = pOwnr->Overlay()) return pOwnrOvrl;
  // no overlay-owner
  return NULL;
}
...
bool C4MCNode::SetField(C4MCParser *pParser, const char *szField,
                        const char *szSVal, int32_t iVal,
                        C4MCTokenType ValType) {
  // no fields in base class
  return false;
}

int32_t C4MCNode::IntPar(C4MCParser *pParser, const char *szSVal, int32_t iVal,
                         C4MCTokenType ValType) {
  // check if int32_t
...
const char *C4MCNode::StrPar(C4MCParser *pParser, const char *szSVal,
                             int32_t iVal, C4MCTokenType ValType) {
  // check if identifier
  if (ValType != MCT_IDTF)
    throw C4MCParserErr(pParser, C4MCErr_FieldValInvalid, szSVal);
  return szSVal;
...
#define IntPar \
  IntPar(pParser, szSVal, iVal, ValType)  // shortcut for checked int32_t param
#define StrPar \
  StrPar(pParser, szSVal, iVal, ValType)  // shortcut for checked str param
...
C4MCOverlay::C4MCOverlay(C4MCNode *pOwner, C4MCOverlay &rTemplate, bool fClone)
    : C4MCNode(pOwner, rTemplate, fClone) {
  // copy fields
  X = rTemplate.X;
  Y = rTemplate.Y;
  Wdt = rTemplate.Wdt;
  ...
  OffX = rTemplate.OffX;
  OffY = rTemplate.OffY;
  ROffX = rTemplate.ROffX;
  ROffY = rTemplate.ROffY;
  Material = rTemplate.Material;
  SCopy(rTemplate.Texture, Texture, C4MaxName);
  Algorithm = rTemplate.Algorithm;
  Sub = rTemplate.Sub;
  ZoomX = rTemplate.ZoomX;
  ZoomY = rTemplate.ZoomY;
  MatClr = rTemplate.MatClr;
  Seed = rTemplate.Seed;
  Alpha = rTemplate.Alpha;
  Beta = rTemplate.Beta;
  Turbulence = rTemplate.Turbulence;
  Lambda = rTemplate.Lambda;
  Rotate = rTemplate.Rotate;
  Invert = rTemplate.Invert;
  LooseBounds = rTemplate.LooseBounds;
  Group = rTemplate.Group;
  Mask = rTemplate.Mask;
...
void C4MCOverlay::Default() {
  ...
  *Texture = 0;
  ...
  RWdt.Set(C4MC_SizeRes, true);
  RHgt.Set(C4MC_SizeRes, true);
  // def zoom
...
bool C4MCOverlay::SetField(C4MCParser *pParser, const char *szField,
                           const char *szSVal, int32_t iVal,
                           C4MCTokenType ValType) {
  int32_t iMat;
  C4MCAlgorithm *pAlgo;
  ...
  for (C4MCNodeAttr *pAttr = &C4MCOvrlMap[0]; *pAttr->Name; pAttr++)
    if (SEqual(szField, pAttr->Name)) {
      ...
      switch (pAttr->Type) {
        ...
        case C4MCV_Percent: {
          ((int_bool *)pTarget)
              ->Set(IntPar, ValType == MCT_PERCENT || ValType == MCT_INT);
          break;
        }
        case C4MCV_Pixels: {
        ...
        case C4MCV_Material:
          ...
          // check validity
          if (iMat == MNone)
            throw C4MCParserErr(pParser, C4MCErr_MatNotFound, StrPar);
          // store
        ...
        case C4MCV_Algorithm:
          ...
          // check validity
          if (!pAlgo)
            throw C4MCParserErr(pParser, C4MCErr_AlgoNotFound, StrPar);
          // store
        ...
        case C4MCV_Zoom:
          ...
          *((int32_t *)pTarget) =
              BoundBy<int32_t>(C4MC_ZoomRes - IntPar, 1, C4MC_ZoomRes * 2);
          break;
...
// get script func of main script
          ...
          if (!pSFunc)
            throw C4MCParserErr(pParser, C4MCErr_SFuncNotFound, StrPar);
          // add to main
          *((C4MCCallbackArray **)pTarget) =
              new C4MCCallbackArray(pSFunc, MapCreator);
#endif
        }
      }
      // done
...
void C4MCOverlay::Evaluate() {
  ...
  if (Inside<int32_t>(Material, 0, MapCreator->MatMap->Num - 1)) {
    MatClr = MapCreator->TexMap->GetIndexMatTex(
        MapCreator->MatMap->Map[Material].Name, *Texture ? Texture : NULL);
    if (Sub) MatClr += 128;
  } else
    MatClr = 0;
  // calc size
  if (Owner) {
    ...
    if (pOwnrOvrl = OwnerOverlay()) {
      ...
      int32_t iOwnerWdt = pOwnrOvrl->Wdt;
      int32_t iOwnerHgt = pOwnrOvrl->Hgt;
...
bool C4MCOverlay::CheckMask(int32_t iX, int32_t iY) {
  // bounds match?
  if (!LooseBounds)
    if (iX < X || iY < Y || iX >= X + Wdt || iY >= Y + Hgt) return false;
#ifdef DEBUGREC
  C4RCTrf rc;
  rc.x = iX;
  rc.y = iY;
  rc.Rotate = Rotate;
  rc.Turbulence = Turbulence;
    ...
    for (int32_t i = 10; i <= Turbulence; i *= 10) {
      ...
      for (int32_t l = 0; l <= Lambda; ++l) {
        for (FIXED d = itofix(2); d < 6; d += FIXED10(15)) {
          dX +=
              Sin(((dX / 7 + itofix(Seed2) / ZoomX + dY) / j + d) * Rad2Grad) *
              j / 2;
          dY +=
              Cos(((dY / 7 + itofix(Seed2) / ZoomY + dX) / j - d) * Rad2Grad) *
              j / 2;
        }
  ...
  if (LooseBounds)
    if (iX < X * ZoomX || iY < Y * ZoomY || iX >= (X + Wdt) * ZoomX ||
        iY >= (Y + Hgt) * ZoomY)
      return Invert;
  // query algorithm
...
bool C4MCOverlay::RenderPix(int32_t iX, int32_t iY, BYTE &rPix,
                            C4MCTokenType eLastOp, bool fLastSet, bool fDraw,
                            C4MCOverlay **ppPixelSetOverlay) {
  // algo match?
  ...
  if ((DoSet && fDraw && Op == MCT_NONE) || Group) {
    ...
    bool fLastSetC = false;
    eLastOp = MCT_NONE;
    // evaluate children overlays, if this was painted, too
    for (C4MCNode *pChild = Child0; pChild; pChild = pChild->Next)
      if (C4MCOverlay *pOvrl = pChild->Overlay()) {
        fLastSetC = pOvrl->RenderPix(iX, iY, rPix, eLastOp, fLastSetC, fDraw,
                                     ppPixelSetOverlay);
        if (Group && (pOvrl->Op == MCT_NONE)) DoSet |= fLastSetC;
        eLastOp = pOvrl->Op;
      }
...
C4MCPoint::C4MCPoint(C4MCNode *pOwner, C4MCPoint &rTemplate, bool fClone)
    : C4MCNode(pOwner, rTemplate, fClone) {
  // copy fields
  X = rTemplate.X;
  Y = rTemplate.Y;
...
void C4MCPoint::Default() { X = Y = 0; }

bool C4MCPoint::SetField(C4MCParser *pParser, const char *szField,
                         const char *szSVal, int32_t iVal,
                         C4MCTokenType ValType) {
  // only explicit %/px
...
C4MCMap::C4MCMap(C4MCNode *pOwner, C4MCMap &rTemplate, bool fClone)
    : C4MCOverlay(pOwner, rTemplate, fClone) {}
...
void C4MCMap::Default() {
  ...
  if (MapCreator->Landscape->MapPlayerExtend)
    Wdt = Min(Wdt * Min(MapCreator->PlayerCount, (int)C4S_MaxMapPlayerExtend),
              (int)MapCreator->Landscape->MapWdt.Max);
}
...
C4MapCreatorS2::C4MapCreatorS2(C4SLandscape *pLandscape, C4TextureMap *pTexMap,
                               C4MaterialMap *pMatMap, int iPlayerCount)
    : C4MCNode(NULL) {
  // me r b creator
  MapCreator = this;
  // store members
...
C4MCMap *C4MapCreatorS2::GetMap(const char *szMapName) {
  ...
  if (szMapName && *szMapName) {
    ...
    if (pNode = GetNodeByName(szMapName))
      if (pNode->Type() == MCN_Map) pMap = (C4MCMap *)pNode;
  } else {
...
BYTE *C4MapCreatorS2::RenderBuf(const char *szMapName, int32_t &sfcWdt,
                                int32_t &sfcHgt) {
  // get map
  C4MCMap *pMap = GetMap(szMapName);
  if (!pMap) return NULL;
...
C4MCParserErr::C4MCParserErr(C4MCParser *pParser, const char *szMsg) {
  ...
  sprintf(Msg, "%s: %s (%d)", pParser->Filename, szMsg,
          pParser->Code ? SGetLine(pParser->Code, pParser->CPos) : 0);
}
...
C4MCParserErr::C4MCParserErr(C4MCParser *pParser, const char *szMsg,
  ...
  sprintf(Msg, "%s: %s (%d)", pParser->Filename, Buf,
          pParser->Code ? SGetLine(pParser->Code, pParser->CPos) : 0);
}
    ...
    switch (State) {
      case TGS_None:
        ...
        if (((C >= '0') && (C <= '9') || (C == '+') || (C == '-')))
          ...
          CPos++;
          CurrToken = MCT_BLOPEN;
          return TRUE;
        }  // "{"
        else if (C == '}') {
          CPos++;
          CurrToken = MCT_BLCLOSE;
          return TRUE;
        }  // "}"
        else if (C == '&') {
          CPos++;
          ...
          State = TGS_Ident;  // identifier by all non-special chars
        else {
          // unrecognized char
          CPos++;
...
void C4MCParser::ParseTo(C4MCNode *pToNode) {
  ...
  enum ParseState {
    ...
    PS_GOTIDTF,    // got identifier, expect '=', ';' or '{'; identifier remains
                   // in CurrTokenIdtf
  ...
  while (GetNextToken()) {
    switch (State) {
        ...
        switch (CurrToken) {
          ...
          case MCT_IDTF:
            ...
            if (SEqual(CurrTokenIdtf, C4MC_Overlay)) {
              ...
              pNewNode =
                  new C4MCOverlay(pToNode, MapCreator->DefaultOverlay, false);
              State = PS_KEYWD1;
            } else if (SEqual(CurrTokenIdtf, C4MC_Point) &&
                       !pToNode->GetNodeByName(CurrTokenIdtf)) {
              // only in overlays
              if (!pToNode->Type() == MCN_Overlay)
                throw C4MCParserErr(this, C4MCErr_PointOnlyOvl);
              // create point node, using default template
              pNewNode =
                  new C4MCPoint(pToNode, MapCreator->DefaultPoint, false);
              State = PS_KEYWD1;
              ...
              // '=', ';' or '{' may follow, none of these will clear the
              // CurrTokenIdtf
      ...
      case PS_KEYWD1N:
        ...
        ParseTo(pNewNode);
        // check file end
        if (CurrToken == MCT_EOF) throw C4MCParserErr(this, C4MCErr_EOF);
        // reset state
        State = PS_AFTERNODE;
        break;
      case PS_GOTIDTF:
          ...
          case MCT_XOR:
            ...
            switch (pCpyNode->Type()) {
              case MCN_Overlay:
                // create overlay
                pNewNode =
                    new C4MCOverlay(pToNode, *((C4MCOverlay *)pCpyNode), false);
                break;
              case MCN_Map:
                // maps not allowed
      ...
      case PS_AFTERNODE:
        ...
        // we mustn't evaluate everything immediately, because parents must be
        // evaluated first!
      ...
      case PS_SETFIELD:
        ...
        switch (CurrToken)
                ...
                case MCT_INT:
                        break;
                default:
                        throw C4MCParserErr(this, C4MCErr_FieldConstExp,
        CurrTokenIdtf);
                        break;
                }
        // set field
        if (!pToNode->SetField(this, FieldName, CurrTokenIdtf, CurrTokenVal,
        CurrToken))
                // field not found
...
void C4MCParser::ParseValue(C4MCNode *pToNode, const char *szFieldName) {
  ...
  switch (CurrToken) {
    case MCT_IDTF: {
      ...
      if (!GetNextToken()) throw C4MCParserErr(this, C4MCErr_EOF);
      break;
    }
    case MCT_INT:
    case MCT_PX:
    case MCT_PERCENT: {
      ...
      if (MCT_RANGE == CurrToken) {
        ...
        if (!GetNextToken()) throw C4MCParserErr(this, C4MCErr_EOF);
        if (MCT_INT == CurrToken || MCT_PX == CurrToken ||
            MCT_PERCENT == CurrToken) {
  ...
  // now, the one and only thing to get is a semicolon
  if (CurrToken != MCT_SCOLON) throw C4MCParserErr(this, C4MCErr_SColonExp);
  ...
  switch (CurrToken)
          ...
          case MCT_IDTF:
                  ...
                  CurrTokenVal=0;
                  // set field
                  if (!pToNode->SetField(this, szFieldName, CurrTokenIdtf,
  CurrTokenVal, CurrToken))
                          // field not found
                          throw C4MCParserErr(this, C4MCErr_Field404,
  szFieldName);
                  break;
          case MCT_INT:
                  Value1 = CurrTokenVal;
                  while (GetNextToken ())
                          ...
                          switch (CurrToken)
                                  ...
                                  case MCT_SCOLON:
                                          // set field
                                          if (!pToNode->SetField(this,
  szFieldName, CurrTokenIdtf, Value1, MCT_INT))
                                                  // field not found
                                                  throw C4MCParserErr(this,
  C4MCErr_Field404, szFieldName);
                                          return;
                                          break;
                                  case MCT_RANGE:
                                          break;
...
// helper func
bool PreparePeek(C4MCOverlay **ppOvrl, int32_t &iX, int32_t &iY,
  ...
  for (*ppTopOvrl = pOvrl2; pNextOvrl = (*ppTopOvrl)->OwnerOverlay();
       *ppTopOvrl = pNextOvrl)
    ;
...
bool AlgoRandom(C4MCOverlay *pOvrl, int32_t iX, int32_t iY) {
  // totally random
  return !((((s ^ (iX << 2) ^ (iY << 5)) ^ ((s >> 16) + 1 + iX + (iY << 2))) /
            17) %
           (a.Evaluate(C4MC_SizeRes) + 2));
}
...
bool AlgoBozo(C4MCOverlay *pOvrl, int32_t iX, int32_t iY) {
  // do some bozo stuff - keep it regular here, since it may be modified by
  // turbulence
...
bool AlgoBorder(C4MCOverlay *pOvrl, int32_t iX, int32_t iY) {
  ...
  int32_t la = a.Evaluate(pOvrl->Wdt);
  int32_t lb = b.Evaluate(pOvrl->Hgt);
  // prepare a pixel peek from owner
  if (!PreparePeek(&pOvrl, iX, iY, &pTopOvrl)) return false;
  // query a/b pixels in x/y-directions
  for (int32_t x = iX - la; x <= iX + la; x++)
    if (pTopOvrl->InBounds(x, iY))
      if (!pOvrl->PeekPix(x, iY)) return true;
  for (int32_t y = iY - lb; y <= iY + lb; y++)
    if (pTopOvrl->InBounds(iX, y))
      if (!pOvrl->PeekPix(iX, y)) return true;
  // nothing found
  return false;
}
...
bool AlgoMandel(C4MCOverlay *pOvrl, int32_t iX, int32_t iY) {
  // how many iterations?
  uint32_t iMandelIter =
      a.Evaluate(C4MC_SizeRes) != 0 ? a.Evaluate(C4MC_SizeRes) : 1000;
  if (iMandelIter < 10) iMandelIter = 10;
  // calc c & ci values
  double c =
      ((double)iX / z / pOvrl->Wdt - .5 * ((double)pOvrl->ZoomX / z)) * 4;
  double ci =
      ((double)iY / z / pOvrl->Hgt - .5 * ((double)pOvrl->ZoomY / z)) * 4;
  // create _z & _zi
  double _z = c, _zi = ci;
  double xz;
  uint32_t i;
...
bool AlgoScript(C4MCOverlay *pOvrl, int32_t iX, int32_t iY) {
  ...
  // ok, call func
  C4AulParSet Pars(C4VInt(iX), C4VInt(iY),
                   C4VInt(pOvrl->Alpha.Evaluate(C4MC_SizeRes)),
                   C4VInt(pOvrl->Beta.Evaluate(C4MC_SizeRes)));
...
bool AlgoPolygon(C4MCOverlay *pOvrl, int32_t iX, int32_t iY) {
  ...
  for (pChild = pStartChild;; pChild = pChild->Next) {
    ...
    if (!pChild->Next)
      if (pStartChild == pOvrl->Child0) break;
  }
...
C4MCAlgorithm C4MCAlgoMap[] = {{"solid", &AlgoSolid},
                               {"random", &AlgoRandom},
                               {"checker", &AlgoChecker},
                               {"bozo", &AlgoBozo},
                               {"sin", &AlgoSin},
