...
/* Notes
   ...
   (which
         might not correctly indicate attach loss). Otherwise 1 pixel clefts can
   lead
   ...
   diagonal.
         C4PF_Ray_Crawl try new ray: don't worry about checking backwards jump
   if
   ...
   (is
         done by C4Command::Transfer on demand and would only cause
   no-good-entry-point
...
const int32_t C4PF_MaxDepth = 35, C4PF_MaxCrawl = 800, C4PF_MaxRay = 350,
              C4PF_Threshold = 10,
  ...
  switch (Status) {
    ...
    case C4PF_Ray_Launch:
      ...
      // Path intersected by transfer zone
        ...
        if (!pZone->At(X, Y)) pZone->GetEntryPoint(X2, Y2, X2, Y2);
        // Add use-zone ray
...
BOOL C4PathFinderRay::PathFree(int32_t &rX, int32_t &rY, int32_t iToX,
                               int32_t iToY, C4TransferZone **ppZone) {
...
/*void C4PathFinderRay::DrawLine(SURFACE sfcSurface, int32_t rX, int32_t rY,
  int32_t iToX, int32_t iToY, BYTE byCol)
        {
  int32_t d,dx,dy,aincr,bincr,xincr,yincr,x,y;
        // Y based
  if (Abs(iToX-rX)<Abs(iToY-rY))
...
BOOL C4PathFinderRay::Crawl() {
  ...
  if (CrawlLength)
    if (!IsCrawlAttach(X2, Y2, CrawlAttach)) {
      ...
      if (!IsCrawlAttach(X2, Y2, CrawlAttach)) return FALSE;
      // Corner okay
      return TRUE;
    }
...
void C4PathFinderRay::SetCompletePath() {
  ...
  for (pRay = this; pRay->From; pRay = pRay->From) {
    // Transfer waypoint
    if (pRay->UseZone)
      pPathFinder->SetWaypoint(pRay->X2, pRay->Y2,
                               (intptr_t)pRay->UseZone->Object,
...
BOOL C4PathFinderRay::CrawlTargetFree(int32_t iX, int32_t iY, int32_t iAttach,
                                      int32_t iDirection) {
  CrawlByAttach(iX, iY, iAttach, iDirection);
  return PointFree(iX, iY);
}

void C4PathFinderRay::CrawlByAttach(int32_t &rX, int32_t &rY, int32_t iAttach,
                                    int32_t iDirection) {
  switch (iAttach) {
    case C4PF_Crawl_Top:
      rX += iDirection;
...
int32_t C4PathFinderRay::FindCrawlAttachDiagonal(int32_t iX, int32_t iY,
                                                 int32_t iDirection) {
  // Going left
...
void C4PathFinder::Init(bool (*fnPointFree)(int32_t, int32_t),
...
void C4PathFinder::SetLevel(int iLevel) { Level = BoundBy(iLevel, 1, 10); }
...
BOOL C4PathFinder::Execute() {
  ...
  for (C4PathFinderRay *pRay = FirstRay; pRay && !Success;
    ...
    if (pRay->Execute()) fContinue = TRUE;
...
BOOL C4PathFinder::Find(int32_t iFromX, int32_t iFromY, int32_t iToX,
                        int32_t iToY, bool (*fnSetWaypoint)(int32_t, int32_t,
                                                            intptr_t, intptr_t),
                        intptr_t iWaypointParameter) {
  // Prepare
  Clear();
...
BOOL C4PathFinder::AddRay(int32_t iFromX, int32_t iFromY, int32_t iToX,
                          int32_t iToY, int32_t iDepth, int32_t iDirection,
                          C4PathFinderRay *pFrom, C4TransferZone *pUseZone) {
  // Max depth
