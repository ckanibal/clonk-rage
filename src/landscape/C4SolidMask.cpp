...
int32_t C4SolidMask::DensityProvider::GetDensity(int32_t x, int32_t y) const {
  ...
  if (!rSolidMaskData.MaskPutRotation) {
    pPix = rSolidMaskData.pSolidMask +
           (y + rSolidMaskData.MaskPutRect.ty) *
               rSolidMaskData.pForObject->SolidMask.Wdt +
           rSolidMaskData.MaskPutRect.tx + x;
    if (*pPix == 0xff)
      return C4M_Solid;
    else
      return 0;
    ...
    // for SolidMask-pixels not put because there was another SolidMask already,
    // this will not return solid
    pPix = rSolidMaskData.pSolidMaskMatBuff +
           (y + rSolidMaskData.MaskPutRect.ty) * rSolidMaskData.MatBuffPitch +
           rSolidMaskData.MaskPutRect.tx + x;
    if (*pPix == MCVehic)
      return 0;
    else
...
void C4SolidMask::Clear() {
  ...
  if (pSolidMaskMatBuff) {
    delete[] pSolidMaskMatBuff;
    pSolidMaskMatBuff = NULL;
  }
...
C4SolidMask::C4SolidMask(C4Object *pForObject) : pForObject(pForObject) {
  ...
  for (ycnt = 0; ycnt < pForObject->SolidMask.Hgt; ycnt++)
    for (xcnt = 0; xcnt < pForObject->SolidMask.Wdt; xcnt++) {
      ...
      // center.
      pSolidMask[xcnt + ycnt * pForObject->SolidMask.Wdt] =
          sfcBitmap->IsPixTransparent(pForObject->SolidMask.x + xcnt,
                                      pForObject->SolidMask.y + ycnt)
              ? 0x00
              : 0xff;
    }
  // create mat buff to store the material replaced by the solid mask
  // the upper left corner is here the
  // [objpos]+rot([shapexy]+[targetxy]+[realWH]/2)-maxWH/2
  MatBuffPitch =
      (int)sqrt(double(pForObject->SolidMask.Wdt * pForObject->SolidMask.Wdt +
                       pForObject->SolidMask.Hgt * pForObject->SolidMask.Hgt)) +
      1;
