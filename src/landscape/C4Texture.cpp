...
C4TexMapEntry::C4TexMapEntry() : pMaterial(NULL), iMaterialIndex(MNone) {}
...
bool C4TexMapEntry::Init() {
  ...
  if (!MatValid(iMaterialIndex)) {
    DebugLogF("Error initializing material %s-%s: Invalid material!",
              Material.getData(), Texture.getData());
    return false;
  }
  ...
  if (DensityLiquid(pMaterial->Density))
    if (SEqualNoCase(szTexture, "Smooth")) szTexture = "Liquid";
  // Find texture
  C4Texture *sfcTexture = Game.TextureMap.GetTexture(szTexture);
  if (!sfcTexture) {
    DebugLogF("Error initializing material %s-%s: Invalid texture!",
              Material.getData(), Texture.getData());
    Clear();
    return false;
  }
...
BOOL C4TextureMap::AddEntry(BYTE byIndex, const char *szMaterial,
                            const char *szTexture) {
  // Security
  if (byIndex <= 0 || byIndex >= C4M_MaxTexIndex) return FALSE;
...
void C4TextureMap::Clear() {
  for (int32_t i = 1; i < C4M_MaxTexIndex; i++) Entry[i].Clear();
...
BOOL C4TextureMap::LoadFlags(C4Group &hGroup, const char *szEntryName,
                             BOOL *pOverloadMaterials,
                             BOOL *pOverloadTextures) {
  // Load the file
  StdStrBuf TexMap;
  if (!hGroup.LoadEntryString(szEntryName, TexMap)) return FALSE;
  // Reset flags
  ...
  for (const char *pPos = TexMap.getData(); pPos && *pPos;
       pPos = SSearch(pPos + 1, "\n")) {
    // Go over newlines
    while (*pPos == '\r' || *pPos == '\n') pPos++;
    // Flag?
...
int32_t C4TextureMap::LoadMap(C4Group &hGroup, const char *szEntryName,
                              BOOL *pOverloadMaterials,
                              BOOL *pOverloadTextures) {
  char *bpMap;
  char szLine[100 + 1];
  ...
  for (cnt = 0; SCopySegment(bpMap, cnt, szLine, 0x0A, 100); cnt++)
    if ((szLine[0] != '#') && (SCharCount('=', szLine) == 1)) {
      ...
      if (Inside<int32_t>(iIndex = strtol(szLine, NULL, 10), 0,
                          C4M_MaxTexIndex - 1)) {
        const char *szMapping = szLine + SCharPos('=', szLine) + 1;
        StdStrBuf Material, Texture;
        Material.CopyUntil(szMapping, '-');
        Texture.Copy(SSearch(szMapping, "-"));
        if (AddEntry(iIndex, Material.getData(), Texture.getData()))
          iTextures++;
      }
    } else {
      if (SEqual2(szLine, "OverloadMaterials")) {
        fOverloadMaterials = TRUE;
        if (pOverloadMaterials) *pOverloadMaterials = TRUE;
      }
      if (SEqual2(szLine, "OverloadTextures")) {
        fOverloadTextures = TRUE;
        if (pOverloadTextures) *pOverloadTextures = TRUE;
      }
    }
...
bool C4TextureMap::SaveMap(C4Group &hGroup, const char *szEntryName) {
  ...
  sTexMapFile.Append(
      "# Contains material-texture-combinations added at runtime" LineFeed);
  // add overload-entries
  if (fOverloadMaterials)
    sTexMapFile.Append("# Import materials from global file as well" LineFeed
                       "OverloadMaterials" LineFeed);
  if (fOverloadTextures)
    sTexMapFile.Append("# Import textures from global file as well" LineFeed
                       "OverloadTextures" LineFeed);
  sTexMapFile.Append(LineFeed);
  // add entries
  for (int32_t i = 0; i < C4M_MaxTexIndex; i++)
    if (!Entry[i].isNull()) {
      // compose line
      sTexMapFile.AppendFormat("%d=%s-%s" LineFeed, i,
                               Entry[i].GetMaterialName(),
                               Entry[i].GetTextureName());
    }
  // create new buffer allocated with new [], because C4Group cannot handle
  // StdStrBuf-buffers
...
bool C4TextureMap::HasTextures(C4Group &hGroup) {
  return hGroup.EntryCount(C4CFN_PNGFiles) ||
         hGroup.EntryCount(C4CFN_BitmapFiles);
}
...
int32_t C4TextureMap::GetIndex(const char *szMaterial, const char *szTexture,
                               BOOL fAddIfNotExist,
  ...
  for (byIndex = 1; byIndex < C4M_MaxTexIndex; byIndex++)
    if (!Entry[byIndex].isNull())
      if (SEqualNoCase(Entry[byIndex].GetMaterialName(), szMaterial))
        if (!szTexture ||
            SEqualNoCase(Entry[byIndex].GetTextureName(), szTexture))
          return byIndex;
  // Add new entry
  if (fAddIfNotExist)
    for (byIndex = 1; byIndex < C4M_MaxTexIndex; byIndex++)
      if (Entry[byIndex].isNull()) {
        ...
        if (szErrorIfFailed)
          DebugLogF(
              "Error getting MatTex %s-%s for %s from TextureMap: Init failed.",
              szMaterial, szTexture, szErrorIfFailed);
#endif
        return 0;
      }
// Else, fail
#ifdef C4ENGINE
  if (szErrorIfFailed)
    DebugLogF("Error getting MatTex %s-%s for %s from TextureMap: %s.",
              szMaterial, szTexture, szErrorIfFailed,
              fAddIfNotExist ? "Map is full!" : "Entry not found.");
#endif
  return 0;
}
...
C4Texture *C4TextureMap::GetTexture(const char *szTexture) {
  ...
  for (pTexture = FirstTexture; pTexture; pTexture = pTexture->Next)
    if (SEqualNoCase(pTexture->Name, szTexture)) return pTexture;
  return NULL;
}
...
const char *C4TextureMap::GetTexture(int32_t iIndex) {
  ...
  for (pTexture = FirstTexture, cindex = 0; pTexture;
    ...
    if (cindex == iIndex) return pTexture->Name;
  return NULL;
}
