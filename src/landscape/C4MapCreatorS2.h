...
class C4MCCallbackArray {
 ...
 public:
  void EnablePixel(int32_t iX,
                   int32_t iY);  // enable pixel in map; create map if necessary
...
class C4MCNode {
 ...
 public:
  ...
  C4MCNode(C4MCNode *pOwner, C4MCNode &rTemplate,
           bool fClone);  // constructor using template
  virtual ~C4MCNode();    // destructor

  virtual C4MCNode *clone(C4MCNode *pToNode) {
    return new C4MCNode(pToNode, *this, true);
  }
 ...
 protected:
  virtual BOOL GlobalScope() {
    return FALSE;
  }  // whether node is a global scope
  virtual BOOL SetOp(C4MCTokenType eOp) {
    return FALSE;
  }                                             // set following operator
  C4MCNode *GetNodeByName(const char *szName);  // search node by name

  virtual bool SetField(C4MCParser *pParser, const char *szField,
                        const char *szSVal, int32_t iVal,
                        C4MCTokenType ValType);  // set field
  int32_t IntPar(C4MCParser *pParser, const char *szSVal, int32_t iVal,
                 C4MCTokenType ValType);  // ensure par is int32_t
  const char *StrPar(C4MCParser *pParser, const char *szSVal, int32_t iVal,
                     C4MCTokenType ValType);  // ensure par is string
  ...
  class int_bool {
   public:
    int32_t Evaluate(int32_t relative_to) {
      if (percent)
        return value * relative_to / C4MC_SizeRes;
      else
        return value;
    }
...
class C4MCOverlay : public C4MCNode {
 public:
  ...
  C4MCOverlay(C4MCNode *pOwner, C4MCOverlay &rTemplate,
              bool fClone);  // construct of template

  C4MCNode *clone(C4MCNode *pToNode) {
    return new C4MCOverlay(pToNode, *this, true);
  }
 ...
 public:
  ...
  int_bool RX, RY, RWdt, RHgt, ROffX,
      ROffY;                      // extends/offset relatively to owner
  ...
  C4MCCallbackArray *
      pEvaluateFunc;  // function called for nodes being evaluated and fulfilled
  C4MCCallbackArray *pDrawFunc;  // function called when this node is drawn -
                                 // pass drawcolor as first param, return color
                                 // to be actually used
  ...
  bool SetField(C4MCParser *pParser, const char *szField, const char *szSVal,
                int32_t iVal, C4MCTokenType ValType);  // set field

  void Evaluate();  // called when all fields are initialized

  C4MCOverlay *Overlay() { return this; }  // this is an overlay
  C4MCOverlay *
  FirstOfChain();  // go backwards in op chain until first overlay of chain

  bool CheckMask(int32_t iX,
                 int32_t iY);  // check whether algorithms succeeds at iX/iY
  bool RenderPix(int32_t iX, int32_t iY, BYTE &rPix,
                 C4MCTokenType eLastOp = MCT_NONE, bool fLastSet = false,
                 bool fDraw = true,
                 C4MCOverlay **ppPixelSetOverlay = NULL);  // render this pixel
  bool PeekPix(int32_t iX, int32_t iY);  // check mask; regard operator chain
  bool InBounds(int32_t iX, int32_t iY) {
    return iX >= X && iY >= Y && iX < X + Wdt && iY < Y + Hgt;
  }  // return whether point iX/iY is inside bounds
...
class C4MCPoint : public C4MCNode {
 public:
  ...
  C4MCPoint(C4MCNode *pOwner, C4MCPoint &rTemplate,
            bool fClone);  // construct of template

  C4MCNode *clone(C4MCNode *pToNode) {
    return new C4MCPoint(pToNode, *this, true);
  }
...
class C4MCMap : public C4MCOverlay {
 public:
  ...
  C4MCMap(C4MCNode *pOwner, C4MCMap &rTemplate,
          bool fClone);  // construct of template

  C4MCNode *clone(C4MCNode *pToNode) {
    return new C4MCMap(pToNode, *this, true);
  }
...
class C4MapCreatorS2 : public C4MCNode {
 public:
  C4MapCreatorS2(C4SLandscape *pLandscape, C4TextureMap *pTexMap,
                 C4MaterialMap *pMatMap, int iPlayerCount);  // constructor
...
#endif
  BYTE *RenderBuf(const char *szMapName, int32_t &sfcWdt,
                  int32_t &sfcHgt);  // create buffer and render it
...
class C4MCParserErr {
 public:
  ...
  C4MCParserErr(C4MCParser *pParser,
                const char *szMsg);  // construct setting error msg
  C4MCParserErr(C4MCParser *pParser, const char *szMsg,
                const char *szPar);  // construct setting error msg
  void show();                       // log error
};
...
class C4MCParser {
 private:
  ...
  BOOL
  AdvanceSpaces();  // advance to next token char; return whether EOF is reached
  BOOL GetNextToken();  // get token, store in fields and advance to next;
                        // return whether not EOF
