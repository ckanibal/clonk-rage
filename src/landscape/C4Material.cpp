...
void C4MaterialReaction::CompileFunc(StdCompiler *pComp) {
  ...
  int32_t i = 0;
  while (ReactionFuncMap[i].szRFName && (ReactionFuncMap[i].pFunc != pFunc))
    ++i;
  sReactionFuncName = ReactionFuncMap[i].szRFName;
  pComp->Value(mkNamingAdapt(sReactionFuncName, "Type", StdStrBuf()));
  i = 0;
  while (ReactionFuncMap[i].szRFName &&
         !SEqual(ReactionFuncMap[i].szRFName, sReactionFuncName.getData()))
    ++i;
...
void C4MaterialReaction::ResolveScriptFuncs(const char *szMatName) {
  ...
  if (pFunc == &C4MaterialMap::mrfScript)
    pScriptFunc = Game.ScriptEngine.GetSFuncWarn(
        this->ScriptFunc.getData(), AA_PROTECTED,
        FormatString("Material reaction of \"%s\"", szMatName).getData());
  else
    pScriptFunc = NULL;
}
#endif
...
BOOL C4MaterialCore::Load(C4Group &hGroup, const char *szEntryName) {
  StdStrBuf Source;
  if (!hGroup.LoadEntryString(szEntryName, Source)) return FALSE;
  StdStrBuf Name = hGroup.GetFullName() + DirSep + szEntryName;
  if (!CompileFromBuf_LogWarn<StdCompilerINIRead>(*this, Source,
                                                  Name.getData()))
    return FALSE;
...
DWORD C4MaterialCore::GetDWordColor(int32_t iIndex) {
  ...
  return RGB(Color[iClrIndex * 3 + 2], Color[iClrIndex * 3 + 1],
             Color[iClrIndex * 3]) |
         (Alpha[iIndex] << 24);
}
...
void C4MaterialCore::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(mkNamingAdapt(
      mkParAdapt(sTextureOverlay, StdCompiler::RCT_IdtfAllowEmpty),
      "TextureOverlay", ""));
  pComp->Value(mkNamingAdapt(OverlayType, "OverlayType", 0));
  pComp->Value(mkNamingAdapt(
      mkParAdapt(sPXSGfx, StdCompiler::RCT_IdtfAllowEmpty), "PXSGfx", ""));
  ...
  pComp->Value(
      mkNamingAdapt(mkParAdapt(sBlastShiftTo, StdCompiler::RCT_IdtfAllowEmpty),
                    "BlastShiftTo", ""));
  pComp->Value(
      mkNamingAdapt(mkParAdapt(sInMatConvert, StdCompiler::RCT_IdtfAllowEmpty),
                    "InMatConvert", ""));
  pComp->Value(mkNamingAdapt(
      mkParAdapt(sInMatConvertTo, StdCompiler::RCT_IdtfAllowEmpty),
      "InMatConvertTo", ""));
  ...
  pComp->Value(mkNamingAdapt(
      mkParAdapt(sAboveTempConvertTo, StdCompiler::RCT_IdtfAllowEmpty),
      "AboveTempConvertTo", ""));
  ...
  pComp->Value(mkNamingAdapt(
      mkParAdapt(sBelowTempConvertTo, StdCompiler::RCT_IdtfAllowEmpty),
      "BelowTempConvertTo", ""));
...
C4MaterialMap::C4MaterialMap()
    : DefReactConvert(&mrfConvert),
      DefReactPoof(&mrfPoof),
      DefReactCorrode(&mrfCorrode),
      DefReactIncinerate(&mrfIncinerate),
      DefReactInsert(&mrfInsert) {
  Default();
}
...
int32_t C4MaterialMap::Load(C4Group &hGroup, C4Group *OverloadFile) {
  ...
  while (hGroup.FindNextEntry(C4CFN_MaterialFiles, entryname)) {
    ...
    // A new material?
    if (Get(pNewMap[cnt].Name) == MNone) cnt++;
  }
...
int32_t C4MaterialMap::Get(const char *szMaterial) {
  ...
  for (cnt = 0; cnt < Num; cnt++)
    if (SEqualNoCase(szMaterial, Map[cnt].Name)) return cnt;
  return MNone;
}
...
void C4MaterialMap::CrossMapMaterials()  // Called after load
{
  ...
  for (int32_t iMatPXS = -1; iMatPXS < Num; iMatPXS++) {
    ...
    for (int32_t iMatLS = -1; iMatLS < Num; iMatLS++) {
      ...
      if (pMatPXS && pMatPXS->sInMatConvert.getLength() &&
          SEqualNoCase(pMatPXS->sInMatConvert.getData(),
                       pMatLS ? pMatLS->Name : C4TLS_MatSky))
        pReaction = &DefReactConvert;
      // the rest is happening for same/higher densities only
      else if ((MatDensity(iMatPXS) <= MatDensity(iMatLS)) && pMatPXS &&
               pMatLS) {
        // incindiary vs extinguisher
        if ((pMatPXS->Incindiary && pMatLS->Extinguisher) ||
            (pMatPXS->Extinguisher && pMatLS->Incindiary))
          pReaction = &DefReactPoof;
        // incindiary vs inflammable
        else if ((pMatPXS->Incindiary && pMatLS->Inflammable) ||
                 (pMatPXS->Inflammable && pMatLS->Incindiary))
  ...
  for (cnt = 0; cnt < Num; cnt++) {
    ...
    if (Map[cnt].sTextureOverlay.getLength())
      if (Game.TextureMap.GetTexture(Map[cnt].sTextureOverlay.getLength())) {
        ...
        // backwards compatibility: if a pattern was specified although the
        // no-pattern flag was set, overwrite that flag
        if (Map[cnt].OverlayType & C4MatOv_None) {
          DebugLogF(
              "Error in overlay of material %s: Flag C4MatOv_None ignored "
              "because a custom overlay (%s) was specified!",
              Map[cnt].Name, szTextureOverlay);
          Map[cnt].OverlayType &= ~C4MatOv_None;
        }
      }
    // default to smooth
    if (!szTextureOverlay) szTextureOverlay = "Smooth";
    // search/create entry in texmap
    Map[cnt].DefaultMatTex = Game.TextureMap.GetIndex(
        Map[cnt].Name, szTextureOverlay, TRUE,
        FormatString("DefaultMatTex of mat %s", Map[cnt].Name).getData());
    const C4TexMapEntry *pTex =
        Game.TextureMap.GetEntry(Map[cnt].DefaultMatTex);
    if (pTex) {
      // take pattern
      Map[cnt].MatPattern = pTex->getPattern();
      // special zooming for overlay
      Map[cnt].MatPattern.SetZoom((Map[cnt].OverlayType & C4MatOv_Exact) ? 1
                                                                         : 2);
    }
    // init PXS facet
    SURFACE sfcTexture;
    ...
    if (Map[cnt].sPXSGfx.getLength())
      if (Texture = Game.TextureMap.GetTexture(Map[cnt].sPXSGfx.getData()))
        if (sfcTexture = Texture->Surface32)
          Map[cnt].PXSFace.Set(sfcTexture, Map[cnt].PXSGfxRt.x,
                               Map[cnt].PXSGfxRt.y, Map[cnt].PXSGfxRt.Wdt,
                               Map[cnt].PXSGfxRt.Hgt);
    // evaluate reactions for that material
    for (unsigned int iRCnt = 0; iRCnt < pMat->CustomReactionList.size();
         ++iRCnt) {
      C4MaterialReaction *pReact = &(pMat->CustomReactionList[iRCnt]);
      if (pReact->sConvertMat.getLength())
        pReact->iConvertMat = Get(pReact->sConvertMat.getData());
      else
        pReact->iConvertMat = -1;
  ...
  for (cnt = 0; cnt < Num; cnt++) {
    if (Map[cnt].sBlastShiftTo.getLength())
      Map[cnt].BlastShiftTo = Game.TextureMap.GetIndexMatTex(
          Map[cnt].sBlastShiftTo.getData(), NULL, TRUE,
          FormatString("BlastShiftTo of mat %s", Map[cnt].Name).getData());
    ...
    if (Map[cnt].sBelowTempConvertTo.getLength())
      Map[cnt].BelowTempConvertTo = Game.TextureMap.GetIndexMatTex(
          Map[cnt].sBelowTempConvertTo.getData(), NULL, TRUE,
          FormatString("BelowTempConvertTo of mat %s", Map[cnt].Name)
              .getData());
    if (Map[cnt].sAboveTempConvertTo.getLength())
      Map[cnt].AboveTempConvertTo = Game.TextureMap.GetIndexMatTex(
          Map[cnt].sAboveTempConvertTo.getData(), NULL, TRUE,
          FormatString("AboveTempConvertTo of mat %s", Map[cnt].Name)
              .getData());
  }
#if 0
	int32_t i=0;
...
void C4MaterialMap::SetMatReaction(int32_t iPXSMat, int32_t iLSMat,
...
BOOL C4MaterialMap::LoadEnumeration(C4Group &hGroup) {
  ...
  int32_t cmat = 0;
  if (!(csearch = SSearch(mapbuf.getData(), "[Enumeration]"))) {
    return FALSE;
  }
...
BOOL C4MaterialMap::SortEnumeration(int32_t iMat, const char *szMatName) {
  ...
  for (cmat = iMat; cmat < Num; cmat++)
    if (SEqual(szMatName, Map[cmat].Name)) break;
  // Not found
...
bool mrfInsertCheck(int32_t &iX, int32_t &iY, FIXED &fXDir, FIXED &fYDir,
                    int32_t &iPxsMat, int32_t iLsMat, bool *pfPosChanged) {
  // always manipulating pos/speed here
  if (pfPosChanged) *pfPosChanged = true;

  // Rough contact? May splash
  if (fYDir > itofix(1))
    if (Game.Material.Map[iPxsMat].SplashRate &&
        !Random(Game.Material.Map[iPxsMat].SplashRate)) {
      fYDir = -fYDir / 8;
  ...
  if (Game.Landscape.FindMatSlide(iSlideX, iSlideY, Sign(GravAccel),
                                  Game.Material.Map[iPxsMat].Density,
                                  Game.Material.Map[iPxsMat].MaxSlide)) {
    if (iPxsMat == iLsMat) {
      iX = iSlideX;
      iY = iSlideY;
      fXDir = 0;
...
bool mrfUserCheck(C4MaterialReaction *pReaction, int32_t &iX, int32_t &iY,
                  int32_t iLSPosX, int32_t iLSPosY, FIXED &fXDir, FIXED &fYDir,
...
bool C4MaterialMap::mrfConvert(C4MaterialReaction *pReaction, int32_t &iX,
  ...
  if (pReaction->fUserDefined)
    if (!mrfUserCheck(pReaction, iX, iY, iLSPosX, iLSPosY, fXDir, fYDir,
                      iPxsMat, iLsMat, evEvent, pfPosChanged))
      return false;
  switch (evEvent) {
    case meePXSMove:  // PXS movement
      // for hardcoded stuff: only InMatConvert is Snow in Water, which does not
      // have any collision proc
    ...
    {
      // Check depth
      int32_t iDepth = pReaction->fUserDefined
                           ? pReaction->iDepth
                           : Game.Material.Map[iPxsMat].InMatConvertDepth;
      if (!iDepth || GBackMat(iX, iY - iDepth) == iLsMat) {
        // Convert
        iPxsMat = pReaction->fUserDefined
                      ? pReaction->iConvertMat
                      : Game.Material.Map[iPxsMat].InMatConvertTo;
        if (!MatValid(iPxsMat))
          // Convert failure (target mat not be loaded, or target may be
          // C4TLS_MatSky): Kill Pix
          return true;
...
bool C4MaterialMap::mrfPoof(C4MaterialReaction *pReaction, int32_t &iX,
                            int32_t &iY, int32_t iLSPosX, int32_t iLSPosY,
                            FIXED &fXDir, FIXED &fYDir, int32_t &iPxsMat,
                            int32_t iLsMat, MaterialInteractionEvent evEvent,
                            bool *pfPosChanged) {
  ...
  switch (evEvent) {
    case meeMassMove:  // MassMover-movement
    case meePXSPos:    // PXS check before movement: Kill both landscape and PXS
                       // mat
    ...
    case meePXSMove:  // PXS movement
      // incindiary/extinguisher/corrosives are always same density proc; so do
      // insertion check first
      if (!pReaction->fUserDefined)
        if (!mrfInsertCheck(iX, iY, fXDir, fYDir, iPxsMat, iLsMat,
                            pfPosChanged))
...
bool C4MaterialMap::mrfCorrode(C4MaterialReaction *pReaction, int32_t &iX,
                               int32_t &iY, int32_t iLSPosX, int32_t iLSPosY,
                               FIXED &fXDir, FIXED &fYDir, int32_t &iPxsMat,
                               int32_t iLsMat, MaterialInteractionEvent evEvent,
                               bool *pfPosChanged) {
  ...
  switch (evEvent) {
    ...
    case meeMassMove:  // MassMover-movement
    {
      ...
      if (pReaction->fUserDefined)
        ...
        fDoCorrode = (Random(100) < Game.Material.Map[iPxsMat].Corrosive) &&
                     (Random(100) < Game.Material.Map[iLsMat].Corrode);
      if (fDoCorrode) {
        ClearBackPix(iLSPosX, iLSPosY);
        // Game.Landscape.CheckInstabilityRange(iLSPosX,iLSPosY); - more
        // correct, but makes acid too effective as well
    ...
    case meePXSMove:  // PXS movement
    {
      // corrodes to corrosives are always same density proc; so do insertion
      // check first
      if (!pReaction->fUserDefined)
        if (!mrfInsertCheck(iX, iY, fXDir, fYDir, iPxsMat, iLsMat,
                            pfPosChanged))
...
bool C4MaterialMap::mrfIncinerate(C4MaterialReaction *pReaction, int32_t &iX,
                                  int32_t &iY, int32_t iLSPosX, int32_t iLSPosY,
                                  FIXED &fXDir, FIXED &fYDir, int32_t &iPxsMat,
                                  int32_t iLsMat,
  ...
  switch (evEvent) {
    ...
    case meePXSMove:  // PXS movement
      // incinerate to inflammables are always same density proc; so do
      // insertion check first
      if (!mrfInsertCheck(iX, iY, fXDir, fYDir, iPxsMat, iLsMat, pfPosChanged))
        // either splash or slide prevented interaction
        return false;
...
bool C4MaterialMap::mrfInsert(C4MaterialReaction *pReaction, int32_t &iX,
                              int32_t &iY, int32_t iLSPosX, int32_t iLSPosY,
                              FIXED &fXDir, FIXED &fYDir, int32_t &iPxsMat,
                              int32_t iLsMat, MaterialInteractionEvent evEvent,
                              bool *pfPosChanged) {
...
bool C4MaterialMap::mrfScript(C4MaterialReaction *pReaction, int32_t &iX,
                              int32_t &iY, int32_t iLSPosX, int32_t iLSPosY,
                              FIXED &fXDir, FIXED &fYDir, int32_t &iPxsMat,
                              int32_t iLsMat, MaterialInteractionEvent evEvent,
                              bool *pfPosChanged) {
  // do generic checks for user-defined reactions
  if (!mrfUserCheck(pReaction, iX, iY, iLSPosX, iLSPosY, fXDir, fYDir, iPxsMat,
                    iLsMat, evEvent, pfPosChanged))
    return false;
  ...
  C4AulParSet pars(C4VInt(iX), C4VInt(iY), C4VInt(iLSPosX), C4VInt(iLSPosY),
                   C4VInt(iXDir1 = fixtoi(fXDir, 100)),
                   C4VInt(iYDir1 = fixtoi(fYDir, 100)), C4VInt(iPxsMat),
                   C4VInt(iLsMat), C4VInt(evEvent));
  ...
  iXDir2 = pars[4].getInt();
  iYDir2 = pars[5].getInt();
