...
BOOL C4Scenario::Load(C4Group &hGroup, bool fLoadSection) {
  ...
  // Compile
  if (!Compile(pSource, fLoadSection)) {
    delete[] pSource;
    return FALSE;
  }
  delete[] pSource;
...
BOOL C4Scenario::Save(C4Group &hGroup, bool fSaveSection) {
  char *Buffer;
  int32_t BufferSize;
  if (!Decompile(&Buffer, &BufferSize, fSaveSection)) return FALSE;
  if (!hGroup.Add(C4CFN_ScenarioCore, Buffer, BufferSize, FALSE, TRUE)) {
    StdBuf Buf;
    Buf.Take(Buffer, BufferSize);
    return FALSE;
  }
  return TRUE;
}
...
void C4Scenario::CompileFunc(StdCompiler *pComp, bool fSection) {
  ...
  for (int32_t i = 0; i < C4S_MaxPlayer; i++)
    pComp->Value(
        mkNamingAdapt(PlrStart[i], FormatString("Player%d", i + 1).getData()));
...
int32_t C4Scenario::GetMinPlayer() {
  ...
  if (Head.MinPlayer != 0) return Head.MinPlayer;
  // Melee? Need at least two.
  if (Game.IsMelee()) return 2;
...
void C4SGame::CompileFunc(StdCompiler *pComp, bool fSection) {
  ...
  pComp->Value(mkNamingAdapt(Realism.ConstructionNeedsMaterial,
                             "StructNeedMaterial", FALSE));
  pComp->Value(
      mkNamingAdapt(Realism.StructuresNeedEnergy, "StructNeedEnergy", TRUE));
  if (!fSection) {
    pComp->Value(
        mkNamingAdapt(Realism.ValueOverloads, "ValueOverloads", C4IDList()));
  }
  pComp->Value(mkNamingAdapt(mkRuntimeValueAdapt(Realism.LandscapePushPull),
                             "LandscapePushPull", FALSE));
  pComp->Value(mkNamingAdapt(mkRuntimeValueAdapt(Realism.LandscapeInsertThrust),
                             "LandscapeInsertThrust", TRUE));
  ...
  pComp->Value(
      mkNamingAdapt(mkRuntimeValueAdapt(mkBitfieldAdapt<int32_t>(
                        Realism.BaseFunctionality, BaseFunctionalities)),
                    "BaseFunctionality", BASEFUNC_Default));
  pComp->Value(
      mkNamingAdapt(mkRuntimeValueAdapt(Realism.BaseRegenerateEnergyPrice),
...
bool C4SPlrStart::EquipmentEqual(C4SPlrStart &rhs) { return *this == rhs; }

bool C4SPlrStart::operator==(const C4SPlrStart &rhs) {
  return (NativeCrew == rhs.NativeCrew) && (Crew == rhs.Crew) &&
         (Wealth == rhs.Wealth) && (ReadyCrew == rhs.ReadyCrew) &&
         (ReadyBase == rhs.ReadyBase) && (ReadyVehic == rhs.ReadyVehic) &&
         (ReadyMaterial == rhs.ReadyMaterial) &&
         (BuildKnowledge == rhs.BuildKnowledge) &&
         (HomeBaseMaterial == rhs.HomeBaseMaterial) &&
         (HomeBaseProduction == rhs.HomeBaseProduction) && (Magic == rhs.Magic);
}

void C4SPlrStart::CompileFunc(StdCompiler *pComp) {
  pComp->Value(
      mkNamingAdapt(mkC4IDAdapt(NativeCrew), "StandardCrew", C4ID_None));
  ...
  pComp->Value(
      mkNamingAdapt(HomeBaseProduction, "HomeBaseProduction", C4IDList()));
...
void C4SLandscape::GetMapSize(int32_t &rWdt, int32_t &rHgt,
                              int32_t iPlayerNum) {
...
void C4SLandscape::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(
      mkNamingAdapt(VegLevel, "VegetationLevel", C4SVal(50, 30, 0, 100), true));
  pComp->Value(mkNamingAdapt(InEarth, "InEarth", C4IDList()));
  pComp->Value(
      mkNamingAdapt(InEarthLevel, "InEarthLevel", C4SVal(50, 0, 0, 100), true));
  ...
  pComp->Value(
      mkNamingAdapt(MapWdt, "MapWidth", C4SVal(100, 0, 64, 250), true));
  pComp->Value(
      mkNamingAdapt(MapHgt, "MapHeight", C4SVal(50, 0, 40, 250), true));
  ...
  pComp->Value(
      mkNamingAdapt(Gravity, "Gravity", C4SVal(100, 0, 10, 200), true));
  ...
  pComp->Value(mkNamingAdapt(
      FoWRes, "FoWRes", static_cast<int32_t>(CClrModAddMap::iDefResolutionX)));
}
...
void C4SWeather::CompileFunc(StdCompiler *pComp) {
  ...
  pComp->Value(
      mkNamingAdapt(mkStringAdaptMA(Precipitation), "Precipitation", "Water"));
...
BOOL C4Scenario::Compile(const char *szSource, bool fLoadSection) {
  ...
  return CompileFromBuf_LogWarn<StdCompilerINIRead>(
      mkParAdapt(*this, fLoadSection), StdStrBuf(szSource), C4CFN_ScenarioCore);
}

BOOL C4Scenario::Decompile(char **ppOutput, int32_t *ipSize,
                           bool fSaveSection) {
  try {
    // Decompile
    StdStrBuf Buf =
        DecompileToBuf<StdCompilerINIWrite>(mkParAdapt(*this, fSaveSection));
    // Return
...
void C4Scenario::SetExactLandscape() {
  ...
  // - because Map and Landscape are handled differently in NET2 (Map.bmp vs
  Landscape.bmp), and the zoomed Map.bmp may be used
  //   to reconstruct the textures on the Landscape.bmp in case of e.g. runtime
  joins. In this sense, C4S.Landscape.ExactLandscape
  //   only marks that the landscape.bmp is an exact one, and there may or may
  not be an accompanying Map.bmp
...
bool C4SDefinitions::GetModules(StdStrBuf *psOutModules) const {
  ...
  for (; cnt < C4S_MaxDefinitions; cnt++)
    if (Definition[cnt][0]) fSpecified = true;
  // No valid entries
...
void C4SDefinitions::SetModules(const char *szList,
  ...
  for (cnt = 0; cnt < C4S_MaxDefinitions; cnt++) {
    ...
    if (szRelativeToPath2 && *szRelativeToPath2)
      if (SEqualNoCase(Definition[cnt], szRelativeToPath2,
                       SLen(szRelativeToPath2)))
...
void C4SDefinitions::CompileFunc(StdCompiler *pComp) {
  ...
  for (int32_t i = 0; i < C4S_MaxDefinitions; i++)
    pComp->Value(mkNamingAdapt(mkStringAdaptMA(Definition[i]),
                               FormatString("Definition%i", i + 1).getData(),
                               ""));
...
void C4SGame::ConvertGoals(C4SRealism &rRealism) {
  ...
  if (rRealism.StructuresNeedEnergy) Rules.SetIDCount(C4Id("ENRG"), 1, TRUE);
  rRealism.StructuresNeedEnergy = 0;

  // Convert rules
  if (EnableRemoveFlag) Rules.SetIDCount(C4Id("FGRV"), 1, TRUE);
  EnableRemoveFlag = 0;
...
bool C4ScenarioSection::ScenarioLoad(char *szFilename) {
  ...
  if (Game.ScenarioFile.IsPacked())
    if (!EnsureTempStore(true, true)) return false;
  // donce, success
  return true;
}
...
C4Group *C4ScenarioSection::GetGroupfile(C4Group &rGrp) {
  // check temp filename
  if (szTempFilename)
    if (rGrp.Open(szTempFilename))
      return &rGrp;
    else
      return NULL;
  // check filename within scenario
  if (szFilename)
    if (rGrp.OpenAsChild(&Game.ScenarioFile, szFilename))
      return &rGrp;
    else
      return NULL;
