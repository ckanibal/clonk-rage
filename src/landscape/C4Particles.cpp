...
bool C4ParticleDefCore::Compile(char *szSource, const char *szName) {
  return CompileFromBuf_LogWarn<StdCompilerINIRead>(
      mkNamingAdapt(*this, "Particle"), StdStrBuf(szSource), szName);
}
...
C4ParticleDef::~C4ParticleDef() {
  ...
  if (pPrev)
    ...
    ParticleSystem.pDef0 = pNext;
  if (pNext)
    pNext->pPrev = pPrev;
  else
    ParticleSystem.pDefL = pPrev;
}

void C4ParticleDef::Clear() { Name.Clear(); }
...
bool C4ParticleDef::Load(C4Group &rGrp) {
  ...
  if (rGrp.LoadEntry(C4CFN_ParticleCore, &pSource, NULL, 1)) {
    ...
    if (GfxFace.Wdt)
      Gfx.Set(Gfx.Surface, GfxFace.x, GfxFace.y, GfxFace.Wdt, GfxFace.Hgt,
              GfxFace.tx, GfxFace.ty);
    // set phase num
    int32_t Q;
    Gfx.GetPhaseNum(Length, Q);
    if (!Length) {
    ...
    if (!(InitProc = ParticleSystem.GetProc(InitFn.getData()))) {
      DebugLogF("init proc for particle '%s' not found: '%s'", Name.getData(),
                InitFn.getData());
      return false;
    }
    if (!(ExecProc = ParticleSystem.GetProc(ExecFn.getData()))) {
      DebugLogF("exec proc for particle '%s' not found: '%s'", Name.getData(),
                ExecFn.getData());
      return false;
    }
    if (CollisionFn && CollisionFn[0])
      if (!(CollisionProc = ParticleSystem.GetProc(CollisionFn.getData()))) {
        DebugLogF("collision proc for particle '%s' not found: '%s'",
                  Name.getData(), CollisionFn.getData());
        return false;
      }
    if (!(DrawProc = ParticleSystem.GetDrawProc(DrawFn.getData()))) {
      DebugLogF("draw proc for particle '%s' not found: '%s'", Name.getData(),
                DrawFn.getData());
      return false;
    }
    ...
    if (pDefOverload = ParticleSystem.GetDef(Name.getData(), this)) {
      if (Config.Graphics.VerboseObjectLoading >= 1) {
        char ostr[250];
        sprintf(ostr, LoadResStr("IDS_PRC_DEFOVERLOAD"),
                pDefOverload->Name.getData(), "<particle>");
        Log(ostr);
      }
      delete pDefOverload;
    }
...
void C4ParticleSystem::ClearParticles() {
  ...
  // adjust counts
  for (C4ParticleDef *pDef = pDef0; pDef; pDef = pDef->pNext) pDef->Count = 0;
}
...
C4Particle *C4ParticleSystem::Create(C4ParticleDef *pOfDef, float x, float y,
                                     float xdir, float ydir, float a, int32_t b,
                                     C4ParticleList *pPxList, C4Object *pObj) {
  // safety
...
bool C4ParticleSystem::Cast(C4ParticleDef *pOfDef, int32_t iAmount, float x,
                            float y, int32_t level, float a0, DWORD b0,
                            float a1, DWORD b1, C4ParticleList *pPxList,
                            C4Object *pObj) {
  // safety
  ...
  DWORD db = b1 - b0;
  BYTE db1 = BYTE(db >> 24), db2 = BYTE(db >> 16), db3 = BYTE(db >> 8),
       db4 = BYTE(db);
  // create them
  for (int32_t i = iAmount; i > 0; --i)
    Create(pOfDef, x, y, (float)(SafeRandom(level + 1) - level / 2) / 10.0f,
           (float)(SafeRandom(level + 1) - level / 2) / 10.0f,
           (float)(iA0 + SafeRandom(iAd)) / 100.0f,
           b0 + (SafeRandom(db1) << 24) + (SafeRandom(db2) << 16) +
               (SafeRandom(db3) << 8) + SafeRandom(db4),
           pPxList, pObj);
  // success
  return true;
}
...
C4ParticleDef *C4ParticleSystem::GetDef(const char *szName,
                                        C4ParticleDef *pExclude) {
  // seek list
  for (C4ParticleDef *pDef = pDef0; pDef; pDef = pDef->pNext)
    if (pDef != pExclude && pDef->Name == szName) return pDef;
  // nothing found
  return NULL;
}
...
void C4ParticleSystem::SetDefParticles() {
  ...
  if (Config.Graphics.FireParticles) {
    ...
    pFire1 = pFire2 = NULL;
  // if fire is drawn w/o background fct: unload fire face if both fire
...
int32_t C4ParticleSystem::Push(C4ParticleDef *pOfDef, float dxdir,
                               float dydir) {
  int32_t iNumPushed = 0;
  // go through all particle chunks
  for (C4ParticleChunk *pChnk = &Chunk; pChnk; pChnk = pChnk->pNext) {
    // go through all particles
    C4Particle *pPrt = pChnk->Data;
    int32_t i = C4Px_BufSize;
    while (i--) {
      // def fits?
      if (!pOfDef || pPrt->pDef == pOfDef) {
        // push it!
...
bool fxSmokeExec(C4Particle *pPrt, C4Object *pTarget) {
  ...
  // color change
  DWORD dwClr = pPrt->b;
  pPrt->b = (LightenClrBy(dwClr, 1) & 0xffffff) |
            Min<int32_t>((dwClr >> 24) + 1, 255) << 24;
  // wind to float
  if (!(pPrt->b % 12) || fBuilding) {
    pPrt->xdir =
        0.025f * Game.Weather.GetWind(int32_t(pPrt->x), int32_t(pPrt->y));
    if (pPrt->xdir < -2.0f)
      pPrt->xdir = -2.0f;
    else if (pPrt->xdir > 2.0f)
      pPrt->xdir = 2.0f;
  ...
  if (GBackSolid(int32_t(pPrt->x), int32_t(pPrt->y - pPrt->a))) {
    ...
    if (!GBackSolid(int32_t(pPrt->x), int32_t(pPrt->y)))
      pPrt->y += 0.4f;
    else
      pPrt->a -= 2;
  } else
...
void fxSmokeDraw(C4Particle *pPrt, C4FacetEx &cgo, C4Object *pTarget) {
  ...
  pDef->Gfx.DrawX(cgo.Surface, int32_t(cx - pPrt->a), int32_t(cy - pPrt->a),
                  int32_t(pPrt->a * 2), int32_t(pPrt->a * 2), ipx, ipy);
...
bool fxStdExec(C4Particle *pPrt, C4Object *pTarget) {
  ...
  if (pPrt->xdir || pPrt->ydir) {
    if (pPrt->pDef->VertexCount &&
        GBackSolid(int32_t(dx + pPrt->xdir),
                   int32_t(dy + pPrt->ydir +
                           pPrt->pDef->VertexY * pPrt->a / 100.0f))) {
      // collision
  ...
  // apply gravity
  if (pPrt->pDef->GravityAcc)
    pPrt->ydir += fixtof(GravAccel * pPrt->pDef->GravityAcc) / 100.0f;
  // apply WindDrift
  ...
  bool kp;
  if (dxdir > 0)
    kp = (dx - pPrt->a < GBackWdt);
  else
    kp = (dx + pPrt->a > 0);
  if (dydir > 0)
    kp = kp && (dy - pPrt->a < GBackHgt);
  else
    kp = kp && (dy + pPrt->a > pPrt->pDef->YOff);
  return kp;
}
...
void fxStdDraw(C4Particle *pPrt, C4FacetEx &cgo, C4Object *pTarget) {
  ...
  if (pDef->RByV == 3)  // random rotation - currently a pseudo random rotation
                        // by x/y position
  ...
  if (r)
    pDef->Gfx.DrawXR(cgo.Surface, cx - iDrawWdt, cy - iDrawHgt, iDrawWdt * 2,
                     iDrawHgt * 2, iPhase, 0, r);
  else
    pDef->Gfx.DrawX(cgo.Surface, cx - iDrawWdt, cy - iDrawHgt, iDrawWdt * 2,
                    iDrawHgt * 2, iPhase, 0);
...
C4ParticleProcRec C4ParticleProcMap[] = {{"SmokeInit", fxSmokeInit},
...
C4ParticleDrawProcRec C4ParticleDrawProcMap[] = {
    {"Smoke", fxSmokeDraw}, {"Std", fxStdDraw}, {"", 0}};
