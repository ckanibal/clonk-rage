...
typedef bool (*C4ParticleProc)(C4Particle *,
                               C4Object *);  // generic particle proc
typedef C4ParticleProc C4ParticleInitProc;   // particle init proc - init and
                                             // return whether particle could be
                                             // created
typedef C4ParticleProc C4ParticleExecProc;  // particle execution proc - returns
                                            // whether particle died
typedef C4ParticleProc C4ParticleCollisionProc;  // particle collision proc -
                                                 // returns whether particle
                                                 // died
typedef void (*C4ParticleDrawProc)(C4Particle *, C4FacetEx &,
                                   C4Object *);  // particle drawing code
...
class C4ParticleDefCore {
 public:
  ...
  int32_t
      MaxCount;  // maximum number of particles that may coexist of this type
  int32_t MinLifetime, MaxLifetime;  // used by exec proc; number of frames this
                                     // particle can exist
  ...
  int32_t FadeOutLen,
      FadeOutDelay;  // phases used for letting the particle fade out
  int32_t RByV;  // if set, rotation will be adjusted according to the movement;
                 // if 2, the particle does not move
  ...
  StdStrBuf CollisionFn;  // proc to be called upon collision with the
                          // landscape; may be left out
...
class C4ParticleDef : public C4ParticleDefCore {
 public:
  ...
  StdStrBuf Filename;  // path to group this particle was loaded from (for EM
                       // reloading)
  ...
  C4ParticleInitProc
      InitProc;  // procedure called once upon creation of the particle
  C4ParticleExecProc ExecProc;  // procedure used for execution of one particle
  C4ParticleCollisionProc CollisionProc;  // procedure called upon collision
                                          // with the landscape; may be NULL
  ...
  bool Load(C4Group &rGrp);  // load particle from group; assume file to be
                             // accessed already
...
class C4Particle {
 protected:
  C4Particle *pPrev,
      *pNext;  // previous/next particle of the same list in the buffer

  void MoveList(C4ParticleList &rFrom,
                C4ParticleList &rTo);  // move from one list to another
...
class C4ParticleList {
 public:
  ...
  operator bool() {
    return !!pFirst;
  }  // checks whether list contains particles
};
...
class C4ParticleSystem {
 protected:
  ...
  C4ParticleProc GetProc(
      const char *szName);  // get init/exec proc for a particle type
  C4ParticleDrawProc GetDrawProc(
      const char *szName);  // get draw proc for a particle type
 ...
 public:
  ...
  C4Particle *Create(
      C4ParticleDef *pOfDef,  // create one particle of given type
      float x, float y, float xdir = 0.0f, float ydir = 0.0f, float a = 0.0f,
      int32_t b = 0, C4ParticleList *pPxList = NULL, C4Object *pObj = NULL);
  bool Cast(C4ParticleDef *pOfDef,  // create several particles with different
                                    // speeds and params
            int32_t iAmount, float x, float y, int32_t level, float a0 = 0.0f,
            DWORD b0 = 0, float a1 = 0.0f, DWORD b1 = 0,
            C4ParticleList *pPxList = NULL, C4Object *pObj = NULL);

  C4ParticleDef *GetDef(
      const char *szName,
      C4ParticleDef *pExclude = NULL);  // get particle def by name
  void SetDefParticles();  // seek and assign default particels (smoke, etc.)

  int32_t Push(C4ParticleDef *pOfDef, float dxdir,
               float dydir);  // add movement to all particles of type
...
extern C4ParticleProcRec
    C4ParticleProcMap[];  // particle init/execution function map
extern C4ParticleDrawProcRec
    C4ParticleDrawProcMap[];  // particle drawing function map
