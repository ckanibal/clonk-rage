...
void C4PXS::Execute() {
  ...
  if (pReact && (*pReact->pFunc)(pReact, iX, iY, iX, iY, xdir, ydir, Mat, inmat,
                                 meePXSPos, NULL)) {
    Deactivate();
    return;
  }
  ...
  // In bounds?
  if (Inside<int32_t>(iToX, 0, GBackWdt - 1) &&
      Inside<int32_t>(iToY, 0, GBackHgt - 1))
    // Check path
  ...
  do {
    ...
    C4MaterialReaction *pReact = Game.Material.GetReactionUnsafe(Mat, inmat);
    if (pReact)
      if ((*pReact->pFunc)(pReact, iX, iY, inX, inY, xdir, ydir, Mat, inmat,
                           meePXSMove, &fStopMovement)) {
        // destructive contact
        Deactivate();
        return;
      } else {
        // no destructive contact, but speed or position changed: Stop moving
        // for now
        if (fStopMovement) {
          x = itofix(iX);
          y = itofix(iY);
        ...
        // there was a reaction func, but it didn't do anything - continue
        // movement
      }
    iX = inX;
    iY = inY;
...
BOOL C4PXSSystem::Create(int32_t mat, FIXED ix, FIXED iy, FIXED ixdir,
                         FIXED iydir) {
  C4PXS *pxp;
...
// Lock primary surface
#ifdef USE_DIRECTX
  if (pD3D) Application.DDraw->lpBack->Lock();
#endif
  ...
  for (cnt = 0; cnt < PXSMaxChunk; cnt++)
    if (Chunk[cnt] && iChunkPXS[cnt]) {
      ...
      for (unsigned int cnt2 = 0; cnt2 < PXSChunkSize; cnt2++, pxp++)
        if (pxp->Mat != MNone &&
            VisibleRect.Contains(fixtoi(pxp->x), fixtoi(pxp->y))) {
          C4Material *pMat = &Game.Material.Map[pxp->Mat];
          if (pMat->PXSFace.Surface && Config.Graphics.PXSGfx) continue;
          // old-style: unicolored pixels or lines
          DWORD dwMatClr = Game.Landscape.GetPal()->GetClr(
              (BYTE)(Mat2PixColDefault(pxp->Mat)));
          if (fixtoi(pxp->xdir) || fixtoi(pxp->ydir)) {
            // lines for stuff that goes whooosh!
            int len = fixtoi(Abs(pxp->xdir) + Abs(pxp->ydir));
            dwMatClr = uint32_t(Max<int>(dwMatClr >> 24,
                                         195 - (195 - (dwMatClr >> 24)) / len))
                           << 24 |
                       (dwMatClr & 0xffffff);
            Application.DDraw->DrawLineDw(
                cgo.Surface, fixtof(pxp->x - pxp->xdir) + cgox,
                fixtof(pxp->y - pxp->ydir) + cgoy, fixtof(pxp->x) + cgox,
                fixtof(pxp->y) + cgoy, dwMatClr);
          } else
            // single pixels for slow stuff
            Application.DDraw->DrawPix(cgo.Surface, fixtof(pxp->x) + cgox,
                                       fixtof(pxp->y) + cgoy, dwMatClr);
        }
    }
...
#ifdef USE_DIRECTX
  if (pD3D) Application.DDraw->lpBack->Unlock();
#endif

  // PXS graphics disabled?
  if (!Config.Graphics.PXSGfx) return;
    ...
    if (Chunk[cnt] && iChunkPXS[cnt]) {
      ...
      for (unsigned int cnt2 = 0; cnt2 < PXSChunkSize; cnt2++, pxp++)
        if (pxp->Mat != MNone &&
            VisibleRect.Contains(fixtoi(pxp->x), fixtoi(pxp->y))) {
          C4Material *pMat = &Game.Material.Map[pxp->Mat];
          if (!pMat->PXSFace.Surface) continue;
          // new-style: graphics
          int32_t pnx, pny;
          pMat->PXSFace.GetPhaseNum(pnx, pny);
          int32_t fcWdt = pMat->PXSFace.Wdt;
          int32_t fcWdtH = Max(fcWdt / 3, 1);
          // calculate draw width and tile to use (random-ish)
          int32_t z =
              1 +
              ((cnt2 / Max<int32_t>(pnx * pny, 1)) ^ 341) % pMat->PXSGfxSize;
          pny = (cnt2 / pnx) % pny;
          pnx = cnt2 % pnx;
          // draw
          Application.DDraw->ActivateBlitModulation(
              Min((fcWdtH - z) * 16, 255) << 24 | 0xffffff);
          pMat->PXSFace.DrawX(
              cgo.Surface,
              fixtoi(pxp->x) + cgox + z * pMat->PXSGfxRt.tx / fcWdt,
              fixtoi(pxp->y) + cgoy + z * pMat->PXSGfxRt.ty / fcWdt, z,
              z * pMat->PXSFace.Hgt / fcWdt, pnx, pny);
          Application.DDraw->DeactivateBlitModulation();
        }
    }
}

void C4PXSSystem::Cast(int32_t mat, int32_t num, int32_t tx, int32_t ty,
                       int32_t level) {
  int32_t cnt;
  for (cnt = 0; cnt < num; cnt++)
    Create(mat, itofix(tx), itofix(ty),
...
BOOL C4PXSSystem::Save(C4Group &hGroup) {
  ...
  int32_t iChunks = 0;
  for (cnt = 0; cnt < PXSMaxChunk; cnt++)
    if (Chunk[cnt] && iChunkPXS[cnt]) iChunks++;
  if (!iChunks) {
  ...
  CStdFile hTempFile;
  if (!hTempFile.Create(Config.AtTempPath(C4CFN_TempPXS))) return FALSE;
#ifdef USE_FIXED
  int32_t iNumFormat = 1;
#else
  int32_t iNumFormat = 2;
#endif
  if (!hTempFile.Write(&iNumFormat, sizeof(iNumFormat))) return FALSE;
  for (cnt = 0; cnt < PXSMaxChunk; cnt++)
    if (Chunk[cnt])  // must save all chunks in order to keep order consistent
                     // on all clients
      if (!hTempFile.Write(Chunk[cnt], PXSChunkSize * sizeof(C4PXS)))
        return FALSE;

  if (!hTempFile.Close()) return FALSE;

  // Move temp file to group
  if (!hGroup.Move(Config.AtTempPath(C4CFN_TempPXS), C4CFN_PXS)) return FALSE;
...
#ifdef USE_FIXED
        if (iNumForm == 2) {
          FLOAT_TO_FIXED(&pxp->x);
          FLOAT_TO_FIXED(&pxp->y);
          FLOAT_TO_FIXED(&pxp->xdir);
          FLOAT_TO_FIXED(&pxp->ydir);
        }
#else
        if (iNumForm == 1) {
          FIXED_TO_FLOAT(&pxp->x);
          FIXED_TO_FLOAT(&pxp->y);
          FIXED_TO_FLOAT(&pxp->xdir);
          FIXED_TO_FLOAT(&pxp->ydir);
        }
#endif
      }
  }
...
void C4PXSSystem::Delete(C4PXS *pPXS) {
  ...
  for (cnt = 0; cnt < PXSMaxChunk; cnt++)
    if (Chunk[cnt] && iChunkPXS[cnt])
      if (pPXS >= Chunk[cnt] && pPXS < Chunk[cnt] + PXSChunkSize) break;
  // decrease pxs counter
  if (cnt < PXSMaxChunk) iChunkPXS[cnt]--;
}
