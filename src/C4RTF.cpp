...
C4RTFFile::~C4RTFFile() { ClearState(); }
...
void C4RTFFile::TranslateKeyword(StdStrBuf &sResult, const char *szKeyword,
                                 int iParam, bool fHasIntParam) {
  // get keyword from table
  KeywordTableEntry *pKw = RTFKeywordTable;
  while (pKw->szKeyword)
    if (!strcmp(szKeyword, pKw->szKeyword))
      break;
    else
      ++pKw;
  // no found?
  ...
  switch (pKw->eType) {
    case KeywordTableEntry::kwdProp:
      ...
      ApplyPropChange(pKw->idx, (pKw->fForceDefaultParam || !fHasIntParam)
                                    ? pKw->iDefaultParam
                                    : iParam);
      break;
...
void C4RTFFile::ParseKeyword(StdStrBuf &sResult, size_t &iPos) {
  ...
  *szParameter = 0;
  ...
  if (!isalpha((unsigned char)c)) {
    ...
    do {
      *szWrite = c;
      // do not overflow buffer - longer keywords will be read, not recognized
    ...
    *szWrite = 0;
    ...
    if (isdigit((unsigned char)c)) {
      ...
      do {
        *szWrite = c;
        // do not overflow buffer - longer parameters will be read, not
    ...
    // if next char is not a spacing for the command, it does not belong to the
    // keyword and must be re-parsed
    if (c != ' ') --iPos;
  }
