...
bool StdSchedulerProc::ExecuteUntil(int iTimeout) {
  ...
  for (;;) {
    ...
    if (!Execute(Max(iTimeout, 0))) return false;
    // Calculate timeout
    unsigned int iTime = timeGetTime();
    if (iTime >= iStopTime) break;
...
static bool FD_INTERSECTS(int n, fd_set *a, fd_set *b) {
  for (int i = 0; i < n; ++i)
    if (FD_ISSET(i, a) && FD_ISSET(i, b)) return true;
  return false;
}
#endif
...
StdScheduler::StdScheduler() : ppProcs(NULL), iProcCnt(0), iProcCapacity(0) {
#ifdef STDSCHEDULER_USE_EVENTS
...
void StdScheduler::Set(StdSchedulerProc **ppnProcs, int inProcCnt) {
  ...
  // Copy new
  iProcCnt = inProcCnt;
  for (int i = 0; i < iProcCnt; i++) ppProcs[i] = ppnProcs[i];
}
...
bool StdScheduler::Execute(int iTimeout) {
  ...
  for (i = 0; i < iProcCnt; i++)
    if ((iProcTimeout = ppProcs[i]->GetTimeout()) >= 0)
      if (iTimeout == -1 || iTimeout > iProcTimeout) iTimeout = iProcTimeout;
  ...
  DWORD ret = WaitForMultipleObjects(iEventCnt, pEventHandles, FALSE,
                                     iTimeout < 0 ? INFINITE : iTimeout);
  ...
  for (i = 0; i < iProcCnt; i++) ppProcs[i]->GetFDs(fds, &iMaxFDs);
  ...
  // Wait for something to happen
  int cnt =
      select(iMaxFDs + 1, &fds[0], &fds[1], NULL, iTimeout < 0 ? NULL : &to);
...
void StdScheduler::Enlarge(int iBy) {
  ...
  // Set data
  for (int i = 0; i < iProcCnt; i++) ppnProcs[i] = ppProcs[i];
  delete[] ppProcs;
  ppProcs = ppnProcs;
#ifdef STDSCHEDULER_USE_EVENTS
  // Allocate dummy arrays (one handle neede for unlocker!)
  delete[] pEventHandles;
  pEventHandles = new HANDLE[iProcCapacity + 1];
  delete[] ppEventProcs;
  ppEventProcs = new StdSchedulerProc *[iProcCapacity];
#endif
}

// *** StdSchedulerThread

StdSchedulerThread::StdSchedulerThread() : fThread(false) {}

StdSchedulerThread::~StdSchedulerThread() { Clear(); }
...
bool StdThread::IsStopSignaled() { return fStopSignaled; }
