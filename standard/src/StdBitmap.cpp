...
BOOL CStdBitmap::Create(int iWdt, int iHgt, int iBitsPerPixel) {
  // Init head
  int coloroffset = 0;
  if (iBitsPerPixel == 8) coloroffset = 256 * sizeof(RGBQUAD);
...
int CStdBitmap::getWidth() { return Info.biWidth; }

int CStdBitmap::getHeight() { return Info.biHeight; }

int CStdBitmap::getPitch() { return Info.Pitch(); }

BOOL CStdBitmap::Ensure(int iWdt, int iHgt, int iBitsPerPixel) {
  // Create new
  if (!Bits) return Create(iWdt, iHgt, iBitsPerPixel);
