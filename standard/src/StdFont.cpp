 ...
 public:
  CStdVectorFont(const char *filepathname) {
    ...
    if (e = FT_New_Face(library, filepathname, 0, &face))
      throw std::runtime_error(std::string("Cannot load ") + filepathname +
                               ": " + FormatString("%d", e).getData());
  }
  CStdVectorFont(const StdBuf &Data) {
    ...
    // Load the font
    FT_Error e;
    if (e = FT_New_Memory_Face(library,
                               static_cast<const FT_Byte *>(Data.getData()),
                               Data.getSize(), 0, &face))
      throw std::runtime_error(std::string("Cannot load font: ") +
                               FormatString("%d", e).getData());
  }
  ~CStdVectorFont() {
    FT_Done_Face(face);
...
void CStdFont::DestroyFont(CStdVectorFont *pFont) { delete pFont; }
#elif(defined _WIN32)
class CStdVectorFont {
 private:
  StdStrBuf sFontName;
 ...
 public:
  CStdVectorFont(const char *name) { sFontName.Copy(name); }
...
void CStdFont::DestroyFont(CStdVectorFont *pFont) { delete pFont; }
#else
CStdVectorFont *CStdFont::CreateFont(const StdBuf &Data) { return 0; }
...
CStdFont::CStdFont() {
  ...
  iFontZoom = 1;  // default: no internal font zooming - likely no antialiasing
                  // either...
  iHSpace = -1;
...
bool CStdFont::AddSurface() {
  ...
  if (iNumFontSfcs)
    memcpy(pNewSfcs, psfcFontData, iNumFontSfcs * sizeof(CSurface *));
  delete[] psfcFontData;
  psfcFontData = pNewSfcs;
  CSurface *sfcNew = psfcFontData[iNumFontSfcs] = new CSurface();
  ++iNumFontSfcs;
  if (iSfcSizes)
    if (!sfcNew->Create(iSfcSizes, iSfcSizes)) return false;
  sfcCurrent = sfcNew;
...
bool CStdFont::CheckRenderedCharSpace(uint32_t iCharWdt, uint32_t iCharHgt) {
  ...
  if (iCurrentSfcX + iCharWdt >= (uint32_t)iSfcSizes)
    if (iCurrentSfcX) {
      iCurrentSfcX = 0;
...
bool CStdFont::AddRenderedChar(uint32_t dwChar, CFacet *pfctTarget) {
  ...
  for (int y = 0; y < size.cy; ++y)
    for (int x = 0; x < size.cx; ++x) {
      ...
      if (x && y && fDoShadow)
        bAlphaShadow =
            255 -
            (BYTE)((pBitmapBits[iBitmapSize * (y - 1) + x - 1] & 0xff) * 1 / 1);
      else
        bAlphaShadow = 255;
  ...
  if (FT_Load_Char(*pVectorFont, dwChar, FT_LOAD_RENDER | FT_LOAD_NO_HINTING)) {
    // although the character was not drawn, assume it's not in the font and
    // won't be needed
  ...
  if (slot->bitmap.pixel_mode != FT_PIXEL_MODE_GRAY) {
    // although the character was drawn in a strange way, assume it's not in the
    // font and won't be needed
    // so return success here
    return true;
  }
  // linebreak/ new surface check
  int width = Max<int>(slot->advance.x / 64,
                       Max(slot->bitmap_left, 0) + slot->bitmap.width) +
              fDoShadow;
  if (!CheckRenderedCharSpace(width, iGfxLineHgt)) return false;
  // offset from the top
  int at_y = iCurrentSfcY +
             dwDefFontHeight * (*pVectorFont)->ascender /
                 (*pVectorFont)->units_per_EM -
             slot->bitmap_top;
  ...
  for (int y = 0; y < slot->bitmap.rows + fDoShadow; ++y) {
    for (int x = 0; x < slot->bitmap.width + fDoShadow; ++x) {
      ...
      if (x < slot->bitmap.width && y < slot->bitmap.rows)
        ...
        bAlpha = 255;
      // Make a shadow from the upper-left pixel, and blur with the eight
      // neighbors
      DWORD dwPixVal = 0u;
      bAlphaShadow = 255;
      if ((x || y) && fDoShadow) {
        ...
        if (x < slot->bitmap.width && y < slot->bitmap.rows)
          iShadow +=
              slot->bitmap.buffer[(x - 0) + slot->bitmap.width * (y - 0)];
        if (x > 1 && y < slot->bitmap.rows)
          iShadow +=
              slot->bitmap.buffer[(x - 2) + slot->bitmap.width * (y - 0)];
        if (x > 0 && y < slot->bitmap.rows)
          iShadow +=
              slot->bitmap.buffer[(x - 1) + slot->bitmap.width * (y - 0)];
        if (x < slot->bitmap.width && y > 1)
          iShadow +=
              slot->bitmap.buffer[(x - 0) + slot->bitmap.width * (y - 2)];
        if (x > 1 && y > 1)
          iShadow +=
              slot->bitmap.buffer[(x - 2) + slot->bitmap.width * (y - 2)];
        if (x > 0 && y > 1)
          iShadow +=
              slot->bitmap.buffer[(x - 1) + slot->bitmap.width * (y - 2)];
        if (x < slot->bitmap.width && y > 0)
          iShadow +=
              slot->bitmap.buffer[(x - 0) + slot->bitmap.width * (y - 1)];
        if (x > 1 && y > 0)
          iShadow +=
              slot->bitmap.buffer[(x - 2) + slot->bitmap.width * (y - 1)];
        if (x > 0 && y > 0)
          iShadow +=
              slot->bitmap.buffer[(x - 1) + slot->bitmap.width * (y - 1)] * 8;
        bAlphaShadow -= iShadow / 16;
        // because blitting on a black pixel reduces luminosity as compared to
        // shadowless font,
        // assume luminosity as if blitting shadowless font on a 50% gray
        // background
...
uint32_t CStdFont::GetNextUTF8Character(const char **pszString) {
  ...
  if (c > 191 && c < 224) {
    ...
    dwResult = (int(c & 15) << 12) | (int(c2 & 63) << 6) |
               int(c3 & 63);  // three char code
  } else if (c >= 240 && c <= 247) {
    unsigned char c2 = *szString++;
    if ((c2 & 192) != 128) {
      *pszString = szString;
    ...
    dwResult = (int(c & 7) << 18) | (int(c2 & 63) << 12) | (int(c3 & 63) << 6) |
               int(c4 & 63);  // four char code
  }
  *pszString = szString;
  return dwResult;
}
...
CFacet &CStdFont::GetUnicodeCharacterFacet(uint32_t c) {
  ...
  // rendering might have failed, in which case rFacet remains empty. Should be
  // OK; char won't be printed then
  return rFacet;
}

void CStdFont::Init(CStdVectorFont &VectorFont, DWORD dwHeight,
                    DWORD dwFontWeight, const char *szCharset, bool fDoShadow) {
  // clear previous
  Clear();
  // set values
  ...
  if (!AddSurface()) {
    Clear();
    throw std::runtime_error(std::string("Cannot create surface (") +
                             szFontName + ")");
  }
  ...
  if (!hDC) {
    Clear();
    throw std::runtime_error(std::string("Cannot create DC (") + szFontName +
                             ")");
  }
  hbmBitmap = CreateDIBSection(hDC, &bmi, DIB_RGB_COLORS, (VOID **)&pBitmapBits,
                               NULL, 0);
  if (!hbmBitmap) {
    Clear();
    throw std::runtime_error(std::string("Cannot create DIBSection (") +
                             szFontName + ")");
  }
  ...
  if (szFontName && *szFontName)
    szFontName2 = szFontName;
  else
    szFontName2 = "Comic Sans MS";
  int iFontHeight =
      dwDefFontHeight * GetDeviceCaps(hDC, LOGPIXELSY) * iFontZoom / 72;
  hFont = ::CreateFont(iFontHeight, 0, 0, 0, dwFontWeight, FALSE, FALSE, FALSE,
                       bCharset, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, 5,
                       VARIABLE_PITCH, szFontName2);

  // ClearType failed: try antialiased (not guaranteed)
  if (!hFont)
    hFont =
        ::CreateFont(iFontHeight, 0, 0, 0, dwFontWeight, FALSE, FALSE, FALSE,
                     bCharset, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                     ANTIALIASED_QUALITY, VARIABLE_PITCH, szFontName2);
  ...
  iLineHgt = (VectorFont->ascender - VectorFont->descender) * dwHeight /
             VectorFont->units_per_EM;
  iGfxLineHgt = iLineHgt + fDoShadow;  // vertical shadow
#else
...
// in case of UTF8, unicode characters will be created on the fly and extended
// ASCII characters (128-255) are not needed
// now render all characters!
  ...
  struct iconv_t_wrapper {
    ...
    iconv_t_wrapper(const char *to, const char *from) {
      ...
      if (i == iconv_t(-1))
        throw std::runtime_error(std::string("Cannot open iconv (") + to +
                                 ", " + from + ")");
    }
  ...
  for (int c = ' '; c <= cMax; ++c) {
    ...
    if (!fUTF8) {
      ...
      size_t insize = 1;
      size_t outsize = 4;
      iconv(iconv_handle, const_cast<ICONV_CONST char **>(&in), &insize, &out,
            &outsize);
    }
#elif defined(_WIN32)
    ...
    if (!fUTF8 && c >= 128) {
      char cc[2] = {c, '\0'};
      wchar_t outbuf[4];
      if (MultiByteToWideChar(iCodePage, 0, cc, -1, outbuf,
                              4))  // 2do: Convert using proper codepage
      {
        // now convert from UTF-16 to UCS-4
        if (((outbuf[0] & 0xfc00) == 0xd800) &&
            ((outbuf[1] & 0xfc00) == 0xdc00)) {
          dwChar =
              0x10000 + (((outbuf[0] & 0x3ff) << 10) | (outbuf[1] & 0x3ff));
        } else
          dwChar = outbuf[0];
      } else {
        // conversion error. Shouldn't be fatal; just pretend it were a Unicode
        // character
      }
    }
#else
// no conversion available? Just break for non-iso8859-1.
    ...
    if (!AddRenderedChar(dwChar, &(fctAsciiTexCoords[c - ' ']))) {
      ...
      throw std::runtime_error(
          std::string("Cannot render characters for Font (") + szFontName +
          ")");
    }
  }
  // adjust line height
  ...
  fPrerenderedFont = false;
  if (0)
    for (int i = 0; i < iNumFontSfcs; ++i) {
      StdStrBuf pngfilename = FormatString(
          "%s%i%s_%d.png", szFontName, dwHeight, fDoShadow ? "_shadow" : "", i);
      psfcFontData[i]->SavePNG(pngfilename.getData(), true, false, false);
    }
}

const DWORD FontDelimeterColor = 0xff0000, FontDelimiterColorLB = 0x00ff00,
...
void CStdFont::Init(const char *szFontName, CSurface *psfcFontSfc,
                    int iIndent) {
  // clear previous
  Clear();
  // grab surface
  ...
  if (!AddSurface()) {
    Clear();
    throw std::runtime_error(std::string("Error creating surface for ") +
                             szFontName);
  }
  ...
  while (iGfxLineHgt < sfcCurrent->Hgt) {
    ...
    if (ColorMatch(dwPix, FontDelimeterColor) ||
        ColorMatch(dwPix, FontDelimiterColorLB) ||
        ColorMatch(dwPix, FontDelimeterColorIndent1) ||
        ColorMatch(dwPix, FontDelimeterColorIndent2))
      break;
    ++iGfxLineHgt;
  }
  ...
  for (int c = ' '; c < 256; ++c) {
    ...
    while (iX < sfcCurrent->Wdt) {
      ...
      if (ColorMatch(dwPix, FontDelimeterColor) ||
          ...
          ColorMatch(dwPix, FontDelimeterColorIndent2))
        break;
      if (ColorMatch(dwPix, FontDelimiterColorLB)) {
        IsLB = true;
        break;
      }
      ++iX;
...
bool CStdFont::GetTextExtent(const char *szText, int32_t &rsx, int32_t &rsy,
                             bool fCheckMarkup) {
  // safety
  ...
  while (*szText) {
    ...
    // image?
    int iImgLgt;
    if (fCheckMarkup && c == '{' && szText[0] == '{' && szText[1] != '{' &&
        (iImgLgt = SCharPos('}', szText + 1)) > 0 &&
        szText[iImgLgt + 2] == '}') {
      char imgbuf[101];
      ...
      if (fct.Hgt) {
        ...
        // image renderer not hooked or ID not found, or surface not present:
        // just ignore it
...
int CStdFont::BreakMessage(const char *szMsg, int iWdt, char *szOut,
                           int iMaxOutLen, bool fCheckMarkup, float fZoom) {
  ...
  const char *szPos = szMsg,    // current parse position in the text
      *szLastBreakPos = szMsg,  // points to the char after at (whitespace) or
                                // after ('-') which text can be broken
      *szLastEmergenyBreakPos,  // same, but at last char in case no suitable
                                // linebreak could be found
      *szLastPos;  // last position until which buffer has been transferred to
                   // output
  char *szLastBreakOut,
      *szLastEmergencyBreakOut;  // position of output pointer at break
                                 // positions
  int iX = 0,                    // current text width at parse pos
      iXBreak = 0,               // text width as it was at last break pos
      iXEmergencyBreak,  // same, but at last char in case no suitable linebreak
                         // could be found
  ...
  while (*(szLastPos = szPos)) {
    ...
    if (c != '\n' && (!fCheckMarkup || c != '|')) {
      // image?
      int iImgLgt;
      if (fCheckMarkup && c == '{' && szPos[0] == '{' && szPos[1] != '{' &&
          (iImgLgt = SCharPos('}', szPos + 1)) > 0 &&
          szPos[iImgLgt + 2] == '}') {
        char imgbuf[101];
        ...
        if (pCustomImages)
          ...
          pCustomImages->GetFontImage(imgbuf, fct);
        if (fct.Hgt) {
          // image found: adjust aspect by font height and calc appropriate
          // width
        ...
        if (c >= ' ')
          iCharWdt =
              int(fZoom * GetCharacterFacet(c).Wdt / iFontZoom) + iHSpace;
        else
      ...
      if ((iX += iCharWdt) <= iWdt || fIsFirstLineChar) {
        ...
        if (c < 256)
          if (isspace((unsigned char)c) || c == '-') {
            ...
            // first char spaces must remain, in case the output area is just
            // one char width
            if (c != '-' && !fIsFirstLineChar)
              --szLastBreakPos;  // because c<256, the character length can be
                                 // safely assumed to be 1 here
            iXBreak = iX;
          }
      ...
      if (uint8_t(*szLastBreakPos) < 128 &&
          isspace((unsigned char)*szLastBreakPos))
        *(szLastBreakOut - 1) = '\n';
      else {
        ...
        if (!--iMaxOutLen)
          ...
          break;
        char *szOut2 = szOut;
        while (--szOut2 >= szLastBreakOut) szOut2[1] = *szOut2;
        szOut2[1] = '\n';
      }
    ...
    // forced or manual line break: set new line beginning to char after line
    // break
...
int CStdFont::BreakMessage(const char *szMsg, int iWdt, StdStrBuf *pOut,
                           bool fCheckMarkup, float fZoom) {
  // safety
  ...
  const char *szPos = szMsg,    // current parse position in the text
      ...
      *szLastPos;  // last position until which buffer has been transferred to
                   // output
  int iLastBreakOutLen,
      iLastEmergencyBreakOutLen;  // size of output string at break positions
  int iX = 0,                     // current text width at parse pos
      iXBreak = 0,                // text width as it was at last break pos
      iXEmergencyBreak,  // same, but at last char in case no suitable linebreak
                         // could be found
  ...
  while (*(szLastPos = szPos)) {
    ...
    if (c != '\n' && (!fCheckMarkup || c != '|')) {
      ...
      // only check for space and not for other breakable characters (such as
      // '-'), because the break would happen after those characters instead of
      // at them
...
// 2do: Function not ready for UTF-8, markup or inline images. Remove its usage
// using standardized BreakMessage
int CStdFont::GetMessageBreak(const char *szMsg, const char **ppNewPos,
                              int iBreakWidth, float fZoom) {
  // safety
  if (!szMsg || !*szMsg) {
    *ppNewPos = szMsg;
    return 0;
  }
  const char *szPos = szMsg;
  unsigned char c;
  int iWdt = 0;
  int iPos = 0;
  ...
  while (c = *szPos++) {
    ...
    // get char width
    int iCharWdt =
        int(fZoom * fctAsciiTexCoords[c - ' '].Wdt / iFontZoom) + iHSpace;
...
void CStdFont::DrawText(SURFACE sfcDest, int iX, int iY, DWORD dwColor,
  ...
  DWORD dwAlphaMod =
      BoundBy<int>((((int)(dwColor >> 0x18) - 0x50) * 0xff) / 0xaf, 0, 255)
          << 0x18 |
      0xffffff;
  /*	char TEXT[8192];
          sprintf(TEXT, "%s(%x-%x-%x)", szText, dwAlphaMod>>0x18, dwColor>>0x15,
     (((int)(dwColor>>0x15)-0x50)*0xff)/0xaf); szText=TEXT;*/
  ...
  while (c = GetNextCharacter(&szText)) {
    ...
    if (c == '<' && (~dwFlags & STDFONT_NOMARKUP)) {
      ...
      if (Markup.Read(&--szText)) {
        ...
        // (done only if tag was found, so most normal blits don't init a
    ...
    // custom image?
    int iImgLgt;
    if (c == '{' && szText[0] == '{' && szText[1] != '{' &&
        (iImgLgt = SCharPos('}', szText + 1)) > 0 &&
        szText[iImgLgt + 2] == '}' && !(dwFlags & STDFONT_NOMARKUP)) {
      fctFromBlt.Default();
      char imgbuf[101];
      SCopy(szText + 1, imgbuf, Min(iImgLgt, 100));
      szText += iImgLgt + 3;
      ...
      w2 = int(fctFromBlt.Wdt * fZoom);
      h2 = int(fctFromBlt.Hgt * fZoom);
    ...
    lpDDraw->Blit(fctFromBlt.Surface, float(fctFromBlt.X), float(fctFromBlt.Y),
                  float(fctFromBlt.Wdt), float(fctFromBlt.Hgt), sfcDest, iX, iY,
                  w2, h2, TRUE, pbt);
...
BYTE GetCharsetCode(const char *strCharset) {
  ...
  if (SEqualNoCase(strCharset, "CHINESEBIG5"))
    return 136;                                         // CHINESEBIG5_CHARSET
  ...
  if (SEqualNoCase(strCharset, "UTF-8"))
    return 0;  // ANSI_CHARSET - UTF8 needs special handling
  // Default
