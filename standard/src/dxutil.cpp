...
const TCHAR *DXUtil_GetDXSDKMediaPath() {
  ...
  lResult = RegQueryValueEx(hKey, _T("DX8SDK Samples Path"), NULL, &dwType,
                            (BYTE *)strPath, &dwSize);
  RegCloseKey(hKey);
...
FLOAT __stdcall DXUtil_Timer(TIMER_COMMAND command) {
  ...
  if (FALSE == m_bTimerInitialized) {
    ...
    if (m_bUsingQPF) m_llQPFTicksPerSec = qwTicksPerSec.QuadPart;
  }
...
VOID DXUtil_ConvertWideStringToAnsi(CHAR *strDestination,
  ...
  if (strDestination == NULL || wstrSource == NULL) return;
...
VOID DXUtil_ConvertGenericStringToAnsi(CHAR *strDestination,
                                       ...
                                       int cchDestChar) {
  if (strDestination == NULL || tstrSource == NULL) return;
...
VOID DXUtil_ConvertGenericStringToWide(WCHAR *wstrDestination,
                                       const TCHAR *tstrSource,
                                       int cchDestChar) {
  if (wstrDestination == NULL || tstrSource == NULL) return;
...
VOID DXUtil_ConvertAnsiStringToGeneric(TCHAR *tstrDestination,
                                       const CHAR *strSource, int cchDestChar) {
  if (tstrDestination == NULL || strSource == NULL) return;
...
VOID DXUtil_ConvertWideStringToGeneric(TCHAR *tstrDestination,
                                       ...
                                       int cchDestChar) {
  if (tstrDestination == NULL || wstrSource == NULL) return;
