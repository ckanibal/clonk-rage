...
CStdWindow::CStdWindow() : Active(false) {}

CStdWindow::~CStdWindow() { Clear(); }

// Only set title.
// FIXME: Read from application bundle on the Mac.

CStdWindow *CStdWindow::Init(CStdApp *pApp) { return Init(pApp, STD_PRODUCT); }
...
bool CStdWindow::StorePosition(const char *, const char *, bool) {
  return true;
}
...
void CStdWindow::SetTitle(const char *Title) { SDL_WM_SetCaption(Title, 0); }
