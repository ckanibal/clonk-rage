...
BOOL CStdD3D::FindDisplayMode(unsigned int iXRes, unsigned int iYRes, unsigned int iColorDepth, unsigned int iMonitor)
	...
	D3DFORMAT format = iColorDepth == 32 ? D3DFMT_X8R8G8B8 : D3DFMT_R5G6B5;
	// enumerate modes
	...
	if (DDrawCfg.ClipManuallyE)
		...
		if (x1 < ClipX1)
			if (x2 < ClipX1)
				...
				{ y1+=(y2-y1)*((float)ClipX1-x1)/(x2-x1); x1=(float)ClipX1; } // clip left
		else if (x2 > ClipX2)
			if (x1 > ClipX2)
				return; // right out
			else
				{ y2-=(y2-y1)*(x2-ClipX2)/(x2-x1); x2=(float)ClipX2; } // clip right
		// sort top/bottom
		...
		if (y1 < ClipY1)
			if (y2 < ClipY1)
				...
				{ x1+=(x2-x1)*(ClipY1-y1)/(y2-y1); y1=(float)ClipY1; } // clip top
		else if (y2 > ClipY2)
			if (y1 > ClipY2)
				return; // bottom out
			else
				{ x2-=(x2-x1)*(y2-ClipY2)/(y2-y1); y2=(float)ClipY2; } // clip bottom
		}
	// apply color modulation
	if (BlitModulated)
...
IDirect3DStateBlock9* CStdD3D::CreateStateBlock(bool fTransparent, bool fSolid, bool fBaseTex, bool fAdditive, bool fMod2)
	...
	// capture
	IDirect3DStateBlock9 *stateBlock; lpDevice->EndStateBlock(&stateBlock);
