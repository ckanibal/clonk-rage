...
int Angle(int iX1, int iY1, int iX2, int iY2) {
  ...
  int iAngle =
      (int)(180.0 * atan2(float(Abs(iY1 - iY2)), float(Abs(iX1 - iX2))) / pi);
  if (iX2 > iX1) {
    if (iY2 < iY1)
      iAngle = 90 - iAngle;
    else
...
bool SCopySegment(const char *szString, int iSegment, char *sTarget,
  ...
  // Advance whitespace
  if (fSkipWhitespace) szString = SAdvanceSpace(szString);
...
const char *SSearchIdentifier(const char *szString, const char *szIndex) {
  ...
  for (cscr = szString; cscr && *cscr; cscr++) {
    ...
    if (match >= indexlen)
      if (frontok)
        if (!IsIdentifier(*(cscr + 1))) return cscr + 1;
...
void SInsert(char *szString, const char *szInsert, int iPosition, int iMaxLen) {
  ...
  memmove(szString + iPosition + insertlen, szString + iPosition,
...
bool SIsModule(const char *szList, const char *szString, int *ipIndex,
  ...
  for (int iMod = 0; SGetModule(szList, iMod, szModule, 1024); iMod++)
    if (fCaseSensitive ? SEqual(szString, szModule)
                       : SEqualNoCase(szString, szModule)) {
...
const char *SGetParameter(const char *strCommandLine, int iParameter,
                          char *strTarget, int iSize, bool *pWasQuoted) {
  // Safeties
  ...
  while (c && *c) {
    ...
    // Spaced parameter
      ...
      bool fWrongQuote =
          (SCharPos('"', c) > -1) && (SCharPos('"', c) < SCharPos(' ', c));
...
void BufferBlitAspectDw(uint32_t *bypSource, int iSourcePitch,
  ...
  BufferBlitDw(bypSource, iSourcePitch, iSrcBufHgt, iSrcX, iSrcY, iSrcWdt,
               iSrcHgt, bypTarget, iTargetPitch, iTrgBufHgt, iTrgX, iTrgY,
               iTrgWdt, iTrgHgt);
}
...
#if defined(HAVE_PTHREAD) && defined(C4ENGINE) && \
    defined(NEED_FALLBACK_ATOMIC_FUNCS)
